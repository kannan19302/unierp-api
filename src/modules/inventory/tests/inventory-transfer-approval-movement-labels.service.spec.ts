...
describe("InventoryService — transfer approval workflow, movement history, barcode labels", () => {
  ...
  describe("transfer approval workflow", () => {
    it("auto-submits a transfer below the configured threshold", async () => {
      db.stockEntry.findFirst.mockResolvedValue({
        id: "se1",
        totalValue: { toString: () => "500" },
        fromWarehouseId: "w1",
        toWarehouseId: null,
        status: "DRAFT",
      });
      db.transferApprovalRule.findMany.mockResolvedValue([
        {
          warehouseId: "w1",
          thresholdValue: { toString: () => "1000" },
          isActive: true,
        },
      ]);
      const submitSpy = vi
        .spyOn(service, "submitStockEntry")
        .mockResolvedValue({ id: "se1", status: "SUBMITTED" } as any);
    ...
    it("creates a PENDING approval and does not submit when above threshold", async () => {
      ...
      db.stockTransferApproval.create.mockResolvedValue({
        id: "appr1",
        status: "PENDING",
      });
    ...
    it("prefers a warehouse-specific rule over the tenant-wide rule", async () => {
      ...
      db.transferApprovalRule.findMany.mockResolvedValue([
        {
          warehouseId: null,
          thresholdValue: { toString: () => "500" },
          isActive: true,
        },
        {
          warehouseId: "w1",
          thresholdValue: { toString: () => "10000" },
          isActive: true,
        },
      ]);
      const submitSpy = vi
        .spyOn(service, "submitStockEntry")
        .mockResolvedValue({ id: "se3", status: "SUBMITTED" } as any);
    ...
    it("approves a pending transfer and submits the underlying stock entry", async () => {
      db.stockTransferApproval.findFirst.mockResolvedValue({
        id: "appr1",
        stockEntryId: "se2",
        status: "PENDING",
      });
      db.stockTransferApproval.update.mockResolvedValue({
        id: "appr1",
        status: "APPROVED",
      });
      const submitSpy = vi
        .spyOn(service, "submitStockEntry")
        .mockResolvedValue({ id: "se2", status: "SUBMITTED" } as any);
    ...
    it("rejects a pending transfer without submitting", async () => {
      ...
      const result = await service.rejectTransfer("t1", "appr1", "u1", {
    ...
    it("throws NotFound approving an already-resolved approval", async () => {
      db.stockTransferApproval.findFirst.mockResolvedValue(null);
      await expect(
        service.approveTransfer("t1", "missing", "u1"),
  ...
  describe("movement history", () => {
    it("maps ledger entries into a consolidated movement timeline", async () => {
      ...
      const result = await service.getMovementHistory("t1", {
        productId: "p1",
      });
  ...
  describe("barcode labels", () => {
    it("throws NotFound for an unknown product label", async () => {
      ...
      await expect(service.getProductLabel("t1", "missing")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("returns license plate label data", async () => {
      db.licensePlate.findFirst.mockResolvedValue({
        code: "LP-1",
        warehouseId: "w1",
        status: "OPEN",
      });
    ...
    it("returns bin label data", async () => {
      db.binLocation.findFirst.mockResolvedValue({
        code: "A-01-03",
        zone: "A",
        aisle: "01",
        rack: "03",
