...
export class CommunicationEnterpriseController {
  ...
  @ApiOperation({
    summary: "Unified inbox for messages, mentions, tasks, notifications",
  })
  @Get("unified-inbox")
  @Permissions("communication.message.read")
  async getUnifiedInbox(@Req() req: AuthenticatedRequest) {
    return this.service.getUnifiedInbox(req.user.tenantId, req.user.userId, {
      limit: 50,
      offset: 0,
    });
  }
  ...
  @ApiOperation({
    summary: "Message volume, response times, busiest channels, active users",
  })
  ...
  async getMessageAnalytics(
    ...
    return this.service.getMessageAnalytics(
      req.user.tenantId,
      dateRange || "MONTHLY",
    );
  }
  ...
  @ApiOperation({
    summary: "File uploads by type, size, storage usage, most shared files",
  })
  @Get("file-analytics")
  ...
  async getFileSharingAnalytics(
    ...
    return this.service.getFileSharingAnalytics(
      req.user.tenantId,
      dateRange || "MONTHLY",
    );
  }
  ...
  @ApiOperation({
    summary: "Cross-team collaboration, network analysis, silo detection",
  })
  ...
  async getCollaborationInsights(
    ...
    return this.service.getCollaborationInsights(
      req.user.tenantId,
      dateRange || "MONTHLY",
    );
  }
  ...
  @ApiOperation({
    summary: "Meeting frequency, duration, attendance, decision tracking",
  })
  ...
  async getMeetingEffectiveness(
    ...
    return this.service.getMeetingEffectiveness(
      req.user.tenantId,
      dateRange || "MONTHLY",
    );
  }
  ...
  @ApiOperation({
    summary: "Unified search across messages, files, channels, meetings",
  })
  @Get("search")
  @Permissions("communication.message.read")
  ...
  async getBotAnalytics(
    ...
    return this.service.getBotAnalytics(
      req.user.tenantId,
      dateRange || "MONTHLY",
    );
  }
  ...
  @ApiOperation({
    summary: "Message retention, e-discovery, compliance rule violations",
  })
  @Get("compliance")
  @Permissions("communication.settings.read")
  async getComplianceMonitoring(
    ...
    return this.service.getComplianceMonitoring(
      req.user.tenantId,
      dateRange || "MONTHLY",
    );
  }
