...
describe("AdvancedFinanceService — consolidation", () => {
  ...
  describe("getConsolidation", () => {
    it("aggregates revenue/expenses/assets per entity and nets out eliminations from the consolidated totals", async () => {
      ...
      (prisma.account.findMany as ReturnType<typeof vi.fn>).mockImplementation(
        ({ where }: { where: { orgId: string; type?: string } }) => {
          const all = [
            account({
              id: `${where.orgId}-rev`,
              orgId: where.orgId,
          ...
          return Promise.resolve(
            where.type ? all.filter((a) => a.type === where.type) : all,
          );
        },
      );
        ...
        ({ where }: { where: { accountId: { in: string[] } } }) => {
          return Promise.resolve(
            where.accountId.in.map((accountId: string) => {
              if (accountId.endsWith("-rev"))
                return { accountId, credit: 1000, debit: 0 };
              if (accountId.endsWith("-exp"))
                return { accountId, credit: 0, debit: 400 };
      ...
      (
        prisma.interCompanyTransfer.findMany as ReturnType<typeof vi.fn>
      ).mockResolvedValue([
        {
          id: "xfer-1",
          fromOrgId: "org-1",
  ...
  describe("runConsolidation", () => {
    it("persists real revenue and expense totals, not the hardcoded zero from the original bug", async () => {
      ...
      (
        prisma.consolidationRun.create as ReturnType<typeof vi.fn>
  ...
  describe("getConsolidationRuns", () => {
    it("scopes run history to the tenant", async () => {
      (
        prisma.consolidationRun.findMany as ReturnType<typeof vi.fn>
      ).mockResolvedValue([]);
