...
describe("CrmPipelineStagesService", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    (
      prisma.salesPipeline.findFirst as ReturnType<typeof vi.fn>
  ...
  it("creates a stage under a pipeline", async () => {
    (
      prisma.pipelineStage.create as ReturnType<typeof vi.fn>
    ).mockImplementation(({ data }) => Promise.resolve({ id: "s1", ...data }));
    const stage = await service.createStage(TENANT, "p1", {
      name: "Qualification",
      order: 0,
      probability: 20,
    });
  ...
  it("reorders stages by updating each order in a transaction", async () => {
    ...
    expect(
      (prisma.pipelineStage.update as ReturnType<typeof vi.fn>).mock.calls
        .length,
    ).toBe(3);
  });
});
