...
export type CreateInboundShipmentInput = z.infer<
...
export type CreateOutboundShipmentInput = z.infer<
...
export class InventoryLogisticsService {
  ...
  async updateCarrier(
    tenantId: string,
    id: string,
    dto: Partial<CreateCarrierInput>,
  ) {
  ...
  async addServiceLevel(tenantId: string, dto: CreateServiceLevelInput) {
    ...
    if (existing)
      throw new BadRequestException(
        `Service level '${dto.code}' already exists for this carrier`,
      );
  ...
  async listAsns(
    tenantId: string,
    filters: { vendorId?: string; warehouseId?: string; status?: string },
  ) {
  ...
  async markAsnInTransit(
    ...
    if (asn.status !== "PENDING")
      throw new BadRequestException(
        `ASN is ${asn.status}, cannot mark in-transit`,
      );
    return prisma.advanceShippingNotice.update({
      where: { id },
      data: {
        status: "IN_TRANSIT",
        trackingNumber: trackingNumber ?? asn.trackingNumber,
      },
    });
  }
  ...
  async markAsnArrived(tenantId: string, id: string) {
    ...
    if (!["PENDING", "IN_TRANSIT"].includes(asn.status)) {
      throw new BadRequestException(
        `ASN is ${asn.status}, cannot mark arrived`,
      );
    }
    return prisma.advanceShippingNotice.update({
      where: { id },
  ...
  async receiveAsn(tenantId: string, id: string, dto: ReceiveAsnInput) {
    ...
    if (asn.status === "RECEIVED")
      throw new BadRequestException("ASN already received");
    if (asn.status === "CANCELLED")
      throw new BadRequestException("ASN is cancelled");
      ...
      for (const item of dto.lineItems) {
        ...
        if (!lineItem)
          throw new NotFoundException(
            `Line item ${item.lineItemId} not found in ASN`,
          );
        await tx.aSNLineItem.update({
          where: { id: item.lineItemId },
          data: {
            receivedQty: new Prisma.Decimal(item.receivedQty.toFixed(4)),
          },
        });
      }
  ...
  async cancelAsn(tenantId: string, id: string) {
    ...
    if (!asn) throw new NotFoundException("ASN not found");
    if (asn.status === "RECEIVED")
      throw new BadRequestException("Cannot cancel a received ASN");
    return prisma.advanceShippingNotice.update({
      where: { id },
  ...
  async createInboundShipment(
    tenantId: string,
    dto: CreateInboundShipmentInput,
  ) {
      ...
      data: {
        ...
        totalWeight:
          dto.totalWeight != null
            ? new Prisma.Decimal(dto.totalWeight.toFixed(3))
            : null,
        notes: dto.notes ?? null,
      },
  ...
  async updateInboundShipmentStatus(
    tenantId: string,
    id: string,
    status: string,
    details?: { arrivedAt?: string; completedAt?: string },
  ) {
    ...
    return prisma.inboundShipment.update({
      ...
      data: {
        status,
        arrivedAt:
          status === "ARRIVED" && details?.arrivedAt
            ? new Date(details.arrivedAt)
            : status === "ARRIVED"
              ? new Date()
              : undefined,
        completedAt:
          status === "COMPLETE" && details?.completedAt
  ...
  async addInboundTrackingEvent(
    tenantId: string,
    shipmentId: string,
    dto: AddTrackingEventInput,
  ) {
    const shipment = await prisma.inboundShipment.findFirst({
  ...
  async listOutboundShipments(
    tenantId: string,
    filters: { salesOrderId?: string; warehouseId?: string; status?: string },
  ) {
  ...
  async createOutboundShipment(
    tenantId: string,
    dto: CreateOutboundShipmentInput,
  ) {
  ...
  async shipOutbound(tenantId: string, id: string, trackingNumber?: string) {
    ...
    if (!["PENDING", "PACKED"].includes(shipment.status)) {
      throw new BadRequestException(
        `Cannot ship from status ${shipment.status}`,
      );
    }
  ...
  async recordDelivery(
    ...
    if (!["SHIPPED", "IN_TRANSIT"].includes(shipment.status)) {
      throw new BadRequestException(
        `Cannot record delivery from status ${shipment.status}`,
      );
    }
    return prisma.outboundShipment.update({
      where: { id },
  ...
  async addOutboundTrackingEvent(
    tenantId: string,
    shipmentId: string,
    dto: AddTrackingEventInput,
  ) {
  ...
  async getLogisticsDashboard(tenantId: string) {
    const [
      ...
      prisma.advanceShippingNotice.count({
      ...
      prisma.inboundShipment.count({
        where: {
          tenantId,
          status: { in: ["EXPECTED", "IN_TRANSIT", "ARRIVED", "RECEIVING"] },
        },
      }),
    ...
    return {
      asns: {
        total: totalAsns,
        pending: pendingAsns,
        inTransit: inTransitAsns,
      },
      inbound: { total: totalInbound, open: openInbound },
      outbound: {
        total: totalOutbound,
        pending: pendingOutbound,
        inTransit: inTransitOutbound,
        exceptions,
      },
      carrierCount,
    };
