...
import {
  ...
  ConnectMailboxInput,
  OauthCallbackInput,
} from "../services/crm-mailbox.service";
...
export class CrmMailboxController {
  ...
  async connect(
    ...
    @ZodBody(connectMailboxSchema) dto: ConnectMailboxInput,
  ) {
    return this.svc.buildAuthorizationUrl(
      req.user.tenantId,
      req.user.userId,
      dto,
    );
  ...
  async callback(
    @Req() req: AuthenticatedRequest,
    @ZodBody(oauthCallbackSchema) dto: OauthCallbackInput,
  ) {
