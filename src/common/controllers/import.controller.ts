...
export class ImportController {
  ...
  async importCsv(
    ...
    @Body() body: { skipDuplicates?: string; updateExisting?: string; batchSize?: string },
    ...
    return this.importService.importCsv(tenantId, userId, modelName, file.buffer, {
  ...
  async importXlsx(
    ...
    const tenantId = req.tenantId || req.user?.tenantId;
    const userId = req.user?.userId;
    return this.importService.importXlsx(tenantId, userId, modelName, file.buffer, {
      skipDuplicates: body.skipDuplicates === 'true',
      updateExisting: body.updateExisting === 'true',
      batchSize: body.batchSize ? parseInt(body.batchSize, 10) : 100,
    });
  ...
  async importJson(
    @Param('modelName') modelName: string,
    @Body() body: { data: any[]; skipDuplicates?: boolean; updateExisting?: boolean; batchSize?: number },
    @Req() req: AuthenticatedRequest,
  ) {
    const tenantId = req.tenantId || req.user?.tenantId;
    const userId = req.user?.userId;
    return this.importService.importJson(tenantId, userId, modelName, body.data, {
