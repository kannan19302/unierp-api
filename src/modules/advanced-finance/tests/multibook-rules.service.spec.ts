...
describe("MultiBook Rules Engine", () => {
  ...
  describe("getAccountingBookRules", () => {
    it("should return accounting book rules", async () => {
      ...
      vi.mocked(prisma.accountingBookRule.findMany).mockResolvedValue(
        mockRules as any,
      );
  ...
  describe("createAccountingBookRule", () => {
    it("should create a new accounting book rule", async () => {
      ...
      vi.mocked(prisma.accountingBookRule.create).mockResolvedValue(
        mockRule as any,
      );

      const result = await service.createAccountingBookRule(
        "tenant-1",
        "org-1",
        dto,
      );
  ...
  describe("deleteAccountingBookRule", () => {
    it("should throw NotFoundException if rule does not exist", async () => {
      ...
      await expect(
        service.deleteAccountingBookRule("tenant-1", "rule-1"),
      ).rejects.toThrow(NotFoundException);
    });

    it("should delete accounting book rule if it exists", async () => {
      const mockRule = { id: "rule-1", tenantId: "tenant-1" };
      vi.mocked(prisma.accountingBookRule.findFirst).mockResolvedValue(
        mockRule as any,
      );
      vi.mocked(prisma.accountingBookRule.delete).mockResolvedValue(
        mockRule as any,
      );

      const result = await service.deleteAccountingBookRule(
        "tenant-1",
        "rule-1",
      );
      expect(prisma.accountingBookRule.delete).toHaveBeenCalledWith({
        where: { id: "rule-1" },
      });
      expect(result).toEqual(mockRule);
    });
