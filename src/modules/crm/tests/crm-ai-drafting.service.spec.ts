...
describe("CrmAiDraftingService", () => {
  ...
  beforeEach(() => {
    ...
    (prisma.crmAiDraft.create as ReturnType<typeof vi.fn>).mockImplementation(
      ({ data }: { data: unknown }) =>
        Promise.resolve({
          id: "draft-1",
          status: "DRAFT",
  ...
  describe("generateOpportunityFollowup", () => {
    it("throws NotFoundException when the opportunity does not exist", async () => {
      ...
      await expect(
        service.generateOpportunityFollowup(
          TENANT,
          ORG,
          "opp-x",
          "PROFESSIONAL",
    ...
    it("generates a follow-up email referencing deal context and persists it", async () => {
      (
        ...
        name: "Acme Renewal",
        stage: "Negotiation",
        amount: { toString: () => "50000", valueOf: () => 50000 },
        expectedCloseDate: new Date(Date.now() + 5 * 86400_000),
        customer: { name: "Acme Corp" },
        lineItems: [{ description: "Enterprise Plan" }],
      });

      const draft = await service.generateOpportunityFollowup(
        TENANT,
        ORG,
        "opp-1",
        "PROFESSIONAL",
      ...
      expect(prisma.crmAiDraft.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            draftType: "FOLLOWUP_EMAIL",
  ...
  describe("generateQuoteCoverNote", () => {
    it("throws NotFoundException when the quotation does not exist", async () => {
      ...
      await expect(
        service.generateQuoteCoverNote(TENANT, ORG, "q-x", "FRIENDLY"),
    ...
    it("generates a cover note including quote number and total", async () => {
      ...
      const draft = await service.generateQuoteCoverNote(
        TENANT,
        ORG,
        "q-1",
        "FRIENDLY",
  ...
  describe("generateLeadOutreach", () => {
    it("generates an outreach email referencing the lead source", async () => {
      ...
      const draft = await service.generateLeadOutreach(
        TENANT,
        ORG,
        "lead-1",
        "CONCISE",
  ...
  describe("draft lifecycle", () => {
    it("marks a draft as used", async () => {
      (
        prisma.crmAiDraft.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "d1", status: "DRAFT" });
      (prisma.crmAiDraft.update as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "d1",
        status: "USED",
      });
    ...
    it("rejects marking an already-used draft as used again", async () => {
      (
        prisma.crmAiDraft.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "d1", status: "USED" });
      await expect(service.markUsed(TENANT, "d1")).rejects.toThrow(
        "already used",
      );
    });
    ...
    it("rejects editing a non-draft-status draft", async () => {
      ...
      await expect(
        service.updateDraftText(TENANT, "d1", "Subj", "body text"),
    ...
    it("rejects an empty-body edit", async () => {
      ...
      await expect(
        service.updateDraftText(TENANT, "d1", "Subj", "   "),
  ...
  describe("regenerate", () => {
    ...
    it("throws NotFoundException when the draft to regenerate does not exist", async () => {
      ...
      await expect(
        service.regenerate(TENANT, "missing", "PROFESSIONAL"),
  ...
  describe("listDrafts / getDraft", () => {
    it("lists drafts scoped to tenant and optional context", async () => {
      (
        prisma.crmAiDraft.findMany as ReturnType<typeof vi.fn>
      ).mockResolvedValue([{ id: "d1" }]);
      const result = await service.listDrafts(TENANT, "LEAD", "lead-1");
      expect(result).toHaveLength(1);
      expect(prisma.crmAiDraft.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it("throws NotFoundException for a missing draft", async () => {
      ...
      await expect(service.getDraft(TENANT, "missing")).rejects.toThrow(
        "Draft not found",
      );
    });
  });
