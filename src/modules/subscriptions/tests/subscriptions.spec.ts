...
describe("SubscriptionsService", () => {
  ...
  describe("findById", () => {
    ...
    it("throws NotFoundException if not found", async () => {
      ...
      await expect(service.findById(tenantId, "sub-invalid")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("update", () => {
    it("updates a subscription", async () => {
      ...
      const result = await service.update(tenantId, "sub-1", {
        name: "Enterprise",
      });
  ...
  describe("usage tracking", () => {
    it("records usage", async () => {
      ...
      vi.mocked(prisma.subscriptionUsage.create).mockResolvedValue({
        id: "use-1",
      } as any);
    ...
    it("gets usage logs", async () => {
      ...
      vi.mocked(prisma.subscriptionUsage.findMany).mockResolvedValue(
        mockUsage as any,
      );
    ...
    it("gets usage summary grouped by metric", async () => {
      ...
      vi.mocked(prisma.subscriptionUsage.groupBy).mockResolvedValue([
        {
          metricName: "seats",
          _sum: { quantity: 10, totalAmount: new Prisma.Decimal(100) },
        },
      ] as any);

      const result = await service.getUsageSummary(tenantId, "sub-1");
      expect(result).toEqual([
        { metricName: "seats", totalQuantity: 10, totalAmount: 100 },
      ]);
    });
  });
  ...
  describe("runBilling", () => {
    it("processes due subscriptions and generates invoices", async () => {
      ...
      vi.mocked(prisma.invoice.create).mockResolvedValue({
        id: "inv-1",
        invoiceNumber: "SUB-org-1-00001",
      } as any);
      vi.mocked(prisma.subscriptionInvoice.count).mockResolvedValue(0);
      vi.mocked(prisma.subscriptionInvoice.create).mockResolvedValue({
        id: "sub-inv-1",
      } as any);
