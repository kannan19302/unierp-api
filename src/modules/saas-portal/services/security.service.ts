...
export class SaasPortalSecurityService {
  ...
  async saveMfaSettings(
    tenantId: string,
    mfa: { enabled: boolean; mfaType: string; enforced: boolean },
  ) {
    return prisma.setting.upsert({
      where: { tenantId_key: { tenantId, key: "security.mfa-settings" } },
  ...
  async createIpRestriction(
    tenantId: string,
    data: { ipRange: string; description?: string; ruleType: string },
  ) {
  ...
  async impersonateUser(tenantId: string, targetUserId: string) {
    ...
    if (!user)
      throw new NotFoundException(
        "User to impersonate not found in this tenant",
      );
    ...
    const checks = [
      {
        ...
        score:
          mfaSettings.enabled && mfaSettings.enforced
            ? 25
            : mfaSettings.enabled
              ? 15
              : 0,
      ...
      },
      {
        ...
        score:
          (passwordPolicy.minLength >= 10 ? 10 : 5) +
          (passwordPolicy.requireUppercase ? 5 : 0) +
          (passwordPolicy.requireNumbers ? 5 : 0) +
          (passwordPolicy.requireSpecial ? 5 : 0),
        maxScore: 25,
      ...
      },
      {
        ...
        details:
          ipRestrictions.length > 0
            ...
            : "No IP restriction rules defined.",
      },
      {
   ...
   */
  private async resolveCallerPermissions(
    tenantId: string,
    userId: string,
  ): Promise<string[]> {
  ...
  async createApiKey(
    ...
    if (dto.permissions && dto.permissions.length > 0) {
      const callerPermissions = await this.resolveCallerPermissions(
        tenantId,
        userId,
      );
      const excess = dto.permissions.filter(
        (p) => !hasPermission(callerPermissions, p),
      );
      if (excess.length > 0) {
        throw new ForbiddenException(
          `Cannot grant permission(s) you do not hold: ${excess.join(", ")}`,
        );
      }
    }
