...
describe("SalesCpqExtensionService", () => {
  ...
  it("should get bundle by id", async () => {
    vi.mocked(prisma.productBundle.findFirst).mockResolvedValue({
      id: "b1",
      name: "Starter Kit",
      items: [],
    } as never);
  ...
  it("should create bundle", async () => {
    ...
    vi.mocked(prisma.productBundle.create).mockResolvedValue({
      id: "b1",
      name: "New Bundle",
      items: [],
    } as never);
  ...
  it("should create cross-sell rule", async () => {
    ...
    vi.mocked(prisma.crossSellRule.create).mockResolvedValue({
      id: "cs-1",
    } as never);
    const result = await service.createCrossSellRule("tenant-1", "org-1", {
      productId: "p1",
  ...
  it("should create upsell rule", async () => {
    ...
    vi.mocked(prisma.upsellRule.create).mockResolvedValue({
      id: "us-1",
    } as never);
    const result = await service.createUpsellRule("tenant-1", "org-1", {
      productId: "p1",
      upgradeProductId: "p3",
    });
    expect(result).toBeDefined();
  ...
  it("should get guided selling recommendations", async () => {
    ...
    vi.mocked(prisma.productBundle.findMany).mockResolvedValue([]);
    const result = await service.getGuidedSellingRecommendations(
      "tenant-1",
      "p1",
    );
  ...
  it("should validate configuration", async () => {
    ...
    const result = await service.validateConfiguration("tenant-1", "p1", {
      quantity: 5,
      selectedOptions: { SUPPORT_LEVEL: "premium" },
    });
