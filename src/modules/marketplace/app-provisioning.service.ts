...
export class AppProvisioningService {
  async provision(
    ...
    for (const s of manifest.schemas || []) {
      ...
      const schemaSlug = `${moduleSlug}_${s.slug}`
        .toLowerCase()
        .replace(/[^a-z0-9_-]/g, "_");
      const schema = await prisma.schemaRegistry.upsert({
        ...
        create: {
          ...
          name: s.name,
          slug: schemaSlug,
          description: s.description || `Installed from ${sourceLabel}`,
          fields: (s.fields || []) as any,
          settings: (s.settings || {}) as any,
          status: "ACTIVE",
        },
      ...
      if (Array.isArray(s.sampleData) && s.sampleData.length) {
        const existing = await prisma.customRecord.count({
          where: { tenantId, schemaId: schema.id },
        });
        if (existing === 0) {
          await prisma.customRecord.createMany({
            data: s.sampleData.map((data) => ({
              tenantId,
              schemaId: schema.id,
              data: data as any,
              createdBy: "system",
    ...
    for (const p of manifest.pages || []) {
      const backingSchemaId = p.schema
        ? schemaIdBySlug.get(p.schema) || null
        : null;
      const backingSchema = (manifest.schemas || []).find(
        (s) => s.slug === p.schema,
      );
      ...
      const page = await prisma.pageRegistry.upsert({
        where: {
          tenantId_module_slug: { tenantId, module: pageModule, slug: p.slug },
        },
        update: {
  ...
  async teardown(
    ...
    const schemaIds = Array.isArray(provisioned.schemaRegistryIds)
      ...
      : [];
    const ruleIds = Array.isArray(provisioned.automationRuleIds)
      ? provisioned.automationRuleIds
      : [];
    if (ruleIds.length)
