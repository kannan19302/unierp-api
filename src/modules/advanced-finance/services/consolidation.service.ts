...
export class ConsolidationService {
  ...
  async getConsolidation(tenantId: string) {
    ...
    const eliminationsTotal = eliminationTransfers.reduce(
  ...
  async runMultiCurrencyConsolidation(
    ...
    for (const org of orgs) {
      ...
      const rate =
        orgCurrency === reportingCurrency
          ? 1
          : ((await this.getRateAsOf(
              tenantId,
              orgCurrency,
              reportingCurrency,
              end,
    ...
    const eliminationTotal = transfers.reduce(
  ...
  private async getConsolidatedQuarterlyTrend(
    tenantId: string,
    orgIds: string[],
  ) {
