...
async function callWithRole(
  ...
  return guard.canActivate(buildContext({ userId: 'u1' }, handler, controllerClass));
}
...
describe('Admin RBAC regression sweep (US-P0-1a, real controller metadata)', () => {
  ...
  it('sanity check: each targeted handler carries exactly ONE fine-grained permission in real metadata (proves the decorator-stacking bug is fixed, not just registry-filled)', () => {
    ...
    for (const [controllerClass, handlerName, expectedCode] of targets) {
      const metadata = Reflect.getMetadata(PERMISSIONS_KEY, controllerClass.prototype[handlerName]);
      expect(metadata, `${controllerClass.name}.${handlerName} metadata`).toEqual([expectedCode]);
    }
  });

  describe('(a) a role with ONLY the matching fine-grained permission succeeds', () => {
    it('GET /admin/security/sessions succeeds with only admin.security.read', async () => {
      await expect(
        callWithRole(guard, SecurityController, 'getActiveSessions', ['admin.security.read']),
      ).resolves.toBe(true);
    });

    it('GET /admin/user-groups (getGroups) succeeds with only admin.user-group.read', async () => {
      await expect(
        callWithRole(guard, AdminController, 'getGroups', ['admin.user-group.read']),
      ).resolves.toBe(true);
    });

    it('POST /admin/automation-rules succeeds with only admin.automation.create', async () => {
      await expect(
        callWithRole(guard, AutomationRulesController, 'createRule', ['admin.automation.create']),
      ).resolves.toBe(true);
    });

    it('GET /super-admin/tenants succeeds with only system.tenant.read', async () => {
      await expect(
        callWithRole(guard, SuperAdminController, 'getTenants', ['system.tenant.read']),
      ).resolves.toBe(true);
    });

    it('GET /admin/operations/backups succeeds with only system.operations.backup', async () => {
      await expect(
        callWithRole(guard, OperationsController, 'getBackups', ['system.operations.backup']),
      ).resolves.toBe(true);
    });
  });
  ...
  describe('(b) a role with an unrelated fine-grained permission is rejected (403)', () => {
    it('admin.security.read does NOT grant GET /admin/user-groups', async () => {
      await expect(
        callWithRole(guard, AdminController, 'getGroups', ['admin.security.read']),
      ).rejects.toThrow(ForbiddenException);
    });

    it('admin.user-group.read does NOT grant POST /admin/automation-rules', async () => {
      await expect(
        callWithRole(guard, AutomationRulesController, 'createRule', ['admin.user-group.read']),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('admin.automation.read does NOT grant GET /super-admin/tenants (tenant-scoped fine-grained code must not cross into system-scoped surface)', async () => {
      await expect(
        callWithRole(guard, SuperAdminController, 'getTenants', ['admin.automation.read']),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('admin.operations.read does NOT grant GET /admin/operations/backups (tenant-scoped operations permission must not reach the platform-wide backup surface)', async () => {
      await expect(
        callWithRole(guard, OperationsController, 'getBackups', ['admin.operations.read']),
      ).rejects.toThrow(ForbiddenException);
    });

    it('system.tenant.read does NOT grant GET /admin/security/sessions', async () => {
      await expect(
        callWithRole(guard, SecurityController, 'getActiveSessions', ['system.tenant.read']),
      ).rejects.toThrow(ForbiddenException);
    });
  });

  describe('(c) a role with ONLY the legacy coarse admin.read is rejected everywhere (no more backdoor)', () => {
    it('admin.read does NOT grant GET /admin/security/sessions', async () => {
      await expect(
        callWithRole(guard, SecurityController, 'getActiveSessions', ['admin.read']),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('admin.read does NOT grant GET /admin/automation-rules (getRules)', async () => {
      await expect(
        callWithRole(guard, AutomationRulesController, 'getRules', ['admin.read']),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('the coarse admin.* wildcard (a role granted blanket admin module access) is the only way coarse grants still work — a bare admin.read string grants nothing on its own', async () => {
      ...
      await expect(
        callWithRole(guard, SecurityController, 'getActiveSessions', ['admin.*']),
      ).resolves.toBe(true);
    });
  });
  ...
  describe('cross-check: GET /admin/operations/jobs (tenant-scoped admin.operations.read) still works normally and is distinct from the backup surface', () => {
    ...
    it('system.operations.backup does NOT grant GET /admin/operations/jobs (permission boundary is not accidentally bidirectional)', async () => {
      await expect(
        callWithRole(guard, OperationsController, 'getBackgroundJobs', ['system.operations.backup']),
      ).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('Backup permission boundary (P1-1 RBAC boundary): a realistic full Tenant Admin persona is rejected', () => {
    ...
    const realisticTenantAdminPermissions = [
      ...
      'admin.role.read', 'admin.setting.read', 'admin.setting.update',
      'admin.security.read', 'admin.security.update',
      'admin.automation.read', 'admin.automation.create', 'admin.automation.update', 'admin.automation.delete',
      'admin.operations.read', 'admin.operations.update',
      'admin.user-group.read', 'admin.user-group.create', 'admin.user-group.update', 'admin.user-group.delete',
      'admin.platform.read', 'admin.platform.update',
    ];
    ...
    it('POST /admin/operations/backups/create is rejected for a fully-provisioned Tenant Admin lacking system.operations.backup', async () => {
      await expect(
        callWithRole(guard, OperationsController, 'createBackup', realisticTenantAdminPermissions),
      ).rejects.toThrow(ForbiddenException);
    });

    it('GET /admin/operations/backups (list) is rejected for the same Tenant Admin persona', async () => {
      await expect(
        callWithRole(guard, OperationsController, 'getBackups', realisticTenantAdminPermissions),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('the same Tenant Admin persona legitimately succeeds on tenant-scoped admin.operations.* endpoints (proves the rejection above is a real boundary, not a broken guard)', async () => {
      await expect(
        callWithRole(guard, OperationsController, 'getBackgroundJobs', realisticTenantAdminPermissions),
      ).resolves.toBe(true);
      await expect(
        callWithRole(guard, OperationsController, 'retryJobs', realisticTenantAdminPermissions),
      ).resolves.toBe(true);
    });

    it('only a role literally granted system.operations.backup (or a "*"/"system.*" wildcard) can create or list backups', async () => {
      await expect(
        callWithRole(guard, OperationsController, 'createBackup', ['system.operations.backup']),
      ).resolves.toBe(true);
      await expect(
