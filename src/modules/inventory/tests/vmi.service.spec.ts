...
describe("VmiService", () => {
  ...
  it("createAgreement auto-numbers VMI-XXXXXX", async () => {
    ...
    expect(prisma.vmiAgreement.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ agreementNumber: "VMI-000003" }),
      }),
    );
  ...
  it("activateAgreement transitions DRAFT → ACTIVE", async () => {
    ...
    vi.mocked(prisma.vmiAgreement.update).mockResolvedValue({
      id: "ag1",
      status: "ACTIVE",
    } as any);
  ...
  it("activateAgreement throws for non-DRAFT", async () => {
    ...
    await expect(svc.activateAgreement(T, "ag1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("terminateAgreement throws when already terminated", async () => {
    ...
    await expect(svc.terminateAgreement(T, "ag1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("recordSnapshot creates snapshot and auto-triggers order when below min", async () => {
    ...
    vi.mocked(prisma.vmiStockSnapshot.create).mockResolvedValue({
      id: "ss1",
    } as any);
    vi.mocked(prisma.vmiOrder.count).mockResolvedValue(0);
    vi.mocked(prisma.vmiOrder.create).mockResolvedValue({
      id: "ord1",
      orderNumber: "VMIO-000001",
    } as any);

    await svc.recordSnapshot(T, "u1", {
      agreementId: "ag1",
  ...
  it("createOrder auto-numbers VMIO-XXXXXX", async () => {
    ...
    vi.mocked(prisma.vmiOrder.create).mockResolvedValue({
      id: "ord1",
      orderNumber: "VMIO-000005",
    } as any);
    const result = await svc.createOrder(T, "u1", {
      agreementId: "ag1",
      triggeredBy: "MANUAL",
      orderedQty: 50,
    });
    expect(prisma.vmiOrder.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ orderNumber: "VMIO-000005" }),
      }),
    );
  ...
  it("advanceOrderStatus PENDING → CONFIRMED", async () => {
    ...
    vi.mocked(prisma.vmiOrder.update).mockResolvedValue({
      id: "ord1",
      status: "CONFIRMED",
    } as any);
    const result = await svc.advanceOrderStatus(T, "ord1", {
      status: "CONFIRMED",
    });
  ...
  it("advanceOrderStatus throws for invalid transition", async () => {
    ...
    await expect(
      svc.advanceOrderStatus(T, "ord1", { status: "RECEIVED" }),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("getDashboard returns aggregate stats", async () => {
    ...
    vi.mocked(prisma.vmiOrder.count)
