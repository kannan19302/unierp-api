...
describe("CrmSlaService", () => {
  ...
  it("creates an SLA policy", async () => {
    (prisma.slaPolicy.create as ReturnType<typeof vi.fn>).mockImplementation(
      ({ data }) => Promise.resolve({ id: "p1", ...data }),
    );
  ...
  it("applyToCase stamps first-response and resolve-by deadlines from the matching policy", async () => {
    ...
    expect(result?.slaFirstResponseAt.getTime()).toBe(
      createdAt.getTime() + 60 * 60_000,
    );
    expect(result?.slaResolveBy.getTime()).toBe(
      createdAt.getTime() + 480 * 60_000,
    );
  });
  ...
  it("detectBreaches creates SlaBreach rows for missed deadlines and marks the case", async () => {
    ...
    (prisma.slaBreach.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(
      null,
    );
