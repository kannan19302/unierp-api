...
export class OrganizationEntitlementsService {
  ...
  async getSummary(tenantId: string) {
    ...
    const allocations = this.allocations[tenantId] || [];
    const totalSeats = 150;
    const allocatedSeats = allocations.filter((a) => a.status === "ACTIVE").length;
    ...
    return {
      ...
      reclaimableSeats: allocations.filter((a) => a.status === "INACTIVE").length,
      entitlements: items,
    };
  }
