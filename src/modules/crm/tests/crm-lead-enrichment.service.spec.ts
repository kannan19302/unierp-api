...
describe("CrmLeadEnrichmentService", () => {
  ...
  describe("listSources", () => {
    it("returns sources for tenant", async () => {
      vi.mocked(prisma.crmEnrichmentSource.findMany).mockResolvedValue([
        { id: "s1", name: "Clearbit", provider: "CLEARBIT", tenantId: TENANT },
      ] as any);
      const result = await service.listSources(TENANT);
      expect(result).toHaveLength(1);
      expect(prisma.crmEnrichmentSource.findMany).toHaveBeenCalledWith(
  ...
  describe("getSource", () => {
    it("throws if not found", async () => {
      ...
      await expect(service.getSource(TENANT, "bad")).rejects.toThrow(
        NotFoundException,
      );
    });
    it("returns source if found", async () => {
      vi.mocked(prisma.crmEnrichmentSource.findFirst).mockResolvedValue({
        id: "s1",
      } as any);
  ...
  describe("createSource", () => {
    it("creates and returns source", async () => {
      ...
      vi.mocked(prisma.crmEnrichmentSource.create).mockResolvedValue({
        id: "new",
        ...dto,
        tenantId: TENANT,
      } as any);
  ...
  describe("testSource", () => {
    it("updates lastTestedAt and result", async () => {
      ...
      expect(prisma.crmEnrichmentSource.update).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: "s1" },
          data: expect.objectContaining({ lastTestResult: "SUCCESS" }),
        }),
      );
    });
  });
  ...
  describe("rules", () => {
    it("listRules returns rules with source include", async () => {
      vi.mocked(prisma.crmEnrichmentRule.findMany).mockResolvedValue([
        { id: "r1", name: "Rule 1" },
      ] as any);
    ...
    it("createRule validates source exists", async () => {
      ...
      await expect(service.createRule(TENANT, dto as any)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("fieldMappings", () => {
    it("listFieldMappings filters by source", async () => {
      ...
      expect(prisma.crmEnrichmentFieldMapping.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { tenantId: TENANT, sourceId: "s1" },
        }),
      );
    });
  });
  ...
  describe("enrichLead", () => {
    it("throws if lead not found", async () => {
      ...
      await expect(service.enrichLead(TENANT, "bad")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("enriches lead with available sources", async () => {
      ...
      vi.mocked(prisma.crmEnrichmentSource.findMany).mockResolvedValue([
        { id: "s1", name: "Clearbit", enabled: true },
      ] as any);
      vi.mocked(prisma.crmLeadEnrichmentData.create).mockResolvedValue({
        id: "e1",
      } as any);
  ...
  describe("schedules", () => {
    it("createSchedule validates rule", async () => {
      ...
      await expect(service.createSchedule(TENANT, dto as any)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("stats", () => {
    it("returns computed stats", async () => {
      ...
      vi.mocked(prisma.crmEnrichmentLog.count)
