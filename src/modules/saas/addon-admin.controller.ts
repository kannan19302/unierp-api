...
export class AddonAdminController {
  ...
  async listAddons(@Req() _req: AuthReq) {
    return this.saasService.db.saaSAddOn.findMany({ orderBy: { createdAt: "desc" } }).catch(() => []);
  }
  ...
  async createAddon(@Req() _req: AuthReq, @ZodBody(createAddonSchema) body: z.infer<typeof createAddonSchema>) {
    return this.saasService.db.saaSAddOn.create({
      data: body as any,
    }).catch(() => ({ success: false }));
  }
  ...
  async getAddon(@Req() _req: AuthReq, @Param("id") id: string) {
    return this.saasService.db.saaSAddOn.findUnique({ where: { id } }).catch(() => null);
  }
  ...
  async updateAddon(@Req() _req: AuthReq, @Param("id") id: string, @ZodBody(updateAddonSchema) body: z.infer<typeof updateAddonSchema>) {
    return this.saasService.db.saaSAddOn.update({ where: { id }, data: body as any }).catch(() => ({ success: false }));
  }
  ...
  async deleteAddon(@Req() _req: AuthReq, @Param("id") id: string) {
    return this.saasService.db.saaSAddOn.delete({ where: { id } }).catch(() => ({ success: false }));
  }
  ...
  async listAddonCategories(@Req() _req: AuthReq) {
    const addons = await this.saasService.db.saaSAddOn.findMany({ select: { billingPeriod: true } }).catch(() => []);
  ...
  async featureAddon(@Req() _req: AuthReq, @Param("id") id: string) {
    return this.saasService.db.saaSAddOn.update({ where: { id }, data: { status: "ACTIVE" } }).catch(() => ({ success: false }));
  }
  ...
  async getAddonStats(@Req() _req: AuthReq) {
    const [total, activePurchases] = await Promise.all([
      ...
      this.saasService.db.tenantAddOn.count({ where: { status: "ACTIVE" } }).catch(() => 0),
    ]);
