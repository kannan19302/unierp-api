...
vi.mock("@unerp/database", () => {
  ...
  return {
    prisma: {
      ...
      organization: createMockPrismaCollection(),
    },
  };
});
...
describe("InterCompanyService", () => {
  ...
  describe("getTransactions", () => {
    it("should query intercompany transactions with skip and take limit", async () => {
      const mockTxs = [
        { id: "tx-1", status: "PENDING", fromOrgId: "org-a", toOrgId: "org-b" },
      ];
  ...
  describe("manualMatchTransactions", () => {
    it("should throw BadRequestException if matching orgs are same", async () => {
      ...
      vi.mocked(prisma.paymentSchedule.findFirst).mockResolvedValue({
        id: "sched-1",
        orgId: "org-A",
        amount: 3000,
      } as any);
      ...
      await expect(
        service.manualMatchTransactions("tenant-1", {
          fromInvoiceId: "inv-1",
          toInvoiceId: "sched-1",
        }),
    ...
    it("should create manual intercompany log if valid mismatching orgs are linked", async () => {
      ...
      await service.manualMatchTransactions("tenant-1", {
        fromInvoiceId: "inv-1",
        toInvoiceId: "sched-1",
      });
  ...
  describe("Elimination Rules CRUD", () => {
    it("should create an elimination rule", async () => {
      ...
      vi.mocked(prisma.eliminationRule.create).mockResolvedValue(
        mockRule as any,
      );
    ...
    it("should update an elimination rule", async () => {
      const mockRule = { id: "rule-1", name: "Test Rule" };
      vi.mocked(prisma.eliminationRule.findFirst).mockResolvedValue(
        mockRule as any,
      );
      vi.mocked(prisma.eliminationRule.update).mockResolvedValue({
        ...mockRule,
        name: "Updated",
      } as any);

      const result = await service.updateEliminationRule("tenant-1", "rule-1", {
        name: "Updated",
      });
  ...
  describe("executeEliminationRun", () => {
    ...
    it("should execute elimination run and generate draft journal entry", async () => {
      ...
      vi.mocked(prisma.eliminationRun.create).mockResolvedValue({
        id: "run-1",
        status: "DRAFT",
      } as any);

      const result = await service.executeEliminationRun(
        "tenant-1",
        "2026-01-01",
        "2026-01-31",
      );
  ...
  describe("postEliminationRun", () => {
    it("should post elimination run, mark transactions as ELIMINATED and post journal", async () => {
      ...
      vi.mocked(prisma.eliminationRun.findFirst).mockResolvedValue(
        mockRun as any,
      );
      vi.mocked(prisma.interCompanyTransaction.findFirst).mockResolvedValue(
        mockTx as any,
      ...
      vi.mocked(prisma.interCompanyTransaction.update).mockResolvedValue(
        {} as any,
      );
      vi.mocked(prisma.eliminationRun.update).mockResolvedValue({
        id: "run-1",
        status: "POSTED",
      } as any);
