...
export class CatchWeightRecallService {
  ...
  async listConfigs(tenantId: string) {
    return prisma.catchWeightConfig.findMany({
      where: { tenantId },
  ...
  async getConfig(tenantId: string, id: string) {
    const cfg = await prisma.catchWeightConfig.findFirst({
      where: { tenantId, id },
    });
  ...
  async upsertConfig(
    ...
    return prisma.catchWeightConfig.upsert({
      ...
      create: {
        ...
        pricingBasis,
        tareWeightKg: new Prisma.Decimal(tareWeightKg),
        uomNominal,
        uomWeight,
      },
      update: {
  ...
  async deactivateConfig(tenantId: string, id: string) {
    ...
    return prisma.catchWeightConfig.update({
      where: { id },
      data: { active: false },
    });
  }
  ...
  async captureReading(
    ...
    const cfg = await prisma.catchWeightConfig.findFirst({
      where: { tenantId, productId: data.productId, active: true },
    });
    if (!cfg)
      throw new NotFoundException(
        `No active catch-weight config for product ${data.productId}`,
      );
    ...
    const variancePct =
      nominalExpected > 0 ? (varianceKg / nominalExpected) * 100 : 0;

    let varianceStatus: string;
    if (variancePct > Number(cfg.tolerancePctPlus))
      varianceStatus = "OVER_TOLERANCE";
    else if (variancePct < -Number(cfg.tolerancePctMinus))
      varianceStatus = "UNDER_TOLERANCE";
  ...
  async listTares(tenantId: string) {
    return prisma.catchWeightTare.findMany({
      where: { tenantId },
      orderBy: { containerLabel: "asc" },
    });
  }
  ...
  async upsertTare(
    ...
    return prisma.catchWeightTare.upsert({
      ...
      create: {
        tenantId,
        ...data,
        tareWeightKg: new Prisma.Decimal(data.tareWeightKg),
      },
      update: {
        tareWeightKg: new Prisma.Decimal(data.tareWeightKg),
  ...
  async listRecalls(tenantId: string, status?: string) {
    return prisma.productRecall.findMany({
      ...
      include: {
        affectedStock: {
          select: { id: true, qtyAffected: true, qtyRecovered: true },
        },
  ...
  async createRecall(
    ...
    return prisma.productRecall.create({
      data: {
        ...
        affectedDateFrom: data.affectedDateFrom
          ? new Date(data.affectedDateFrom)
          : null,
        affectedDateTo: data.affectedDateTo
          ? new Date(data.affectedDateTo)
          : null,
  ...
  async quarantineStock(
    ...
    const stock = await prisma.recallAffectedStock.findFirst({
      where: { id: stockId, recallId },
    });
    if (!stock)
      throw new NotFoundException(`RecallAffectedStock ${stockId} not found`);
    return prisma.recallAffectedStock.update({
      where: { id: stockId },
      data: {
        qtyQuarantined: new Prisma.Decimal(qty),
        quarantinedAt: new Date(),
      },
    });
  }

  async addCustomerNotice(
    tenantId: string,
    recallId: string,
    data: {
      customerId: string;
  ...
  async acknowledgeCustomerNotice(
    _tenantId: string,
    recallId: string,
    noticeId: string,
    qtyReturned?: number,
  ) {
    const notice = await prisma.recallCustomerNotice.findFirst({
      where: { id: noticeId, recallId },
    });
    if (!notice)
      throw new NotFoundException(`RecallCustomerNotice ${noticeId} not found`);
    return prisma.recallCustomerNotice.update({
      where: { id: noticeId },
      data: {
        acknowledgedAt: new Date(),
        ...(qtyReturned !== undefined
  ...
  async addDisposalRecord(
    ...
    await prisma.productRecall.update({
      where: { id: recallId },
      data: {
        totalUnitsRecovered: { increment: Math.ceil(data.qtyProcessed) },
      },
    });
    return record;
  }
  ...
  async getRecallImpactReport(tenantId: string, id: string) {
    ...
    const totalAffected = recall.affectedStock.reduce(
      (s, r) => s + Number(r.qtyAffected),
      0,
    );
    const totalQuarantined = recall.affectedStock.reduce(
      (s, r) => s + Number((r as any).qtyQuarantined),
      0,
    );
    const totalRecovered = Number(recall.totalUnitsRecovered);
    const noticesTotal = recall.customerNotices.length;
    const noticesSent = recall.customerNotices.filter(
      (n: any) => n.noticeSentAt,
    ).length;
    const noticesAcknowledged = recall.customerNotices.filter(
    ...
    return {
      ...
      customerNotices: {
        total: noticesTotal,
        sent: noticesSent,
        acknowledged: noticesAcknowledged,
      },
    ...
    const [recallByStatus, configs, outOfTolerance] = await Promise.all([
      ...
      prisma.catchWeightReading.count({
        where: {
          tenantId,
          varianceStatus: {
            in: ["OVER_TOLERANCE", "UNDER_TOLERANCE"] as any[],
          },
        },
      }),
    ]);
    const recallMap = Object.fromEntries(
      recallByStatus.map((r) => [r.status, r._count]),
    );
    return {
  ...
  private async requireRecall(
    ...
    if (!allowedStatuses.includes(recall.status)) {
      throw new BadRequestException(
        `Recall is in status ${recall.status}; expected one of ${allowedStatuses.join(", ")}`,
      );
    }
    return recall;
  }
