...
export const updateSpiffSchema = createSpiffSchema
  .partial()
  .extend({ isActive: z.boolean().optional() });
...
export class CrmCommissionAutomationService {
  ...
  async listPlans(tenantId: string) {
    return prisma.commissionPlan.findMany({
      ...
      include: {
        ...
        _count: { select: { payouts: true, spiffs: true } },
      },
  ...
  async addTier(tenantId: string, planId: string, dto: CreateTierInput) {
    ...
    if (!plan) throw new NotFoundException("Commission plan not found");
    if (
      dto.maxAttainmentPct !== undefined &&
      dto.maxAttainmentPct <= dto.minAttainmentPct
    ) {
      throw new BadRequestException(
        "maxAttainmentPct must be greater than minAttainmentPct",
      );
    }
    return prisma.commissionPlanTier.create({
      data: {
        ...
        maxAttainmentPct:
          dto.maxAttainmentPct !== undefined
            ? new Prisma.Decimal(dto.maxAttainmentPct)
            : null,
  ...
  async removeTier(tenantId: string, tierId: string) {
    const existing = await prisma.commissionPlanTier.findFirst({
  ...
  async listSpiffs(tenantId: string) {
    return prisma.commissionSpiff.findMany({
      where: { tenantId },
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async updateSpiff(tenantId: string, id: string, dto: UpdateSpiffInput) {
    const existing = await prisma.commissionSpiff.findFirst({
      where: { id, tenantId },
    });
  ...
  async calculatePayouts(
    ...
    if (plan.tiers.length === 0)
      throw new BadRequestException("Plan has no attainment tiers configured");
    ...
    if (quotas.length === 0)
      throw new BadRequestException(
        `No Quota rows found for period "${dto.period}" — set rep quotas first`,
      );
    ...
    for (const quota of quotas) {
      const wonOpps = await prisma.opportunity.findMany({
        where: {
          ...
          deletedAt: null,
          stage: { in: CLOSED_WON },
          assignedToId: quota.userId,
          actualCloseDate: { gte: start, lt: end },
        },
      ...
      const attainmentPct =
        quotaAmount > 0 ? (attained / quotaAmount) * 100 : 0;

      const tier =
        plan.tiers.find((t) => {
          const min = Number(t.minAttainmentPct);
          const max =
            t.maxAttainmentPct !== null ? Number(t.maxAttainmentPct) : Infinity;
      ...
      for (const spiff of plan.spiffs) {
        ...
        if (spiff.criteriaType === "ATTAINMENT_ABOVE") {
          const threshold = Number(
            (spiff.criteriaValue as { pct?: number })?.pct ?? 0,
          );
          if (attainmentPct >= threshold) {
            const amount =
              spiff.bonusType === "FLAT"
                ? Number(spiff.bonusAmount)
                : attained * (Number(spiff.bonusAmount) / 100);
            spiffBonus += amount;
            spiffLines.push({
              spiffId: spiff.id,
              amount,
              reason: `Attainment ${attainmentPct.toFixed(1)}% >= ${threshold}%`,
            });
          }
        } else if (spiff.criteriaType === "DEAL_SIZE_ABOVE") {
          const threshold = Number(
            (spiff.criteriaValue as { amount?: number })?.amount ?? 0,
          );
          for (const opp of wonOpps) {
            if (Number(opp.amount || 0) >= threshold) {
              const amount =
                spiff.bonusType === "FLAT"
                  ? Number(spiff.bonusAmount)
                  : Number(opp.amount) * (Number(spiff.bonusAmount) / 100);
              spiffBonus += amount;
              spiffLines.push({
                spiffId: spiff.id,
                amount,
                reason: `Deal "${opp.name}" >= ${threshold}`,
              });
            }
          }
          ...
          for (const opp of wonOpps) {
            ...
            if (priorWins === 0) {
              const amount =
                spiff.bonusType === "FLAT"
                  ? Number(spiff.bonusAmount)
                  : Number(opp.amount || 0) * (Number(spiff.bonusAmount) / 100);
              spiffBonus += amount;
              spiffLines.push({
                spiffId: spiff.id,
                amount,
                reason: `New-logo deal "${opp.name}"`,
              });
            }
          }
        }
      ...
      const payout = await prisma.commissionPayout.upsert({
        ...
        create: {
          ...
          attainedAmount: new Prisma.Decimal(attained),
          attainmentPct: new Prisma.Decimal(attainmentPct),
          appliedTierRate: new Prisma.Decimal(appliedRate),
          tieredCommission: new Prisma.Decimal(tieredCommission),
          spiffBonus: new Prisma.Decimal(spiffBonus),
          totalPayout: new Prisma.Decimal(totalPayout),
          status: "DRAFT",
        },
        update: {
          quotaAmount: new Prisma.Decimal(quotaAmount),
      ...
      await prisma.commissionPayoutSpiffLine.deleteMany({
        where: { tenantId, payoutId: payout.id },
      });
      if (spiffLines.length > 0) {
        await prisma.commissionPayoutSpiffLine.createMany({
          data: spiffLines.map((l) => ({
            tenantId,
            payoutId: payout.id,
            spiffId: l.spiffId,
      ...
      results.push(payout);
      this.eventEmitter.emit("crm.commission.payout_calculated", {
        tenantId,
        userId: quota.userId,
        planId: plan.id,
        period: dto.period,
  ...
  async listPayouts(
    tenantId: string,
    filters?: { planId?: string; period?: string; userId?: string },
  ) {
