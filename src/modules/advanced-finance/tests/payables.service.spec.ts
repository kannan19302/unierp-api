...
describe("PayablesService", () => {
  ...
  describe("listMatchRules", () => {
    it("returns all active match rules for tenant", async () => {
      ...
      (
        prisma.aPMatchRule.findMany as ReturnType<typeof vi.fn>
  ...
  describe("getMatchRule", () => {
    it("returns a rule by id", async () => {
      ...
      (
        prisma.aPMatchRule.findFirst as ReturnType<typeof vi.fn>
    ...
    it("throws NotFoundException when rule not found", async () => {
      ...
      await expect(service.getMatchRule(TENANT, "missing")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("createMatchRule", () => {
    it("creates a match rule with valid tolerances", async () => {
      ...
      (prisma.aPMatchRule.create as ReturnType<typeof vi.fn>).mockResolvedValue(
        created,
      );
  ...
  describe("updateMatchRule", () => {
    it("updates a match rule", async () => {
      ...
      (prisma.aPMatchRule.update as ReturnType<typeof vi.fn>).mockResolvedValue(
        { ...rule, status: "INACTIVE" },
      );
      const result = await service.updateMatchRule(TENANT, "r1", USER_ID, {
        status: "INACTIVE",
      });
  ...
  describe("runMatch", () => {
    it("returns PENDING when no goods received", async () => {
      ...
      (
        prisma.purchaseOrder.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue(po);
      (
        prisma.aPMatchRule.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue(null);
      const result = await service.runMatch(TENANT, USER_ID, {
    ...
    it("returns EXCEPTION when out of tolerance and records it", async () => {
      ...
      (
        prisma.aPMatchException.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue(null);
      (
        prisma.aPMatchException.create as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "exc-1" });
      const result = await service.runMatch(TENANT, USER_ID, {
        purchaseOrderId: "po-1",
      });
    ...
    it("throws NotFoundException when PO not found", async () => {
      ...
      await expect(
        service.runMatch(TENANT, USER_ID, { purchaseOrderId: "missing" }),
  ...
  describe("listExceptions", () => {
    it("lists exceptions filtered by status", async () => {
      ...
      (
        prisma.aPMatchException.findMany as ReturnType<typeof vi.fn>
      ).mockResolvedValue(excs);
  ...
  describe("approveException", () => {
    it("approves a pending exception", async () => {
      (
        prisma.aPMatchException.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "exc-1", status: "PENDING" });
      (
        prisma.aPMatchException.update as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "exc-1", status: "APPROVED" });
      const result = await service.approveException(
        TENANT,
        "exc-1",
        USER_ID,
        "ok",
    ...
    it("throws BadRequestException if exception already approved", async () => {
      ...
      await expect(
        service.approveException(TENANT, "exc-1", USER_ID),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("rejectException", () => {
    it("rejects a pending exception", async () => {
      ...
      (
        prisma.aPMatchException.update as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "exc-1", status: "REJECTED" });
      const result = await service.rejectException(
        TENANT,
        "exc-1",
        USER_ID,
        "not ok",
  ...
  describe("createPaymentBatch", () => {
    it("creates a DRAFT payment batch", async () => {
      (prisma.paymentBatch.count as ReturnType<typeof vi.fn>).mockResolvedValue(
        0,
      );
      (
        prisma.paymentBatch.create as ReturnType<typeof vi.fn>
      ).mockResolvedValue({
        id: "b1",
        status: "DRAFT",
        batchNumber: "PB-00001",
      });
      const result = await service.createPaymentBatch(TENANT, USER_ID, {
        paymentMethod: "ACH",
      });
  ...
  describe("addLineToBatch", () => {
    it("adds a line to a DRAFT batch and recalculates total", async () => {
      (
        prisma.paymentBatch.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue({
        id: "b1",
        status: "DRAFT",
        lines: [],
      ...
      (
        prisma.paymentBatchLine.create as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "l1", amount: 100 });
      (
        prisma.paymentBatchLine.aggregate as ReturnType<typeof vi.fn>
      ).mockResolvedValue({
        _sum: { amount: 100 },
      });
      (
        prisma.paymentBatch.update as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "b1", totalAmount: 100 });
  ...
  describe("runPaymentBatch", () => {
    it("executes batch, settles lines, posts GL journal, marks COMPLETED", async () => {
      ...
      (prisma.journal.create as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "j1",
      });
      (
        prisma.paymentBatchLine.updateMany as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ count: 1 });
      (
        prisma.paymentBatch.update as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ ...batch, status: "COMPLETED" });

      const result = await service.runPaymentBatch(
        TENANT,
        "b1",
        USER_ID,
        ORG_ID,
    ...
    it("throws BadRequestException on empty batch", async () => {
      ...
      await expect(
        service.runPaymentBatch(TENANT, "b1", USER_ID, ORG_ID),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("reportDrilldown", () => {
    it("drills into P&L by account and period", async () => {
      ...
      (
        prisma.journalEntry.findMany as ReturnType<typeof vi.fn>
  ...
  describe("getPayablesStats", () => {
    it("returns aggregated payables dashboard stats", async () => {
      (
        prisma.purchaseOrder.count as ReturnType<typeof vi.fn>
      ).mockResolvedValue(12);
      (
        prisma.aPMatchException.count as ReturnType<typeof vi.fn>
      ).mockResolvedValue(3);
      (
        prisma.paymentBatch.groupBy as ReturnType<typeof vi.fn>
      ).mockResolvedValue([
        { status: "DRAFT", _count: 2, _sum: { totalAmount: 20000 } },
        { status: "COMPLETED", _count: 5, _sum: { totalAmount: 95000 } },
      ]);
      (prisma.aPMatchRule.count as ReturnType<typeof vi.fn>).mockResolvedValue(
        4,
      );
