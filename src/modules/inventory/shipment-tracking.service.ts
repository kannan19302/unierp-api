...
export class ShipmentTrackingService {
  ...
  async addTrackingEvent(
    ...
    if (isInbound) {
      const s = await prisma.inboundShipment.findFirst({
        where: { id: data.shipmentId, tenantId },
      });
      if (!s) throw new NotFoundException("Inbound shipment not found");
    } else {
      const s = await prisma.outboundShipment.findFirst({
        where: { id: data.shipmentId, tenantId },
      });
    ...
    return prisma.shipmentTrackingEvent.create({
      data: {
        ...
        ...(isInbound
          ...
          : { outboundShipmentId: data.shipmentId }),
  ...
  async updateExceptionStatus(
    ...
    const exc = await prisma.shipmentException.findFirst({
      where: { id: exceptionId, tenantId },
    });
    if (!exc) throw new NotFoundException("Exception not found");

    const resolvedAt =
      status === ShipmentExceptionStatus.RESOLVED ? new Date() : undefined;
  ...
  async getDashboard(tenantId: string) {
    const [inboundByStatus, outboundByStatus, openExceptions, recentTracking] =
      await Promise.all([
        ...
        prisma.outboundShipment.groupBy({
          by: ["status"],
          where: { tenantId },
          _count: { id: true },
        }),
        prisma.shipmentException.count({
          where: {
            tenantId,
            status: { not: ShipmentExceptionStatus.RESOLVED },
          },
        }),
