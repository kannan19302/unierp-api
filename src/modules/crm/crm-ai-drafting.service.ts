...
export class CrmAiDraftingService {
  async generateOpportunityFollowup(
    ...
    const urgencyLine =
      daysToClose !== null && daysToClose <= 14 && daysToClose >= 0
  ...
  async generateQuoteCoverNote(
    ...
    const quote = await prisma.quotation.findFirst({
      ...
      include: {
        customer: { select: { name: true } },
        lineItems: { select: { description: true }, take: 5 },
      },
    });
  ...
  async generateLeadOutreach(
    ...
    const sourceLine = lead.source?.name
      ? ` I noticed you came in through ${lead.source.name}`
      : "";
  ...
  async regenerate(
    ...
    if (existing.draftType === "QUOTE_COVER_NOTE") {
      return this.generateQuoteCoverNote(
        tenantId,
        existing.orgId,
        existing.contextId,
        tone,
    ...
    return this.generateLeadOutreach(
      tenantId,
      existing.orgId,
      existing.contextId,
      tone,
  ...
  async getDraft(tenantId: string, draftId: string) {
    const draft = await prisma.crmAiDraft.findFirst({
      where: { id: draftId, tenantId },
    });
  ...
  async markUsed(tenantId: string, draftId: string) {
    ...
    if (draft.status !== "DRAFT")
      throw new BadRequestException(
        `Draft is already ${draft.status.toLowerCase()}`,
      );
    return prisma.crmAiDraft.update({
      where: { id: draftId },
  ...
  async updateDraftText(
    ...
    if (draft.status !== "DRAFT")
      throw new BadRequestException(
        "Only drafts in DRAFT status can be edited",
      );
