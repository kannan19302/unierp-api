...
export class PayablesService {
  ...
  async createMatchRule(
    ...
    if (dto.quantityTolerancePercent < 0 || dto.priceTolerancePercent < 0) {
      throw new BadRequestException(
        "Tolerance percentages must be non-negative",
      );
    }
    return prisma.aPMatchRule.create({
      data: {
        ...
        quantityTolerancePercent: new Prisma.Decimal(
          dto.quantityTolerancePercent,
        ),
  ...
  async updateMatchRule(
    ...
    if (dto.quantityTolerancePercent !== undefined)
      data.quantityTolerancePercent = new Prisma.Decimal(
        dto.quantityTolerancePercent,
      );
    if (dto.priceTolerancePercent !== undefined)
      data.priceTolerancePercent = new Prisma.Decimal(
        dto.priceTolerancePercent,
      );
  ...
  async runMatch(
    ...
    const receivedQty = po.receipts.reduce(
      (sum, r) =>
        sum + r.lineItems.reduce((s, li) => s + Number(li.acceptedQty), 0),
      0,
    );
    const orderedQty = po.lineItems.reduce(
    ...
    const qtyVariancePct =
      orderedQty > 0
        ? Math.abs((receivedQty - orderedQty) / orderedQty) * 100
        : 0;
    // Estimate received amount as received_qty / ordered_qty * po_amount
    const receivedAmount =
      orderedQty > 0 ? (receivedQty / orderedQty) * poAmount : 0;
    const priceVariancePct =
      poAmount > 0 ? Math.abs((receivedAmount - poAmount) / poAmount) * 100 : 0;
    ...
    const isMatched =
      receivedQty > 0 && isWithinQtyTolerance && isWithinPriceTolerance;

    const status =
      receivedQty === 0 ? "PENDING" : isMatched ? "MATCHED" : "EXCEPTION";

    // If exception, record it
    if (status === "EXCEPTION") {
      const exceptionType = !isWithinQtyTolerance
        ? "QUANTITY_VARIANCE"
        : "PRICE_VARIANCE";
  ...
  async approveException(
    ...
    const exc = await prisma.aPMatchException.findFirst({
      where: { id, tenantId },
    });
  ...
  async addLineToBatch(
    ...
    if (!["DRAFT", "READY"].includes(batch.status)) {
      throw new BadRequestException(
        "Can only add lines to DRAFT or READY batches",
      );
    }
    ...
    await prisma.paymentBatch.update({
      where: { id: batchId },
      data: {
        totalAmount: totalResult._sum.amount ?? new Prisma.Decimal(0),
        updatedBy: userId,
      },
    });
    return line;
  ...
  async removeLineFromBatch(
    ...
    if (!["DRAFT", "READY"].includes(batch.status)) {
      throw new BadRequestException(
        "Cannot modify a submitted or completed batch",
      );
    }
    const line = await prisma.paymentBatchLine.findFirst({
  ...
  async runPaymentBatch(
      ...
      journalEntries.push(
        {
          ...
          description: `AP clearing — batch ${batch.batchNumber}`,
    ...
    return {
      ...
      linesSettled: batch.lines.length,
    };
  }
  ...
  async exportPaymentBatch(tenantId: string, batchId: string, format: string) {
    ...
    if (format === "NACHA") {
      ...
      const header =
        `101 ${fileDate}${batch.batchNumber.padEnd(23)}ACH Payment Batch`.slice(
          0,
          94,
        );
      const detail = lines
...
<Document>
  ...
  async reportDrilldown(
    ...
    if (reportType === "profit_loss")
      accountTypeFilter = ["REVENUE", "EXPENSE"];
    else if (reportType === "balance_sheet")
      accountTypeFilter = ["ASSET", "LIABILITY", "EQUITY"];

    const entries = await prisma.journalEntry.findMany({
      where,
      include: {
        journal: {
          select: { entryNumber: true, notes: true, status: true, date: true },
        },
    ...
    const filteredEntries = accountTypeFilter
      ? entries.filter(
          (e) => e.account && accountTypeFilter!.includes(e.account.type),
        )
      : entries;
    ...
    return {
      ...
      entries: filteredEntries.map((e) => ({
        ...
        account: e.account
          ? { code: e.account.code, name: e.account.name, type: e.account.type }
          : null,
        debit: e.debit,
        credit: e.credit,
  ...
  async getPayablesStats(tenantId: string) {
    const [openPOs, exceptionCount, batchStats, matchRuleCount] =
      await Promise.all([
        prisma.purchaseOrder.count({
          where: { tenantId, status: { notIn: ["CANCELLED", "RECEIVED"] } },
        }),
        prisma.aPMatchException.count({
    ...
    return {
      ...
      draftBatches: draftBatches
        ? { count: draftBatches._count, total: draftBatches._sum.totalAmount }
        : { count: 0, total: 0 },
      completedBatches: completedBatches
        ? {
            count: completedBatches._count,
            total: completedBatches._sum.totalAmount,
          }
        : { count: 0, total: 0 },
