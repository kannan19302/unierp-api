...
export class AiCopilotService {
  ...
  async askData(tenantId: string, question: string) {
    ...
    const schemaDescription = semanticLayer
      .map(
        (e) =>
          `- "${e.name}" (${e.label}): fields = ${e.fields.map((f) => `${f.name}:${f.type}${f.aggregatable ? "(aggregatable)" : ""}`).join(", ")}`,
      )
      .join("\n");
      ...
      [
        {
          role: "system",
          content:
            `You translate a business question into a structured report query against these entities:\n${schemaDescription}\n` +
    ...
    try {
      ...
      return {
        answer:
          "Could not understand that question well enough to build a report. Try rephrasing it.",
        query: null,
        data: [],
      };
    }
      ...
      [
        {
          role: "system",
          content:
            "You summarize ERP report data into a direct, concise natural-language answer (2-3 sentences). Base the answer ONLY on the JSON data provided — never invent figures.",
        },
        {
          role: "user",
    ...
    return {
      answer: narrationResult.content,
      query: plannedQuery,
      data: result.data ?? [],
    };
  }
  ...
  async draftEmail(
    tenantId: string,
    context: { to: string; regarding: string; tone?: string },
  ) {
    if (!this.ai.isConfigured()) {
    ...
    const result = await this.ai.chat(
      [
        {
          role: "system",
          content: `You are a professional business email writer. Write a ${context.tone || "professional"} email. Respond as JSON: {"subject": "...", "body": "..."}`,
        },
        {
          role: "user",
          content: `Write an email to ${context.to} regarding: ${context.regarding}`,
        },
      ],
      { tenantId },
    );
  ...
  async generateFormFromPrompt(tenantId: string, prompt: string) {
    ...
    const result = await this.ai.chat(
      [
        {
          role: "system",
          content: `You design data collection forms for an ERP Builder. Given a description, generate a form definition as JSON: {"name": "...", "description": "...", "fields": [{"name": "field_name", "label": "Field Label", "type": "text|number|email|date|select|textarea|checkbox", "required": true/false, "options": ["opt1","opt2"] (for select only)}]}`,
        },
  ...
  async generateWorkflowFromPrompt(tenantId: string, prompt: string) {
    ...
    const result = await this.ai.chat(
      [
        {
          role: "system",
          content: `You design approval workflows for an ERP system. Given a description, generate a workflow definition as JSON: {"name": "...", "triggerType": "PO_CREATED|INVOICE_CREATED|LEAVE_REQUESTED|CUSTOM", "steps": [{"actionType": "APPROVAL|NOTIFICATION", "assigneeRole": "Manager|Finance Manager|HR Manager|Admin", "slaLimitHours": number}]}`,
        },
        { role: "user", content: prompt },
      ],
  ...
  async processInvoiceDocument(
    ...
    const extracted = (await this.ai.extractFields(
      documentText,
      fields,
      tenantId,
    ...
    if (createDraft && extracted) {
      try {
        const vendorName =
          typeof extracted.vendorName === "string"
            ? extracted.vendorName
            : null;
        const totalAmount =
          typeof extracted.totalAmount === "number"
            ? extracted.totalAmount
            : typeof extracted.totalAmount === "string"
              ? parseFloat(extracted.totalAmount)
              : 0;
        const currency =
          typeof extracted.currency === "string" ? extracted.currency : "USD";
        const dueDate =
          typeof extracted.dueDate === "string"
    ...
    return {
      ...
      confidence: 0.85,
      ...(draftPo
        ? { draftPoId: draftPo.id, draftPoNumber: draftPo.poNumber }
        : {}),
    };
  }
}
