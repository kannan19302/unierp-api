  ...
  items: [
    {
      ...
      productListing: { productId: 'prod-stripe-1', displayName: 'Super Widget', product: { name: 'Super Widget' } },
    },
  ],
};
...
describe('Stripe Payment Gateway Integration & Webhooks', () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(prisma.organization).findFirst.mockResolvedValue({ id: 'org-stripe-1', tenantId: 't1' } as never);
    vi.mocked(prisma.customer).findFirst.mockResolvedValue(null);
    vi.mocked(prisma.customer).create.mockResolvedValue({ id: 'cust-stripe-1' } as never);
    vi.mocked(prisma.storefrontOrderPayment).findFirst.mockResolvedValue(null);
    vi.mocked(prisma.storefrontOrderPayment).create.mockResolvedValue({ id: 'pay-stripe-1' } as never);
    vi.mocked(prisma.cart).update.mockResolvedValue({ id: 'cart-stripe-1', status: 'CONVERTED' } as never);
  });
  ...
  describe('StripePaymentGatewayService API Requests', () => {
    it('creates PaymentIntent correctly mapping cents', async () => {
      ...
      const result = await stripeService.createIntent(49.99, 'USD', { cartId: 'c1' });
  ...
  describe('Webhook Cryptographic Signature Verification', () => {
    it('verifies valid HMAC webhook signatures correctly', async () => {
      ...
      expect(result.orderNumber).toBe('ONL-WEBHOOK-999');
  ...
  describe('Webhook Idempotency Protection', () => {
    it('skips duplicate webhook runs if payment is already recorded', async () => {
      ...
      vi.mocked(prisma.storefrontOrderPayment).findFirst.mockResolvedValue({ id: 'existing-pay-record' } as never);
      ...
      const result = await checkoutService.handleStripeWebhook(Buffer.from(payload), signature);
