...
describe("CrmMailboxService", () => {
  ...
  describe("buildAuthorizationUrl", () => {
    ...
    it("throws if the provider client id is not configured", () => {
      ...
      expect(() =>
        service.buildAuthorizationUrl(TENANT, USER, {
          provider: "MICROSOFT",
  ...
  describe("handleCallback", () => {
    it("exchanges the code for tokens, resolves the email address, and stores an encrypted connection", async () => {
      ...
      (
        prisma.mailboxConnection.upsert as ReturnType<typeof vi.fn>
    ...
    it("throws BadRequestException when token exchange fails", async () => {
      ...
      await expect(
        service.handleCallback(TENANT, USER, {
          provider: "GOOGLE",
          code: "bad",
          redirectUri: "https://app.example.com/cb",
        }),
  ...
  describe("disconnect", () => {
    it("marks the connection DISCONNECTED and stamps disconnectedAt", async () => {
      (
        prisma.mailboxConnection.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "mbx-1", tenantId: TENANT, userId: USER });
      (
        prisma.mailboxConnection.update as ReturnType<typeof vi.fn>
      ).mockImplementation(({ data }) =>
        Promise.resolve({
          id: "mbx-1",
          provider: "GOOGLE",
          emailAddress: "rep@tenant.com",
          createdAt: new Date(),
          scope: null,
          ...data,
        }),
      ...
      expect(emit).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ id: "mbx-1" }),
      );
    });
    ...
    it("throws NotFoundException for an unknown connection id", async () => {
      ...
      await expect(
        service.disconnect(TENANT, USER, "missing"),
      ).rejects.toThrow();
    });
  });
  ...
  describe("syncNow", () => {
    const baseConnection = {
      ...
      orgId: "org-1",
      status: "CONNECTED",
      accessTokenEnc: "enc:AT1",
      refreshTokenEnc: "enc:RT1",
    ...
    it("matches an inbound message to a known Contact and writes an EMAIL Activity", async () => {
          ...
          json: {
            ...
            payload: {
              headers: [
                {
                  name: "From",
                  value: "Jane Prospect <jane@known-contact.com>",
                },
      ...
      (prisma.contact.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([
        { id: "contact-1", customerId: null, email: "jane@known-contact.com" },
      ]);
      ...
      (prisma.activity.create as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "act-1",
      });
      (
        prisma.mailboxConnection.update as ReturnType<typeof vi.fn>
    ...
    it("throws if the mailbox is not CONNECTED", async () => {
      (
        prisma.mailboxConnection.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ ...baseConnection, status: "DISCONNECTED" });
