...
describe("CrmCasesService", () => {
  ...
  describe("createCase", () => {
    it("rejects a case with no subject", async () => {
      await expect(
        service.createCase(TENANT, "", { subject: "" }),
    ...
    it("assigns a sequential case number and a priority-based SLA deadline", async () => {
      ...
      (prisma.case.create as ReturnType<typeof vi.fn>).mockImplementation(
        ({ data }) => Promise.resolve({ id: "case-1", ...data }),
      );

      const before = Date.now();
      const result = await service.createCase(TENANT, "", {
  ...
  describe("updateCase", () => {
    it("throws if the case does not exist", async () => {
      ...
      await expect(
        service.updateCase(TENANT, "missing", { status: "RESOLVED" }),
    ...
    it("stamps resolvedAt when transitioning to RESOLVED for the first time", async () => {
      ...
      (prisma.case.update as ReturnType<typeof vi.fn>).mockImplementation(
        ({ data }) => Promise.resolve({ id: "case-1", ...data }),
      );

      const result = await service.updateCase(TENANT, "case-1", {
        status: "RESOLVED",
      });
    ...
    it("does not overwrite an existing resolvedAt", async () => {
      ...
      const updateCallData = (prisma.case.update as ReturnType<typeof vi.fn>)
        .mock.calls[0][0].data;
  ...
  describe("updateCase (status transition guard)", () => {
    ...
    it("rejects CLOSED -> OPEN via a plain status update, with a message pointing at the reopen endpoint", async () => {
      ...
      await expect(
        service.updateCase(TENANT, "case-1", { status: "OPEN" }),
    ...
    it("rejects CLOSED -> any other status", async () => {
      ...
      await expect(
        service.updateCase(TENANT, "case-1", { status: "IN_PROGRESS" }),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe("reopenCase", () => {
    it("throws if the case does not exist", async () => {
      ...
      await expect(service.reopenCase(TENANT, "missing")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("rejects reopening a case that is not CLOSED", async () => {
      ...
      await expect(service.reopenCase(TENANT, "case-1")).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("addComment", () => {
    it("throws if the case does not exist", async () => {
      ...
      await expect(
        service.addComment(TENANT, "missing", { body: "hi" }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it("rejects an empty comment body", async () => {
      ...
      await expect(
        service.addComment(TENANT, "case-1", { body: "" }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("sets firstResponseAt on the first non-internal comment", async () => {
      ...
      await service.addComment(TENANT, "case-1", {
  ...
  describe("getSlaStatus", () => {
      ...
      (prisma.case.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([
        {
          ...
          caseNumber: "CASE-1",
          status: "OPEN",
          slaDeadline: new Date(now - 60 * 60 * 1000),
        }, // breached (1h ago)
        {
          id: "2",
          caseNumber: "CASE-2",
        ...
        }, // at risk (1h from now)
        {
          ...
          caseNumber: "CASE-3",
          status: "OPEN",
          slaDeadline: new Date(now + 48 * 60 * 60 * 1000),
        }, // on track (2 days out)
      ]);
    ...
    it("only considers cases not yet RESOLVED/CLOSED", async () => {
      ...
      const call = (prisma.case.findMany as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
  ...
  describe("bulkUpdateCaseStatus", () => {
    it("rejects invalid status values", async () => {
      await expect(
        service.bulkUpdateCaseStatus(TENANT, ["case-1"], "INVALID"),
      ).rejects.toThrow(BadRequestException);
    });

    it("updates multiple cases with a valid status", async () => {
      (prisma.case.updateMany as ReturnType<typeof vi.fn>).mockResolvedValue({
        count: 3,
      });

      const result = await service.bulkUpdateCaseStatus(
        TENANT,
      ...
      const call = (prisma.case.updateMany as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
  ...
  describe("getCaseSummary", () => {
    it("throws if the case does not exist", async () => {
      ...
      await expect(service.getCaseSummary(TENANT, "missing")).rejects.toThrow(
        NotFoundException,
      );
    });
