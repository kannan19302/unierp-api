...
describe("BuilderController", () => {
  ...
  for (const { name, createDto } of endpoints) {
    ...
    describe(name, () => {
      ...
      it("should get by id", async () => {
        expect(
          await (controller as any)[`get${singularName}ById`](req, "1"),
        ).toEqual({});
      });
      it("should create", async () => {
        expect(
          await (controller as any)[`create${singularName}`](req, createDto),
        ).toEqual({ id: "1" });
      });
      it("should update", async () => {
        expect(
          await (controller as any)[`update${singularName}`](req, "1", {}),
        ).toEqual({ id: "1" });
      });
      it("should delete", async () => {
        expect(
          await (controller as any)[`delete${singularName}`](req, "1"),
        ).toEqual(true);
      });
    });
  }
  ...
  describe("DataImports", () => {
    ...
    it("should create", async () => {
      expect(
        await controller.createDataImport(req as any, {
          name: "n",
          targetModel: "t",
          fileName: "f",
          fileSize: 1,
  ...
  describe("ReadOnly endpoints", () => {
    it("should get assets", async () =>
      expect(await controller.getWebAssets(req as any)).toEqual([]));
    it("should get templates", async () =>
      expect(await controller.getWebTemplates(req as any)).toEqual([]));
    it("should get menus", async () =>
      expect(await controller.getWebMenus(req as any)).toEqual([]));
    it("should get seo", async () =>
      expect(await controller.getWebSeo(req as any)).toEqual([]));
    it("should get stats", async () =>
      expect(await controller.getStats(req as any)).toEqual({}));
