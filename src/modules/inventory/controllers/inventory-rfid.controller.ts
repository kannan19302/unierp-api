...
} from "../services/inventory-rfid.service";
...
export class InventoryRfidController {
  ...
  bulkRegisterTags(
    @Req() req: AuthReq,
    @ZodBody(z.array(registerTagSchema)) dto: any,
  ) {
  ...
  updateTag(
    @Req() req: AuthReq,
    @Param("id") id: string,
    @ZodBody(registerTagSchema.partial()) dto: any,
  ) {
  ...
  updateTagLocation(
    @Req() req: AuthReq,
    @Param("id") id: string,
    @ZodBody(updateTagLocationSchema) dto: any,
  ) {
  ...
  recordReadEvent(
    @Req() req: AuthReq,
    @ZodBody(recordReadEventSchema) dto: any,
  ) {
  ...
  @Get("dashboard")
  @ApiOperation({
    summary: "RFID dashboard with status counts and recent reads",
  })
