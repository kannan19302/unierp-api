...
describe("ProjectPreviewService", () => {
  it("stores only a hash of a short-lived preview token", async () => {
    ...
    const previewSession = { count: vi.fn(async () => 0), create, update: vi.fn(async () => ({})) }; const service = new ProjectPreviewService({ currentComposition: vi.fn(async () => ({ fingerprint: "fp" })) } as any, {} as any); (service as any).db = { $transaction: (callback: any) => callback({ $executeRawUnsafe: vi.fn(), projectPreviewSession: previewSession }), projectPreviewSession: previewSession, developerAuditEvent: { create: audit } };
