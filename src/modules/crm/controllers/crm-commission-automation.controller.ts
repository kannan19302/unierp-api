...
} from "../services/crm-commission-automation.service";
...
export class CrmCommissionAutomationController {
  ...
  @ApiOperation({
    summary: "Add a quota-attainment accelerator tier to a plan",
  })
  @Post(":id/tiers")
  @Permissions("crm.commission.manage")
  ...
  async addTier(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(createTierSchema) dto: CreateTierInput,
  ) {
  ...
  async createSpiff(
    @Req() req: AuthenticatedRequest,
    @ZodBody(createSpiffSchema) dto: CreateSpiffInput,
  ) {
  ...
  async updateSpiff(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(updateSpiffSchema) dto: UpdateSpiffInput,
  ) {
  ...
  @ApiOperation({
    summary:
      "Calculate quota-attainment-tiered + SPIFF payouts for a plan/period",
  })
  ...
  async calculatePayouts(
    @Req() req: AuthenticatedRequest,
    @ZodBody(calculatePayoutsSchema) dto: CalculatePayoutsInput,
  ) {
    return this.svc.calculatePayouts(
      req.user.tenantId,
      req.user.orgId ?? "",
      dto,
    );
