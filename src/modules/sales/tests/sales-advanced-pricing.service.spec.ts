...
describe("SalesAdvancedPricingService", () => {
  ...
  beforeEach(() => {
    service = new SalesAdvancedPricingService();
    vi.clearAllMocks();
  });

  it("should list customer price lists", async () => {
    vi.mocked(prisma.customerPriceList.findMany).mockResolvedValue([
      { id: "pl-1", name: "Acme Corp Pricing", _count: { items: 5 } },
    ] as never);
  ...
  it("should create customer price list", async () => {
    ...
    vi.mocked(prisma.customerPriceList.create).mockResolvedValue({
      id: "pl-1",
      name: "Custom Pricing",
    } as never);
    const dto = {
      customerId: "c1",
      name: "Custom Pricing",
      items: [{ productId: "p1", unitPrice: 50 }],
    };
    const result = await service.createCustomerPriceList(
      "tenant-1",
      "org-1",
      dto,
    );
  ...
  it("should add price list item", async () => {
    vi.mocked(prisma.customerPriceList.findFirst).mockResolvedValue({
      id: "pl-1",
    } as never);
    vi.mocked(prisma.customerPriceListItem.create).mockResolvedValue({
      id: "pli-1",
    } as never);
    const result = await service.addPriceListItem("tenant-1", "pl-1", {
      productId: "p1",
      unitPrice: 45,
    });
  ...
  it("should create contract pricing override", async () => {
    ...
    vi.mocked(prisma.contractPricingOverride.create).mockResolvedValue({
      id: "cpo-1",
    } as never);
    const dto = {
      contractId: "ct-1",
    ...
    const result = await service.createContractPricing(
      "tenant-1",
      "org-1",
      dto,
    );
  ...
  it("should create and approve floor price", async () => {
    ...
    vi.mocked(prisma.floorPriceOverride.create).mockResolvedValue({
      id: "fp-1",
      floorPrice: 10,
    } as never);
    vi.mocked(prisma.floorPriceOverride.findFirst).mockResolvedValue({
      id: "fp-1",
      isActive: false,
    } as never);
    const created = await service.createFloorPrice("tenant-1", "org-1", {
      productId: "p1",
      floorPrice: 10,
    });
    vi.mocked(prisma.floorPriceOverride.findFirst).mockResolvedValue({
    ...
    const approved = await service.approveFloorPrice(
      "tenant-1",
      "fp-1",
      true,
      "user-1",
  ...
  it("should calculate tiered price", async () => {
    ...
    const result = await service.calculateTieredPrice("tenant-1", {
      productId: "p1",
      quantity: 100,
    });
