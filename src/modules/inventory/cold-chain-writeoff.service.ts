...
export class ColdChainWriteoffService {
  ...
  async deactivateRequirement(tenantId: string, productId: string) {
    ...
    if (!req)
      throw new NotFoundException(
        `No cold-chain requirement for product ${productId}`,
      );
    return prisma.coldChainRequirement.update({
      where: { tenantId_productId: { tenantId, productId } },
      data: { active: false },
  ...
  async logExcursion(
    ...
    if (!req)
      throw new NotFoundException(
        `No cold-chain requirement for product ${data.productId}`,
      );

    const startAt = new Date(data.excursionStartAt);
    const endAt = data.excursionEndAt
      ? new Date(data.excursionEndAt)
      : undefined;
    const durationMins = endAt
    ...
    const severity =
      tempDiff > 10 ? "CRITICAL" : tempDiff > 5 ? "MODERATE" : "MINOR";
    ...
    return prisma.temperatureExcursion.create({
      data: {
        ...
        recordedHumidityPct: data.recordedHumidityPct
          ? new Prisma.Decimal(data.recordedHumidityPct)
          : undefined,
  ...
  async reviewExcursion(
    ...
    const exc = await prisma.temperatureExcursion.findFirst({
      where: { tenantId, id },
    });
  ...
  async getExcursionSummary(tenantId: string) {
    const [open, underReview, quarantined, total] = await Promise.all([
      prisma.temperatureExcursion.count({
      ...
      prisma.temperatureExcursion.count({
        where: { tenantId, status: "QUARANTINED" },
      }),
      prisma.temperatureExcursion.count({ where: { tenantId } }),
    ]);
    const critical = await prisma.temperatureExcursion.count({
  ...
  async createWriteDown(
    ...
    if (data.proposedValuePerUnit >= data.originalValuePerUnit) {
      throw new BadRequestException(
        "Proposed value must be less than original value for a write-down",
      );
    }
    const count = await prisma.stockWriteDownRequest.count({
      where: { tenantId },
    });
  ...
  async approveWriteDown(tenantId: string, id: string, approvedById: string) {
    const req = await prisma.stockWriteDownRequest.findFirst({
      where: { tenantId, id },
    });
    if (!req) throw new NotFoundException(`Write-down ${id} not found`);
    if (req.status !== "PENDING_APPROVAL")
      throw new BadRequestException(
        `Cannot approve a ${req.status} write-down`,
      );
  ...
  async rejectWriteDown(tenantId: string, id: string, rejectionNotes: string) {
    ...
    if (!req) throw new NotFoundException(`Write-down ${id} not found`);
    if (req.status !== "PENDING_APPROVAL")
      throw new BadRequestException(`Cannot reject a ${req.status} write-down`);
    return prisma.stockWriteDownRequest.update({
      where: { id },
  ...
  async applyWriteDown(tenantId: string, id: string) {
    ...
    if (req.status !== "APPROVED")
      throw new BadRequestException(`Cannot apply a ${req.status} write-down`);
    return prisma.stockWriteDownRequest.update({
      where: { id },
  ...
  async createWriteOff(
    ...
    const count = await prisma.stockWriteOffRecord.count({
      where: { tenantId },
    });
  ...
  async approveWriteOff(tenantId: string, id: string, approvedById: string) {
    const rec = await prisma.stockWriteOffRecord.findFirst({
      where: { tenantId, id },
    });
    if (!rec) throw new NotFoundException(`Write-off ${id} not found`);
    if (rec.status !== "PENDING_APPROVAL")
      throw new BadRequestException(`Cannot approve a ${rec.status} write-off`);
  ...
  async rejectWriteOff(tenantId: string, id: string, rejectionNotes: string) {
    ...
    if (!rec) throw new NotFoundException(`Write-off ${id} not found`);
    if (rec.status !== "PENDING_APPROVAL")
      throw new BadRequestException(`Cannot reject a ${rec.status} write-off`);
    return prisma.stockWriteOffRecord.update({
      where: { id },
      data: { status: "REJECTED", rejectionNotes },
    });
  ...
  async completeWriteOff(tenantId: string, id: string) {
    ...
    if (rec.status !== "APPROVED")
      throw new BadRequestException(
        `Cannot complete a ${rec.status} write-off`,
      );
    return prisma.stockWriteOffRecord.update({
      where: { id },
  ...
  async getDashboard(tenantId: string) {
    const [
      ...
      prisma.stockWriteDownRequest.count({
        where: { tenantId, status: "PENDING_APPROVAL" },
      }),
      prisma.stockWriteOffRecord.count({
        where: { tenantId, status: "PENDING_APPROVAL" },
      }),
    ...
    return {
      ...
      totalWriteOffValue: Number(
        totalWriteOffValue[0]?._sum?.totalWriteOff ?? 0,
      ),
    };
  }
}
