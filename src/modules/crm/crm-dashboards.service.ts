...
export class CrmDashboardsService {
  ...
  async updateDashboard(
    tenantId: string,
    id: string,
    dto: UpdateCrmDashboardInput,
  ) {
    const existing = await prisma.crmDashboard.findFirst({
  ...
  async addWidget(
    ...
    return prisma.crmDashboardWidget.create({
      data: {
        ...
        refreshInterval: dto.refreshInterval || 0,
      },
    });
  }
  ...
  async updateWidget(
    ...
    return prisma.crmDashboardWidget.update({
      where: { id: widgetId },
      data: {
