...
describe("CustomerPortalService", () => {
  ...
  describe("inviteUser", () => {
    ...
    it("rejects a duplicate email within the tenant", async () => {
      ...
      (
        prisma.customerPortalUser.findFirst as ReturnType<typeof vi.fn>
    ...
    it("creates a portal user with a hashed temp password", async () => {
      ...
      (
        prisma.customerPortalUser.create as ReturnType<typeof vi.fn>
      ).mockImplementation(({ data }) =>
        Promise.resolve({ id: "pu1", ...data }),
      );
      const result = await service.inviteUser(TENANT, CUSTOMER, {
        email: "new@b.com",
      });
  ...
  describe("login", () => {
    it("rejects invalid credentials", async () => {
      (
        prisma.customerPortalUser.findMany as ReturnType<typeof vi.fn>
      ).mockResolvedValue([]);
      await expect(
        service.login({ email: "x@y.com", password: "bad" }),
      ).rejects.toThrow("Invalid portal credentials");
    });
    ...
    it("issues a portal-scoped token on success", async () => {
      ...
      (
        prisma.customerPortalUser.update as ReturnType<typeof vi.fn>
      ).mockResolvedValue({});
  ...
  describe("quotation self-service", () => {
    it("accepts a SENT quotation", async () => {
      (
        prisma.quotation.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "q1", status: "SENT" });
      (prisma.quotation.update as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "q1",
        status: "ACCEPTED",
      });
    ...
    it("refuses to accept a quotation not in SENT status", async () => {
      (
        prisma.quotation.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "q1", status: "DRAFT" });
      await expect(
        service.acceptQuotation(TENANT, CUSTOMER, "q1"),
    ...
    it("rejects a quotation and appends the reason to notes", async () => {
      (
        prisma.quotation.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "q1", status: "SENT", notes: null });
      (prisma.quotation.update as ReturnType<typeof vi.fn>).mockImplementation(
        ({ data }) => Promise.resolve({ id: "q1", ...data }),
      );
      const result = await service.rejectQuotation(TENANT, CUSTOMER, "q1", {
    ...
    it("throws when the quotation does not belong to this customer", async () => {
      ...
      await expect(
        service.getMyQuotationDetail(TENANT, CUSTOMER, "q-other"),
  ...
  describe("support cases", () => {
    it("creates a case with WEB channel and an auto-generated case number", async () => {
      (prisma.customer.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(
        { id: CUSTOMER, orgId: "org-1" },
      );
      (prisma.case.count as ReturnType<typeof vi.fn>).mockResolvedValue(4);
      (prisma.case.create as ReturnType<typeof vi.fn>).mockImplementation(
        ({ data }) => Promise.resolve({ id: "c1", ...data }),
      );
      const result = await service.createCase(TENANT, CUSTOMER, "pu1", {
        subject: "Help",
        priority: "MEDIUM",
      });
    ...
    it("scopes case detail lookup to the caller customerId", async () => {
      ...
      await expect(
        service.getMyCaseDetail(TENANT, CUSTOMER, "not-mine"),
    ...
    it("adds a portal-authored, non-internal comment", async () => {
      ...
      (
        prisma.caseComment.create as ReturnType<typeof vi.fn>
      ).mockImplementation(({ data }) =>
  ...
  describe("dashboard summary", () => {
    it("aggregates counts scoped to the customer", async () => {
      ...
      (prisma.customer.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(
        { id: CUSTOMER, name: "Acme", email: "a@acme.com" },
      );
