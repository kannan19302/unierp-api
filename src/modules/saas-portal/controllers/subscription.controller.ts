...
export class SaasPortalSubscriptionController {
  constructor(
    private readonly subscriptionService: SaasPortalSubscriptionService,
  ) {}
  ...
  async updateSeats(
    @Req() req: AuthenticatedRequest,
    @ZodBody(updateSeatsSchema) body: z.infer<typeof updateSeatsSchema>,
  ) {
  ...
  async validateSubscriptionAccess(@Req() req: AuthenticatedRequest) {
    return this.subscriptionService.validateSubscriptionAccess(
      req.user.tenantId,
    );
  }
