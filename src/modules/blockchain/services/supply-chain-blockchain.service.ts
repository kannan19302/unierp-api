...
export class SupplyChainBlockchainService {
  ...
  async getProvenance(
    ...
    const rawContract = this.fabricGateway.getContract(
      FABRIC_CHAINCODES.SUPPLY_CHAIN,
    );
  ...
  async issueRecall(
    ...
    if (!rawContract) {
      this.logger.warn(
        `Fabric unavailable — recall for batch ${args.batchId} deferred`,
      );
    ...
    try {
      ...
      this.logger.warn(
        `RECALL issued: Batch ${args.batchId} — ${args.reason} (Severity: ${args.severity})`,
      );
      return {
