...
export class SaasPortalEnterpriseService {
  ...
  async getTenantAnalytics(tenantId: string, dateRange?: string) {
    ...
    const churned = tenants.filter(
      (t) => t.status === "CANCELLED" || t.status === "SUSPENDED",
    ).length;
    const recent = tenants.filter(
      (t) => t.createdAt >= new Date(Date.now() - 30 * 86400000),
    ).length;
    return {
      totalTenants: total,
  ...
  async getSubscriptionMetrics(
    ...
    const mrr =
      saasInvoices
        ...
        .reduce((s, i) => s + Number(i.totalAmount || 0), 0) / 12;
    const expansionRevenue =
      subscriptions.filter((s) => s.status === "ACTIVE").length * 100;
    const contraction =
      subscriptions.filter(
        (s) => s.status === "CANCELLED" || s.status === "PAST_DUE",
      ).length * 50;
    return {
  ...
  async getUsageMetering(tenantId: string, period?: string) {
    ...
    const meters = await this.p.saasUsageMeter.findMany({
      where: { tenantId },
    });
    const subUsageBilling = await this.p.subscriptionUsageBilling.findMany();
    const apiCalls = usageRecords
      .filter((r) => r.metric === "API_CALLS_COUNT")
      .reduce((s, r) => s + r.currentValue, 0);
    const storageMb = usageRecords
      .filter((r) => r.metric === "STORAGE_MB")
      .reduce((s, r) => s + r.currentValue, 0);
    const activeUsers = usageRecords
      .filter((r) => r.metric === "USERS_COUNT")
      .reduce((s, r) => s + r.currentValue, 0);
    return {
      ...
      usageByTier: {
        free: { apiCalls: 10000, storageMb: 100, users: 5 },
        pro: { apiCalls: 100000, storageMb: 1024, users: 25 },
        enterprise: { apiCalls: 1000000, storageMb: 10240, users: 100 },
      },
