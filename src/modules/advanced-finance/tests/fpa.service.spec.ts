...
describe("FpaService", () => {
  ...
  describe("listCloseTasks", () => {
    it("should return close tasks filtered by status", async () => {
      ...
      (prisma.closeTask.findMany as ReturnType<typeof vi.fn>).mockResolvedValue(
        mockTasks,
      );
  ...
  describe("getCloseTask", () => {
    it("should return a close task by id", async () => {
      ...
      (
        prisma.closeTask.findFirst as ReturnType<typeof vi.fn>
    ...
    it("should throw NotFoundException if task does not exist", async () => {
      ...
      await expect(
        service.getCloseTask(TENANT, "task-missing"),
  ...
  describe("createCloseTask", () => {
    it("should create a new close task when period exists", async () => {
      ...
      (prisma.closeTask.create as ReturnType<typeof vi.fn>).mockResolvedValue(
        mockCreated,
      );
  ...
  describe("updateCloseTask", () => {
    it("should update task and assign completed dates if status is DONE", async () => {
      ...
      (prisma.closeTask.update as ReturnType<typeof vi.fn>).mockImplementation(
        ({ data }) => Promise.resolve({ ...mockTask, ...data }),
      );

      const result = await service.updateCloseTask(TENANT, "task-1", USER_ID, {
        status: "DONE",
      });
  ...
  describe("generateCloseTasksFromTemplate", () => {
    it("should bulk generate standard tasks from templates", async () => {
      ...
      (
        prisma.closeTask.createMany as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ count: 13 });

      const result = await service.generateCloseTasksFromTemplate(
        TENANT,
        USER_ID,
        "period-1",
      );
  ...
  describe("runVarianceFlagEngine", () => {
    it("should execute comparison and create flags when variances exceed threshold", async () => {
      ...
      (
        prisma.varianceFlag.createMany as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ count: 1 });

      const result = await service.runVarianceFlagEngine(
        TENANT,
        "period-current",
        10,
      );
  ...
  describe("createScenario", () => {
    it("should create a budget scenario in DRAFT state", async () => {
      ...
      (
        prisma.budgetScenario.create as ReturnType<typeof vi.fn>
  ...
  describe("lockScenario", () => {
    it("should set scenario to locked and status to APPROVED", async () => {
      ...
      ).mockResolvedValue(mockScenario);
      (
        prisma.budgetScenario.update as ReturnType<typeof vi.fn>
  ...
  describe("cloneScenario", () => {
    it("should duplicate scenario metadata and all budget lines", async () => {
      ...
      (
        prisma.budgetScenario.create as ReturnType<typeof vi.fn>
      ).mockResolvedValue(mockClone);
      (
        prisma.budgetScenarioLine.createMany as ReturnType<typeof vi.fn>
  ...
  describe("applyDriver", () => {
    it("should generate monthly lines using headcount * salary driver calculation", async () => {
      ...
      (
        prisma.budgetScenarioLine.upsert as ReturnType<typeof vi.fn>
      ).mockResolvedValue({});
