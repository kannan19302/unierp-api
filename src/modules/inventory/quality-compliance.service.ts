...
export const createCapaSchema = z.object({
  ...
  source: z
    .enum(["INTERNAL", "CUSTOMER", "AUDIT", "NCR", "DEVIATION"])
    .default("INTERNAL"),
...
export const createSopSchema = z.object({
  title: z.string().min(1),
  category: z
    .enum(["SOP", "WORK_INSTRUCTION", "FORM", "POLICY", "SPECIFICATION"])
    .default("SOP"),
...
export class QualityComplianceService {
  ...
  async updateCapa(
    tenantId: string,
    id: string,
    dto: Partial<z.infer<typeof createCapaSchema>>,
  ) {
    const record = await prisma.capaRecord.findFirst({
      where: { id, tenantId },
    });
  ...
  async transitionCapaStatus(tenantId: string, id: string, newStatus: string) {
    ...
    if (!allowed[record.status]?.includes(newStatus)) {
      throw new BadRequestException(
        `Cannot transition CAPA from ${record.status} to ${newStatus}`,
      );
    }
  ...
  async addCapaAction(
    tenantId: string,
    capaId: string,
    dto: z.infer<typeof addCapaActionSchema>,
  ) {
  ...
  async completeCapaAction(tenantId: string, actionId: string, notes?: string) {
    ...
    if (action.status === "COMPLETE")
      throw new BadRequestException("Action already complete");
  ...
  async getCapaDashboard(tenantId: string) {
    const [open, inProgress, pendingVerification, closed, overdue] =
      await Promise.all([
  ...
  async listCalibrations(
    tenantId: string,
    filters: { status?: string; instrumentId?: string },
  ) {
    const where: Record<string, unknown> = { tenantId };
    if (filters.status) where["status"] = filters.status;
  ...
  async getCalibration(tenantId: string, id: string) {
    const record = await prisma.calibrationRecord.findFirst({
      where: { id, tenantId },
    });
  ...
  async scheduleCalibration(
    tenantId: string,
    dto: z.infer<typeof createCalibrationSchema>,
  ) {
  ...
  async recordCalibrationResult(
    tenantId: string,
    id: string,
    dto: z.infer<typeof recordCalibrationResultSchema>,
  ) {
    const record = await prisma.calibrationRecord.findFirst({
      where: { id, tenantId },
    });
  ...
  async getDeviation(tenantId: string, id: string) {
    const record = await prisma.deviationRecord.findFirst({
      where: { id, tenantId },
    });
  ...
  async createDeviation(
    tenantId: string,
    dto: z.infer<typeof createDeviationSchema>,
  ) {
  ...
  async reviewDeviation(tenantId: string, id: string, reviewNotes: string) {
    ...
    if (record.status !== "OPEN")
      throw new BadRequestException("Deviation is not in OPEN status");
  ...
  async closeDeviation(
    ...
    if (record.status === "CLOSED")
      throw new BadRequestException("Deviation already closed");
    return prisma.deviationRecord.update({
      where: { id },
  ...
  async escalateDeviationToCapa(
    tenantId: string,
    id: string,
    capaDto: z.infer<typeof createCapaSchema>,
  ) {
    const deviation = await prisma.deviationRecord.findFirst({
      where: { id, tenantId },
    });
    ...
    await prisma.deviationRecord.update({
      where: { id },
      data: { capaId: capa.id },
    });
    return capa;
  ...
  async getDeviationDashboard(tenantId: string) {
    const [open, underReview, closed, critical, major, minor] =
      await Promise.all([
        prisma.deviationRecord.count({ where: { tenantId, status: "OPEN" } }),
        prisma.deviationRecord.count({
        ...
        prisma.deviationRecord.count({
          where: { tenantId, severity: "MAJOR" },
        }),
        prisma.deviationRecord.count({
          where: { tenantId, severity: "MINOR" },
        }),
      ]);
    return {
      open,
  ...
  async listSops(
    tenantId: string,
    filters: { status?: string; category?: string; department?: string },
  ) {
    const where: Record<string, unknown> = { tenantId };
    if (filters.status) where["status"] = filters.status;
  ...
  async approveSop(
    ...
    if (doc.status !== "UNDER_REVIEW")
      throw new BadRequestException("Document must be UNDER_REVIEW to approve");
    return prisma.sopDocument.update({
      where: { id },
      data: {
        status: "APPROVED",
  ...
  async getSopsDueSoon(tenantId: string, days = 30) {
    ...
    return prisma.sopDocument.findMany({
      where: {
        tenantId,
        status: "APPROVED",
        OR: [{ reviewDate: { lte: cutoff } }, { expiryDate: { lte: cutoff } }],
      },
