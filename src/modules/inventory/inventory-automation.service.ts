...
export class InventoryAutomationService {
  ...
  async listReplenishmentRules(tenantId: string, warehouseId?: string) {
    ...
    return prisma.binReplenishmentRule.findMany({
      where,
      orderBy: { createdAt: "desc" },
    });
  }

  async createReplenishmentRule(
    tenantId: string,
    dto: CreateReplenishmentRuleInput,
  ) {
    ...
    if (existing) {
      throw new BadRequestException(
        "A replenishment rule for this product and bin already exists",
      );
    }
  ...
  async updateReplenishmentRule(
    ...
    dto: Partial<
      Pick<
        ...
        "triggerQty" | "replenishQty" | "reserveBinCode"
      >
    >,
  ) {
    const rule = await prisma.binReplenishmentRule.findFirst({
      where: { id, tenantId },
    });
    ...
    return prisma.binReplenishmentRule.update({
      where: { id },
      data: {
        ...(dto.triggerQty !== undefined && {
          triggerQty: new Prisma.Decimal(dto.triggerQty.toFixed(4)),
        }),
        ...(dto.replenishQty !== undefined && {
          replenishQty: new Prisma.Decimal(dto.replenishQty.toFixed(4)),
        }),
        ...(dto.reserveBinCode !== undefined && {
          reserveBinCode: dto.reserveBinCode,
        }),
      },
    });
  ...
  async listHolds(
    tenantId: string,
    query: { productId?: string; warehouseId?: string; status?: string },
  ) {
  ...
  async releaseHold(tenantId: string, id: string, dto: ReleaseHoldInput) {
    ...
    if (hold.status !== "ACTIVE")
      throw new BadRequestException(
        `Hold is ${hold.status.toLowerCase()}, cannot release`,
      );
  ...
  async cancelHold(tenantId: string, id: string) {
    ...
    if (hold.status !== "ACTIVE")
      throw new BadRequestException(
        `Hold is ${hold.status.toLowerCase()}, cannot cancel`,
      );
    return prisma.inventoryHold.update({
      where: { id },
