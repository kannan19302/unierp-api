...
export class Asc606DeepService {
  ...
  async getPerformanceObligationById(tenantId: string, id: string) {
    const ob = await prisma.performanceObligation.findFirst({
      where: { id, tenantId },
    });
  ...
  async createPerformanceObligation(tenantId: string, orgId: string, dto: any) {
    return prisma.performanceObligation.create({
      data: {
        ...
        tenantId,
        orgId,
        allocatedAmount: dto.allocatedAmount ?? dto.transactionPrice ?? 0,
      },
    });
  }
  ...
  async satisfyPerformanceObligation(
    ...
    if (ob.status === "SATISFIED")
      throw new BadRequestException("Already satisfied");
    return prisma.performanceObligation.update({
      where: { id },
      data: {
        status: "SATISFIED",
        satisfiedDate: satisfiedDate ? new Date(satisfiedDate) : new Date(),
      },
    });
  }

  async partiallySatisfyPerformanceObligation(
    tenantId: string,
    id: string,
    ...
    if (newRecognized > totalAllocated)
      throw new BadRequestException(
        "Cannot recognize more than allocated amount",
      );
    const status =
      Math.abs(newRecognized - totalAllocated) < 0.01
        ? "SATISFIED"
        : "PARTIALLY_SATISFIED";
    return prisma.performanceObligation.update({
      where: { id },
      data: {
        ...
        revenueDeferred: new Prisma.Decimal(
          Math.max(0, totalAllocated - newRecognized),
        ),
        status,
  ...
  async allocateTransactionPrice(
    ...
    if (obligations.length === 0)
      throw new NotFoundException("No active performance obligations found");

    const totalSsp = obligations.reduce((s, o) => s + Number(o.ssp || 0), 0);
    if (totalSsp === 0)
      throw new BadRequestException("No SSP values found for allocation");

    const transactionPrice = obligations.reduce(
      (s, o) => s + Number(o.transactionPrice),
      0,
    );
    const results: any[] = [];
  ...
  async getObligationHierarchy(tenantId: string, parentId: string) {
    ...
    const children = await prisma.performanceObligation.findMany({
  ...
  async getContractModificationById(tenantId: string, id: string) {
    const mod = await prisma.asc606ContractModification.findFirst({
      where: { id, tenantId },
    });
  ...
  async createContractModification(tenantId: string, orgId: string, dto: any) {
    return prisma.asc606ContractModification.create({
      data: { ...dto, tenantId, orgId },
    });
  }
  ...
  async approveContractModification(
    ...
    if (mod.status !== "DRAFT")
      throw new BadRequestException("Only draft modifications can be approved");
    return prisma.asc606ContractModification.update({
      where: { id },
  ...
  async applyContractModification(tenantId: string, id: string) {
    ...
    if (mod.status !== "APPROVED")
      throw new BadRequestException("Modification must be approved first");
    return prisma.asc606ContractModification.update({
      where: { id },
      data: { status: "APPLIED" },
    });
  ...
  async getDeferredRevenueRollForwards(tenantId: string, period?: string) {
    ...
    return prisma.asc606DeferredRevenueRollForward.findMany({
      where,
      orderBy: { period: "desc" },
    });
  }
  ...
  async getDeferredRevenueRollForwardById(tenantId: string, id: string) {
    const rf = await prisma.asc606DeferredRevenueRollForward.findFirst({
      where: { id, tenantId },
    });
    if (!rf)
      throw new NotFoundException("Deferred revenue roll-forward not found");
    return rf;
  }

  async createDeferredRevenueRollForward(
    tenantId: string,
    orgId: string,
    dto: any,
  ) {
  ...
  async updateDeferredRevenueRollForward(
    tenantId: string,
    id: string,
    dto: any,
  ) {
    ...
    if (dto.additions !== undefined)
      updateData.additions = new Prisma.Decimal(dto.additions);
    if (dto.recognized !== undefined)
      updateData.recognized = new Prisma.Decimal(dto.recognized);
    if (dto.writeOffs !== undefined)
      updateData.writeOffs = new Prisma.Decimal(dto.writeOffs);
    if (
      dto.openingBalance !== undefined ||
      dto.additions !== undefined ||
      dto.recognized !== undefined ||
      dto.writeOffs !== undefined
    ) {
      const existing = await this.getDeferredRevenueRollForwardById(
        tenantId,
        id,
      );
      const opening =
      ...
      const additions =
        dto.additions !== undefined
          ? Number(dto.additions)
          : Number(existing.additions);
      const recognized =
        dto.recognized !== undefined
          ? Number(dto.recognized)
          : Number(existing.recognized);
      const writeOffs =
        dto.writeOffs !== undefined
          ? Number(dto.writeOffs)
          : Number(existing.writeOffs);
      const closing = opening + additions - recognized - writeOffs;
    ...
    return prisma.asc606DeferredRevenueRollForward.update({
      where: { id },
      data: updateData,
    });
  }
  ...
  async getRollForwardByPeriod(
    ...
    if (!rf)
      throw new NotFoundException("Roll-forward not found for this period");
    return rf;
  }

  async computeRollForwardFromSchedules(
    tenantId: string,
    orgId: string,
    period: string,
  ) {
    ...
    const openingBalance = schedules.reduce(
      (s, sch) => s + Number(sch.deferredAmount),
      0,
    );
    ...
    const additions = newSchedulesInPeriod.reduce(
      (s, sch) => s + Number(sch.totalAmount),
      0,
    );
    ...
    const recognized = completedInPeriod.reduce(
      (s, sch) => s + Number(sch.recognizedAmount),
      0,
    );
  ...
  async getMultiElementSummary(tenantId: string, contractRef?: string) {
    ...
    const totalPrice = obligations.reduce(
      (s, o) => s + Number(o.transactionPrice),
      0,
    );
    const totalAllocated = obligations.reduce(
      (s, o) => s + Number(o.allocatedAmount),
      0,
    );
    const totalRecognized = obligations.reduce(
      (s, o) => s + Number(o.revenueRecognized),
      0,
    );
    const totalDeferred = obligations.reduce(
      (s, o) => s + Number(o.revenueDeferred),
      0,
    );
    const byStatus = {
      active: obligations.filter((o) => o.status === "ACTIVE").length,
      satisfied: obligations.filter((o) => o.status === "SATISFIED").length,
      partiallySatisfied: obligations.filter(
        (o) => o.status === "PARTIALLY_SATISFIED",
      ).length,
  ...
  async getVariableConsiderationEstimate(
    tenantId: string,
    contractRef: string,
  ) {
    const mods =
      (await prisma.asc606ContractModification.findMany({
        where: { tenantId, contractRef, status: "APPROVED" },
      })) ?? [];
    const totalCatchUp = mods.reduce(
      (s, m) => s + Number(m.cumulativeCatchUp),
      0,
    );
    const totalModified = mods.reduce(
      (s, m) => s + Number(m.modifiedConsideration),
      0,
    );
    const totalOriginal = mods.reduce(
      (s, m) => s + Number(m.originalConsideration),
      0,
    );
    return {
      contractRef,
      ...
      totalModified,
      totalOriginal,
      estimatedVariableConsideration: totalCatchUp,
    };
  }
  ...
  async getRevenueAging(tenantId: string, orgId: string) {
    ...
    for (const sch of schedules) {
      ...
      const daysSinceEnd = Math.max(
        0,
        Math.floor(
          (now.getTime() - new Date(sch.endDate).getTime()) / (1000 * 86400),
        ),
      );
    ...
    return {
      agingBuckets: aging,
      totalDeferred: Object.values(aging).reduce(
        (s: number, v: any) => s + v,
        0,
      ),
    };
    ...
    for (let i = 0; i < months; i++) {
      ...
      for (const sch of schedules) {
        ...
        const totalMonths = Math.max(
          1,
          (schEnd.getFullYear() - schStart.getFullYear()) * 12 +
            (schEnd.getMonth() - schStart.getMonth()) +
            1,
        );
        const monthly = total / totalMonths;
        if (monthStart >= schStart && monthStart <= schEnd)
          expectedRecognition += monthly;
      }
      void monthEnd;
      forecast.push({
  ...
  async getContractRevenueSummary(tenantId: string, contractRef: string) {
    ...
    return {
      contractRef,
      totalObligations: obligations.length,
      totalRevenue: obligations.reduce(
        (s, o) => s + Number(o.transactionPrice),
        0,
      ),
      totalRecognized: obligations.reduce(
        (s, o) => s + Number(o.revenueRecognized),
        0,
      ),
      totalDeferred: obligations.reduce(
        (s, o) => s + Number(o.revenueDeferred),
        0,
      ),
      modificationCount: mods.length,
  ...
  async getAsc606Dashboard(tenantId: string, orgId: string) {
    ...
    const activeCount = obligations.filter(
      (o) => o.status === "ACTIVE" || o.status === "PARTIALLY_SATISFIED",
    ).length;
    return {
      totalObligations: obligations.length,
      ...
      totalDeferred,
      totalRecognized,
      recentRollForwards: rollForwards,
    };
  }
}
