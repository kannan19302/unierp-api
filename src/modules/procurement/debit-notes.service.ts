...
export class DebitNotesService {
  ...
  async updateStatus(tenantId: string, id: string, status: string) {
    ...
    const validTransitions: Record<string, string[]> = {
      ...
      ISSUED: ["RECEIVED", "DISPUTED", "CANCELLED"],
      RECEIVED: ["SETTLED"],
    ...
    if (!allowed || !allowed.includes(status)) {
      throw new BadRequestException(
        `Cannot transition debit note from ${note.status} to ${status}`,
      );
    }
  ...
  async update(
    tenantId: string,
    id: string,
    dto: { amount?: number; reason?: string; lineItems?: any },
  ) {
    const note = await prisma.debitNote.findFirst({
    ...
    if (!note)
      throw new NotFoundException(
        "Debit note not found or not in DRAFT status",
      );
  ...
  async getStats(tenantId: string) {
    ...
    const pendingAmount = notes
      .filter((n) => !["SETTLED", "CANCELLED"].includes(n.status))
      .reduce((s, n) => s + Number(n.amount), 0);
    return {
      total: notes.length,
      totalAmount,
      pendingAmount,
      byStatus: notes.reduce((acc: Record<string, number>, n) => {
  ...
  async getDebitNotesByDateRange(
    tenantId: string,
    startDate: string,
    endDate: string,
    params: PaginationParams = {},
  ...
  async getDebitNoteStatsDetailed(tenantId: string) {
    ...
    const totalAmount = notes.reduce((s, n) => s + Number(n.amount), 0);
    const settledAmount = notes
      .filter((n) => n.status === "SETTLED")
      .reduce((s, n) => s + Number(n.amount), 0);
    const disputedAmount = notes
      .filter((n) => n.status === "DISPUTED")
      .reduce((s, n) => s + Number(n.amount), 0);

    const byVendor: Record<
      string,
      { vendorName: string; count: number; total: number }
    > = {};
    for (const note of notes) {
      const key = note.vendorId;
      if (!byVendor[key])
        byVendor[key] = { vendorName: note.vendor.name, count: 0, total: 0 };
    ...
    return {
      ...
      settlementRate:
        totalAmount > 0
          ? Math.round((settledAmount / totalAmount) * 10000) / 100
          : 0,
  ...
  async processDebitNoteRefund(
    ...
    if (note.status !== "ISSUED" && note.status !== "RECEIVED") {
      throw new BadRequestException(
        `Debit note status ${note.status} cannot be refunded`,
      );
    }
