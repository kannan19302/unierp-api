...
export class ReportsController {
  ...
  private readonly availableReports = [
    {
      ...
      name: "Usage Summary",
      category: "usage",
      description: "Overview of resource usage",
    },
    {
      id: "billing-summary",
    ...
    },
    {
      ...
      name: "User Activity",
      category: "analytics",
      description: "User login and action log",
    },
    {
      id: "team-collaboration",
      name: "Team Collaboration",
      category: "analytics",
      description: "Team activity metrics",
    },
    {
      id: "invoice-summary",
    ...
    },
    {
      ...
      name: "Growth Report",
      category: "analytics",
      description: "Tenant growth metrics",
    },
  ];
  ...
  async getReportHistory(@Req() req: AuthReq) {
    return this.dataExportService
      .listExportJobs(req.user.tenantId)
      .catch(() => []);
  }
  ...
  async getReportDetail(@Req() req: AuthReq, @Param("id") id: string) {
    return this.dataExportService
      .getExportJob(req.user.tenantId, id)
      .catch(() => null);
  }
  ...
  async downloadReport(@Req() req: AuthReq, @Param("id") id: string) {
    return this.dataExportService
      .downloadExport(req.user.tenantId, id)
      .catch(() => null);
  }
  ...
  async getUserActivityReport(@Req() req: AuthReq) {
    return this.auditLogService
      .listAuditLogs(req.user.tenantId, 1, 200, {})
      .catch(() => ({ items: [], total: 0 }));
  }
  ...
  async getTeamCollaborationReport(@Req() req: AuthReq) {
    ...
    return {
      ...
      uniqueUsers:
        new Set((logs as any).items?.map((l: any) => l.resourceId)).size || 0,
    };
  }
  ...
  async scheduleReport(
    @Req() _req: AuthReq,
    @ZodBody(scheduleReportSchema) body: z.infer<typeof scheduleReportSchema>,
  ) {
