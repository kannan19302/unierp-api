...
export class CommunicationService {
  ...
  async getDirectory(tenantId: string) {
    ...
    return users.map((u) => {
      ...
      const hidden = p?.visibility === "NOBODY";
      return {
        id: u.id,
        ...
        presence: (hidden
          ? "INACTIVE"
          : (p?.presence ?? "INACTIVE")) as Presence,
        statusText: hidden ? null : (p?.statusText ?? null),
        statusEmoji: hidden ? null : (p?.statusEmoji ?? null),
  ...
  async getWorkspace(tenantId: string, userId: string, orgId?: string) {
    ...
    const [spaces, directory, channels, memberships] = await Promise.all([
      prisma.connectSpace.findMany({
        where: { tenantId },
        orderBy: { createdAt: "asc" },
      }),
      this.getDirectory(tenantId),
      prisma.channel.findMany({
        where: { tenantId, kind: "CHANNEL", archived: false },
        orderBy: { name: "asc" },
      }),
      prisma.channelMember.findMany({
        where: { tenantId, userId },
        select: { channelId: true, starred: true, muted: true },
      }),
    ]);
    ...
    return {
      me: directory.find((d) => d.id === userId) ?? {
        id: userId,
        name: "You",
        email: "",
        avatar: null,
  ...
  private assertRole(
    ...
    allowed: ChannelMemberRole[],
  ) {
    if (
      !membership ||
      !allowed.includes(membership.role as ChannelMemberRole)
    ) {
  ...
  async updateChannel(
    ...
    const { channel, membership } = await this.getMembership(
      tenantId,
      channelId,
      userId,
    );
    if (channel.kind !== "CHANNEL")
      throw new BadRequestException("Only channels can be renamed/archived.");
  ...
  async removeChannelMember(
    ...
    await prisma.message.create({
      data: {
        ...
        userId,
        kind: "SYSTEM",
        content: `${target ? this.displayName(target) : "A member"} left the channel`,
      },
    });
    return { ok: true };
  }
  ...
  async getChannelMembers(tenantId: string, channelId: string, userId: string) {
    ...
    return members.map((m) => ({
      userId: m.userId,
      role: m.role as ChannelMemberRole,
    }));
  }
  ...
  async browseChannels(tenantId: string, userId: string) {
    ...
    const channels = await prisma.channel.findMany({
      where: {
        ...
        type: "PUBLIC",
        archived: false,
        id: { notIn: myChannelIds.length ? myChannelIds : ["_"] },
      },
  ...
  async joinChannel(tenantId: string, channelId: string, userId: string) {
    ...
    await prisma.message.create({
      data: {
        ...
        userId,
        kind: "SYSTEM",
        content: `${user ? this.displayName(user) : "A member"} joined the channel`,
      },
    });
    return member;
  }
  ...
  async searchMessages(
    ...
    return rows.map((r) => {
      ...
      const snippet =
        idx >= 0
          ? `${start > 0 ? "…" : ""}${r.content.slice(start, idx + q.length + 40)}${idx + q.length + 40 < r.content.length ? "…" : ""}`
          : r.content.slice(0, 80);
      return {
        messageId: r.id,
        channelId: r.channelId,
        channelName:
          r.channelKind === "CHANNEL" ? `#${r.channelName}` : r.channelName,
        authorId: r.userId,
        authorName,
        snippet,
  ...
  async uploadAttachment(
    ...
    if (file.size > MAX_ATTACHMENT_BYTES) {
      throw new BadRequestException(
        `File exceeds the ${MAX_ATTACHMENT_BYTES / (1024 * 1024)}MB attachment limit.`,
      );
    }
  ...
  async createMessage(
    ...
    this.notificationsGateway.broadcastChatMessage(channelId, {
      ...serialized,
      tenantId,
    });
    return serialized;
  ...
  private async notifyMentions(
    ...
    const tokens = Array.from(content.matchAll(/@(\w+)/g)).map((m) =>
      m[1]!.toLowerCase(),
    );
    ...
    const targets = users.filter(
      (u) => u.id !== authorId && tokens.includes(u.firstName.toLowerCase()),
    );
    const snippet =
  ...
  async getMeetings(tenantId: string) {
    return prisma.connectMeeting.findMany({
  ...
  async createMeeting(
    ...
    const meeting = await prisma.connectMeeting.create({
      data: {
        ...
        title: dto.title || "Connect meeting",
        channelId: dto.conversationId || null,
        active: true,
  ...
  async getLinkPreview(urlStr: string) {
    ...
    if (!(await isSafeUrl(cleanUrl))) {
      throw new BadRequestException(
        "Access to the requested URL is restricted.",
      );
    }
    ...
    try {
      ...
      const result = {
        ...
        title:
          ...
          this.extractMeta(html, /<title>([^<]*)<\/title>/i) ||
          undefined,
        description:
          this.extractMeta(
            html,
  ...
  async forwardMessage(
    ...
    const newMsg = await prisma.message.create({
      data: {
        ...
        userId,
        content: `${forwardNote}${msg.content}`,
        kind: "USER",
        attachments: msg.attachments as Prisma.InputJsonValue,
      },
      include: { reactions: { select: { emoji: true, userId: true } } },
    });
    await prisma.channel.update({
  ...
  async searchMessagesFiltered(
    ...
    if (filters.authorId)
      conditions.push(Prisma.sql`m.user_id = ${filters.authorId}`);
  ...
  async createTaskFromMessage(
    ...
    const task = await prisma.task.create({
      data: {
        tenantId,
        projectId,
        name: `From Connect: ${msg.content.slice(0, 80)}${msg.content.length > 80 ? "\u2026" : ""}`,
  ...
  async leaveMeeting(_tenantId: string, meetingId: string, userId: string) {
    ...
    return prisma.meetingParticipant.update({
      ...
      data: { leftAt: new Date(), isHandRaised: false, isScreenSharing: false },
    });
  }
  ...
  async toggleVideoSelf(_tenantId: string, meetingId: string, userId: string) {
    ...
    return prisma.meetingParticipant.update({
      where: { id: participant.id },
      data: { isVideoOn: !participant.isVideoOn },
    });
  }
  ...
  async toggleScreenShare(tenantId: string, meetingId: string, userId: string) {
    ...
    return prisma.meetingParticipant.update({
      where: { id: participant.id },
      data: { isScreenSharing: newSharing },
    });
  }
  ...
  async getMeetingParticipants(tenantId: string, meetingId: string) {
    ...
    return participants.map((p) => ({
      ...
      name: userMap.get(p.userId)
        ? `${userMap.get(p.userId)!.firstName} ${userMap.get(p.userId)!.lastName}`.trim()
        : "Unknown",
  ...
  async getMeetingChat(tenantId: string, meetingId: string) {
    ...
    return messages.map((m) => ({
      ...
      authorName: userMap.get(m.userId)
        ? `${userMap.get(m.userId)!.firstName} ${userMap.get(m.userId)!.lastName}`.trim()
        : "Unknown",
    }));
  }
  ...
  async sendMeetingChat(
    ...
    return prisma.meetingChatMessage.create({
      data: { tenantId, meetingId, userId, content: content.trim() },
    });
  }
  ...
  async startRecording(tenantId: string, meetingId: string, userId: string) {
    ...
    return prisma.meetingRecording.create({
      data: { tenantId, meetingId, startedBy: userId, status: "RECORDING" },
    });
  }
  ...
  async createPoll(
    ...
    this.notificationsGateway.broadcastChatMessage(dto.channelId, {
      type: "poll",
      poll,
      tenantId,
    });
  ...
  async closePoll(tenantId: string, pollId: string, userId: string) {
    ...
    this.notificationsGateway.broadcastChatMessage(poll.channelId, {
      type: "poll_closed",
      pollId,
      tenantId,
    });
    ...
    switch (command) {
      ...
      case "reminder": {
        const match = args.match(
          /^(?:me\s+)?(?:to\s+)?(.+?)\s+in\s+(\d+)\s*(m|min|minute|h|hr|hour|d|day)s?$/i,
        );
        if (match) {
          ...
          const ms =
            parseInt(num) *
            (unit === "m" ? 60000 : unit === "h" ? 3600000 : 86400000);
          return this.createReminder(tenantId, userId, {
            text: reminderText,
        ...
        throw new BadRequestException(
          "Usage: /remind me to <text> in <N> min/h/d",
        );
      }
      case "poll": {
        ...
        if (pollMatch) {
          ...
          const options = optionsStr
            .split(",")
            .map((o) => ({ label: o.trim() }));
          return this.createPoll(tenantId, userId, {
            channelId,
            question,
            options,
          });
      ...
      case "me":
      case "meet":
        return this.createMeeting(tenantId, userId, {
          title: args || "Quick meeting",
        });
      case "dnd": {
        const duration = parseInt(args) || 60;
        return this.setPresence(tenantId, userId, {
          presence: "DND" as Presence,
          statusText: `Do not disturb (${duration}m)`,
        });
      }
      case "status": {
        if (!args) throw new BadRequestException("Usage: /status <text>");
        return this.setPresence(tenantId, userId, {
          presence: "ACTIVE" as Presence,
          statusText: args,
        });
      }
      case "help":
      ...
      case "msg": {
        ...
        if (msgMatch) {
          ...
          const dm = await this.getOrCreateDM(
            tenantId,
            org?.id || "default",
            userId,
            targetUser.id,
  ...
  async getReminders(tenantId: string, userId: string) {
    return prisma.reminder.findMany({
      where: { tenantId, userId, isSent: false },
  ...
  async snoozeReminder(
    ...
    return prisma.reminder.update({
      where: { id: reminderId },
      data: { remindAt: new Date(Date.now() + minutes * 60000), snoozed: true },
    });
  }
  ...
  async getScheduledMessages(tenantId: string, userId: string) {
    return prisma.message.findMany({
      where: { tenantId, userId, scheduledAt: { not: null }, deletedAt: null },
  ...
  async translateMessage(
    ...
    try {
      const { OllamaClient } =
        await import("../../common/integrations/ollama-client");
      ...
      if (simpleMap[targetLang]) {
        return {
          translatedText: `[${targetLang}] ${msg.content}`,
          sourceLang: "auto",
          messageId,
          note: "AI translation unavailable — prefix-only mode",
        };
      }
      return {
  ...
  async generateMeetingSummary(tenantId: string, meetingId: string) {
    ...
    if (existing) {
      return prisma.meetingSummary.update({
        where: { id: existing.id },
        data: { summary, keyPoints, actionItems, generatedAt: new Date() },
      });
    }
    return prisma.meetingSummary.create({
      data: { tenantId, meetingId, summary, keyPoints, actionItems },
    });
  }
  ...
  async getChannelTemplates(tenantId: string) {
    return prisma.channelTemplate.findMany({
      where: { tenantId, isPreset: true },
      orderBy: { name: "asc" },
    });
  }

  async createChannelFromTemplate(
    tenantId: string,
    orgId: string,
    userId: string,
    ...
    if (template.tabs && Array.isArray(template.tabs)) {
      for (const tab of template.tabs as {
        type?: string;
        label?: string;
        url?: string;
        icon?: string;
  ...
  async sendEphemeralMessage(
    ...
    const msg = await prisma.message.create({
      data: {
        ...
        userId,
        content: dto.content,
        expiresAt: dto.expiresInSecs
          ? new Date(Date.now() + dto.expiresInSecs * 1000)
