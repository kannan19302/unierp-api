...
import {
  createLoadBuildSchema,
  createAppointmentSchema,
  createPodSchema,
} from "./logistics-execution.schemas";
...
export class LogisticsExecutionController {
  ...
  listCarrierRates(
    ...
    return this.logiSvc.getCarrierRates(req.user.tenantId, {
      carrierId,
      originZip,
      destZip,
    });
  ...
  listLoadBuilds(
    ...
    return this.logiSvc.getLoadBuilds(req.user.tenantId, {
  ...
  createLoadBuild(
    @Req() req: AuthRequest,
    @ZodBody(createLoadBuildSchema) body: z.infer<typeof createLoadBuildSchema>,
  ) {
    return this.logiSvc.createLoadBuild(
      req.user.tenantId,
      body,
      req.user.userId,
    );
  ...
  updateLoadBuildStatus(
    @Req() req: AuthRequest,
    @Param("id") id: string,
    @ZodBody(z.object({ status: z.string() })) body: { status: string },
  ) {
    return this.logiSvc.updateLoadBuildStatus(
      req.user.tenantId,
      id,
      body.status,
    );
  ...
  listAppointments(
    ...
    return this.logiSvc.getAppointments(req.user.tenantId, {
      page: page ? Number(page) : undefined,
      limit: limit ? Number(limit) : undefined,
      status,
      warehouseId,
  ...
  createAppointment(
    @Req() req: AuthRequest,
    @ZodBody(createAppointmentSchema)
    body: z.infer<typeof createAppointmentSchema>,
  ) {
    return this.logiSvc.createAppointment(
      req.user.tenantId,
      body,
      req.user.userId,
    );
  ...
  @ApiOperation({
    summary: "Update appointment status (check-in, progress, complete)",
  })
  updateAppointmentStatus(
    ...
    return this.logiSvc.updateAppointmentStatus(
      req.user.tenantId,
      id,
      body.status,
    );
  ...
  createPod(
    @Req() req: AuthRequest,
    @ZodBody(createPodSchema) body: z.infer<typeof createPodSchema>,
  ) {
    return this.logiSvc.createDeliveryConfirmation(
      req.user.tenantId,
      body,
      req.user.userId,
    );
  ...
  listPods(
    ...
    return this.logiSvc.getDeliveryConfirmations(req.user.tenantId, {
      shipmentId,
      page: page ? Number(page) : undefined,
      limit: limit ? Number(limit) : undefined,
    });
  ...
  @Permissions("supply-chain.freight.read")
  @ApiOperation({
    summary: "Rate shop across carriers based on origin/dest/weight",
  })
  rateShop(
    @Req() req: AuthRequest,
    @Query("originZip") originZip?: string,
    ...
    @Query("pallets") pallets?: string,
  ) {
    return this.logiSvc.rateShop(req.user.tenantId, {
      originZip,
      destZip,
