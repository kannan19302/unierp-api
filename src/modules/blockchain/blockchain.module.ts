...
export class BlockchainModule implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    this.outboxHandlerRegistry.register("blockchain-anchor", (event) =>
      this.blockchainOutboxHandler.handle(event),
    );

    if (!this.isEnabled) {
      this.logger.log(
        "Blockchain layer is DISABLED (BLOCKCHAIN_ENABLED != true). All blockchain operations will be no-ops.",
      );
      return;
    }
