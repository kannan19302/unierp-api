...
export class LeaseAccountingService {
  ...
  async getLeases(
    ...
    const where: Prisma.FinanceLeaseWhereInput = {
      ...
      ...(params.search
        ? {
            OR: [
              { leaseRef: { contains: params.search, mode: "insensitive" } },
  ...
  async getLeaseSummary(tenantId: string) {
    const leases = await prisma.financeLease.findMany({
      where: { tenantId, status: "ACTIVE" },
    });
    const totalROU = leases.reduce(
      (s, l) => s + Number(l.carryingAmount ?? 0),
      0,
    );
    const totalLiability = leases.reduce(
      (s, l) => s + Number(l.carryingAmount ?? 0),
      0,
    );
    const financeCount = leases.filter((l) => l.leaseType === "FINANCE").length;
    const operatingCount = leases.filter(
      (l) => l.leaseType === "OPERATING",
    ).length;
    return {
      totalROU,
      totalLiability,
  ...
  async getLeaseAnalytics(tenantId: string) {
    ...
    const now = new Date();
    for (const l of all) {
      byType[l.leaseType as keyof typeof byType] =
        (byType[l.leaseType as keyof typeof byType] ?? 0) + 1;
      byStatus[l.status] = (byStatus[l.status] ?? 0) + 1;
      const months =
        (l.endDate.getFullYear() - now.getFullYear()) * 12 +
        l.endDate.getMonth() -
        now.getMonth();
  ...
  async createLease(
    ...
    const lease = await prisma.financeLease.create({
      data: {
        ...
        presentValue:
          payload.presentValue != null
            ? new Prisma.Decimal(payload.presentValue)
            : null,
        interestRate:
          payload.interestRate != null
            ? new Prisma.Decimal(payload.interestRate)
            : null,
        initialRecognition:
          payload.presentValue != null
            ? new Prisma.Decimal(payload.presentValue)
  ...
  async updateLease(
    ...
    return prisma.financeLease.update({
      where: { id },
      data: {
        ...(payload.leaseRef !== undefined
          ? { leaseRef: payload.leaseRef }
          : {}),
        ...(payload.description !== undefined
          ? { description: payload.description }
          : {}),
        ...(payload.leaseType !== undefined
  ...
  async deleteLease(tenantId: string, id: string) {
    ...
    if (lease.status !== "INACTIVE")
      throw new BadRequestException("Only INACTIVE leases can be deleted");
  ...
  async postMonthlyEntry(tenantId: string, leaseId: string, period: string) {
    ...
    if (!schedule)
      throw new NotFoundException(
        `No unposted schedule entry found for ${period}`,
      );
          ...
          create: [
            {
              ...
              debit: new Prisma.Decimal(
                Number(schedule.principalRepayment ?? 0),
              ),
    ...
    const newCarrying = Math.max(
      0,
      Number(lease.carryingAmount ?? 0) -
        Number(schedule.principalRepayment ?? 0),
    );
    await prisma.financeLease.update({
      where: { id: leaseId },
      data: { carryingAmount: new Prisma.Decimal(newCarrying) },
    });

    this.events.emit("finance.lease.entry_posted", {
      tenantId,
      leaseId,
      period,
  ...
  async bulkPostMonth(tenantId: string, period: string) {
    ...
    for (const s of schedules) {
      try {
        const result = await this.postMonthlyEntry(
          tenantId,
          s.financeLeaseId,
          period,
        );
        results.push({
          leaseId: s.financeLeaseId,
          status: "posted",
  ...
  async terminateLease(tenantId: string, id: string, terminationDate: string) {
    ...
    return prisma.financeLease.update({
      ...
      data: {
        status: "TERMINATED",
        carryingAmount: new Prisma.Decimal(0),
        endDate: tDate,
      },
    });
  }
  ...
  async renewLease(
    ...
    const months =
      (extEnd.getFullYear() - extStart.getFullYear()) * 12 +
      (extEnd.getMonth() - extStart.getMonth()) +
      1;
    if (months <= 0)
    ...
    const pv =
      newPresentValue ??
      Number(lease.carryingAmount ?? lease.presentValue ?? 0);
    if (pv > 0) {
      const rows = computeAmortizationSchedule(
        pv,
        Number(lease.interestRate ?? 0) || null,
        extStart,
        months,
      );
      await prisma.leaseSchedule.createMany({
    ...
    return prisma.financeLease.update({
      ...
      data: {
        endDate: extEnd,
        ...(newPresentValue
          ? { presentValue: new Prisma.Decimal(newPresentValue) }
          : {}),
      },
    });
  }
