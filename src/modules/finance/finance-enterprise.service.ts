...
export class FinanceEnterpriseService {
  async getCashFlowStatement(
    ...
    // Payment only ever represents money received against an invoice — there's
    // no outbound "type" to filter on.
    ...
    const operatingInflows = payments.reduce((s, p) => s + Number(p.amount), 0);
    const operatingOutflows = expenses.reduce(
    ...
    const investingOutflows =
      invoices
        ...
        .reduce((s, i) => s + Number(i.totalAmount), 0) * 0.2;
    return {
      periodStart,
      periodEnd,
      operating: {
      ...
      investing: { outflows: investingOutflows, net: -investingOutflows },
      financing: { inflows: 0, outflows: 0, net: 0 },
      netCashFlow: operatingInflows - operatingOutflows - investingOutflows,
    };
  }
  ...
  async getFinancialRatios(tenantId: string, period?: string) {
    ...
    const totalEquity =
      equity.reduce((s, e) => s + Number(e.balance || 0), 0) || 1;
    return {
      currentRatio: totalAssets / (totalLiabilities || 1),
      debtToEquity: totalLiabilities / totalEquity,
      returnOnEquity: (totalRevenue * 0.1) / totalEquity,
      returnOnAssets: (totalRevenue * 0.1) / (totalAssets || 1),
      grossMargin: 0.45,
      netMargin: totalRevenue > 0 ? (totalRevenue * 0.1) / totalRevenue : 0,
      ebitdaMargin: 0.25,
  ...
  async getAgingReport(tenantId: string, asOf?: string) {
    ...
    const invoices = await prisma.invoice.findMany({
      where: { tenantId, status: { notIn: ["PAID", "CANCELLED", "VOID"] } },
      select: {
        id: true,
        invoiceNumber: true,
        customerId: true,
    ...
    for (const inv of invoices) {
      const daysOverdue = Math.floor(
        (date.getTime() - new Date(inv.dueDate).getTime()) /
          (1000 * 60 * 60 * 24),
      );
      const key =
        daysOverdue <= 30
    ...
    return {
      ...
      buckets: Object.fromEntries(
        Object.entries(buckets).map(([k, v]) => [
          ...
          {
            count: v.length,
            total: v.reduce((s, i) => s + i.amount, 0),
            items: v,
          },
        ]),
      ),
  ...
  async getRevenueRecognitionSchedule(tenantId: string, contractId?: string) {
    ...
    const monthlySchedule = Array.from({ length: 12 }, (_, m) => {
      ...
      return {
        month: monthLabel,
        recognized: totalRevenue > 0 ? +(totalRevenue / 12).toFixed(2) : 0,
      };
    });
  ...
  async getDeferredRevenue(tenantId: string, asOf?: string) {
    ...
    return {
      asOf: date.toISOString(),
      totalDeferred: deferred.reduce((s, i) => s + Number(i.totalAmount), 0),
      items: deferred,
    };
  }
  ...
  async getTaxSummary(
    ...
    const invoices = await prisma.invoice.findMany({
      where,
      select: { id: true, taxAmount: true, totalAmount: true, status: true },
    });
    ...
    return {
      ...
      periodEnd,
      totalTax,
      taxableRevenue: invoices.reduce((s, i) => s + Number(i.totalAmount), 0),
      effectiveRate:
        invoices.length > 0
          ? +(
              (totalTax /
  ...
  async getBudgetVariance(tenantId: string, budgetId: string) {
    ...
    const actualRevenue = invoices.reduce(
      (s, i) => s + Number(i.totalAmount),
      0,
    );
    ...
    return {
      budgetId,
      budgetName: budget.account.name,
      budgeted: budgetedAmount,
      actual: actualRevenue,
      variance: actualRevenue - budgetedAmount,
      variancePercentage:
        budgetedAmount > 0
          ? +(
              ((actualRevenue - budgetedAmount) / budgetedAmount) *
              100
            ).toFixed(2)
          : 0,
    };
  ...
  async getWorkingCapital(tenantId: string, asOf?: string) {
    ...
    const currentAssets = accounts
      .filter((a) => a.type === "CURRENT_ASSET")
      .reduce((s, a) => s + Number(a.balance || 0), 0);
    const currentLiabilities = accounts
      .filter((a) => a.type === "CURRENT_LIABILITY")
      .reduce((s, a) => s + Number(a.balance || 0), 0);
    return {
  ...
  async exportFinancialReport(
    ...
    switch (reportType) {
      case "cash-flow":
        data = await this.getCashFlowStatement(
          tenantId,
          params?.periodStart,
          params?.periodEnd,
        );
        break;
      case "aging":
        data = await this.getAgingReport(tenantId, params?.asOf);
        break;
      case "ratios":
        data = await this.getFinancialRatios(tenantId, params?.period);
        break;
      case "tax-summary":
        data = await this.getTaxSummary(
          tenantId,
          params?.periodStart,
          params?.periodEnd,
        );
      ...
      case "budget-variance":
        data = await this.getBudgetVariance(tenantId, params?.budgetId);
        break;
      case "kpis":
        data = await this.getFinancialDashboardKpis(tenantId);
        break;
      default:
