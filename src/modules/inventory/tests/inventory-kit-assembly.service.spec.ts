...
const kit = {
  ...
  product: {
    name: "Starter Bundle Product",
    costPrice: { toString: () => "40" },
  },
  components: [
    {
      productId: "comp-1",
      quantity: { toString: () => "2" },
      product: { name: "Component A", costPrice: { toString: () => "10" } },
    },
    {
      productId: "comp-2",
      quantity: { toString: () => "1" },
      product: { name: "Component B", costPrice: { toString: () => "5" } },
    },
  ],
};
...
describe("InventoryService — kit assembly/disassembly", () => {
  ...
  it("rejects assembling more kits than components allow", async () => {
    vi.spyOn(service, "getKitAvailability").mockResolvedValue({
      maxBuildable: 2,
    } as any);
    await expect(
      service.assembleKit("t1", "org-1", "u1", "kit-1", {
        warehouseId: "w1",
        quantity: 5,
      }),
  ...
  it("assembles kits by creating and submitting a STOCK_ADJUSTMENT entry", async () => {
    ...
    const createSpy = vi
      .spyOn(service, "createStockEntry")
      .mockResolvedValue({ id: "se-1" } as any);
    const submitSpy = vi
      .spyOn(service, "submitStockEntry")
      .mockResolvedValue({ id: "se-1", status: "SUBMITTED" } as any);

    const result = await service.assembleKit("t1", "org-1", "u1", "kit-1", {
      warehouseId: "w1",
      quantity: 2,
    });
    ...
    expect(createSpy).toHaveBeenCalledWith(
      ...
      "org-1",
      "u1",
      expect.objectContaining({ type: "STOCK_ADJUSTMENT" }),
    );
  ...
  it("rejects disassembling more kits than are on hand", async () => {
    ...
    await expect(
      service.disassembleKit("t1", "org-1", "u1", "kit-1", {
        warehouseId: "w1",
        quantity: 5,
      }),
    ).rejects.toThrow(BadRequestException);
  ...
  it("disassembles kits by creating and submitting a STOCK_ADJUSTMENT entry", async () => {
    ...
    const createSpy = vi
      .spyOn(service, "createStockEntry")
      .mockResolvedValue({ id: "se-2" } as any);
    const submitSpy = vi
      .spyOn(service, "submitStockEntry")
      .mockResolvedValue({ id: "se-2", status: "SUBMITTED" } as any);

    const result = await service.disassembleKit("t1", "org-1", "u1", "kit-1", {
      warehouseId: "w1",
      quantity: 2,
    });
