...
describe("LotSerialTrackingService", () => {
  ...
  describe("quarantineBatch", () => {
    ...
    it("throws if batch is already in quarantine", async () => {
      ...
      await expect(svc.quarantineBatch(TENANT, "b1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("releaseBatchFromQuarantine", () => {
    ...
    it("throws if batch is not in quarantine", async () => {
      ...
      await expect(
        svc.releaseBatchFromQuarantine(TENANT, "b1"),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("createSerial", () => {
    it("creates a serial when serialNo is unique", async () => {
      ...
      const result = await svc.createSerial(TENANT, {
        productId: "p1",
        warehouseId: "wh1",
        serialNo: "SN-001",
      });
  ...
  describe("markSerialSold", () => {
    ...
    it("throws if serial is neither AVAILABLE nor RESERVED", async () => {
      ...
      await expect(svc.markSerialSold(TENANT, "s1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("markSerialReturned", () => {
    it("marks a SOLD serial as RETURNED", async () => {
      ...
      vi.mocked(prisma.$transaction).mockResolvedValue([
        { id: "s1", status: "RETURNED" },
        {},
      ] as any);
      const result = await svc.markSerialReturned(
        TENANT,
        "s1",
        "Customer return",
      );
    ...
    it("throws if serial is not SOLD", async () => {
      ...
      await expect(svc.markSerialReturned(TENANT, "s1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("generatePickSuggestions", () => {
    it("generates FEFO suggestions from active batches", async () => {
      ...
      vi.mocked(prisma.pickSuggestion.create).mockResolvedValue({
        id: "ps1",
        batchId: "b1",
      } as any);
  ...
  describe("confirmPickSuggestion", () => {
    it("confirms a PENDING suggestion with valid qty", async () => {
      ...
      vi.mocked(prisma.pickSuggestion.findFirst).mockResolvedValue({
        id: "ps1",
        status: "PENDING",
        suggestedQty: { toString: () => "30" },
      } as any);
      vi.mocked(prisma.pickSuggestion.update).mockResolvedValue({
        id: "ps1",
        status: "CONFIRMED",
      } as any);
      const result = await svc.confirmPickSuggestion(TENANT, "ps1", {
        pickedQty: 25,
      });
    ...
    it("throws if picked qty exceeds suggested qty", async () => {
      ...
      vi.mocked(prisma.pickSuggestion.findFirst).mockResolvedValue({
        id: "ps1",
        status: "PENDING",
        suggestedQty: { toString: () => "10" },
      } as any);
      await expect(
        svc.confirmPickSuggestion(TENANT, "ps1", { pickedQty: 20 }),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("generateExpiryAlerts", () => {
    it("generates alerts for batches expiring within threshold", async () => {
      ...
      vi.mocked(prisma.expiryAlert.create).mockResolvedValue({
        id: "a1",
        severity: "CRITICAL",
      } as any);
      const result = await svc.generateExpiryAlerts(TENANT, {
        thresholdDays: 30,
      });
  ...
  describe("acknowledgeExpiryAlert", () => {
    it("acknowledges an alert", async () => {
      ...
      vi.mocked(prisma.expiryAlert.update).mockResolvedValue({
        id: "a1",
        acknowledged: true,
      } as any);
  ...
  describe("createQuarantineOrder", () => {
    it("creates a quarantine order with auto-number and quarantines batch", async () => {
      ...
      expect(prisma.batch.update).toHaveBeenCalledWith(
        expect.objectContaining({ data: { status: "QUARANTINE" } }),
      );
    });
  });
  ...
  describe("releaseQuarantineOrder", () => {
    it("releases an ACTIVE quarantine order and restores batch", async () => {
      ...
      vi.mocked(prisma.quarantineOrder.update).mockResolvedValue({
        id: "qo1",
        status: "RELEASED",
      } as any);
      vi.mocked(prisma.batch.update).mockResolvedValue({} as any);
      const result = await svc.releaseQuarantineOrder(TENANT, "qo1", USER, {
        releaseNotes: "Passed QC",
      });
    ...
    it("throws if order is not ACTIVE", async () => {
      ...
      await expect(
        svc.releaseQuarantineOrder(TENANT, "qo1", USER, { releaseNotes: "x" }),
      ).rejects.toThrow(BadRequestException);
    });
  });
