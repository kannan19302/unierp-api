...
export class SalesCpqService {
      ...
      options: [
        {
          ...
          items: [
            ...
            },
            {
              ...
              price: Number(product.sellPrice || 0) * 0.2,
              selected: false,
            },
            {
              id: "opt-enterprise",
              name: "Enterprise Support (dedicated CSM)",
              price: Number(product.sellPrice || 0) * 0.4,
              selected: false,
            },
          ],
        },
  ...
  async calculateDynamicPrice(
    ...
    if (data.quantity >= 100) {
      ...
      appliedRules.push({
        rule: "Volume Discount (100+ units, 15% off)",
        impact: -discount,
        type: "VOLUME",
      });
      ...
      adjustedPrice -= discount;
      appliedRules.push({
        rule: "Volume Discount (50+ units, 10% off)",
        impact: -discount,
        type: "VOLUME",
      });
      ...
      adjustedPrice -= discount;
      appliedRules.push({
        rule: "Volume Discount (10+ units, 5% off)",
        impact: -discount,
        type: "VOLUME",
      });
    }
    ...
    if (data.priceBookId) {
      ...
      if (entry) {
        ...
        if (pbPrice < adjustedPrice) {
          appliedRules.push({
            rule: `Price Book Override`,
            impact: pbPrice - adjustedPrice,
            type: "PRICE_BOOK",
          });
    ...
    if (data.customerId) {
      ...
      if (orderCount >= 10) {
        ...
        appliedRules.push({
          rule: "Loyalty Discount (10+ previous orders, 5% off)",
          impact: -loyalty,
          type: "LOYALTY",
        });
      }
    ...
    const discountPct =
      basePrice > 0
        ? Math.round(((basePrice - adjustedPrice) / basePrice) * 100)
        : 0;
  ...
  async calculateSubscriptionPrice(
    ...
    let termDiscount = 0;
    if (data.termMonths >= 36) {
      termDiscount = 10;
      monthlyPrice *= 0.9;
    } else if (data.termMonths >= 24) {
      termDiscount = 5;
      monthlyPrice *= 0.95;
    }
    ...
    const periodMonths =
      data.billingFrequency === "ANNUAL"
        ? 12
        : data.billingFrequency === "QUARTERLY"
          ? 3
          : 1;
  ...
  async getQuoteMarginAnalysis(
    ...
    riskItems: Array<{ item: string; reason: string }>;
  }> {
    ...
    const productIds = quotation.lineItems
      .map((li) => li.productId)
    ...
    const lineItems = quotation.lineItems.map((li) => {
      ...
      const unitCost = li.productId
        ? (productCostMap.get(li.productId) ?? sellPrice * 0.6)
        : sellPrice * 0.6;
      const costPrice = unitCost * Number(li.quantity);
      const margin = sellPrice - costPrice;
      const marginPct =
        sellPrice > 0 ? Math.round((margin / sellPrice) * 100) : 0;
      ...
      if (marginPct < 20) {
        riskItems.push({
          item: li.description || "Line item",
          reason: `Low margin: ${marginPct}%`,
        });
      }
    ...
    const overallMarginPct =
      totalSell > 0 ? Math.round((overallMargin / totalSell) * 100) : 0;
  ...
  async calculateTax(
    ...
    return {
      ...
      taxBreakdown: [
        {
          jurisdiction: `${data.shipToState} State Tax`,
          rate: stateTaxRate,
          amount: Math.round(subtotal * (stateTaxRate / 100) * 100) / 100,
        },
        {
          jurisdiction: "Local Tax",
          rate: localTaxRate,
          amount: Math.round(subtotal * (localTaxRate / 100) * 100) / 100,
        },
      ],
    };
  }
    ...
    return [
      {
        ...
        service: "Ground",
        estimatedDays: 5,
        cost: Math.round(billableWeight * 0.45 * 100) / 100,
      ...
      },
      {
        ...
        service: "Express",
        estimatedDays: 2,
        cost: Math.round(billableWeight * 1.2 * 100) / 100,
      ...
      },
      {
        ...
        service: "Ground",
        estimatedDays: 5,
        cost: Math.round(billableWeight * 0.48 * 100) / 100,
        guaranteedDelivery: false,
      },
      {
        carrier: "UPS",
        service: "2nd Day Air",
        estimatedDays: 2,
        cost: Math.round(billableWeight * 1.15 * 100) / 100,
        guaranteedDelivery: true,
      },
      {
        carrier: "USPS",
        ...
        estimatedDays: 3,
        cost: Math.round(billableWeight * 0.55 * 100) / 100,
        guaranteedDelivery: false,
      },
  ...
  async getQuoteProfitabilityDashboard(tenantId: string): Promise<{
    ...
    bottomProfitableProducts: Array<{
      productName: string;
      avgMargin: number;
      quoteCount: number;
    }>;
    ...
    for (const q of quotations) {
      ...
      const margin =
        totalSell > 0
          ? Math.round(((totalSell - estimatedCost) / totalSell) * 100)
          : 0;
    ...
    const avgMargin =
      quotations.length > 0
        ? Math.round(totalMarginSum / quotations.length)
        : 0;
