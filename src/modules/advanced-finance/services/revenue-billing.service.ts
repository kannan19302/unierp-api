...
export class RevenueBillingService {
  ...
  async getBillingRule(tenantId: string, id: string) {
    const r = await prisma.billingRule.findFirst({
      where: { id, tenantId },
      include: { milestones: true },
    });
  ...
  async completeMilestone(tenantId: string, id: string) {
    const m = await prisma.billingMilestone.findFirst({
      where: { id, tenantId },
    });
  ...
  async triggerMilestoneInvoice(tenantId: string, id: string) {
    ...
    if (m.status !== "COMPLETED")
      throw new BadRequestException(
        "Milestone must be COMPLETED before invoicing",
      );
    if (!m.billingRule.customerId)
      throw new BadRequestException("Billing rule has no customer");
    // Create invoice
  ...
  async approveContractModification(tenantId: string, id: string) {
    const mod = await prisma.contractModification.findFirst({
      where: { id, tenantId },
    });
    if (!mod) throw new NotFoundException("Contract modification not found");
    return prisma.contractModification.update({
      where: { id },
  ...
  private calculateRecognizedAsOf(
    s: {
      totalAmount: unknown;
      startDate: Date;
      endDate: Date;
  ...
  async listDeferredRevenueRollForwards(tenantId: string) {
    return prisma.deferredRevenueRollForward.findMany({
      where: { tenantId },
      orderBy: { period: "desc" },
    });
  }
  ...
  async listTieredPricingTables(tenantId: string) {
    return prisma.tieredPricingTable.findMany({
      where: { tenantId, isActive: true },
    });
  }
  ...
  async createTieredPricingTable(
    ...
    dto: {
      ...
      currency?: string;
      unit?: string;
      tiers: { from: number; to?: number; ratePerUnit: number }[];
    },
  ) {
    return prisma.tieredPricingTable.create({
      data: { tenantId, ...dto, tiers: dto.tiers as never },
    });
  }

  async rateUsage(tenantId: string, tableId: string, usage: number) {
    const table = await prisma.tieredPricingTable.findFirst({
      where: { id: tableId, tenantId },
    });
    if (!table) throw new NotFoundException("Pricing table not found");
    const tiers = table.tiers as {
      from: number;
      to?: number;
      ratePerUnit: number;
    }[];
  ...
  async getRevenueBacklog(tenantId: string) {
    ...
    return {
      ...
      totalRemainingObligation: backlog.reduce(
        (s, b) => s + b.remainingObligation,
        0,
      ),
      contracts: backlog,
    };
        ...
        if (scheduleStart <= end && scheduleEnd >= start) {
          const totalMonths = Math.max(
            ...
            (scheduleEnd.getFullYear() - scheduleStart.getFullYear()) * 12 +
              (scheduleEnd.getMonth() - scheduleStart.getMonth()) +
              1,
          );
