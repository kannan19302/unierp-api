    ...
    return [
          ...
          parameters: {
            ...
            properties: {
              question: {
                type: "string",
                description:
                  "The natural-language business question to answer.",
              },
            },
            required: ["question"],
          },
      ...
      },
      {
        ...
        function: {
          ...
          description:
            "Summarize a specific ERP record (e.g. an invoice, purchase order, customer) given its entity type and ID.",
          parameters: {
            type: "object",
            properties: {
              entityType: {
                type: "string",
                description:
                  'The entity/model name, e.g. "Invoice", "PurchaseOrder".',
              },
              entityId: {
                type: "string",
                description: "The record ID to summarize.",
              },
            },
      ...
      },
      {
        ...
        function: {
          name: "draft_email",
          description:
            "Draft a professional business email to a recipient about a given subject.",
          parameters: {
            type: "object",
            properties: {
              to: {
                type: "string",
                description: "The email recipient (name or address).",
              },
              regarding: {
                type: "string",
              ...
              tone: {
                type: "string",
                description:
                  'Optional tone, e.g. "professional", "friendly", "formal".',
              },
            },
      ...
      },
      {
        ...
        function: {
          name: "generate_form",
          description:
            "Generate a data-collection form definition for the ERP Builder from a plain-language description.",
          parameters: {
            type: "object",
            properties: {
              prompt: {
                type: "string",
                description: "Description of the form to generate.",
              },
            },
            required: ["prompt"],
          },
      ...
      },
      {
        ...
        function: {
          ...
          description:
            "Generate an approval workflow definition for the ERP system from a plain-language description.",
          parameters: {
            type: "object",
            properties: {
              prompt: {
      ...
      },
      {
        ...
        function: {
          ...
          description:
            "Extract structured fields (vendor, amount, dates, line items, etc.) from raw invoice document text, optionally creating a draft purchase order.",
          parameters: {
            type: "object",
            properties: {
              documentText: {
                type: "string",
                description: "The raw text content of the invoice document.",
              },
              createDraft: {
                type: "boolean",
                description:
                  "Whether to create a draft purchase order from the extracted data.",
              },
            },
  ...
  private async executeTool(
    ...
    switch (name) {
      ...
      case "draft_email": {
        ...
        const regarding =
          typeof args.regarding === "string" ? args.regarding : "";
      ...
      case "process_invoice_text": {
        const documentText =
          typeof args.documentText === "string" ? args.documentText : "";
        const createDraft =
          typeof args.createDraft === "boolean" ? args.createDraft : false;
  ...
  async converse(
    ...
    while (iterations < MAX_TOOL_LOOP_ITERATIONS) {
      ...
      try {
        ...
        return {
          reply: `Sorry, the AI assistant is unavailable right now. (${message})`,
          actions,
        };
      }
      ...
      for (const call of toolCalls) {
        try {
          ...
          messages = [
            ...messages,
            { role: "tool", content: JSON.stringify(result ?? null) },
          ];
        } catch (err) {
          const message = err instanceof Error ? err.message : String(err);
          messages = [
            ...messages,
            { role: "tool", content: JSON.stringify({ error: message }) },
          ];
        }
      }
    }
    ...
    return {
      reply:
        fallbackText ||
        "I was unable to finish processing that request within the allotted steps. Please try rephrasing or breaking it into smaller requests.",
      actions,
    };
  }
}
