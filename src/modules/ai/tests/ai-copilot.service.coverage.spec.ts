...
describe("AiCopilotService coverage", () => {
  ...
  it("draftEmail returns a parsed subject/body on success", async () => {
    chatSpy.mockResolvedValue({
      ...
      content: JSON.stringify({
        subject: "Re: PO",
        body: "Hello, following up on the PO.",
      }),
    });

    const result = await service.draftEmail("t1", {
      to: "test@t.com",
      regarding: "PO",
    });
  ...
  it("generateFormFromPrompt returns a parsed form definition on success", async () => {
    ...
    const result = await service.generateFormFromPrompt(
      "t1",
  ...
  it("generateWorkflowFromPrompt returns a parsed workflow definition on success", async () => {
    ...
    const result = await service.generateWorkflowFromPrompt(
      "t1",
      "approval workflow",
    );
  ...
  it("propagates a friendly error when the underlying AI call fails (e.g. Ollama unreachable)", async () => {
    chatSpy.mockRejectedValue(
      new Error("Could not reach the local Ollama server"),
    );

    await expect(
      service.draftEmail("t1", { to: "test@t.com", regarding: "PO" }),
    ).rejects.toThrow("Could not reach the local Ollama server");
  });
});
