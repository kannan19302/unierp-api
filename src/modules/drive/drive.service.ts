...
export class DriveService {
  ...
  async createFolder(
    tenantId: string,
    userId: string,
    dto: CreateDriveFolderInput,
  ) {
    return prisma.driveFolder.create({
      data: { tenantId, ownerId: userId, ...dto },
    });
  }
  ...
  async createFile(
    ...
    const file = await prisma.driveFile.create({
      data: { tenantId, ownerId: userId, ...dto, currentVersion: 1 } as any,
    });
  ...
  async updateFile(
    ...
    return prisma.driveFile.update({ where: { id }, data: dto as any });
  }
  ...
  async createFileVersion(
    ...
    fileId: string,
    userId: string,
    dto: { size: number; storagePath: string; notes?: string },
  ) {
    const file = await prisma.driveFile.findFirst({
  ...
  async getComments(tenantId: string, fileId: string) {
    return prisma.driveFileComment.findMany({
      where: { tenantId, fileId, parentId: null },
  ...
  async createComment(
    tenantId: string,
    userId: string,
    dto: CreateDriveCommentInput,
  ) {
    const file = await prisma.driveFile.findFirst({
      where: { id: dto.fileId, tenantId },
    });
    if (!file) throw new NotFoundException("File not found");
    return prisma.driveFileComment.create({
      data: {
        tenantId,
        fileId: dto.fileId,
        userId,
  ...
  async getStorageQuota(tenantId: string) {
    let quota = await prisma.driveStorageQuota.findUnique({
      where: { tenantId },
    });
    if (!quota) {
  ...
  async getRecentActivity(tenantId: string) {
    return prisma.driveActivity.findMany({
      where: { tenantId },
      orderBy: { createdAt: "desc" },
      take: 50,
    });
