...
export class CrmGuidedSellingService {
  ...
  async listActionConfigs(
    tenantId: string,
    objectType?: string,
    stageId?: string,
  ) {
    const where: Prisma.CrmNextBestActionConfigWhereInput = {
      tenantId,
      deletedAt: null,
    };
    if (objectType) where.objectType = objectType;
    if (stageId) where.stageId = stageId;
    return prisma.crmNextBestActionConfig.findMany({
      where,
  ...
  async getActionConfig(tenantId: string, id: string) {
    const c = await prisma.crmNextBestActionConfig.findFirst({
  ...
  async createActionConfig(
    tenantId: string,
    dto: z.infer<typeof nextBestActionConfigSchema>,
  ) {
    const { conditions, ...rest } = dto;
    return prisma.crmNextBestActionConfig.create({
      data: {
        ...rest,
  ...
  async updateActionConfig(
    tenantId: string,
    id: string,
    dto: Partial<z.infer<typeof nextBestActionConfigSchema>>,
  ) {
    await this.getActionConfig(tenantId, id);
    const { conditions, ...rest } = dto;
    return prisma.crmNextBestActionConfig.update({
      where: { id },
      data: {
        ...rest,
  ...
  async generateSuggestions(
    ...
    for (const cfg of configs) {
      const sug = await prisma.crmActionSuggestion.create({
        data: {
          ...
          description: cfg.actionDescription || undefined,
          priority:
            cfg.priority > 5 ? "HIGH" : cfg.priority > 2 ? "MEDIUM" : "LOW",
          reason: `Based on ${cfg.name} configuration`,
        },
      });
    ...
    if (!suggestions.length) {
      suggestions.push(
        await prisma.crmActionSuggestion.create({
          data: {
            tenantId,
            objectId,
            objectType,
            ...
            description:
              "No specific action configured — schedule a general follow-up",
            priority: "MEDIUM",
  ...
  async listSuggestions(tenantId: string, objectId?: string, status?: string) {
    ...
    return prisma.crmActionSuggestion.findMany({
      where,
  ...
  async getSuggestion(tenantId: string, id: string) {
    const s = await prisma.crmActionSuggestion.findFirst({
      where: { id, tenantId },
    });
  ...
  async acceptSuggestion(tenantId: string, id: string) {
    ...
    if (s.status !== "SUGGESTED")
      throw new BadRequestException("Suggestion is not in SUGGESTED status");
    return prisma.crmActionSuggestion.update({
      where: { id },
  ...
  async completeSuggestion(tenantId: string, id: string) {
    ...
    if (s.status !== "ACCEPTED")
      throw new BadRequestException(
        "Only accepted suggestions can be completed",
      );
    return prisma.crmActionSuggestion.update({
      where: { id },
  ...
  async listPlaybooks(tenantId: string, objectType?: string) {
    const where: Prisma.CrmGuidedSellingPlaybookWhereInput = {
      tenantId,
      deletedAt: null,
    };
    if (objectType) where.objectType = objectType;
    return prisma.crmGuidedSellingPlaybook.findMany({
      where,
  ...
  async getPlaybook(tenantId: string, id: string) {
    const p = await prisma.crmGuidedSellingPlaybook.findFirst({
      where: { id, tenantId, deletedAt: null },
    });
  ...
  async createPlaybook(
    tenantId: string,
    dto: z.infer<typeof guidedSellingPlaybookSchema>,
  ) {
    const { content, tags, ...rest } = dto;
    return prisma.crmGuidedSellingPlaybook.create({
      data: {
        ...rest,
  ...
  async updatePlaybook(
    tenantId: string,
    id: string,
    dto: Partial<z.infer<typeof guidedSellingPlaybookSchema>>,
  ) {
    await this.getPlaybook(tenantId, id);
    const { content, tags, ...rest } = dto;
    return prisma.crmGuidedSellingPlaybook.update({
      where: { id },
      data: {
        ...rest,
        ...(content !== undefined
  ...
  async recommendPlaybook(
    ...
    const existing = await prisma.crmActionSuggestion.findFirst({
      where: {
        ...
        objectId,
        objectType,
        title: `Playbook: ${(await this.getPlaybook(tenantId, playbookId)).name}`,
        status: "SUGGESTED",
      },
    });
    ...
    return prisma.crmActionSuggestion.create({
      data: {
        ...
        actionType: "REVIEW",
        title: `Playbook: ${(await this.getPlaybook(tenantId, playbookId)).name}`,
        description: `Recommended playbook for ${objectType}`,
        priority: "HIGH",
        reason: "Playbook recommendation",
      },
    });
  }
  ...
  async scoreDealReadiness(tenantId: string, opportunityId: string) {
    ...
    const factors = Object.entries(dimensions).map(([name, score]) => ({
      name,
      score,
      weight: 20,
    }));
    ...
    if (totalScore < 40)
      recommendation = "Significant risk — consider qualification";
    else if (totalScore < 65)
      recommendation = "Moderate risk — focus on identified gaps";
  ...
  async getDealReadiness(tenantId: string, opportunityId: string) {
    await prisma.opportunity
      .findFirstOrThrow({ where: { id: opportunityId, tenantId } })
      .catch(() => {
        throw new NotFoundException("Opportunity not found");
      });
    return prisma.crmDealReadinessScore.findFirst({
  ...
  async getDealReadinessHistory(tenantId: string, opportunityId: string) {
    ...
    return prisma.crmDealReadinessScore.findMany({
      where: { tenantId, opportunityId },
      orderBy: { calculatedAt: "desc" },
      take: 20,
    });
  ...
  async getSuggestionAnalytics(tenantId: string) {
    const [total, accepted, dismissed, completed, expired] = await Promise.all([
      ...
      prisma.crmActionSuggestion.count({
    ...
    const byType = await prisma.crmActionSuggestion.groupBy({
      by: ["actionType"],
      where: { tenantId },
      _count: true,
    });
  ...
  async getDashboard(tenantId: string) {
    const [actionConfigs, playbooks, pendingSuggestions, avgScore] =
      await Promise.all([
        prisma.crmNextBestActionConfig.count({
          where: { tenantId, isActive: true, deletedAt: null },
        }),
        prisma.crmGuidedSellingPlaybook.count({
          where: { tenantId, isActive: true, deletedAt: null },
        }),
        prisma.crmActionSuggestion.count({
          where: { tenantId, status: "SUGGESTED" },
        }),
        prisma.crmDealReadinessScore.aggregate({
          where: { tenantId },
          _avg: { score: true },
        }),
      ]);
    return {
      ...
      playbooks,
      pendingSuggestions,
      avgDealReadiness: Math.round(avgScore._avg.score || 0),
    };
  }
}
