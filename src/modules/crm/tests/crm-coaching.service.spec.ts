...
describe("CrmCoachingService", () => {
  ...
  it("creates a rubric with weighted criteria", async () => {
    (
      prisma.coachingRubric.create as ReturnType<typeof vi.fn>
    ).mockImplementation(({ data }: never) =>
      Promise.resolve({ id: "rub1", ...data }),
    );
  ...
  it("rejects a scorecard referencing an unknown criterion key", async () => {
    ...
    (
      prisma.coachingRubric.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({
      id: "rub1",
      tenantId: TENANT,
      criteria: [
    ...
    await expect(
      service.createScorecard(TENANT, ORG, REVIEWER, {
        ...
        rubricId: "rub1",
        criteriaScores: [
          { key: "unknown_key", label: "x", score: 5, maxScore: 10 },
        ],
      }),
  ...
  it("rejects a scorecard where score exceeds maxScore", async () => {
    ...
    await expect(
      service.createScorecard(TENANT, ORG, REVIEWER, {
        ...
        rubricId: "rub1",
        criteriaScores: [
          { key: "talk_ratio", label: "x", score: 15, maxScore: 10 },
        ],
      }),
  ...
  it("creates a valid scorecard and computes total/max score", async () => {
    ...
    (
      prisma.callScorecard.create as ReturnType<typeof vi.fn>
    ).mockImplementation(({ data }: never) =>
      Promise.resolve({ id: "sc1", ...data }),
    );
  ...
  it("acknowledges a scorecard, setting status and timestamp", async () => {
    (
      prisma.callScorecard.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({
      id: "sc1",
      tenantId: TENANT,
      status: "SUBMITTED",
    ...
    (
      prisma.callScorecard.update as ReturnType<typeof vi.fn>
    ).mockImplementation(({ data }: never) =>
      Promise.resolve({ id: "sc1", ...data }),
    );
  ...
  it("adds a coaching library item with tags", async () => {
    (
      prisma.coachingLibraryItem.create as ReturnType<typeof vi.fn>
    ).mockImplementation(({ data }: never) =>
      Promise.resolve({ id: "lib1", ...data }),
    );
