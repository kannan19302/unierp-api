...
export class CrmSalesOpsService {
  ...
  async createSalesTarget(
    ...
    return prisma.salesTarget.create({
      data: {
        ...
        period: dto.period,
        targetType: dto.targetType || "REVENUE",
  ...
  async addTeamMember(
    ...
    const territory = await prisma.salesTerritory.findFirst({
      where: { id: territoryId, tenantId, deletedAt: null },
    });
  ...
  async getTerritoryPerformance(tenantId: string) {
    ...
    for (const t of territories) {
      ...
      results.push({
        ...
        revenue: wonOpps.reduce((s, o) => s + Number(o.amount || 0), 0),
      });
    }
