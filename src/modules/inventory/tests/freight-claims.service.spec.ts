...
describe("FreightClaimsService", () => {
  ...
  it("createDamageReport auto-numbers CDR-XXXXXX", async () => {
    ...
    expect(prisma.cargoDamageReport.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ reportNumber: "CDR-000004" }),
      }),
    );
  ...
  it("submitDamageReport transitions DRAFT → SUBMITTED", async () => {
    ...
    vi.mocked(prisma.cargoDamageReport.update).mockResolvedValue({
      id: "dr1",
      status: "SUBMITTED",
    } as any);
  ...
  it("submitDamageReport throws BadRequestException for non-DRAFT", async () => {
    ...
    await expect(svc.submitDamageReport(T, "dr1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("fileClaim auto-numbers FC-XXXXXX and marks report CLAIM_FILED", async () => {
    ...
    vi.mocked(prisma.freightClaim.create).mockResolvedValue({
      id: "fc1",
      claimNumber: "FC-000001",
    } as any);
    ...
    expect(prisma.freightClaim.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ claimNumber: "FC-000001" }),
      }),
    );
    expect(prisma.cargoDamageReport.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: "CLAIM_FILED" }),
      }),
    );
  ...
  it("fileClaim throws when duplicate claim exists", async () => {
    ...
    vi.mocked(prisma.freightClaim.findUnique).mockResolvedValue({
      id: "fc-existing",
    } as any);
    await expect(
      svc.fileClaim(T, "u1", {
  ...
  it("updateClaimStatus transitions DRAFT → FILED", async () => {
    ...
    vi.mocked(prisma.freightClaim.update).mockResolvedValue({
      id: "fc1",
      status: "FILED",
    } as any);
    const result = await svc.updateClaimStatus(T, "fc1", "u1", {
      status: "FILED",
    });
  ...
  it("updateClaimStatus throws for invalid transition", async () => {
    ...
    await expect(
      svc.updateClaimStatus(T, "fc1", "u1", { status: "ACCEPTED" }),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("addClaimEvent creates event record", async () => {
    ...
    const result = await svc.addClaimEvent(
      T,
      "fc1",
      "u1",
    ...
    expect(prisma.freightClaimEvent.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ claimId: "fc1", eventType: "NOTE" }),
      }),
    );
