...
export class BuilderDevOpsService {
  ...
  async getGitDiff(_tenantId: string) {
    ...
    return [
      ...
      {
        file: "apps/api/src/modules/builder/builder.controller.ts",
        status: "modified",
        additions: 80,
        deletions: 0,
      },
  ...
  async executeGitCommit(tenantId: string, message: string) {
    ...
    return {
      success: true,
      message: "Changes committed and pushed to remote branch.",
  ...
  async triggerNativeBuild(
    ...
    setTimeout(async () => {
      try {
        ...
        this.logger.error(
          `Async build ${build.id} failed to finalize`,
  ...
  async getNativeBuildLogs(tenantId: string, id: string) {
    const build = await prisma.nativeBuild.findFirst({
      where: { id, tenantId },
    });
    return {
      id: build?.id,
      status: build?.status,
      logs: build?.logSummary || "No build logs available.",
    };
  }
}
