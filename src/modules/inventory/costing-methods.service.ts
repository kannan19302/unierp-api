...
export class CostingMethodsService {
    ...
    if (existing) {
      return prisma.inventoryCostProfile.update({
        ...
        data: {
          ...
          standardCost:
            dto.standardCost != null ? dto.standardCost : existing.standardCost,
  ...
  async listProfiles(
    ...
    const [items, total] = await Promise.all([
      prisma.inventoryCostProfile.findMany({
        where,
        skip: params.skip ?? 0,
        take: params.take ?? 20,
  ...
  async getProfile(tenantId: string, id: string) {
    const p = await prisma.inventoryCostProfile.findFirst({
      where: { tenantId, id },
    });
  ...
  async addCostLayer(
    ...
    if (dto.unitCost <= 0)
      throw new BadRequestException("unitCost must be positive");
    if (dto.qty <= 0) throw new BadRequestException("qty must be positive");
    const profile = await prisma.inventoryCostProfile.findFirst({
      where: { tenantId, id: dto.profileId },
    });
    ...
    if (profile.method === "WAC") {
      const openLayers = await prisma.inventoryCostLayer.findMany({
        where: {
          tenantId,
          profileId: dto.profileId,
          status: { not: "FULLY_CONSUMED" },
        },
      });
      const totalQty = openLayers.reduce(
        (sum, l) => sum + Number(l.qtyRemaining),
        0,
      );
      const totalValue = openLayers.reduce(
        (sum, l) => sum + Number(l.qtyRemaining) * Number(l.unitCost),
        0,
      );
      const wac = totalQty > 0 ? totalValue / totalQty : dto.unitCost;
      await prisma.inventoryCostProfile.update({
        where: { id: dto.profileId },
        data: { standardCost: wac },
      });
    }
    return layer;
  }
  ...
  async consumeLayer(
    ...
    if (qtyToConsume <= 0)
      throw new BadRequestException("qtyToConsume must be positive");
    const profile = await prisma.inventoryCostProfile.findFirst({
    ...
    for (const layer of openLayers) {
      ...
      consumed.push({
        layerId: layer.id,
        qty: take,
        cost: take * Number(layer.unitCost),
      });
    ...
    return {
      ...
      totalCost,
      avgCost: totalCost / qtyToConsume,
      layers: consumed,
    };
  }
  ...
  async createAdjustment(
    ...
    if (!profile) throw new NotFoundException("Cost profile not found");
    const count = await prisma.inventoryCostAdjustment.count({
      where: { tenantId },
    });
  ...
  async listAdjustments(
    ...
    const [items, total] = await Promise.all([
      prisma.inventoryCostAdjustment.findMany({
        where,
        skip: params.skip ?? 0,
        take: params.take ?? 20,
