...
import {
  ...
  RenewContractInput,
  InviteSignInput,
} from "../services/crm-contracts.service";
...
export class CrmContractsController {
  ...
  @ApiOperation({
    summary: "Contract KPI stats (active/expiring-soon/expired/total value)",
  })
  @Get("stats")
  @Permissions("crm.contracts.read")
  ...
  @ApiOperation({
    summary: "Scan and auto-transition contracts nearing renewal/expiry",
  })
  @Post("scan-renewals")
  ...
  async create(
    @Req() req: AuthenticatedRequest,
    @ZodBody(createContractSchema) dto: CreateContractInput,
  ) {
  ...
  async update(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(updateContractSchema) dto: UpdateContractInput,
  ) {
  ...
  async updateStatus(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(contractStatusSchema) dto: ContractStatusInput,
  ) {
    return this.svc.updateStatus(req.user.tenantId, id, dto);
  }

  @ApiOperation({
    summary: "Renew contract (extend in place or create a follow-on contract)",
  })
  @Post(":id/renew")
  @Permissions("crm.contracts.update")
  @TrackChanges("Contract")
  ...
  async renew(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(renewContractSchema) dto: RenewContractInput,
  ) {
    return this.svc.renewContract(
      req.user.tenantId,
      req.user.orgId || "",
      id,
      dto,
  ...
  @ApiOperation({
    summary: "Revise contract (creates a new draft copy of an active contract)",
  })
  @Post(":id/revise")
  @Permissions("crm.contracts.update")
  @TrackChanges("Contract")
  ...
  @Post(":id/sales-order")
  @Permissions("crm.contracts.update")
  async convertSalesOrder(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
  ) {
    return this.svc.convertToSalesOrder(
      req.user.tenantId,
      req.user.orgId || "",
      id,
      req.user.userId,
  ...
  @ApiOperation({
    summary:
      "Trigger milestone invoicing (generates a draft Invoice in Finance module)",
  })
  ...
  async triggerMilestoneInvoice(
    ...
    return this.svc.triggerMilestoneInvoice(
      req.user.tenantId,
      req.user.orgId || "",
      contractId,
      id,
