...
export class FinanceLedgerBlockchainService {
  ...
  async verifyJournalEntry(params: {
    ...
    try {
      ...
      const onChainRecord = await contract.verifyJournalEntry(
        params.tenantId,
        params.journalId,
      );
      ...
      const result =
        onChainRecord.dataHash === currentHash
          ? VerificationResult.VERIFIED
          : VerificationResult.TAMPERED;

      if (result === VerificationResult.TAMPERED) {
        this.logger.warn(
          `GL JOURNAL TAMPER: ${params.journalId} — hash mismatch detected!`,
        );
      }
      ...
      this.logger.error(
        `Verification error for GL journal ${params.journalId}: ${(err as Error).message}`,
      );
