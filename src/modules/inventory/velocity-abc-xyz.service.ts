...
export class VelocityAbcXyzService {
  ...
  async getRun(tenantId: string, runId: string) {
    const run = await prisma.velocityClassificationRun.findFirst({
      where: { id: runId, tenantId },
    });
    if (!run)
      throw new NotFoundException(`Classification run ${runId} not found`);
    return run;
  }
  ...
  async createRun(
    ...
    const count = await prisma.velocityClassificationRun.count({
      where: { tenantId },
    });
  ...
  async deleteRun(tenantId: string, runId: string) {
    ...
    if (run.status === "ACTIVE")
      throw new BadRequestException(
        "Cannot delete an ACTIVE classification run",
      );
  ...
  async computeClassification(
    ...
    if (!run)
      throw new NotFoundException(`Classification run ${runId} not found`);
    if (run.status !== "DRAFT")
      throw new BadRequestException("Can only compute on a DRAFT run");
    ...
    return prisma.velocityClassificationRun.update({
      ...
      data: {
        ...
        classACount: counts.A,
        classBCount: counts.B,
        classCCount: counts.C,
        classXCount: counts.X,
        classYCount: counts.Y,
        classZCount: counts.Z,
      },
    });
  }
  ...
  async activateRun(tenantId: string, runId: string) {
    ...
    if (!run)
      throw new NotFoundException(`Classification run ${runId} not found`);
    if (run.status !== "DRAFT")
      throw new BadRequestException("Only DRAFT runs can be activated");
    if (run.totalProducts === 0)
      throw new BadRequestException(
        "Run has no computed items — run computeClassification first",
      );
  ...
  async upsertPolicy(
    ...
    const existing = await prisma.velocitySlottingPolicy.findUnique({
      where: {
        tenantId_combinedClass: { tenantId, combinedClass: dto.combinedClass },
      },
    });
    const data = {
      ...
      safetyStockMultiplier:
        dto.safetyStockMultiplier !== undefined
          ? new Prisma.Decimal(dto.safetyStockMultiplier)
          : undefined,
      preferredZone: dto.preferredZone,
    };
    if (existing) {
      return prisma.velocitySlottingPolicy.update({
    ...
    return prisma.velocitySlottingPolicy.create({
      data: {
        tenantId,
        combinedClass: dto.combinedClass,
        ...data,
        safetyStockMultiplier: new Prisma.Decimal(
          dto.safetyStockMultiplier ?? 1,
        ),
        reviewFrequency: dto.reviewFrequency,
        reorderMethod: dto.reorderMethod,
      },
  ...
  async deletePolicy(tenantId: string, policyId: string) {
    const policy = await prisma.velocitySlottingPolicy.findFirst({
  ...
  async recordSnapshot(
    ...
    const data = {
      ...
      avgSellingPrice:
        dto.avgSellingPrice !== undefined
          ? new Prisma.Decimal(dto.avgSellingPrice)
          : null,
      abcClass: (dto.abcClass as any) ?? null,
      xyzClass: (dto.xyzClass as any) ?? null,
    };
    if (existing) {
      return prisma.productVelocitySnapshot.update({
        where: { id: existing.id },
        data,
      });
    }
  ...
  async getDashboard(tenantId: string) {
    ...
    if (activeRun) {
      ...
      items.forEach((i) => {
        classBreakdown[i.combinedClass] = i._count.id;
      });
    }

    const policyCount = await prisma.velocitySlottingPolicy.count({
    ...
    return {
      ...
      activeRun: activeRun
        ? {
            ...
            totalProducts: activeRun.totalProducts,
            activatedAt: activeRun.activatedAt,
            classACount: activeRun.classACount,
            classBCount: activeRun.classBCount,
            classCCount: activeRun.classCCount,
            classXCount: activeRun.classXCount,
            classYCount: activeRun.classYCount,
            classZCount: activeRun.classZCount,
          }
        : null,
      classBreakdown,
      totalSnapshots,
