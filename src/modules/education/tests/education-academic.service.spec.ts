...
describe("EducationAcademicService", () => {
  ...
  beforeEach(() => {
    mockPrisma = {
      educationReportCard: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "rc-1", studentId: "std-1" }]),
        create: vi
          .fn()
      ...
      educationScholarship: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "sch-1", studentId: "std-1" }]),
        create: vi
          .fn()
          .mockImplementation(({ data }) =>
            Promise.resolve({ id: "sch-1", ...data }),
          ),
      },
      educationAssignmentSubmission: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "sub-1", studentId: "std-1" }]),
        create: vi
          .fn()
          .mockImplementation(({ data }) =>
  ...
  it("should list report cards for tenant", async () => {
    const result = await service.getReportCards("tenant-1", {
      studentId: "std-1",
    });
