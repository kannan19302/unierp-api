...
export class LotExpiryService {
  ...
  async listLots(
    ...
    const [items, total] = await Promise.all([
      prisma.lotExpiryRecord.findMany({
        where,
        skip: params.skip ?? 0,
        take: params.take ?? 20,
  ...
  async getLot(tenantId: string, id: string) {
    const lot = await prisma.lotExpiryRecord.findFirst({
      where: { tenantId, id },
    });
  ...
  async quarantineLot(tenantId: string, id: string, reason: string) {
    ...
    if (lot.status !== "ACTIVE")
      throw new BadRequestException("Only ACTIVE lots can be quarantined");
    return prisma.lotExpiryRecord.update({
      where: { id },
      data: { status: "QUARANTINE", notes: reason },
    });
  }
  ...
  async releaseLot(tenantId: string, id: string) {
    ...
    if (lot.status !== "QUARANTINE")
      throw new BadRequestException("Only QUARANTINE lots can be released");
    return prisma.lotExpiryRecord.update({
      where: { id },
      data: { status: "ACTIVE" },
    });
  ...
  async getFEFOPick(
    ...
    if (qtyNeeded <= 0)
      throw new BadRequestException("qtyNeeded must be positive");
    ...
    if (remaining > 0)
      throw new BadRequestException(
        `Insufficient FEFO stock — short by ${remaining}`,
      );
  ...
  async consumeFromLot(tenantId: string, id: string, qty: number) {
    ...
    if (!lot) throw new NotFoundException("Lot not found");
    if (lot.status !== "ACTIVE")
      throw new BadRequestException("Can only consume from ACTIVE lots");
    ...
    return prisma.lotExpiryRecord.update({
      where: { id },
      data: { remainingQty: remaining, status: newStatus },
    });
  }
  ...
  async scanExpiryAlerts(
    tenantId: string,
    params?: { warnDays?: number; criticalDays?: number },
  ) {
    ...
    for (const lot of lots) {
      ...
      if (daysToExpiry <= 0) {
        await prisma.lotExpiryRecord.update({
          where: { id: lot.id },
          data: { status: "EXPIRED" },
        });
      }
  ...
  async dismissAlert(tenantId: string, id: string) {
    const alert = await prisma.lotExpiryAlert.findFirst({
      where: { tenantId, id },
    });
    if (!alert) throw new NotFoundException("Alert not found");
    return prisma.lotExpiryAlert.update({
      where: { id },
      data: { dismissed: true },
    });
  }
  ...
  async disposeLot(
    ...
    const lot = await prisma.lotExpiryRecord.findFirst({
      where: { tenantId, id: dto.lotId },
    });
    if (!lot) throw new NotFoundException("Lot not found");
    if (!["ACTIVE", "QUARANTINE", "EXPIRED"].includes(lot.status)) {
      throw new BadRequestException("Cannot dispose a lot in current status");
    }
    if (dto.qtyDisposed <= 0)
      throw new BadRequestException("qtyDisposed must be positive");
    if (dto.qtyDisposed > Number(lot.remainingQty))
      throw new BadRequestException("qtyDisposed exceeds remaining qty");
  ...
  async listDisposals(tenantId: string, lotId?: string) {
    ...
    return prisma.lotDisposalRecord.findMany({
      where,
  ...
  async getDashboard(tenantId: string) {
    ...
    const [
      ...
      prisma.lotExpiryRecord.count({
        where: { tenantId, status: "QUARANTINE" },
      }),
      prisma.lotExpiryRecord.count({ where: { tenantId, status: "EXPIRED" } }),
      prisma.lotExpiryRecord.count({ where: { tenantId, status: "DISPOSED" } }),
      prisma.lotExpiryRecord.count({
        where: { tenantId, status: "ACTIVE", expiryDate: { lte: in7 } },
      }),
      prisma.lotExpiryRecord.count({
        where: { tenantId, status: "ACTIVE", expiryDate: { lte: in30 } },
      }),
