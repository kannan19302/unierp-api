...
export class EcommerceCheckoutService {
  ...
  async checkout(tenantId: string, storefrontSlug: string, dto: CheckoutDto) {
    const cart = await prisma.cart.findFirst({
      ...
      include: {
        items: { include: { productListing: { include: { product: true } } } },
      },
    });
    if (!cart) throw new NotFoundException("Cart not found");
    if (cart.status !== "ACTIVE") {
      throw new BadRequestException(
        `Cart is ${cart.status.toLowerCase()} and cannot be checked out`,
      );
    }
    ...
    const confirmed = await this.paymentGateway.confirmIntent(
      intent.id,
      dto.simulateDecline ?? false,
    );

    if (confirmed.status !== "succeeded") {
      this.logger.warn(
        `[MOCK PAYMENT GATEWAY] checkout declined tenantId=${tenantId} cartId=${cart.id}`,
      );
    ...
    const onlineOrderInput = {
      ...
      lineItems: cart.items.map((item) => ({
        ...
        description:
          item.productListing.displayName || item.productListing.product.name,
    ...
    const provider =
      this.paymentGateway.constructor.name === "StripePaymentGatewayService"
        ? "stripe"
        : "mock_gateway";
  ...
  async getCheckoutStateBySession(
    tenantId: string,
    sessionToken: string,
  ): Promise<{
    status: string;
  ...
  async handleStripeWebhook(
    ...
    if (!webhookSecret) {
      this.logger.warn(
        "Stripe webhook signature verification skipped: STRIPE_WEBHOOK_SECRET is not configured.",
      );
    } else {
      const verified = this.verifyStripeSignature(
        rawBody,
        signature,
        webhookSecret,
      );
  ...
  private async completePaymentFromIntent(
    ...
    if (existingPayment) {
      this.logger.log(
        `Stripe PaymentIntent ${intentId} was already processed. Skipping.`,
      );
    ...
    if (!cartId) {
      this.logger.warn(
        `Stripe PaymentIntent ${intentId} has no cartId in metadata. Skipping order creation.`,
      );
    ...
    if (!cart) {
      throw new NotFoundException(
        `Cart ${cartId} associated with PaymentIntent ${intentId} not found.`,
      );
    }
    ...
    if (!org) {
      throw new BadRequestException(
        `No Organization found for Tenant ${tenantId}`,
      );
    }
    ...
    this.logger.log(
      `Webhook converted cart ${cartId} to outbox event (checkout state ${checkoutState.id})`,
    );
