...
export class CashFlowForecastService {
  ...
  async createScenario(
    ...
    return prisma.forecastScenario.create({
      data: {
        ...
        inflowFactor: dto.inflowFactor
          ? new Prisma.Decimal(dto.inflowFactor)
          : new Prisma.Decimal(1.0),
        outflowFactor: dto.outflowFactor
          ? new Prisma.Decimal(dto.outflowFactor)
          : new Prisma.Decimal(1.0),
  ...
  async updateScenario(
    ...
    if (dto.inflowFactor !== undefined)
      updateData.inflowFactor = new Prisma.Decimal(dto.inflowFactor);
    if (dto.outflowFactor !== undefined)
      updateData.outflowFactor = new Prisma.Decimal(dto.outflowFactor);
  ...
  async get13WeekForecast(tenantId: string, scenarioId?: string) {
    ...
    const endOf13Weeks = new Date(
      currentMonday.getTime() + 13 * 7 * 24 * 60 * 60 * 1000,
    );

    const [unpaidInvoices, pendingSchedules, weeklyOverrides] =
      await Promise.all([
    ...
    for (let i = 0; i < 13; i++) {
      ...
      const baselineInflow = weekInvoices.reduce(
      ...
      const weekSchedules = pendingSchedules.filter(
        (sched) =>
          sched.dueDate && sched.dueDate >= wStart && sched.dueDate <= wEnd,
      );
      const baselineOutflow = weekSchedules.reduce(
  ...
  async exportForecastCsv(tenantId: string) {
    ...
    let csv =
      "Week Start,Projected Inflow,Projected Outflow,Manual Adjustments,Net Flow,Cumulative Cash Balance,Comments\n";
