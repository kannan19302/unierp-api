...
export class SupplyChainBudgetController {
  ...
  list(@Req() req: AuthRequest, @Query("page") page?: string, @Query("limit") limit?: string, @Query("fiscalYear") fiscalYear?: string, @Query("status") status?: string) {
    return this.svc.list(req.user.tenantId, { page: page ? Number(page) : undefined, limit: limit ? Number(limit) : undefined, fiscalYear, status });
  }

  @Get("vs-actual")
  @Permissions("supply-chain.budget.read")
  @ApiOperation({ summary: "Get budget vs actual comparison" })
  getBudgetVsActual(@Req() req: AuthRequest, @Query("fiscalYear") fiscalYear?: string) {
    return this.svc.getBudgetVsActual(req.user.tenantId, fiscalYear);
  }
  ...
  @HttpCode(HttpStatus.CREATED)
  addLineItem(@Req() req: AuthRequest, @Param("id") id: string, @ZodBody(lineItemSchema) body: z.infer<typeof lineItemSchema>) {
  ...
  updateLineItem(@Req() req: AuthRequest, @Param("lineItemId") lineItemId: string, @ZodBody(updateLineItemSchema) body: z.infer<typeof updateLineItemSchema>) {
    return this.svc.updateLineItem(req.user.tenantId, lineItemId, body);
  }
}
