...
vi.mock("@kannan19302/database/prisma", () => {
  class Decimal {
    ...
    add(other: Decimal | number) {
      return new Decimal(this.value + Number(other instanceof Decimal ? other.value : other));
    }
    plus(other: Decimal | number) {
    ...
    equals(other: Decimal | number) {
      return Math.abs(this.value - Number(other instanceof Decimal ? other.value : other)) < 1e-9;
    }
    toString() {
...
vi.mock("@kannan19302/database", () => {
  ...
  const txMocks = {
    ...
    journal: {
      ...createGenericPrismaMock(),
      findUnique: vi.fn().mockResolvedValue({ id: "journal-id" }),
    },
  ...
  return {
    prisma: {
      ...
      organization: {
        findFirst: vi
          .fn()
          .mockResolvedValue({ id: "org-1", name: "Seller Inc" }),
      },
      accountingBook: createGenericPrismaMock(),
      accountingBookRule: createGenericPrismaMock(),
...
describe("AdvancedFinanceService", () => {
      ...
      if (model && typeof model === "object") {
        Object.keys(model).forEach((m) => {
          if (typeof model[m] === "function" && vi.isMockFunction(model[m])) {
            ...
            if (m === "create")
              vi.mocked(model[m]).mockResolvedValue({ id: "new-id" });
            if (m === "update")
              vi.mocked(model[m]).mockResolvedValue({ id: "updated-id" });
  ...
  getMethods.forEach((method) => {
    it(`should run ${method} without errors`, async () => {
        ...
        if (
          (
            prisma as unknown as Record<
              string,
              Record<string, import("vitest").Mock>
            >
          )[key]?.findMany
        ) {
          vi.mocked(
      ...
      const res = await (
        service as unknown as Record<string, import("vitest").Mock>
      )[method]!("tenant-123");
  ...
  const createMethods = [
    {
      ...
      args: [
        "tenant-1",
        "org-1",
        { code: "1000", name: "Test", type: "ASSET" },
      ],
    },
    {
      method: "createCostCenter",
      args: ["tenant-1", "org-1", { code: "C100", name: "Test CC" }],
    },
    {
      method: "createBankReconciliation",
      args: [
        "tenant-1",
        {
          accountId: "acc-1",
    ...
    },
    {
      ...
      args: [
        "tenant-1",
        "org-1",
        { customerId: "cust-1", noteNumber: "CN-001", amount: 100 },
      ],
    },
    {
      ...
      args: [
        "tenant-1",
        "org-1",
        { vendorId: "ven-1", noteNumber: "DN-001", amount: 50 },
      ],
    },
    {
      method: "createDunningLevel",
      args: ["tenant-1", "org-1", { levelName: "Level 1", daysOverdue: 30 }],
    },
    {
      ...
      args: [
        "tenant-1",
        "org-1",
        { vendorId: "ven-1", dueDate: new Date().toISOString(), amount: 100 },
      ],
    },
    {
      method: "createPaymentRun",
      args: [
        "tenant-1",
        "org-1",
        { bankAccountId: "bank-1", runDate: new Date().toISOString() },
      ],
    },
    {
      method: "createForecastScenario",
      args: ["tenant-1", "org-1", { name: "Best Case" }],
    },
    {
      method: "createTaxRule",
      args: ["tenant-1", "org-1", { name: "GST 18%", isDefault: true }],
    },
    {
      method: "createWithholdingTax",
      args: [
        "tenant-1",
        "org-1",
        { name: "TDS 10%", rate: 10, accountId: "acc-1" },
      ],
    },
    {
      method: "createTaxFiling",
      args: [
        "tenant-1",
        "org-1",
        {
    ...
    {
      method: "createInvestmentPortfolio",
      args: [
        "tenant-1",
        "org-1",
        {
    ...
    {
      method: "createTreasuryTransaction",
      args: [
        "tenant-1",
        "org-1",
        {
    ...
    it(`should run ${method} without errors`, async () => {
      ...
      Object.keys(prisma).forEach((key) => {
        ...
        if (
          ...
          ).mockResolvedValueOnce({ id: "updated-id" });
        }
      });
  ...
  describe("createJournal", () => {
    ...
    it("should throw error if journal is not balanced", async () => {
      ...
      await expect(
        service.createJournal("tenant-1", "org-1", dto),
      ...
      vi.mocked(prisma.invoice.findMany).mockResolvedValueOnce([
        {
          ...
          dueDate: daysAgo(45),
          subtotal: 1000 as never,
          taxAmount: 100 as never,
          discountAmount: 0 as never,
          totalAmount: 1100 as never,
        ...
        } as never,
        {
          ...
          dueDate: daysAgo(95),
          subtotal: 2000 as never,
          taxAmount: 200 as never,
          discountAmount: 0 as never,
          totalAmount: 2200 as never,
      ...
      vi.mocked(prisma.purchaseOrder.findMany).mockResolvedValueOnce([
        {
          ...
          expectedDate: null,
          subtotal: 500 as never,
          taxAmount: 50 as never,
          discountAmount: 0 as never,
          totalAmount: 550 as never,
          currency: "USD",
  ...
  describe("runCurrencyRevaluation", () => {
    it("recognises an unrealized FX gain when the current rate exceeds the booking rate", async () => {
      ...
      vi.mocked(prisma.currencyRevaluation.create).mockResolvedValue({
        id: "reval-1",
      } as never);

      const res = await service.runCurrencyRevaluation(
        "t",
        "org-1",
  ...
  describe("generateEInvoice", () => {
    it("produces a UBL document for an issued invoice", async () => {
      ...
      vi.mocked(prisma.invoice.findFirst).mockResolvedValueOnce({
        ...
        subtotal: 1000 as never,
        discountAmount: 0 as never,
        taxAmount: 180 as never,
        totalAmount: 1180 as never,
      ...
      vi.mocked(prisma.organization.findFirst).mockResolvedValue({
        id: "org-1",
        name: "Seller Inc",
        legalName: "Seller Incorporated",
        taxId: "GB999",
      } as never);
      vi.mocked(prisma.eInvoice.upsert).mockResolvedValue({
        id: "einv-1",
        format: "UBL",
        documentXml: "<Invoice/>",
      } as never);
      ...
      const arg = vi.mocked(prisma.eInvoice.upsert).mock.calls[0]![0] as {
        create: { documentXml: string; format: string };
      };
      expect(arg.create.format).toBe("UBL");
      expect(arg.create.documentXml).toContain("<cbc:ID>INV-9</cbc:ID>");
      expect(arg.create.documentXml).toContain(
    ...
    it("computes a deterministic IRN and QR payload for the GST format", async () => {
      ...
      vi.mocked(prisma.organization.findFirst).mockResolvedValue({
        id: "org-1",
        name: "Seller",
        legalName: "Seller LLP",
        taxId: "27AAAAA0000A1Z5",
      } as never);
      vi.mocked(prisma.eInvoice.upsert).mockImplementation(((args: {
        create: unknown;
      }) =>
        Promise.resolve({
          id: "einv-2",
      ...
      const res = (await service.generateEInvoice(
        "t",
        "org-1",
        "inv-2",
        "GST_IRN",
      )) as { irn: string; qrPayload: string };
    ...
    it("rejects e-invoicing a draft invoice", async () => {
      ...
      await expect(
        service.generateEInvoice("t", "org-1", "inv-3", "UBL"),
      ).rejects.toThrow();
    });
  });
});
