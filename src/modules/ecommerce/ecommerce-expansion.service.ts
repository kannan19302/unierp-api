...
export class EcommerceExpansionService {
  ...
  async getStores(tenantId: string) {
    return this.p.ecommerceStore.findMany({
      where: { tenantId },
      orderBy: { name: "asc" },
    });
  }
  ...
  async getStoreById(tenantId: string, id: string) {
    const store = await this.p.ecommerceStore.findFirst({
      where: { id, tenantId },
    });
  ...
  async createStore(tenantId: string, dto: any) {
    const existing = await this.p.ecommerceStore.findFirst({
  ...
  async deleteStore(tenantId: string, id: string) {
    ...
    if (!store) throw new NotFoundException("Store not found");
    return this.p.ecommerceStore.update({
      where: { id },
      data: { isActive: false },
    });
  }
  ...
  async getCategories(tenantId: string, storeId: string) {
    return this.p.ecommerceCategory.findMany({
  ...
  async getCategoryById(tenantId: string, id: string) {
    const cat = await this.p.ecommerceCategory.findFirst({
      where: { id, tenantId },
  ...
  async createCategory(tenantId: string, storeId: string, dto: any) {
    const existing = await this.p.ecommerceCategory.findFirst({
      where: { tenantId, storeId, slug: dto.slug },
    });
    if (existing)
      throw new BadRequestException("Category with this slug already exists");
    return this.p.ecommerceCategory.create({
  ...
  async deleteCategory(tenantId: string, id: string) {
    ...
    if (!cat) throw new NotFoundException("Category not found");
    return this.p.ecommerceCategory.update({
      where: { id },
      data: { isActive: false },
    });
  }
  ...
  async getListings(
    ...
    const [data, total] = await Promise.all([
      this.p.ecommerceProductListing.findMany({
        where,
  ...
  async createListing(tenantId: string, storeId: string, dto: any) {
    const existing = await this.p.ecommerceProductListing.findFirst({
      where: { tenantId, storeId, slug: dto.slug },
    });
    if (existing)
      throw new BadRequestException("Listing with this slug already exists");
    return this.p.$transaction(async (tx: any) => {
      ...
      if (dto.variants && dto.variants.length > 0) {
        for (const v of dto.variants) {
          ...
          await tx.ecommerceInventory.create({
            data: {
              tenantId,
      ...
      return tx.ecommerceProductListing.findFirst({
        where: { id: listing.id },
  ...
  async deleteListing(tenantId: string, id: string) {
    ...
    if (!listing) throw new NotFoundException("Listing not found");
    return this.p.ecommerceProductListing.update({
      where: { id },
      data: { isActive: false },
    });
  }
  ...
  async createVariant(tenantId: string, listingId: string, dto: any) {
    return this.p.$transaction(async (tx: any) => {
      ...
      await tx.ecommerceInventory.create({
        data: { tenantId, variantId: variant.id, quantity: dto.quantity || 0 },
      });
      return variant;
    });
  }
  ...
  async updateVariant(tenantId: string, id: string, dto: any) {
    const variant = await this.p.ecommerceProductVariant.findFirst({
      where: { id, tenantId },
    });
  ...
  async updateInventory(tenantId: string, variantId: string, quantity: number) {
    const inv = await this.p.ecommerceInventory.findFirst({
      where: { tenantId, variantId },
    });
    if (!inv) throw new NotFoundException("Inventory record not found");
    return this.p.ecommerceInventory.update({
      where: { id: inv.id },
      data: { quantity },
    });
  }
  ...
  async getCoupons(tenantId: string, storeId: string) {
    return this.p.ecommerceCoupon.findMany({
      where: { tenantId, storeId },
  ...
  async createCoupon(tenantId: string, storeId: string, dto: any) {
    const existing = await this.p.ecommerceCoupon.findFirst({
      where: { tenantId, storeId, code: dto.code.toUpperCase() },
    });
  ...
  async updateCoupon(tenantId: string, id: string, dto: any) {
    const coupon = await this.p.ecommerceCoupon.findFirst({
      where: { id, tenantId },
    });
  ...
  async getShippingZones(tenantId: string, storeId: string) {
    return this.p.ecommerceShippingZone.findMany({
      where: { tenantId, storeId },
      orderBy: { sortOrder: "asc" },
      include: { rates: true },
    });
  ...
  async createShippingZone(tenantId: string, storeId: string, dto: any) {
    return this.p.ecommerceShippingZone.create({
      data: { tenantId, storeId, ...dto },
    });
  }

  async updateShippingZone(tenantId: string, id: string, dto: any) {
    const zone = await this.p.ecommerceShippingZone.findFirst({
      where: { id, tenantId },
    });
  ...
  async createShippingRate(tenantId: string, dto: any) {
    return this.p.ecommerceShippingRate.create({
      data: {
        tenantId,
        ...dto,
  ...
  async updateShippingRate(tenantId: string, id: string, dto: any) {
    const rate = await this.p.ecommerceShippingRate.findFirst({
      where: { id, tenantId },
    });
  ...
  async getTaxClasses(tenantId: string, storeId: string) {
    return this.p.ecommerceTaxClass.findMany({
      where: { tenantId, storeId },
      include: { rates: true },
      orderBy: { name: "asc" },
    });
  ...
  async createTaxClass(tenantId: string, storeId: string, dto: any) {
    if (dto.isDefault) {
      await this.p.ecommerceTaxClass.updateMany({
        where: { tenantId, storeId },
        data: { isDefault: false },
      });
    }
    return this.p.ecommerceTaxClass.create({
      data: { tenantId, storeId, ...dto },
    });
  }

  async createTaxRate(tenantId: string, dto: any) {
    return this.p.ecommerceTaxRate.create({
      data: { tenantId, ...dto, rate: new Prisma.Decimal(dto.rate) },
    });
  }

  async updateTaxRate(tenantId: string, id: string, dto: any) {
    const rate = await this.p.ecommerceTaxRate.findFirst({
      where: { id, tenantId },
    });
  ...
  async getStoreSettings(tenantId: string, storeId: string) {
    return this.p.ecommerceStoreSetting.findMany({
      where: { tenantId, storeId },
    });
  }
  ...
  async deleteStoreSetting(tenantId: string, storeId: string, key: string) {
    const setting = await this.p.ecommerceStoreSetting.findFirst({
  ...
  async getThemes(tenantId: string, storeId: string) {
    return this.p.ecommerceStoreTheme.findMany({
      where: { tenantId, storeId },
    });
  }
  ...
  async createTheme(tenantId: string, storeId: string, dto: any) {
    if (dto.isActive) {
      await this.p.ecommerceStoreTheme.updateMany({
        where: { tenantId, storeId },
        data: { isActive: false },
      });
    }
    return this.p.ecommerceStoreTheme.create({
      data: { tenantId, storeId, ...dto, config: dto.config || {} },
    });
  }

  async activateTheme(tenantId: string, storeId: string, id: string) {
    const theme = await this.p.ecommerceStoreTheme.findFirst({
    ...
    return this.p.ecommerceStoreTheme.update({
      where: { id },
      data: { isActive: true },
    });
  }
  ...
  async getReviews(
    ...
    const [data, total] = await Promise.all([
      this.p.ecommerceReview.findMany({
        where,
        orderBy: { createdAt: "desc" },
        skip,
        take,
  ...
  async createReview(
    ...
    return this.p.$transaction(async (tx: any) => {
      ...
      return tx.ecommerceReview.findFirst({
        where: { id: review.id },
        include: { media: true },
      });
    });
  ...
  async moderateReview(tenantId: string, id: string, approved: boolean) {
    const review = await this.p.ecommerceReview.findFirst({
      where: { id, tenantId },
    });
    if (!review) throw new NotFoundException("Review not found");
    return this.p.ecommerceReview.update({
      where: { id },
  ...
  async getOrders(
    ...
    const [data, total] = await Promise.all([
      this.p.ecommerceOrder.findMany({
        where,
        orderBy: { createdAt: "desc" },
        skip,
        take,
