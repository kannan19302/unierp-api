...
describe("DebitNotesService (extra)", () => {
  ...
  it("should get debit note by id", async () => {
    ...
    (prisma.debitNote.findFirst as any).mockResolvedValue(mockNote);
  ...
  it("should throw NotFoundException for missing debit note", async () => {
    (prisma.debitNote.findFirst as any).mockResolvedValue(null);
  ...
  it("should update debit note in DRAFT status", async () => {
    ...
    const result = await service.update("tenant-1", "1", {
      amount: 600,
      notes: "Updated",
    });
  ...
  it("should throw NotFoundException when updating non-DRAFT debit note", async () => {
    (prisma.debitNote.findFirst as any).mockResolvedValue(null);
    await expect(
      service.update("tenant-1", "1", { amount: 600 }),
  ...
  it("should process debit note refund from ISSUED status", async () => {
    ...
    (prisma.debitNote.update as any).mockResolvedValue({
      id: "1",
      status: "SETTLED",
      notes: "Refund processed: REF-123",
    });

    const result = await service.processDebitNoteRefund("tenant-1", "1", {
      refundRef: "REF-123",
    });
  ...
  it("should throw BadRequestException when refunding non-refundable status (DRAFT)", async () => {
    ...
    await expect(
      service.processDebitNoteRefund("tenant-1", "1", { refundRef: "REF-123" }),
  ...
  it("should get debit notes by vendor with tenant isolation", async () => {
    ...
    const result = await service.getDebitNotesByVendor(
      "tenant-1",
      "vendor-1",
      {},
    );
  ...
  it("should get debit notes by date range", async () => {
    ...
    const result = await service.getDebitNotesByDateRange(
      "tenant-1",
      "2026-01-01",
      "2026-06-30",
      {},
