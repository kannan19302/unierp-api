...
describe("HealthcareClinicalService", () => {
  ...
  beforeEach(() => {
    mockPrisma = {
      healthcareClinicalNote: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "note-1", patientId: "pat-1" }]),
        create: vi
          .fn()
          .mockImplementation(({ data }) =>
            Promise.resolve({ id: "note-1", ...data }),
          ),
      },
      healthcareTelemedicineSession: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "session-1", patientId: "pat-1" }]),
        create: vi
          .fn()
          .mockImplementation(({ data }) =>
      ...
      healthcareMedicalBill: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "bill-1", patientId: "pat-1" }]),
        create: vi
          .fn()
          .mockImplementation(({ data }) =>
  ...
  it("should list clinical notes for tenant", async () => {
    const result = await service.getClinicalNotes("tenant-1", {
      patientId: "pat-1",
    });
