...
export class RtvService {
  ...
  async createReasonCode(tenantId: string, dto: CreateReasonCodeInput) {
    ...
    if (existing)
      throw new ConflictException(`Reason code '${dto.code}' already exists`);
  ...
  async updateReasonCode(
    tenantId: string,
    id: string,
    dto: UpdateReasonCodeInput,
  ) {
    const rc = await prisma.returnReasonCode.findFirst({
      where: { id, tenantId },
    });
  ...
  async createRmaRequest(
    ...
    this.eventEmitter.emit("inventory.rtv.rma_created", {
      tenantId,
      rmaId: rma.id,
    });
    return rma;
  ...
  async submitRmaRequest(tenantId: string, id: string) {
    ...
    if (rma.status !== "PENDING") {
      throw new BadRequestException(
        `RMA is ${rma.status.toLowerCase()}, cannot submit`,
      );
    }
  ...
  async authorizeRmaRequest(
    tenantId: string,
    id: string,
    dto: UpdateRmaStatusInput,
  ) {
    const rma = await this.getRmaRequest(tenantId, id);
    if (rma.status !== "SUBMITTED") {
      throw new BadRequestException(
        `RMA is ${rma.status.toLowerCase()}, cannot authorize`,
      );
    }
    return prisma.vendorRmaRequest.update({
      where: { id },
  ...
  async rejectRmaRequest(tenantId: string, id: string, dto: RejectRmaInput) {
    ...
    if (!["SUBMITTED", "PENDING"].includes(rma.status)) {
      throw new BadRequestException(
        `RMA is ${rma.status.toLowerCase()}, cannot reject`,
      );
    }
    return prisma.vendorRmaRequest.update({
      where: { id },
  ...
  async completeRmaRequest(tenantId: string, id: string) {
    ...
    if (rma.status !== "AUTHORIZED") {
      throw new BadRequestException(
        `RMA is ${rma.status.toLowerCase()}, cannot complete`,
      );
    }
    ...
    this.eventEmitter.emit("inventory.rtv.rma_completed", {
      tenantId,
      rmaId: id,
    });
    return updated;
  ...
  async listShipments(
    ...
    const [data, total] = await Promise.all([
      prisma.vendorReturnShipment.findMany({
        ...
        include: {
          rmaRequest: {
            select: { id: true, rmaNumber: true, vendorRmaRef: true },
          },
  ...
  async createShipment(tenantId: string, dto: CreateShipmentInput) {
    ...
    if (rma.status !== "AUTHORIZED") {
      throw new BadRequestException(
        "RMA must be AUTHORIZED before creating a shipment",
      );
    }
  ...
  async packShipment(tenantId: string, id: string) {
    ...
    if (shipment.status !== "PENDING") {
      throw new BadRequestException(
        `Shipment is ${shipment.status.toLowerCase()}, cannot pack`,
      );
    }
  ...
  async markShipped(tenantId: string, id: string) {
    ...
    if (shipment.status !== "PACKED") {
      throw new BadRequestException(
        `Shipment is ${shipment.status.toLowerCase()}, cannot mark shipped`,
      );
    }
    return prisma.vendorReturnShipment.update({
      where: { id },
  ...
  async markDelivered(tenantId: string, id: string) {
    ...
    if (shipment.status !== "SHIPPED") {
      throw new BadRequestException(
        `Shipment is ${shipment.status.toLowerCase()}, cannot mark delivered`,
      );
    }
    return prisma.vendorReturnShipment.update({
      where: { id },
  ...
  async recordCreditMemo(
    tenantId: string,
    id: string,
    dto: RecordCreditMemoInput,
  ) {
    const shipment = await this.getShipment(tenantId, id);
    if (shipment.status !== "DELIVERED") {
      throw new BadRequestException(
        "Credit memo can only be recorded after delivery",
      );
    }
  ...
  async getRtvDashboard(tenantId: string) {
    const [totalRmas, byStatus, pendingShipments, totalCreditAmount] =
      await Promise.all([
        ...
        prisma.vendorReturnShipment.count({
          where: { tenantId, status: { in: ["PENDING", "PACKED", "SHIPPED"] } },
        }),
