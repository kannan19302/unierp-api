...
describe("InvoiceCaptureService", () => {
  ...
  describe("getCapture", () => {
    it("should return capture record if found", async () => {
      ...
      vi.mocked(prisma.aPInvoiceCapture.findFirst).mockResolvedValue(
        mockResult as any,
      );
    ...
    it("should throw NotFoundException if capture record does not exist", async () => {
      ...
      await expect(service.getCapture(tenantId, "c-1")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("createCapture", () => {
    it("should create initial draft record and run simulated OCR if rawText provided", async () => {
      ...
      vi.mocked(prisma.aPInvoiceCapture.create).mockResolvedValue(
        mockCapture as any,
      );
      vi.mocked(prisma.aPInvoiceCapture.findFirst).mockResolvedValue(
        mockCapture as any,
      ...
      const res = await service.createCapture(tenantId, userId, {
        fileName: "inv.pdf",
        rawText:
          "This is a sample invoice Globex INV-10020 due total: $1200.00 PO #PO-909",
      });
  ...
  describe("updateCapture", () => {
    it("should update metadata fields successfully", async () => {
      ...
      vi.mocked(prisma.aPInvoiceCapture.update).mockResolvedValue({
        ...mockCapture,
        notes: "new",
      } as any);

      const res = await service.updateCapture(tenantId, "c-1", userId, {
        notes: "new",
      });
  ...
  describe("deleteCapture", () => {
    it("should delete draft record but throw if already processed", async () => {
      ...
      await expect(service.deleteCapture(tenantId, "c-1")).rejects.toThrow(
        BadRequestException,
      );

      vi.mocked(prisma.aPInvoiceCapture.delete).mockResolvedValue(
        mockDraft as any,
      );
  ...
  describe("autoCode", () => {
    it("should fetch suggestions based on historical vendor matches", async () => {
      ...
      vi.mocked(prisma.aPInvoiceCaptureLine.findFirst).mockResolvedValue({
        suggestedAccountId: "acc-99",
        suggestedCostCenterId: "cc-01",
      } as any);
        ...
        lines: [
          {
            id: "l-1",
            amount: 1200,
            suggestedAccountId: "exp-1",
            suggestedCostCenterId: "cc-1",
          },
        ],
      };
      vi.mocked(prisma.aPInvoiceCapture.findFirst).mockResolvedValue(
  ...
  describe("line operations", () => {
    it("should allow adding manual lines", async () => {
      ...
      vi.mocked(prisma.aPInvoiceCaptureLine.create).mockResolvedValue({
        id: "l-1",
        amount: 100,
      } as any);

      const res = await service.createLine(tenantId, "c-1", {
    ...
    it("should allow updating captured lines", async () => {
      ...
      vi.mocked(prisma.aPInvoiceCaptureLine.update).mockResolvedValue({
        id: "l-1",
        quantity: 1.5,
        unitPrice: 100,
        amount: 150,
      ...
      const res = await service.updateLine(tenantId, "c-1", "l-1", {
        quantity: 1.5,
      });
      expect(res).toBeDefined();
      expect(prisma.aPInvoiceCapture.update).toHaveBeenCalled();
