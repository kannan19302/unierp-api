...
export class DeveloperAuditService {
  ...
  async record(input: { tenantId: string; projectId: string; action: string; actorId?: string | null; correlationId?: string | null; metadata?: Record<string, unknown> }) {
    return await this.db.developerAuditEvent.create({ data: { tenantId: input.tenantId, projectId: input.projectId, action: input.action, actorId: input.actorId ?? null, correlationId: input.correlationId ?? null, metadata: input.metadata ?? {} } });
  }
}
