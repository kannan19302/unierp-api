...
const mkRule = (o: Record<string, unknown> = {}) => ({
  ...
  minApprovedVendors: 2,
  requiresQualification: true,
  qualificationValidityDays: 365,
  requiresPreferred: false,
  ...o,
});
...
describe("AslService", () => {
  ...
  describe("createApprovedSupplier", () => {
    it("creates a new ASL entry", async () => {
      ...
      vi.mocked(prisma.approvedSupplier.create).mockResolvedValue(
        mkAsl() as never,
      );
      vi.mocked(prisma.aslChangeLog.create).mockResolvedValue({} as never);
      await svc.createApprovedSupplier(TENANT, USER, {
        productId: "prod1",
        vendorId: "ven1",
      });
    ...
    it("throws if vendor already in ASL for product", async () => {
      ...
      await expect(
        svc.createApprovedSupplier(TENANT, USER, {
          productId: "prod1",
          vendorId: "ven1",
        }),
  ...
  describe("approveSupplier", () => {
    it("approves a PENDING_APPROVAL supplier", async () => {
      ...
      vi.mocked(prisma.aslChangeLog.create).mockResolvedValue({} as never);
      vi.mocked(prisma.approvedSupplier.update).mockResolvedValue(
    ...
    it("throws if already approved", async () => {
      ...
      await expect(svc.approveSupplier(TENANT, "asl1", USER)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("disqualifySupplier", () => {
    it("disqualifies a supplier", async () => {
      ...
      const result = await svc.disqualifySupplier(
        TENANT,
        "asl1",
        USER,
        "Failed audit",
    ...
    it("throws if already disqualified", async () => {
      ...
      await expect(
        svc.disqualifySupplier(TENANT, "asl1", USER, "x"),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("setPreferred", () => {
    it("sets an APPROVED supplier as preferred", async () => {
      ...
      vi.mocked(prisma.aslChangeLog.create).mockResolvedValue({} as never);
      vi.mocked(prisma.approvedSupplier.update).mockResolvedValue(
        mkAsl({ isPreferred: true, preferenceRank: 1 }) as never,
      );
    ...
    it("throws if not APPROVED", async () => {
      ...
      await expect(svc.setPreferred(TENANT, "asl1", USER)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("addPriceTier", () => {
    it("creates a price tier", async () => {
      ...
      vi.mocked(prisma.supplierPriceTier.create).mockResolvedValue(
        mkTier() as never,
      );
  ...
  describe("upsertAttribute", () => {
    it("upserts a vendor item attribute", async () => {
      ...
      await svc.upsertAttribute(TENANT, {
        productId: "prod1",
        vendorId: "ven1",
        attributeKey: "COLOR",
        attributeValue: "RED",
  ...
  describe("upsertComplianceRule", () => {
    it("creates or updates compliance rule", async () => {
      ...
      const result = await svc.upsertComplianceRule(TENANT, {
        productCategory: "CHEMICALS",
        minApprovedVendors: 2,
      });
  ...
  describe("checkProductCompliance", () => {
    it("marks compliant when requirements are met", async () => {
      ...
      const result = await svc.checkProductCompliance(
        TENANT,
        "prod1",
        "CHEMICALS",
      );
  ...
  describe("getChangeLog", () => {
    it("returns change log for an ASL entry", async () => {
      ...
      vi.mocked(prisma.aslChangeLog.findMany).mockResolvedValue([
        { id: "log1", changeType: "APPROVED" },
      ] as never);
  ...
  describe("getApprovedSupplier - not found", () => {
    it("throws NotFoundException", async () => {
      ...
      await expect(svc.getApprovedSupplier(TENANT, "bad")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
