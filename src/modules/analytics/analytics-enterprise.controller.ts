...
export class AnalyticsEnterpriseController {
  ...
  @ApiOperation({ summary: 'Cross-entity composite report from multiple source modules' })
  @Post('multi-source-report')
  @Permissions('analytics.report.create')
  async getMultiSourceReport(
    @Req() req: AuthenticatedRequest,
    @Body() body: { entities: string[]; dimensions: string[]; measures: string[] },
  ) {
    return this.service.getMultiSourceReport(req.user.tenantId, body.entities, body.dimensions, body.measures);
  }

  @ApiOperation({ summary: 'Hierarchical drill-down analysis with breadcrumb trail' })
  ...
  async getWhatIfAnalysis(
    @Req() req: AuthenticatedRequest,
    @Body() body: { modelId: string; scenarios: { name: string; adjustments: Record<string, number> }[] },
  ) {
    return this.service.getWhatIfAnalysis(req.user.tenantId, body.modelId, body.scenarios);
  }
  ...
  async getForecastModel(
    ...
    return this.service.getForecastModel(req.user.tenantId, metric, horizon || 'MONTHLY', method || 'linear');
  }

  @ApiOperation({ summary: 'Safe ad-hoc query execution with field-level access control' })
  @Post('ad-hoc-query')
  @Permissions('analytics.report.create')
  async getAdHocQuery(@Req() req: AuthenticatedRequest, @Body() body: { query: string }) {
  ...
  async getDashboardExport(
    ...
    return this.service.getDashboardExport(req.user.tenantId, dashboardId, format || 'pdf');
  }

  @ApiOperation({ summary: 'AI-generated natural language insights on key metrics' })
  @Get('insights')
  @Permissions('analytics.report.read')
  async getAutomatedInsights(
    ...
    return this.service.getAutomatedInsights(req.user.tenantId, dateRange || 'MONTHLY');
  }

  @ApiOperation({ summary: 'Data quality completeness, consistency, timeliness metrics' })
  @Get('data-quality')
  @Permissions('analytics.report.read')
  ...
  @ApiOperation({ summary: 'Configurable anomaly detection rules and thresholds' })
  ...
  async getBenchmarkComparison(
    ...
    return this.service.getBenchmarkComparison(req.user.tenantId, metric, industry);
  }
}
