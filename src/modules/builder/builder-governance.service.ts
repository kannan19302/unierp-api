...
export class BuilderGovernanceService {
  async getModuleEnvironments(
    ...
    const production = releases.find(
      (r) => r.channel === "PRODUCTION" && r.status === "PUBLISHED",
    );
    const staging = releases.find(
      (r) => r.channel === "STAGING" && r.status === "PUBLISHED",
    );
  ...
  async promoteToProduction(
    ...
    if (!stagingRelease)
      throw new BadRequestException(
        "No staging release to promote. Promote to staging first.",
      );
    ...
    return {
      moduleId,
      version: stagingRelease.version,
  ...
  async getModulePermissions(tenantId: string, moduleId: string) {
    ...
    const permissions =
      mod.permissions && typeof mod.permissions === "object"
    ...
    return {
      ...
      moduleName: mod.name,
      permissions: {
        allowedRoles: (permissions as any).allowedRoles || [
          "Super Admin",
          "Admin",
        ],
  ...
  async compareVersions(
    ...
    for (const key of [
      ...
      for (const id of toIds) {
        if (!fromIds.has(id))
          changes.push({ type: key, entity: id, action: "ADDED" });
      }
      for (const id of fromIds) {
        if (!toIds.has(id))
          changes.push({ type: key, entity: id, action: "REMOVED" });
      }
    }
        ...
        data: [
          {
            tenantId,
            level: "INFO",
            message: "Sync CRM leads to ERP module: Completed successfully.",
            payload: { duration: "342ms" },
          },
          {
            tenantId: tenantId,
            level: "WARN",
            message:
              "Slow DB query detected: SELECT * FROM invoice WHERE status = 'PAID'",
            stackTrace: "Explain plan cost: 420.50",
          },
          {
            tenantId: tenantId,
            level: "ERROR",
            message:
              "Sync failed: Connection timed out to external API endpoint.",
            stackTrace: "TimeoutException at fetch()",
          },
        ],
      });
    }
  ...
  async getConnectors(tenantId: string) {
    ...
    const count = await prisma.thirdPartyConnector.count({
      where: { tenantId },
    });
    if (count === 0) {
        ...
        data: [
          {
            ...
            type: "REST",
            config: { url: "https://shopify.com/api/v2", auth: "Bearer token" },
          },
          {
            tenantId,
            name: "Salesforce Sync Stream",
            type: "GraphQL",
            config: { url: "https://salesforce.com/graphql", auth: "OAuth2" },
          },
        ],
      });
    }
  ...
  async getAllPermissions(tenantId: string) {
    const list = await prisma.studioPermission.findMany({
      where: { tenantId },
    });
    if (list.length === 0) {
  ...
  async getMarketplaceList() {
    return [
      {
        id: "1",
        name: "Fleet Maintenance Template",
        description:
          "Complete system tracking service orders, gas expenses, and logs.",
        author: "UniERP Team",
        downloads: 1420,
      },
      {
        id: "2",
        name: "Stripe Payment Gateway Plugin",
        description: "Enable visual checkout forms directly bound to invoices.",
        author: "Community Partner",
        downloads: 890,
      },
      {
        id: "3",
        name: "Healthcare EMR Extension",
        description:
          "Adds EHR files, encounter trackers, and scheduler overlays.",
        author: "MedTech Labs",
        downloads: 650,
      },
    ];
