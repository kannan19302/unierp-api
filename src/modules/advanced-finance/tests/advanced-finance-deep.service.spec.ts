...
describe("Advanced Finance Deep Services (Batches 1-8)", () => {
  ...
  describe("TaxEngineDeepService", () => {
    ...
    it("should create tax jurisdiction", async () => {
      ...
      vi.mocked(prisma.taxJurisdiction.create).mockResolvedValue(
        mockResult as any,
      );
    ...
    it("should compute tax reconciliation", async () => {
      ...
      vi.mocked(prisma.taxReconciliation.create).mockResolvedValue(
        mockRec as any,
      );

      const result = await service.computeTaxReconciliation(
        "tenant-1",
        "org-1",
        {
  ...
  describe("TreasuryDeepService", () => {
    ...
    it("should record debt drawdown", async () => {
      vi.mocked(prisma.debtFacility.findFirst).mockResolvedValue({
        id: "fac-1",
        facilityLimit: 10000,
        drawnAmount: 1000,
      } as any);
      vi.mocked(prisma.debtFacility.update).mockResolvedValue({
        id: "fac-1",
        drawnAmount: 2000,
      } as any);

      const result = await service.recordDebtDrawdown(
        "tenant-1",
        "fac-1",
        1000,
      );
  ...
  describe("ApIntelligenceService", () => {
    ...
    it("should scan for duplicates", async () => {
      ...
      vi.mocked(prisma.aPDuplicateFlag.create).mockResolvedValue({
        id: "flag-1",
      } as any);
    ...
    it("should get GRNI aging categories", async () => {
      ...
      vi.mocked(prisma.grniRecord.findMany).mockResolvedValue(
        mockRecords as any,
      );
  ...
  describe("ArCollectionsService", () => {
    ...
    it("should check broken promises", async () => {
      ...
      vi.mocked(prisma.aRPromiseToPay.findMany).mockResolvedValue(
        mockBroken as any,
      );
      vi.mocked(prisma.aRPromiseToPay.update).mockResolvedValue({
        id: "p-1",
        status: "BROKEN",
      } as any);
    ...
    it("should calculate bad debt provisions", async () => {
      ...
      const result = await service.computeBadDebtProvision(
        "tenant-1",
        "2026-07",
      );
  ...
  describe("FixedAssetDeepService", () => {
    ...
    it("should preview depreciation schedule", async () => {
      ...
      const result = await service.getDepreciationSchedulePreview(
        "tenant-1",
        "asset-1",
      );
  ...
  describe("RevenueBillingService", () => {
    ...
    it("should calculate revenue schedule recognition asOf", async () => {
      ...
      const result = (service as any).calculateRecognizedAsOf(
        mockSchedule,
        new Date("2026-06-30"),
      );
  ...
  describe("ComplianceControlsService", () => {
    ...
    it("should compute control effectiveness pass rates", async () => {
      ...
      vi.mocked(prisma.controlTest.findMany).mockResolvedValue(
        mockTests as any,
      );
