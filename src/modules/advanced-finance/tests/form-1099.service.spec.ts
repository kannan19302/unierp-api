...
describe("Form1099Service", () => {
  ...
  describe("getVendorProfile", () => {
    it("throws NotFoundException when vendor missing", async () => {
      ...
      await expect(service.getVendorProfile("tenant-1", "v1")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("upsertVendorProfile", () => {
    it("upserts a profile with defaults", async () => {
      ...
      vi.mocked(prisma.vendor1099Profile.upsert).mockResolvedValue({
        id: "p1",
        is1099Vendor: true,
      } as any);

      const result = await service.upsertVendorProfile("tenant-1", "v1", {
        is1099Vendor: true,
      });
  ...
  describe("runTinMatch", () => {
    it("throws BadRequestException if no profile exists", async () => {
      ...
      await expect(service.runTinMatch("tenant-1", "v1")).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("marks MATCHED when TIN and name are present", async () => {
      ...
      vi.mocked(prisma.vendor1099Profile.findUnique).mockResolvedValue({
        taxIdMasked: "XX-1234",
      } as any);
      vi.mocked(prisma.vendor1099Profile.update).mockResolvedValue({
      ...
      expect(prisma.vendor1099Profile.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ tinMatchStatus: "MATCHED" }),
        }),
      );
  ...
  describe("setBackupWithholding", () => {
    it("rejects an invalid rate", async () => {
      vi.mocked(prisma.vendor.findFirst).mockResolvedValue({ id: "v1" } as any);
      await expect(
        service.setBackupWithholding("tenant-1", "v1", true, 150),
  ...
  describe("getForm", () => {
    it("throws NotFoundException when form missing", async () => {
      ...
      await expect(service.getForm("tenant-1", "f1")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("scopes the lookup by tenantId — a form belonging to another tenant is not found", async () => {
      ...
      vi.mocked(prisma.form1099.findFirst).mockResolvedValue(null);
      await expect(service.getForm("tenant-other", "f1")).rejects.toThrow(
        NotFoundException,
      );
      expect(prisma.form1099.findFirst).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: "f1", tenantId: "tenant-other" },
        }),
      );
    });
  });
  ...
  describe("updateForm", () => {
    it("rejects editing a non-DRAFT form", async () => {
      ...
      await expect(
        service.updateForm("tenant-1", "f1", "user-1", { notes: "x" }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("recomputes totalAmount from new box amounts", async () => {
      ...
      vi.mocked(prisma.form1099.update).mockResolvedValue({
        id: "f1",
        totalAmount: 700,
      } as any);

      await service.updateForm("tenant-1", "f1", "user-1", {
        boxAmounts: { "1": 700 },
      });
      expect(prisma.form1099.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ boxAmounts: { "1": 700 } }),
        }),
      );
    });
  });
  ...
  describe("markReady / fileForm", () => {
    it("only allows marking DRAFT forms ready", async () => {
      ...
      await expect(
        service.markReady("tenant-1", "f1", "user-1"),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("only allows filing READY forms", async () => {
      ...
      await expect(
        service.fileForm("tenant-1", "f1", "user-1"),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe("correctForm", () => {
    it("rejects correcting a non-FILED form", async () => {
      ...
      await expect(
        service.correctForm("tenant-1", "f1", "user-1", {
          boxAmounts: { "1": 100 },
        }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("creates a correction linked to the original", async () => {
      ...
      vi.mocked(prisma.form1099.create).mockResolvedValue({
        id: "f2",
        isCorrection: true,
        correctedFromId: "f1",
      } as any);

      const result = await service.correctForm("tenant-1", "f1", "user-1", {
        boxAmounts: { "1": 900 },
      });
  ...
  describe("createBatch", () => {
    it("rejects an empty form list", async () => {
      await expect(
        service.createBatch("tenant-1", "user-1", {
          taxYear: 2026,
          name: "Batch A",
          formIds: [],
        }),
    ...
    it("rejects if not all forms are READY / found", async () => {
      vi.mocked(prisma.form1099.findMany).mockResolvedValue([
        { id: "f1", totalAmount: 100 },
      ] as any);
      await expect(
        service.createBatch("tenant-1", "user-1", {
          taxYear: 2026,
    ...
    it("creates a batch and links forms", async () => {
      vi.mocked(prisma.form1099.findMany).mockResolvedValue([
        { id: "f1", totalAmount: 100 },
        { id: "f2", totalAmount: 200 },
      ] as any);
      vi.mocked(prisma.form1099Batch.create).mockResolvedValue({
        id: "b1",
        formCount: 2,
        totalAmount: 300,
      } as any);
      vi.mocked(prisma.form1099.updateMany).mockResolvedValue({
        count: 2,
      } as any);

      const result = await service.createBatch("tenant-1", "user-1", {
        taxYear: 2026,
        name: "Batch A",
        formIds: ["f1", "f2"],
      });
      expect(prisma.form1099.updateMany).toHaveBeenCalledWith({
        where: { id: { in: ["f1", "f2"] }, tenantId: "tenant-1" },
        data: { batchId: "b1" },
      });
  ...
  describe("efileBatch", () => {
    it("rejects a batch not in GENERATED status", async () => {
      vi.mocked(prisma.form1099Batch.findFirst).mockResolvedValue({
        id: "b1",
        status: "SUBMITTED",
        forms: [],
      } as any);
      await expect(
        service.efileBatch("tenant-1", "b1", "user-1"),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("submits a GENERATED batch and marks its forms FILED", async () => {
      ...
      vi.mocked(prisma.form1099Batch.update).mockResolvedValue({
        id: "b1",
        status: "SUBMITTED",
      } as any);
      vi.mocked(prisma.form1099.updateMany).mockResolvedValue({
