...
describe("AllocationService", () => {
  ...
  describe("Rules CRUD", () => {
    it("should list all rules", async () => {
      ...
      vi.mocked(prisma.allocationRule.findMany).mockResolvedValue(
        mockRules as any,
      );
    ...
    it("should throw on getRuleById if not found", async () => {
      ...
      await expect(service.getRuleById("tenant-1", "rule-1")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("should create a rule", async () => {
      ...
      vi.mocked(prisma.allocationRule.create).mockResolvedValue(
        mockCreated as any,
      );
    ...
    it("should throw on createRule if static percentage total is not 100%", async () => {
      ...
      await expect(
        service.createRule("tenant-1", dto, "user-1"),
  ...
  describe("executeAllocationRun", () => {
    it("should execute static percentage run", async () => {
      ...
      vi.mocked(prisma.allocationRule.findFirst).mockResolvedValue(
        mockRule as any,
      );
      ...
      vi.mocked(prisma.allocationRun.create).mockResolvedValue(
        mockRunCreated as any,
      );

      const dto = { periodStart: "2026-01-01", periodEnd: "2026-01-31" };
      const result = await service.executeAllocationRun(
        "tenant-1",
        "org-1",
        "rule-1",
        dto,
        ...
        "org-1",
        {
          ...
          entries: [
            ...
            },
            {
              ...
              credit: 1000,
              description:
                "Allocation source pool clearing for rule: Static rule",
            },
          ],
        },
      );
