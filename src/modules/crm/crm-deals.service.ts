...
export class CrmDealsService {
  ...
  async getOpportunitySummary(tenantId: string, id: string) {
    ...
    const daysInCurrentStage = Math.floor(
      (now.getTime() - stageEnteredRef.getTime()) / (1000 * 60 * 60 * 24),
    );
    ...
    const lineItemsTotal = opp.lineItems.reduce(
      (sum, li) => sum + Number(li.totalAmount),
      0,
    );
  ...
  async addOpportunityLineItem(
    ...
    if (!opp) throw new NotFoundException("Opportunity not found");
    const totalAmount =
      dto.quantity * dto.unitPrice * (1 - (dto.discount || 0) / 100);
  ...
  async updateOpportunityLineItem(
    ...
    const item = await prisma.opportunityLineItem.findFirst({
      where: { id: itemId, tenantId, opportunityId },
    });
  ...
  private async recalcOpportunityAmount(opportunityId: string) {
    ...
    await prisma.opportunity.update({
      ...
      data: {
        ...
        weightedAmount: new Prisma.Decimal((total * prob) / 100),
      },
    });
  }
  ...
  async addPriceBookEntry(
    ...
    const pb = await prisma.priceBook.findFirst({
      where: { id: priceBookId, tenantId, deletedAt: null },
    });
  ...
  async getCrmProducts(
    ...
    if (query?.sortBy === "name") orderBy.name = query.sortOrder || "asc";
    else if (query?.sortBy === "sellPrice")
      orderBy.sellPrice = query.sortOrder || "asc";
    else orderBy.name = "asc";
    const [data, totalCount] = await Promise.all([
  ...
  async updateCrmProduct(
    ...
    if (dto.status) {
      ...
      updateData.discontinuedAt =
        dto.status === "DISCONTINUED" ? new Date() : null;
    }
  ...
  async getWinRate(tenantId: string) {
    ...
    const lost = await prisma.opportunity.count({
  ...
  async getRevenueForecast(tenantId: string) {
    ...
    const byMonth: Record<
      string,
      { totalAmount: number; weightedAmount: number; count: number }
    > = {};
    for (const opp of opps) {
      const month = opp.expectedCloseDate
        ...
        : "Unscheduled";
      if (!byMonth[month])
        byMonth[month] = { totalAmount: 0, weightedAmount: 0, count: 0 };
  ...
  async getStageDuration(tenantId: string) {
    ...
    const totalDays = opps.reduce((sum, o) => {
      ...
      return (
        sum + (end.getTime() - o.createdAt.getTime()) / (1000 * 60 * 60 * 24)
      );
    }, 0);
    return {
      avgDaysToClose: Math.round(totalDays / opps.length),
  ...
  async getPipelineHealth(tenantId: string) {
    ...
    const avgAge =
      open.length > 0
        ? Math.round(
            open.reduce(
              (s, o) => s + (now.getTime() - o.createdAt.getTime()) / 86400000,
              0,
            ) / open.length,
          )
        : 0;
      ...
      won.length > 0
        ? Math.round(
            won.reduce(
              (s, o) =>
                s +
                ((o.actualCloseDate || now).getTime() - o.createdAt.getTime()) /
                  86400000,
              0,
            ) / won.length,
          )
    ...
    const weightedPipeline = open.reduce(
      (s, o) => s + Number(o.amount || 0) * (o.probability / 100),
      0,
    );
  ...
  async getForecast(tenantId: string) {
    ...
    const worstCase = opps
      .filter((o) => o.probability >= 90)
      .reduce((s, o) => s + Number(o.amount || 0), 0);
    return {
      bestCase,
      commit,
  ...
  async getWeightedForecastByRep(tenantId: string) {
    ...
    const byRep: Record<
      string,
      { pipelineAmount: number; weightedAmount: number; openDeals: number }
    > = {};
    for (const opp of opps) {
      const rep = opp.assignedToId!;
      if (!byRep[rep])
        byRep[rep] = { pipelineAmount: 0, weightedAmount: 0, openDeals: 0 };
      byRep[rep].pipelineAmount += Number(opp.amount || 0);
      byRep[rep].weightedAmount +=
        Number(opp.amount || 0) * (opp.probability / 100);
      byRep[rep].openDeals++;
    }
    return Object.entries(byRep)
      .map(([userId, d]) => ({
        ...
        pipelineAmount: d.pipelineAmount,
        weightedAmount: Math.round(d.weightedAmount),
  ...
  async getRepPerformance(tenantId: string) {
    ...
    const byRep: Record<
      string,
      { deals: number; revenue: number; totalDays: number }
    > = {};
    for (const opp of wonOpps) {
      ...
      const days =
        (opp.actualCloseDate || new Date()).getTime() - opp.createdAt.getTime();
  ...
  async getConversionFunnel(tenantId: string) {
    ...
    return stages.map((s) => ({
      ...s,
      percentage: Math.round((s.value / max) * 100),
    }));
  }
  ...
  async getCohortAnalysis(tenantId: string) {
    ...
    for (const l of leads) {
      ...
      if (!cohorts[m])
        cohorts[m] = { total: 0, converted: 0, qualified: 0, disqualified: 0 };
      cohorts[m].total++;
  ...
  async updatePlaybook(tenantId: string, id: string, dto: UpdatePlaybookInput) {
    const existing = await prisma.salesPlaybook.findFirst({
      where: { id, tenantId, deletedAt: null },
    });
  ...
  async createBattlecard(
    ...
    return prisma.battlecard.create({
      data: {
        ...
        strengths: dto.strengths as Prisma.InputJsonValue,
  ...
  async validateStageAdvance(
    ...
    const stageConfig = playbook.stages.find(
      (s: { stageName: string }) => s.stageName === targetStage,
    );
