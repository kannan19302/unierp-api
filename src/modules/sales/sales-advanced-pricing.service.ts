...
export class SalesAdvancedPricingService {
  async getCustomerPriceLists(tenantId: string, customerId?: string) {
    const where: Prisma.CustomerPriceListWhereInput = {
      tenantId,
      deletedAt: null,
    };
    if (customerId) where.customerId = customerId;
    return prisma.customerPriceList.findMany({
      where,
  ...
  async getCustomerPriceListById(tenantId: string, id: string) {
    const pl = await prisma.customerPriceList.findFirst({
        ...
        items: dto.items?.length
          ? {
              create: dto.items.map((i: any) => ({
                ...
                minQuantity: new Prisma.Decimal(i.minQuantity || 1),
              })),
            }
          : undefined,
      },
  ...
  async updateCustomerPriceList(tenantId: string, id: string, dto: any) {
    const existing = await prisma.customerPriceList.findFirst({
      where: { id, tenantId },
    });
    ...
    return prisma.customerPriceList.update({
      where: { id },
      data,
      include: { items: true },
    });
  ...
  async addPriceListItem(tenantId: string, priceListId: string, dto: any) {
    ...
    if (!pl) throw new NotFoundException("Customer price list not found");
    return prisma.customerPriceListItem.create({
      data: {
        tenantId,
        priceListId,
  ...
  async removePriceListItem(tenantId: string, itemId: string) {
    const item = await prisma.customerPriceListItem.findFirst({
  ...
  async getContractPricing(tenantId: string, contractId?: string) {
    ...
    return (
      (prisma as any).contractPricingOverride?.findMany?.({
        where,
        orderBy: { createdAt: "desc" },
      }) ?? []
    );
  ...
  async createContractPricing(tenantId: string, orgId: string, dto: any) {
    ...
    return (prisma as any).contractPricingOverride.create({
      data: {
        tenantId,
        orgId: resolvedOrgId,
  ...
  async updateContractPricing(tenantId: string, id: string, dto: any) {
    const existing = await (prisma as any).contractPricingOverride.findFirst({
      where: { id, tenantId },
    });
    ...
    if (dto.discountPct !== undefined)
      data.discountPct = new Prisma.Decimal(dto.discountPct);
    if (dto.expiryDate !== undefined)
      data.expiryDate = dto.expiryDate ? new Date(dto.expiryDate) : null;
    if (dto.isActive !== undefined) data.isActive = dto.isActive;
    return (prisma as any).contractPricingOverride.update({
      where: { id },
      data,
    });
  }
  ...
  async createFloorPrice(tenantId: string, orgId: string, dto: any) {
    ...
    return prisma.floorPriceOverride.create({
      data: {
        ...
        floorPrice: new Prisma.Decimal(dto.floorPrice),
        currency: dto.currency || "USD",
      },
    });
  }
  ...
  async updateFloorPrice(tenantId: string, id: string, dto: any) {
    const existing = await prisma.floorPriceOverride.findFirst({
      where: { id, tenantId },
    });
    ...
    if (dto.floorPrice !== undefined)
      data.floorPrice = new Prisma.Decimal(dto.floorPrice);
    if (dto.isActive !== undefined) data.isActive = dto.isActive;
  ...
  async approveFloorPrice(
    ...
    return prisma.floorPriceOverride.update({
      where: { id },
      data: {
        isActive: approved,
  ...
  async calculateTieredPrice(tenantId: string, dto: any) {
    ...
    if (dto.quantity >= 1000) {
      discountPct = 15;
      appliedRule = "TIER_1000+";
    } else if (dto.quantity >= 500) {
      discountPct = 12;
      appliedRule = "TIER_500+";
    } else if (dto.quantity >= 250) {
      discountPct = 10;
      appliedRule = "TIER_250+";
    } else if (dto.quantity >= 100) {
      discountPct = 7;
      appliedRule = "TIER_100+";
    ...
    if (dto.customerId && !dto.priceListId) {
      ...
      if (priceList?.items.length) {
        const matched =
          priceList.items.find((i) => Number(i.minQuantity) <= dto.quantity) ||
          priceList.items[priceList.items.length - 1];
    ...
    // Contract pricing
    if (dto.customerId) {
      const contractPricing = await (
        prisma as any
      ).contractPricingOverride?.findFirst?.({
        where: {
          tenantId,
          productId: dto.productId,
          isActive: true,
      ...
      if (
        contractPricing &&
        Number(contractPricing.unitPrice) < finalUnitPrice
      ) {
    ...
    // Floor price check
    const floor = await (prisma as any).floorPriceOverride?.findFirst?.({
      where: {
        tenantId,
        productId: dto.productId,
        isActive: true,
    ...
    if (floorPrice && finalUnitPrice < floorPrice) {
      return {
        needsApproval: true,
        message: `Price ${finalUnitPrice} below floor ${floorPrice}`,
        basePrice,
        finalUnitPrice: floorPrice,
        floorPrice,
        discountPct,
        appliedRule: "FLOOR_PRICING_APPROVAL_REQUIRED",
        priceListName,
        contractName,
      };
    }
    ...
    return {
      ...
      finalUnitPrice,
      discountPct,
      totalPrice: Math.round(finalUnitPrice * dto.quantity * 100) / 100,
      appliedRule,
      priceListName,
      contractName,
      floorPrice,
  ...
  async getPricingAnalytics(tenantId: string) {
    const [priceLists, overrides, floors] = await Promise.all([
      ...
      (prisma as any).contractPricingOverride?.count?.({
        where: { tenantId, isActive: true },
      }) ?? 0,
      (prisma as any).floorPriceOverride?.count?.({
        where: { tenantId, isActive: true },
      }) ?? 0,
    ]);
    return {
      activePriceLists: priceLists,
      activeContractOverrides: overrides,
      activeFloorPrices: floors,
    };
  }
}
