...
export type CreateAutoEnrollRuleInput = z.infer<
  typeof createAutoEnrollRuleSchema
>;
export type UpdateAutoEnrollRuleInput = z.infer<
  typeof updateAutoEnrollRuleSchema
>;
...
export class CrmCadencesService {
  ...
  async createAutoEnrollRule(
    tenantId: string,
    orgId: string,
    dto: CreateAutoEnrollRuleInput,
  ) {
    const sequence = await prisma.emailSequence.findFirst({
      where: { id: dto.sequenceId, tenantId, deletedAt: null },
    });
  ...
  async updateAutoEnrollRule(
    tenantId: string,
    id: string,
    dto: UpdateAutoEnrollRuleInput,
  ) {
    const existing = await prisma.cadenceAutoEnrollRule.findFirst({
      where: { id, tenantId, deletedAt: null },
    });
  ...
  private matchesLead(
    lead: { industry: string | null; score: number; sourceId: string | null },
    conditions: EnrollConditions,
  ): boolean {
    if (
      conditions.industries?.length &&
      (!lead.industry || !conditions.industries.includes(lead.industry))
    )
      return false;
    if (conditions.minScore !== undefined && lead.score < conditions.minScore)
      return false;
    if (
      conditions.sourceIds?.length &&
      (!lead.sourceId || !conditions.sourceIds.includes(lead.sourceId))
    )
      return false;
    return true;
  }
  ...
  async evaluateAutoEnrollForLead(tenantId: string, leadId: string) {
    ...
    const rules = await prisma.cadenceAutoEnrollRule.findMany({
      where: { tenantId, isActive: true, deletedAt: null, entityType: "LEAD" },
    });
    const enrolled: string[] = [];
    for (const rule of rules) {
      if (!this.matchesLead(lead, (rule.conditions ?? {}) as EnrollConditions))
        continue;
      const already = await prisma.emailSequenceEnrollment.findFirst({
        where: {
          tenantId,
      ...
      const firstStep = await prisma.emailSequenceStep.findFirst({
        where: { sequenceId: rule.sequenceId },
      ...
      await prisma.emailSequenceEnrollment.create({
        data: { tenantId, sequenceId: rule.sequenceId, leadId, nextSendAt },
      });
  ...
  async processDueSteps(tenantId: string) {
    ...
    for (const enrollment of due) {
      ...
      if (step.channel !== "EMAIL") {
        const existingTask = await prisma.cadenceStepTask.findFirst({
    ...
    return {
      ...
      emailStepsAdvanced: advanced,
      tasksCreated,
    };
  }

  /** Rep marks a CALL/TASK/LINKEDIN step task done (or skipped), which advances the enrollment to the next step. */
  async completeStepTask(
    tenantId: string,
    taskId: string,
    dto: CompleteStepTaskInput,
    completedBy: string,
  ) {
    const task = await prisma.cadenceStepTask.findFirst({
    ...
    if (task.status !== "PENDING")
      throw new BadRequestException("Task already resolved");

    await prisma.cadenceStepTask.update({
      where: { id: taskId },
      data: {
        status: dto.status,
        completedAt: new Date(),
    ...
    return {
      taskId,
      status: dto.status,
      enrollmentAdvancedToStep: enrollment.currentStep + 1,
    };
  }
  ...
  async listMyStepTasks(tenantId: string, status?: string) {
    return prisma.cadenceStepTask.findMany({
      ...
      include: {
        step: true,
        enrollment: { include: { sequence: { select: { name: true } } } },
      },
      orderBy: { dueAt: "asc" },
      take: 200,
    });
