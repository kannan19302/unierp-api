...
import {
  ...
  updateBadgeSchema,
  UpdateBadgeInput,
} from "../services/crm-gamification.service";
...
export class CrmGamificationController {
  ...
  @ApiOperation({
    summary: "Recompute + persist the leaderboard snapshot for a period",
  })
  ...
  async recomputeLeaderboard(
    ...
    return this.svc.computeLeaderboard(
      req.user.tenantId,
      req.user.orgId ?? "",
      period,
    );
  ...
  @ApiOperation({
    summary: "Recompute activity/deals-won streaks for every rep",
  })
  ...
  async createBadge(
    @Req() req: AuthenticatedRequest,
    @ZodBody(createBadgeSchema) dto: CreateBadgeInput,
  ) {
  ...
  async updateBadge(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(updateBadgeSchema) dto: UpdateBadgeInput,
  ) {
  ...
  @ApiOperation({
    summary:
      "Evaluate every active badge against current deal/activity data and award new ones",
  })
  ...
  @ApiOperation({
    summary: "Get my own gamification summary (rank, badges, streaks)",
  })
  @Get("me")
  @Permissions("crm.commission.read")
  async mySummary(
