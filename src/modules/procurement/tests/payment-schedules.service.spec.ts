...
describe("PaymentSchedulesService", () => {
  ...
  it("should create payment schedule", async () => {
    (prisma.paymentSchedule.create as any).mockResolvedValue({
      id: "new-ps",
      vendor: { name: "Vendor A" },
    });
    const result = await service.create("tenant-1", "org-1", {
      vendorId: "vendor-1",
  ...
  it("should bulk create from PO", async () => {
    ...
    const result = await service.bulkCreateFromPo("tenant-1", "org-1", "po-1", [
      { dueDate: new Date().toISOString(), amount: 500 },
    ]);
  ...
  it("should get upcoming payments", async () => {
    (prisma.paymentSchedule.findMany as any).mockResolvedValue([
      { vendor: { name: "Vendor A" }, amount: 500, dueDate: new Date() },
    ]);
