...
export class VmiService {
  ...
  async createAgreement(
    ...
    if (dto.minQty < 0)
      throw new BadRequestException("minQty must be non-negative");
    if (dto.minQty >= dto.maxQty)
      throw new BadRequestException("minQty must be less than maxQty");
    if (dto.targetQty < dto.minQty || dto.targetQty > dto.maxQty) {
      throw new BadRequestException(
        "targetQty must be between minQty and maxQty",
      );
    }
    ...
    return prisma.vmiAgreement.create({
      data: {
        ...
        reviewCycleDays: dto.reviewCycleDays ?? 7,
        vendorLeadDays: dto.vendorLeadDays ?? 3,
  ...
  async activateAgreement(tenantId: string, id: string) {
    ...
    return prisma.vmiAgreement.update({
      where: { id },
  ...
  async suspendAgreement(tenantId: string, id: string) {
    ...
    if (!ag) throw new NotFoundException("VMI agreement not found");
    if (ag.status !== "ACTIVE")
      throw new BadRequestException("Only ACTIVE agreements can be suspended");
    return prisma.vmiAgreement.update({
      where: { id },
  ...
  async terminateAgreement(tenantId: string, id: string) {
    const ag = await prisma.vmiAgreement.findFirst({ where: { tenantId, id } });
    if (!ag) throw new NotFoundException("VMI agreement not found");
    if (ag.status === "TERMINATED")
      throw new BadRequestException("Agreement already terminated");
    return prisma.vmiAgreement.update({
      where: { id },
  ...
  async recordSnapshot(
    ...
    const ag = await prisma.vmiAgreement.findFirst({
      where: { tenantId, id: dto.agreementId },
    });
    if (!ag) throw new NotFoundException("VMI agreement not found");
  ...
  async createOrder(
    ...
    if (dto.orderedQty <= 0)
      throw new BadRequestException("orderedQty must be positive");
    const ag = await prisma.vmiAgreement.findFirst({
      where: { tenantId, id: dto.agreementId },
    });
    if (!ag) throw new NotFoundException("VMI agreement not found");
    ...
    return prisma.vmiOrder.create({
      data: {
        ...
        triggeredBy: dto.triggeredBy as any,
  ...
  async advanceOrderStatus(
    tenantId: string,
    id: string,
    dto: { status: string; receivedQty?: number; cancelReason?: string },
  ) {
    ...
    if (dto.status === "RECEIVED") {
      ...
      if (dto.receivedQty != null) data.receivedQty = dto.receivedQty;
    }
