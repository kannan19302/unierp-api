...
describe("CrmTerritoryRulesService", () => {
  ...
  it("creates a rule after verifying the territory exists", async () => {
    (
      prisma.salesTerritory.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "t1" });
    (
      prisma.territoryAssignmentRule.create as ReturnType<typeof vi.fn>
  ...
  it("assigns a lead to the highest-priority matching GEOGRAPHY rule and logs it", async () => {
    ...
    (
      prisma.territoryAssignmentRule.findMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue([
      {
        id: "r1",
        ruleType: "GEOGRAPHY",
    ...
    (
      prisma.territoryAssignmentLog.create as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
  ...
  it('leaves a lead unassigned and logs "no rule matched" when nothing matches', async () => {
    ...
    expect(prisma.territoryAssignmentLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ reason: "No territory rule matched" }),
      }),
    );
  });
  ...
  it("round-robins across territory members using the persisted cursor", async () => {
    ...
    (
      prisma.salesTeamMember.findMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue([
      { userId: "u1", createdAt: new Date() },
      { userId: "u2", createdAt: new Date() },
    ]);
    (
      prisma.territoryRoundRobinState.findUnique as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ lastMemberIndex: 0 });
    (
      prisma.territoryRoundRobinState.upsert as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
    (prisma.lead.update as ReturnType<typeof vi.fn>).mockResolvedValue({});
    (
      prisma.territoryAssignmentLog.create as ReturnType<typeof vi.fn>
