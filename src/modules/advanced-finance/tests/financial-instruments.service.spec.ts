...
vi.mock("@unerp/database", () => {
  ...
  return {
    prisma: { fairValueMeasurement: m(), expectedCreditLossProvision: m() },
  };
});
...
describe("FinancialInstrumentsService", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new FinancialInstrumentsService();
  });

  it("getFairValueMeasurements returns measurements", async () => {
    vi.mocked(prisma.fairValueMeasurement.findMany).mockResolvedValue([
      { id: "fv-1" },
    ] as any);
  ...
  it("getFairValueMeasurementById throws on not found", async () => {
    ...
    await expect(
      service.getFairValueMeasurementById("t1", "bad"),
    ).rejects.toThrow(NotFoundException);
  });

  it("createFairValueMeasurement computes unrealized GL", async () => {
    vi.mocked(prisma.fairValueMeasurement.create).mockResolvedValue({
      id: "fv-1",
    } as any);
    const result = await service.createFairValueMeasurement("t1", "o1", {
  ...
  it("getLatestMeasurement throws if no measurements", async () => {
    vi.mocked(prisma.fairValueMeasurement.findFirst).mockResolvedValue(null);
    await expect(
      service.getLatestMeasurement("t1", "EQUITY", "inst-1"),
    ).rejects.toThrow(NotFoundException);
  });

  it("getFairValueHierarchySummary aggregates by level", async () => {
    vi.mocked(prisma.fairValueMeasurement.findMany).mockResolvedValue([
      { fairValue: 100, unrealizedGL: 10, hierarchyLevel: "LEVEL_1" },
      { fairValue: 200, unrealizedGL: 20, hierarchyLevel: "LEVEL_2" },
    ] as any);
  ...
  it("getExpectedCreditLossProvisions returns provisions", async () => {
    vi.mocked(prisma.expectedCreditLossProvision.findMany).mockResolvedValue([
      { id: "ecl-1" },
    ] as any);
  ...
  it("createExpectedCreditLossProvision computes expected loss", async () => {
    vi.mocked(prisma.expectedCreditLossProvision.create).mockResolvedValue({
      id: "ecl-1",
    } as any);
    const result = await service.createExpectedCreditLossProvision("t1", "o1", {
      provisionDate: "2026-06-30",
      period: "2026-06",
      stage: "STAGE_1",
      grossCarryingAmount: 10000,
      lossRate: 0.02,
      methodology: "ROLL_RATE",
  ...
  it("reviewExpectedCreditLossProvision throws if not draft", async () => {
    vi.mocked(prisma.expectedCreditLossProvision.findFirst).mockResolvedValue({
      id: "ecl-1",
      status: "REVIEWED",
    } as any);
    await expect(
      service.reviewExpectedCreditLossProvision("t1", "ecl-1", "user1"),
  ...
  it("computeLossRate returns avg loss rate", async () => {
    ...
    const result = await service.computeLossRate(
      "t1",
      "o1",
      "TRADE_RECEIVABLES",
    );
