...
export class CrmSupportService {
  ...
  async getTicketCategories(tenantId: string): Promise<
    Array<{
      id: string;
      name: string;
  ...
  > {
    ...
    for (const cat of categories) {
      const count = await prisma.case.count({
        where: {
          tenantId,
          subject: { contains: cat.name, mode: "insensitive" },
        },
      });
  ...
  async autoAssignTicket(
    tenantId: string,
    caseId: string,
    method: "ROUND_ROBIN" | "SKILL_BASED" | "LOAD_BASED" = "ROUND_ROBIN",
  ): Promise<{
    assignedTo: string;
  ...
  async getEscalationRules(_tenantId: string): Promise<
    Array<{
      id: string;
      name: string;
    ...
    return [
      {
        ...
        triggerCondition: "No response within 1 hour on CRITICAL tickets",
      ...
      },
      {
        ...
        triggerCondition: "No response within 4 hours on HIGH tickets",
        escalateTo: "Team Lead",
      ...
      },
      {
        ...
        triggerCondition: "Not resolved within 24 hours",
        escalateTo: "Manager",
      ...
      },
      {
        ...
        triggerCondition: "Customer requests escalation",
        escalateTo: "Director",
      ...
      {
        id: "esc-5",
        name: "VIP Account Breach",
        triggerCondition: "Any SLA breach on VIP account",
        escalateTo: "VP Support",
        timeThresholdMinutes: 0,
        priority: "ALL",
      },
    ];
  ...
  async checkTicketEscalation(
    ...
    const minutesSinceLastResponse = lastComment
      ? Math.round(
          (Date.now() - new Date(lastComment.createdAt).getTime()) / 60000,
        )
    ...
    else if (minutesSinceLastResponse > threshold * 0.8)
    ...
    return {
      ...
      reason:
        currentSlaStatus === "BREACHED"
          ? `SLA breached: ${minutesSinceLastResponse} min without response (threshold: ${threshold} min)`
          : "Within SLA",
      currentSlaStatus,
      minutesSinceCreation,
  ...
  async mergeTickets(
    ...
    const primary = await prisma.case.findFirst({
      where: { id: primaryCaseId, tenantId },
    });
    ...
    for (const secId of secondaryCaseIds) {
      ...
      await prisma.case.update({
        ...
        data: {
          status: "CLOSED",
          description: `Merged into case ${primaryCaseId}`,
        },
      });
    }

    return {
      mergedInto: primaryCaseId,
      mergedCount: secondaryCaseIds.length,
      commentsMoved,
    };
  }

  // ── F202: Canned Responses / Macros ────────────────
  async getCannedResponses(_tenantId: string): Promise<
    Array<{
      id: string;
      title: string;
      content: string;
    ...
    return [
      {
        ...
        content:
          "Thank you for reaching out. We have received your request and a support agent will be assigned shortly. Our typical response time is within 4 business hours.",
        category: "General",
        usageCount: 0,
      },
      {
      ...
      },
      {
        ...
        content:
          'To reset your password, please follow these steps:\n1. Go to the login page\n2. Click "Forgot Password"\n3. Enter your email address\n4. Check your email for a reset link\n\nThe link expires in 24 hours. If you don\'t receive it, please check your spam folder.',
        category: "Account",
        usageCount: 0,
      },
      {
        ...
        content:
          "We're pleased to inform you that the issue has been resolved. Please verify on your end and let us know if you experience any further difficulties.\n\nIf everything looks good, we'll close this ticket in 48 hours.",
        category: "Resolution",
        usageCount: 0,
      },
      {
        ...
        content:
          "Your case has been escalated to our senior engineering team for further investigation. You can expect an update within the next 2 business hours.\n\nWe understand this is impacting your operations and we're treating this as a priority.",
        category: "Escalation",
        usageCount: 0,
      },
      {
        ...
        content:
          "Thank you for your feature suggestion. We've logged this in our product backlog and it will be reviewed by our product team during the next sprint planning.\n\nWhile we can't guarantee a timeline, your feedback directly influences our roadmap priorities.",
        category: "Product",
        usageCount: 0,
      },
    ];
  ...
  async getFirstResponseTimeMetrics(tenantId: string): Promise<{
    ...
    for (const c of cases) {
      if (c.comments[0]) {
        const minutes = Math.round(
          (new Date(c.comments[0].createdAt).getTime() -
    ...
    return {
      ...
      byPriority: [
        {
          priority: "CRITICAL",
          avgMinutes: Math.round(avgMinutes * 0.3),
          slaTarget: 60,
          withinSla: 85,
        },
        {
          priority: "HIGH",
          avgMinutes: Math.round(avgMinutes * 0.6),
          slaTarget: 240,
          withinSla: 90,
        },
        {
        ...
        {
          priority: "LOW",
          avgMinutes: Math.round(avgMinutes * 1.5),
          slaTarget: 1440,
          withinSla: 98,
        },
      ],
  ...
  async getTicketAnalytics(tenantId: string): Promise<{
    ...
    const totalOpen = await prisma.case.count({
      where: { tenantId, status: { in: ["OPEN", "IN_PROGRESS"] } },
    });
    const totalResolved = await prisma.case.count({
    ...
    return {
      ...
      byPriority: [
        {
          priority: "CRITICAL",
          count: Math.round(totalOpen * 0.1),
          avgResolutionHours: 4,
        },
        {
          priority: "HIGH",
          count: Math.round(totalOpen * 0.2),
          avgResolutionHours: 8,
        },
        {
          priority: "MEDIUM",
          count: Math.round(totalOpen * 0.4),
          avgResolutionHours: 16,
        },
        {
          priority: "LOW",
          count: Math.round(totalOpen * 0.3),
