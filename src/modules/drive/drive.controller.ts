...
export class DriveController {
  ...
  async getFolders(
    ...
    return this.svc.getFolders(req.user.tenantId, {
  ...
  async createFolder(
    @Req() req: AuthReq,
    @ZodBody(createDriveFolderSchema) body: CreateDriveFolderInput,
  ) {
    const folder = await this.svc.createFolder(
      req.user.tenantId,
      req.user.userId,
      body,
    );
    await this.svc.logActivity(
      req.user.tenantId,
      req.user.userId,
      "CREATE_FOLDER",
  ...
  async updateFolder(
    @Req() req: AuthReq,
    @Param("id") id: string,
    @ZodBody(updateDriveFolderSchema) body: UpdateDriveFolderInput,
  ) {
  ...
  async getFiles(@Req() req: AuthReq, @Query() query: Record<string, string>) {
    const page = parseInt(query.page || "1", 10);
    const limit = parseInt(query.limit || "100", 10);
    return this.svc.getFiles(req.user.tenantId, {
  ...
  async createFile(
    ...
    const file = await this.svc.createFile(
      req.user.tenantId,
      req.user.userId,
      body,
    );
  ...
  async createFileVersion(
    ...
    return this.svc.createFileVersion(
      req.user.tenantId,
      fileId,
      req.user.userId,
      body,
  ...
  async createComment(
    @Req() req: AuthReq,
    @Param("fileId") fileId: string,
    @ZodBody(createDriveCommentSchema) body: CreateDriveCommentInput,
  ) {
    return this.svc.createComment(req.user.tenantId, req.user.userId, {
      ...body,
      fileId,
    });
  }
  ...
  async createShareLink(
    @Req() req: AuthReq,
    @Param("fileId") fileId: string,
    @ZodBody(createDriveShareLinkSchema) body: CreateDriveShareLinkInput,
  ) {
    return this.svc.createShareLink(req.user.tenantId, req.user.userId, {
      ...body,
      fileId,
    });
  }
