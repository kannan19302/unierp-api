...
export class BuilderService {
  ...
  async getModuleWithComponents(tenantId: string, id: string) {
    ...
    const workflowIds = components
      ...
      .map((c) => c.refId);
    const dashboardIds = components
      .filter((c) => c.type === "dashboard")
      .map((c) => c.refId);
    const automationIds = components
    ...
    const [forms, workflows, dashboards, automations] = await Promise.all([
      ...
      workflowIds.length > 0
        ? prisma.builderWorkflow.findMany({
            where: { tenantId, id: { in: workflowIds } },
          })
        : [],
      dashboardIds.length > 0
        ? prisma.builderDashboard.findMany({
            where: { tenantId, id: { in: dashboardIds } },
          })
        : [],
  ...
  async addComponentToModule(
    tenantId: string,
    moduleId: string,
    component: { type: string; refId: string; name: string },
  ) {
    const mod = await prisma.builderModule.findFirst({
    ...
    if (
      ...
      throw new BadRequestException(
        "This component is already linked to the app",
      );
    }
  ...
  async addDataModelToModule(
    tenantId: string,
    moduleId: string,
    dataModel: { name: string; fields: any[]; relationships?: any[] },
  ) {
    const mod = await prisma.builderModule.findFirst({
      where: { id: moduleId, tenantId },
    });
    ...
    if (
      dataModels.some(
        (dm) => dm.name.toLowerCase() === dataModel.name.toLowerCase(),
      )
    ) {
      throw new BadRequestException(
        "A data model with this name already exists in the app",
      );
    }
  ...
  async runAppTests(tenantId: string, moduleId: string) {
    ...
    if (components.length === 0) {
      errors.push({
        code: "NO_COMPONENTS",
        severity: "error",
        message: "App has no linked components",
        suggestion: "Add at least one form, workflow, or dashboard to your app",
      });
    } else {
      passed++;
    }
    ...
    if (pages.length === 0) {
      errors.push({
        code: "NO_PAGES",
        severity: "error",
        message: "App has no pages defined",
        suggestion: "Add at least one page to define app navigation",
      });
    } else {
      passed++;
    }
    ...
    if (!mod.icon) {
      errors.push({
        code: "NO_ICON",
        severity: "info",
        message: "App does not have an icon set",
        suggestion:
          "Set an icon for better visual identification in the app store",
      });
    } else {
      passed++;
    }
    ...
    for (const fc of formComponents) {
      ...
      const form = await prisma.builderForm.findFirst({
        where: { id: fc.refId, tenantId },
      });
      if (!form) {
        errors.push({
          code: "ORPHAN_FORM",
          severity: "error",
          message: `Linked form "${fc.name}" (${fc.refId}) not found`,
          suggestion: "Remove the orphaned form reference or recreate the form",
        });
      } else {
        passed++;
        ...
        if (fields.length === 0) {
          errors.push({
            code: "EMPTY_FORM",
            severity: "warning",
            message: `Form "${form.name}" has no fields defined`,
            suggestion: "Add fields to the form using the Form Builder",
          });
        } else {
          passed++;
          ...
          if (missingLabels.length > 0) {
            errors.push({
              code: "MISSING_LABELS",
              severity: "warning",
              message: `${missingLabels.length} required fields in "${form.name}" are missing labels`,
              suggestion: "Add labels to all required fields for better UX",
            });
          } else {
            passed++;
          }
    ...
    for (const wc of workflowComponents) {
      ...
      const wf = await prisma.builderWorkflow.findFirst({
        where: { id: wc.refId, tenantId },
      });
      if (!wf) {
        errors.push({
          code: "ORPHAN_WORKFLOW",
          severity: "error",
          message: `Linked workflow "${wc.name}" (${wc.refId}) not found`,
          suggestion: "Remove the orphaned workflow reference or recreate it",
        });
      } else {
        passed++;
        ...
        if (nodes.length === 0) {
          errors.push({
            code: "EMPTY_WORKFLOW",
            severity: "warning",
            message: `Workflow "${wf.name}" has no nodes defined`,
            suggestion: "Add at least one action node to the workflow",
          });
        } else {
          passed++;
        }
    ...
    for (const dm of dataModels) {
      ...
      if (!dm.fields || dm.fields.length === 0) {
        errors.push({
          ...
          severity: "warning",
          message: `Data model "${dm.name}" has no fields`,
          suggestion: "Add fields to define the data structure",
        });
      } else {
        passed++;
      }
    ...
    for (const page of pages) {
      ...
      if (page.type === "form" && page.formId) {
        const hasFormComponent = components.some(
          (c) => c.type === "form" && c.refId === page.formId,
        );
        if (!hasFormComponent) {
          errors.push({
            code: "PAGE_ORPHAN_FORM",
            severity: "warning",
            message: `Page "${page.name}" references form not linked to app`,
            suggestion: "Link the referenced form as a component first",
          });
        } else {
          passed++;
        }
      } else if (page.type === "dashboard" && page.dashboardId) {
        const hasDashComponent = components.some(
          (c) => c.type === "dashboard" && c.refId === page.dashboardId,
        );
        if (!hasDashComponent) {
          errors.push({
            code: "PAGE_ORPHAN_DASHBOARD",
            severity: "warning",
            message: `Page "${page.name}" references dashboard not linked to app`,
            suggestion: "Link the referenced dashboard as a component first",
          });
        } else {
          passed++;
        }
    ...
    if (mod.version && /^\d+\.\d+\.\d+$/.test(mod.version)) {
      ...
      errors.push({
        ...
        severity: "info",
        message: "Version format should be semantic (e.g., 1.0.0)",
    ...
    for (const fc of formComponentsSec) {
      ...
      const protectedFields = fields.filter(
        (f) =>
          (f.readRoles && String(f.readRoles).trim()) ||
          (f.writeRoles && String(f.writeRoles).trim()),
      );
      totalTests++;
      if (fields.length > 0 && protectedFields.length === 0) {
        errors.push({
          ...
          severity: "info",
          category: "security",
          message: `Form "${form?.name}" has no field-level role restrictions`,
          suggestion: "Consider adding read/write roles to sensitive fields",
        });
      } else {
        passed++;
      }
    ...
    const automationComponents = components.filter(
      (c) => c.type === "automation",
    );
    for (const ac of automationComponents) {
      totalTests++;
      const rule = await prisma.automationRule.findFirst({
        where: { id: ac.refId, tenantId },
      });
      if (!rule) {
        errors.push({
          code: "ORPHAN_AUTOMATION",
          severity: "error",
          category: "automation",
          message: `Linked automation "${ac.name}" not found`,
          suggestion: "Remove the orphaned automation reference",
        });
      } else {
        const acts = Array.isArray(rule.actions) ? (rule.actions as any[]) : [];
        if (acts.length === 0 || !rule.trigger) {
          errors.push({
            code: "INCOMPLETE_AUTOMATION",
            severity: "warning",
            category: "automation",
            message: `Automation "${rule.name}" is missing a trigger or actions`,
            suggestion: "Configure at least one trigger and one action",
          });
        } else {
          passed++;
        }
    ...
    if (pages.length > 25) {
      errors.push({
        ...
        message: `App has ${pages.length} pages`,
        suggestion:
          "Large apps may load slower; consider splitting into multiple apps",
      });
    } else {
      passed++;
    }
    ...
    const validateSchema = (name: string, type: string, fields: any[]) => {
      ...
      for (const f of fields) {
        ...
        if (
          ...
          issues.push(
            `required field "${f.label || f.name}" has no sample/default value`,
          );
        }
        if (f.type === "Select" && f.options) {
          const opts = String(f.options)
            ...
            .filter(Boolean);
          if (opts.length === 0)
            issues.push(`select field "${f.name}" has no options`);
        }
        if (
          (f.type === "Int" || f.type === "Float" || f.type === "Currency") &&
          sample != null &&
          isNaN(Number(sample))
        ) {
          issues.push(
            `numeric field "${f.name}" produced a non-numeric sample`,
          );
        }
        if (f.regexPattern) {
          try {
            new RegExp(f.regexPattern);
          } catch {
            issues.push(`field "${f.name}" has an invalid regex pattern`);
          }
        }
      }
      totalTests++;
      if (issues.length === 0) {
        ...
        simulations.push({
          ...
          type,
          status: "pass",
          message: `Sample ${type} validated (${fields.length} fields)`,
        });
      } else {
        errors.push({
          ...
          severity: "warning",
          category: "data",
          message: `Sandbox validation failed for "${name}"`,
    ...
    for (const dm of dataModels) {
      validateSchema(
        dm.name,
        "data model",
        Array.isArray(dm.fields) ? dm.fields : [],
      );
    }
    for (const fc of formComponentsSec) {
      const form = await prisma.builderForm.findFirst({
  ...
  private sampleValueForField(f: any): unknown {
    ...
    switch (f.type) {
      ...
      case "Column Break":
        return "n/a";
      default:
        return `Sample ${f.label || f.name || "value"}`;
    }
  }
  ...
  private nextVersion(
    ...
    opts: { version?: string; bump?: "major" | "minor" | "patch" } = {},
  ): string {
  ...
  private async buildModuleSnapshot(tenantId: string, mod: any) {
    ...
    const dashboardIds = components
      ...
      .map((c: any) => c.refId);
    const automationIds = components
      .filter((c: any) => c.type === "automation")
      .map((c: any) => c.refId);
  ...
  async publishModule(
    ...
    if (existingVersion) {
      throw new BadRequestException(
        `Version ${version} already exists for this app. Use a different version.`,
      );
    }
    ...
    const release = await prisma.appRelease.create({
      data: {
        ...
        testScore:
          typeof testResults.score === "number" ? testResults.score : null,
        status: "PUBLISHED",
        publishedBy,
      },
    });
  ...
  async installBuilderApp(
    ...
    if (prior)
      await this.teardownProvisioned(tenantId, prior.provisioned as any);
    ...
    const dataModels: any[] = Array.isArray(snap.dataModels)
      ? snap.dataModels
      : [];
    ...
    const provisionPage = async (
      ...
      if (type !== "custom") {
        const schemaSlug = `${moduleSlug}_${pageSlug}`
          .toLowerCase()
          .replace(/[^a-z0-9_]/g, "_");
        const schema = await prisma.schemaRegistry.upsert({
          ...
          create: {
            ...
            name: title,
            slug: schemaSlug,
            description: `Installed from ${mod.name}`,
            fields: fields as any,
            settings: settings as any,
            status: "ACTIVE",
          },
      ...
      const pageLayout =
        type === "custom" ? layout || [] : { fields, settings };

      const page = await prisma.pageRegistry.upsert({
        where: {
          tenantId_module_slug: {
            tenantId,
            module: moduleSlug,
            slug: pageSlug,
          },
    ...
    for (const dm of dataModels) {
      const dmSlug = String(dm.name || "model")
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/(^-|-$)/g, "");
  ...
  private async teardownProvisioned(
    tenantId: string,
    provisioned: {
    ...
    const pageIds = Array.isArray(provisioned.pageRegistryIds)
      ? provisioned.pageRegistryIds
      : [];
    const schemaIds = Array.isArray(provisioned.schemaRegistryIds)
      ? provisioned.schemaRegistryIds
      : [];
    if (pageIds.length)
      await prisma.pageRegistry.deleteMany({
    ...
    if (schemaIds.length)
      await prisma.schemaRegistry.deleteMany({
        where: { tenantId, id: { in: schemaIds } },
      });
  }
  ...
  async uninstallBuilderApp(tenantId: string, moduleId: string) {
    ...
    await prisma.builderModule.updateMany({
      where: { id: moduleId, installCount: { gt: 0 } },
  ...
  async getModuleStats(tenantId: string, moduleId: string) {
    ...
    const automationRuns = (linkedAutomations as { runCount: number }[]).reduce(
      (sum, r) => sum + (r.runCount || 0),
      0,
    );
    const scoreTrend = Array.isArray(testResults.history)
      ? testResults.history.map((h: any) => h.score).reverse()
      : [];
  ...
  async createDataImport(tenantId: string, dto: CreateDataImportInput) {
  ...
  async executeDataImport(tenantId: string, id: string, rows: ImportRow[]) {
    ...
    try {
        ...
        switch (targetModel) {
          case "customer":
            result = await prisma.customer.createMany({
              data: dataToInsert as Prisma.CustomerCreateManyInput[],
              skipDuplicates: true,
            });
            break;
          case "vendor":
            result = await prisma.vendor.createMany({
              data: dataToInsert as Prisma.VendorCreateManyInput[],
              skipDuplicates: true,
            });
            break;
          case "product":
            result = await prisma.product.createMany({
              data: dataToInsert as Prisma.ProductCreateManyInput[],
              skipDuplicates: true,
            });
            break;
          case "employee":
            result = await prisma.employee.createMany({
              data: dataToInsert as Prisma.EmployeeCreateManyInput[],
              skipDuplicates: true,
            });
            break;
          case "warehouse":
            result = await prisma.warehouse.createMany({
              data: dataToInsert as Prisma.WarehouseCreateManyInput[],
              skipDuplicates: true,
            });
            break;
          default:
            throw new BadRequestException(
              `Unsupported import target: ${job.targetModel}`,
            );
        }
      ...
      this.logger.error(
        `Data import failed for job ${id}: ${e instanceof Error ? e.message : String(e)}`,
      );
      failedCount = rows.length;
      successCount = 0;
    }
  ...
  async getSchemaRegistryById(tenantId: string, id: string) {
    const schema = await prisma.schemaRegistry.findFirst({
      where: { id, tenantId },
    });
  ...
  async publishForm(tenantId: string, pageRegistryId: string) {
    ...
    const layoutObj =
      typeof page.layout === "string" ? JSON.parse(page.layout) : page.layout;
    const layoutFields: any[] = Array.isArray(layoutObj)
      ? layoutObj
      : layoutObj?.fields || [];
    ...
    const settings =
      layoutObj?.settings && typeof layoutObj.settings === "object"
        ? layoutObj.settings
        : {};
    ...
    const schemaSlug = `${page.module}_${page.slug}`
      .toLowerCase()
      .replace(/[^a-z0-9_]/g, "_");
  ...
  async logAnalyticsEvent(tenantId: string, data: BuilderAnalyticsEventInput) {
    ...
    this.logger.log(`Builder analytics event ${data.event}`, {
      tenantId,
  ...
  async generateSchemaFromPrompt(tenantId: string, prompt: string) {
    ...
    const wantsPeople =
      /\b(patient|student|employee|customer|lead|contact|tenant)\b/.test(
        normalizedPrompt,
      );
    const wantsScheduling =
      /\b(appointment|schedule|booking|visit|shift)\b/.test(normalizedPrompt);
    const wantsMoney =
      /\b(invoice|payment|fee|rent|claim|budget|cost|amount)\b/.test(
        normalizedPrompt,
      );
    const wantsInventory =
      /\b(product|item|asset|drug|stock|batch|serial)\b/.test(normalizedPrompt);
    ...
    this.logger.log(`Generated builder schema from prompt`, {
      tenantId,
  ...
  async createSchemaRegistry(
    tenantId: string,
    data: CreateSchemaRegistryInput,
  ) {
  ...
  async updateSchemaRegistry(
    tenantId: string,
    id: string,
    data: UpdateSchemaRegistryInput,
  ) {
    // Used tenantId to suppress ts error and enforce security
    const existing = await prisma.schemaRegistry.findFirst({
      where: { id, tenantId },
    });
  ...
  async updatePageRegistry(
    tenantId: string,
    id: string,
    data: UpdatePageRegistryInput,
  ) {
    const existing = await prisma.pageRegistry.findFirst({
      where: { id, tenantId },
    });
    ...
    if (
      data.layout &&
      JSON.stringify(data.layout) !== JSON.stringify(existing.layout)
    ) {
      const currentHistory = Array.isArray(existing.history)
        ? (existing.history as LayoutHistoryEntry[])
        : [];
      ...
      scrubbed = records.map((record) => {
        ...
        fields.forEach((f) => {
          if (f.readRoles && typeof f.readRoles === "string") {
            const allowedRoles = f.readRoles
              .split(",")
              .map((r: string) => r.trim())
              .filter(Boolean);
            if (
              allowedRoles.length > 0 &&
              !userRoles.some((ur: string) => allowedRoles.includes(ur))
            ) {
  ...
  async triggerWebhooks(
    ...
    try {
      const page = await prisma.pageRegistry.findFirst({
        where: { schemaId: schemaId, tenantId },
      });
      ...
      for (const wh of webhooks) {
        if (wh.event === event && wh.url) {
          fetch(wh.url, {
            ...
            this.logger.error(
              `Webhook failed for ${String(wh.url)}: ${err instanceof Error ? err.message : String(err)}`,
            );
          });
        }
      }
      ...
      this.logger.error(
        `Error triggering webhooks: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
  ...
  async executeScripts(
    ...
    try {
      ...
      this.logger.error(
        `Error executing scripts: ${e instanceof Error ? e.message : String(e)}`,
      );
      return data;
    }
  }
  ...
  async createCustomRecord(
    ...
    fields.forEach((f) => {
      if (f.writeRoles && typeof f.writeRoles === "string") {
        const allowedRoles = f.writeRoles
          .split(",")
          .map((r: string) => r.trim())
          .filter(Boolean);
        if (
    ...
    secureData = await this.executeScripts(
      tenantId,
      schemaId,
      "record.created",
      secureData,
  ...
  async getNavOverlay(tenantId: string, moduleId: string) {
    ...
    const [overlay, pages] = await Promise.all([
      prisma.appNavOverlay.findUnique({
    ...
    > = (config.submodules || []).reduce((acc: Record<string, any>, s: any) => {
      acc[s.slug] = s;
      return acc;
    }, {});
    ...
    const submodules = [...groups.values()]
      .map((g) => ({
        ...
        name:
          meta[g.slug]?.name ||
          g.slug.replace(/[-_]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
  ...
  async resetNavOverlay(tenantId: string, moduleId: string) {
    const mod = moduleId.toLowerCase();
    await prisma.appNavOverlay.deleteMany({
      where: { tenantId, moduleId: mod },
    });
    return { ok: true };
  }
  ...
  async createSubmodule(
    ...
    if (input.schema) {
      ...
      const schema = await prisma.schemaRegistry.upsert({
        ...
        update: {
          name: input.name,
          module: mod,
          fields: (input.schema.fields || []) as any,
          status: "ACTIVE",
        },
        create: {
          tenantId,
    ...
    for (const pd of pageDefs) {
      ...
      const page = await prisma.pageRegistry.upsert({
        where: {
          tenantId_module_slug: { tenantId, module: mod, slug: pageSlug },
        },
        update: {
          title: pd.title,
          type: (pd.type || "LIST").toUpperCase(),
          submodule: subSlug,
          schemaId,
          navIcon: pd.navIcon,
          sortOrder: order,
    ...
    const overlay = await prisma.appNavOverlay.findUnique({
      where: { tenantId_moduleId: { tenantId, moduleId: mod } },
    });
    const config: any = (overlay?.config as any) || {};
    config.submodules = Array.isArray(config.submodules)
      ? config.submodules
      : [];
    ...
    if (existing) {
      ...
      config.submodules.push({
        ...
        name: input.name,
        icon: input.icon,
        order: config.submodules.length,
      });
  ...
  async deleteSubmodule(
    ...
    await prisma.pageRegistry.deleteMany({
      where: { tenantId, module: mod, submodule: sub },
    });
    const overlay = await prisma.appNavOverlay.findUnique({
      where: { tenantId_moduleId: { tenantId, moduleId: mod } },
    });
    if (overlay) {
      const config: any = (overlay.config as any) || {};
      config.submodules = (config.submodules || []).filter(
        (s: any) => s.slug !== sub,
      );
