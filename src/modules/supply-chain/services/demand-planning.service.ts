...
export class DemandPlanningService {
  async generateForecast(
    ...
    for (const order of salesHistory) {
      ...
      const qty = order.lineItems.reduce(
        (s: number, li: { quantity: any }) => s + Number(li.quantity),
        0,
      );
        ...
        ? Math.sqrt(
            historicalData.reduce(
              (s: number, p: { quantity: number }) =>
                s + Math.pow(p.quantity - smoothed, 2),
              0,
  ...
  async getReorderSuggestions(tenantId: string) {
        ...
        (
          ...
          urgency:
            Number(item.quantity) === 0
              ? "CRITICAL"
              : Number(item.quantity) < Number(item.reorderPoint) * 0.5
                ? "HIGH"
                : "MEDIUM",
        }),
      );
    ...
    return {
      ...
      critical: suggestions.filter(
        (s: { urgency: string }) => s.urgency === "CRITICAL",
      ).length,
