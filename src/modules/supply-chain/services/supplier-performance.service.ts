...
export class SupplierPerformanceService {
  async listKpis(tenantId: string) {
    return prisma.supplierPerformanceKpi.findMany({
      where: { tenantId },
      orderBy: { name: "asc" },
    });
  }
  ...
  async createKpi(
    tenantId: string,
    dto: Prisma.SupplierPerformanceKpiCreateInput,
  ) {
    const existing = await prisma.supplierPerformanceKpi.findFirst({
      where: { tenantId, kpiCode: dto.kpiCode },
    });
    if (existing)
      throw new BadRequestException(`KPI code already exists: ${dto.kpiCode}`);
  ...
  async updateKpi(
    tenantId: string,
    id: string,
    dto: Prisma.SupplierPerformanceKpiUpdateInput,
  ) {
    const kpi = await prisma.supplierPerformanceKpi.findFirst({
      where: { id, tenantId },
    });
  ...
  async getScorecards(
    tenantId: string,
    opts: { vendorId?: string; periodStart?: string; periodEnd?: string },
  ) {
    const where: Prisma.SupplierScorecardWhereInput = {
      tenantId,
      ...(opts.vendorId ? { vendorId: opts.vendorId } : {}),
      ...(opts.periodStart
        ? { periodStart: { gte: new Date(opts.periodStart) } }
        : {}),
      ...(opts.periodEnd
        ? { periodEnd: { lte: new Date(opts.periodEnd) } }
        : {}),
    };
  ...
  async calculateScorecard(
    ...
    const [orders, receipts] = await Promise.all([
      ...
      prisma.purchaseReceiptItem.findMany({
        where: { purchaseReceipt: { tenantId, purchaseOrder: { vendorId } } },
        include: {
          purchaseReceipt: {
            include: {
    ...
    const completedOrders = orders.filter(
      (o: any) => o.status === "RECEIVED" || o.status === "CLOSED",
    ).length;
    const totalUnits = receipts.reduce(
      (s: any, r: any) => s + Number(r.quantityReceived ?? 0),
      0,
    );
    const defectiveUnits = 0;
    const qualityScore =
      totalUnits > 0
        ? Math.max(0, 100 - (defectiveUnits / totalUnits) * 100)
        : 100;
    const deliveryScore =
    ...
    return prisma.$transaction(async (tx) => {
      return tx.supplierScorecard.upsert({
        ...
        create: {
          ...
          deliveryScore: new Prisma.Decimal(deliveryScore),
          fillRateScore: new Prisma.Decimal(100),
          overallScore: new Prisma.Decimal(overallScore),
          onTimeDeliveries: completedOrders,
          lateDeliveries: totalOrders - completedOrders,
          defectiveUnits,
