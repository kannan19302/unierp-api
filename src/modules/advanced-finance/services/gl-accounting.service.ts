...
export class GlAccountingService {
  ...
  async updateAccount(
    tenantId: string,
    accountId: string,
    dto: { name?: string; type?: string; parentId?: string | null; isActive?: boolean },
  ) {
  ...
  async deleteAccount(tenantId: string, accountId: string) {
    ...
    const entryCount = await prisma.journalEntry.count({ where: { accountId, tenantId } });
    if (entryCount > 0) {
    ...
    const childCount = await prisma.account.count({ where: { parentId: accountId, tenantId } });
    if (childCount > 0) {
      throw new BadRequestException(
  ...
  async getAccountLedger(
    ...
    const entries = await prisma.journalEntry.findMany({
      ...
      include: {
        journal: { select: { id: true, entryNumber: true, date: true, notes: true, status: true } },
      },
  ...
  async createCostCenter(tenantId: string, orgId: string, dto: { code: string; name: string; parentId?: string }) {
    ...
    return prisma.costCenter.create({
      data: { tenantId, orgId: resolvedOrgId, code: dto.code, name: dto.name, parentId: dto.parentId || null },
    });
  }

  async updateCostCenter(tenantId: string, costCenterId: string, dto: { name?: string; parentId?: string | null }) {
    const cc = await prisma.costCenter.findFirst({ where: { id: costCenterId, tenantId } });
  ...
  async createJournal(
    ...
    return prisma.$transaction(async (tx) => {
      ...
      await this.logAudit(tx, tenantId, 'Journal', journal.id, 'CREATE', { entries: dto.entries.length }, 'system');
  ...
  async approveJournal(tenantId: string, journalId: string, approvedBy: string) {
    const journal = await prisma.journal.findFirst({ where: { id: journalId, tenantId } });
    if (!journal) throw new NotFoundException('Journal entry not found');
    if (journal.status !== 'SUBMITTED' && journal.status !== 'DRAFT') {
      throw new BadRequestException('Only DRAFT or SUBMITTED journals can be approved.');
    }
    const updated = await prisma.journal.update({
      where: { id: journalId },
      data: { status: 'APPROVED' },
    });
    await this.logAudit(prisma, tenantId, 'Journal', journalId, 'APPROVE', { approvedBy }, approvedBy);
    return updated;
  }
  ...
  async rejectJournal(tenantId: string, journalId: string, reason: string, rejectedBy: string) {
    const journal = await prisma.journal.findFirst({ where: { id: journalId, tenantId } });
    if (!journal) throw new NotFoundException('Journal entry not found');
    ...
    const updated = await prisma.journal.update({
      where: { id: journalId },
      data: { status: 'DRAFT', notes: `${journal.notes || ''}\n[REJECTED by ${rejectedBy}]: ${reason}`.trim() },
    });
    await this.logAudit(prisma, tenantId, 'Journal', journalId, 'REJECT', { reason, rejectedBy }, rejectedBy);
    return updated;
  }

  async reverseJournal(tenantId: string, journalId: string, reversalDate?: string) {
    const journal = await prisma.journal.findFirst({
      where: { id: journalId, tenantId },
      include: { entries: true },
    });
    ...
    const reversalEntries = journal.entries.map((entry) => ({
      ...
      description: `Reversal of ${journal.entryNumber}: ${entry.description || ''}`.trim(),
    ...
    return prisma.$transaction(async (tx) => {
      ...
      await tx.journal.update({
        where: { id: journalId },
        data: { status: 'REVERSED', notes: `${journal.notes || ''}\n[REVERSED by ${reversal.entryNumber}]`.trim() },
      });

      await this.logAudit(tx, tenantId, 'Journal', journalId, 'REVERSE', { reversalJournalId: reversal.id }, 'system');
      return tx.journal.findFirst({ where: { id: reversal.id }, include: { entries: true } });
    });
  }
  ...
  async logAudit(
    ...
    try {
      await (txOrPrisma as { financeAuditLog: { create: (args: unknown) => Promise<unknown> } })
  ...
  async createRecurringSchedule(tenantId: string, orgId: string, dto: { entryTemplate: any; frequency: string; nextRunDate: string }) {
    ...
    return prisma.recurringJournal.create({
      data: { tenantId, orgId: resolvedOrgId, entryTemplate: dto.entryTemplate, frequency: dto.frequency, nextRunDate: new Date(dto.nextRunDate), status: 'ACTIVE' },
    });
  }
