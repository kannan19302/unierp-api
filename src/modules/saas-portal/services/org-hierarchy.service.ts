...
export class SaasPortalOrgHierarchyService {
  ...
  async deleteCostCenter(tenantId: string, id: string) {
    ...
    if (children > 0) {
      throw new Error(
        "Cannot delete cost center with children. Reassign or delete children first.",
      );
    }
  ...
  async getOrgTree(tenantId: string) {
    ...
    const buildTree = (
      ...
      items
        ...
        .map((i) => ({
          ...(i as OrgTreeNode),
          children: buildTree(items, i.id as string),
        }));
    ...
    return {
      ...
      costCenters: buildTree(
        costCenters as unknown as Record<string, unknown>[],
        null,
      ),
    };
  }
