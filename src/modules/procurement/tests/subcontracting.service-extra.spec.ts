...
describe("SubcontractingService (extra)", () => {
  ...
  it("should issue material successfully", async () => {
    prisma.subcontractingOrder.findFirst.mockResolvedValue({
      id: "1",
      status: "SENT",
    });
    prisma.subcontractingMaterial.findFirst.mockResolvedValue({
      id: "mat-1",
      requiredQty: 10,
      issuedQty: 0,
      consumedQty: 0,
    });
    prisma.subcontractingMaterial.update.mockResolvedValue({
      id: "mat-1",
      issuedQty: "5",
    });
  ...
  it("should throw NotFoundException when issuing material for missing order", async () => {
    ...
    await expect(
      service.issueMaterial("tenant-1", "x", "mat-1", 5),
  ...
  it("should throw NotFoundException when issuing missing material line", async () => {
    ...
    await expect(
      service.issueMaterial("tenant-1", "1", "missing-mat", 5),
    ).rejects.toThrow(NotFoundException);
  });
  ...
  it("should record consumption successfully", async () => {
    ...
    const result = await service.recordConsumption("tenant-1", "1", "mat-1", 5);
    expect(result.id).toBe("mat-1");
  });
  ...
  it("should throw NotFoundException when recording consumption for missing order", async () => {
    prisma.subcontractingOrder.findFirst.mockResolvedValue(null);
    await expect(
      service.recordConsumption("tenant-1", "x", "mat-1", 5),
    ).rejects.toThrow(NotFoundException);
  });
  ...
  it("should update order fields", async () => {
    ...
    prisma.subcontractingOrder.update.mockResolvedValue({
      id: "1",
      notes: "Updated notes",
    });

    const result = await service.updateOrder("tenant-1", "1", {
      notes: "Updated notes",
    });
  ...
  it("should throw NotFoundException when updating missing order", async () => {
    prisma.subcontractingOrder.findFirst.mockResolvedValue(null);
    await expect(
      service.updateOrder("tenant-1", "x", { notes: "Updated" }),
    ).rejects.toThrow(NotFoundException);
  });

  it("should close order when completed", async () => {
    prisma.subcontractingOrder.findFirst.mockResolvedValue({
      id: "1",
      status: "SENT",
    });
  ...
  it("should throw BadRequestException when closing already completed order", async () => {
    ...
    await expect(service.closeOrder("tenant-1", "1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("should get material consumption report", async () => {
    prisma.subcontractingMaterial.findMany.mockResolvedValue([
      {
        product: { name: "Material A", sku: "MAT1" },
        requiredQty: 10,
        issuedQty: 8,
        consumedQty: 6,
        subcontractingOrder: { id: "1", status: "SENT", vendorId: "v1" },
      },
      {
        product: { name: "Material B", sku: "MAT2" },
        requiredQty: 5,
        issuedQty: 5,
        consumedQty: 5,
        subcontractingOrder: { id: "2", status: "COMPLETED", vendorId: "v1" },
      },
    ]);
