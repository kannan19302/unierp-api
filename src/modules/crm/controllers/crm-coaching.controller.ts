...
} from "../services/crm-coaching.service";
...
export class CrmCoachingController {
  ...
  async createRubric(
    ...
    @ZodBody(createRubricSchema) dto: CreateRubricInput,
  ) {
  ...
  async updateRubric(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(updateRubricSchema) dto: UpdateRubricInput,
  ) {
    return this.svc.updateRubric(req.user.tenantId, id, dto);
  }

  @ApiOperation({
    summary: "Score a logged call against a rubric (manager review)",
  })
  @Post("scorecards")
  ...
  async createScorecard(
    @Req() req: AuthenticatedRequest,
    @ZodBody(createScorecardSchema) dto: CreateScorecardInput,
  ) {
    const orgId = req.user.orgId ?? req.user.tenantId;
    return this.svc.createScorecard(
      req.user.tenantId,
      orgId,
      req.user.userId,
      dto,
  ...
  @ApiOperation({
    summary: "Rep acknowledges a coaching scorecard (closes the review loop)",
  })
  @Put("scorecards/:id/acknowledge")
  @Permissions("crm.coaching.update")
  @TrackChanges("CallScorecard")
  async acknowledgeScorecard(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
  ) {
  ...
  @ApiOperation({
    summary: "Per-rep coaching summary: average score, talk ratio, trend",
  })
  ...
  @ApiOperation({
    summary: "Add an exemplar call/note to the coaching library",
  })
  @Post("library")
  @Permissions("crm.coaching.manage")
  ...
  async createLibraryItem(
    @Req() req: AuthenticatedRequest,
    @ZodBody(createLibraryItemSchema) dto: CreateLibraryItemInput,
  ) {
    const orgId = req.user.orgId ?? req.user.tenantId;
    return this.svc.createLibraryItem(
      req.user.tenantId,
      orgId,
      req.user.userId,
      dto,
