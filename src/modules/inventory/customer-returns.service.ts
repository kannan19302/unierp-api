...
export class CustomerReturnsService {
      ...
      data: {
        ...
        lines: {
          create: dto.lines.map((l) => ({
            ...
            unitCost:
              l.unitCost !== undefined ? new Prisma.Decimal(l.unitCost) : null,
          })),
        },
      },
      include: { lines: true },
  ...
  async approveRma(tenantId: string, rmaId: string, userId: string) {
    ...
    if (rma.status !== "REQUESTED")
      throw new BadRequestException(
        `RMA must be REQUESTED to approve; current: ${rma.status}`,
      );
  ...
  async rejectRma(
    ...
    if (rma.status !== "REQUESTED")
      throw new BadRequestException(
        `RMA must be REQUESTED to reject; current: ${rma.status}`,
      );
    return prisma.customerRma.update({
      where: { id: rmaId },
      data: {
  ...
  async receiveRma(
    ...
    if (rma.status !== "APPROVED")
      throw new BadRequestException(
        `RMA must be APPROVED to receive; current: ${rma.status}`,
      );
  ...
  async inspectLine(
    ...
    if (!["RECEIVED", "INSPECTED"].includes(rma.status)) {
      throw new BadRequestException(
        `RMA must be RECEIVED or INSPECTED to inspect lines`,
      );
    }

    const line = await prisma.customerRmaLine.findFirst({
      where: { id: lineId, rmaId },
    });
    if (!line)
      throw new NotFoundException(`Line ${lineId} not found on RMA ${rmaId}`);
    ...
    const uninspected = await prisma.customerRmaLine.count({
      where: { rmaId, disposition: null },
    });
    if (uninspected === 0) {
  ...
  async closeRma(tenantId: string, rmaId: string) {
    ...
    if (rma.status !== "INSPECTED")
      throw new BadRequestException(`RMA must be INSPECTED before closing`);
    return prisma.customerRma.update({
      where: { id: rmaId },
      data: { status: "CLOSED" },
    });
  ...
  async issueCredit(
    ...
    if (existing)
      throw new BadRequestException(`Credit already exists for RMA ${rmaId}`);
  ...
  async voidCredit(
    tenantId: string,
    creditId: string,
    dto: { voidedById: string; voidReason: string },
  ) {
    const credit = await prisma.returnCredit.findFirst({
      where: { id: creditId, tenantId },
    });
    if (!credit) throw new NotFoundException(`Credit ${creditId} not found`);
    if (credit.status !== "ISSUED")
      throw new BadRequestException("Only ISSUED credits can be voided");
    return prisma.returnCredit.update({
      where: { id: creditId },
      data: {
        status: "VOIDED",
        voidedById: dto.voidedById,
  ...
  async restockLine(
    ...
    const line = await prisma.customerRmaLine.findFirst({
      where: { id: dto.rmaLineId, tenantId },
    });
    if (!line)
      throw new NotFoundException(`RMA line ${dto.rmaLineId} not found`);
    if (line.disposition !== "RESTOCK")
      throw new BadRequestException("Line disposition must be RESTOCK");
  ...
  async listRestocks(
    tenantId: string,
    query: { rmaLineId?: string; productId?: string },
  ) {
  ...
  async getDashboard(tenantId: string) {
    ...
    const totalCreditIssued =
      creditAgg.find((r) => r.status === "ISSUED")?._sum?.creditAmount ?? 0;
