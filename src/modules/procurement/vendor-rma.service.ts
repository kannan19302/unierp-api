...
export class VendorRmaService {
  async listRmas(
    ...
    if (params.search) {
      where.OR = [
        { rmaNumber: { contains: params.search, mode: "insensitive" } },
  ...
  async createRma(
    ...
    const count = await prisma.vendorRmaRequest.count({
  ...
  async updateRmaStatus(
    ...
    if (!allowed || !allowed.includes(status)) {
      throw new BadRequestException(
        `Cannot transition RMA from ${rma.status} to ${status}`,
      );
    }
  ...
  async createShipment(
    ...
    const rma = await prisma.vendorRmaRequest.findFirst({
      where: { id: dto.rmaRequestId, tenantId },
    });
    if (!rma) throw new NotFoundException("RMA request not found");

    const count = await prisma.vendorReturnShipment.count({
      where: { tenantId },
    });
  ...
  async updateShipmentStatus(
    ...
    if (status === "DELIVERED") {
      ...
      if (creditMemoRef) updateData.creditMemoRef = creditMemoRef;
      if (creditAmount) updateData.creditAmount = creditAmount;
    }
  ...
  async getStats(tenantId: string) {
    const rmas = await prisma.vendorRmaRequest.findMany({
      where: { tenantId },
    });
    const shipments = await prisma.vendorReturnShipment.findMany({
      where: { tenantId },
    });
    return {
      totalRmas: rmas.length,
      rmaByStatus: rmas.reduce((acc: Record<string, number>, r) => {
        acc[r.status] = (acc[r.status] || 0) + 1;
        return acc;
      }, {}),
      totalShipments: shipments.length,
      shipmentByStatus: shipments.reduce((acc: Record<string, number>, s) => {
        acc[s.status] = (acc[s.status] || 0) + 1;
        return acc;
      }, {}),
      totalCredits: shipments
        .filter((s) => s.creditAmount)
        .reduce((sum, s) => sum + Number(s.creditAmount), 0),
    };
  }

  async getRmasByVendor(
    tenantId: string,
    vendorId: string,
    params: PaginationParams = {},
  ): Promise<PaginatedResult<any>> {
  ...
  async resolveRma(
    tenantId: string,
    id: string,
    dto: { resolution: string; creditAmount?: number; creditMemoRef?: string },
  ) {
    const rma = await prisma.vendorRmaRequest.findFirst({
      where: { id, tenantId },
    });
    ...
    if (rma.status !== "AUTHORIZED")
      throw new BadRequestException("RMA must be AUTHORIZED before resolving");
    ...
    return prisma.vendorRmaRequest.update({
      where: { id },
      data: updateData,
      include: { vendor: { select: { name: true } }, shipments: true },
    });
  }
  ...
  async cancelRma(tenantId: string, id: string, reason?: string) {
    ...
    if (["COMPLETED", "CANCELLED"].includes(rma.status)) {
      throw new BadRequestException(
        `RMA cannot be cancelled in ${rma.status} status`,
      );
    }
  ...
  async getRmaStatsDetailed(tenantId: string) {
    ...
    for (const rma of rmas) {
      ...
      if (!byVendor[key])
        byVendor[key] = { vendorName: rma.vendor.name, count: 0 };
    ...
    return {
      ...
      byVendor: Object.entries(byVendor).map(([vendorId, data]) => ({
        vendorId,
        ...data,
      })),
      authorizedRate:
        rmas.length > 0
          ? Math.round(
              (rmas.filter(
                (r) => r.status === "AUTHORIZED" || r.status === "COMPLETED",
              ).length /
                rmas.length) *
                10000,
            ) / 100
