...
export class AiExpansionService {
  ...
  async createModel(tenantId: string, dto: CreateAiModelInput) {
    return prisma.aiModel.create({ data: { tenantId, ...dto } as any });
  }
  ...
  async updateModel(tenantId: string, id: string, dto: UpdateAiModelInput) {
    ...
    return prisma.aiModel.update({ where: { id }, data: dto as any });
  }
  ...
  async createDeployment(
    tenantId: string,
    modelId: string,
    dto: CreateAiDeploymentInput,
  ) {
    const model = await prisma.aiModel.findFirst({
      where: { id: modelId, tenantId },
    });
    if (!model) throw new NotFoundException("AI model not found");
    const { modelId: _, ...rest } = dto as any;
    return prisma.aiModelDeployment.create({
      data: { tenantId, modelId, ...rest } as any,
    });
  }
  ...
  async createConversation(
    tenantId: string,
    userId: string,
    dto: CreateAiConversationInput,
  ) {
    return prisma.aiConversation.create({
      data: { tenantId, userId, ...dto } as any,
    });
  }
  ...
  async createDocument(tenantId: string, dto: CreateAiDocumentInput) {
    return prisma.aiDocument.create({ data: { tenantId, ...dto } as any });
  }
  ...
  async createAgent(tenantId: string, dto: CreateAiAgentInput) {
    return prisma.aiAgent.create({ data: { tenantId, ...dto } as any });
  }
  ...
  async updateAgent(tenantId: string, id: string, dto: UpdateAiAgentInput) {
    ...
    return prisma.aiAgent.update({ where: { id }, data: dto as any });
  }
  ...
  async createTrainingJob(tenantId: string, dto: CreateAiTrainingJobInput) {
    return prisma.aiTrainingJob.create({ data: { tenantId, ...dto } as any });
  }

  async deleteTrainingJob(tenantId: string, id: string) {
    const existing = await prisma.aiTrainingJob.findFirst({
      where: { id, tenantId },
    });
