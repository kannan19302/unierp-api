...
export class GovernanceController {
  ...
  async promoteToStaging(
    ...
    return this.governance.promoteToStaging(
      req.user.tenantId,
      moduleId,
      req.user.userId,
    );
  ...
  async promoteToProduction(
    ...
    return this.governance.promoteToProduction(
      req.user.tenantId,
      moduleId,
      req.user.userId,
    );
  ...
  async updatePermissions(
    ...
    return this.governance.updateModulePermissions(
      req.user.tenantId,
      moduleId,
      body,
    );
  ...
  async compareVersions(
    ...
    return this.governance.compareVersions(
      req.user.tenantId,
      moduleId,
      from,
      to,
  ...
  async executeScript(
    @Req() req: AuthReq,
    @ZodBody(z.any())
    body: { script: string; context?: Record<string, unknown> },
  ) {
    return this.scripting.executeScript(
      req.user.tenantId,
      body.script,
      body.context,
    );
  ...
  async executeFormHook(
    ...
    return this.scripting.executeFormHook(
      req.user.tenantId,
      formId,
      body.hookType,
      body.record,
  ...
  @Permissions("builder.read")
  @Post("query/run")
  async runQueryDryRun(
    @Req() _req: AuthReq,
    @ZodBody(z.object({ query: z.string(), limit: z.number().optional() }))
    ...
    if (!sql.toLowerCase().startsWith("select")) {
      return {
        success: false,
        error: "Only SELECT queries are allowed for security dry-runs.",
      };
    }
  ...
  async createConnector(
    ...
    return this.governance.saveConnector(
      req.user.tenantId,
      body.name,
      body.type,
      body.config,
  ...
  async saveStudioPermission(
    ...
    return this.governance.saveStudioPermission(
      req.user.tenantId,
      body.roleId,
      body.moduleSlug,
      body.canRead,
