...
import { CrmAiDraftingService, CrmDraftTone } from "../services/crm-ai-drafting.service";
...
const VALID_TONES: CrmDraftTone[] = [
  "PROFESSIONAL",
  "FRIENDLY",
  "URGENT",
  "CONCISE",
...
function resolveTone(tone?: string): CrmDraftTone {
  return VALID_TONES.includes(tone as CrmDraftTone)
    ? (tone as CrmDraftTone)
    : "PROFESSIONAL";
}
...
export class CrmAiDraftingController {
  ...
  @ApiOperation({
    summary: "Generate a follow-up email draft for an opportunity/deal",
  })
  ...
  async generateOpportunityFollowup(
    ...
    return this.svc.generateOpportunityFollowup(
      req.user.tenantId,
      orgId,
      opportunityId,
      resolveTone(tone),
  ...
  async generateQuoteCoverNote(
    ...
    const orgId = req.user.orgId ?? req.user.tenantId;
    return this.svc.generateQuoteCoverNote(
      req.user.tenantId,
      orgId,
      quotationId,
      resolveTone(tone),
  ...
  async generateLeadOutreach(
    ...
    const orgId = req.user.orgId ?? req.user.tenantId;
    return this.svc.generateLeadOutreach(
      req.user.tenantId,
      orgId,
      leadId,
      resolveTone(tone),
  ...
  @ApiOperation({
    summary:
      "Regenerate an existing draft (same context, optionally a new tone)",
  })
  @ApiQuery({ name: "tone", required: false, enum: VALID_TONES })
  ...
  @ApiOperation({
    summary:
      "List drafts, optionally scoped to a context (opportunity/quotation/lead)",
  })
  ...
  async update(
    ...
    return this.svc.updateDraftText(
      req.user.tenantId,
      draftId,
      body.subject,
      body.body,
  ...
  @ApiOperation({
    summary:
      "Mark a draft as used (sent by the rep through their own email client)",
  })
