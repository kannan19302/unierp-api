...
export class CrmRevenueIntelligenceController {
  ...
  @ApiOperation({
    summary:
      "Generate and send the deal-risk digest to reps + managers (admin/scheduler-triggered)",
  })
  @ApiQuery({
    name: "windowHours",
    required: false,
    description: "Digest cadence window in hours (24=daily, 168=weekly)",
  })
  ...
  async generateDigest(
    ...
    return this.svc.generateAndSendDigests(
      req.user.tenantId,
      orgId,
  ...
  @ApiOperation({
    summary:
      "List past digest runs (audit history), optionally scoped to one recipient",
  })
  @Get("digest/runs")
