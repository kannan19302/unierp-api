...
export class SalesSubscriptionService {
  ...
  async updateSubscription(tenantId: string, id: string, dto: any) {
    ...
    if (dto.unitAmount !== undefined)
      data.unitAmount = new Prisma.Decimal(dto.unitAmount);
  ...
  async getUsageMetrics(
    ...
    const where: Prisma.SubscriptionUsageWhereInput = {
      tenantId,
      subscriptionId,
    };
    if (period) {
    ...
    const byMetric = records.reduce(
      (acc: Record<string, { total: number; count: number }>, r) => {
  ...
  async generateRecurringInvoice(tenantId: string, subscriptionId: string) {
    const sub = await prisma.subscription.findFirst({
      ...
      include: {
        lines: true,
        invoices: { orderBy: { sequenceNumber: "desc" }, take: 1 },
      },
    });
    if (!sub) throw new NotFoundException("Subscription not found");
    if (sub.status !== "ACTIVE")
      throw new BadRequestException(
        "Cannot generate invoice for non-active subscription",
      );
    ...
    if (existingInvoice)
      throw new BadRequestException(
        `Invoice ${invoiceNumber} already exists for this period`,
      );
    ...
    const invoice = await prisma.invoice.create({
      data: {
        ...
        notes: `Auto-generated for subscription ${sub.name} (period ${periodStart.toISOString().slice(0, 10)} - ${periodEnd.toISOString().slice(0, 10)})`,
        createdBy: "system",
      },
    });
  ...
  async dunningProcess(tenantId: string, invoiceIds?: string[]) {
    ...
    for (const si of pendingInvoices) {
      const daysOverdue = Math.floor(
        (Date.now() - new Date(si.invoice.dueDate).getTime()) / 86400000,
      );
      let action = "OK";
      if (daysOverdue > 30) {
        action = "FLAGGED_FOR_SUSPENSION";
      } else if (daysOverdue > 15) {
        action = "ESCALATED_COLLECTIONS";
      } else if (daysOverdue > 7) {
        action = "SENT_REMINDER_2";
      } else if (daysOverdue > 0) {
        action = "SENT_REMINDER_1";
      }
      results.push({
        invoiceId: si.invoiceId,
        subscriptionName: si.subscription.name,
        action,
      });
    }
    return {
      dunningRun: new Date().toISOString(),
  ...
  async getSubscriptionAnalytics(tenantId: string) {
    ...
    return {
      ...
      totalMRR: Number(totalRevenue._sum.amount || 0) / (active || 1),
      totalARR: (Number(totalRevenue._sum.amount || 0) / (active || 1)) * 12,
      totalSubscriptions: totalSubs,
      churnRate: totalSubs > 0 ? Math.round((canceled / totalSubs) * 100) : 0,
    };
  }
}
