...
describe("RfqAuctionsService (extra)", () => {
  ...
  it("should throw NotFoundException for missing auction", async () => {
    ...
    await expect(
      service.getAuctionById("tenant-1", "nonexistent"),
    ).rejects.toThrow(NotFoundException);
  });

  it("should enforce tenant isolation on getAuctionById", async () => {
    prisma.rFQ.findFirst.mockResolvedValue(null);
    await expect(service.getAuctionById("tenant-2", "1")).rejects.toThrow(
      NotFoundException,
    );
    expect(prisma.rFQ.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: "1", tenantId: "tenant-2", isAuction: true },
      }),
    );
  });
  ...
  it("should reject invalid auction transition from SENT to DRAFT", async () => {
    ...
    await expect(
      service.updateAuctionStatus("tenant-1", "1", "DRAFT"),
  ...
  it("should extend auction end date", async () => {
    ...
    prisma.rFQ.update.mockResolvedValue({
      id: "1",
      auctionEndsAt: new Date("2026-07-01"),
    });
  ...
  it("should throw BadRequestException when extending non-open auction", async () => {
    ...
    await expect(
      service.extendAuction("tenant-1", "1", "2026-07-01"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("should throw BadRequestException when cancelling completed auction", async () => {
    ...
    await expect(service.cancelAuction("tenant-1", "1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("should get auctions by vendor", async () => {
    prisma.rFQ.findMany.mockResolvedValue([
      { id: "1", _count: { auctionBids: 2 } },
    ]);
    prisma.rFQ.count.mockResolvedValue(1);
    const result = await service.getAuctionsByVendor(
      "tenant-1",
      "vendor-1",
      {},
    );
  ...
  it("should get bid history with pagination", async () => {
    ...
    const result = await service.getBidHistory("tenant-1", "auction-1", {
      page: 1,
      limit: 20,
    });
    expect(result.meta.total).toBe(1);
    ...
    prisma.rFQ.findMany.mockResolvedValue([
      {
        id: "1",
        status: "COMPLETED",
        _count: { auctionBids: 3 },
        auctionBids: [{ bidAmount: 1000, status: "WINNING" }],
      },
