...
export class DriveDeepService {
  ...
  async shareFolder(tenantId: string, folderId: string, sharedWithUserId: string, permission: string) {
    const folder = await prisma.driveFolder.findFirst({ where: { id: folderId, tenantId } });
    if (!folder) throw new NotFoundException("Folder not found");
    const existing = await prisma.driveFolderShare.findFirst({ where: { tenantId, folderId, sharedWithUserId } });
    if (existing) {
      return prisma.driveFolderShare.update({ where: { id: existing.id }, data: { permission } });
    }
    return prisma.driveFolderShare.create({ data: { tenantId, folderId, sharedWithUserId, permission } });
  }

  async removeFolderShare(tenantId: string, folderId: string, shareId: string) {
    const share = await prisma.driveFolderShare.findFirst({ where: { id: shareId, folderId, tenantId } });
  ...
  async getFolderShares(tenantId: string, folderId: string) {
    return prisma.driveFolderShare.findMany({ where: { tenantId, folderId }, orderBy: { createdAt: "desc" } });
  }
  ...
  async getSharedWithMe(tenantId: string, userId: string) {
    ...
    const files = await prisma.driveFile.findMany({
      ...
      select: { id: true, name: true, mimeType: true, size: true, ownerId: true, folderId: true, createdAt: true },
      take: 50,
      orderBy: { createdAt: "desc" },
    });
  ...
  async getFilePreview(tenantId: string, fileId: string) {
    ...
    const previewableTypes = ["image/png", "image/jpeg", "image/gif", "image/webp", "application/pdf", "text/plain", "text/csv"];
    ...
    return {
      ...
      thumbnailUrl: file.mimeType.startsWith("image/") ? file.storagePath : null,
      latestVersion: latestVersion ? { version: latestVersion.version, storagePath: latestVersion.storagePath } : null,
    };
  }
  ...
  async trashFile(tenantId: string, fileId: string, deletedBy: string) {
    ...
    if (!file) throw new NotFoundException("File not found");
    if (file.isDeleted) throw new BadRequestException("File is already in trash");
  ...
  async getTrashItems(tenantId: string) {
    return prisma.driveTrashItem.findMany({
      where: { tenantId },
      include: { file: { select: { id: true, name: true, mimeType: true, size: true, ownerId: true } } },
      orderBy: { deletedAt: "desc" },
    });
  }

  async emptyTrash(tenantId: string) {
    const items = await prisma.driveTrashItem.findMany({ where: { tenantId } });
    const fileIds = items.map((i) => i.fileId);
    await prisma.driveFile.deleteMany({ where: { id: { in: fileIds }, tenantId } });
  ...
  async autoPurgeTrash(tenantId: string) {
    ...
    const oldItems = await prisma.driveTrashItem.findMany({ where: { tenantId, deletedAt: { lte: cutoff } } });
    if (oldItems.length === 0) return { purged: 0 };
    const fileIds = oldItems.map((i) => i.fileId);
    await prisma.driveFile.deleteMany({ where: { id: { in: fileIds }, tenantId } });
    await prisma.driveTrashItem.deleteMany({ where: { tenantId, deletedAt: { lte: cutoff } } });
  ...
  async getStorageUsage(tenantId: string) {
    const quota = await prisma.driveStorageQuota.findUnique({ where: { tenantId } });
    const files = await prisma.driveFile.findMany({ where: { tenantId, isDeleted: false } });
    ...
    return {
      ...
      quota: { used: quota?.storageUsed || 0, limit: quota?.storageLimit || 1073741824 },
  ...
  private computeTypeBreakdown(files: any[]) {
    ...
    for (const f of files) {
      const cat = f.mimeType.startsWith("image/") ? "images" : f.mimeType.startsWith("video/") ? "videos" : f.mimeType.includes("pdf") ? "pdfs" : "others";
  ...
  async getStorageUsageByFolder(tenantId: string, folderId?: string) {
    ...
    const files = await prisma.driveFile.findMany({ where, select: { id: true, name: true, size: true, mimeType: true } });
    const totalSize = files.reduce((s, f) => s + Number(f.size), 0);
    return { folderId: folderId || "root", fileCount: files.length, totalSize, files };
  }
  ...
  async createTag(tenantId: string, dto: { name: string; color?: string }) {
    const existing = await prisma.driveFileTag.findFirst({ where: { tenantId, name: dto.name } });
    if (existing) throw new BadRequestException("Tag with this name already exists");
    return prisma.driveFileTag.create({ data: { tenantId, name: dto.name, color: dto.color || "#6366f1" } });
  }

  async updateTag(tenantId: string, id: string, dto: { name?: string; color?: string }) {
    const tag = await prisma.driveFileTag.findFirst({ where: { id, tenantId } });
  ...
  async assignTags(tenantId: string, fileId: string, tagIds: string[]) {
    const file = await prisma.driveFile.findFirst({ where: { id: fileId, tenantId } });
    if (!file) throw new NotFoundException("File not found");
    const existing = await prisma.driveFileTagMapping.findMany({ where: { fileId } });
    const existingTagIds = existing.map((e) => e.tagId);
    const toAdd = tagIds.filter((t) => !existingTagIds.includes(t));
    for (const tagId of toAdd) {
      const tag = await prisma.driveFileTag.findFirst({ where: { id: tagId, tenantId } });
    ...
    if (toRemove.length > 0) {
      await prisma.driveFileTagMapping.deleteMany({ where: { fileId, tagId: { in: toRemove } } });
    }
  ...
  async getFilesByTag(tenantId: string, tagName: string) {
    const tag = await prisma.driveFileTag.findFirst({ where: { tenantId, name: tagName } });
    if (!tag) return [];
    const mappings = await prisma.driveFileTagMapping.findMany({
      where: { tagId: tag.id },
      include: { file: { select: { id: true, name: true, mimeType: true, size: true, createdAt: true, ownerId: true } } },
    });
  ...
  async generateDownloadLink(tenantId: string, fileId: string, expiresInHours: number) {
    const file = await prisma.driveFile.findFirst({ where: { id: fileId, tenantId } });
    if (!file) throw new NotFoundException("File not found");
    ...
    const link = await prisma.driveShareLink.create({
      data: { tenantId, fileId, token, permission: "VIEW", expiresAt, createdBy: "system" },
    });
    return { url: `/api/drive/download/${token}`, token: link.token, expiresAt: link.expiresAt };
  }

  async resolveDownloadToken(token: string) {
    const link = await prisma.driveShareLink.findUnique({ where: { token } });
    if (!link) throw new NotFoundException("Download link not found or expired");
    if (link.expiresAt && link.expiresAt < new Date()) throw new BadRequestException("Download link has expired");
    const file = await prisma.driveFile.findFirst({ where: { id: link.fileId } });
    if (!file) throw new NotFoundException("File not found");
    await prisma.driveShareLink.update({ where: { id: link.id }, data: { downloadCount: { increment: 1 } } });
