...
vi.mock("@unerp/database", () => {
  ...
  return {
    prisma: {
      nettingGroup: m(),
      nettingGroupMember: m(),
      nettingRun: m(),
      nettingRunDetail: m(),
      settlementInstruction: m(),
      interCompanyTransaction: m(),
    },
  };
});
...
describe("NettingDeepService", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new NettingDeepService();
  });

  it("getNettingGroups returns groups", async () => {
    vi.mocked(prisma.nettingGroup.findMany).mockResolvedValue([
      { id: "ng-1" },
    ] as any);
  ...
  it("getNettingGroupById throws on not found", async () => {
    ...
    await expect(service.getNettingGroupById("t1", "bad")).rejects.toThrow(
      NotFoundException,
    );
  });

  it("createNettingGroup creates record", async () => {
    vi.mocked(prisma.nettingGroup.create).mockResolvedValue({
      id: "ng-1",
    } as any);
    const result = await service.createNettingGroup("t1", "o1", {
      name: "EU Netting",
      nettingMethod: "MULTILATERAL",
    });
    expect(result.id).toBe("ng-1");
  });

  it("addNettingGroupMember adds member", async () => {
    vi.mocked(prisma.nettingGroup.findFirst).mockResolvedValue({
      id: "ng-1",
      status: "ACTIVE",
      members: [],
      runs: [],
    } as any);
    vi.mocked(prisma.nettingGroupMember.create).mockResolvedValue({
      id: "m-1",
    } as any);
    const result = await service.addNettingGroupMember("t1", "ng-1", {
      orgId: "org-2",
  ...
  it("getNettingRuns returns runs", async () => {
    vi.mocked(prisma.nettingRun.findMany).mockResolvedValue([
      { id: "nr-1" },
    ] as any);
  ...
  it("createNettingRun throws if group not active", async () => {
    ...
    await expect(
      service.createNettingRun("t1", "ng-1", {
        nettingDate: "2026-07-01",
  ...
  it("deleteNettingRun throws if not draft", async () => {
    ...
    await expect(service.deleteNettingRun("t1", "nr-1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("submitNettingRunForApproval throws if not draft", async () => {
    ...
    await expect(
      service.submitNettingRunForApproval("t1", "nr-1"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("approveNettingRun throws if not pending approval", async () => {
    ...
    await expect(
      service.approveNettingRun("t1", "nr-1", "user1"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("cancelNettingRun throws if settled", async () => {
    ...
    await expect(service.cancelNettingRun("t1", "nr-1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("computeNettingRun processes transactions", async () => {
    ...
    vi.mocked(prisma.nettingGroup.findFirst).mockResolvedValue({
      id: "ng-1",
      nettingMethod: "BILATERAL",
      members: [{ orgId: "org-1" }, { orgId: "org-2" }],
    } as any);
    vi.mocked(prisma.interCompanyTransaction.findMany).mockResolvedValue([
      {
        id: "tx-1",
        fromOrgId: "org-1",
        toOrgId: "org-2",
    ...
    vi.mocked(prisma.nettingRun.update).mockResolvedValue({
      id: "nr-1",
    } as any);
  ...
  it("getSettlementInstructions returns instructions", async () => {
    vi.mocked(prisma.settlementInstruction.findMany).mockResolvedValue([
      { id: "si-1" },
    ] as any);
  ...
  it("createSettlementInstruction creates record", async () => {
    ...
    vi.mocked(prisma.settlementInstruction.create).mockResolvedValue({
      id: "si-1",
    } as any);
    const result = await service.createSettlementInstruction("t1", "nr-1", {
      fromOrgId: "org-1",
      toOrgId: "org-2",
  ...
  it("executeSettlementInstruction throws if not pending", async () => {
    vi.mocked(prisma.settlementInstruction.findFirst).mockResolvedValue({
      id: "si-1",
      status: "EXECUTED",
    } as any);
    await expect(
      service.executeSettlementInstruction("t1", "si-1"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("confirmSettlementInstruction throws if not executed", async () => {
    ...
    await expect(
      service.confirmSettlementInstruction("t1", "si-1"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("settleNettingRun throws if not approved", async () => {
    ...
    await expect(service.settleNettingRun("t1", "nr-1")).rejects.toThrow(
      BadRequestException,
    );
  });
