// @ts-nocheck
import {
  Controller, Get, Post, Param, Body, Query, Res, Req, UseGuards,
} from '@nestjs/common';
...
export class ExportV2Controller {
  ...
  async exportWithTemplate(
    ...
    await this.exportV2Service.exportWithTemplates(
      res, modelName, body.fields, body.rows, body.template, body.format || 'xlsx',
    );
  }
  ...
  async bulkExport(
    ...
    const tenantId = req.tenantId || req.user?.tenantId;
    await this.exportV2Service.bulkExport(tenantId, modelName, body.ids, body.format || 'csv', res);
  }
}
