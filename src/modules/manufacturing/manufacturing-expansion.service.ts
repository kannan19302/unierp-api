...
import type {
  ...
  ManufacturingRouteDto,
  QualityCheckTemplateDto,
  QualityCheckDto,
  ScrapRecordDto,
  TimeEntryDto,
...
export class ManufacturingExpansionService {
  ...
  async updateWorkCenterCapacity(
    ...
    const existing = await prisma.workCenterCapacity.findFirst({
      where: { id, tenantId },
    });
    if (!existing)
      throw new NotFoundException("Work center capacity record not found");
    return prisma.workCenterCapacity.update({
      where: { id },
      data: {
        utilizedHours:
          dto.utilizedHours !== undefined
            ? new Prisma.Decimal(dto.utilizedHours)
            : undefined,
        availableHours:
          dto.availableHours !== undefined
            ? new Prisma.Decimal(dto.availableHours)
            : undefined,
        overtimeHours:
  ...
  async createRoute(tenantId: string, dto: ManufacturingRouteDto) {
    const existing = await prisma.manufacturingRoute.findFirst({
    ...
    return prisma.$transaction(async (tx) => {
      const route = await tx.manufacturingRoute.create({
        data: {
          ...
          totalLeadTimeMin: dto.operations.reduce(
            (sum, op) => sum + op.durationMinutes + op.setupMinutes,
            0,
          ),
        },
      });
  ...
  async createQualityCheckTemplate(
    tenantId: string,
    dto: QualityCheckTemplateDto,
  ) {
    const existing = await prisma.manufacturingQualityCheckTemplate.findFirst({
      where: { tenantId, code: dto.code },
    });
    if (existing)
      throw new BadRequestException(
        `Template code ${dto.code} already exists.`,
      );
  ...
  async performQualityCheck(tenantId: string, dto: QualityCheckDto) {
    const template = await prisma.manufacturingQualityCheckTemplate.findFirst({
      where: { id: dto.templateId, tenantId },
    });
    if (!template)
      throw new NotFoundException("Quality check template not found");
