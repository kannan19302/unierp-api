...
describe("CrmGamificationService", () => {
  ...
  it("computes and persists a leaderboard snapshot ranked by points", async () => {
    ...
    const result = await service.computeLeaderboard(
      TENANT,
      "org-system-default",
      "2026-07",
    );
    ...
    expect(prisma.leaderboardSnapshot.deleteMany).toHaveBeenCalledWith({
      where: { tenantId: TENANT, period: "2026-07" },
    });
    ...
    expect(emit).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ tenantId: TENANT, period: "2026-07" }),
    );
  });

  it("creates a badge definition", async () => {
    (
      prisma.gamificationBadge.create as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "badge1", name: "Deal Closer" });
  ...
  it("awards a badge when a rep meets DEALS_WON_COUNT criteria and skips if already awarded", async () => {
    (
      prisma.gamificationBadge.findMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue([
      {
        id: "badge1",
        criteriaType: "DEALS_WON_COUNT",
    ...
    (
      prisma.gamificationBadgeAward.findUnique as ReturnType<typeof vi.fn>
    ).mockResolvedValue(null);
    (
      prisma.gamificationBadgeAward.create as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});

    const result = await service.evaluateBadges(TENANT, "org-system-default");

    expect(result.awarded).toBe(1);
    expect(prisma.gamificationBadgeAward.create).toHaveBeenCalledWith(
    ...
    expect(emit).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ userId: "user-a", badgeId: "badge1" }),
    );
  });
  ...
  it("does not re-award an already-awarded badge", async () => {
    ...
    (prisma.opportunity.findMany as ReturnType<typeof vi.fn>).mockResolvedValue(
      [{ assignedToId: "user-a", amount: 5000 }],
    );
    (prisma.salesStreak.findMany as ReturnType<typeof vi.fn>).mockResolvedValue(
      [],
    );
    (
      prisma.gamificationBadgeAward.findUnique as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "existing-award" });
  ...
  it("computes streaks from consecutive activity days", async () => {
    ...
    expect(prisma.salesStreak.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
  ...
  it("throws BadRequestException from getMySummary when no data exists yet", async () => {
    (
      prisma.leaderboardSnapshot.findUnique as ReturnType<typeof vi.fn>
    ).mockResolvedValue(null);
    (
    ...
    await expect(service.getMySummary(TENANT, "user-x")).rejects.toThrow(
