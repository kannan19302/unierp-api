...
export class CustomsDocumentService {
  ...
  async getById(tenantId: string, id: string) {
    const doc = await prisma.customsDocument.findFirst({
      where: { id, tenantId },
    });
  ...
  async create(tenantId: string, dto: Prisma.CustomsDocumentCreateInput) {
    const existing = await prisma.customsDocument.findFirst({
      where: { tenantId, docNumber: dto.docNumber },
    });
    if (existing)
      throw new BadRequestException(
        `Document number already exists: ${dto.docNumber}`,
      );
  ...
  async update(
    tenantId: string,
    id: string,
    dto: Prisma.CustomsDocumentUpdateInput,
  ) {
  ...
  async submit(tenantId: string, id: string) {
    ...
    if (doc.status !== "DRAFT")
      throw new BadRequestException(
        `Cannot submit document in status: ${doc.status}`,
      );
  ...
  async markCleared(
    ...
    return prisma.customsDocument.update({
      ...
      data: {
        status: "CLEARED",
        clearedAt: new Date(dto.clearedAt),
        dutyAmount: new Prisma.Decimal(dto.dutyAmount),
        taxAmount: new Prisma.Decimal(dto.taxAmount),
        totalFees: new Prisma.Decimal(dto.totalFees),
      },
    });
  }
}
