...
export class CrmAutomationService {
  async getWorkflows(_tenantId: string): Promise<
    Array<{
      id: string;
      name: string;
      trigger: string;
  ...
  > {
    return [
      ...
      },
      {
        ...
        action: "notification.slack.dispatch",
        active: true,
      },
      {
        id: "wf-4",
        name: "Onboarding Checklist Auto-Gen",
        trigger: "customer.status = ACTIVE",
        action: "crm.account.initChecklist",
        active: true,
      },
      {
        id: "wf-5",
        name: "CSAT Dispatch on Ticket Close",
        trigger: "ticket.status = CLOSED",
        action: "crm.support.dispatchCsat",
        active: true,
      },
    ];
  }
