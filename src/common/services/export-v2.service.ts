...
import { buildPaginationValues, paginatedResult, PaginatedResult, PaginationParams } from '../../common/utils/pagination.util';
...
export class ExportV2Service {
  ...
  async exportWithTemplates(
    ...
    if (format === 'csv') {
      ...
      await this.writePdf(res, columns, rows, `export-${modelName}`, templateConfig.title || `Export: ${modelName}`);
    } else {
      await this.writeStyledXlsx(res, columns, rows, `export-${modelName}`, templateConfig);
    }
  }
  ...
  async exportWithCharts(
    ...
    if (format === 'xlsx') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="chart-export.xlsx"`);
      ...
      if (chartConfig.title) {
        doc.fontSize(18).font('Helvetica-Bold').text(chartConfig.title, { align: 'center' });
        doc.moveDown(1);
      }
      ...
      if (chartConfig.type === 'bar') {
        this.drawBarChart(doc, chartConfig, chartX, chartY, chartWidth, chartHeight);
      } else if (chartConfig.type === 'line') {
        this.drawLineChart(doc, chartConfig, chartX, chartY, chartWidth, chartHeight);
      } else if (chartConfig.type === 'pie' || chartConfig.type === 'donut') {
        this.drawPieChart(doc, chartConfig, chartX, chartY, chartWidth, chartHeight);
      }
      ...
      const tableCols = [
        ...
        ...chartConfig.datasets.map((ds) => ({ header: ds.label, key: ds.label, width: 80 })),
      ];

      doc.fontSize(12).font('Helvetica-Bold').text('Data Table', { align: 'center' });
      doc.moveDown(0.5);
      ...
      for (const row of chartRows) {
        ...
        const rowY = doc.y;
        for (const col of tableCols) {
          doc.text(row[col.key] != null ? String(row[col.key]) : '', x, rowY, { width: colWidth, continued: false });
          x += colWidth;
        }
        doc.moveDown(0.2);
      }
  ...
  private drawBarChart(doc: typeof PDFDocument.prototype, config: ChartConfig, x: number, y: number, width: number, height: number): void {
    const colors = ['#4472C4', '#ED7D31', '#70AD47', '#FFC000', '#5B9BD5', '#264478'];
    ...
    for (let i = 0; i < numBars; i++) {
      ...
      doc.text(config.labels[i].substring(0, 8), labelX - 15, y + chartAreaHeight + 5, { width: 30, align: 'center' });
    }

    doc.restore();
  }

  private drawLineChart(doc: typeof PDFDocument.prototype, config: ChartConfig, x: number, y: number, width: number, height: number): void {
    ...
    for (let j = 0; j < config.datasets.length; j++) {
      ...
      doc.moveTo(x, y + chartAreaHeight - (firstVal / maxVal) * chartAreaHeight);
    ...
    for (let i = 0; i < config.labels.length; i++) {
      ...
      doc.text(config.labels[i].substring(0, 6), labelX - 12, y + chartAreaHeight + 5, { width: 24, align: 'center' });
    }

    doc.restore();
  }

  private drawPieChart(doc: typeof PDFDocument.prototype, config: ChartConfig, x: number, y: number, width: number, height: number): void {
    const colors = ['#4472C4', '#ED7D31', '#70AD47', '#FFC000', '#5B9BD5', '#264478', '#A5A5A5'];
    ...
    for (let i = 0; i < ds.data.length; i++) {
      const sliceAngle = (ds.data[i] / total) * 2 * Math.PI;
    ...
    for (let i = 0; i < ds.data.length; i++) {
      ...
      doc
        ...
        .fill();
      doc.fillColor('#333333');
      doc.text(`${config.labels[i]} (${pct}%)`, x + 12, legendY, { width: 200 });
      legendY += 12;
    }
  }
  ...
  async scheduleExport(
    ...
    if (this.eventEmitter) {
      this.eventEmitter.emit('export.scheduled', {
        tenantId, userId, exportId: scheduledExport.id, config,
      });
    }
  ...
  async bulkExport(
    ...
    const fields = Object.keys(records[0])
      .filter((k) => !['id', 'tenantId', 'deletedAt', 'passwordHash'].includes(k))
      .map((k) => ({
        header: k.replace(/([A-Z])/g, ' $1').replace(/^./, (s) => s.toUpperCase()).trim(),
        key: k,
        width: 20,
      }));

    await this.exportWithTemplates(res!, modelName, fields, records, 'default', format);

    if (this.eventEmitter) {
      this.eventEmitter.emit('export.bulk.completed', {
        tenantId, modelName, recordCount: records.length, format,
      });
    }
  }

  private async writeCsv(res: Response, columns: { header: string; key: string }[], rows: Record<string, any>[], filename: string): Promise<void> {
  ...
  private async writeStyledXlsx(
    ...
    headerRow.font = { bold: true, color: { argb: template.headerFontColor || 'FFFFFFFF' } };
    headerRow.fill = {
      type: 'pattern',
      pattern: 'solid',
  ...
  private async writePdf(res: Response, columns: { header: string; key: string }[], rows: Record<string, any>[], filename: string, title: string): Promise<void> {
    const doc = new PDFDocument({ margin: 40, size: 'A4', layout: 'landscape' });
    res.setHeader('Content-Type', 'application/pdf');
    ...
    doc.fontSize(7).text(`Generated by UniERP on ${new Date().toISOString().slice(0, 10)}`, 40, doc.page.height - 30, { align: 'center' });
    doc.end();
  }
}
