...
export class RfqAuctionsService {
  async listAuctions(
    ...
    const [items, total] = await Promise.all([
      prisma.rFQ.findMany({
        ...
        include: {
          _count: { select: { auctionBids: true, supplierQuotations: true } },
        },
      }),
  ...
  async getAuctionById(tenantId: string, id: string) {
    const rfq = await prisma.rFQ.findFirst({
      ...
      include: {
        ...
        supplierQuotations: { include: { vendor: { select: { name: true } } } },
      },
    });
  ...
  async placeBid(
    ...
    const rfq = await prisma.rFQ.findFirst({
      where: { id: rfqId, tenantId, isAuction: true },
    });
    if (!rfq) throw new NotFoundException("Auction not found");
    if (rfq.status !== "SENT")
      throw new BadRequestException("Auction is not open for bids");
  ...
  async selectWinningBid(tenantId: string, rfqId: string, bidId: string) {
    ...
    const bid = await prisma.rFQAuctionBid.findFirst({
      where: { id: bidId, rfqId },
    });
    if (!bid) throw new NotFoundException("Bid not found");

    await prisma.rFQAuctionBid.updateMany({
      where: { rfqId, status: "SUBMITTED" },
      data: { status: "REJECTED" },
    });
    await prisma.rFQAuctionBid.update({
      where: { id: bidId },
      data: { status: "WINNING" },
    });
  ...
  async updateAuctionStatus(tenantId: string, id: string, status: string) {
    ...
    if (!allowed || !allowed.includes(status)) {
      throw new BadRequestException(
        `Cannot transition auction from ${rfq.status} to ${status}`,
      );
    }
  ...
  async getStats(tenantId: string) {
    ...
    return {
      ...
      auctionByStatus: auctions.reduce((acc: Record<string, number>, a) => {
      ...
      averageBidsPerAuction:
        auctions.length > 0
          ? Math.round((bids.length / auctions.length) * 10) / 10
          : 0,
      totalSavings: bids
  ...
  async getAuctionsByVendor(
    ...
    const [items, total] = await Promise.all([
      prisma.rFQ.findMany({
        ...
        take,
        orderBy: orderBy as any,
        include: { _count: { select: { auctionBids: true } } },
      }),
      prisma.rFQ.count({ where }),
    ]);
    return paginatedResult(items, total, params);
  ...
  async extendAuction(tenantId: string, id: string, newEndDate: string) {
    ...
    if (!rfq) throw new NotFoundException("Auction not found");
    if (rfq.status !== "SENT")
      throw new BadRequestException("Only open auctions can be extended");
  ...
  async cancelAuction(tenantId: string, id: string, reason?: string) {
    ...
    if (rfq.status === "COMPLETED" || rfq.status === "CANCELLED") {
      throw new BadRequestException(
        "Auction is already completed or cancelled",
      );
    }
  ...
  async getAuctionAnalytics(tenantId: string) {
    const auctions = await prisma.rFQ.findMany({
      ...
      include: {
        ...
        auctionBids: { where: { status: "WINNING" } },
      },
    });
    ...
    const avgBidsPerAuction =
      totalAuctions > 0 ? Math.round((totalBids / totalAuctions) * 10) / 10 : 0;
    const completionRate =
      totalAuctions > 0
        ? Math.round((completedAuctions / totalAuctions) * 10000) / 100
        : 0;

    const winningBids = auctions.flatMap((a) => a.auctionBids);
    const totalSavings = winningBids.reduce(
      (s, b) => s + Number(b.bidAmount),
      0,
    );
    ...
    return {
      ...
      completionRate,
      totalSavings,
      byStatus: auctions.reduce((acc: Record<string, number>, a) => {
        acc[a.status] = (acc[a.status] || 0) + 1;
        return acc;
      }, {}),
    };
