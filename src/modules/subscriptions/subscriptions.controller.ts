...
export class SubscriptionsController {
  constructor(
    ...
    private readonly usageBillingService: SubscriptionUsageService,
  ) {}

  @Get("plan-tiers")
  @ApiOperation({ summary: "List plan tiers" })
  @Permissions("finance.subscription.read")
  async getPlanTiers(@Req() req: AuthenticatedRequest) {
    return this.plansService.getPlanTiers(req.user.tenantId);
  }

  @Post("plan-tiers")
  @ApiOperation({ summary: "Create plan tier" })
  @Permissions("finance.subscription.manage")
  async createPlanTier(
    @Req() req: AuthenticatedRequest,
    @Body() dto: Record<string, unknown>,
  ) {
    return this.plansService.createPlanTier(req.user.tenantId, dto);
  }

  @Get()
  @ApiOperation({
    summary: "List subscriptions (paginated, sortable, filterable)",
  })
  @Permissions("finance.subscription.read")
  async findAll(
    @Req() req: AuthenticatedRequest,
