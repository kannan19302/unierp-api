...
export class SalesTerritoryService {
  ...
  async getTerritoryById(tenantId: string, id: string) {
    const territory = await prisma.salesTerritory.findFirst({
      ...
      include: {
        ...
        salesTerritoryForecasts: { orderBy: { period: "desc" }, take: 8 },
        salesTerritoryRealignments: {
  ...
  async createTerritory(tenantId: string, orgId: string, dto: any) {
    ...
    if (dto.parentId) {
      const parent = await prisma.salesTerritory.findFirst({
  ...
  async updateTerritory(tenantId: string, id: string, dto: any) {
    const existing = await prisma.salesTerritory.findFirst({
      where: { id, tenantId },
    });
  ...
  async deleteTerritory(tenantId: string, id: string) {
    ...
    if (!existing) throw new NotFoundException("Territory not found");
    const children = await prisma.salesTerritory.count({
      where: { parentId: id, deletedAt: null },
    });
    if (children > 0)
      throw new BadRequestException(
        "Cannot delete territory with active children",
      );
  ...
  async getTerritoryHierarchy(tenantId: string) {
    const territories = await prisma.salesTerritory.findMany({
      where: { tenantId, deletedAt: null },
      include: { members: true, children: { where: { deletedAt: null } } },
      orderBy: { name: "asc" },
    });
  ...
  async addMember(tenantId: string, territoryId: string, dto: any) {
    ...
    if (!territory) throw new NotFoundException("Territory not found");
    const existing = await prisma.salesTeamMember.findFirst({
      where: { territoryId, userId: dto.userId },
    });
    if (existing)
      throw new BadRequestException(
  ...
  async removeMember(tenantId: string, territoryId: string, userId: string) {
    const member = await prisma.salesTeamMember.findFirst({
      where: { territoryId, userId, tenantId },
    });
  ...
  async createRule(tenantId: string, orgId: string, dto: any) {
    ...
    const territory = await prisma.salesTerritory.findFirst({
      where: { id: dto.territoryId, tenantId },
    });
    if (!territory) throw new NotFoundException("Territory not found");
  ...
  async getForecasts(tenantId: string, territoryId?: string) {
    ...
    if (territoryId) where.territoryId = territoryId;
    return prisma.salesTerritoryForecast.findMany({
      where,
      include: { territory: { select: { id: true, name: true } } },
  ...
  async updateForecast(tenantId: string, id: string, dto: any) {
    const existing = await prisma.salesTerritoryForecast.findFirst({
      where: { id, tenantId },
    });
    ...
    if (dto.pipelineValue !== undefined)
      data.pipelineValue = new Prisma.Decimal(dto.pipelineValue);
    if (dto.expectedValue !== undefined)
      data.expectedValue = new Prisma.Decimal(dto.expectedValue);
    if (dto.forecastValue !== undefined)
      data.forecastValue = new Prisma.Decimal(dto.forecastValue);
    if (dto.confidence !== undefined)
      data.confidence = new Prisma.Decimal(dto.confidence);
  ...
  async realignTerritory(
    ...
    if (dto.newParentId !== undefined)
      updateData.parent = dto.newParentId
        ? { connect: { id: dto.newParentId } }
        : { disconnect: true };
    ...
    await prisma.salesTerritoryRealignment.create({
      data: {
        ...
        orgId: resolvedOrgId,
        territoryId: dto.territoryId,
        previousManagerId: territory.managerId,
        newManagerId: dto.newManagerId || null,
        previousParentId: territory.parentId,
  ...
  async getTerritoryAnalytics(tenantId: string) {
    const [totalTerritories, totalMembers, activeRules, totalForecasts] =
      await Promise.all([
    ...
    const forecasts = await prisma.salesTerritoryForecast.findMany({
      where: { tenantId },
    });
    return {
      totalTerritories,
      ...
      activeRules,
      totalForecasts,
      totalPipelineValue: forecasts.reduce(
        (s, f) => s + Number(f.pipelineValue),
        0,
      ),
      totalForecastValue: forecasts.reduce(
        (s, f) => s + Number(f.forecastValue),
        0,
      ),
    };
  }
