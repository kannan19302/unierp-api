...
export class PosExpansionService {
  ...
  async getShifts(
    ...
    const [data, total] = await Promise.all([
      this.p.posShift.findMany({
        where,
        orderBy: { startTime: "desc" },
        skip,
        take,
        include: { _count: { select: { shiftTransactions: true } } },
      }),
  ...
  async getShiftById(tenantId: string, id: string) {
    const shift = await this.p.posShift.findFirst({
      where: { id, tenantId },
      include: { cashDrawers: true, shiftTransactions: true },
    });
  ...
  async startShift(
    ...
    const active = await this.p.posShift.findFirst({
      where: { tenantId, registerId: dto.registerId, status: "OPEN" },
    });
    if (active)
      throw new BadRequestException(
        "An active shift already exists on this register",
      );
  ...
  async closeShift(
    tenantId: string,
    shiftId: string,
    dto: { closingCash: number; declaredCash: number; notes?: string },
  ) {
    const shift = await this.p.posShift.findFirst({
    ...
    const variance = new Prisma.Decimal(dto.declaredCash).sub(
    ...
    return this.p.posShift.update({
      ...
      data: {
        ...
        declaredCash: new Prisma.Decimal(dto.declaredCash),
  ...
  async addCashDrawer(
    tenantId: string,
    shiftId: string,
    dto: { type: string; amount: number; reason?: string; createdBy: string },
  ) {
    const shift = await this.p.posShift.findFirst({
      where: { id: shiftId, tenantId },
    });
  ...
  async getCashDrawers(tenantId: string, shiftId: string) {
    return this.p.posShiftCashDrawer.findMany({
  ...
  async getRegisterById(tenantId: string, id: string) {
    const reg = await this.p.posRegister.findFirst({
      where: { id, tenantId },
      include: { terminal: true },
    });
  ...
  async closeRegister(
    ...
    if (reg.status === "CLOSED")
      throw new BadRequestException("Register is already closed");
    const variance = new Prisma.Decimal(dto.actualCash).sub(
  ...
  async getPaymentMethods(tenantId: string) {
    return this.p.posPaymentMethod.findMany({
      where: { tenantId },
  ...
  async createPaymentMethod(tenantId: string, dto: any) {
    const existing = await this.p.posPaymentMethod.findFirst({
  ...
  async updatePaymentMethod(tenantId: string, id: string, dto: any) {
    const method = await this.p.posPaymentMethod.findFirst({
      where: { id, tenantId },
    });
  ...
  async getRefundById(tenantId: string, id: string) {
    const refund = await this.p.posRefund.findFirst({
      where: { id, tenantId },
      include: { items: true },
    });
  ...
  async createRefund(tenantId: string, orgId: string, dto: any) {
    ...
    const originalOrder = await this.p.pOSOrder.findFirst({
    ...
    const totalRefund = dto.items.reduce(
      (sum: number, i: any) => sum + i.refundAmount,
      0,
    );
    return this.p.$transaction(async (tx: any) => {
      ...
      for (const item of dto.items) {
        ...
        await tx.posRefundItem.create({
          data: {
            ...
            unitPrice: new Prisma.Decimal(orderItem?.unitPrice || 0),
      ...
      await tx.pOSOrder.update({
        ...
        data: { status: dto.type === "EXCHANGE" ? "EXCHANGED" : "RETURNED" },
      });
      return refund;
    });
  }
  ...
  async approveRefund(
    ...
    if (!refund) throw new NotFoundException("Refund not found");
    if (dto.approved) {
      return this.p.posRefund.update({
        where: { id },
        data: {
          status: "APPROVED",
          approvedAt: new Date(),
    ...
    return this.p.posRefund.update({
      where: { id },
      data: { status: "REJECTED", notes: dto.notes || null },
    });
  }
  ...
  async getGiftCards(
    ...
    if (query.search) {
      where.OR = [
        ...
        { issuedTo: { contains: query.search, mode: "insensitive" } },
      ];
    }
    const [data, total] = await Promise.all([
      this.p.posGiftCard.findMany({
        where,
        orderBy: { createdAt: "desc" },
        skip,
        take,
  ...
  async issueGiftCard(
    ...
    const resolvedOrgId = await resolveOrgId(tenantId, orgId);
    const existing = await this.p.posGiftCard.findFirst({
      where: { tenantId, code: dto.code },
    });
    if (existing)
    ...
    await this.p.posGiftCardTransaction.create({
      data: {
        ...
        giftCardId: card.id,
        type: "ISSUE",
        amount: new Prisma.Decimal(dto.initialBalance),
  ...
  async topUpGiftCard(
    ...
    const card = await this.p.posGiftCard.findFirst({
  ...
  async getGiftCardTransactions(tenantId: string, giftCardId: string) {
    return this.p.posGiftCardTransaction.findMany({
  ...
  async getOrderTypes(tenantId: string) {
    return this.p.posOrderType.findMany({
  ...
  async createOrderType(tenantId: string, dto: any) {
    const existing = await this.p.posOrderType.findFirst({
      where: { tenantId, code: dto.code },
    });
    if (existing)
      throw new BadRequestException("Order type with this code already exists");
    if (dto.isDefault) {
      await this.p.posOrderType.updateMany({
        where: { tenantId },
  ...
  async getDiscountRules(
    ...
    if (query.status) where.status = query.status;
    const [data, total] = await Promise.all([
      this.p.posDiscountRule.findMany({
        where,
  ...
  async createDiscountRule(tenantId: string, orgId: string, dto: any) {
    const resolvedOrgId = await resolveOrgId(tenantId, orgId);
    return this.p.posDiscountRule.create({
      data: {
        tenantId,
        orgId: resolvedOrgId,
        ...dto,
  ...
  async updateDiscountRule(tenantId: string, id: string, dto: any) {
    const rule = await this.p.posDiscountRule.findFirst({
      where: { id, tenantId },
    });
  ...
  async getTaxRules(tenantId: string) {
    return this.p.posTaxRule.findMany({
      where: { tenantId, isActive: true },
      orderBy: { sortOrder: "asc" },
    });
  }
  ...
  async createTaxRule(tenantId: string, dto: any) {
    if (dto.isDefault) {
      await this.p.posTaxRule.updateMany({
        where: { tenantId },
        data: { isDefault: false },
      });
    }
    return this.p.posTaxRule.create({
      data: { tenantId, ...dto, rate: new Prisma.Decimal(dto.rate) },
    });
  }
  ...
  async getKitchenDisplays(tenantId: string) {
    return this.p.posKitchenDisplay.findMany({
      where: { tenantId },
      orderBy: { name: "asc" },
    });
  }
  ...
  async createKitchenDisplay(tenantId: string, dto: any) {
    const existing = await this.p.posKitchenDisplay.findFirst({
      where: { tenantId, code: dto.code },
    });
    if (existing)
      throw new BadRequestException(
        "Kitchen display with this code already exists",
      );
    return this.p.posKitchenDisplay.create({ data: { tenantId, ...dto } });
  }

  async updateKitchenDisplay(tenantId: string, id: string, dto: any) {
    const kd = await this.p.posKitchenDisplay.findFirst({
      where: { id, tenantId },
    });
  ...
  async getKitchenOrders(
    ...
    return this.p.posKitchenOrder.findMany({
      where,
  ...
  async processSplitPayment(
    ...
    const order = await this.p.pOSOrder.findFirst({
      where: { id: dto.orderId, tenantId },
    });
    if (!order) throw new NotFoundException("Order not found");
    const totalSplit = dto.splits.reduce((s, sp) => s + sp.amount, 0);
    if (Math.abs(totalSplit - Number(order.grandTotal)) > 0.01)
      throw new BadRequestException(
        "Split payment totals do not match order grand total",
      );
    return this.p.$transaction(async (tx: any) => {
      for (const split of dto.splits) {
        await tx.posSplitPayment.create({
          data: {
            ...
            authCode: split.authCode || null,
            giftCardId: split.giftCardId || null,
            status: "COMPLETED",
          },
        });
        if (split.giftCardId) {
          const gc = await tx.posGiftCard.findFirst({
            where: { id: split.giftCardId, tenantId },
          });
          if (gc) {
            const nb = Number(gc.currentBalance) - split.amount;
            await tx.posGiftCard.update({
              where: { id: split.giftCardId },
              data: { currentBalance: new Prisma.Decimal(Math.max(0, nb)) },
            });
            await tx.posGiftCardTransaction.create({
              data: {
                tenantId,
                ...
                type: "REDEEM",
                amount: new Prisma.Decimal(split.amount),
                balance: new Prisma.Decimal(Math.max(0, nb)),
              },
            });
          }
        }
  ...
  async getSplitPayments(tenantId: string, orderId: string) {
    return this.p.posSplitPayment.findMany({
  ...
  async deductInventory(
    ...
    const inv = await tx.inventory.findFirst({
      where: { tenantId, orgId, productId },
    });
    if (inv) {
