...
export class DataQualityService {
  ...
  private getModel(modelName: string): any {
    if (!QUALITY_MODELS.has(modelName)) {
      throw new BadRequestException(`Model '${modelName}' is not supported for data quality operations`);
    }
  ...
  async deduplicate(tenantId: string, modelName: string, fields: string[]): Promise<DuplicateGroup[]> {
    if (!fields || fields.length === 0) {
      throw new BadRequestException('At least one field is required for deduplication');
    }
    ...
    for (const record of allRecords) {
      const key = fields.map((f) => String(record[f] ?? '')).join('||').toLowerCase();
      if (!key || key === '||||') continue;
  ...
  async mergeDuplicates(
    ...
    const primary = await model.findUnique({ where: { id: primaryId, tenantId } });
    if (!primary) {
    ...
    const merged = await prisma.$transaction(async (tx) => {
      ...
      for (const strategy of mergeStrategy) {
        ...
        const values = [primary, ...duplicates].map((r) => r[field]).filter((v) => v != null);
        ...
        switch (strategy.strategy) {
          ...
          case 'concat':
            ...
            break;
          case 'sum':
            updateData[field] = values.reduce((acc: number, v: any) => acc + (Number(v) || 0), 0);
            break;
          case 'latest':
            updateData[field] = values[values.length - 1];
            break;
    ...
    if (this.eventEmitter) {
      this.eventEmitter.emit('data-quality.merged', {
        tenantId, modelName, primaryId, mergedCount: duplicateIds.length,
      });
    }
  ...
  async validateDataQuality(tenantId: string, modelName: string): Promise<QualityCheckResult> {
    this.getModel(modelName);
    const model = (prisma as any)[modelName];
    ...
    for (const field of requiredFields) {
      const missing = allRecords.filter((r: any) => r[field] == null || String(r[field]).trim() === '');
      if (missing.length > 0) {
        ...
        issues.push({
          ...
          examples: missing.slice(0, 3).map((r: any) => ({ id: r.id, [field]: r[field] })),
        });
      }
    }
    ...
    for (const field of emailFields) {
      ...
      const invalid = allRecords.filter((r: any) => r[field] != null && r[field] !== '' && !this.isValidEmail(r[field]));
      if (invalid.length > 0) {
        ...
        issues.push({
          ...
          examples: invalid.slice(0, 3).map((r: any) => ({ id: r.id, [field]: r[field] })),
        });
      }
    }
    ...
    for (const field of phoneFields) {
      if (!allRecords[0] || !(field in allRecords[0])) continue;
      const invalid = allRecords.filter((r: any) => r[field] != null && r[field] !== '' && !this.isValidPhone(String(r[field])));
      if (invalid.length > 0) {
        issueCount += invalid.length;
        issues.push({
          type: 'format_error',
    ...
    const totalChecks = allRecords.length * (requiredFields.length + emailFields.length + phoneFields.length);
  ...
  standardizeAddress(address: {
    ...
    if (address.addressLine1) {
      standardized.addressLine1 = address.addressLine1.trim().replace(/\s+/g, ' ');
    }
    if (address.addressLine2) {
      standardized.addressLine2 = address.addressLine2.trim().replace(/\s+/g, ' ');
    }
    if (address.city) {
  ...
  validateEmail(email: string): { valid: boolean; normalized?: string; reason?: string } {
    ...
    if (disposableDomains.has(domain)) {
      return { valid: false, reason: 'Disposable email domain not allowed', normalized };
    }
