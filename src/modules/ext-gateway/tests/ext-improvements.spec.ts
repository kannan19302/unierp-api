...
describe("RBAC helpers (#1)", () => {
  it("hasScope / assertScopes all + any", () => {
    ...
    expect(() => assertScopes(claims(), ["healthcare:write"])).toThrow(
      ExtForbiddenError,
    );
    expect(() =>
      assertScopes(claims({ scopes: ["a"] }), ["a", "b"], "any"),
    ).not.toThrow();
  });
  ...
  it("RequireScopes guard reads req.tenantContext", () => {
    ...
    const ctx = (scopes: string[]) => ({
      switchToHttp: () => ({
        getRequest: () => ({ tenantContext: claims({ scopes }) }),
      }),
    });
...
describe("per-app secret selection (#2)", () => {
  it("decodeTokenUnverified exposes appSlug for secret selection then verify confirms", () => {
    const tok = signTenantToken(
      claims({ appSlug: "education" }) as any,
      "edu-secret",
    );
...
describe("webhook signing (#6)", () => {
  it("round-trips and rejects tamper/replay", () => {
    ...
    expect(() => verifyWebhook(body + "x", sig, ts, SECRET)).toThrow(
      /signature/,
    );
    expect(() => verifyWebhook(body, sig, ts - 10000, SECRET)).toThrow(
      /tolerance|replay/,
    );
  });
});
