  ...
  return {
    prisma: {
      ...
      backgroundJob: {
        groupBy: vi.fn(() =>
          Promise.resolve([
            { queueName: "default", status: "COMPLETED", _count: 5 },
          ]),
        ),
      ...
      scheduledTask: {
        findMany: vi.fn(() =>
          Promise.resolve([{ id: "cron-1", name: "Backup Task" }]),
        ),
        findFirst: vi.fn(() =>
          Promise.resolve({ id: "cron-1", name: "Backup Task" }),
        ),
...
describe("OperationsService", () => {
  ...
  beforeEach(() => {
    fakeEmailQueue = {
      add: vi.fn().mockResolvedValue({ id: "bull-email-1" }),
      name: "email",
    };
    fakeExportQueue = {
