...
export class CrmLeadsService {
  constructor(
    @Inject(CrmLeadScoringService)
    private readonly leadScoring: CrmLeadScoringService,
  ) {}
  ...
  async convertLead(
    ...
    const custName =
      customerName || lead.company || `${lead.firstName} ${lead.lastName}`;
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const oppName =
        opportunityName ||
        `Opportunity from ${lead.firstName} ${lead.lastName}`;
  ...
  async reactivateLead(tenantId: string, id: string, reason?: string) {
    ...
    if (existing.status !== "DISQUALIFIED") {
      throw new BadRequestException(
        "Only DISQUALIFIED leads can be reactivated.",
      );
    }
    const notes = reason
