...
describe("InventoryService — QA disposition routing/templates, reorder automation", () => {
  ...
  describe("QA disposition routing", () => {
    it("throws BadRequest routing an inspection with no disposition yet", async () => {
      db.qualityInspection.findFirst.mockResolvedValue({
        id: "qa1",
        disposition: null,
      });
      await expect(
        service.routeQAInspectionDisposition("t1", "qa1", "u1"),
    ...
    it("quarantines the linked batch when disposition is QUARANTINE", async () => {
      db.qualityInspection.findFirst.mockResolvedValue({
        ...
        referenceId: "se1",
        inspectionNumber: "QA-1",
      });
      db.batch.findFirst.mockResolvedValue({ id: "b1", status: "ACTIVE" });

      const result = await service.routeQAInspectionDisposition(
        "t1",
        "qa1",
        "u1",
      );
      ...
      expect(db.batchQuarantineLog.create).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe("QA inspection templates", () => {
    it("creates an inspection pre-populated from a template checklist", async () => {
      db.qAInspectionTemplate.findFirst.mockResolvedValue({
        id: "tpl1",
        checklist: [{ parameter: "Weight", criteria: "±5%" }],
      });
      db.qualityInspection.count.mockResolvedValue(0);
      db.qualityInspection.create.mockResolvedValue({ id: "qa3" });

      await service.createQAInspectionFromTemplate(
        "t1",
        "org-1",
        "u1",
        "tpl1",
      ...
      expect(db.qualityInspection.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            tenantId: "t1",
            orgId: "org-1",
    ...
    it("throws NotFound creating from an unknown template", async () => {
      ...
      await expect(
        service.createQAInspectionFromTemplate(
          "t1",
          "org-1",
          "u1",
          "missing",
  ...
  describe("createRequisitionFromReorderRule", () => {
    it("creates a requisition and stamps lastTriggeredAt", async () => {
      db.reorderRule.findFirst.mockResolvedValue({
        id: "r1",
        productId: "p1",
        reorderQty: { toString: () => "50" },
        minQty: { toString: () => "10" },
        leadTimeDays: 7,
        product: { name: "Widget", costPrice: { toString: () => "3" } },
      });
      db.purchaseRequisition.count.mockResolvedValue(0);
      db.purchaseRequisition.create.mockImplementation(({ data }: any) =>
        Promise.resolve({ id: "req1", ...data }),
      );
      db.reorderRule.update.mockResolvedValue({});

      const result = await service.createRequisitionFromReorderRule(
        "t1",
        "org-1",
        "u1",
        "r1",
      ...
      expect(db.reorderRule.update).toHaveBeenCalledWith(
    ...
    it("throws NotFound for an unknown reorder rule", async () => {
      ...
      await expect(
        service.createRequisitionFromReorderRule(
          "t1",
          "org-1",
          "u1",
          "missing",
