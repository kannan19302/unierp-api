...
describe("CrmCadencesService", () => {
  ...
  it("creates a cadence with mixed-channel steps", async () => {
    (
      prisma.emailSequence.create as ReturnType<typeof vi.fn>
    ).mockImplementation(({ data }) =>
      Promise.resolve({
        id: "seq1",
        ...data,
        steps: [{ channel: "EMAIL" }, { channel: "CALL" }],
      }),
    );
    const cadence = await service.createCadence(
      TENANT,
      "org1",
      {
        name: "Outbound Q3",
        steps: [
          ...
          {
            channel: "CALL",
            instructions: "Call and pitch demo",
            delayDays: 2,
            sortOrder: 1,
          },
        ],
  ...
  it("enrolls a lead when it matches an auto-enroll rule and is not already enrolled", async () => {
    ...
    (
      prisma.cadenceAutoEnrollRule.findMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue([
      { id: "rule1", sequenceId: "seq1", conditions: { minScore: 50 } },
    ]);
    (
      prisma.emailSequenceEnrollment.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue(null);
    (
      prisma.emailSequenceStep.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ delayDays: 1 });
    (
      prisma.emailSequenceEnrollment.create as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
  ...
  it("advances EMAIL steps immediately and creates a task for non-EMAIL steps", async () => {
    (
      prisma.emailSequenceEnrollment.findMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue([
      {
        id: "enr-1",
        currentStep: 0,
        ...
        sequence: {
          steps: [
            { id: "s1", channel: "EMAIL", sortOrder: 0 },
            { id: "s2", channel: "CALL", delayDays: 1, sortOrder: 1 },
          ],
        },
      },
      {
    ...
    (
      prisma.emailSequenceEnrollment.update as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
    (
      prisma.cadenceStepTask.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue(null);
    (
      prisma.cadenceStepTask.create as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
      ...
      enrollment: {
        ...
        sequence: {
          steps: [
            { sortOrder: 0 },
            { sortOrder: 1 },
            { delayDays: 3, sortOrder: 2 },
          ],
        },
      },
    });
    (
      prisma.cadenceStepTask.update as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
    (
      prisma.emailSequenceEnrollment.update as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});

    const result = await service.completeStepTask(
      TENANT,
      "task1",
      { status: "COMPLETED" },
      "user-1",
  ...
  it("rejects completing an already-resolved task", async () => {
    (
      prisma.cadenceStepTask.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "task1", status: "COMPLETED" });
    await expect(
      service.completeStepTask(
        TENANT,
        "task1",
        { status: "COMPLETED" },
        "user-1",
