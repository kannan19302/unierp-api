...
export class CrmContractLifecycleService {
  ...
  async createAmendment(
    tenantId: string,
    dto: z.infer<typeof amendmentSchema>,
    createdBy: string,
  ) {
    ...
    return prisma.contractAmendment.create({
      data: {
        ...
        tenantId,
        amendmentNumber: `AMD-${contract.contractNumber}-${count + 1}`,
        createdBy,
      },
    });
  }
  ...
  async updateAmendment(
    tenantId: string,
    id: string,
    dto: Partial<z.infer<typeof amendmentSchema>>,
  ) {
  ...
  async approveAmendment(tenantId: string, id: string, approvedBy: string) {
    ...
    if (a.status !== "PENDING_APPROVAL")
      throw new BadRequestException(
        "Only PENDING_APPROVAL amendments can be approved",
      );
    return prisma.contractAmendment.update({
      where: { id },
  ...
  async rejectAmendment(tenantId: string, id: string) {
    ...
    if (a.status !== "PENDING_APPROVAL")
      throw new BadRequestException(
        "Only PENDING_APPROVAL amendments can be rejected",
      );
    return prisma.contractAmendment.update({
      where: { id },
  ...
  async executeAmendment(tenantId: string, id: string) {
    ...
    if (a.status !== "APPROVED")
      throw new BadRequestException("Only APPROVED amendments can be executed");
    if (a.amendmentType === "PRICE_CHANGE" && a.newValue) {
      await prisma.contract.update({
        where: { id: a.contractId },
        data: { value: a.newValue },
      });
    }
  ...
  async listPriceEscalationRules(tenantId: string, contractId?: string) {
    ...
    if (contractId) where.contractId = contractId;
    return prisma.contractPriceEscalationRule.findMany({
      where,
      orderBy: { createdAt: "desc" },
      include: {
        contract: {
          select: { id: true, contractNumber: true, title: true, value: true },
        },
      },
    });
  }
  ...
  async getPriceEscalationRule(tenantId: string, id: string) {
    const r = await prisma.contractPriceEscalationRule.findFirst({
      where: { id, tenantId },
    });
  ...
  async createPriceEscalationRule(
    tenantId: string,
    dto: z.infer<typeof priceEscalationRuleSchema>,
  ) {
    if (dto.contractId)
      await prisma.contract
        .findFirstOrThrow({ where: { id: dto.contractId, tenantId } })
        .catch(() => {
          throw new NotFoundException("Contract not found");
        });
    return prisma.contractPriceEscalationRule.create({
      data: { ...dto, tenantId, escalationValue: dto.escalationValue },
    });
  }

  async updatePriceEscalationRule(
    tenantId: string,
    id: string,
    dto: Partial<z.infer<typeof priceEscalationRuleSchema>>,
  ) {
  ...
  async applyEscalation(tenantId: string, ruleId: string, contractId: string) {
    ...
    if (rule.escalationType === "PERCENTAGE")
      newValue += newValue * (Number(rule.escalationValue) / 100);
    else if (rule.escalationType === "FIXED_AMOUNT")
      newValue += Number(rule.escalationValue);
  ...
  async processAutoRenewal(tenantId: string, contractId: string) {
    ...
    if (contract.status === "EXPIRED" || contract.status === "TERMINATED")
      throw new BadRequestException("Cannot renew expired/terminated contract");
    ...
    let newValue = Number(contract.value);
    const escalationRule = await prisma.contractPriceEscalationRule.findFirst({
      where: { tenantId, contractId, isActive: true },
    });
    let escalationApplied = 0;
    if (
      escalationRule &&
      escalationRule.nextEscalationDate &&
      new Date(escalationRule.nextEscalationDate) <= new Date()
    ) {
      if (escalationRule.escalationType === "PERCENTAGE")
        newValue += newValue * (Number(escalationRule.escalationValue) / 100);
      else if (escalationRule.escalationType === "FIXED_AMOUNT")
        newValue += Number(escalationRule.escalationValue);
      if (escalationRule.maxCap)
        newValue = Math.min(newValue, Number(escalationRule.maxCap));
    ...
    await prisma.contract.update({
      ...
      data: {
        ...
        value: newValue,
        status: "ACTIVE",
        renewedFromId: contract.renewedFromId || contract.id,
      },
    });

    const count = await prisma.contractAutoRenewalLog.count({
  ...
  async listAutoRenewals(tenantId: string, contractId?: string) {
    ...
    if (contractId) where.contractId = contractId;
    return prisma.contractAutoRenewalLog.findMany({
      where,
  ...
  async getAutoRenewal(tenantId: string, id: string) {
    const r = await prisma.contractAutoRenewalLog.findFirst({
      where: { id, tenantId },
    });
  ...
  async updateRenewalPreferences(
    ...
    return prisma.contract.update({
      ...
      data: {
        autoRenew,
        renewalTermMonths: renewalTermMonths ?? contract.renewalTermMonths,
      },
    });
  }
    ...
    for (const c of contracts) {
      ...
      const existing = await prisma.contractExpirationPipelineItem.findFirst({
        where: {
          tenantId,
          contractId: c.id,
          status: { notIn: ["RENEWED", "DISMISSED", "EXPIRED"] },
        },
      });
      if (existing) {
        await prisma.contractExpirationPipelineItem.update({
          ...
          data: {
            ...
            actionRequired:
              daysToExpiry <= 30
                ? "Immediate renewal action needed"
                : daysToExpiry <= 90
  ...
  async getExpirationPipelineItem(tenantId: string, id: string) {
    const p = await prisma.contractExpirationPipelineItem.findFirst({
      where: { id, tenantId },
    });
  ...
  async startRenewalFromPipeline(tenantId: string, id: string, userId: string) {
    ...
    await prisma.contractExpirationPipelineItem.update({
      where: { id },
      data: { status: "IN_PROGRESS", assignedTo: userId },
    });
  ...
  async dismissPipelineItem(tenantId: string, id: string, userId: string) {
    ...
    return prisma.contractExpirationPipelineItem.update({
      where: { id },
      data: {
        status: "DISMISSED",
  ...
  async createTemplate(
    tenantId: string,
    dto: z.infer<typeof contractTemplateSchema>,
  ) {
    ...
    return prisma.contractTemplate.create({
      data: {
        ...
        clauses: clauses as Prisma.InputJsonValue,
        tenantId,
      },
    });
  }
  ...
  async updateTemplate(
    tenantId: string,
    id: string,
    dto: Partial<z.infer<typeof contractTemplateSchema>>,
  ) {
  ...
  async generateFromTemplate(
    ...
    const templateData =
      typeof template.template === "object" && template.template !== null
        ? (template.template as Record<string, unknown>)
        : {};
    return {
      template,
      generatedData: { ...templateData, ...overrides },
      message: "Contract data prepared from template",
    };
  }
  ...
  async listClauses(tenantId: string, category?: string) {
    const where: Prisma.ContractClauseWhereInput = {
      tenantId,
      deletedAt: null,
    };
  ...
  async getClause(tenantId: string, id: string) {
    const c = await prisma.contractClause.findFirst({
      where: { id, tenantId, deletedAt: null },
    });
  ...
  async createClause(
    tenantId: string,
    dto: z.infer<typeof contractClauseSchema>,
  ) {
  ...
  async updateClause(
    tenantId: string,
    id: string,
    dto: Partial<z.infer<typeof contractClauseSchema>>,
  ) {
  ...
  async getAnalytics(tenantId: string) {
    const [
      ...
      prisma.contract.count({
        where: { tenantId, status: "EXPIRING_SOON", deletedAt: null },
      }),
      prisma.contractExpirationPipelineItem.count({
        where: { tenantId, status: "OPEN" },
      }),
      prisma.contractAmendment.count({ where: { tenantId, deletedAt: null } }),
      prisma.contractPriceEscalationRule.count({
    ...
    return {
      ...
      activeContracts,
      expiringSoon,
      expiringPipelineCount: expiringCount,
      amendments,
      activeEscalations: escalations,
      templates,
      totalContractValue: totalValue._sum.value || 0,
    };
  }

  async getDashboard(tenantId: string) {
    const [expiringByStage, renewalsByMonth] = await Promise.all([
      prisma.contractExpirationPipelineItem.groupBy({
        by: ["stage"],
        where: { tenantId },
        _count: true,
      }),
      prisma.contractAutoRenewalLog.groupBy({
        by: ["status"],
        where: { tenantId },
        _count: true,
      }),
