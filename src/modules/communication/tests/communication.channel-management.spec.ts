...
describe("CommunicationService — channel management & roles (US-B1/B2/B3)", () => {
  ...
  it("never finds a channel belonging to another tenant when updating", async () => {
    ...
    await expect(
      svc.updateChannel("tenant-A", "channel-in-tenant-B", "u1", {
        name: "hacked",
      }),
    ).rejects.toThrow("Channel not found");
    expect(prisma.channel.findFirst).toHaveBeenCalledWith({
      where: { id: "channel-in-tenant-B", tenantId: "tenant-A" },
    });
  });
  ...
  it("createChannel seeds the creator as a ChannelMember with role OWNER", async () => {
    ...
    vi.mocked(prisma.channel.create).mockImplementation(async (args: never) => {
      const a = args as {
        data: {
          members?: {
            create?: { tenantId: string; userId: string; role: string }[];
          };
        };
      };
      return {
        ...
        name: "qa-owner-test",
        kind: "CHANNEL",
        members: a.data.members?.create ?? [],
      } as never;
    });

    const created = await svc.createChannel("t1", "org1", "creator-1", {
      name: "qa-owner-test",
    });
    ...
    expect(prisma.channel.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          members: {
            create: [{ tenantId: "t1", userId: "creator-1", role: "OWNER" }],
          },
        }),
      }),
    );
    expect(
      (created as unknown as { members: { userId: string; role: string }[] })
        .members,
    ).toEqual([{ tenantId: "t1", userId: "creator-1", role: "OWNER" }]);
  });
  ...
  it("creator of a freshly created channel can immediately list members, rename, and archive it (no 403)", async () => {
    ...
    const renamed = await svc.updateChannel("t1", "c-new", "creator-1", {
      name: "renamed",
    });
    ...
    await expect(
      svc.updateChannel("t1", "c-new", "creator-1", { archived: true }),
  ...
  it("blocks a plain MEMBER from renaming a channel (403)", async () => {
    ...
    await expect(
      svc.updateChannel("t1", "c1", "u1", { name: "new-name" }),
    ).rejects.toThrow("You do not have permission to manage this channel.");
  });
  ...
  it("blocks an ADMIN (not OWNER) from archiving a channel", async () => {
    ...
    await expect(
      svc.updateChannel("t1", "c1", "u1", { archived: true }),
    ).rejects.toThrow("You do not have permission to manage this channel.");
  });
  ...
  it("blocks a plain MEMBER from adding a channel member", async () => {
    ...
    await expect(
      svc.addChannelMember("t1", "c1", "u1", "newUser"),
    ).rejects.toThrow("You do not have permission to manage this channel.");
  });
  ...
  it("allows OWNER/ADMIN to add a member and posts a SYSTEM join announcement", async () => {
    ...
    vi.mocked(prisma.channelMember.create).mockResolvedValue({
      id: "cm1",
      role: "MEMBER",
    } as never);
    ...
    expect(prisma.message.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          kind: "SYSTEM",
          content: expect.stringContaining("joined the channel"),
        }),
      }),
    );
  });
  ...
  it("removes a member, retains history (no message deletion), and posts a departure announcement", async () => {
    ...
    expect(prisma.message.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          kind: "SYSTEM",
          content: expect.stringContaining("left the channel"),
        }),
      }),
    );
  });
  ...
  it("blocks removing the channel OWNER", async () => {
    ...
    await expect(
      svc.removeChannelMember("t1", "c1", "admin1", "owner1"),
    ).rejects.toThrow("Cannot remove the channel owner.");
  });
  ...
  it("rejects joining a PRIVATE channel directly", async () => {
    ...
    await expect(svc.joinChannel("t1", "c1", "u1")).rejects.toThrow(
