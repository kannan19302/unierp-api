...
describe("PaymentSchedulesService (extra)", () => {
  ...
  it("should get payment schedule by id", async () => {
    ...
    (prisma.paymentSchedule.findFirst as any).mockResolvedValue(mockSchedule);
  ...
  it("should throw NotFoundException for missing payment schedule", async () => {
    (prisma.paymentSchedule.findFirst as any).mockResolvedValue(null);
  ...
  it("should update schedule status", async () => {
    (prisma.paymentSchedule.findFirst as any).mockResolvedValue({
      id: "1",
      status: "PENDING",
    });
    (prisma.paymentSchedule.update as any).mockResolvedValue({
      id: "1",
      status: "PAID",
    });
  ...
  it("should throw NotFoundException when updating status for missing schedule", async () => {
    (prisma.paymentSchedule.findFirst as any).mockResolvedValue(null);
    await expect(service.updateStatus("tenant-1", "x", "PAID")).rejects.toThrow(
      NotFoundException,
    );
  });

  it("should get schedules by purchase order", async () => {
    (prisma.paymentSchedule.findMany as any).mockResolvedValue([
      {
        id: "1",
        amount: 500,
        dueDate: new Date(),
    ...
    const result = await service.getSchedulesByPurchaseOrder(
      "tenant-1",
      "po-1",
    );
    ...
    expect(prisma.paymentSchedule.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { tenantId: "tenant-1", purchaseOrderId: "po-1" },
      }),
    );
  });
  ...
  it("should get schedules by date range", async () => {
    ...
    (prisma.paymentSchedule.count as any).mockResolvedValue(1);

    const result = await service.getSchedulesByDateRange(
      "tenant-1",
      "2026-01-01",
      "2026-06-30",
      {},
  ...
  it("should throw BadRequestException when marking non-pending schedule as overdue", async () => {
    ...
    await expect(service.markOverdue("tenant-1", "1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("should throw NotFoundException when bulk creating from missing PO", async () => {
    ...
    await expect(
      service.bulkCreateFromPo("tenant-1", "org-1", "missing-po", [
        { dueDate: "2026-07-01", amount: 500 },
      ]),
