...
export class LotSerialTrackingService {
  ...
  async quarantineBatch(tenantId: string, id: string) {
    ...
    if (batch.status === "QUARANTINE")
      throw new BadRequestException("Batch is already in quarantine");
  ...
  async recordLotMovement(
    tenantId: string,
    performedBy: string,
    dto: z.infer<typeof recordLotMovementSchema>,
  ) {
    const batch = await prisma.batch.findFirst({
  ...
  async createSerial(
    tenantId: string,
    dto: z.infer<typeof createSerialSchema>,
  ) {
    ...
    if (existing)
      throw new BadRequestException(
        "Serial number already exists for this product",
      );
  ...
  async markSerialSold(tenantId: string, id: string, salesOrderId?: string) {
    ...
    if (serial.status !== "AVAILABLE" && serial.status !== "RESERVED") {
      throw new BadRequestException(
        "Serial must be AVAILABLE or RESERVED to mark as sold",
      );
    }
    return prisma.$transaction([
      prisma.serialNumber.update({
        ...
        data: {
          status: "SOLD",
          salesOrderId: salesOrderId ?? serial.salesOrderId,
        },
      }),
  ...
  async markSerialReturned(tenantId: string, id: string, notes?: string) {
    ...
    if (serial.status !== "SOLD")
      throw new BadRequestException("Only SOLD serials can be returned");
    return prisma.$transaction([
      prisma.serialNumber.update({
        where: { id },
  ...
  async generatePickSuggestions(
    tenantId: string,
    dto: z.infer<typeof generatePickSuggestionsSchema>,
  ) {
    ...
    for (const batch of batches) {
      ...
      const available =
        ...
        parseFloat((batch.usedQty ?? 0).toString());
  ...
  async listPickSuggestions(
    tenantId: string,
    filters: { referenceId?: string; status?: string },
  ) {
    const where: Record<string, unknown> = { tenantId };
    if (filters.referenceId) where["referenceId"] = filters.referenceId;
    if (filters.status) where["status"] = filters.status;
    return prisma.pickSuggestion.findMany({
      where,
      orderBy: { createdAt: "asc" },
    });
  }

  async confirmPickSuggestion(
    tenantId: string,
    id: string,
    dto: z.infer<typeof confirmPickSchema>,
  ) {
    const suggestion = await prisma.pickSuggestion.findFirst({
      where: { id, tenantId },
    });
    ...
    return prisma.pickSuggestion.update({
      ...
      data: {
        status: "CONFIRMED",
        pickedQty: new Prisma.Decimal(dto.pickedQty),
      },
    });
  }
  ...
  async generateExpiryAlerts(
    tenantId: string,
    dto: z.infer<typeof generateExpiryAlertsSchema>,
  ) {
  ...
  async listQuarantineOrders(tenantId: string, status?: string) {
    ...
    return prisma.quarantineOrder.findMany({
      where,
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async getQuarantineOrder(tenantId: string, id: string) {
    const order = await prisma.quarantineOrder.findFirst({
      where: { id, tenantId },
    });
  ...
  async createQuarantineOrder(
    tenantId: string,
    createdBy: string,
    dto: z.infer<typeof quarantineOrderSchema>,
  ) {
  ...
  async releaseQuarantineOrder(
    tenantId: string,
    id: string,
    releasedBy: string,
    dto: z.infer<typeof releaseQuarantineSchema>,
  ) {
    const order = await prisma.quarantineOrder.findFirst({
      where: { id, tenantId },
    });
    ...
    if (order.status !== "ACTIVE")
      throw new BadRequestException("Quarantine order is not ACTIVE");
  ...
  async scrapQuarantineOrder(tenantId: string, id: string) {
    ...
    if (order.status !== "ACTIVE")
      throw new BadRequestException("Quarantine order is not ACTIVE");
    return prisma.quarantineOrder.update({
      where: { id },
      data: { status: "SCRAPPED" },
    });
  }
  ...
  async getLotSerialDashboard(tenantId: string) {
    const [
      ...
      prisma.expiryAlert.count({
        where: { tenantId, severity: "CRITICAL", acknowledged: false },
      }),
  ...
  async getExpiryReport(tenantId: string) {
    ...
    const [expired, expiringIn7, expiringIn30, expiringIn90] =
      await Promise.all([
        prisma.batch.count({
          where: { tenantId, status: "ACTIVE", expiryDate: { lt: now } },
        }),
        prisma.batch.count({
          where: {
            tenantId,
