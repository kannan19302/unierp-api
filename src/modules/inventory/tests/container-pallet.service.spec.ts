...
describe("ContainerPalletService", () => {
  ...
  it("listPalletTypes returns all types for tenant", async () => {
    ...
    expect(prisma.palletType.findMany).toHaveBeenCalledWith(
  ...
  it("createPalletType persists and returns record", async () => {
    ...
    const result = await svc.createPalletType(T, {
      code: "EUR",
      name: "Euro Pallet",
      category: "EURO",
    });
  ...
  it("createContainerType persists and returns record", async () => {
    ...
    const result = await svc.createContainerType(T, {
      code: "20GP",
      name: "20ft Dry Van",
      category: "DRY_VAN",
    });
  ...
  it("transitionLoadPlan DRAFT→OPTIMIZING succeeds", async () => {
    ...
    vi.mocked(prisma.loadPlan.update).mockResolvedValue({
      id: "lp1",
      status: "OPTIMIZING",
    } as any);
  ...
  it("transitionLoadPlan throws BadRequestException for invalid transition", async () => {
    ...
    await expect(
      svc.transitionLoadPlan(T, "lp1", "cancel", "user1"),
  ...
  it("transitionLoadPlan throws BadRequestException for unknown action", async () => {
    ...
    await expect(
      svc.transitionLoadPlan(T, "lp1", "explode", "user1"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("addPalletToLoadPlan creates pallet and recalculates stats", async () => {
    ...
    vi.mocked(prisma.loadPlanPallet.create).mockResolvedValue({
      id: "pal1",
    } as any);
    vi.mocked(prisma.loadPlanPallet.findMany).mockResolvedValue([
      { grossWeightKg: 500 },
    ] as any);
    vi.mocked(prisma.loadPlan.update).mockResolvedValue({} as any);
    const result = await svc.addPalletToLoadPlan(T, "lp1", {
      palletTypeId: "pt1",
      grossWeightKg: 500,
    });
  ...
  it("removePalletFromLoadPlan deletes pallet", async () => {
    ...
    vi.mocked(prisma.loadPlanPallet.deleteMany).mockResolvedValue({
      count: 1,
    } as any);
  ...
  it("transitionPackingPlan DRAFT→CONFIRMED succeeds", async () => {
    ...
    vi.mocked(prisma.packingPlan.update).mockResolvedValue({
      id: "pp1",
      status: "CONFIRMED",
    } as any);
  ...
  it("transitionPackingPlan throws BadRequestException for invalid status", async () => {
    ...
    await expect(
      svc.transitionPackingPlan(T, "pp1", "confirm"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("addCartonToPackingPlan creates carton", async () => {
    ...
    vi.mocked(prisma.loadCarton.create).mockResolvedValue({
      id: "c1",
      cartonNumber: "CTN-001",
    } as any);
    vi.mocked(prisma.loadCarton.findMany).mockResolvedValue([
      { grossWeightKg: 10 },
    ] as any);
    vi.mocked(prisma.packingPlan.update).mockResolvedValue({} as any);
    const result = await svc.addCartonToPackingPlan(T, "pp1", {
      cartonNumber: "CTN-001",
    });
  ...
  it("sealCarton marks carton as sealed", async () => {
    ...
    vi.mocked(prisma.loadCarton.update).mockResolvedValue({
      id: "c1",
      sealed: true,
    } as any);
