...
describe("PeopleOperationsService", () => {
  ...
  beforeEach(() => {
    mockPrisma = {
      peopleOnboardingTask: {
        findMany: vi.fn().mockResolvedValue([{ id: "tsk-1", employeeId: "emp-1" }]),
        create: vi.fn().mockImplementation(({ data }) => Promise.resolve({ id: "tsk-1", ...data })),
      },
      peopleTimeOffRequest: {
        findMany: vi.fn().mockResolvedValue([{ id: "tof-1", employeeId: "emp-1" }]),
        create: vi.fn().mockImplementation(({ data }) => Promise.resolve({ id: "tof-1", ...data })),
      },
      peoplePeerRecognition: {
        findMany: vi.fn().mockResolvedValue([{ id: "rec-1", receiverId: "emp-1" }]),
  ...
  it("should list onboarding tasks", async () => {
    const result = await service.getOnboardingTasks("tenant-1", { employeeId: "emp-1" });
  ...
  it("should create time off request", async () => {
    const data = { employeeId: "emp-1", leaveType: "VACATION", startDate: "2026-08-10", endDate: "2026-08-15", days: 5 };
  ...
  it("should create peer recognition", async () => {
    const data = { giverId: "emp-1", receiverId: "emp-2", badge: "HERO", message: "Great support on outage!" };
