...
describe("PickWavesService", () => {
  ...
  it("createWave — auto-numbers WV-000001", async () => {
    ...
    const result = await svc.createWave(tenantId, userId, {
      warehouseId: "wh-1",
    });
    expect(vi.mocked(prisma.pickWave.create)).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ waveNumber: "WV-000001" }),
      }),
    );
  ...
  it("addOrderToWave — throws on non-OPEN wave", async () => {
    ...
    await expect(svc.addOrderToWave(tenantId, waveId, "so-1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("addOrderToWave — happy path", async () => {
    ...
    vi.mocked(prisma.pickWaveOrder.create).mockResolvedValue({
      id: "wo1",
    } as never);
  ...
  it("addItemToWave — zero qty throws", async () => {
    ...
    await expect(
      svc.addItemToWave(tenantId, waveId, { productId: "p1", quantity: 0 }),
  ...
  it("startWave — throws if no items", async () => {
    ...
    await expect(svc.startWave(tenantId, waveId)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("startWave — OPEN→PICKING", async () => {
    ...
    vi.mocked(prisma.pickWave.update).mockResolvedValue({
      id: waveId,
      status: "PICKING",
    } as never);
  ...
  it("confirmPick — full pick sets item PICKED", async () => {
    ...
    vi.mocked(prisma.pickWaveItem.update).mockResolvedValue({
      id: "i1",
      status: "PICKED",
    } as never);
    vi.mocked(prisma.pickTask.updateMany).mockResolvedValue({
      count: 0,
    } as never);
    vi.mocked(prisma.pickWaveItem.count).mockResolvedValue(0);
  ...
  it("packWave — throws if PENDING items remain", async () => {
    ...
    await expect(svc.packWave(tenantId, waveId)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("cancelWave — cascades to tasks and items", async () => {
    ...
    vi.mocked(prisma.pickWaveItem.updateMany).mockResolvedValue({
      count: 3,
    } as never);
    vi.mocked(prisma.pickWave.update).mockResolvedValue({
      id: waveId,
  ...
  it("assignTask — happy path", async () => {
    ...
    vi.mocked(prisma.pickTask.create).mockResolvedValue({
      id: "task-1",
      assignedTo: "user-2",
    } as never);

    const result = await svc.assignTask(tenantId, {
      pickWaveId: waveId,
      pickItemId: "i1",
      assignedTo: "user-2",
    });
  ...
  it("startTask — ASSIGNED→IN_PROGRESS", async () => {
    ...
    vi.mocked(prisma.pickTask.update).mockResolvedValue({
      id: "task-1",
      status: "IN_PROGRESS",
    } as never);
