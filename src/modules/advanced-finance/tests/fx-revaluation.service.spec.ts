...
describe("FxRevaluationService", () => {
  ...
  describe("getRevaluationRuns", () => {
    it("should retrieve list of runs", async () => {
      ...
      vi.mocked(prisma.fxRevaluationRun.findMany).mockResolvedValue(
        mockRuns as any,
      );
  ...
  describe("createRevaluationRun", () => {
    it("should scan open customer invoices in targetCurrency and calculate draft gains", async () => {
      ...
      vi.mocked(prisma.fxRevaluationRun.create).mockResolvedValue({
        id: "run-1",
        status: "DRAFT",
      } as any);
  ...
  describe("postRevaluationRun", () => {
    it("should throw BadRequestException if run is already posted", async () => {
      vi.mocked(prisma.fxRevaluationRun.findFirst).mockResolvedValue({
        id: "run-1",
        status: "POSTED",
        details: [],
      } as any);
      ...
      await expect(
        service.postRevaluationRun("tenant-1", "run-1"),
    ...
    it("should post general ledger entries for gains and losses", async () => {
      const mockDetails = [
        {
          ...
          entityType: "INVOICE",
          entityId: "inv-1",
          unrealizedGainLoss: new Prisma.Decimal(150),
        },
        {
          accountId: "acc-ap",
          entityType: "PAYMENT_SCHEDULE",
          entityId: "sched-1",
          unrealizedGainLoss: new Prisma.Decimal(-80),
        },
      ];
      vi.mocked(prisma.fxRevaluationRun.findFirst).mockResolvedValue({
        id: "run-1",
      ...
      vi.mocked(prisma.fxRevaluationRun.update).mockResolvedValue({
        id: "run-1",
        status: "POSTED",
      } as any);
