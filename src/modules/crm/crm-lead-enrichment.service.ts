...
export class CrmLeadEnrichmentService {
  ...
  async listSources(tenantId: string, includeDeleted = false) {
    ...
    return prisma.crmEnrichmentSource.findMany({
      where,
      orderBy: { name: "asc" },
    });
  }
  ...
  async getSource(tenantId: string, id: string) {
    const s = await prisma.crmEnrichmentSource.findFirst({
  ...
  async createSource(
    tenantId: string,
    dto: z.infer<typeof enrichmentSourceSchema>,
  ) {
    const { config, ...rest } = dto;
    return prisma.crmEnrichmentSource.create({
      data: { ...rest, config: config as Prisma.InputJsonValue, tenantId },
    });
  }
  ...
  async updateSource(
    tenantId: string,
    id: string,
    dto: Partial<z.infer<typeof enrichmentSourceSchema>>,
  ) {
  ...
  async listRules(tenantId: string, objectType?: string) {
    const where: Prisma.CrmEnrichmentRuleWhereInput = {
      tenantId,
      deletedAt: null,
    };
    if (objectType) where.objectType = objectType;
    return prisma.crmEnrichmentRule.findMany({
      where,
      orderBy: { priority: "desc" },
      include: { source: { select: { id: true, name: true, provider: true } } },
    });
  }

  async getRule(tenantId: string, id: string) {
    const r = await prisma.crmEnrichmentRule.findFirst({
      where: { id, tenantId, deletedAt: null },
      include: { source: { select: { id: true, name: true, provider: true } } },
    });
  ...
  async createRule(
    tenantId: string,
    dto: z.infer<typeof enrichmentRuleSchema>,
  ) {
    await this.getSource(tenantId, dto.sourceId);
    const { conditions, ...rest } = dto;
    return prisma.crmEnrichmentRule.create({
      data: {
        ...rest,
  ...
  async updateRule(
    tenantId: string,
    id: string,
    dto: Partial<z.infer<typeof enrichmentRuleSchema>>,
  ) {
    await this.getRule(tenantId, id);
    const { conditions, ...rest } = dto;
    return prisma.crmEnrichmentRule.update({
      where: { id },
      data: {
        ...rest,
  ...
  async listFieldMappings(
    ...
    return prisma.crmEnrichmentFieldMapping.findMany({
      where,
  ...
  async createFieldMapping(
    tenantId: string,
    dto: z.infer<typeof fieldMappingSchema>,
  ) {
    await this.getSource(tenantId, dto.sourceId);
    return prisma.crmEnrichmentFieldMapping.create({
      data: { ...dto, tenantId },
    });
  }
  ...
  async updateFieldMapping(
    tenantId: string,
    id: string,
    dto: Partial<z.infer<typeof fieldMappingSchema>>,
  ) {
    const m = await prisma.crmEnrichmentFieldMapping.findFirst({
      where: { id, tenantId },
    });
    if (!m) throw new NotFoundException("Field mapping not found");
    return prisma.crmEnrichmentFieldMapping.update({
      where: { id },
      data: dto,
    });
  }
  ...
  async enrichLead(
    ...
    const sources = sourceId
      ...
      : await prisma.crmEnrichmentSource.findMany({
          where: { tenantId, enabled: true, deletedAt: null },
        });

    if (!sources.length)
      throw new BadRequestException("No active enrichment sources");

    const results: any[] = [];
    for (const source of sources) {
      const enrichedData = {
        company: lead.company || lead.email?.split("@")[1] || "Unknown",
      };
      const newData = await prisma.crmLeadEnrichmentData.create({
        data: {
          ...
          leadId,
          sourceId: source.id,
          enrichedData: enrichedData as Prisma.InputJsonValue,
          rawResponse: {} as Prisma.InputJsonValue,
          confidence: 0.85,
          matchedTo: lead.company || undefined,
        },
      });
  ...
  async getEnrichmentData(tenantId: string, leadId: string) {
    await prisma.lead
      .findFirstOrThrow({ where: { id: leadId, tenantId } })
      .catch(() => {
        throw new NotFoundException("Lead not found");
      });
    return prisma.crmLeadEnrichmentData.findMany({
  ...
  async bulkEnrich(
    ...
    for (const leadId of leadIds) {
      try {
        const res = await this.enrichLead(
          tenantId,
          leadId,
          sourceId,
          enrichedBy,
  ...
  async getSchedule(tenantId: string, id: string) {
    const s = await prisma.crmEnrichmentSchedule.findFirst({
      where: { id, tenantId },
      include: { rule: { select: { id: true, name: true, objectType: true } } },
    });
  ...
  async createSchedule(
    tenantId: string,
    dto: z.infer<typeof enrichmentScheduleSchema>,
  ) {
    await this.getRule(tenantId, dto.ruleId);
    const { conditions, ...rest } = dto;
    return prisma.crmEnrichmentSchedule.create({
      data: {
        ...rest,
        conditions: conditions as Prisma.InputJsonValue,
        tenantId,
  ...
  async updateSchedule(
    tenantId: string,
    id: string,
    dto: Partial<z.infer<typeof enrichmentScheduleSchema>>,
  ) {
    await this.getSchedule(tenantId, id);
    const { conditions, ...rest } = dto;
    return prisma.crmEnrichmentSchedule.update({
      where: { id },
      data: {
        ...rest,
        ...(conditions !== undefined
  ...
  async listLogs(
    ...
    return prisma.crmEnrichmentLog.findMany({
      where,
  ...
  async getLog(tenantId: string, id: string) {
    const l = await prisma.crmEnrichmentLog.findFirst({
      where: { id, tenantId },
    });
  ...
  async getStats(tenantId: string) {
    const [sources, rules, totalEnriched, successCount, failureCount] =
      await Promise.all([
        prisma.crmEnrichmentSource.count({
          where: { tenantId, deletedAt: null },
        }),
        prisma.crmEnrichmentRule.count({
          where: { tenantId, deletedAt: null },
        }),
        prisma.crmEnrichmentLog.count({ where: { tenantId } }),
        prisma.crmEnrichmentLog.count({
    ...
    return {
      ...
      successCount,
      failureCount,
      successRate: totalEnriched
        ? Math.round((successCount / totalEnriched) * 100)
        : 0,
    };
  }

  async getSourceEfficacy(tenantId: string) {
    const sources = await prisma.crmEnrichmentSource.findMany({
      where: { tenantId, deletedAt: null },
    });
    const results: any[] = [];
    for (const s of sources) {
      const total = await prisma.crmEnrichmentLog.count({
        where: { tenantId, sourceId: s.id },
      });
      const success = await prisma.crmEnrichmentLog.count({
        where: { tenantId, sourceId: s.id, status: "SUCCESS" },
      });
      results.push({
        sourceId: s.id,
        name: s.name,
