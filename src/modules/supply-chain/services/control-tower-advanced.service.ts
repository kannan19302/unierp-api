...
export class ControlTowerAdvancedService {
  async getEvents(
    ...
    const data = await prisma.controlTowerEvent.findMany({
      where,
  ...
  async getEventById(tenantId: string, id: string) {
    const event = await prisma.controlTowerEvent.findFirst({
      where: { id, tenantId },
      include: { actions: true },
    });
  ...
  async createEvent(tenantId: string, dto: any) {
    ...
    return prisma.controlTowerEvent.create({
      data: {
        tenantId,
        eventNumber,
  ...
  async assignEvent(tenantId: string, id: string, assignedTo: string) {
    ...
    if (!event) throw new NotFoundException("Event not found");
    return prisma.controlTowerEvent.update({
      where: { id },
      data: { assignedTo },
    });
  }
  ...
  async executeAction(
    ...
    if (!event) throw new NotFoundException("Event not found");
    return prisma.controlTowerAction.create({
      data: {
        tenantId,
        eventId,
        actionType,
  ...
  async getKpis(
    ...
    return prisma.controlTowerKpi.findMany({
      where,
  ...
  async updateKpiValue(tenantId: string, id: string, currentValue: number) {
    const kpi = await prisma.controlTowerKpi.findFirst({
      where: { id, tenantId },
    });
    if (!kpi) throw new NotFoundException("KPI not found");
    return prisma.controlTowerKpi.update({
      where: { id },
      data: { currentValue: new Prisma.Decimal(currentValue) },
    });
  }

  async getAlertConfigs(tenantId: string) {
    return prisma.controlTowerAlertConfig.findMany({
  ...
  async createAlertConfig(tenantId: string, dto: any) {
    return prisma.controlTowerAlertConfig.create({
      data: {
        tenantId,
        alertName: dto.alertName,
        description: dto.description || null,
        eventType: dto.eventType || null,
        kpiCode: dto.kpiCode || null,
        condition: dto.condition || Prisma.JsonNull,
        severity: dto.severity || "MEDIUM",
        notificationChannels: dto.notificationChannels || Prisma.JsonNull,
        recipients: dto.recipients || Prisma.JsonNull,
  ...
  async getControlTowerDashboard(tenantId: string) {
    const [openEvents, criticalEvents, kpis, alertConfigs] = await Promise.all([
      prisma.controlTowerEvent.count({
        where: {
          tenantId,
      ...
      prisma.controlTowerKpi.findMany({
        where: { tenantId, isActive: true },
        orderBy: { category: "asc" },
      }),
      prisma.controlTowerAlertConfig.count({
        where: { tenantId, isActive: true },
      }),
    ]);
    const eventsByType = await prisma.controlTowerEvent.groupBy({
      by: ["eventType"],
      where: {
        tenantId,
    ...
    return {
      ...
      activeAlertConfigs: alertConfigs,
