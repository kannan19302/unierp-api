...
export class ProjectsEnterpriseController {
  ...
  @ApiOperation({
    summary:
      "Portfolio health dashboard with schedule, budget, resource, risk scores",
  })
  ...
  @ApiOperation({
    summary:
      "Resource capacity planning with supply/demand and overallocation alerts",
  })
  ...
  async getResourceCapacityPlanning(
    ...
    return this.service.getResourceCapacityPlanning(
      req.user.tenantId,
      periodStart,
      periodEnd,
    );
  ...
  async getEarnedValueAnalysis(
    ...
    return this.service.getEarnedValueAnalysis(
      req.user.tenantId,
      projectId,
      asOf,
    );
  ...
  @ApiOperation({
    summary: "Schedule risk assessment with Monte Carlo simulation",
  })
  @Get("schedule-risk/:projectId")
  @Permissions("projects.project.read")
  async getScheduleRiskAssessment(
    @Req() req: AuthenticatedRequest,
    @Param("projectId") projectId: string,
  ) {
  ...
  @ApiOperation({
    summary: "Resource utilization rates by resource/project/department",
  })
  ...
  @ApiOperation({
    summary: "Portfolio optimization under resource or budget constraints",
  })
  ...
  async getProjectPortfolioOptimization(
    ...
    return this.service.getProjectPortfolioOptimization(
      req.user.tenantId,
      constraint || "resource",
    );
  }
  ...
  async getTimesheetCompliance(
    ...
    return this.service.getTimesheetCompliance(
      req.user.tenantId,
      projectId,
      period || "MONTHLY",
    );
  ...
  async getProjectFinancialForecast(
    ...
    return this.service.getProjectFinancialForecast(
      req.user.tenantId,
      projectId,
    );
  }
  ...
  @ApiOperation({
    summary: "Task dependency network and critical chain analysis",
  })
  @Get("dependency-network/:projectId")
  @Permissions("projects.project.read")
  async getDependencyNetwork(
    @Req() req: AuthenticatedRequest,
    @Param("projectId") projectId: string,
  ) {
