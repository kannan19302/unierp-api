...
export class SupplyChainAnalyticsService {
  async getDashboard(tenantId: string) {
    const [shipments, inbound, outbound, carriers, exceptions, returns] =
      await Promise.all([
    ...
    const totalWeight = [...shipments, ...inbound, ...outbound].reduce(
      (sum, s) =>
        sum + Number((s as any).weight || (s as any).totalWeight || 0),
      0,
    );
    const openExceptions = exceptions.filter((e) => e.status === "OPEN").length;
    const pendingReturns = returns.filter(
      (r) => r.status === "PENDING" || r.status === "PACKED",
    ).length;
  ...
  async getCarrierPerformance(tenantId: string) {
    ...
    const perfMap = new Map<
      string,
      { total: number; delivered: number; onTime: number }
    >();
    for (const s of allShipments) {
      if (!(s as any).carrierId) continue;
      const key = (s as any).carrierId;
      if (!perfMap.has(key))
        perfMap.set(key, { total: 0, delivered: 0, onTime: 0 });
  ...
  async getOnTimeDelivery(tenantId: string) {
    const outbound = await prisma.outboundShipment.findMany({
      where: { tenantId },
    });
  ...
  async getCostAnalysis(tenantId: string) {
    const shipments = await prisma.shipment.findMany({
      where: { tenantId, shippingCost: { not: null } },
    });
    const totalShippingCost = shipments.reduce(
      (sum, s) => sum + Number(s.shippingCost),
      0,
    );
  ...
  async getLeadTime(tenantId: string) {
    ...
    const avgLeadTime =
      leadTimes.length > 0
        ? Math.round((totalLeadDays / leadTimes.length) * 10) / 10
        : 0;
