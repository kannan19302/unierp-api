...
import { hashPassword, comparePassword, signToken } from "@kannan19302/auth";
...
export type InviteCustomerPortalUserInput = z.infer<
  typeof inviteCustomerPortalUserSchema
>;
...
export type PortalQuotationDecisionInput = z.infer<
  typeof portalQuotationDecisionSchema
>;

export const portalInitiatePaymentSchema = z.object({
  amount: z.number().positive(),
});
export type PortalInitiatePaymentInput = z.infer<
  typeof portalInitiatePaymentSchema
>;

export const portalConfirmPaymentSchema = z.object({
  simulateDecline: z.boolean().optional().default(false),
});
export type PortalConfirmPaymentInput = z.infer<
  typeof portalConfirmPaymentSchema
>;
...
export class CustomerPortalService {
  constructor(
    private readonly paymentGateway: CrmPortalPaymentGatewayService,
  ) {}
  ...
  async inviteUser(
    tenantId: string,
    customerId: string,
    dto: InviteCustomerPortalUserInput,
  ) {
    ...
    if (dto.contactId) {
      const contact = await prisma.contact.findFirst({
        where: { id: dto.contactId, tenantId, customerId },
      });
      if (!contact)
        throw new BadRequestException(
  ...
  async disableUser(tenantId: string, userId: string) {
    ...
    return prisma.customerPortalUser.update({
      where: { id: userId },
  ...
  async getDashboardSummary(tenantId: string, customerId: string) {
    const [openCases, pendingQuotes, unpaidInvoices, recentOrders, customer] =
      await Promise.all([
        prisma.case.count({
          where: {
            tenantId,
        ...
        prisma.quotation.count({
          where: { tenantId, customerId, status: "SENT", deletedAt: null },
        }),
  ...
  async acceptQuotation(tenantId: string, customerId: string, id: string) {
    ...
    if (q.status !== "SENT")
      throw new BadRequestException(
        `Cannot accept a quotation in status ${q.status}`,
      );
  ...
  async rejectQuotation(
    ...
    customerId: string,
    id: string,
    dto: PortalQuotationDecisionInput,
  ) {
    const q = await prisma.quotation.findFirst({
      where: { id, tenantId, customerId, deletedAt: null },
    });
    ...
    if (q.status !== "SENT")
      throw new BadRequestException(
        `Cannot reject a quotation in status ${q.status}`,
      );
    const notes = dto.reason
      ? `${q.notes ?? ""}\n[Rejected by customer] ${dto.reason}`.trim()
      : q.notes;
    return prisma.quotation.update({
      where: { id },
  ...
  async addCaseComment(
    ...
    portalUserId: string,
    caseId: string,
    dto: PortalCaseCommentInput,
  ) {
    const c = await prisma.case.findFirst({
      where: { id: caseId, tenantId, customerId },
    });
    if (!c) throw new NotFoundException("Case not found");
  ...
  async confirmInvoicePayment(
    ...
    customerId: string,
    intentId: string,
    dto: PortalConfirmPaymentInput,
  ) {
    const record = await prisma.portalPaymentIntent.findFirst({
      where: { id: intentId, tenantId, customerId },
    });
    if (!record) throw new NotFoundException("Payment intent not found");
    if (record.status !== "REQUIRES_CONFIRMATION") {
      throw new BadRequestException(
        `Payment intent is already ${record.status}`,
      );
    }

    const result = await this.paymentGateway.confirmIntent(
      record.gatewayIntentId,
      dto.simulateDecline,
    );

    if (result.status === "failed") {
      await prisma.portalPaymentIntent.update({
        where: { id: intentId },
        data: { status: "FAILED" },
      });
      throw new BadRequestException(
        "Payment was declined by the gateway (mock)",
      );
    }

    const invoice = await prisma.invoice.findFirst({
      where: { id: record.invoiceId, tenantId },
    });
    if (!invoice) throw new NotFoundException("Invoice not found");
