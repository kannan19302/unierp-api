...
export class DocumentsDeepController {
  ...
  async renderTemplate(
    @Req() req: AuthReq,
    @Body() body: z.infer<typeof renderTemplateSchema>,
  ) {
  ...
  async createCategory(
    @Req() req: AuthReq,
    @Body() body: z.infer<typeof createCategorySchema>,
  ) {
  ...
  async updateCategory(
    @Req() req: AuthReq,
    @Param("id") id: string,
    @Body() body: z.infer<typeof updateCategorySchema>,
  ) {
  ...
  async bulkUpload(
    @Req() req: AuthReq,
    @Body() body: z.infer<typeof bulkUploadSchema>,
  ) {
    const parsed = bulkUploadSchema.parse(body);
    return this.svc.bulkUpload(
      req.user.tenantId,
      req.user.orgId || "",
      parsed.files,
      req.user.userId,
  ...
  async submitForApproval(
    ...
    @Body() body: z.infer<typeof submitApprovalSchema>,
  ) {
    const parsed = submitApprovalSchema.parse(body);
    return this.svc.submitForApproval(
      req.user.tenantId,
      documentId,
      parsed.approverId,
    );
  ...
  async approveOrReject(
    ...
    @Body() body: z.infer<typeof reviewApprovalSchema>,
  ) {
    const parsed = reviewApprovalSchema.parse(body);
    return this.svc.approveOrReject(
      req.user.tenantId,
      approvalId,
      parsed.status,
      parsed.comment,
  ...
  async getVersionDiff(
    ...
    return this.svc.getVersionDiff(
      req.user.tenantId,
      documentId,
      parseInt(v1, 10),
      parseInt(v2, 10),
