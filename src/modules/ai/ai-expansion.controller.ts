...
export class AiExpansionController {
  ...
  async getModels(@Req() req: AuthReq, @Query() query: Record<string, string>) {
    ...
    return this.svc.getModels(req.user.tenantId, {
      page,
      limit,
      search: query.search,
    });
  ...
  async createModel(
    @Req() req: AuthReq,
    @ZodBody(createAiModelSchema) body: CreateAiModelInput,
  ) {
  ...
  async updateModel(
    @Req() req: AuthReq,
    @Param("id") id: string,
    @ZodBody(updateAiModelSchema) body: UpdateAiModelInput,
  ) {
  ...
  async createDeployment(
    ...
    @ZodBody(createAiDeploymentSchema) body: CreateAiDeploymentInput,
  ) {
  ...
  async getPrompts(
    ...
    const page = parseInt(query.page || "1", 10);
    const limit = parseInt(query.limit || "50", 10);
    return this.svc.getPrompts(req.user.tenantId, {
      page,
      limit,
      search: query.search,
  ...
  async createConversation(
    @Req() req: AuthReq,
    @ZodBody(createAiConversationSchema) body: CreateAiConversationInput,
  ) {
  ...
  async sendMessage(
    @Req() req: AuthReq,
    @Param("id") id: string,
    @ZodBody(sendAiMessageSchema) body: SendAiMessageInput,
  ) {
  ...
  async createDocument(
    @Req() req: AuthReq,
    @ZodBody(createAiDocumentSchema) body: CreateAiDocumentInput,
  ) {
  ...
  async getAgents(@Req() req: AuthReq, @Query() query: Record<string, string>) {
    const page = parseInt(query.page || "1", 10);
    const limit = parseInt(query.limit || "50", 10);
    return this.svc.getAgents(req.user.tenantId, {
      page,
      limit,
      search: query.search,
    });
  ...
  async createAgent(
    @Req() req: AuthReq,
    @ZodBody(createAiAgentSchema) body: CreateAiAgentInput,
  ) {
  ...
  async updateAgent(
    @Req() req: AuthReq,
    @Param("id") id: string,
    @ZodBody(updateAiAgentSchema) body: UpdateAiAgentInput,
  ) {
  ...
  async createTrainingJob(
    @Req() req: AuthReq,
    @ZodBody(createAiTrainingJobSchema) body: CreateAiTrainingJobInput,
  ) {
