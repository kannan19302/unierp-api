...
describe("WarehouseOpsService", () => {
  ...
  describe("assignTask", () => {
    it("assigns a QUEUED task to a worker", async () => {
      ...
      vi.mocked(prisma.warehouseTask.update).mockResolvedValue({
        id: "t1",
        status: "ASSIGNED",
      } as any);
  ...
  describe("startTask", () => {
    ...
    it("throws if task is not ASSIGNED", async () => {
      ...
      await expect(svc.startTask(TENANT, "t1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("approveBinTransfer", () => {
    it("approves a PENDING transfer", async () => {
      ...
      vi.mocked(prisma.binTransferRequest.update).mockResolvedValue({
        id: "bt1",
        status: "APPROVED",
      } as any);
    ...
    it("throws if transfer is not PENDING", async () => {
      ...
      await expect(svc.approveBinTransfer(TENANT, "bt1", USER)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("createGrn", () => {
    it("creates a GRN with auto-generated number and line items", async () => {
      ...
      const result = await svc.createGrn(TENANT, {
        ...
        lineItems: [
          { productId: "p1", orderedQty: 100, receivedQty: 95, uom: "EA" },
        ],
      });
  ...
  describe("verifyGrn", () => {
    ...
    it("throws if GRN is not in DRAFT status", async () => {
      ...
      await expect(
        svc.verifyGrn(TENANT, "grn1", { lineItems: [] }),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("addCarton", () => {
    ...
    it("throws if session is not OPEN", async () => {
      ...
      await expect(svc.addCarton(TENANT, "ps1", {})).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("completePackingSession", () => {
    it("completes a packing session and sums weight", async () => {
      ...
      vi.mocked(prisma.packingSession.findFirst).mockResolvedValue({
        id: "ps1",
        status: "OPEN",
        cartons: [{ weight: 5.5 }, { weight: 7.0 }],
      } as any);
      vi.mocked(prisma.packingSession.update).mockResolvedValue({
        id: "ps1",
        status: "COMPLETE",
        totalWeight: 12.5,
      } as any);
  ...
  describe("sealCarton", () => {
    it("seals an unsealed carton", async () => {
      ...
      vi.mocked(prisma.packingCarton.findFirst).mockResolvedValue({
        id: "c1",
        sealedAt: null,
      } as any);
      vi.mocked(prisma.packingCarton.update).mockResolvedValue({
        id: "c1",
        sealedAt: new Date(),
      } as any);
