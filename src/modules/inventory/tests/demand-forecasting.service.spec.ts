...
describe("DemandForecastingService", () => {
  ...
  describe("updateActual", () => {
    it("updates actual qty and calculates MAPE", async () => {
      ...
      vi.mocked(prisma.demandForecast.update).mockResolvedValue({
        id: "f1",
        actualQty: "90",
      } as never);
      const result = await service.updateActual(TENANT, "f1", {
        actualQty: 90,
      });
      expect(prisma.demandForecast.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ actualQty: expect.anything() }),
        }),
      );
      expect(result).toHaveProperty("id", "f1");
    });
  ...
  describe("deactivateReorderPoint", () => {
    ...
    it("sets isActive to false", async () => {
      ...
      vi.mocked(prisma.reorderPoint.update).mockResolvedValue({
        id: "rp1",
        isActive: false,
      } as never);
  ...
  describe("approveReplenishmentOrder", () => {
    it("throws BadRequestException when not PENDING", async () => {
      ...
      await expect(
        service.approveReplenishmentOrder(TENANT, "ro1", USER, {
          approvedQty: 100,
        }),
    ...
    it("approves a pending order", async () => {
      ...
      vi.mocked(prisma.replenishmentOrder.update).mockResolvedValue({
        id: "ro1",
        status: "APPROVED",
      } as never);
      const result = await service.approveReplenishmentOrder(
        TENANT,
        "ro1",
        USER,
        { approvedQty: 80 },
  ...
  describe("updateReplenishmentStatus", () => {
    it("throws BadRequestException for invalid transition", async () => {
      ...
      await expect(
        service.updateReplenishmentStatus(TENANT, "ro1", "RECEIVED"),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("acknowledgeStockoutPrediction", () => {
    ...
    it("sets acknowledged true", async () => {
      ...
      vi.mocked(prisma.stockoutPrediction.update).mockResolvedValue({
        id: "sp1",
        acknowledged: true,
      } as never);
      const result = await service.acknowledgeStockoutPrediction(
        TENANT,
        "sp1",
        { acknowledgedBy: USER },
      );
  ...
  describe("getDashboard", () => {
    it("returns all metric keys", async () => {
      ...
      vi.mocked(prisma.demandForecast.aggregate).mockResolvedValue({
        _avg: { mape: null },
      } as never);
