...
describe("CrmQuoteSignatureService", () => {
  ...
  it("requests a signature with a unique token and expiry", async () => {
    ...
    (
      prisma.quotationSignature.create as ReturnType<typeof vi.fn>
    ).mockImplementation(({ data }) =>
      Promise.resolve({ id: "sig1", ...data }),
    );
  ...
  it("throws when requesting a signature for a missing quotation", async () => {
    ...
    await expect(
      service.requestSignature(TENANT, {
        quotationId: "missing",
        signerName: "Jane",
        signerEmail: "jane@example.com",
        expiresInDays: 14,
  ...
  it("signs a quotation, issues a certificate with a deterministic hash, and returns it", async () => {
    ...
    (
      prisma.quotationSignature.findUnique as ReturnType<typeof vi.fn>
    ).mockResolvedValue({
      id: "sig1",
      tenantId: TENANT,
      quotationId: "q1",
    ...
    (
      prisma.quotationSignatureCertificate.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({
      id: "cert1",
      tenantId: TENANT,
      signatureId: "sig1",
    ...
    const result = await service.signQuotation({
      token: "tok1",
  ...
  it("rejects signing an already-signed request", async () => {
    ...
    await expect(
      service.signQuotation({ token: "tok1", signatureData: "x" }),
  ...
  it("rejects signing an expired request", async () => {
    ...
    (
      prisma.quotationSignature.update as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
    await expect(
      service.signQuotation({ token: "tok1", signatureData: "x" }),
