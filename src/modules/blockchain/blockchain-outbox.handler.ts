...
export class BlockchainOutboxHandler {
  ...
  constructor(private readonly anchorService: BlockchainAnchorService) {}

  async handle(event: OutboxEventPayload): Promise<void> {
    this.logger.debug(
      `Handling blockchain-anchor event: ${event.eventName} (${event.aggregateType}:${event.aggregateId})`,
    );
    ...
    this.logger.log(
      `Anchored ${event.eventName} for ${event.aggregateType}:${event.aggregateId}`,
    );
  }
}
