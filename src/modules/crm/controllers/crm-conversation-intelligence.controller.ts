...
import {
  ...
  logCallSchema,
  LogCallInput,
} from "../services/crm-conversation-intelligence.service";
...
export class CrmConversationIntelligenceController {
  ...
  @ApiOperation({
    summary:
      "Log a call and auto-generate its AI summary/sentiment/action items",
  })
  @Post("calls")
  ...
  async logCall(
    ...
    @ZodBody(logCallSchema) dto: LogCallInput,
  ) {
  ...
  @ApiOperation({
    summary:
      "Re-run AI analysis on a logged call (e.g. after a transcript correction)",
  })
  ...
  @ApiOperation({
    summary: "Get one logged call with its full transcript + AI analysis",
  })
  @Get("calls/:id")
  ...
  @ApiOperation({
    summary:
      "List logged calls, optionally filtered by deal/lead/customer/sentiment",
  })
  @Get("calls")
  @Permissions("crm.activity.read")
  async listCalls(
    ...
    return this.svc.listCalls(req.user.tenantId, {
      opportunityId,
      leadId,
      customerId,
      sentiment,
  ...
  @ApiOperation({
    summary:
      "Tenant-wide conversation intelligence rollup (sentiment mix, avg engagement score)",
  })
