...
describe("CrmLeadsService", () => {
  ...
  describe("bulkUpdateLeadStatus", () => {
    it("rejects invalid status values", async () => {
      await expect(
        service.bulkUpdateLeadStatus(TENANT, ["lead-1"], "INVALID"),
    ...
    it("updates multiple leads with a valid status", async () => {
      ...
      const call = (prisma.lead.updateMany as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
  ...
  describe("exportLeads", () => {
    ...
    it("filters by status when provided", async () => {
      ...
      const call = (prisma.lead.findMany as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
  ...
  describe("updateLeadStatus (transition guard)", () => {
    it("throws if the lead does not exist", async () => {
      ...
      await expect(
        service.updateLeadStatus(TENANT, "missing", "QUALIFIED"),
    ...
    it("rejects DISQUALIFIED -> CONVERTED (terminal status, no transitions allowed)", async () => {
      ...
      await expect(
        service.updateLeadStatus(TENANT, "lead-1", "CONVERTED"),
      ).rejects.toThrow(BadRequestException);
    ...
    it("rejects CONVERTED -> anything (terminal status)", async () => {
      ...
      await expect(
        service.updateLeadStatus(TENANT, "lead-1", "NEW"),
      ).rejects.toThrow(BadRequestException);
      expect(prisma.lead.update).not.toHaveBeenCalled();
    });
  ...
  describe("getLeadSummary", () => {
    it("throws if the lead does not exist", async () => {
      ...
      await expect(service.getLeadSummary(TENANT, "missing")).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe("reactivateLead", () => {
    it("throws if the lead does not exist", async () => {
      ...
      await expect(service.reactivateLead(TENANT, "missing")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("rejects reactivating a lead that is not DISQUALIFIED", async () => {
      ...
      await expect(service.reactivateLead(TENANT, "lead-1")).rejects.toThrow(
        BadRequestException,
      );
      expect(prisma.lead.update).not.toHaveBeenCalled();
    });
    ...
    it("reactivates a DISQUALIFIED lead back to NEW and appends the reason to notes", async () => {
      ...
      const result = await service.reactivateLead(
        TENANT,
        "lead-1",
        "Budget freed up next quarter",
      );

      expect(result.status).toBe("NEW");
      const call = (prisma.lead.update as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
