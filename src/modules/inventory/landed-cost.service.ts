...
export class LandedCostService {
  ...
  async submitVoucher(tenantId: string, id: string) {
    const v = await this.getVoucher(tenantId, id);
    if (v.status !== "DRAFT")
      throw new BadRequestException("Voucher must be in DRAFT status");
    if (v.chargeLines.length === 0)
      throw new BadRequestException(
        "Voucher must have at least one charge line",
      );
    return prisma.landedCostVoucher.update({
      where: { id },
  ...
  async cancelVoucher(tenantId: string, id: string) {
    ...
    if (v.status === "ALLOCATED")
      throw new BadRequestException("Allocated vouchers cannot be cancelled");
    return prisma.landedCostVoucher.update({
      where: { id },
  ...
  async listChargeLines(tenantId: string, voucherId: string) {
    ...
    return prisma.landedCostChargeLine.findMany({
  ...
  async addChargeLine(
    ...
    if (v.status !== "DRAFT")
      throw new BadRequestException(
        "Can only add charge lines to DRAFT vouchers",
      );
  ...
  async removeChargeLine(tenantId: string, voucherId: string, lineId: string) {
    ...
    if (v.status !== "DRAFT")
      throw new BadRequestException(
        "Can only remove charge lines from DRAFT vouchers",
      );
  ...
  async listReceiptLinks(tenantId: string, voucherId: string) {
    await this.getVoucher(tenantId, voucherId);
    return prisma.landedCostReceiptLink.findMany({
      where: { tenantId, voucherId },
    });
  }
  ...
  async linkReceipt(
    ...
    const v = await this.getVoucher(tenantId, voucherId);
    if (v.status !== "DRAFT")
      throw new BadRequestException("Can only link receipts to DRAFT vouchers");
    return prisma.landedCostReceiptLink.create({
      data: {
        ...
        totalWeight:
          dto.totalWeight != null ? new Prisma.Decimal(dto.totalWeight) : null,
        totalVolume:
          dto.totalVolume != null ? new Prisma.Decimal(dto.totalVolume) : null,
      },
    });
  }
  ...
  async unlinkReceipt(
    ...
    if (v.status !== "DRAFT")
      throw new BadRequestException(
        "Can only unlink receipts from DRAFT vouchers",
      );
    await prisma.landedCostReceiptLink.deleteMany({
      where: { tenantId, voucherId, stockEntryId },
    });
    return { deleted: true };
  }
  ...
  async allocate(tenantId: string, id: string) {
    ...
    if (v.status !== "SUBMITTED")
      throw new BadRequestException(
        "Voucher must be SUBMITTED before allocation",
      );
    if (v.receiptLinks.length === 0)
      throw new BadRequestException("No receipt links found");

    const receiptLinks = await prisma.landedCostReceiptLink.findMany({
      where: { tenantId, voucherId: id },
    });
    const chargeLines = await prisma.landedCostChargeLine.findMany({
      where: { tenantId, voucherId: id },
    });
    ...
    const basis = receiptLinks.map((r) => {
      switch (method) {
        case "QTY":
          return Number(r.totalQty);
        case "VALUE":
          return Number(r.totalValue);
        case "WEIGHT":
          return Number(r.totalWeight ?? 0);
        case "VOLUME":
          return Number(r.totalVolume ?? 0);
        default:
          return 1; // EQUAL
      }
    });
  ...
  async listAllocations(tenantId: string, voucherId: string) {
    await this.getVoucher(tenantId, voucherId);
    return prisma.landedCostAllocation.findMany({
      where: { tenantId, voucherId },
    });
  }
  ...
  async getAllocationReport(tenantId: string, voucherId?: string) {
    ...
    for (const a of allocs) {
      byType[a.chargeType] =
        (byType[a.chargeType] ?? 0) + Number(a.allocatedAmount);
    }
