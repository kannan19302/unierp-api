...
describe("BudgetDeepService", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new BudgetDeepService();
  });

  it("getBudgetTemplates returns templates", async () => {
    vi.mocked(prisma.budgetTemplate.findMany).mockResolvedValue([
      { id: "bt-1" },
    ] as any);
  ...
  it("getBudgetTemplateById throws on not found", async () => {
    ...
    await expect(service.getBudgetTemplateById("t1", "bad")).rejects.toThrow(
      NotFoundException,
    );
  });

  it("createBudgetTemplate creates record", async () => {
    vi.mocked(prisma.budgetTemplate.create).mockResolvedValue({
      id: "bt-1",
    } as any);
    const result = await service.createBudgetTemplate("t1", "o1", {
      name: "FY2027",
      fiscalYear: "2027",
    });
  ...
  it("generateBudgetsFromTemplate throws if template not active", async () => {
    vi.mocked(prisma.budgetTemplate.findFirst).mockResolvedValue({
      id: "bt-1",
      status: "DRAFT",
      lines: [],
    } as any);
    await expect(
      service.generateBudgetsFromTemplate("t1", "o1", "bt-1"),
    ).rejects.toThrow(BadRequestException);
  });

  it("getBudgetCommitments returns commitments", async () => {
    vi.mocked(prisma.budgetCommitment.findMany).mockResolvedValue([
      { id: "bc-1" },
    ] as any);
  ...
  it("createBudgetCommitment creates record", async () => {
    vi.mocked(prisma.budgetCommitment.create).mockResolvedValue({
      id: "bc-1",
    } as any);
    const result = await service.createBudgetCommitment("t1", "o1", {
      budgetId: "b-1",
      commitmentRef: "PO-001",
  ...
  it("liquidateBudgetCommitment updates amounts", async () => {
    vi.mocked(prisma.budgetCommitment.findFirst).mockResolvedValue({
      id: "bc-1",
      amount: 1000,
      liquidatedAmount: 0,
    } as any);
    vi.mocked(prisma.budgetCommitment.update).mockResolvedValue({
      id: "bc-1",
      liquidatedAmount: 500,
  ...
  it("liquidateBudgetCommitment throws if over-commitment", async () => {
    ...
    await expect(
      service.liquidateBudgetCommitment("t1", "bc-1", 200),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("computeCarryForward throws if no rules", async () => {
    vi.mocked(prisma.budgetCarryForwardRule.findMany).mockResolvedValue([]);
    await expect(
      service.computeCarryForward("t1", "o1", "2026", "2027"),
    ).rejects.toThrow(BadRequestException);
  });

  it("getBudgetRevisions returns revisions", async () => {
    vi.mocked(prisma.budgetRevision.findMany).mockResolvedValue([
      { id: "br-1" },
    ] as any);
  ...
  it("createBudgetRevision computes change amount", async () => {
    ...
    vi.mocked(prisma.budgetRevision.create).mockResolvedValue({
      id: "br-1",
    } as any);
    const result = await service.createBudgetRevision("t1", "o1", {
      budgetId: "b-1",
  ...
  it("approveBudgetRevision updates budget amount", async () => {
    vi.mocked(prisma.budgetRevision.findFirst).mockResolvedValue({
      id: "br-1",
      status: "PENDING",
      budgetId: "b-1",
      revisedAmount: 1200,
  ...
  it("rejectBudgetRevision throws if not pending", async () => {
    ...
    await expect(
      service.rejectBudgetRevision("t1", "br-1", "user1"),
    ).rejects.toThrow(BadRequestException);
  });
