...
export class StockTakeService {
  ...
  async updateStockTake(
    tenantId: string,
    id: string,
    dto: { notes?: string; countDate?: string },
  ) {
    const st = await this.getStockTake(tenantId, id);
    if (!["DRAFT"].includes(st.status))
      throw new BadRequestException("Only DRAFT stock takes can be updated");
  ...
  async startStockTake(tenantId: string, id: string) {
    const st = await this.getStockTake(tenantId, id);
    if (st.status !== "DRAFT")
      throw new BadRequestException("Only DRAFT stock takes can be started");
    if (st.sheets.length === 0)
      throw new BadRequestException(
        "Add at least one count sheet before starting",
      );
    return prisma.stockTake.update({
      where: { id },
  ...
  async cancelStockTake(tenantId: string, id: string) {
    const st = await this.getStockTake(tenantId, id);
    if (["POSTED", "CANCELLED"].includes(st.status))
      throw new BadRequestException(
        "Cannot cancel a posted or cancelled stock take",
      );
    return prisma.stockTake.update({
      where: { id },
  ...
  async createCountSheet(
    ...
    if (!["DRAFT", "IN_PROGRESS"].includes(st.status)) {
      throw new BadRequestException(
        "Sheets can only be added to DRAFT or IN_PROGRESS stock takes",
      );
    }
    const seq = await prisma.countSheet.count({
      where: { tenantId, stockTakeId },
    });
  ...
  async addItemsToSheet(
    ...
    if (!["DRAFT", "IN_PROGRESS"].includes(st.status)) {
      throw new BadRequestException(
        "Cannot add items to a non-active stock take",
      );
    }
    const sheet = await prisma.countSheet.findFirst({
      where: { tenantId, id: sheetId, stockTakeId },
    });
      ...
      data: {
        items: {
          create: items.map((i) => ({
            ...
            unitCost:
              i.unitCost != null ? new Prisma.Decimal(i.unitCost) : undefined,
          })),
        },
      },
      include: { items: true },
  ...
  async recordCount(
    ...
    if (!["IN_PROGRESS", "COUNTING"].includes(st.status)) {
      throw new BadRequestException(
        "Stock take must be IN_PROGRESS or COUNTING to record counts",
      );
    }
    ...
    for (const c of counts) {
      ...
      if (!item)
        throw new BadRequestException(`Item ${c.itemId} not found in sheet`);
      ...
      const varianceValue = item.unitCost
        ? varianceQty.times(item.unitCost)
        : undefined;
    ...
    return prisma.countSheet.findFirst({
      where: { id: sheetId },
      include: { items: true },
    });
  }
  ...
  async generateVariances(tenantId: string, stockTakeId: string) {
    ...
    if (!["COUNTING", "IN_PROGRESS"].includes(st.status)) {
      throw new BadRequestException(
        "Stock take must be COUNTING or IN_PROGRESS to generate variances",
      );
    }

    // Collect all items with a non-zero variance across all sheets
    const allItems = st.sheets.flatMap((s) => s.items);
    const withVariance = allItems.filter(
      (i) => i.countedQty !== null && !i.varianceQty?.equals(0),
    );

    // Upsert a variance record per product+bin
    await prisma.stockTakeVariance.deleteMany({
      where: { tenantId, stockTakeId },
    });
  ...
  async approveVariance(
    ...
    const v = await prisma.stockTakeVariance.findFirst({
      where: { tenantId, id: varianceId },
    });
    if (!v) throw new NotFoundException("Variance not found");
    if (v.status !== "PENDING")
      throw new BadRequestException("Variance must be PENDING to approve");
  ...
  async rejectVariance(
    ...
    if (!v) throw new NotFoundException("Variance not found");
    if (v.status !== "PENDING")
      throw new BadRequestException("Variance must be PENDING to reject");
    return prisma.stockTakeVariance.update({
      where: { id: varianceId },
  ...
  async approveStockTake(tenantId: string, id: string, approvedBy: string) {
    const st = await this.getStockTake(tenantId, id);
    if (st.status !== "VARIANCE_REVIEW")
      throw new BadRequestException(
        "Stock take must be in VARIANCE_REVIEW to approve",
      );
    ...
    if (pendingVariances > 0)
      throw new BadRequestException(
        `${pendingVariances} variances still pending review`,
      );
  ...
  async getDashboard(tenantId: string) {
    const [
      ...
      prisma.stockTake.count({
        where: { tenantId, status: "VARIANCE_REVIEW" },
      }),
    ...
    const approvedVariances = await prisma.stockTakeVariance.count({
  ...
  async getVarianceReport(tenantId: string, stockTakeId: string) {
    ...
    const totalVarianceValue = variances.reduce(
      (s, v) => s + (v.varianceValue ? Number(v.varianceValue) : 0),
      0,
    );
    const positiveVariances = variances.filter(
      (v) => Number(v.varianceQty) > 0,
    );
    const negativeVariances = variances.filter(
      (v) => Number(v.varianceQty) < 0,
    );
    ...
    return {
      ...
      materialVariances: variances.filter(
        (v) => Math.abs(Number(v.variancePct)) >= materialThreshold,
      ),
      variances,
    };
  }
  ...
  async getAccuracyReport(tenantId: string, warehouseId?: string) {
    ...
    return posted.map((st) => {
      ...
      const nonZero = st.variances.filter(
        (v) => !v.varianceQty.equals(0),
      ).length;
