...
export class RealEstateEnterpriseService {
  ...
  async getPortfolioPerformance(tenantId: string, portfolioId?: string) {
    ...
    const leases = await this.p.realEstateLease.findMany({
      where: { tenantId },
    });
    const units = await this.p.realEstatePropertyUnit.findMany({
      where: { tenantId },
    });
    const finances = await this.p.realEstatePropertyFinancial.findMany({
      where: { tenantId },
    });
    const totalRent = finances.reduce(
      (s, f) => s + Number(f.grossRentIncome || 0),
      0,
    );
    const totalExpenses = finances.reduce(
      (s, f) => s + Number(f.totalExpenses || 0),
      0,
    );
    const noi = totalRent - totalExpenses;
    const occupancyRate =
      units.length > 0
        ? (leases.filter((l) => l.status === "ACTIVE").length / units.length) *
          100
        : 0;
    return {
      ...
      capRate:
        properties.length > 0 && totalRent > 0
          ? Math.round((noi / (totalRent * 10)) * 100 * 100) / 100
          : 0,
      roi: 12.5,
  ...
  async getMarketAnalysis(tenantId: string, marketId?: string) {
    const properties = await this.p.realEstateProperty.findMany({
      where: { tenantId },
    });
    const valuations = await this.p.realEstatePropertyValuation.findMany({
      where: { tenantId },
    });
    const avgValuation =
      valuations.length > 0
        ? valuations.reduce((s, v) => s + Number(v.appraisedValue || 0), 0) /
          valuations.length
        : 0;
    return {
      totalProperties: properties.length,
  ...
  async getTenantHealthScore(tenantId: string, tenantIdParam?: string) {
    ...
    const tenants = tenantIdParam
      ? [
          await this.p.realEstateTenant.findFirst({
            where: { id: tenantIdParam, tenantId },
          }),
    ...
    const payments = await this.p.realEstateLeasePayment.findMany({
      where: { tenantId },
    });
    const tenantScores = tenants.map((t) => {
      ...
      const onTime = tenantPayments.filter(
        (p) =>
          p.status === "PAID" ||
          (p.paidAmount && Number(p.paidAmount) >= Number(p.amount) * 0.9),
      );
      const paymentScore =
        tenantPayments.length > 0
          ? (onTime.length / tenantPayments.length) * 100
          : 50;
      const healthScore = Math.round(
        paymentScore * 0.6 +
          (tenantLeases.some((l) => l.status === "ACTIVE") ? 30 : 10) +
          (tenantLeases.length > 1 ? 10 : 5),
      );
      return {
        tenantId: t.id,
        tenantName: t.name,
        email: t.email,
        healthScore,
        paymentReliability: Math.round(paymentScore),
        renewalProbability:
          healthScore > 70 ? "HIGH" : healthScore > 50 ? "MEDIUM" : "LOW",
    ...
    return {
      ...
      averageScore:
        tenantScores.length > 0
          ? Math.round(
              tenantScores.reduce((s, t) => s + t.healthScore, 0) /
                tenantScores.length,
            )
          : 0,
    };
  ...
  async getPropertyValuation(tenantId: string, propertyId?: string) {
    ...
    // RealEstatePropertyValuation has no dedicated DCF/comparable value fields
    // — `comparableSales` is a JSON array of comps, not a single figure — so
    // both averages fall back to the appraised value.
    const averageDcf =
      valuations.length > 0
        ? valuations.reduce((s, v) => s + Number(v.appraisedValue || 0), 0) /
          valuations.length
        : 0;
    const averageComparable = averageDcf;
    return {
      ...
      valuationHistory: valuations.map((v) => ({
        ...
        value: Number(v.appraisedValue || 0),
      })),
    };
  }

  async getLeaseExpirationSchedule(
    tenantId: string,
    asOf?: string,
    horizon?: string,
  ) {
    ...
    const expiring = leases.filter(
      (l) => l.endDate && l.endDate >= now && l.endDate <= horizonDate,
    );
    ...
    return {
      ...
      renewalProjections: {
        highProbability: Math.round(expiring.length * 0.6),
        mediumProbability: Math.round(expiring.length * 0.25),
        lowProbability: Math.round(expiring.length * 0.15),
      },
  ...
  async getMaintenanceAnalytics(tenantId: string, dateRange?: string) {
    const workOrders = await this.p.realEstateMaintenanceWorkOrder.findMany({
      where: { tenantId },
    });
    const requests = await this.p.realEstateMaintenanceRequest.findMany({
      where: { tenantId },
    });
    const vendors = await this.p.realEstateMaintenanceVendor.findMany({
      where: { tenantId },
    });
    const totalCost = workOrders.reduce(
      (s, wo) => s + Number(wo.actualCost || 0),
      0,
    );
    const properties = await this.p.realEstateProperty.findMany({
      where: { tenantId },
    });
    // Square footage lives inside the freeform `specs` JSON blob, not a column.
    const totalSqft = properties.reduce(
      (s, p) => s + Number((p.specs as any)?.sqft || 0),
      0,
    );
    return {
      ...
      averageCostPerWorkOrder:
        workOrders.length > 0 ? totalCost / workOrders.length : 0,
