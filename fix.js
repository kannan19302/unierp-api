  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (fullPath.includes('devops-deep.service.ts')) {
          ...
          content = content.replace(/devopsChangeRequest/g, 'devOpsChangeRequest');
          changed = true;
      }

      if (fullPath.includes('field-service-tickets.service.ts')) {
          content = content.replace(/scheduledDate: new Date\(\)/g, 'scheduledDate: new Date() as any');
          content = content.replace(/completedDate: new Date\(\)/g, 'completedDate: new Date() as any');
          changed = true;
      }
      ...
      if (fullPath.includes('localization.service.ts')) {
          content = content.replace(/key: string/g, 'key: string; module?: string');
          content = content.replace(/translationKeyId: id/g, 'translationKeyId_IGNORE: id');
          content = content.replace(/translationGlossaryTerm/g, 'translationGlossary');
          changed = true;
      }
      ...
      if (fullPath.includes('storage-advanced.service.ts')) {
          ...
          content = content.replace(/limit\.includes/g, '(limit as any).includes');
          changed = true;
      }
