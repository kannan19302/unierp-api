...
export class GlAccountingService {
  ...
  async createJournal(
    ...
    const addDec = (s: any, val: any) => {
      const d = val instanceof Prisma.Decimal ? val : new Prisma.Decimal(val || 0);
      if (typeof s?.plus === "function") return s.plus(d);
      if (typeof s?.add === "function") return s.add(d);
      return new Prisma.Decimal(Number(s || 0) + Number(d));
    };
    const debits = dto.entries.reduce(
      (sum, e) => addDec(sum, e.debit),
      new Prisma.Decimal(0),
    );
    const credits = dto.entries.reduce(
      (sum, e) => addDec(sum, e.credit),
      new Prisma.Decimal(0),
    );
