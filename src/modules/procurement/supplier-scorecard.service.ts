...
export class SupplierScorecardService {
  ...
  async computeAndSave(
    ...
    const receipts = await prisma.purchaseReceipt.findMany({
      ...
      include: {
        lineItems: true,
        purchaseOrder: { select: { orderDate: true, poNumber: true } },
      },
    });
      ...
      if (receipt.purchaseOrder?.orderDate) {
        const deliveryDiff =
          (receipt.receivedDate.getTime() -
            receipt.purchaseOrder.orderDate.getTime()) /
          (1000 * 60 * 60 * 24);
    ...
    const qualityScore =
      totalUnitsReceived > 0
        ? Math.round(
            ((totalUnitsReceived - defectiveUnits) / totalUnitsReceived) *
              100 *
              100,
          ) / 100
        : 100;
    const deliveryScore =
      totalReceipts > 0
        ? Math.round((onTimeCount / totalReceipts) * 100 * 100) / 100
        : 100;
    ...
    const fillRateScore =
      orderedQty > 0
        ? Math.round((receivedQty / orderedQty) * 100 * 100) / 100
        : 100;

    const overallScore =
      Math.round(
        (qualityScore * 0.4 + deliveryScore * 0.35 + fillRateScore * 0.25) *
          100,
      ) / 100;
  ...
  async getVendorTrend(tenantId: string, vendorId: string, lastN: number = 6) {
    ...
    return {
      ...
      trend: {
        ...
        labels: scorecards
          .map((s) => `${s.periodStart.toISOString().substring(0, 7)}`)
          .reverse(),
      },
    };
  }
  ...
  async getStats(tenantId: string) {
    ...
    const avgOverall =
      latestScores.length > 0
        ? Math.round(
            (latestScores.reduce(
              (s, sc) => s + Number(sc.overallScore || 0),
              0,
            ) /
              latestScores.length) *
              100,
    ...
    return {
      ...
      qualityAvg:
        Math.round(
          (latestScores.reduce((s, sc) => s + Number(sc.qualityScore || 0), 0) /
      ...
      deliveryAvg:
        Math.round(
          (latestScores.reduce(
            (s, sc) => s + Number(sc.deliveryScore || 0),
            0,
          ) /
            (latestScores.length || 1)) *
            100,
      ...
      fillRateAvg:
        Math.round(
          (latestScores.reduce(
            (s, sc) => s + Number(sc.fillRateScore || 0),
            0,
          ) /
            (latestScores.length || 1)) *
            100,
  ...
  async getScorecardsByDateRange(
    tenantId: string,
    startDate: string,
    endDate: string,
    params: PaginationParams = {},
