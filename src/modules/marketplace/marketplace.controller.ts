...
export class MarketplaceController {
  ...
  @ApiOperation({
    summary:
      "Get installed app bundle manifest (Flutter mobile/desktop generic renderer, .ai/MULTI_CLIENT_MASTER_PLAN.md § 6)",
  })
  @Get("installed/:slug/manifest")
  @Permissions("admin.platform.read")
  async getInstalledAppManifest(
    @Req() req: AuthenticatedRequest,
    @Param("slug") slug: string,
  ) {
    return this.marketplaceService.getInstalledAppManifest(
      req.user.tenantId,
      slug,
    );
  }
  ...
  async updateAppConfig(
    @Req() req: AuthenticatedRequest,
    @Param("slug") slug: string,
    @ZodBody(z.any()) body: { config: Record<string, any> },
  ) {
    return this.marketplaceService.updateAppConfig(
      req.user.tenantId,
      slug,
      body.config,
    );
  ...
  @Permissions("admin.platform.read")
  async getInstalledAppModules(
    @Req() req: AuthenticatedRequest,
    @Param("slug") slug: string,
  ) {
    return this.marketplaceService.getInstalledAppModules(
      req.user.tenantId,
      slug,
    );
  }
  ...
  @Permissions("admin.platform.read")
  async getInstalledAppMetrics(
    @Req() req: AuthenticatedRequest,
    @Param("slug") slug: string,
  ) {
    return this.marketplaceService.getInstalledAppMetrics(
      req.user.tenantId,
      slug,
    );
  }
  ...
  async setModuleEnabled(
    ...
    @ZodBody(z.any()) body: { enabled: boolean },
  ) {
    return this.marketplaceService.setModuleEnabled(
      req.user.tenantId,
      slug,
      moduleSlug,
      body.enabled,
  ...
  async createReview(
    @Req() req: AuthenticatedRequest,
    @Param("slug") slug: string,
    @ZodBody(z.any()) body: { rating: number; title?: string; body?: string },
  ) {
  ...
  async addAppToCollection(
    @Param("id") id: string,
    @ZodBody(z.any()) body: { appId: string; sortOrder?: number },
  ) {
    return this.marketplaceService.addAppToCollection(
      id,
      body.appId,
      body.sortOrder,
    );
  ...
  async createSubmission(
    ...
    return this.marketplaceService.createSubmission(
      req.user.tenantId,
      req.user.userId,
      req.user.name || req.user.email,
      body,
  ...
  async rejectSubmission(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(z.any()) body: { reviewNotes: string },
  ) {
    return this.marketplaceService.rejectSubmission(
      id,
      req.user.userId,
      body.reviewNotes,
    );
