...
export class ContractsService {
  ...
  async getContractCompliance(tenantId: string, contractId: string) {
    ...
    const utilizationPct =
      totalContractValue > 0 ? (releasedAmount / totalContractValue) * 100 : 0;
    ...
    if (daysRemaining > 0 && daysRemaining < 30)
      alerts.push("Contract expiring within 30 days");
    if (utilizationPct > 90) alerts.push("Contract utilization above 90%");
    if (utilizationPct < 30 && daysRemaining < 90)
      alerts.push("Low utilization with less than 90 days remaining");
    ...
    return {
      ...
      status: isActive
        ? "ACTIVE"
        : now < new Date(agreement.startDate)
          ? "FUTURE"
          : "EXPIRED",
      ...
      releasedAmount,
      remainingValue:
        Math.round((totalContractValue - releasedAmount) * 100) / 100,
