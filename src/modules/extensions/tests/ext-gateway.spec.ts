...
import { validateManifest } from "../../marketplace/services/manifest";
...
describe("tenant-context token", () => {
  it("signs and verifies a round trip", () => {
    ...
    const claims = verifyTenantToken(token, SECRET, {
      expectedAppSlug: "field-service",
    });
  ...
  it("rejects a tampered token", () => {
    const token = signTenantToken(
      { tenantId: "t1", userId: "u1", roles: [], appSlug: "a", scopes: [] },
      SECRET,
    );
  ...
  it("rejects a token for a different app", () => {
    ...
    expect(() =>
      verifyTenantToken(token, SECRET, { expectedAppSlug: "healthcare" }),
    ).toThrow(/issued for app/);
  });
});
...
describe("manifest service section", () => {
  ...
  it("accepts declarative+service with a valid service section", () => {
    const m = validateManifest({
      ...
      service: {
        ...
        defaultBaseUrl: "http://localhost:4103",
      },
    });
  ...
  it("allows zero pages for service-backed bundles only", () => {
    expect(() => validateManifest({ ...base, pages: [] })).toThrow(
      /at least one page/,
    );
    const m = validateManifest({
      ...base,
  ...
  it("rejects a service section without declarative+service runtime", () => {
    expect(() =>
      validateManifest({ ...base, service: { healthcheck: "/svc/health" } }),
    ).toThrow(/runtime/);
  });
  ...
  it("rejects missing/relative healthcheck and future apiVersion", () => {
    ...
    expect(() => validateManifest({ ...base, apiVersion: 99 })).toThrow(
      /apiVersion/,
    );
    expect(() => validateManifest({ ...base, apiVersion: 0 })).toThrow(
      /apiVersion/,
    );
    expect(() => validateManifest({ ...base, apiVersion: 1.5 })).toThrow(
      /apiVersion/,
    );
  });
...
describe("ServiceRegistryService gating", () => {
  ...
  it("404s when the app is not installed", async () => {
    ...
    await expect(registry.resolve("t1", "field-service")).rejects.toThrow(
      /not installed/,
    );
  });
...
describe("ExtProxyService", () => {
  it("maps connection failure to a 503 envelope", async () => {
    ...
    const req: any = {
      method: "GET",
      path: "/api/v1/ext/field-service/tickets",
      originalUrl: "/api/v1/ext/field-service/tickets",
      headers: {},
    };
