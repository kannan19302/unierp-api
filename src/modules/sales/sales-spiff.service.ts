...
export class SalesSpiffService {
  ...
  async getSpiffCampaignById(tenantId: string, id: string) {
    const spiff = await prisma.commissionSpiff.findFirst({
      where: { id, tenantId },
    });
  ...
  async updateSpiffCampaign(tenantId: string, id: string, dto: any) {
    ...
    if (dto.criteriaValue !== undefined)
      data.criteriaValue = dto.criteriaValue as Prisma.InputJsonValue;
    if (dto.bonusType !== undefined) data.bonusType = dto.bonusType;
    if (dto.bonusAmount !== undefined)
      data.bonusAmount = new Prisma.Decimal(dto.bonusAmount);
  ...
  async getTeamSplits(tenantId: string) {
    return prisma.teamSplit.findMany({
      where: { tenantId },
  ...
  async createTeamSplit(tenantId: string, orgId: string, dto: any) {
    ...
    return prisma.teamSplit.create({
      data: {
        ...
        members: {
          create: dto.members.map((m: any) => ({
            tenantId,
            userId: m.userId,
            share: new Prisma.Decimal(m.share),
            role: m.role || "MEMBER",
          })),
        },
      },
  ...
  async updateTeamSplit(tenantId: string, id: string, dto: any) {
    const existing = await prisma.teamSplit.findFirst({
      where: { id, tenantId },
    });
    ...
    return prisma.teamSplit.update({
      where: { id },
      data,
      include: { members: true },
    });
  ...
  async processClawback(tenantId: string, dto: any) {
    const payout = await prisma.commissionPayout.findFirst({
      where: { id: dto.payoutId, tenantId },
    });
    if (!payout) throw new NotFoundException("Commission payout not found");
    if (payout.status === "PAID")
      throw new BadRequestException("Cannot clawback an already paid payout");
    return prisma.commissionPayout.update({
      where: { id: dto.payoutId },
      data: {
        totalPayout: new Prisma.Decimal(
          Math.max(0, Number(payout.totalPayout) - dto.amount),
        ),
        status: "DRAFT",
      },
    });
  ...
  async getSpiffDashboard(tenantId: string) {
    const [activeCampaigns, totalPayouts, pendingPayouts] = await Promise.all([
      ...
      prisma.commissionPayout.aggregate({
        where: { tenantId },
        _sum: { spiffBonus: true },
      }),
      prisma.commissionPayout.count({ where: { tenantId, status: "DRAFT" } }),
    ]);
    return {
      activeCampaigns,
      totalSpiffPayouts: Number(totalPayouts._sum.spiffBonus || 0),
      pendingApprovals: pendingPayouts,
    };
  }
  ...
  async calculateSpiffEligibility(tenantId: string, salesOrderId: string) {
    ...
    const activeSpiffs = await prisma.commissionSpiff.findMany({
    ...
    for (const spiff of activeSpiffs) {
      if (spiff.criteriaType === "DEAL_SIZE_ABOVE") {
        ...
        if (
          spiff.criteriaType === "DEAL_SIZE_ABOVE" &&
          orderAmount >= threshold
        ) {
          matched.push({
            spiffId: spiff.id,
            name: spiff.name,
            bonusType: spiff.bonusType,
            bonusAmount: Number(spiff.bonusAmount),
            matchedCriteria: `Order $${orderAmount} >= threshold $${threshold}`,
          });
        }
      }
      if (spiff.criteriaType === "NEW_LOGO" && order.salesChannel === "B2C") {
        matched.push({
          ...
          bonusType: spiff.bonusType,
          bonusAmount: Number(spiff.bonusAmount),
          matchedCriteria: "New logo (B2C first order)",
        });
      }
    }
    return {
      salesOrderId,
      orderAmount,
      matchedSpiffs: matched,
      totalBonus: matched.reduce(
        (s, m) =>
          s +
          (m.bonusType === "FLAT"
            ? m.bonusAmount
            : (orderAmount * m.bonusAmount) / 100),
        0,
      ),
    };
  }
