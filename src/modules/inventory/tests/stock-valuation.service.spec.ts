...
describe("StockValuationService", () => {
  ...
  describe("getPolicy", () => {
    ...
    it("throws NotFoundException when not found", async () => {
      ...
      await expect(svc.getPolicy(TENANT, "missing")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("upsertPolicy", () => {
    it("creates or updates policy", async () => {
      ...
      const result = await svc.upsertPolicy(TENANT, USER, {
        method: "WEIGHTED_AVG",
        productId: "prod1",
      });
  ...
  describe("deactivatePolicy", () => {
    it("deactivates active policy", async () => {
      ...
      vi.mocked(prisma.stockValuationPolicy.update).mockResolvedValue(
        mockPolicy({ isActive: false }) as never,
      );
  ...
  describe("postLedgerEntry", () => {
    it("posts entry with running totals", async () => {
      ...
      vi.mocked(prisma.stockValuationLedger.create).mockResolvedValue({
        id: "le1",
      } as never);
  ...
  describe("computeIssueCost", () => {
    it("uses weighted average for WEIGHTED_AVG method", async () => {
      ...
      const result = await svc.computeIssueCost(
        TENANT,
        "p1",
        undefined,
        100,
  ...
  describe("approveAdjustment", () => {
    it("approves PENDING adjustment", async () => {
      ...
      vi.mocked(prisma.costAdjustment.update).mockResolvedValue(
    ...
    it("throws if not PENDING", async () => {
      ...
      await expect(svc.approveAdjustment(TENANT, "a1", USER)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("postAdjustment", () => {
    it("posts APPROVED adjustment and creates ledger entry", async () => {
      ...
      vi.mocked(prisma.costAdjustment.update).mockResolvedValue(
        mockAdj({ status: "POSTED" }) as never,
      );
    ...
    it("throws if not APPROVED", async () => {
      ...
      await expect(svc.postAdjustment(TENANT, "a1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("rejectAdjustment", () => {
    it("rejects PENDING adjustment", async () => {
      ...
      vi.mocked(prisma.costAdjustment.update).mockResolvedValue(
        mockAdj({ status: "REJECTED" }) as never,
      );
  ...
  describe("postRevaluation", () => {
    it("posts DRAFT revaluation", async () => {
      ...
      vi.mocked(prisma.stockRevaluation.update).mockResolvedValue(
        mockReval({ status: "POSTED" }) as never,
      );
    ...
    it("throws if not DRAFT", async () => {
      ...
      await expect(svc.postRevaluation(TENANT, "r1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("getDashboard", () => {
    it("returns all dashboard stats", async () => {
      ...
      vi.mocked(prisma.stockRevaluation.aggregate).mockResolvedValue({
        _sum: { totalImpact: "5000" },
      } as never);
      vi.mocked(prisma.costAdjustment.aggregate).mockResolvedValue({
        _sum: { impactAmount: "1000" },
      } as never);
