...
describe("InventoryLaborService", () => {
  ...
  it("createLaborStandard: throws on duplicate taskType", async () => {
    ...
    await expect(
      svc.createLaborStandard(TENANT, { taskType: "PICK", standardMins: 5 }),
  ...
  it("createLaborStandard: creates when no duplicate", async () => {
    ...
    const res = await svc.createLaborStandard(TENANT, {
      taskType: "PICK",
      standardMins: 5,
    });
  ...
  it("deleteLaborStandard: soft-deletes when found", async () => {
    ...
    vi.mocked(prisma.laborStandard.update).mockResolvedValue({
      id: "s1",
      isActive: false,
    } as never);
  ...
  it("logTask: creates task log record", async () => {
    ...
    vi.mocked(prisma.workerTaskLog.create).mockResolvedValue({
      id: "log1",
    } as never);
...
describe("SupplierQualityService", () => {
  ...
  it("closeNcr: throws when already CLOSED", async () => {
    ...
    await expect(svc.closeNcr(TENANT, "n1", "notes")).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  ...
  it("closeNcr: marks CLOSED when open", async () => {
    ...
    vi.mocked(prisma.supplierNcr.update).mockResolvedValue({
      id: "n1",
      status: "CLOSED",
    } as never);
  ...
  it("createCar: uses $transaction and returns car", async () => {
    ...
    vi.mocked(prisma.supplierCarRequest.create).mockResolvedValue({
      id: "car1",
    } as never);
...
describe("InventoryAutomationService", () => {
  ...
  it("createReplenishmentRule: throws on duplicate", async () => {
    ...
    await expect(
      svc.createReplenishmentRule(TENANT, {
        warehouseId: "wh1",
        productId: "p1",
        activeBinCode: "A1",
        reserveBinCode: "R1",
  ...
  it("deleteReplenishmentRule: soft-deletes", async () => {
    ...
    vi.mocked(prisma.binReplenishmentRule.update).mockResolvedValue({
      id: "r1",
      isActive: false,
    } as never);
    ...
    vi.mocked(prisma.binReplenishmentRule.findMany).mockResolvedValue([
      {
        ...
        productId: "p1",
        activeBinCode: "A1",
        triggerQty: new Prisma.Decimal(10),
      },
    ] as never);
    vi.mocked(prisma.inventoryItemBin.findFirst).mockResolvedValue({
  ...
  it("releaseHold: throws when not ACTIVE", async () => {
    ...
    await expect(
      svc.releaseHold(TENANT, "h1", { releasedBy: "user1" }),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it("releaseHold: updates to RELEASED", async () => {
    ...
    vi.mocked(prisma.inventoryHold.update).mockResolvedValue({
      id: "h1",
      status: "RELEASED",
    } as never);
  ...
  it("getSlowMovingInventory: identifies products with no outbound movement", async () => {
    ...
    vi.mocked(prisma.inventoryItemBin.groupBy).mockResolvedValue([
      ...
      },
      {
        productId: "p2",
        warehouseId: "wh1",
        _sum: { quantity: new Prisma.Decimal(20) },
      },
    ] as never);
    ...
    vi.mocked(prisma.inventoryItemBin.groupBy).mockResolvedValue([
      {
        warehouseId: "wh1",
        _count: { _all: 10 },
        _sum: { quantity: new Prisma.Decimal(500) },
      },
    ] as never);
    vi.mocked(prisma.warehouse.findMany).mockResolvedValue([
      { id: "wh1", name: "Main", code: "MN" },
    ] as never);
