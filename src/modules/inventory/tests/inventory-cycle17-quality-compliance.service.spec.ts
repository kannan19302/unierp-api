...
describe("QualityComplianceService", () => {
  ...
  it("createCapa — generates CAPA number and creates record", async () => {
    ...
    const dto = {
      ...
      description: "Root cause TBD",
    };
    vi.mocked(prisma.capaRecord.create).mockResolvedValue({
      id: "cap1",
      capaNumber: "CAPA-001",
      ...dto,
    } as any);
  ...
  it("transitionCapaStatus — allows OPEN → IN_PROGRESS", async () => {
    ...
    vi.mocked(prisma.capaRecord.update).mockResolvedValue({
      id: "cap1",
      status: "IN_PROGRESS",
    } as any);
    const result = await svc.transitionCapaStatus(
      TENANT,
      "cap1",
      "IN_PROGRESS",
    );
  ...
  it("transitionCapaStatus — rejects invalid transition from CLOSED", async () => {
    ...
    await expect(
      svc.transitionCapaStatus(TENANT, "cap1", "IN_PROGRESS"),
  ...
  it("addCapaAction — creates action linked to CAPA", async () => {
    ...
    vi.mocked(prisma.capaAction.create).mockResolvedValue({
      id: "act1",
      capaId: "cap1",
    } as any);
    const dto = {
      actionType: "CONTAINMENT" as const,
      description: "Quarantine affected batches",
    };
  ...
  it("completeCapaAction — marks action complete with timestamp", async () => {
    ...
    vi.mocked(prisma.capaAction.update).mockResolvedValue({
      id: "act1",
      status: "COMPLETE",
    } as any);
  ...
  it("completeCapaAction — rejects already completed action", async () => {
    ...
    await expect(svc.completeCapaAction(TENANT, "act1")).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  ...
  it("scheduleCalibration — creates calibration with DUE status", async () => {
    ...
    vi.mocked(prisma.calibrationRecord.create).mockResolvedValue({
      id: "cal1",
      status: "DUE",
    } as any);
    ...
    expect(vi.mocked(prisma.calibrationRecord.create)).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: "DUE", intervalDays: 90 }),
      }),
    );
  });
  ...
  it("recordCalibrationResult — marks PASSED and sets nextDueDate when interval set", async () => {
    ...
    vi.mocked(prisma.calibrationRecord.update).mockResolvedValue({
      id: "cal1",
      status: "PASSED",
    } as any);
    await svc.recordCalibrationResult(TENANT, "cal1", {
      result: "PASS",
      performedBy: "John",
    });
  ...
  it("recordCalibrationResult — rejects duplicate result recording", async () => {
    ...
    await expect(
      svc.recordCalibrationResult(TENANT, "cal1", { result: "PASS" }),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it("createDeviation — generates deviation number", async () => {
    ...
    vi.mocked(prisma.deviationRecord.create).mockResolvedValue({
      id: "dev1",
  ...
  it("reviewDeviation — transitions OPEN to UNDER_REVIEW", async () => {
    ...
    vi.mocked(prisma.deviationRecord.update).mockResolvedValue({
      id: "dev1",
      status: "UNDER_REVIEW",
    } as any);
    const result = await svc.reviewDeviation(
      TENANT,
      "dev1",
      "Impact: 5 affected pallets",
    );
  ...
  it("reviewDeviation — rejects non-OPEN deviation", async () => {
    ...
    await expect(
      svc.reviewDeviation(TENANT, "dev1", "notes"),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it("closeDeviation — links optional CAPA id", async () => {
    ...
    expect(vi.mocked(prisma.deviationRecord.update)).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: "CLOSED", capaId: "cap1" }),
      }),
    );
  });
  ...
  it("submitSopForReview — transitions DRAFT to UNDER_REVIEW", async () => {
    ...
    vi.mocked(prisma.sopDocument.update).mockResolvedValue({
      id: "sop1",
      status: "UNDER_REVIEW",
    } as any);
  ...
  it("submitSopForReview — rejects non-DRAFT document", async () => {
    ...
    await expect(svc.submitSopForReview(TENANT, "sop1")).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  ...
  it("reviseSOp — bumps version and logs revision", async () => {
    ...
    vi.mocked(prisma.sopRevision.create).mockResolvedValue({
      id: "rev1",
    } as any);
    await svc.reviseSOp(
      TENANT,
