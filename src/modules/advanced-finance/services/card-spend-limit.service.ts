...
export class CardSpendLimitService {
  ...
  async getSpendLimits(tenantId: string, cardId: string) {
    await this.getCard(tenantId, cardId);
    return prisma.cardSpendLimit.findMany({
  ...
  async updateSpendLimit(
    ...
    const existing = await prisma.cardSpendLimit.findFirst({
      where: { id: limitId, tenantId },
    });
  ...
  async getCategoryLimits(tenantId: string, cardId: string) {
    await this.getCard(tenantId, cardId);
    return prisma.cardCategoryLimit.findMany({
      where: { tenantId, cardId },
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async updateCategoryLimit(
    ...
    const existing = await prisma.cardCategoryLimit.findFirst({
      where: { id: limitId, tenantId },
    });
  ...
  async checkAuthorization(
    ...
    const employee = await prisma.employee.findFirst({
      where: { id: card.employeeId, tenantId },
    });
    ...
    const scopeFilter = {
      OR: [
        ...
        ...(employee?.departmentId
          ? [{ scopeType: "DEPARTMENT", scopeId: employee.departmentId }]
          : []),
      ],
    };
    ...
    for (const limit of spendLimits) {
      const reserved = await this.tryReserveSpend(
        "cardSpendLimit",
        limit.id,
        amount,
      );
      if (!reserved) {
        ...
        return {
          allowed: false,
          reason: `Exceeds ${limit.period} spend limit of ${limit.amountCap} for scope ${limit.scopeType}`,
        };
      }
      await this.alertIfOverThreshold(
        tenantId,
        "Card Spend Limit",
        Number(limit.currentSpend) + amount,
        Number(limit.amountCap),
      );
    }

    let categoryLimits: Awaited<
      ReturnType<typeof prisma.cardCategoryLimit.findMany>
    > = [];
    if (mccCategory) {
      ...
      for (const limit of categoryLimits) {
        ...
        if (!reserved) {
          ...
          return {
            allowed: false,
            reason: `Exceeds ${limit.period} category limit of ${limit.amountCap} for ${mccCategory}`,
          };
        }
        await this.alertIfOverThreshold(
          tenantId,
          `Category Limit (${mccCategory})`,
          Number(limit.currentSpend) + amount,
          Number(limit.amountCap),
        );
      }
    ...
    const rows =
      model === "cardSpendLimit"
        ? await prisma.$queryRaw<Array<{ id: string }>>`
          UPDATE "card_spend_limits" SET current_spend = current_spend + ${amount}, updated_at = now()
          WHERE id = ${limitId} AND (current_spend + ${amount}) <= amount_cap AND (current_spend + ${amount}) >= 0
          RETURNING id`
        : await prisma.$queryRaw<Array<{ id: string }>>`
          UPDATE "card_category_limits" SET current_spend = current_spend + ${amount}, updated_at = now()
          WHERE id = ${limitId} AND (current_spend + ${amount}) <= amount_cap AND (current_spend + ${amount}) >= 0
          RETURNING id`;
    return rows.length > 0;
  }

  private async alertIfOverThreshold(
    tenantId: string,
    label: string,
    newSpend: number,
    cap: number,
    ...
    if (pct >= 100) {
      await this.raiseThresholdAlert(
        tenantId,
        label,
        newSpend,
        cap,
        "CRITICAL",
        "reached/exceeded 100%",
      );
  ...
  private async recordBreach(
    ...
    const model =
      limitType === "SPEND" ? prisma.cardSpendLimit : prisma.cardCategoryLimit;
    const updated = await (
      model as { update: (args: unknown) => Promise<{ breachCount: number }> }
    ).update({
      where: { id: limitId },
    ...
    if (updated.breachCount >= BREACH_FREEZE_THRESHOLD) {
      ...
      await this.raiseThresholdAlert(
        ...
        "Corporate Card",
        updated.breachCount,
        BREACH_FREEZE_THRESHOLD,
        "CRITICAL",
        "exceeded repeated breach threshold — card auto-frozen",
      );
    }
  }
  ...
  async getUtilization(tenantId: string, cardId: string) {
    ...
    const [spendLimits, categoryLimits] = await Promise.all([
      prisma.cardSpendLimit.findMany({
        where: { tenantId, cardId, isActive: true },
      }),
      prisma.cardCategoryLimit.findMany({
        where: { tenantId, cardId, isActive: true },
      }),
    ]);
    ...
    const toUtil = (cap: Prisma.Decimal, spend: Prisma.Decimal) => {
      ...
      return {
        ...
        cap: capNum,
        utilizationPct:
          capNum > 0 ? Number(((spendNum / capNum) * 100).toFixed(2)) : 0,
      };
    };
  ...
  async approveLimitIncrease(
    ...
    const request = await prisma.cardLimitIncreaseRequest.findFirst({
    ...
    const limit = await prisma.cardSpendLimit.findFirst({
      where: { id: request.limitId, tenantId },
    });
    if (!limit) throw new NotFoundException("Spend limit not found");
  ...
  async resetExpiredPeriods(): Promise<{
    spendLimitsReset: number;
    categoryLimitsReset: number;
  }> {
    const now = new Date();
    const expiredSpendLimits = await prisma.cardSpendLimit.findMany({
      where: { isActive: true, periodEnd: { lt: now } },
    });
    ...
    const expiredCategoryLimits = await prisma.cardCategoryLimit.findMany({
      where: { isActive: true, periodEnd: { lt: now } },
    });
    ...
    return {
      spendLimitsReset: expiredSpendLimits.length,
      categoryLimitsReset: expiredCategoryLimits.length,
    };
  }
}
