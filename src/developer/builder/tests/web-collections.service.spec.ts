...
describe("WebCollectionsService", () => {
  ...
  beforeEach(async () => {
    const moduleRef: TestingModule = await Test.createTestingModule({
      providers: [WebCollectionsService],
    }).compile();
  ...
  describe("getCollections", () => {
    it("annotates each collection with item count", async () => {
      (prisma.webCollection.findMany as any).mockResolvedValue([
        { id: "c1" },
        { id: "c2" },
      ]);
      (prisma.webCollectionItem.groupBy as any).mockResolvedValue([
        { collectionId: "c1", _count: { _all: 5 } },
      ]);
  ...
  describe("createCollection", () => {
    it("rejects duplicate slug", async () => {
      (prisma.webCollection.findUnique as any).mockResolvedValue({
        id: "exists",
      });
      await expect(
        service.createCollection("t1", {
          name: "Products",
          slug: "products",
        } as any),
    ...
    it("creates with defaults", async () => {
      ...
      (prisma.webCollection.create as any).mockImplementation(({ data }: any) =>
        Promise.resolve({ id: "c1", ...data }),
      );
      const res = await service.createCollection(
        "t1",
        { name: "Products", slug: "products" } as any,
        "u1",
      );
  ...
  describe("seedCollection", () => {
    it("creates a preset collection plus sample items", async () => {
      (prisma.webCollection.findUnique as any).mockResolvedValue(null);
      (prisma.webCollection.create as any).mockResolvedValue({
        id: "c1",
        name: "Products",
        slug: "products",
      });
      (prisma.webCollectionItem.createMany as any).mockResolvedValue({
        count: 3,
      });
    ...
    it("throws on unknown preset", async () => {
      await expect(service.seedCollection("t1", "nope")).rejects.toThrow(
        /Unknown preset/,
      );
    });
  });
  ...
  describe("createItem", () => {
    it("auto-derives slug from the title field and sets publishedAt when published", async () => {
      (prisma.webCollection.findFirst as any).mockResolvedValue({
        id: "c1",
        settings: { titleField: "name" },
      });
      (prisma.webCollectionItem.findFirst as any).mockResolvedValue(null); // slug free
      (prisma.webCollectionItem.create as any).mockImplementation(
        ({ data }: any) => Promise.resolve({ id: "i1", ...data }),
      );
      const res = await service.createItem(
        "t1",
        "c1",
        { data: { name: "Aurora Headphones" }, status: "PUBLISHED" } as any,
        "u1",
      );
  ...
  describe("getPublicItems", () => {
    it("returns only published items for a collection slug", async () => {
      ...
      (prisma.webCollectionItem.findMany as any).mockResolvedValue([
        { id: "i1", status: "PUBLISHED" },
      ]);
      const res = await service.getPublicItems("t1", "products");
      expect(res.items).toHaveLength(1);
      expect(prisma.webCollectionItem.findMany).toHaveBeenCalledWith(
  ...
  describe("createSubmission", () => {
    it("captures a public form submission", async () => {
      ...
      const res = await service.createSubmission("t1", {
        formName: "contact",
        data: { email: "a@b.com" },
      } as any);
  ...
  describe("checkout", () => {
    it("computes totals server-side and creates an order", async () => {
      (prisma.webOrder.create as any).mockImplementation(({ data }: any) =>
