...
describe("FixedAssetsController", () => {
  ...
  describe("getAssetById", () => {
    it("should fetch single asset by ID", async () => {
      ...
      expect(mockService.getAssetById).toHaveBeenCalledWith(
        "tenant-123",
        "asset-1",
      );
    });
  ...
  describe("transferAsset", () => {
    it("should invoke transferAsset service method", async () => {
      ...
      vi.mocked(mockService.transferAsset).mockResolvedValue({
        id: "log-1",
      } as any);

      const result = await controller.transferAsset(
        mockUserReq,
        "asset-1",
        payload,
      );
      expect(result).toEqual({ id: "log-1" });
      expect(mockService.transferAsset).toHaveBeenCalledWith(
        "tenant-123",
        "asset-1",
        "user-456",
        payload,
  ...
  describe("logMaintenance", () => {
    it("should log maintenance schedule and state", async () => {
      ...
      vi.mocked(mockService.logMaintenance).mockResolvedValue({
        id: "m-log-1",
      } as any);

      const result = await controller.logMaintenance(
        mockUserReq,
        "asset-1",
        payload,
      );
      expect(result).toEqual({ id: "m-log-1" });
      expect(mockService.logMaintenance).toHaveBeenCalledWith(
        "tenant-123",
        "asset-1",
        "user-456",
        payload,
  ...
  describe("postDepreciation", () => {
    it("should post periodic depreciation run", async () => {
      ...
      vi.mocked(mockService.postDepreciation).mockResolvedValue({
        id: "dep-log-1",
      } as any);

      const result = await controller.postDepreciation(
        mockUserReq,
        "asset-1",
        payload,
      );
      expect(result).toEqual({ id: "dep-log-1" });
      expect(mockService.postDepreciation).toHaveBeenCalledWith(
        "tenant-123",
        "org-789",
        "user-456",
        "asset-1",
