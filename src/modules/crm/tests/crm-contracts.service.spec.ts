...
describe("CrmContractsService", () => {
          ...
          const tx = {
            ...
            contractBillingMilestone: {
              update: vi
                .fn()
                .mockImplementation((args) =>
                  Promise.resolve({ id: "ms-1", ...args.data }),
                ),
            },
          };
          return callback(tx);
  ...
  describe("createContract", () => {
    it("creates a contract linked to a customer", async () => {
      ...
      (prisma.contract.create as ReturnType<typeof vi.fn>).mockImplementation(
        ({ data }) => Promise.resolve({ id: "con-1", ...data }),
      );
  ...
  describe("updateStatus", () => {
    it("allows DRAFT -> ACTIVE", async () => {
      (prisma.contract.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(
        { id: "con-1", tenantId: TENANT, status: "DRAFT" },
      );
      ...
      const result = await service.updateStatus(TENANT, "con-1", {
        status: "ACTIVE",
      });
    ...
    it("rejects illegal transition DRAFT -> EXPIRED", async () => {
      ...
      await expect(
        service.updateStatus(TENANT, "con-1", { status: "EXPIRED" }),
      ).rejects.toThrow(BadRequestException);
    });

    it("rejects setting RENEWED directly (must go through renew action)", async () => {
      (prisma.contract.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(
        { id: "con-1", tenantId: TENANT, status: "ACTIVE" },
      );
      await expect(
        service.updateStatus(TENANT, "con-1", { status: "RENEWED" }),
      ).rejects.toThrow(/use POST \/crm\/contracts\/:id\/renew/);
    });

    it("rejects any transition out of terminal TERMINATED", async () => {
      (prisma.contract.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(
        { id: "con-1", tenantId: TENANT, status: "TERMINATED" },
      );
      await expect(
        service.updateStatus(TENANT, "con-1", { status: "ACTIVE" }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("throws NotFoundException for missing contract", async () => {
      ...
      await expect(
        service.updateStatus(TENANT, "missing", { status: "ACTIVE" }),
  ...
  describe("renewContract", () => {
    it("creates a follow-on contract by default, marks original RENEWED", async () => {
      ...
      (prisma.$transaction as ReturnType<typeof vi.fn>).mockImplementation(
        async (ops: any[]) => {
          return [
            { id: "con-1", status: "RENEWED" },
            {
              id: "con-2",
              contractNumber: "CON-00002",
              status: "ACTIVE",
              renewedFromId: "con-1",
            },
          ];
      ...
      const result = await service.renewContract(TENANT, ORG, "con-1", {
        extendInPlace: false,
      } as any);
    ...
    it("extends in place when extendInPlace=true", async () => {
      ...
      (prisma.contract.update as ReturnType<typeof vi.fn>).mockImplementation(
        ({ data }) => Promise.resolve({ id: "con-1", ...data }),
      );
    ...
    it("rejects renewing a TERMINATED contract", async () => {
      ...
      await expect(
        service.renewContract(TENANT, ORG, "con-1", {} as any),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe("tenant isolation", () => {
    it("getContractById scopes by tenantId and returns not found for another tenant", async () => {
      (
        prisma.contract.findFirst as ReturnType<typeof vi.fn>
      ...
      await expect(
        service.getContractById("other-tenant", "con-1"),
      ).rejects.toThrow(NotFoundException);
  ...
  describe("approval flows", () => {
    ...
    it("approves a pending contract", async () => {
      (prisma.contract.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(
        { id: "con-1", tenantId: TENANT, approvalStatus: "PENDING_APPROVAL" },
      );
      (prisma.contract.update as ReturnType<typeof vi.fn>).mockImplementation(
        ({ data }) => Promise.resolve({ id: "con-1", ...data }),
      );
  ...
  describe("signature flows", () => {
    it("invites a signer to sign", async () => {
      ...
      const result = await service.inviteToSign(
        TENANT,
        "con-1",
        "Alice",
        "alice@company.com",
  ...
  describe("convertToSalesOrder", () => {
    it("converts an approved contract into a draft SalesOrder", async () => {
      ...
      (
        prisma.salesOrder.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue(null);
      (prisma.salesOrder.count as ReturnType<typeof vi.fn>).mockResolvedValue(
        0,
      );

      const result = await service.convertToSalesOrder(
        TENANT,
        "org-1",
        "con-1",
        "user-1",
    ...
    it("throws BadRequestException if contract is already converted to a sales order", async () => {
      ...
      (
        prisma.salesOrder.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue({ id: "so-existing" });
  ...
  describe("billingMilestones", () => {
    ...
    it("adds a billing milestone and computes the amount", async () => {
      ...
      (
        prisma.contractBillingMilestone.create as ReturnType<typeof vi.fn>
    ...
    it("deletes a billing milestone if status is pending", async () => {
      ...
      (
        prisma.contractBillingMilestone.findFirst as ReturnType<typeof vi.fn>
      ).mockResolvedValue(mockMilestone);
      (
        prisma.contractBillingMilestone.delete as ReturnType<typeof vi.fn>
      ).mockResolvedValue(mockMilestone);

      const result = await service.deleteBillingMilestone(
        TENANT,
        "con-1",
        "ms-1",
      );
    ...
    it("triggers a draft invoice generation from a milestone", async () => {
      ...
      const invoice = await service.triggerMilestoneInvoice(
        TENANT,
        "org-1",
        "con-1",
        "ms-1",
