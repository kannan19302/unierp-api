...
describe('checkEnv (Track G.6 boot validation)', () => {
  ...
  it('coerces numeric ports from strings and rejects invalid ones', () => {
    expect(checkEnv({ ...validDev, API_PORT: '8080' }).env?.API_PORT).toBe(8080);
  ...
  it('rejects a non-postgres DATABASE_URL', () => {
    const { env, errors } = checkEnv({ ...validDev, DATABASE_URL: 'mysql://x' });
    expect(env).toBeNull();
  ...
  it('rejects short or placeholder secrets in production only', () => {
    ...
    expect(short.errors.some((error) => error.includes('at least 32 characters'))).toBe(true);

    const placeholder = checkEnv({ ...validProd, EXT_SERVICE_JWT_SECRET: 'change-me' });
  ...
  it('rejects a localhost database in production', () => {
    const { env, errors } = checkEnv({ ...validProd, DATABASE_URL: 'postgresql://u:p@localhost:5432/d' });
    expect(env).toBeNull();
    expect(errors.some((error) => error.includes('localhost database'))).toBe(true);
  });

  it('rejects non-boolean feature flags', () => {
    expect(checkEnv({ ...validDev, BLOCKCHAIN_ENABLED: 'yes' }).env).toBeNull();
    expect(checkEnv({ ...validDev, BLOCKCHAIN_ENABLED: 'true' }).env).not.toBeNull();
  });
});
