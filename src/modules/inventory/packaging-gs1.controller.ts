...
export class PackagingGs1Controller {
  ...
  listBarcodes(
    @Request() req: AuthRequest,
    @Query("packagingSpecId") packagingSpecId?: string,
  ) {
  ...
  getLabelForSpec(
    @Request() req: AuthRequest,
    @Query("packagingSpecId") specId: string,
  ...
  allocateSscc(
    @Request() req: AuthRequest,
    @Body() body: { gs1CompanyPrefix: string; extensionDigit?: number },
  ) {
    return this.svc.allocateSscc(
      req.user.tenantId,
      body.gs1CompanyPrefix,
      body.extensionDigit,
    );
  }
  ...
  markSsccUsed(
    @Request() req: AuthRequest,
    @Param("sscc") sscc: string,
    @Body() body: { shipmentRef: string },
  ) {
