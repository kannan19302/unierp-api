    ...
    $transaction: vi.fn((fn: (tx: unknown) => Promise<unknown>) =>
      fn({
        ...
        purchaseReceipt: {
          create: vi
            .fn()
            .mockResolvedValue({ id: "rcpt-1", receiptNumber: "REC-001" }),
        },
        purchaseReceiptItem: { create: vi.fn() },
        debitNote: {
          create: vi
            .fn()
            .mockResolvedValue({ id: "dn-1", noteNumber: "DN-PR-001" }),
        },
        purchaseReturn: {
          create: vi
            .fn()
            .mockResolvedValue({ id: "pr-1", returnNumber: "PR-001" }),
        },
        purchaseReturnItem: {
          create: vi.fn(),
        },
...
describe("ProcurementService", () => {
  ...
  describe("createPurchaseOrder", () => {
    ...
    it("should throw BadRequestException when PO number already exists", async () => {
      ...
      const dto = {
        ...
        poNumber: "PO-001",
        lineItems: [
          { description: "Widget", quantity: 1, unitPrice: 50, taxRate: 0 },
        ],
      };
  ...
  describe("Purchase Returns", () => {
    it("should return mapped purchase returns", async () => {
      ...
      vi.mocked(prisma.purchaseReturn.findMany).mockResolvedValue(
  ...
  describe("Requisition & Blanket Agreement Services", () => {
    ...
    it("should calculate vendor performance metrics", async () => {
          ...
          receipts: [
            {
              ...
              lineItems: [
                {
                  receivedQty: { toString: () => "10" },
                  acceptedQty: { toString: () => "9" },
                },
              ],
            },
          ],
      ...
      const metrics = await service.getVendorPerformanceMetrics(
        "tenant-1",
        "v-1",
      );
