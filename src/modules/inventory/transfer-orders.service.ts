...
export class TransferOrdersService {
      ...
      data: {
        ...
        lines: {
          create: dto.lines.map((l) => ({
            ...
            uom: l.uom ?? "UNIT",
            unitCost:
              l.unitCost != null ? new Prisma.Decimal(l.unitCost) : null,
  ...
  async updateTransferOrder(
    ...
    if (!["DRAFT", "PENDING_APPROVAL"].includes(t.status)) {
      throw new BadRequestException(
        "Can only update DRAFT or PENDING_APPROVAL orders",
      );
    }
  ...
  async approve(tenantId: string, id: string, approvedBy: string) {
    ...
    if (t.status !== "PENDING_APPROVAL")
      throw new BadRequestException("Order must be PENDING_APPROVAL");
    return prisma.transferOrder.update({
      where: { id },
  ...
  async ship(
    ...
    if (t.status !== "APPROVED")
      throw new BadRequestException("Order must be APPROVED before shipping");
    ...
    for (const sl of dto.shippedLines) {
      ...
      if (sl.shippedQty > Number(line.requestedQty)) {
        throw new BadRequestException(
          `Shipped qty ${sl.shippedQty} exceeds requested ${line.requestedQty} for line ${sl.lineId}`,
        );
      }
  ...
  async receive(
    ...
    if (!["IN_TRANSIT", "PARTIALLY_RECEIVED"].includes(t.status)) {
      throw new BadRequestException(
        "Order must be IN_TRANSIT or PARTIALLY_RECEIVED to receive",
      );
    }

    const seq = await prisma.transferOrderReceipt.count({
      ...
      data: {
        ...
        lines: {
          create: dto.lines.map((l) => ({
            ...
            productId:
              t.lines.find((tl) => tl.id === l.lineId)?.productId ?? "",
    ...
    for (const rl of dto.lines) {
      ...
      if (existing) {
        await prisma.transferOrderLine.update({
          where: { id: rl.lineId },
          data: {
            receivedQty: { increment: new Prisma.Decimal(rl.receivedQty) },
          },
        });
      }
    }
    ...
    const allReceived =
      updated?.lines.every(
        (l) => Number(l.receivedQty) >= Number(l.shippedQty),
      ) ?? false;
  ...
  async closeOut(tenantId: string, id: string) {
    ...
    if (t.status !== "PARTIALLY_RECEIVED") {
      throw new BadRequestException(
        "Only PARTIALLY_RECEIVED orders can be closed out",
      );
    }
    return prisma.transferOrder.update({
      where: { id },
  ...
  async addLine(
    ...
    if (!["DRAFT", "PENDING_APPROVAL"].includes(t.status)) {
      throw new BadRequestException(
        "Lines can only be added to DRAFT or PENDING_APPROVAL orders",
      );
    }
  ...
  async removeLine(tenantId: string, id: string, lineId: string) {
    ...
    if (!["DRAFT", "PENDING_APPROVAL"].includes(t.status)) {
      throw new BadRequestException(
        "Lines can only be removed from DRAFT or PENDING_APPROVAL orders",
      );
    }
  ...
  async getDashboard(tenantId: string) {
    const [
      ...
      prisma.transferOrder.count({
        where: { tenantId, status: "PARTIALLY_RECEIVED" },
      }),
  ...
  async getInTransitSummary(tenantId: string) {
    const orders = await prisma.transferOrder.findMany({
      where: {
        tenantId,
        status: { in: ["IN_TRANSIT", "PARTIALLY_RECEIVED"] as never[] },
      },
      include: { lines: true },
    });
  ...
  async getReceivingReport(
    ...
    return {
      orders,
      totalRejections: rejections.reduce((s, r) => s + r.total, 0),
      rejectionsByProduct: rejections,
    };
  }
}
