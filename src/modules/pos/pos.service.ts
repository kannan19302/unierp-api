...
export class PosService {
  ...
  async getTerminals(tenantId: string) {
    return this.p.pOSTerminal.findMany({
      where: { tenantId },
      orderBy: { name: "asc" },
    });
  }
  ...
  async getTerminalById(tenantId: string, id: string) {
    const term = await this.p.pOSTerminal.findFirst({
      where: { id, tenantId },
    });
  ...
  async createTerminal(
    ...
    const existing = await this.p.pOSTerminal.findFirst({
      where: { tenantId, orgId: resolvedOrgId, code: dto.code },
    });
    if (existing)
      throw new BadRequestException(
        `POS Terminal with code ${dto.code} already exists.`,
      );
  ...
  async deleteTerminal(tenantId: string, id: string) {
    ...
    if (!term) throw new NotFoundException("POS Terminal not found");
    return this.p.pOSTerminal.update({
      where: { id },
  ...
  async getRegisters(tenantId: string) {
    return this.p.pOSRegister.findMany({
      where: { tenantId },
  ...
  async openRegister(
    ...
    const active = await this.p.pOSRegister.findFirst({
      where: { tenantId, terminalId: dto.terminalId, status: "OPEN" },
    });
    if (active)
      throw new BadRequestException(
        "This terminal already has an open register session.",
      );
  ...
  async getShifts(tenantId: string, registerId: string) {
    return this.p.pOSShift.findMany({
  ...
  async startShift(
    ...
    const activeShift = await this.p.pOSShift.findFirst({
      where: { tenantId, registerId, status: "OPEN" },
    });
    if (activeShift)
      throw new BadRequestException(
        "There is already an active shift on this register.",
      );
  ...
  async endShift(tenantId: string, shiftId: string) {
    const shift = await this.p.pOSShift.findFirst({
  ...
  async addCashEntry(
    ...
    const register = await this.p.pOSRegister.findFirst({
    ...
    const order = await prisma.$transaction(async (tx) => {
      ...
      const newOrder = await ptx.posOrder.create({
        data: {
          ...
          subtotal: new Prisma.Decimal(
            dto.items.reduce((sum: number, i) => sum + i.lineTotal, 0),
          ),
          ...
          roundingAmount: new Prisma.Decimal(dto.roundingAmount || 0),
          grandTotal: new Prisma.Decimal(dto.grandTotal),
          paidAmount: new Prisma.Decimal(dto.paidAmount),
          changeAmount: new Prisma.Decimal(dto.changeAmount || 0),
          tipAmount: new Prisma.Decimal(dto.tipAmount || 0),
      ...
      for (const payment of dto.payments) {
        ...
        if (payment.method === "GIFT_CARD" && payment.giftCardId) {
          const giftCard = await ptx.posGiftCard.findFirst({
            where: { id: payment.giftCardId, tenantId },
          });
          if (giftCard) {
            const newBalance = Number(giftCard.currentBalance) - payment.amount;
            await ptx.posGiftCard.update({
              where: { id: payment.giftCardId },
              data: {
                currentBalance: new Prisma.Decimal(Math.max(0, newBalance)),
              },
            });
            await ptx.posGiftCardTransaction.create({
              data: {
                tenantId,
      ...
      if (dto.registerId) {
        await ptx.posRegister.update({
          where: { id: dto.registerId },
          data: {
            totalSales: { increment: dto.grandTotal },
            totalTransactions: { increment: 1 },
          },
        });
      }
      if (dto.shiftId) {
        await ptx.posShift.update({
          where: { id: dto.shiftId },
          data: { totalSales: { increment: dto.grandTotal } },
        });
      }
      ...
      if (dto.couponCode) {
        const coupon = await ptx.posCoupon.findFirst({
          where: { tenantId, code: dto.couponCode },
        });
        if (coupon) {
          await ptx.posCoupon.update({
            where: { id: coupon.id },
  ...
  async voidOrder(
    ...
    const updatedOrder = await prisma.$transaction(async (tx) => {
      const ptx = tx as any;
      const voided = await ptx.posOrder.update({
        where: { id: orderId },
  ...
  async getDiscounts(tenantId: string) {
    return prisma.pOSDiscount.findMany({
      where: { tenantId },
      orderBy: { name: "asc" },
    });
  }
  async getDiscountById(tenantId: string, id: string) {
    const d = await prisma.pOSDiscount.findFirst({ where: { id, tenantId } });
  ...
  async validateCoupon(tenantId: string, code: string, orderAmount: number) {
    ...
    if (!discount || discount.status !== "ACTIVE")
      throw new BadRequestException("Associated discount is not active");
    ...
    if (discount.minPurchase && orderAmount < Number(discount.minPurchase))
      throw new BadRequestException(
        `Minimum purchase of $${Number(discount.minPurchase)} required`,
      );
  ...
  async createCoupon(
    ...
    const existing = await prisma.pOSCoupon.findFirst({
      where: { tenantId, code: dto.code },
    });
    if (existing) throw new BadRequestException("Coupon code already exists");
    return prisma.pOSCoupon.create({
      data: {
        tenantId,
  ...
  async getCoupons(tenantId: string) {
    return prisma.pOSCoupon.findMany({
      where: { tenantId },
      include: { discount: true },
      orderBy: { createdAt: "desc" },
    });
  ...
  async createTaxProfile(
    ...
    const resolvedOrgId = await resolveOrgId(tenantId, orgId);
    if (dto.isDefault) {
      await prisma.pOSTaxProfile.updateMany({
        where: { tenantId, orgId: resolvedOrgId },
        data: { isDefault: false },
      });
    }
    return prisma.pOSTaxProfile.create({
      data: {
        tenantId,
        orgId: resolvedOrgId,
        name: dto.name,
  ...
  async getTaxProfiles(tenantId: string) {
    return prisma.pOSTaxProfile.findMany({
      where: { tenantId },
      orderBy: { name: "asc" },
    });
  }
  ...
  async getQuickKeys(tenantId: string, terminalId: string) {
    return prisma.pOSQuickKey.findMany({
      where: { tenantId, terminalId },
  ...
  async saveQuickKeys(
    ...
    if (keys.length > 0) {
      return prisma.pOSQuickKey.createMany({
        data: keys.map((k) => ({
          tenantId,
          terminalId,
          ...
          position: k.position,
          categoryGroup: k.categoryGroup || null,
        })),
      });
    }
    return { count: 0 };
  ...
  async createLoyaltyProgram(
    ...
    const resolvedOrgId = await resolveOrgId(tenantId, orgId);
    return prisma.pOSLoyaltyProgram.create({
      data: {
        tenantId,
        orgId: resolvedOrgId,
        name: dto.name,
        ...
        redeemRate: new Prisma.Decimal(dto.redeemRate),
  ...
  async getLoyaltyPrograms(tenantId: string) {
    return prisma.pOSLoyaltyProgram.findMany({
      where: { tenantId },
      orderBy: { name: "asc" },
    });
  }
  ...
  async getLoyaltyBalance(tenantId: string, customerId: string) {
    const member = await prisma.pOSLoyaltyMember.findFirst({
      where: { tenantId, customerId },
    });
    if (!member)
      return { points: 0, tier: "BRONZE", lifetimePoints: 0, lifetimeSpent: 0 };
    return {
      points: member.points,
      tier: member.tier,
      lifetimePoints: member.lifetimePoints,
      lifetimeSpent: Number(member.lifetimeSpent),
      visitCount: member.visitCount,
      lastVisit: member.lastVisit,
    };
  }

  async getLoyaltyMembers(tenantId: string, programId: string) {
    return prisma.pOSLoyaltyMember.findMany({
  ...
  async redeemLoyaltyPoints(
    ...
    if (!member)
      throw new BadRequestException("Customer is not a loyalty member");
    if (member.points < points)
      throw new BadRequestException("Insufficient loyalty points");
    const program = await prisma.pOSLoyaltyProgram.findFirst({
      where: { id: member.programId },
    });
    if (!program) throw new BadRequestException("Loyalty program not found");
    if (points < program.minRedeemPoints)
      throw new BadRequestException(
        `Minimum ${program.minRedeemPoints} points required for redemption`,
      );
    return prisma.$transaction(async (tx) => {
      const ptx = tx as any;
      const updated = await ptx.posLoyaltyMember.update({
        where: { id: member.id },
        data: { points: { decrement: points } },
      });
      await ptx.posLoyaltyTransaction.create({
        data: {
          tenantId,
  ...
  async issueGiftCard(
    ...
    const resolvedOrgId = await resolveOrgId(tenantId, orgId);
    const existing = await prisma.pOSGiftCard.findFirst({
      where: { tenantId, code: dto.code },
    });
    if (existing)
      throw new BadRequestException("Gift card code already exists");
    return prisma.pOSGiftCard.create({
      data: {
        tenantId,
        orgId: resolvedOrgId,
        code: dto.code.toUpperCase(),
        initialBalance: new Prisma.Decimal(dto.initialBalance),
        currentBalance: new Prisma.Decimal(dto.initialBalance),
  ...
  async getGiftCards(tenantId: string) {
    return prisma.pOSGiftCard.findMany({
      where: { tenantId },
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async getStoreCredit(tenantId: string, customerId: string) {
    const credit = await prisma.pOSStoreCredit.findFirst({
      where: { tenantId, customerId },
    });
    return credit
      ? { balance: Number(credit.balance), currency: credit.currency }
  ...
  async processReturn(
    ...
    const resolvedOrgId = await resolveOrgId(tenantId, orgId);
    const originalOrder = await prisma.pOSOrder.findFirst({
    ...
    const totalRefund = dto.items.reduce(
      (sum: number, i) => sum + i.refundAmount,
      0,
    );
    return prisma.$transaction(async (tx) => {
      const ptx = tx as any;
      const posReturn = await ptx.posReturn.create({
        data: {
          tenantId,
          orgId: resolvedOrgId,
          returnNumber,
          ...
          exchangeOrderId: dto.exchangeOrderId || null,
          processedBy: dto.processedBy,
          processedAt: new Date(),
        },
      });
      for (const item of dto.items) {
        await ptx.posReturnItem.create({
          data: {
            tenantId,
        ...
        if (item.restock !== false) {
          const orderItem = (originalOrder as any).items.find(
      ...
      await ptx.posOrder.update({
        where: { id: dto.originalOrderId },
        data: { status: dto.type === "RETURN" ? "RETURNED" : "EXCHANGED" },
      });
      return posReturn;
    });
  }
  ...
  async getReturnById(tenantId: string, id: string) {
    const r = await prisma.pOSReturn.findFirst({
      where: { id, tenantId },
      include: { items: true },
    });
  ...
  async approveReturn(tenantId: string, id: string) {
    const r = await prisma.pOSReturn.findFirst({ where: { id, tenantId } });
    if (!r) throw new NotFoundException("Return not found");
    return prisma.pOSReturn.update({
      where: { id },
  ...
  async getHeldOrders(tenantId: string, terminalId?: string) {
    const where: any = { tenantId, status: "HELD" };
  ...
  async resumeHeldOrder(tenantId: string, id: string) {
    ...
    if (held.expiresAt && held.expiresAt < new Date())
      throw new BadRequestException("Held order has expired");
    return held;
  }
  async discardHeldOrder(tenantId: string, id: string) {
    const held = await prisma.pOSHeldOrder.findFirst({
      where: { id, tenantId },
    });
    if (!held) throw new NotFoundException("Held order not found");
    return prisma.pOSHeldOrder.update({
      where: { id },
  ...
  async createPriceList(
    ...
    dto: {
      ...
      validFrom?: string;
      validTo?: string;
      items?: Array<{ productId: string; price: number; minQty?: number }>;
    },
  ) {
    const resolvedOrgId = await resolveOrgId(tenantId, orgId);
    if (dto.isDefault) {
      await prisma.pOSPriceList.updateMany({
        where: { tenantId, orgId: resolvedOrgId },
        data: { isDefault: false },
      });
    }
    return prisma.pOSPriceList.create({
      data: {
        tenantId,
        orgId: resolvedOrgId,
        name: dto.name,
        ...
        items: dto.items
          ? {
              create: dto.items.map((i) => ({
                ...
                minQty: new Prisma.Decimal(i.minQty || 1),
              })),
            }
          : undefined,
      },
  ...
  async getPriceLists(tenantId: string) {
    return prisma.pOSPriceList.findMany({
      where: { tenantId },
      include: { items: true },
      orderBy: { name: "asc" },
    });
  ...
  async getPromotions(tenantId: string) {
    return prisma.pOSPromotion.findMany({
      where: { tenantId },
  ...
  async createOpenTab(
    ...
    return prisma.pOSOpenTab.create({
      data: {
        tenantId,
        orgId: resolvedOrgId,
        terminalId: dto.terminalId,
  ...
  async getOpenTabs(tenantId: string) {
    return prisma.pOSOpenTab.findMany({
      where: { tenantId, status: "OPEN" },
      orderBy: { openedAt: "desc" },
    });
  }
  async closeOpenTab(tenantId: string, id: string) {
    const tab = await prisma.pOSOpenTab.findFirst({ where: { id, tenantId } });
    if (!tab) throw new NotFoundException("Open tab not found");
    return prisma.pOSOpenTab.update({
      where: { id },
  ...
  async createLayaway(
    ...
    return prisma.pOSLayaway.create({
      data: {
        tenantId,
        orgId: resolvedOrgId,
        layawayNumber,
        ...
        paidAmount: new Prisma.Decimal(dto.depositAmount),
  ...
  async getLayaways(tenantId: string) {
    return prisma.pOSLayaway.findMany({
      where: { tenantId },
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async recordLayawayPayment(
    ...
    const layaway = await prisma.pOSLayaway.findFirst({
      where: { id: layawayId, tenantId },
    });
    ...
    return prisma.$transaction(async (tx) => {
      ...
      return ptx.posLayawayPayment.create({
        data: {
          tenantId,
          layawayId,
          amount: new Prisma.Decimal(dto.amount),
  ...
  async getCustomerDisplayConfig(tenantId: string, terminalId: string) {
    try {
      const config = await (prisma as any).pOSCustomerDisplay.findFirst({
        where: { tenantId, terminalId },
      });
      return (
        config || {
  ...
  async updateCustomerDisplayConfig(
    ...
    try {
      ...
      const existing = await prismaAny.pOSCustomerDisplay.findFirst({
        where: { tenantId, terminalId },
      });
      if (existing) {
        return prismaAny.pOSCustomerDisplay.update({
          where: { id: existing.id },
          data: dto,
        });
      }
      return prismaAny.pOSCustomerDisplay.create({
        data: {
          tenantId,
          terminalId,
          enabled: dto.enabled,
          template: dto.template || "default",
          showCart: dto.showCart ?? true,
          showTotal: dto.showTotal ?? true,
          showPromo: dto.showPromo ?? false,
        },
      });
    } catch {
      return {
  ...
  async getDailySummary(tenantId: string, date?: string) {
    ...
    const totalSales = orders
      ...
      .reduce((sum: number, o: any) => sum + Number(o.grandTotal), 0);
    const totalReturns = orders
      .filter((o: any) => o.type === "RETURN")
      .reduce((sum: number, o: any) => sum + Number(o.grandTotal), 0);
  ...
  async getShiftReport(tenantId: string, shiftId: string) {
    ...
    return {
      ...
      totalSales: sales.reduce(
        (s: number, o: any) => s + Number(o.grandTotal),
        0,
      ),
  ...
  async getSalesByProduct(
    ...
    for (const order of orders) {
      for (const item of (order as any).items) {
        ...
        productMap[key].totalQty += Number(item.qty);
  ...
  async getSalesByCashier(
    ...
    for (const order of orders) {
      ...
      if (!cashierMap[key]) {
        cashierMap[key] = {
          cashierId: key,
          cashierName: order.cashierName || key,
          totalSales: 0,
          orderCount: 0,
          avgOrderValue: 0,
        };
      }
      cashierMap[key].totalSales += Number(order.grandTotal);
      cashierMap[key].orderCount += 1;
    }
    return Object.values(cashierMap)
      .map((c: any) => ({
        ...c,
        avgOrderValue: c.orderCount > 0 ? c.totalSales / c.orderCount : 0,
      }))
  ...
  async getSalesByHour(tenantId: string, date?: string) {
    ...
    const hourlyData: Record<
      number,
      { hour: number; sales: number; count: number }
    > = {};
    for (let i = 0; i < 24; i++) {
    ...
    for (const order of orders) {
      ...
      if (hourlyData[hour]) {
        hourlyData[hour].sales += Number(order.grandTotal);
  ...
  async getSalesByPaymentMethod(
    ...
    for (const order of orders) {
      for (const payment of (order as any).payments) {
        ...
        methodMap[m].total += Number(payment.amount);
        methodMap[m].count += 1;
      }
    }
  ...
  async getDiscountUsage(tenantId: string, dateFrom?: string, dateTo?: string) {
    ...
    const totalDiscountAmount = ordersWithDiscount.reduce(
      (s: number, o: any) => s + Number(o.discountAmount),
      0,
    );
    ...
    return {
      totalOrdersWithDiscount: ordersWithDiscount.length,
      totalDiscountAmount,
      averageDiscountPerOrder:
        ordersWithDiscount.length > 0
          ? totalDiscountAmount / ordersWithDiscount.length
          : 0,
      couponUsage,
    };
  }
  ...
  async getCustomerInsights(tenantId: string) {
    const members = await prisma.pOSLoyaltyMember.findMany({
      where: { tenantId },
  ...
  async updateTerminalReceiptTemplate(
    ...
    const term = await prisma.pOSTerminal.findFirst({
      where: { id, tenantId },
    });
    if (!term) throw new NotFoundException("POS Terminal not found");
  ...
  async updateTerminalDiagnostics(
    ...
    return prisma.pOSTerminal.update({
      where: { id },
      data: { diagnosticData: dto.diagnosticData },
    });
  }
    ...
    const count = await prisma.pOSOrder.count({
      where: {
        ...
        createdAt: {
          ...
          lte: new Date(date.getFullYear(), 11, 31, 23, 59, 59),
        },
      },
    });
  ...
  private async deductInventory(
    ...
    if (newQty < 0) {
      throw new BadRequestException(
        `Insufficient stock for product ${productId}: requested ${qty}, available ${Number(inventory.quantity)}`,
      );
    }
  ...
  private async earnLoyaltyPoints(
    ...
    const program = await tx.posLoyaltyProgram.findFirst({
      where: { tenantId, status: "ACTIVE" },
    });
    if (!program) return;
    let member = await tx.posLoyaltyMember.findFirst({
      where: { tenantId, customerId },
    });
    ...
    if (member) {
      ...
      const tiers: Array<{ name: string; minPoints: number }> =
        program.tiers || [];
      for (const t of [...tiers].sort((a, b) => b.minPoints - a.minPoints)) {
        if (newPoints >= t.minPoints) {
          tier = t.name;
          break;
        }
      }
      member = await tx.posLoyaltyMember.update({
        where: { id: member.id },
        data: {
          points: newPoints,
      ...
      member = await tx.posLoyaltyMember.create({
        data: {
          tenantId,
          ...
          lifetimeSpent: new Prisma.Decimal(amount),
          visitCount: 1,
          tier: "BRONZE",
          lastVisit: new Date(),
          status: "ACTIVE",
