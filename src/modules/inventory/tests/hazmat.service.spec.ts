...
const mkClass = (o: Record<string, unknown> = {}) => ({
  ...
  hazardClass: "CLASS_3_FLAMMABLE_LIQUIDS",
  subsidiaryHazards: [],
  packingGroup: "II",
  regulation: "ADR",
  sdsRecords: [],
...
const mkManifest = (o: Record<string, unknown> = {}) => ({
  id: "mf1",
  tenantId: TENANT,
  manifestNumber: "HMF-000001",
  regulation: "ADR",
  originAddress: "1 Warehouse Rd",
  destAddress: "5 Depot Ave",
  status: "DRAFT",
  lines: [],
  createdBy: USER,
  ...o,
...
describe("HazmatService", () => {
    ...
    it("filters by productId", async () => {
      ...
      expect(prisma.hazmatClassification.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ productId: "prod2" }),
        }),
      );
    });
  });
  ...
  describe("getClassification", () => {
    ...
    it("throws NotFoundException when not found", async () => {
      ...
      await expect(svc.getClassification(TENANT, "bad")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("createClassification", () => {
    it("creates classification with auto-number", async () => {
      ...
      vi.mocked(prisma.hazmatClassification.create).mockResolvedValue(
        mkClass() as never,
      );
    ...
    it("throws if classification already exists for product+regulation", async () => {
      ...
      await expect(
        svc.createClassification(TENANT, USER, {
          productId: "prod1",
          unNumber: "UN1234",
          properShippingName: "x",
          hazardClass: "CLASS_3_FLAMMABLE_LIQUIDS",
          regulation: "ADR",
        }),
  ...
  describe("deleteClassification", () => {
    it("deletes when no manifest references", async () => {
      ...
      vi.mocked(prisma.hazmatClassification.delete).mockResolvedValue(
        mkClass() as never,
      );
    ...
    it("throws when manifest lines reference the classification", async () => {
      ...
      await expect(svc.deleteClassification(TENANT, "cls1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("createSds", () => {
    it("creates SDS with auto-number", async () => {
      ...
      vi.mocked(prisma.safetyDataSheet.create).mockResolvedValue(
        mkSds() as never,
      );
      ...
      expect(prisma.safetyDataSheet.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ sdsNumber: "SDS-00001" }),
        }),
      );
    });
  ...
  describe("acknowledgeSds", () => {
    it("acknowledges an unacknowledged SDS", async () => {
      ...
      vi.mocked(prisma.safetyDataSheet.update).mockResolvedValue(
        mkSds({ acknowledgedBy: USER }) as never,
      );
    ...
    it("throws if already acknowledged", async () => {
      ...
      await expect(svc.acknowledgeSds(TENANT, "sds1", USER)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("upsertStorageRule", () => {
    it("creates or updates a storage compatibility rule", async () => {
      ...
      vi.mocked(prisma.hazmatStorageRule.upsert).mockResolvedValue({
        id: "rule1",
        tenantId: TENANT,
        hazardClassA: "CLASS_3_FLAMMABLE_LIQUIDS",
        hazardClassB: "CLASS_5_OXIDIZERS",
        result: "INCOMPATIBLE",
        condition: null,
        notes: null,
        createdAt: new Date(),
  ...
  describe("checkCompatibility", () => {
    it("returns rule when found", async () => {
      ...
      const result = await svc.checkCompatibility(
        TENANT,
  ...
  describe("createManifest", () => {
    it("creates manifest with auto-number", async () => {
      ...
      expect(prisma.hazmatManifest.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ manifestNumber: "HMF-000001" }),
        }),
      );
    });
  });
  ...
  describe("submitManifest", () => {
    it("submits a DRAFT manifest with lines", async () => {
      ...
      vi.mocked(prisma.hazmatManifest.update).mockResolvedValue(
        mkManifest({ status: "SUBMITTED" }) as never,
      );
    ...
    it("throws if manifest has no lines", async () => {
      ...
      await expect(svc.submitManifest(TENANT, "mf1")).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("manifest lifecycle", () => {
    it("acknowledges SUBMITTED manifest", async () => {
      ...
      vi.mocked(prisma.hazmatManifest.update).mockResolvedValue(
        mkManifest({ status: "ACKNOWLEDGED" }) as never,
      );
    ...
    it("marks ACKNOWLEDGED manifest as IN_TRANSIT", async () => {
      ...
      vi.mocked(prisma.hazmatManifest.update).mockResolvedValue(
        mkManifest({ status: "IN_TRANSIT" }) as never,
      );
    ...
    it("delivers IN_TRANSIT manifest", async () => {
      ...
      vi.mocked(prisma.hazmatManifest.update).mockResolvedValue(
        mkManifest({ status: "DELIVERED" }) as never,
      );
    ...
    it("cancels a DRAFT manifest", async () => {
      ...
      vi.mocked(prisma.hazmatManifest.update).mockResolvedValue(
        mkManifest({ status: "CANCELLED" }) as never,
      );
    ...
    it("throws on cancel of DELIVERED manifest", async () => {
      ...
      await expect(svc.cancelManifest(TENANT, "mf1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("createIncident", () => {
    it("creates incident with auto-number", async () => {
      ...
      expect(prisma.hazmatIncident.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ incidentNumber: "HMI-00001" }),
        }),
      );
    });
  });
  ...
  describe("closeIncident", () => {
    it("closes an open incident", async () => {
      ...
      vi.mocked(prisma.hazmatIncident.update).mockResolvedValue(
        mkIncident({ closedAt: new Date(), closedBy: USER }) as never,
      );
    ...
    it("throws if already closed", async () => {
      ...
      await expect(svc.closeIncident(TENANT, "inc1", USER)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
