...
export class AdvancedHrService {
  ...
  async getSalaryComponents(tenantId: string) {
    return prisma.salaryComponent.findMany({ where: { tenantId } });
  }

  async createSalaryComponent(
    tenantId: string,
    dto: { name: string; type: string; isPercent: boolean; value: number },
  ) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      for (const struct of structures) {
        ...
        const emp = await tx.employee.findUnique({
          where: { id: struct.employeeId },
        });
        const country =
          (emp &&
            emp.address &&
            typeof emp.address === "object" &&
            (emp.address as any).country) ||
          "US";
  ...
  async createLeaveRequest(
    ...
    if (businessDays === 0) {
      throw new BadRequestException(
        "Requested leave duration contains 0 working days (weekends or public holidays only).",
      );
    }
  ...
  async approveLeaveRequest(
    ...
    return prisma.leaveRequest.update({
      where: { id },
      data: { status, approvedBy: approverId, approvedAt: new Date() },
    });
  }
  ...
  async createOnboardingChecklist(
    ...
    dto: {
      employeeId: string;
      templateName: string;
      items: Array<{ task: string; category: string; sortOrder: number }>;
    },
  ) {
  ...
  async completeOnboardingItem(_tenantId: string, itemId: string) {
    return prisma.onboardingItem.update({
      where: { id: itemId },
  ...
  async addOnboardingItem(
    tenantId: string,
    checklistId: string,
    dto: { task: string; category?: string },
  ) {
    const lastItem = await prisma.onboardingItem.findFirst({
      where: { checklistId },
  ...
  async createOffboardingChecklist(
    ...
    dto: {
      ...
      exitDate: string;
      exitReason?: string;
      items: Array<{ task: string; sortOrder: number }>;
    },
  ) {
  ...
  async addOffboardingItem(
    ...
    const lastItem = await prisma.offboardingItem.findFirst({
      where: { checklistId },
      orderBy: { sortOrder: "desc" },
    });
    const sortOrder = lastItem ? lastItem.sortOrder + 1 : 1;
  ...
  async getGoalComments(tenantId: string, goalId: string) {
    return prisma.goalComment.findMany({
  ...
  async getSuccessionPlans(tenantId: string) {
    return prisma.successionPlan.findMany({ where: { tenantId } });
  }
  ...
  async createSuccessionPlan(
    ...
    return prisma.successionPlan.create({
      data: {
        ...
        readinessLevel: dto.readinessLevel || "NOT_READY",
        successorId: dto.successorId || null,
  ...
  async getAppraisals(tenantId: string) {
    ...
    return appraisals.map((app) => ({
      ...
      reviewerName: reviewerMap.get(app.reviewerId) || "System",
    }));
  }
    ...
    return prisma.training.create({
      data: {
        ...
        enrollmentDeadline: dto.enrollmentDeadline
          ? new Date(dto.enrollmentDeadline)
          : null,
      },
      include: {
        enrollments: {
  ...
  async enrollParticipant(
    ...
    if (
      training.enrollmentDeadline &&
      new Date() > new Date(training.enrollmentDeadline)
    ) {
      throw new BadRequestException(
        "Enrollment deadline has passed for this training",
      );
    }

    if (training.capacity) {
      const activeEnrollmentsCount = training.enrollments.filter(
  ...
  async getEmployeeDashboard(tenantId: string, employeeId: string) {
    const [employee, payslips, leaves, goals, skills, assets, documents] =
      await Promise.all([
  ...
  async getHeadcountAnalytics(tenantId: string) {
    ...
    const [
      ...
      prisma.employee.count({
        where: {
          tenantId,
          NOT: { dateOfLeaving: null },
          dateOfLeaving: { gte: oneYearAgo },
        },
      }),
      prisma.employee.count({ where: { tenantId, status: "ACTIVE" } }),
      prisma.applicant.findMany({
        where: { tenantId, currentStage: "HIRED" },
    ...
    for (const e of tenureBuckets) {
      const years =
        (now.getTime() - new Date(e.dateOfJoining).getTime()) /
        (365.25 * 86400000);
    ...
    const turnoverRate =
      activeCount + leftCount > 0
        ? (leftCount / ((activeCount + (activeCount + leftCount)) / 2)) * 100
        : 0;
    ...
    const timeToHire =
      hiredApplicants.length > 0
        ? Math.round(totalDays / hiredApplicants.length)
        : 15;
  ...
  async getCompensationAnalytics(tenantId: string) {
    ...
    return {
      average: salaries.reduce((s, v) => s + v, 0) / salaries.length,
  ...
  async getHRCostAnalysis(tenantId: string) {
    ...
    const monthly = payrollRuns.map((r) => ({
    ...
    return {
      ...
      costPerEmployee: employeeCount > 0 ? totalPaid / employeeCount : 0,
      employeeCount,
    };
  }
  ...
  async enrollEmployeeBenefit(
    tenantId: string,
    dto: { employeeId: string; schemeId: string; coverageAmount?: number },
  ) {
      ...
      return prisma.employeeBenefit.update({
        ...
        data: {
          ...
          coverageAmount: dto.coverageAmount
            ? new Prisma.Decimal(dto.coverageAmount)
            : null,
        },
      });
    }
  ...
  async upsertSkillRequirement(
    tenantId: string,
    dto: { designation: string; skillName: string; requiredLevel: number },
  ) {
    const existing = await prisma.skillRequirement.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      return prisma.skillRequirement.update({
        where: { id: existing.id },
        data: { requiredLevel: dto.requiredLevel },
      });
    }
  ...
  async getPositionBudgetVariance(tenantId: string) {
    ...
    for (const pos of positions) {
      ...
      const current = deptVarianceMap.get(deptId) || {
  ...
  async runComplianceChecks(tenantId: string) {
    ...
    for (const emp of employees) {
      ...
      const hasGdpr = empDocs.some(
        (d) =>
          d.docType === "GDPR_CONSENT" || d.name.toLowerCase().includes("gdpr"),
      );
      if (!hasGdpr) {
        checksCreated.push({
  ...
  async getTaxTables(tenantId: string) {
    return prisma.taxTable.findMany({
      where: { tenantId },
      orderBy: [{ country: "asc" }, { incomeBracketMin: "asc" }],
    });
  }
  ...
  async createTaxTable(
    ...
    return prisma.taxTable.create({
      data: {
        ...
        incomeBracketMax: dto.incomeBracketMax
          ? new Prisma.Decimal(dto.incomeBracketMax)
          : null,
        taxRate: new Prisma.Decimal(dto.taxRate),
        allowanceAmount: dto.allowanceAmount
          ? new Prisma.Decimal(dto.allowanceAmount)
  ...
  async createHoliday(
    tenantId: string,
    dto: { name: string; date: string; region?: string },
  ) {
  ...
  async getEmployeeDashboardByUserId(tenantId: string, userId: string) {
    ...
    if (!employee) {
      ...
      throw new NotFoundException(
        "Employee record not linked to this user login credentials.",
      );
    }
    return this.getEmployeeDashboard(tenantId, employee.id);
  }

  async updateEmployeeSelfDetails(
    tenantId: string,
    userId: string,
    dto: { phone?: string; address?: any; bankDetails?: any },
  ) {
    const employee = await prisma.employee.findFirst({
      where: { tenantId, userId },
    });
    if (!employee)
      throw new NotFoundException("Employee record not found for updating.");
