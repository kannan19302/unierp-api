...
export class CrmForecastingService {
  ...
  async calculateDealScore(
    ...
    factors.push({
      factor: "Recent activity engagement",
      weight: 20,
      value: activityScore,
    });
    ...
    const sizeScore =
      sizeRatio > 0.5 && sizeRatio < 3 ? 15 : sizeRatio > 0.2 ? 10 : 5;
    factors.push({
      factor: "Deal size alignment",
      weight: 15,
      value: sizeScore,
    });
    ...
    const stageScore =
      stageIdx >= 0 ? Math.round((stageIdx / (stageOrder.length - 1)) * 20) : 5;
    factors.push({
      factor: "Pipeline stage progression",
      weight: 20,
      value: stageScore,
    });
    ...
    factors.push({
      factor: "Products/line items defined",
      weight: 10,
      value: lineItemScore,
    });
    ...
    const daysToClose = opp.expectedCloseDate
      ? Math.round(
          (new Date(opp.expectedCloseDate).getTime() - Date.now()) / 86400000,
        )
      : 999;
    const closeScore =
      daysToClose > 0 && daysToClose < 90 ? 15 : daysToClose > 0 ? 10 : 5;
    factors.push({
      factor: "Close date proximity",
      weight: 15,
      value: closeScore,
    });
    ...
    factors.push({
      factor: "Customer relationship history",
      weight: 20,
      value: relScore,
    });
    totalScore += relScore;
    ...
    return opps
      ...
      .map((o) => ({
        ...
        daysSinceActivity: o.activities[0]
          ? Math.round(
              (Date.now() - new Date(o.activities[0].createdAt).getTime()) /
                86400000,
            )
          : Math.round(
  ...
  async getDealVelocity(
    ...
    const daysToClose = closedDeals.map((d) => {
      ...
      const closed = d.actualCloseDate
    ...
    const avgDealValue =
      closedDeals.length > 0
        ? Math.round(
            closedDeals.reduce((s, d) => s + Number(d.amount || 0), 0) /
              closedDeals.length,
          )
        : 0;
    ...
    const salesVelocity =
      avgDaysToClose > 0
        ? Math.round(
            (avgDealsPerMonth * avgDealValue * winRate) / avgDaysToClose,
          )
        : 0;
    ...
    const velocityByStage = Array.from(stageMap.entries()).map(
      ([stage, v]) => ({
        stage,
    ...
    // Enrich rep names
    const repIds = Array.from(repMap.keys()).filter(
      (id) => id !== "unassigned",
    );
    if (repIds.length > 0) {
      const users = await (idpPrisma as any).user.findMany({
    ...
    const velocityByRep = Array.from(repMap.entries()).map(([repId, r]) => ({
      ...
      winRate:
        r.totalCount > 0 ? Math.round((r.wonCount / r.totalCount) * 100) : 0,
    }));
  ...
  async getRevenueWaterfall(
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      const churnedRevenue = lostDeals
        .filter(
          (d) =>
            d.lossReason === "PROJECT_CANCELLED" ||
            d.lossReason === "NO_BUDGET",
        )
      ...
      periods.push({
        ...
        churnedRevenue,
        netRevenue:
          newRevenue + expansionRevenue - contractionRevenue - churnedRevenue,
      });
    }
  ...
  async getQuotas(
    tenantId: string,
    filters?: { repId?: string; period?: string },
  ): Promise<
    Array<{
      id: string;
      repId: string;
  ...
  > {
    ...
    for (const t of targets) {
      ...
      const achievedAmount = wonOpps.reduce(
      ...
      results.push({
        ...
        repId: t.userId || "",
        repName: t.userId
          ? (userMap.get(t.userId) ?? "Unassigned")
          : "Unassigned",
        period: t.period || "",
        quotaAmount,
        achievedAmount,
        attainmentPct:
          quotaAmount > 0
            ? Math.round((achievedAmount / quotaAmount) * 100)
            : 0,
        gapAmount: Math.max(0, quotaAmount - achievedAmount),
        pipelineCoverage:
          quotaAmount > 0
            ? Math.round((pipelineValue / quotaAmount) * 100) / 100
            : 0,
      });
    }
  ...
  async getDealsByForecastCategory(tenantId: string): Promise<{
    ...
    for (const opp of opps) {
      ...
      const cat =
        categoryMap.find((c) => prob >= c.minProb && prob <= c.maxProb) ??
        categoryMap[3]!;
    ...
    return {
      categories,
      summary: { totalPipeline, totalWeighted, totalCommit },
    };
  }

  // ── F8: Forecast Override & Adjustments ─────────────
  async createForecastOverride(
    tenantId: string,
    data: {
      opportunityId: string;
  ...
  async getDealPushRate(tenantId: string): Promise<{
    ...
    let totalPushed = 0;
    let totalSlipDays = 0;
    const repPush = new Map<
      string,
      { name: string; pushed: number; total: number; slipDays: number }
    >();
    ...
    for (const deal of closedDeals) {
      const expectedClose = deal.expectedCloseDate
        ...
        : null;
      const actualClose = deal.actualCloseDate
        ? new Date(deal.actualCloseDate)
        : new Date(deal.updatedAt);

      const repId = deal.assignedToId || "unassigned";
      const rep = repPush.get(repId) || {
        name: "Unknown",
        pushed: 0,
        total: 0,
        slipDays: 0,
      ...
      if (deal.assignedToId)
        rep.name = userMap.get(deal.assignedToId) ?? "Unknown";
      rep.total += 1;

      if (expectedClose && actualClose > expectedClose) {
        totalPushed += 1;
        const slip = Math.round(
          (actualClose.getTime() - expectedClose.getTime()) / 86400000,
        );
        totalSlipDays += slip;
        rep.pushed += 1;
        rep.slipDays += slip;
    ...
    const overallPushRate =
      total > 0 ? Math.round((totalPushed / total) * 100) : 0;
    const avgSlipDays =
      totalPushed > 0 ? Math.round(totalSlipDays / totalPushed) : 0;
  ...
  async getPipelineCoverage(tenantId: string): Promise<{
    ...
    const overallCoverage =
      totalQuota > 0 ? Math.round((totalPipeline / totalQuota) * 100) / 100 : 0;

    return { overallCoverage, coverageByRep, coverageByPeriod: [] };
  }

  // ── F12: Stage Conversion Rates ─────────────────────
  async getStageConversionRates(tenantId: string): Promise<
    Array<{
      fromStage: string;
      toStage: string;
  ...
  > {
    ...
    for (let i = 0; i < stages.length - 1; i++) {
      ...
      const conversionRate =
        total > 0 ? Math.round((passedStage / total) * 100) : 0;
  ...
  > {
    ...
    for (let i = periods - 1; i >= 0; i--) {
      ...
      const forecastedAmount = highProbDeals.reduce(
        (s, o) => s + Number(o.amount || 0),
        0,
      );
      ...
      const actualAmount = closedWon.reduce(
        (s, o) => s + Number(o.amount || 0),
        0,
      );

      const variance =
        forecastedAmount > 0 ? actualAmount - forecastedAmount : 0;
      const accuracyPct =
        forecastedAmount > 0
  ...
  async getRolling12MonthTrend(tenantId: string): Promise<
    Array<{
      month: string;
  ...
  > {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const closedWonAmount = wonDeals.reduce(
        (s, o) => s + Number(o.amount || 0),
        0,
      );
      const newPipelineAmount = newDeals.reduce(
        (s, o) => s + Number(o.amount || 0),
        0,
      );
      ...
      results.push({
        ...
        avgDealSize:
          wonDeals.length > 0
            ? Math.round(closedWonAmount / wonDeals.length)
            : 0,
      });
    }
  ...
  async getDealRiskIndicators(
    ...
    if (daysSinceActivity > 30) {
      indicators.push({
        ...
        severity: "HIGH",
        description: `No engagement in ${daysSinceActivity} days`,
      });
    ...
    if (opp.expectedCloseDate && new Date(opp.expectedCloseDate) < new Date()) {
      indicators.push({
        risk: "Overdue Close Date",
        severity: "HIGH",
        description: "Expected close date has passed",
      });
    ...
    if ((opp.probability || 0) < 30) {
      indicators.push({
        ...
        severity: "MEDIUM",
        description: `Win probability at ${opp.probability || 0}%`,
      });
      recommendations.push(
        "Reassess deal qualification and consider deprioritizing",
      );
    }
    ...
    if (lineItems === 0) {
      indicators.push({
        risk: "No Products Defined",
        severity: "LOW",
        description: "No line items/products on this deal",
      });
    ...
    if (amount > 50000 && opp.activities.length < 3) {
      indicators.push({
        risk: "Under-Engaged Large Deal",
        severity: "HIGH",
        description: "High-value deal with minimal activity",
      });
      recommendations.push(
        "Increase engagement cadence — multi-thread with additional stakeholders",
      );
    }
  ...
  > {
    ...
    if (opp.expectedCloseDate) {
      ...
      if (daysToClose <= 7 && daysToClose > 0) {
        actions.push({
          action: "Send final proposal and request decision",
          priority: "HIGH",
          reason: `Close date in ${daysToClose} days`,
    ...
    if (
      !hasDemo &&
      ["QUALIFICATION", "NEEDS_ANALYSIS", "VALUE_PROPOSITION"].includes(
        opp.stage,
      )
    ) {
      actions.push({
        ...
        priority: "HIGH",
        reason: "No demo recorded — critical for progression",
        suggestedDate: nextWeek || "",
      });
    }

    // No recent email
    const recentEmail = opp.activities.find(
      (a) =>
        a.type === "EMAIL" &&
        new Date(a.createdAt) > new Date(Date.now() - 7 * 86400000),
    );
    if (!recentEmail) {
      actions.push({
        action: "Send follow-up email with relevant content",
        priority: "MEDIUM",
        reason: "No email communication in 7+ days",
        suggestedDate: tomorrow || "",
      });
    }
    ...
    if (opp.stage === "PROPOSAL") {
      actions.push({
        ...
        priority: "HIGH",
        reason: "Deal is in proposal stage — confirm receipt and discuss",
        suggestedDate: tomorrow || "",
      });
    }
    ...
    if (opp.stage === "NEGOTIATION") {
      actions.push({
        action: "Prepare negotiation counter-offer",
        priority: "HIGH",
        reason: "Deal is in negotiation — have next offer ready",
        suggestedDate: tomorrow || "",
      });
    }
    ...
    if (opp.activities.length < 5 && Number(opp.amount || 0) > 25000) {
      actions.push({
        ...
        priority: "MEDIUM",
        reason: "Large deal with limited contacts — reduce single-thread risk",
        suggestedDate: nextWeek || "",
      });
    }
  ...
  async getPipelineGapAnalysis(tenantId: string): Promise<{
    ...
    const now = new Date();
    const currentQStart = new Date(
      now.getFullYear(),
      Math.floor(now.getMonth() / 3) * 3,
      1,
    );
    const currentQEnd = new Date(
      now.getFullYear(),
      Math.floor(now.getMonth() / 3) * 3 + 3,
      0,
    );
    const nextQStart = new Date(currentQEnd.getTime() + 86400000);
    const nextQEnd = new Date(
      nextQStart.getFullYear(),
      nextQStart.getMonth() + 3,
      0,
    );
    ...
    const currentOpps = await (prisma as any).opportunity.findMany({
      where: {
        ...
        deletedAt: null,
        stage: { notIn: ["CLOSED_WON", "CLOSED_LOST"] },
        expectedCloseDate: { gte: currentQStart, lte: currentQEnd },
      },
    });
    const currentCommitted = currentOpps
      .filter((o) => (o.probability || 0) >= 80)
      .reduce((s, o) => s + Number(o.amount || 0), 0);
    const currentPipeline = currentOpps.reduce(
      (s, o) => s + Number(o.amount || 0),
      0,
    );
    ...
    const wonThisQ = await (prisma as any).opportunity.findMany({
      where: {
        ...
        deletedAt: null,
        stage: "CLOSED_WON",
        actualCloseDate: { gte: currentQStart, lte: currentQEnd },
      },
    });
    const alreadyClosed = wonThisQ.reduce(
      (s, o) => s + Number(o.amount || 0),
      0,
    );
    ...
    const currentGap = Math.max(
      0,
      totalQuota - alreadyClosed - currentCommitted,
    );
    const currentGapPct =
    ...
    const nextOpps = await (prisma as any).opportunity.findMany({
      where: {
        ...
        deletedAt: null,
        stage: { notIn: ["CLOSED_WON", "CLOSED_LOST"] },
        expectedCloseDate: { gte: nextQStart, lte: nextQEnd },
      },
    });
    const nextCommitted = nextOpps
      .filter((o) => (o.probability || 0) >= 80)
      .reduce((s, o) => s + Number(o.amount || 0), 0);
    const nextPipeline = nextOpps.reduce(
      (s, o) => s + Number(o.amount || 0),
      0,
    );
    const nextGap = Math.max(0, totalQuota - nextCommitted);
    const nextGapPct =
      totalQuota > 0 ? Math.round((nextGap / totalQuota) * 100) : 0;

    const recommendations: string[] = [];
    if (currentGapPct > 30)
      recommendations.push(
        "Focus on closing committed deals this quarter — significant gap remains",
      );
    if (nextPipeline < totalQuota * 3)
      recommendations.push(
        "Pipeline coverage for next quarter is below 3x — increase prospecting",
      );
    if (currentCommitted < totalQuota * 0.5)
      recommendations.push(
        "Less than 50% of quota in commit stage — accelerate high-probability deals",
      );

    return {
      currentQuarter: {
        quota: totalQuota,
        committed: Math.round(currentCommitted + alreadyClosed),
        pipeline: Math.round(currentPipeline),
        gap: Math.round(currentGap),
        gapPct: currentGapPct,
      },
      nextQuarter: {
        quota: totalQuota,
        committed: Math.round(nextCommitted),
        pipeline: Math.round(nextPipeline),
        gap: Math.round(nextGap),
        gapPct: nextGapPct,
      },
      recommendations,
    };
  ...
  > {
    ...
    return contacts.map((c) => ({
      ...
      influence:
        c.department?.toLowerCase().includes("exec") ||
      ...
      for (const c of compMatch) {
        ...
        competitors.push({
          ...
          strengths: battlecard?.strengths
            ? (battlecard.strengths as string[])
            : [],
          weaknesses: battlecard?.weaknesses
            ? (battlecard.weaknesses as string[])
            : [],
          status: "ACTIVE",
        });
      }
  ...
  async getLostDealAnalysis(tenantId: string): Promise<{
    ...
    const totalLostAmount = lostDeals.reduce(
    ...
    const byReason = Array.from(reasonMap.entries())
      .map(([reason, r]) => ({
        ...
        pct: totalLost > 0 ? Math.round((r.count / totalLost) * 100) : 0,
      }))
      .sort((a, b) => b.count - a.count);
    ...
    const byStage = Array.from(stageMap.entries()).map(([stage, s]) => ({
      stage,
      count: s.count,
    ...
    const recoverable = lostDeals.filter(
      (d) =>
        ...
        d.actualCloseDate &&
        new Date(d.actualCloseDate) > sixMonthsAgo,
    );
    const recoveryPotential = recoverable.reduce(
      (s, d) => s + Number(d.amount || 0),
      0,
    );
    ...
    return {
      totalLost,
      totalLostAmount: Math.round(totalLostAmount),
      byReason,
      byCompetitor: [],
      byStage,
      topLostDeals,
  ...
  async getSalesCoachingInsights(
    tenantId: string,
    repId: string,
  ): Promise<{
    repName: string;
    ...
    const repWinRate =
      repDeals.length > 0
        ? Math.round((repWon.length / repDeals.length) * 100)
        : 0;
    const repAvgSize =
      repWon.length > 0
        ? Math.round(
            repWon.reduce((s, d) => s + Number(d.amount || 0), 0) /
              repWon.length,
          )
        : 0;
    const repAvgCycle =
    ...
    const teamAvgSize =
      allWon.length > 0
        ? Math.round(
            allWon.reduce((s, d) => s + Number(d.amount || 0), 0) /
              allWon.length,
          )
        : 0;
    const teamAvgCycle =
    ...
    if (repWinRate > teamWinRate)
      strengths.push(
        `Win rate ${repWinRate}% exceeds team average of ${teamWinRate}%`,
      );
    else
      improvements.push(
        `Win rate ${repWinRate}% is below team average of ${teamWinRate}%`,
      );

    if (repAvgSize > teamAvgSize)
      strengths.push(
        `Average deal size $${repAvgSize.toLocaleString()} exceeds team average`,
      );
    else
      improvements.push(
        `Average deal size $${repAvgSize.toLocaleString()} is below team average of $${teamAvgSize.toLocaleString()}`,
      );

    if (repAvgCycle < teamAvgCycle)
      strengths.push("Faster sales cycle than team average");
    else
      improvements.push(
        "Sales cycle is longer than team average — look for deals stuck in stages",
      );

    if (repActivities > teamAvgActivities)
      strengths.push("High activity volume — strong engagement cadence");
    else
      improvements.push(
        "Activity volume below team average — increase outreach frequency",
      );
    ...
    return {
      ...
      metrics: {
        winRate: repWinRate,
        avgDealSize: repAvgSize,
        avgCycleTime: repAvgCycle,
        activityRate: repActivities,
      },
      teamAvg: {
        winRate: teamWinRate,
        avgDealSize: teamAvgSize,
        avgCycleTime: teamAvgCycle,
        activityRate: teamAvgActivities,
      },
      strengths,
      improvements,
    };
  ...
  > {
    ...
    for (let i = weeks - 1; i >= 0; i--) {
      ...
      const totalPipeline = openAtEnd.reduce(
        (s, o) => s + Number(o.amount || 0),
        0,
      );
      const closedWon = wonInWeek.reduce(
        (s, o) => s + Number(o.amount || 0),
        0,
      );
      const closedLost = lostInWeek.reduce(
        (s, o) => s + Number(o.amount || 0),
        0,
      );
  ...
  async getDealAgingByRep(tenantId: string): Promise<
    Array<{
      repId: string;
      repName: string;
  ...
  > {
    ...
    return Array.from(repData.entries()).map(([repId, data]) => {
      const buckets = ranges.map((r) => {
        const matching = data.deals.filter(
          (d) => d.age >= r.min && d.age <= r.max,
        );
        return {
          range: r.range,
      ...
      const avgAge =
        data.deals.length > 0
          ? Math.round(
              data.deals.reduce((s, d) => s + d.age, 0) / data.deals.length,
            )
          : 0;
  ...
  async autoAssignOpportunity(
    tenantId: string,
    opportunityId: string,
    method: "ROUND_ROBIN" | "LOAD_BASED" | "TERRITORY",
  ): Promise<{
    assignedTo: string;
    ...
    if (reps.length === 0)
      throw new BadRequestException("No active reps to assign");
  ...
  async getDealQualification(
    tenantId: string,
    opportunityId: string,
    framework: "BANT" | "MEDDIC" | "SPIN" = "BANT",
  ): Promise<{
    framework: string;
    totalScore: number;
    maxScore: number;
    ...
    if (framework === "BANT") {
      ...
      criteria.push({
        ...
        score: budgetScore,
        maxScore: 25,
        status: budgetScore > 0 ? "Confirmed" : "Unknown",
        notes:
          budgetScore > 0
            ? `$${Number(opp.amount).toLocaleString()} identified`
      ...
      const authorityScore =
        hasExecContact > 0 ? 25 : opp.activities.length > 3 ? 15 : 5;
      criteria.push({
        name: "Authority",
        ...
        notes: `${hasExecContact} executive contacts`,
      });

      // Need
      const needScore = opp.activities.some(
      ...
      criteria.push({
        ...
        score: needScore,
        maxScore: 25,
        status: needScore >= 25 ? "Validated" : "Partially explored",
        notes: `${opp.activities.length} interactions recorded`,
      });
      ...
      criteria.push({
        ...
        score: timelineScore,
        maxScore: 25,
        status: hasCloseDate ? "Defined" : "Undefined",
        notes: hasCloseDate
          ? `Close date: ${new Date(opp.expectedCloseDate!).toLocaleDateString()}`
          : "No close date set",
      });
      ...
      criteria.push({
        name: "Metrics",
        score: opp.lineItems.length > 0 ? 17 : 0,
        maxScore: 17,
        status: opp.lineItems.length > 0 ? "Defined" : "Missing",
        notes: `${opp.lineItems.length} line items`,
      });
      criteria.push({
        name: "Economic Buyer",
        score: 0,
      ...
      criteria.push({
        ...
        score: opp.activities.length > 2 ? 17 : 5,
        maxScore: 17,
        status: opp.activities.length > 2 ? "Partially known" : "Unknown",
        notes: "",
      });
      criteria.push({
      ...
      criteria.push({
        name: "Identify Pain",
        score: opp.activities.some((a) => a.type === "MEETING") ? 16 : 0,
        maxScore: 16,
        status: opp.activities.some((a) => a.type === "MEETING")
          ? "Identified"
          : "Unknown",
        notes: "",
      });
      ...
      criteria.push({
        name: "Situation",
        score: opp.activities.length > 0 ? 25 : 0,
        maxScore: 25,
        status: "Explored",
        notes: "",
      });
      criteria.push({
        name: "Problem",
        score: opp.activities.length > 1 ? 25 : 0,
        maxScore: 25,
        status: "Identified",
        notes: "",
      });
      criteria.push({
        name: "Implication",
        score: opp.activities.length > 3 ? 25 : 0,
        maxScore: 25,
        status: "Discussed",
        notes: "",
      });
      criteria.push({
        name: "Need-Payoff",
        score: opp.lineItems.length > 0 ? 25 : 0,
        maxScore: 25,
        status: "Presented",
        notes: "",
      });
  ...
  async predictCloseDate(
    ...
    if (wonDeals.length > 0) {
      ...
      avgDaysPerStage = Math.round(
        totalDays / wonDeals.length / (stages.length - 1),
      );
    }

    const daysFromNow = Math.max(1, remainingStages * avgDaysPerStage);
    const predictedDateStr = new Date(Date.now() + daysFromNow * 86400000)
      .toISOString()
      .split("T")[0];

    const confidenceLevel =
      wonDeals.length >= 20 ? "HIGH" : wonDeals.length >= 5 ? "MEDIUM" : "LOW";
  ...
  async removeDealTeamMember(
    ...
    return prisma.dealTeamMember.delete({
      where: {
        tenantId_opportunityId_userId: { tenantId, opportunityId, userId },
      },
    });
  }
