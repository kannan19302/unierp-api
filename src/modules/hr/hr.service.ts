...
export class HrService {
  ...
  async getEmployeeStats(tenantId: string) {
    const [total, active, onLeave, terminated] = await Promise.all([
      ...
      prisma.employee.count({
        where: { tenantId, deletedAt: null, status: "TERMINATED" },
      }),
    ]);
    return {
      total,
  ...
  async createPosition(tenantId: string, orgId: string, dto: any) {
    ...
    if (existing)
      throw new BadRequestException(`Position code ${dto.code} already exists`);
    return prisma.position.create({
      data: {
        ...
        title: dto.title,
        code: dto.code,
        budgetedSalary: dto.minSalary || 0,
        status: "VACANT",
      },
    });
  }
  ...
  async updatePosition(tenantId: string, id: string, dto: any) {
    ...
    return prisma.position.update({
      ...
      data: {
        title: dto.title,
        departmentId: dto.departmentId,
        budgetedSalary: dto.minSalary || dto.budgetedSalary,
        status: dto.status,
      },
    });
  }
  ...
  async getAttendanceById(tenantId: string, id: string) {
    const item = await prisma.attendanceRecord.findFirst({
      where: { id, tenantId },
    });
  ...
  async bulkCreateAttendance(tenantId: string, records: any[]) {
    ...
    for (const r of records) {
      try {
        const item = await this.createAttendance(
          tenantId,
          r.orgId || "default",
          r,
        );
    ...
    return {
      ...
      succeeded: created.filter((r) => r.status === "success").length,
      failed: created.filter((r) => r.status === "error").length,
      results: created,
    };
  }
  ...
  async createLeaveType(tenantId: string, orgId: string, dto: any) {
    ...
    if (existing)
      throw new BadRequestException(`Leave type ${dto.name} already exists`);
    return prisma.leavePolicy.create({
      data: {
        tenantId,
        name: dto.name,
        leaveType: dto.code || dto.name,
  ...
  async updateLeaveType(tenantId: string, id: string, dto: any) {
    ...
    return prisma.leavePolicy.update({
      ...
      data: {
        name: dto.name,
        leaveType: dto.code,
        annualAllocation: dto.defaultDays,
        carryForwardLimit: dto.carryForwardDays,
      },
    });
  }
  ...
  async createLeaveRequest(
    ...
    if (!employeeId)
      throw new BadRequestException("Employee not found for user");
  ...
  async approveLeaveRequest(tenantId: string, id: string, userId: string) {
    ...
    if (this.eventEmitter) {
      this.eventEmitter.emit("hr.leave.approved", {
        leaveRequestId: id,
        tenantId,
  ...
  async cancelLeaveRequest(tenantId: string, id: string) {
    ...
    if (item.status !== "PENDING" && item.status !== "APPROVED")
      throw new BadRequestException("Cannot cancel leave request");
    return prisma.leaveRequest.update({
      where: { id },
  ...
  async reclaimLeaveRequest(tenantId: string, id: string) {
    ...
    if (item.status !== "APPROVED")
      throw new BadRequestException("Only approved leave can be reclaimed");
    return prisma.leaveRequest.update({
      where: { id },
  ...
  async allocateLeaveBalance(tenantId: string, orgId: string, dto: any) {
    await resolveOrgId(tenantId, orgId);
    const existing = await prisma.leavePolicy.findFirst({
      where: { tenantId, id: dto.leaveTypeId },
    });
    if (!existing) throw new BadRequestException("Leave type not found");
    return {
      success: true,
      message:
        "Leave allocation recorded (leave balance computed from leave policy and usage)",
    };
  }
  ...
  async processPayRun(tenantId: string, id: string) {
    ...
    if (payRun.status !== "DRAFT")
      throw new BadRequestException("Pay run is not in DRAFT status");
    const updated = await prisma.payrollRun.update({
      where: { id },
      data: { status: "PROCESSED" },
    });
    if (this.eventEmitter) {
      this.eventEmitter.emit("hr.payroll.processed", {
        payRunId: id,
        tenantId,
  ...
  async getPayRunItems(tenantId: string, payRunId: string) {
    return prisma.payrollSlip.findMany({
      where: { tenantId, payrollRunId: payRunId },
    });
  }
  ...
  async createPayRunItem(tenantId: string, dto: any) {
    return prisma.payrollSlip.create({
      data: {
        tenantId,
        payrollRunId: dto.payRunId,
        employeeId: dto.employeeId,
        grossSalary: dto.grossPay,
        deductions: dto.totalDeductions || 0,
  ...
  async updatePayRunItem(tenantId: string, id: string, dto: any) {
    const item = await prisma.payrollSlip.findFirst({
      where: { id, tenantId },
    });
    ...
    return prisma.payrollSlip.update({
      ...
      data: {
        grossSalary: dto.grossPay,
        deductions: dto.totalDeductions,
        netSalary: dto.netPay,
      },
    });
  }
  ...
  async createPerformanceReview(tenantId: string, orgId: string, dto: any) {
    ...
    return prisma.appraisal.create({
      data: {
        ...
        appraisalPeriod: dto.period || "QUARTERLY",
        score: dto.overallRating || 0,
        feedback: dto.notes || null,
        status: "DRAFT",
      },
    });
  }
  ...
  async getTrainingCourseById(tenantId: string, id: string) {
    const item = await prisma.training.findFirst({
      where: { id, tenantId },
      include: {
        enrollments: {
          include: {
  ...
  async createTrainingCourse(tenantId: string, orgId: string, dto: any) {
    ...
    return prisma.training.create({
      data: {
        ...
        instructor: dto.provider || null,
        capacity: dto.maxAttendees || null,
        startDate: new Date(),
        endDate: new Date(),
      },
    });
  ...
  async enrollInTraining(
    ...
    const existing = await prisma.trainingEnrollment.findFirst({
      where: { tenantId, trainingId: sessionId, employeeId },
    });
    if (existing)
      throw new BadRequestException(
    ...
    return prisma.trainingEnrollment.create({
      data: { tenantId, trainingId: sessionId, employeeId, status: "ENROLLED" },
    });
  }

  async completeTrainingEnrollment(
    tenantId: string,
    enrollmentId: string,
    _dto: any,
  ) {
    const item = await prisma.trainingEnrollment.findFirst({
  ...
  async getEmployeeDocumentById(tenantId: string, id: string) {
    const item = await prisma.employeeDocument.findFirst({
      where: { id, tenantId },
    });
  ...
  async getEmployeeSkillById(tenantId: string, id: string) {
    const item = await prisma.employeeSkill.findFirst({
      where: { id, tenantId },
    });
  ...
  async getGoalById(tenantId: string, id: string) {
    const item = await prisma.goal.findFirst({
      where: { id, tenantId },
      include: { keyResults: true, comments: true },
    });
  ...
  async updateGoalProgress(tenantId: string, id: string, progress: number) {
    ...
    return prisma.goal.update({
      where: { id },
      data: { progress: Math.min(100, Math.max(0, progress)), status },
    });
  }
  ...
  async getFeedback360ById(tenantId: string, id: string) {
    const item = await prisma.feedback360.findFirst({
      where: { id, tenantId },
      include: { responses: true },
    });
  ...
  async completeOnboardingItem(
    ...
    const item = await prisma.onboardingItem.findFirst({
      where: { id: itemId, checklistId, tenantId },
    });
  ...
  async getJobPostingById(tenantId: string, id: string) {
    const item = await prisma.jobPosting.findFirst({
      where: { id, tenantId },
  ...
  async getApplicantById(tenantId: string, id: string) {
    const item = await prisma.applicant.findFirst({
      where: { id, tenantId },
  ...
  async updateSuccessionPlan(tenantId: string, id: string, dto: any) {
    const item = await prisma.successionPlan.findFirst({
      where: { id, tenantId },
    });
  ...
  async getLeaveAnalytics(tenantId: string) {
    ...
    const byPolicy = await prisma.leaveRequest.groupBy({
      by: ["policyId"],
      where: { tenantId },
      _count: true,
    });
    ...
    return {
      byStatus: byStatus.map((s) => ({ status: s.status, count: s._count })),
      byLeaveType: byPolicy.map((p) => ({
        leaveTypeId: p.policyId,
        leaveTypeName: policyMap.get(p.policyId) || "Unknown",
        count: p._count,
      })),
    };
  }
  ...
  async getTurnoverReport(tenantId: string, params: any = {}) {
    ...
    const joined = await prisma.employee.count({
      where: { tenantId, deletedAt: null, dateOfJoining: { gte: since } },
    });
    const left = await prisma.employee.count({
