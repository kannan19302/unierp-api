...
export class TreasuryService {
  ...
  async createInvestmentPortfolio(
    tenantId: string,
    orgId: string,
    dto: { name: string; type: string; accountId: string },
  ) {
  ...
  async createTreasuryTransaction(
    tenantId: string,
    orgId: string,
    dto: { type: string; amount: number; bankAccountId: string; date?: string },
  ) {
    const resolvedOrgId = await this.glService.resolveOrgId(tenantId, orgId);
  ...
  async approveInterCompanyTransfer(tenantId: string, transferId: string) {
    const transfer = await prisma.interCompanyTransfer.findFirst({
    ...
    if (!rateRow) {
      ...
      if (!revRateRow)
        throw new NotFoundException(
          `No exchange rate found from ${from} to ${to}`,
        );
  ...
  private async ensureAccount(
    ...
    return prisma.account.create({
      data: { tenantId, orgId, code, name, type },
    });
  }
  ...
  async runCurrencyRevaluation(
    ...
    if (Math.abs(netAdjustment) >= 0.01) {
      const arReserve = await this.ensureAccount(
        tenantId,
        resolvedOrgId,
        "1190",
        "FX Revaluation Reserve (AR)",
        "ASSET",
      );
      const apReserve = await this.ensureAccount(
        tenantId,
        resolvedOrgId,
        "2190",
        "FX Revaluation Reserve (AP)",
        "LIABILITY",
      );
      const fxGain = await this.ensureAccount(
        tenantId,
        resolvedOrgId,
        "4900",
        "Unrealized FX Gain",
      ...
      const fxLoss = await this.ensureAccount(
        tenantId,
        resolvedOrgId,
        "5900",
        "Unrealized FX Loss",
      ...
      if (Math.abs(arNet) >= 0.01) {
        entries.push({
          accountId: arReserve.id,
          debit: arNet > 0 ? arNet : 0,
          credit: arNet < 0 ? -arNet : 0,
          description: "AR FX revaluation",
        });
      }
      if (Math.abs(apNet) >= 0.01) {
        entries.push({
          accountId: apReserve.id,
          debit: apNet > 0 ? apNet : 0,
          credit: apNet < 0 ? -apNet : 0,
          description: "AP FX revaluation",
        });
      }
      ...
      const journal = await this.glService.createJournal(
        ...
        resolvedOrgId,
        {
          ...
          notes: `FX revaluation as of ${asOf.toISOString().slice(0, 10)}`,
          entries,
        },
      );
    ...
    const revaluation = await prisma.currencyRevaluation.create({
      data: {
        ...
        baseCurrency: base,
        status: "POSTED",
        totalGain: new Prisma.Decimal(totalGain),
        totalLoss: new Prisma.Decimal(totalLoss),
        netAdjustment: new Prisma.Decimal(netAdjustment),
        journalId,
    ...
    await this.glService.logAudit(
      ...
      revaluation.id,
      "CREATE",
      { netAdjustment, items: lines.length },
      "system",
    );
  ...
  async createPaymentRun(
    tenantId: string,
    orgId: string,
    dto: { bankAccountId: string; runDate: string; totalAmount?: number },
  ) {
    const resolvedOrgId = await this.glService.resolveOrgId(tenantId, orgId);
