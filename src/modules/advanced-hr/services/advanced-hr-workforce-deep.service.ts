...
export class AdvancedHrWorkforceDeepService {
  async listHeadcountPlans(
    ...
    return prisma.hrHeadcountPlan.findMany({
      where,
  ...
  async getHeadcountPlan(tenantId: string, id: string) {
    const plan = await prisma.hrHeadcountPlan.findFirst({
      where: { id, tenantId },
      include: { lines: true },
    });
  ...
  async updateHeadcountPlan(
    ...
    const existing = await prisma.hrHeadcountPlan.findFirst({
      where: { id, tenantId },
    });
  ...
  async listHeadcountPlanLines(tenantId: string, planId: string) {
    ...
    if (!plan) throw new NotFoundException("Headcount plan not found");
    return prisma.hrHeadcountPlanLine.findMany({
  ...
  async getSuccessionPlan(tenantId: string, id: string) {
    const plan = await prisma.hrSuccessionPlan.findFirst({
      where: { id, tenantId },
      include: { candidates: true },
    });
  ...
  async createSuccessionPlan(
    tenantId: string,
    data: { positionId: string; riskLevel?: string; notes?: string },
    userId: string,
  ) {
  ...
  async listSuccessionCandidates(tenantId: string, planId: string) {
    ...
    if (!plan) throw new NotFoundException("Succession plan not found");
    return prisma.hrSuccessionCandidate.findMany({
      where: { tenantId, planId },
      orderBy: { rank: "asc" },
    });
  }
