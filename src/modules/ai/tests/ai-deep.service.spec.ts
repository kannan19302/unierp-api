...
describe("AiDeepService", () => {
    ...
    vi.mocked(prisma.aiIntentTrainingExample.findMany).mockResolvedValue([
      {
        ...
        text: "hello",
        language: "en",
        entities: [{ entity: "name", value: "world" }],
        tenantId: "t1",
        createdAt: new Date(),
        updatedAt: new Date(),
      },
  ...
  it("should create intent training data", async () => {
    ...
    const result = await service.createIntentTrainingData("t1", {
      intent: "test",
      text: "testing",
      language: "en",
    });
  ...
  it("should update intent training data", async () => {
    ...
    vi.mocked(prisma.aiIntentTrainingExample.update).mockResolvedValue({
      id: "1",
      intent: "updated",
      text: "updated text",
      language: "en",
    ...
    const result = await service.updateIntentTrainingData("t1", "1", {
      intent: "updated",
      text: "updated text",
    });
  ...
  it("should delete intent training data", async () => {
    ...
    expect(prisma.aiIntentTrainingExample.delete).toHaveBeenCalledWith({
      where: { id: "1" },
    });
  });
  ...
  it("should record model accuracy metric", async () => {
    ...
    vi.mocked(prisma.aiModelAccuracyMetric.create).mockResolvedValue({
      id: "met1",
      metric: "f1",
      value: 0.88,
      modelId: "m1",
    ...
    const result = await service.recordModelAccuracy("t1", {
      modelId: "m1",
      metric: "f1",
      value: 0.88,
    });
  ...
  it("should create prompt", async () => {
    ...
    const result = await service.createPrompt("t1", {
      name: "Test",
      prompt: "Hello",
      category: "GENERAL",
    });
