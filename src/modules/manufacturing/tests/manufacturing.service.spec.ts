...
describe("ManufacturingService", () => {
  ...
  describe("getBOMs", () => {
    it("should return all BOMs", async () => {
      const mockBOMs = [
        { id: "bom-1", name: "Laptop Assembly", code: "BOM-LAP" },
      ];
  ...
  describe("getBOMTree (Recursive BOM Explosion)", () => {
    it("should explode multi-level BOM correctly", async () => {
      ...
      vi.mocked(prisma.bOM.findFirst)
        .mockResolvedValueOnce(mockBom as any)
  ...
  describe("createWorkOrder (Shift-Aware Scheduling)", () => {
    it("should schedule work order with capacity & shift offset adjustment", async () => {
      const mockBom = {
        id: "bom-1",
        standardCost: new Prisma.Decimal(50),
        routingJson: [
          {
            sequence: 1,
            name: "Assemble",
      ...
      vi.mocked(prisma.workstation.findFirst).mockResolvedValue(
        mockWorkstation as any,
      );
      vi.mocked(prisma.workOrder.findMany).mockResolvedValue(
        mockExistingWO as any,
      );
  ...
  describe("completeOperationStep (Tool cycle calibration alarm)", () => {
    it("should increment tool cycles and trigger PM ticket if max cycles exceeded", async () => {
      ...
      vi.mocked(prisma.workOrderOperation.findFirst).mockResolvedValue(
        mockOp as any,
      );
      vi.mocked(prisma.workOrderOperation.update).mockResolvedValue({
        id: "op-1",
        status: "COMPLETED",
      } as any);
      vi.mocked(prisma.workstation.findFirst).mockResolvedValue(
      ...
      expect(prisma.maintenanceRequest.create).toHaveBeenCalledWith({
        data: {
          ...
          description:
            "Tool T-DRILL on CNC Station exceeded its limit of 100 cycles.",
          status: "REQUESTED",
        },
      });
    });
  ...
  describe("Engineering Change Orders (ECO)", () => {
    it("should submit an ECO under review", async () => {
      await service.submitECO("tenant-1", {
        bomId: "bom-1",
        changeDescription: "Upgrade component to standard",
        requestedBy: "Eng John",
      });
    ...
    it("should resolve an ECO and update BOM status", async () => {
      vi.mocked(prisma.engineeringChangeOrder.findFirst).mockResolvedValue({
        id: "eco-1",
        bomId: "bom-1",
      } as any);
  ...
  describe("Lot Genealogy Upstream & Downstream Tracking", () => {
    it("should fetch upstream and downstream lot consumption details", async () => {
      const mockConsumptions = [
        {
          ...
          product: { name: "Part A" },
          workOrder: {
            workOrderNumber: "WO-DOWN-101",
            bom: { name: "Finished Product A" },
            lotNumber: "LOT-FIN-A",
          },
        },
      ];
      ...
      vi.mocked(
        prisma.workOrderComponentConsumption.findMany,
      ).mockResolvedValue(mockConsumptions as any);
      vi.mocked(prisma.workOrder.findFirst).mockResolvedValue({
        ...
        componentConsumptions: [
          {
            productId: "sub-p1",
            product: { name: "Sub Component 1", sku: "SKU-SUB" },
            lotNumber: "LOT-SUB-VAL",
            quantityConsumed: new Prisma.Decimal(5),
          },
        ],
      } as any);

      const genealogy = (await service.getLotGenealogy(
        "tenant-1",
        "LOT-XYZ",
      )) as any;
  ...
  describe("Detailed OEE Analytics breakdown", () => {
    it("should compute OEE (Availability, Performance, Quality)", async () => {
      const mockDowntimes = [
        { id: "dt-1", durationMinutes: 120, workstationId: "ws-1" },
      ];
      const mockWorkstations = [
        { id: "ws-1", capacityHours: new Prisma.Decimal(40) },
      ];
      ...
      const mockInspections = [
        {
          id: "insp-1",
          inspectedQty: new Prisma.Decimal(10),
          passedQty: new Prisma.Decimal(9),
        },
      ];

      vi.mocked(prisma.machineDowntimeLog.findMany).mockResolvedValue(
        mockDowntimes as any,
      );
      vi.mocked(prisma.workstation.findMany).mockResolvedValue(
        mockWorkstations as any,
      );
      vi.mocked(prisma.workOrder.findMany).mockResolvedValue(
        mockCompletedWOs as any,
      );
      vi.mocked(prisma.qualityInspection.findMany).mockResolvedValue(
        mockInspections as any,
      );
