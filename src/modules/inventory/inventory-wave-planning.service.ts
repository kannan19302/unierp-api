...
export class InventoryWavePlanningService {
  async getWavePlans(
    ...
    const data = await prisma.wavePlan.findMany({
      where,
  ...
  async getWavePlanById(tenantId: string, id: string) {
    const plan = await prisma.wavePlan.findFirst({
      where: { id, tenantId },
      include: {
  ...
  async createWavePlan(tenantId: string, dto: any, userId?: string) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const totalItems =
        dto.tasks?.reduce((s: number, t: any) => s + t.quantity, 0) || 0;
      const plan = await tx.wavePlan.create({
        data: {
          tenantId,
          planNumber,
          warehouseId: dto.warehouseId || null,
          planType: dto.planType || "PICK",
          optimizationStrategy: dto.optimizationStrategy || "BATCH",
          sortMethod: dto.sortMethod || "ORDER",
          totalOrders: 0,
          totalLines,
        ...
        await tx.wavePlanTask.createMany({
          data: dto.tasks
            .map((t: any) => ({
              ...
              destLocation: t.destLocation || null,
              productId: t.productId || null,
              productSku: t.productSku || null,
  ...
  async releaseWavePlan(tenantId: string, id: string) {
    ...
    if (!plan) throw new NotFoundException("Wave plan not found");
    return prisma.wavePlan.update({
      where: { id },
      data: { status: "RELEASED", startTime: new Date() },
    });
  }

  async assignTask(tenantId: string, taskId: string, assignee: string) {
    const task = await prisma.wavePlanTask.findFirst({
      where: { id: taskId, tenantId },
    });
    if (!task) throw new NotFoundException("Task not found");
    return prisma.wavePlanTask.update({
      where: { id: taskId },
      data: { assignee, status: "ASSIGNED" },
    });
  }
  ...
  async completeTask(tenantId: string, taskId: string, pickedQty?: number) {
    ...
    return prisma.wavePlanTask.update({
      ...
      data: {
        ...
        pickedQty: pickedQty ? new Prisma.Decimal(pickedQty) : task.quantity,
      },
    });
  }
  ...
  async completeWavePlan(tenantId: string, id: string) {
    ...
    if (!plan) throw new NotFoundException("Wave plan not found");
    const totalDuration = plan.startTime
      ? Math.round((Date.now() - plan.startTime.getTime()) / 60000)
      : 0;
    return prisma.wavePlan.update({
      where: { id },
      data: {
        ...
        totalPickers: [
          ...new Set(
            plan.tasks.filter((t) => t.assignee).map((t) => t.assignee),
          ),
        ].length,
      },
    });
  ...
  async getWarehouseKpis(
    ...
    if (sinceDays)
      where.kpiDate = { gte: new Date(Date.now() - sinceDays * 86400000) };
    return prisma.warehouseKpi.findMany({
      where,
  ...
  async recordWarehouseKpi(tenantId: string, dto: any) {
    return prisma.warehouseKpi.create({
      data: {
        tenantId,
        warehouseId: dto.warehouseId || null,
        kpiDate: dto.kpiDate ? new Date(dto.kpiDate) : new Date(),
        linesPicked: dto.linesPicked || null,
        linesPutaway: dto.linesPutaway || null,
        ordersShipped: dto.ordersShipped || null,
        ordersReceived: dto.ordersReceived || null,
        picksPerHour: dto.picksPerHour
          ? new Prisma.Decimal(dto.picksPerHour)
          : null,
        putawayPerHour: dto.putawayPerHour
          ? new Prisma.Decimal(dto.putawayPerHour)
          : null,
        orderAccuracyPct: dto.orderAccuracyPct
          ? new Prisma.Decimal(dto.orderAccuracyPct)
          : null,
        totalLaborHours: dto.totalLaborHours
          ? new Prisma.Decimal(dto.totalLaborHours)
          : null,
        activeWorkers: dto.activeWorkers || null,
        laborCostPerOrder: dto.laborCostPerOrder
  ...
  async getSafetyStockOptimizations(tenantId: string, productId?: string) {
    ...
    return prisma.safetyStockOptimization.findMany({
      where,
  ...
  async optimizeSafetyStock(tenantId: string, dto: any) {
    ...
    const salesItems = await prisma.salesOrderItem.findMany({
      where: {
        ...
        tenantId,
        salesOrder: { status: { in: ["CONFIRMED", "DELIVERED"] } },
      },
    });
    const qtyArr = salesItems.map((i) => Number(i.quantity));
    const mean = qtyArr.length
      ? qtyArr.reduce((s, v) => s + v, 0) / qtyArr.length
      : 10;
    const stdDev =
      qtyArr.length > 1
        ? Math.sqrt(
            qtyArr.reduce((s, v) => s + (v - mean) ** 2, 0) / qtyArr.length,
          )
        : mean * 0.3;
    ...
    const safetyStock = Math.round(
      zScore *
        Math.sqrt(leadTimeDays * stdDev ** 2 + mean ** 2 * leadTimeVariance) ||
        mean * 0.5,
    );
    const reorderPoint = Math.round(mean * leadTimeDays + safetyStock);
    const eoq = Math.round(
      Math.sqrt(
        (2 * mean * 12 * (dto.stockoutCost || 50)) /
          (dto.holdingCostPct || 0.25),
      ),
    );
    ...
    const currentTotalCost =
      currentSafetyStock * unitCost * holdingCostPct +
      ((mean * 12) / eoq) * (dto.stockoutCost || 50);
    const optimizedTotalCost =
      safetyStock * unitCost * holdingCostPct +
      ((mean * 12) / eoq) * (dto.stockoutCost || 50);
    return prisma.safetyStockOptimization.upsert({
      where: {
        tenantId_productId_warehouseId: {
          tenantId,
          productId: dto.productId,
          warehouseId: dto.warehouseId || "__default__",
        },
      } as any,
      create: {
        tenantId,
        ...
        leadTimeDays,
        leadTimeVariance,
        demandMean: Math.round(mean),
        demandStdDev: Math.round(stdDev),
        serviceLevel,
        zScore,
        reorderPoint,
        economicOrderQty: eoq,
        holdingCostPct,
        stockoutCost: dto.stockoutCost || 50,
        totalCostCurrent: new Prisma.Decimal(currentTotalCost),
        totalCostOptimized: new Prisma.Decimal(optimizedTotalCost),
        potentialSavings: new Prisma.Decimal(
          currentTotalCost - optimizedTotalCost,
        ),
  ...
  async getGlobalInventory(tenantId: string, productId?: string) {
    const where: any = { tenantId };
    if (productId) where.productId = productId;
    const views = await prisma.globalInventoryView.findMany({
      where,
    ...
    if (!views.length) {
      ...
      for (const product of products) {
        ...
        await prisma.globalInventoryView.upsert({
          where: {
            tenantId_productId: { tenantId, productId: product.id },
          } as any,
          create: {
            tenantId,
            ...
            warehouseCount: [...new Set(items.map((i) => i.warehouseId))]
              .length,
            totalReserved: 0,
          },
          update: {
      ...
      return prisma.globalInventoryView.findMany({
        where: { tenantId },
        include: { details: true },
        orderBy: { totalOnHand: "desc" },
      });
    }
    return views;
  }
