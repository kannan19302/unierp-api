...
export class BudgetDeepService {
  ...
  async getBudgetTemplateById(tenantId: string, id: string) {
    const t = await prisma.budgetTemplate.findFirst({
      where: { id, tenantId },
    });
    ...
    return prisma.budgetTemplate.create({
      data: {
        ...
        adjustmentPct: dto.adjustmentPct
          ? new Prisma.Decimal(dto.adjustmentPct)
          : null,
      },
    });
  }
  ...
  async updateBudgetTemplate(tenantId: string, id: string, dto: any) {
    ...
    if (dto.adjustmentPct !== undefined)
      data.adjustmentPct = new Prisma.Decimal(dto.adjustmentPct);
  ...
  async generateBudgetsFromTemplate(
    tenantId: string,
    orgId: string,
    templateId: string,
  ) {
    ...
    const lines: any[] =
      typeof template.lines === "string"
        ? JSON.parse(template.lines)
        : (template.lines as any[]) || [];
  ...
  async getBudgetCommitmentById(tenantId: string, id: string) {
    const c = await prisma.budgetCommitment.findFirst({
      where: { id, tenantId },
    });
  ...
  async createBudgetCommitment(tenantId: string, orgId: string, dto: any) {
    return prisma.budgetCommitment.create({
      data: {
        ...
        expectedLiquidationDate: dto.expectedLiquidationDate
          ? new Date(dto.expectedLiquidationDate)
          : null,
      },
    });
  }
  ...
  async updateBudgetCommitment(tenantId: string, id: string, dto: any) {
    ...
    if (dto.outstandingAmount !== undefined)
      data.outstandingAmount = new Prisma.Decimal(dto.outstandingAmount);
    if (dto.liquidatedAmount !== undefined)
      data.liquidatedAmount = new Prisma.Decimal(dto.liquidatedAmount);
    return prisma.budgetCommitment.update({ where: { id }, data });
  }

  async liquidateBudgetCommitment(
    tenantId: string,
    id: string,
    ...
    const newLiquidated =
      Number(commitment.liquidatedAmount) + liquidatedAmount;
    const newOutstanding = Number(commitment.amount) - newLiquidated;
    if (newOutstanding < 0)
      throw new BadRequestException(
        "Liquidated amount exceeds commitment amount",
      );
  ...
  async getCommitmentSummary(
    ...
    const commitments = await prisma.budgetCommitment.findMany({
      where: { tenantId, orgId, budgetId },
    });
    const totalAmount = commitments.reduce((s, c) => s + Number(c.amount), 0);
    const totalOutstanding = commitments.reduce(
      (s, c) => s + Number(c.outstandingAmount),
      0,
    );
    const totalLiquidated = commitments.reduce(
      (s, c) => s + Number(c.liquidatedAmount),
      0,
    );
    return {
      budgetId,
      ...
      liquidatedCount: commitments.filter((c) => c.status === "LIQUIDATED")
        .length,
      cancelledCount: commitments.filter((c) => c.status === "CANCELLED")
        .length,
    };
  }
  ...
  async getCarryForwardRules(tenantId: string) {
    return prisma.budgetCarryForwardRule.findMany({
      where: { tenantId },
  ...
  async getCarryForwardRuleById(tenantId: string, id: string) {
    const r = await prisma.budgetCarryForwardRule.findFirst({
      where: { id, tenantId },
    });
  ...
  async createCarryForwardRule(tenantId: string, orgId: string, dto: any) {
    return prisma.budgetCarryForwardRule.create({
      data: {
        ...
        maxCarryAmount: dto.maxCarryAmount
          ? new Prisma.Decimal(dto.maxCarryAmount)
          : null,
  ...
  async updateCarryForwardRule(tenantId: string, id: string, dto: any) {
    ...
    const data: any = { ...dto };
    if (dto.carryForwardPct !== undefined)
      data.carryForwardPct = new Prisma.Decimal(dto.carryForwardPct);
    if (dto.maxCarryAmount !== undefined)
      data.maxCarryAmount = dto.maxCarryAmount
        ? new Prisma.Decimal(dto.maxCarryAmount)
        : null;
  ...
  async computeCarryForward(
    ...
    const rules = await prisma.budgetCarryForwardRule.findMany({
      where: { tenantId, orgId, isActive: true },
    });
    if (rules.length === 0)
      throw new BadRequestException("No active carry-forward rules found");
    ...
    for (const budget of budgets) {
      ...
      const carryAmount = Math.min(
        unused * Number(rule.carryForwardPct),
        Number(rule.maxCarryAmount || Infinity),
      );
      if (carryAmount > 0) {
        carryForwards.push({
          budgetId: budget.id,
    ...
    return {
      ...
      totalCarryForward: carryForwards.reduce((s, c) => s + c.carryAmount, 0),
      carryForwards,
    };
  }
  ...
  async getBudgetRevisions(tenantId: string, budgetId?: string) {
    const where: any = { tenantId };
    if (budgetId) where.budgetId = budgetId;
    return prisma.budgetRevision.findMany({
      where,
  ...
  async getBudgetRevisionById(tenantId: string, id: string) {
    const r = await prisma.budgetRevision.findFirst({
      where: { id, tenantId },
    });
  ...
  async createBudgetRevision(tenantId: string, orgId: string, dto: any) {
    const budget = await prisma.budget.findFirst({
      where: { id: dto.budgetId, tenantId },
    });
  ...
  async getBudgetDeepDashboard(tenantId: string, orgId: string) {
    ...
    const activeRules = await prisma.budgetCarryForwardRule.count({
      where: { tenantId, orgId, isActive: true },
    });
    return {
      ...
      totalCommitments: commitments.length,
      totalCommitmentAmount: commitments.reduce(
        (s, c) => s + Number(c.amount),
        0,
      ),
      outstandingCommitments: commitments
        .filter((c) => c.status === "ACTIVE")
        .reduce((s, c) => s + Number(c.outstandingAmount), 0),
