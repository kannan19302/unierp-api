...
describe("CustomerReturnsService", () => {
  ...
  it("approveRma transitions REQUESTED → APPROVED", async () => {
    ...
    vi.mocked(prisma.customerRma.update).mockResolvedValue({
      id: "rma1",
      status: "APPROVED",
    } as any);
  ...
  it("approveRma throws BadRequestException for non-REQUESTED RMA", async () => {
    ...
    await expect(svc.approveRma(T, "rma1", "u2")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("receiveRma throws BadRequestException for non-APPROVED RMA", async () => {
    ...
    await expect(svc.receiveRma(T, "rma1", { lines: [] })).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("receiveRma updates lines and transitions to RECEIVED", async () => {
    ...
    vi.mocked(prisma.customerRmaLine.update).mockResolvedValue({
      id: "l1",
    } as any);
    vi.mocked(prisma.customerRma.update).mockResolvedValue({
      id: "rma1",
  ...
  it("inspectLine sets disposition and auto-advances RMA when all lines inspected", async () => {
    ...
    const result = await svc.inspectLine(T, "rma1", "l1", {
      disposition: "RESTOCK",
      inspectedById: "u1",
    });
  ...
  it("issueCredit creates credit memo with auto-number", async () => {
    ...
    vi.mocked(prisma.returnCredit.create).mockImplementation(
      async ({ data }: any) => ({
        id: "cr1",
    ...
    const result = await svc.issueCredit(T, "rma1", {
      creditAmount: 250,
      issuedById: "u1",
    });
  ...
  it("issueCredit throws BadRequestException for zero/negative amount", async () => {
    ...
    await expect(
      svc.issueCredit(T, "rma1", { creditAmount: 0, issuedById: "u1" }),
  ...
  it("issueCredit throws BadRequestException when credit already exists", async () => {
    ...
    vi.mocked(prisma.returnCredit.findUnique).mockResolvedValue({
      id: "cr1",
    } as any);
    await expect(
      svc.issueCredit(T, "rma1", { creditAmount: 100, issuedById: "u1" }),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("voidCredit transitions ISSUED → VOIDED", async () => {
    ...
    vi.mocked(prisma.returnCredit.update).mockResolvedValue({
      id: "cr1",
      status: "VOIDED",
    } as any);
    const result = await svc.voidCredit(T, "cr1", {
      voidedById: "u1",
      voidReason: "Error",
    });
  ...
  it("restockLine creates restock record", async () => {
    ...
    vi.mocked(prisma.returnRestock.create).mockResolvedValue({
      id: "rs1",
      rmaLineId: "l1",
    } as any);
