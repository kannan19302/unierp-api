...
export class BlockchainController {
  ...
  async getDocumentBlockchainRecord(
    ...
    const record = await this.documentBlockchain.getDocumentBlockchainRecord(
      req.user.tenantId,
      documentId,
    );
    if (!record)
      throw new NotFoundException(
        `No blockchain record found for document ${documentId}`,
      );
    return record;
  }
  ...
  @ApiOperation({
    summary: "Verify GL journal entry integrity on the blockchain",
  })
  ...
  @ApiOperation({
    summary: "Get blockchain transaction record for a GL journal entry",
  })
  async getJournalBlockchainRecord(
    @Req() req: AuthenticatedRequest,
    @Param("journalId") journalId: string,
  ) {
    const record = await this.financeLedger.getJournalBlockchainRecord(
      req.user.tenantId,
      journalId,
    );
    if (!record)
      throw new NotFoundException(
        `No blockchain record found for journal ${journalId}`,
      );
    return record;
  }
  ...
  async getProvenance(
    ...
    const records = await this.supplyChain.getProvenance(
      req.user.tenantId,
      dto.productId,
      dto.batchId,
    );
  ...
  @ApiOperation({
    summary: "Execute automated 3-way PO/Receipt/Invoice match on blockchain",
  })
  async executeThreeWayMatch(
    @Req() req: AuthenticatedRequest,
    @Body() dto: ThreeWayMatchDto,
  ) {
    const result = await this.procurement.executeThreeWayMatch(
      req.user.tenantId,
      dto.poId,
    );
    return {
  ...
  @ApiOperation({
    summary: "Get the full blockchain history of a purchase order",
  })
  async getPurchaseOrderHistory(
    @Req() req: AuthenticatedRequest,
    @Param("poId") poId: string,
  ) {
    const history = await this.procurement.getPurchaseOrderHistory(
      req.user.tenantId,
      poId,
    );
    return { poId, history };
  ...
  async listTransactions(
    ...
    return this.documentBlockchain.listTransactions({
      tenantId: req.user.tenantId,
      entityType,
      status,
      page,
