...
import type { WriteEventParams } from "@kannan19302/shared";
...
describe("OutboxService (shared)", () => {
  ...
  beforeEach(() => {
    ...
    mockTx = {
      outboxEvent: {
        create: vi
          .fn()
          .mockResolvedValue({ id: "evt-1", eventKey: "test-key" }),
  ...
  it("should compute next sequence from aggregate", async () => {
    mockTx.outboxEvent.aggregate = vi
      .fn()
      .mockResolvedValue({ _max: { sequence: 5 } });
    ...
    expect(mockTx.outboxEvent.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          sequence: 6,
          eventKey: "SalesOrder:so-1:order.confirmed:6",
        }),
      }),
    );
  });
  ...
  it("should register and retrieve destinations", () => {
    ...
    expect(destinations).toEqual(
      expect.arrayContaining(["email", "analytics"]),
    );
  ...
  it("should not duplicate destinations on re-registration", () => {
    ...
    expect(service.getRegisteredDestinations("order.confirmed")).toEqual([
      "inventory",
    ]);
  });
});
