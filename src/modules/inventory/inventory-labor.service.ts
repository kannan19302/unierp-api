...
export type CreateLaborStandardInput = z.infer<
  typeof createLaborStandardSchema
>;
...
export class InventoryLaborService {
  ...
  async listLaborStandards(tenantId: string, warehouseId?: string) {
    ...
    return prisma.laborStandard.findMany({
      where,
  ...
  async createLaborStandard(tenantId: string, dto: CreateLaborStandardInput) {
    ...
    if (existing) {
      throw new BadRequestException(
        `Labor standard for task type '${dto.taskType}' already exists`,
      );
    }
  ...
  async updateLaborStandard(
    tenantId: string,
    id: string,
    dto: Partial<CreateLaborStandardInput>,
  ) {
    const record = await prisma.laborStandard.findFirst({
      where: { id, tenantId },
    });
  ...
  async completeTask(tenantId: string, id: string, dto: CompleteTaskInput) {
    const log = await prisma.workerTaskLog.findFirst({
      where: { id, tenantId },
    });
  ...
  async listShiftTemplates(tenantId: string, warehouseId?: string) {
    const where: Prisma.WarehouseShiftTemplateWhereInput = {
      tenantId,
      isActive: true,
    };
    if (warehouseId) where.warehouseId = warehouseId;
    return prisma.warehouseShiftTemplate.findMany({
      where,
  ...
  async deleteShiftTemplate(tenantId: string, id: string) {
    const record = await prisma.warehouseShiftTemplate.findFirst({
      where: { id, tenantId },
    });
    if (!record) throw new NotFoundException("Shift template not found");
    return prisma.warehouseShiftTemplate.update({
      where: { id },
      data: { isActive: false },
    });
  }
  ...
  async getLaborDashboard(tenantId: string, warehouseId?: string) {
    ...
    const [totalTasks, completedTasks, byTaskType, topWorkers] =
      await Promise.all([
        prisma.workerTaskLog.count({ where: recentWhere }),
        prisma.workerTaskLog.count({
