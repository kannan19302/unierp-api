...
describe("CommunicationService (Connect)", () => {
  ...
  it("serializes messages with grouped reactions and deleted tombstones", async () => {
    ...
    expect(res[0]?.reactions[0]).toEqual({
      emoji: "👍",
      userIds: ["u1", "u2"],
    });
  ...
  it("adds a reaction when none exists", async () => {
    ...
    vi.mocked(prisma.messageReaction.findFirst).mockResolvedValue(
      null as never,
    );
    vi.mocked(prisma.messageReaction.findMany).mockResolvedValue([
      { emoji: "🎉", userId: "u1" },
    ] as never);
  ...
  it("creates a meeting and posts a system message into the conversation", async () => {
    ...
    const res = await svc.createMeeting("t1", "u1", {
      title: "Standup",
      conversationId: "c1",
    });
