...
export class AddonsController {
  ...
  @Get()
  async listAddons() {
    return this.saasService.db.saaSAddOn.findMany({
      where: { status: "ACTIVE" },
      orderBy: { name: "asc" },
    });
  }
  ...
  async purchaseAddon(
    ...
    @ZodBody(purchaseAddonSchema) body: z.infer<typeof purchaseAddonSchema>,
  ) {
    const addon = await this.saasService.db.saaSAddOn.findUnique({
      where: { id: addonId },
    });
    if (!addon) throw new NotFoundException("Addon not found");
    return this.saasService.db.tenantAddOn.upsert({
      where: { tenantId_addonId: { tenantId: req.user.tenantId, addonId } },
      update: { quantity: body.quantity, status: "ACTIVE" },
      create: { tenantId: req.user.tenantId, addonId, quantity: body.quantity },
    });
  }
  ...
  async cancelAddon(@Req() req: AuthReq, @Param("addonId") addonId: string) {
    return this.saasService.db.tenantAddOn.deleteMany({
      where: { tenantId: req.user.tenantId, addonId },
    });
  }
}
