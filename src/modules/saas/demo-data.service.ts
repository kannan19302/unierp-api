...
/**
 * Seeds evaluation data for a new tenant.
 *
 * This lived in the auth module and was deleted during the platform split
 * without being moved, taking `POST /onboarding/seed-demo` with it. The only
 * surviving seed endpoint (`POST /onboarding/demo-data` in this module) had
 * been reduced to flipping `demoDataLoaded: true` and replying "Demo data
 * seeded successfully" while creating nothing — it reported success for work
 * that never happened.
 *
 * It is restored here rather than in the IdP: it writes customers, vendors and
 * products, which are business records owned by the API, not identity data.
 */
@Injectable()
