...
export class BlockchainDeepService {
  ...
  async getTransaction(tenantId: string, id: string) {
    const tx = await prisma.blockchainTransactionExplorer.findFirst({
      where: { id, tenantId },
    });
  ...
  async listContracts(tenantId: string) {
    return prisma.blockchainSmartContract.findMany({
      where: { tenantId },
  ...
  async updateContract(
    ...
    const contract = await prisma.blockchainSmartContract.findFirst({
      where: { id, tenantId },
    });
  ...
  async listAuditTrails(
    ...
    return runWithTenantSession({ tenantId, userId: "" }, async () => {
      const [items, total] = await Promise.all([
        prisma.blockchainAuditTrail.findMany({
          where,
          orderBy: { timestamp: "desc" },
          skip: (page - 1) * limit,
          take: limit,
  ...
  async upsertNetworkHealth(data: {
    network: string;
    blockHeight: number;
    peers: number;
    syncStatus: string;
