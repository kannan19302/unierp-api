...
describe("CrmContractLifecycleService", () => {
  ...
  describe("amendments", () => {
    it("listAmendments returns amendments", async () => {
      vi.mocked(prisma.contractAmendment.findMany).mockResolvedValue([
        { id: "a1" },
      ] as any);
      const r = await service.listAmendments(TENANT, "c1");
      expect(r).toHaveLength(1);
      expect(prisma.contractAmendment.findMany).toHaveBeenCalledWith(
    ...
    it("getAmendment throws if missing", async () => {
      ...
      await expect(service.getAmendment(TENANT, "bad")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("createAmendment throws if contract missing", async () => {
      ...
      const dto = {
        contractId: "bad",
        title: "Test",
        amendmentType: "SCOPE_CHANGE" as const,
        effectiveDate: new Date(),
      };
      await expect(
        service.createAmendment(TENANT, dto as any, "user1"),
    ...
    it("createAmendment creates with amendment number", async () => {
      ...
      vi.mocked(prisma.contractAmendment.create).mockResolvedValue({
        id: "a1",
      } as any);
      const dto = {
        contractId: "c1",
        title: "Test AMD",
        amendmentType: "PRICE_CHANGE" as const,
        effectiveDate: new Date(),
        newValue: 10000,
      };
    ...
    it("submitAmendmentForApproval throws if not DRAFT", async () => {
      vi.mocked(prisma.contractAmendment.findFirst).mockResolvedValue({
        id: "a1",
        status: "APPROVED",
      } as any);
      await expect(
        service.submitAmendmentForApproval(TENANT, "a1"),
  ...
  describe("price escalation", () => {
    it("createPriceEscalationRule creates rule", async () => {
      ...
      vi.mocked(prisma.contractPriceEscalationRule.create).mockResolvedValue({
        id: "r1",
      } as any);
    ...
    it("applyEscalation applies percentage increase", async () => {
      vi.mocked(prisma.contractPriceEscalationRule.findFirst).mockResolvedValue(
        {
          id: "r1",
      ...
      vi.mocked(prisma.contractPriceEscalationRule.update).mockResolvedValue(
        {} as any,
      );
      await service.applyEscalation(TENANT, "r1", "c1");
      expect(prisma.contract.update).toHaveBeenCalledWith(
        expect.objectContaining({ data: { value: 1100 } }),
      );
    });
  });
  ...
  describe("auto-renewals", () => {
    it("processAutoRenewal throws if contract missing", async () => {
      vi.mocked(prisma.contract.findFirst).mockResolvedValue(null);
      await expect(service.processAutoRenewal(TENANT, "bad")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("processAutoRenewal processes renewal", async () => {
      ...
      vi.mocked(prisma.contractAutoRenewalLog.create).mockResolvedValue({
        id: "log1",
      } as any);
  ...
  describe("expiration pipeline", () => {
    it("scanExpirationPipeline creates items for near-expiry contracts", async () => {
      ...
      vi.mocked(prisma.contract.findMany).mockResolvedValue([
        { id: "c1", endDate: nearEnd, contractNumber: "CN-001", title: "Test" },
      ] as any);
      vi.mocked(
        prisma.contractExpirationPipelineItem.findFirst,
      ).mockResolvedValue(null);
      vi.mocked(prisma.contractExpirationPipelineItem.create).mockResolvedValue(
        {} as any,
      );
  ...
  describe("templates", () => {
    it("createTemplate creates template", async () => {
      ...
      vi.mocked(prisma.contractTemplate.create).mockResolvedValue({
        id: "t1",
      } as any);
  ...
  describe("analytics", () => {
    it("getAnalytics returns computed metrics", async () => {
      ...
      vi.mocked(prisma.contractExpirationPipelineItem.count).mockResolvedValue(
        10,
      );
