...
export class FreightClaimsService {
  ...
  async createDamageReport(
    ...
    return prisma.cargoDamageReport.create({
      data: {
        ...
        affectedSkus: dto.affectedSkus,
        quantityDamaged:
          dto.quantityDamaged != null ? dto.quantityDamaged : undefined,
        estimatedLoss:
          dto.estimatedLoss != null ? dto.estimatedLoss : undefined,
  ...
  async listDamageReports(
    ...
    const [items, total] = await Promise.all([
      prisma.cargoDamageReport.findMany({
        where,
        skip: params.skip ?? 0,
        take: params.take ?? 20,
  ...
  async getDamageReport(tenantId: string, id: string) {
    const report = await prisma.cargoDamageReport.findFirst({
      where: { tenantId, id },
    });
  ...
  async reviewDamageReport(
    ...
    id: string,
    userId: string,
    decision: "UNDER_REVIEW" | "RESOLVED" | "CLOSED",
  ) {
    const report = await prisma.cargoDamageReport.findFirst({
      where: { tenantId, id },
    });
  ...
  async fileClaim(
    ...
    const report = await prisma.cargoDamageReport.findFirst({
      where: { tenantId, id: dto.damageReportId },
    });
    if (!report) throw new NotFoundException("Damage report not found");
    const existing = await prisma.freightClaim.findUnique({
      where: { damageReportId: dto.damageReportId },
    });
    if (existing)
      throw new BadRequestException(
        "A claim already exists for this damage report",
      );
    if (dto.claimedAmount <= 0)
      throw new BadRequestException("Claimed amount must be positive");
    ...
    await prisma.cargoDamageReport.update({
  ...
  async getClaim(tenantId: string, id: string) {
    const claim = await prisma.freightClaim.findFirst({
      where: { tenantId, id },
    });
  ...
  async updateClaimStatus(
    ...
    if (!allowed.includes(dto.status)) {
      throw new BadRequestException(
        `Cannot transition from ${claim.status} to ${dto.status}`,
      );
    }
    ...
    if (dto.carrierRefNumber)
      updateData.carrierRefNumber = dto.carrierRefNumber;
    if (dto.status === "ACCEPTED" || dto.status === "SETTLEMENT_OFFERED") {
      if (dto.settlementAmount != null)
        updateData.settlementAmount = dto.settlementAmount;
  ...
  async getDashboard(tenantId: string) {
    const [
      ...
      prisma.cargoDamageReport.count({
      ...
      prisma.freightClaim.count({
        where: { tenantId, status: "UNDER_INVESTIGATION" },
      }),
      prisma.freightClaim.count({
        where: { tenantId, status: "SETTLEMENT_OFFERED" },
      }),
    ]);
    return {
      totalReports,
