...
describe("InventoryService — batch quarantine, stock reservations, analytics", () => {
  ...
  describe("batch quarantine workflow", () => {
    it("quarantines an active batch and logs the action", async () => {
      ...
      const result = await service.quarantineBatch("t1", "b1", "u1", {
    ...
    it("rejects quarantining an already-quarantined batch", async () => {
      ...
      await expect(
        service.quarantineBatch("t1", "b1", "u1", { reason: "x" }),
    ...
    it("rejects releasing a non-quarantined batch", async () => {
      db.batch.findFirst.mockResolvedValue({ id: "b1", status: "ACTIVE" });
      await expect(
        service.releaseBatchQuarantine("t1", "b1", "u1", {}),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("traceability", () => {
    it("throws NotFound tracing an unknown serial number", async () => {
      ...
      await expect(
        service.getSerialNumberTrace("t1", "missing"),
  ...
  describe("stock reservations", () => {
    it("rejects reserving more than the available quantity", async () => {
      ...
      db.stockReservation.aggregate.mockResolvedValue({
        _sum: { quantity: 8 },
      });
      await expect(
        service.createStockReservation("t1", "u1", {
          productId: "p1",
          warehouseId: "w1",
          quantity: 5,
          sourceType: "MANUAL",
    ...
    it("creates a reservation when enough stock is available", async () => {
      ...
      db.stockReservation.create.mockResolvedValue({
        id: "r1",
        status: "ACTIVE",
      });
      const result = await service.createStockReservation("t1", "u1", {
        productId: "p1",
        warehouseId: "w1",
        quantity: 5,
        sourceType: "MANUAL",
    ...
    it("rejects releasing a non-active reservation", async () => {
      db.stockReservation.findFirst.mockResolvedValue({
        id: "r1",
        status: "RELEASED",
      });
      await expect(service.releaseStockReservation("t1", "r1")).rejects.toThrow(
        BadRequestException,
      );
    });
      ...
      db.inventoryItem.findMany.mockResolvedValue([
        {
          productId: "p1",
          quantity: 1,
          product: { name: "High Value", sku: "HV", costPrice: 80 },
        },
        {
          productId: "p2",
          quantity: 1,
          product: { name: "Mid Value", sku: "MV", costPrice: 15 },
        },
        {
          productId: "p3",
          quantity: 1,
          product: { name: "Low Value", sku: "LV", costPrice: 5 },
        },
      ]);
      ...
      db.inventoryItem.findMany.mockResolvedValue([
        {
          ...
          warehouseId: "w1",
          quantity: 20,
          product: { name: "Stale Item", sku: "SI", costPrice: 10 },
        },
      ]);
      ...
      db.inventoryItem.findMany.mockResolvedValue([
        {
          ...
          warehouseId: "w1",
          quantity: 0,
          product: { name: "Empty", sku: "E", costPrice: 10 },
        },
      ]);
      const result = await service.getDeadStockReport("t1");
      ...
      db.inventoryItem.findMany.mockResolvedValue([
        {
          productId: "p1",
          quantity: 20,
          product: { name: "Fast Mover", sku: "FM" },
        },
      ]);
