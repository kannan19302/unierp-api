...
describe("SalesSpiffService", () => {
  ...
  beforeEach(() => {
    service = new SalesSpiffService();
    vi.clearAllMocks();
  });

  it("should list SPIFF campaigns", async () => {
    vi.mocked(prisma.commissionSpiff.findMany).mockResolvedValue([
      { id: "spiff-1", name: "Q4 Bonus" },
    ] as never);
  ...
  it("should create SPIFF campaign", async () => {
    ...
    vi.mocked(prisma.commissionSpiff.create).mockResolvedValue({
      id: "spiff-1",
      name: "New Deal Bonus",
    } as never);
    const dto = {
      name: "New Deal Bonus",
      criteriaType: "DEAL_SIZE_ABOVE",
      criteriaValue: { threshold: 50000 },
      bonusType: "FLAT",
      bonusAmount: 1000,
    };
  ...
  it("should update SPIFF campaign", async () => {
    vi.mocked(prisma.commissionSpiff.findFirst).mockResolvedValue({
      id: "spiff-1",
    } as never);
    vi.mocked(prisma.commissionSpiff.update).mockResolvedValue({
      id: "spiff-1",
      name: "Updated",
    } as never);
    const result = await service.updateSpiffCampaign("tenant-1", "spiff-1", {
      name: "Updated",
    });
  ...
  it("should delete SPIFF campaign", async () => {
    ...
    vi.mocked(prisma.commissionSpiff.delete).mockResolvedValue({
      id: "spiff-1",
    } as never);
  ...
  it("should create team split", async () => {
    ...
    vi.mocked(prisma.teamSplit.create).mockResolvedValue({
      id: "ts-1",
      name: "50/50 Split",
      members: [],
    } as never);
  ...
  it("should process clawback", async () => {
    ...
    const result = await service.processClawback("tenant-1", {
      payoutId: "po-1",
      amount: 1000,
