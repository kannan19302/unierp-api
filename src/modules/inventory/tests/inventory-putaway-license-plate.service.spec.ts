...
describe("InventoryService — cycle count schedules, license plates, directed putaway", () => {
  ...
  describe("cycle count schedules", () => {
    it("creates a schedule with a parsed due date", async () => {
      ...
      expect(db.cycleCountSchedule.create).toHaveBeenCalledWith(
    ...
    it("throws NotFound updating a schedule that does not belong to the tenant", async () => {
      ...
      await expect(
        service.updateCycleCountSchedule("t1", "missing", {} as any),
      ).rejects.toThrow(NotFoundException);
    });

    it("rolls a MONTHLY schedule forward by 30 days", async () => {
      const due = new Date("2026-07-01T00:00:00.000Z");
      db.cycleCountSchedule.findFirst.mockResolvedValue({
        id: "s1",
        frequency: "MONTHLY",
        nextDueDate: due,
      });
      db.cycleCountSchedule.update.mockImplementation(({ data }: any) =>
    ...
    it("computes accuracy rate from counted items with zero variance", async () => {
      db.cycleCount.findMany.mockResolvedValue([
        {
          items: [
            { countedQty: 10, varianceQty: 0 },
            { countedQty: 8, varianceQty: -2 },
          ],
        },
  ...
  describe("license plates", () => {
    ...
    it("creates an OPEN license plate", async () => {
      ...
      const result = await service.createLicensePlate("t1", {
        code: "LP-2",
        warehouseId: "w1",
      } as any);
    ...
    it("rejects adding items to a closed license plate", async () => {
      ...
      await expect(
        service.addLicensePlateItem("t1", "lp1", {
          inventoryItemId: "i1",
          quantity: 5,
        } as any),
      ).rejects.toThrow(BadRequestException);
    ...
    it("rejects moving a consumed license plate", async () => {
      ...
      await expect(
        service.moveLicensePlate("t1", "lp1", { binId: "b2" } as any),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe("directed put-away", () => {
    ...
    it("throws NotFound suggesting a bin for an unknown item", async () => {
      ...
      await expect(service.suggestPutawayBin("t1", "missing")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("creates a putaway task and auto-suggests a bin when none is provided", async () => {
      ...
      db.binLocation.findMany.mockResolvedValue([
        { id: "b1", capacity: null, _count: { inventoryBins: 0 } },
      ]);
      db.putawayTask.create.mockResolvedValue({
        id: "pt1",
        suggestedBinId: "b1",
      });
      const result = await service.createPutawayTask("t1", {
        stockEntryId: "se1",
        inventoryItemId: "i1",
        quantity: 3,
      } as any);
    ...
    it("rejects completing an already-complete putaway task", async () => {
      db.putawayTask.findFirst.mockResolvedValue({
        id: "pt1",
        status: "COMPLETE",
      });
      await expect(
        service.completePutawayTask("t1", "pt1", {}),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("completes a pending putaway task", async () => {
      ...
      db.putawayTask.update.mockResolvedValue({
        id: "pt1",
        status: "COMPLETE",
      });
