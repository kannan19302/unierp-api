...
export class FabricGatewayProvider {
  ...
  getNetwork(channelName: string): Network | null {
    if (!this.isEnabled || !this.connectionService || !this.isConnected())
      return null;
    try {
      return this.connectionService.getNetwork(channelName);
    } catch (err) {
      this.logger.warn(
        `Failed to get network ${channelName}: ${(err as Error).message}`,
      );
      return null;
    }
  }
  ...
  getContract(chaincodeName: string): Contract | null {
    ...
    try {
      const channelName =
        CHAINCODE_CHANNEL_MAP[
          chaincodeName as keyof typeof CHAINCODE_CHANNEL_MAP
        ];
      if (!channelName) {
      ...
      this.logger.warn(
        `Failed to get contract ${chaincodeName}: ${(err as Error).message}`,
      );
      return null;
    }
  }
