...
describe("PackagingGs1Service", () => {
  ...
  it("upsertSpec creates or updates packaging spec", async () => {
    ...
    const result = await svc.upsertSpec(T, {
      productId: "p1",
      level: "EACH",
      unitsPerLevel: 1,
    });
  ...
  it("deactivateSpec marks spec inactive", async () => {
    ...
    vi.mocked(prisma.packagingSpec.update).mockResolvedValue({
      id: "s1",
      active: false,
    } as any);
  ...
  it("addBarcode clears previous primary when isPrimary=true", async () => {
    ...
    vi.mocked(prisma.itemBarcode.create).mockResolvedValue({
      id: "b2",
      isPrimary: true,
    } as any);
  ...
  it("upsertGs1Ai creates new AI", async () => {
    ...
    const result = await svc.upsertGs1Ai(T, {
      ai: "01",
      title: "GTIN",
      dataFormat: "n14",
    });
  ...
  it("assignLabelToSpec clears previous default before creating new one", async () => {
    ...
    vi.mocked(prisma.labelAssignment.create).mockResolvedValue({
      id: "la1",
      isDefault: true,
    } as any);
    const result = await svc.assignLabelToSpec(T, {
      packagingSpecId: "s1",
      templateId: "tpl1",
      isDefault: true,
    });
  ...
  it("getDashboard returns packaging and GS1 aggregates", async () => {
    ...
    vi.mocked(prisma.ssccRecord.count)
