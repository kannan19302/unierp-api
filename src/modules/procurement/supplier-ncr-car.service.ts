...
export class SupplierNcrCarService {
  async listNcrs(
    ...
    if (params.search) {
      where.OR = [
        { ncrNumber: { contains: params.search, mode: "insensitive" } },
    ...
    const [items, total] = await Promise.all([
      prisma.supplierNcr.findMany({
        ...
        include: {
          ...
          carRequests: { select: { id: true, carNumber: true, status: true } },
        },
      }),
  ...
  async getNcrById(tenantId: string, id: string) {
    const ncr = await prisma.supplierNcr.findFirst({
      ...
      include: {
        vendor: true,
        carRequests: { include: { vendor: { select: { name: true } } } },
      },
    });
  ...
  async updateNcrStatus(
    ...
    const validTransitions: Record<string, string[]> = {
      ...
      UNDER_REVIEW: ["CAR_RAISED", "CLOSED"],
      CAR_RAISED: ["CLOSED"],
      CLOSED: ["OPEN"],
    };
    ...
    if (!allowed || !allowed.includes(status)) {
      throw new BadRequestException(
        `Cannot transition NCR from ${ncr.status} to ${status}`,
      );
    }
  ...
  async listCars(
    ...
    const [items, total] = await Promise.all([
      prisma.supplierCarRequest.findMany({
        where,
        skip,
        take,
        orderBy: orderBy as any,
        include: {
          vendor: { select: { name: true } },
          ncr: { select: { ncrNumber: true, defectType: true } },
        },
      }),
  ...
  async createCarFromNcr(
    tenantId: string,
    ncrId: string,
    dto: { rootCause: string; correctiveAction: string; dueDate?: string },
  ) {
    const ncr = await prisma.supplierNcr.findFirst({
    ...
    const carCount = await prisma.supplierCarRequest.count({
      where: { tenantId },
    });
    const carNumber = `CAR-${String(carCount + 1).padStart(5, "0")}`;

    await prisma.supplierNcr.update({
      where: { id: ncrId },
  ...
  async updateCarStatus(
    ...
    if (!allowed || !allowed.includes(status)) {
      throw new BadRequestException(
        `Cannot transition CAR from ${car.status} to ${status}`,
      );
    }

    const updateData: any = { status };
    if (status === "SUBMITTED" && vendorResponse) {
  ...
  async getStats(tenantId: string) {
    ...
    const cars = await prisma.supplierCarRequest.findMany({
      where: { tenantId },
    });
    return {
      totalNcrs: ncrs.length,
      ncrByStatus: ncrs.reduce((acc: Record<string, number>, n) => {
        acc[n.status] = (acc[n.status] || 0) + 1;
        return acc;
      }, {}),
      ncrBySeverity: ncrs.reduce((acc: Record<string, number>, n) => {
        acc[n.severity] = (acc[n.severity] || 0) + 1;
        return acc;
      }, {}),
      totalCars: cars.length,
      carByStatus: cars.reduce((acc: Record<string, number>, c) => {
        acc[c.status] = (acc[c.status] || 0) + 1;
        return acc;
      }, {}),
      openCritical: ncrs.filter(
        (n) => n.severity === "CRITICAL" && n.status !== "CLOSED",
      ).length,
    };
  }

  async getNcrsBySupplier(
    tenantId: string,
    vendorId: string,
  ...
  async getCarsBySupplier(
    ...
    const [items, total] = await Promise.all([
      prisma.supplierCarRequest.findMany({
        ...
        take,
        orderBy: orderBy as any,
        include: { ncr: { select: { ncrNumber: true, defectType: true } } },
      }),
      prisma.supplierCarRequest.count({ where }),
    ]);
    return paginatedResult(items, total, params);
  ...
  async escalateNcr(
    tenantId: string,
    id: string,
    dto: { severity: string; escalationNotes?: string },
  ) {
    const ncr = await prisma.supplierNcr.findFirst({ where: { id, tenantId } });
    if (!ncr) throw new NotFoundException("NCR not found");
    if (ncr.status === "CLOSED")
      throw new BadRequestException("Cannot escalate a closed NCR");
    ...
    if (newIdx <= currentIdx)
      throw new BadRequestException("New severity must be higher than current");

    return prisma.supplierNcr.update({
      where: { id },
      data: {
        severity: dto.severity,
        status: ncr.status === "CLOSED" ? "UNDER_REVIEW" : undefined,
      },
      include: { vendor: { select: { name: true } } },
    });
  }
  ...
  async reopenCar(tenantId: string, id: string, reason?: string) {
    ...
    if (car.status !== "CLOSED")
      throw new BadRequestException("Only closed CARs can be reopened");
  ...
  async getQualityMetrics(
    ...
    return {
      ...
      criticalOpen: ncrs.filter(
        (n) => n.severity === "CRITICAL" && n.status !== "CLOSED",
      ).length,
      byDefectType: Object.entries(byDefectType).map(([defectType, data]) => ({
        defectType,
        ...data,
      })),
      bySeverity: ncrs.reduce((acc: Record<string, number>, n) => {
        acc[n.severity] = (acc[n.severity] || 0) + 1;
        return acc;
      }, {}),
      byVendor: ncrs.reduce(
        (acc: Record<string, { vendorName: string; count: number }>, n) => {
