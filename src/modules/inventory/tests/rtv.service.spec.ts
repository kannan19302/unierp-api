...
describe("RtvService", () => {
  ...
  describe("createReasonCode", () => {
    it("creates a new reason code", async () => {
      ...
      const result = await service.createReasonCode("t-1", {
        code: "defective",
        name: "Defective item",
      });
  ...
  describe("createRmaRequest", () => {
    it("creates an RMA request and emits event", async () => {
      ...
      expect(emitter.emit).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ rmaId: "rma-1" }),
      );
    });
  });
  ...
  describe("submitRmaRequest", () => {
    it("submits a PENDING RMA", async () => {
      ...
      vi.mocked(prisma.vendorRmaRequest.update).mockResolvedValue({
        id: "rma-1",
        status: "SUBMITTED",
      } as never);
    ...
    it("throws if RMA is not PENDING", async () => {
      ...
      await expect(service.submitRmaRequest("t-1", "rma-1")).rejects.toThrow(
        "cannot submit",
      );
    });
  });
  ...
  describe("rejectRmaRequest", () => {
    it("rejects a SUBMITTED RMA with a reason", async () => {
      ...
      const result = await service.rejectRmaRequest("t-1", "rma-1", {
  ...
  describe("createShipment", () => {
    it("creates a shipment for an AUTHORIZED RMA", async () => {
      ...
      vi.mocked(prisma.vendorReturnShipment.create).mockResolvedValue({
        id: "ship-1",
        status: "PENDING",
      } as never);
  ...
  describe("shipment lifecycle", () => {
    it("advances pack → ship → deliver in order", async () => {
      ...
      vi.mocked(prisma.vendorReturnShipment.update).mockResolvedValue({
        id: "ship-1",
        status: "PACKED",
      } as never);
    ...
    it("throws if pack attempted on non-PENDING shipment", async () => {
      ...
      await expect(service.packShipment("t-1", "ship-1")).rejects.toThrow(
        "cannot pack",
      );
    });
  });
  ...
  describe("getRtvDashboard", () => {
    it("returns aggregate counts and credit total", async () => {
      ...
      vi.mocked(prisma.vendorReturnShipment.aggregate).mockResolvedValue({
        _sum: { creditAmount: 500 },
      } as never);
