...
export class FinancialInstrumentsService {
  ...
  async updateFairValueMeasurement(tenantId: string, id: string, dto: any) {
    ...
    const newFairValue =
      dto.fairValue !== undefined
        ? Number(dto.fairValue)
        : Number(existing.fairValue);
    const newCostBasis =
      dto.costBasis !== undefined
        ? Number(dto.costBasis)
        : Number(existing.costBasis);
    ...
    if (dto.valuationDate !== undefined)
      data.valuationDate = dto.valuationDate
        ? new Date(dto.valuationDate)
        : null;
  ...
  async getMeasurementsByInstrument(
    tenantId: string,
  ...
  async getLatestMeasurement(
    ...
    if (!m)
      throw new NotFoundException("No measurement found for this instrument");
    return m;
  }
  ...
  async getFairValueHierarchySummary(tenantId: string, orgId: string) {
    const measurements = await prisma.fairValueMeasurement.findMany({
      where: { tenantId, orgId },
    });
    const level1 = measurements
      .filter((m) => m.hierarchyLevel === "LEVEL_1")
      .reduce((s, m) => s + Number(m.fairValue), 0);
    const level2 = measurements
      .filter((m) => m.hierarchyLevel === "LEVEL_2")
      .reduce((s, m) => s + Number(m.fairValue), 0);
    const level3 = measurements
      .filter((m) => m.hierarchyLevel === "LEVEL_3")
      .reduce((s, m) => s + Number(m.fairValue), 0);
    const totalUnrealized = measurements.reduce(
      (s, m) => s + Number(m.unrealizedGL),
      0,
    );
    return {
      totalFairValue: level1 + level2 + level3,
      level1,
      level2,
      level3,
      totalUnrealizedGL: totalUnrealized,
  ...
  async getExpectedCreditLossProvisions(
    tenantId: string,
    period?: string,
    stage?: string,
  ) {
  ...
  async getExpectedCreditLossProvisionById(tenantId: string, id: string) {
    const p = await prisma.expectedCreditLossProvision.findFirst({
      where: { id, tenantId },
    });
    if (!p)
      throw new NotFoundException("Expected credit loss provision not found");
    return p;
  }

  async createExpectedCreditLossProvision(
    tenantId: string,
    orgId: string,
    dto: any,
  ) {
    ...
    return prisma.expectedCreditLossProvision.create({
      data: {
        ...
        probabilityDefault: dto.probabilityDefault
          ? new Prisma.Decimal(dto.probabilityDefault)
          : null,
        lossGivenDefault: dto.lossGivenDefault
          ? new Prisma.Decimal(dto.lossGivenDefault)
          : null,
        exposureAtDefault: dto.exposureAtDefault
          ? new Prisma.Decimal(dto.exposureAtDefault)
          : null,
      },
    });
  }

  async updateExpectedCreditLossProvision(
    tenantId: string,
    id: string,
    dto: any,
  ) {
    const existing = await this.getExpectedCreditLossProvisionById(
      tenantId,
      id,
    );
    const data: any = { ...dto };
    const grossAmount =
      dto.grossCarryingAmount !== undefined
        ? Number(dto.grossCarryingAmount)
        : Number(existing.grossCarryingAmount);
    const lossRate =
      dto.lossRate !== undefined
        ? Number(dto.lossRate)
        : Number(existing.lossRate);
    const expectedLoss = grossAmount * lossRate;
    ...
    if (dto.grossCarryingAmount !== undefined)
      data.grossCarryingAmount = new Prisma.Decimal(dto.grossCarryingAmount);
    if (dto.lossRate !== undefined)
      data.lossRate = new Prisma.Decimal(dto.lossRate);
    if (dto.probabilityDefault !== undefined)
      data.probabilityDefault = new Prisma.Decimal(dto.probabilityDefault);
    if (dto.lossGivenDefault !== undefined)
      data.lossGivenDefault = new Prisma.Decimal(dto.lossGivenDefault);
    if (dto.exposureAtDefault !== undefined)
      data.exposureAtDefault = new Prisma.Decimal(dto.exposureAtDefault);
  ...
  async reviewExpectedCreditLossProvision(
    tenantId: string,
    id: string,
    reviewedBy: string,
  ) {
    ...
    if (prov.status !== "DRAFT")
      throw new BadRequestException("Only draft provisions can be reviewed");
  ...
  async approveExpectedCreditLossProvision(
    tenantId: string,
    id: string,
    approvedBy: string,
  ) {
    const prov = await this.getExpectedCreditLossProvisionById(tenantId, id);
    if (prov.status !== "REVIEWED")
      throw new BadRequestException("Provision must be reviewed first");
    return prisma.expectedCreditLossProvision.update({
      where: { id },
  ...
  async postExpectedCreditLossProvision(tenantId: string, id: string) {
    ...
    if (prov.status !== "APPROVED")
      throw new BadRequestException("Provision must be approved first");
    return prisma.expectedCreditLossProvision.update({
      where: { id },
      data: { status: "POSTED" },
    });
  ...
  async getECLSummary(tenantId: string, orgId: string, period: string) {
    ...
    return {
      period,
      totalGrossExposure: provisions.reduce(
        (s, p) => s + Number(p.grossCarryingAmount),
        0,
      ),
      totalExpectedLoss: provisions.reduce(
        (s, p) => s + Number(p.expectedLoss),
        0,
      ),
      totalAllowance: provisions.reduce(
        (s, p) => s + Number(p.allowanceBalance),
        0,
      ),
      totalChargeToPL: provisions.reduce((s, p) => s + Number(p.chargeToPL), 0),
      stage1: {
        count: stage1.length,
        expectedLoss: stage1.reduce((s, p) => s + Number(p.expectedLoss), 0),
      },
      stage2: {
        count: stage2.length,
        expectedLoss: stage2.reduce((s, p) => s + Number(p.expectedLoss), 0),
      },
      stage3: {
        count: stage3.length,
        expectedLoss: stage3.reduce((s, p) => s + Number(p.expectedLoss), 0),
      },
  ...
  async getECLRollForward(tenantId: string, orgId: string, period: string) {
    ...
    const openingAllowance = provisions.reduce(
      (s, p) => s + Number(p.previousAllowance),
      0,
    );
    const newProvisions = provisions
      .filter((p) => Number(p.chargeToPL) > 0)
      .reduce((s, p) => s + Number(p.chargeToPL), 0);
    const releases = provisions
      .filter((p) => Number(p.chargeToPL) < 0)
      .reduce((s, p) => s + Math.abs(Number(p.chargeToPL)), 0);
    const closingAllowance = provisions.reduce(
      (s, p) => s + Number(p.allowanceBalance),
      0,
    );
    return {
  ...
  async computeLossRate(tenantId: string, orgId: string, portfolio: string) {
    ...
    if (provisions.length === 0)
      throw new NotFoundException("No posted provisions found for portfolio");
    const avgLossRate =
      provisions.length > 0
        ? provisions.reduce((s, p) => s + Number(p.lossRate), 0) /
          provisions.length
        : 0;
    ...
    return {
      portfolio,
      periodsAnalyzed: provisions.length,
      averageLossRate: avgLossRate,
      minRate,
      maxRate,
      currentRate: Number(latest.lossRate),
    };
  }
  ...
  async getAllECLDashboard(tenantId: string, orgId: string) {
    ...
    const totalAllowance = provisions.reduce(
      (s, p) => s + Number(p.allowanceBalance),
      0,
    );
    ...
    const portfolios = [
      ...new Set(provisions.map((p) => p.portfolio).filter(Boolean)),
    ];
    const byPortfolio = portfolios.map((port) => {
      ...
      return {
        portfolio: port,
        count: p.length,
        totalAllowance: p.reduce((s, pr) => s + Number(pr.allowanceBalance), 0),
      };
    });
    return {
  ...
  async getFinancialInstrumentsDashboard(tenantId: string, orgId: string) {
    const fvMeasurements = await prisma.fairValueMeasurement.findMany({
      where: { tenantId, orgId },
    });
    const eclProvisions = await prisma.expectedCreditLossProvision.findMany({
      where: { tenantId, orgId },
    });
    return {
      fairValueCount: fvMeasurements.length,
      totalFairValue: fvMeasurements.reduce(
        (s, m) => s + Number(m.fairValue),
        0,
      ),
      totalUnrealizedGL: fvMeasurements.reduce(
        (s, m) => s + Number(m.unrealizedGL),
        0,
      ),
      eclCount: eclProvisions.length,
      totalECLAllowance: eclProvisions.reduce(
        (s, p) => s + Number(p.allowanceBalance),
        0,
      ),
      totalChargeToPL: eclProvisions.reduce(
        (s, p) => s + Number(p.chargeToPL),
        0,
      ),
    };
  }
