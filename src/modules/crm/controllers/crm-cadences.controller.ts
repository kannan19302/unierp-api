...
} from "../services/crm-cadences.service";
...
export class CrmCadencesController {
  ...
  async create(
    ...
    @ZodBody(createCadenceSchema) dto: CreateCadenceInput,
  ) {
  ...
  async createRule(
    @Req() req: AuthenticatedRequest,
    @ZodBody(createAutoEnrollRuleSchema) dto: CreateAutoEnrollRuleInput,
  ) {
    return this.svc.createAutoEnrollRule(
      req.user.tenantId,
      req.user.orgId || "org-system-default",
      dto,
    );
  ...
  async updateRule(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(updateAutoEnrollRuleSchema) dto: UpdateAutoEnrollRuleInput,
  ) {
  ...
  @ApiOperation({
    summary:
      "Process all due cadence steps now (manual trigger; scheduler-ready)",
  })
  ...
  @ApiOperation({
    summary:
      "List my pending cadence step tasks (call/task/LinkedIn touchpoints)",
  })
  @Get("step-tasks/mine")
  ...
  async completeTask(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(completeStepTaskSchema) dto: CompleteStepTaskInput,
  ) {
    return this.svc.completeStepTask(
      req.user.tenantId,
      id,
      dto,
      req.user.userId || "system",
