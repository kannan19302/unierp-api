...
export class SupplyPlanningService {
  async getDemandSenseRuns(tenantId: string) {
    return prisma.demandSenseRun.findMany({
      where: { tenantId },
  ...
  async createDemandSenseRun(tenantId: string, dto: any, userId?: string) {
    const run = await prisma.demandSenseRun.create({
      data: {
        tenantId,
        name: dto.name,
        ...
        algorithm: dto.algorithm || "MOVING_AVERAGE",
    ...
    setImmediate(async () => {
      try {
        await prisma.demandSenseRun.update({
          where: { id: run.id },
          data: { status: "RUNNING" },
        });
        ...
        for (const product of products) {
          ...
          for (let m = 1; m <= (dto.horizonMonths || 12); m++) {
            ...
            results.push({
              ...
              forecastQty,
              lowerBound: Math.round(forecastQty * 0.8),
              upperBound: Math.round(forecastQty * 1.2),
        ...
        if (results.length)
          await prisma.demandSenseResult.createMany({ data: results });
        await prisma.demandSenseRun.update({
          where: { id: run.id },
          data: {
            status: "COMPLETED",
  ...
  async getDemandSenseResults(tenantId: string, runId: string) {
    return prisma.demandSenseResult.findMany({
  ...
  async getSupplyPlans(tenantId: string) {
    return prisma.supplyPlan.findMany({
      where: { tenantId, isActive: true },
      orderBy: { createdAt: "desc" },
      include: { _count: { select: { lines: true, scenarios: true } } },
    });
  }
  ...
  async getSupplyPlanById(tenantId: string, id: string) {
    const plan = await prisma.supplyPlan.findFirst({
      where: { id, tenantId },
      include: {
        lines: { orderBy: { period: "asc" } },
        scenarios: { include: { lines: true } },
      },
    });
    ...
    return prisma.$transaction(async (tx) => {
      const plan = await tx.supplyPlan.create({
        data: {
          ...
          description: dto.description || null,
          planType: dto.planType || "TACTICAL",
          planningHorizon: dto.planningHorizon || 12,
          startDate: dto.startDate ? new Date(dto.startDate) : null,
          endDate: dto.endDate ? new Date(dto.endDate) : null,
          demandSource: dto.demandSource || null,
          constraints: dto.constraints || Prisma.JsonNull,
          assumptions: dto.assumptions || Prisma.JsonNull,
      ...
      if (dto.lines?.length) {
        await tx.supplyPlanLine.createMany({
          data: dto.lines.map((l: any) => ({
            ...
            period: l.period,
            forecastedQty: l.forecastedQty
              ? new Prisma.Decimal(l.forecastedQty)
              : null,
            onHandQty: l.onHandQty ? new Prisma.Decimal(l.onHandQty) : null,
            safetyStockQty: l.safetyStockQty
              ? new Prisma.Decimal(l.safetyStockQty)
              : null,
            reorderPoint: l.reorderPoint
              ? new Prisma.Decimal(l.reorderPoint)
              : null,
          })),
        });
      }
      return tx.supplyPlan.findUnique({
        where: { id: plan.id },
        include: { lines: true },
      });
    });
  ...
  async createSupplyPlanScenario(
    tenantId: string,
    planId: string,
    dto: any,
    userId?: string,
    ...
    return prisma.supplyPlanScenario.create({
      data: {
        ...
        name: dto.name,
        description: dto.description || null,
        scenarioType: dto.scenarioType || "WHAT_IF",
        assumptions: dto.assumptions || Prisma.JsonNull,
  ...
  async getSopPlanById(tenantId: string, id: string) {
    const plan = await prisma.sopPlan.findFirst({
      where: { id, tenantId },
      include: { reviews: { orderBy: { reviewDate: "desc" } }, metrics: true },
    });
    ...
    return prisma.sopPlan.create({
      data: {
        ...
        revenueTarget: dto.revenueTarget
          ? new Prisma.Decimal(dto.revenueTarget)
          : null,
        costBudget: dto.costBudget ? new Prisma.Decimal(dto.costBudget) : null,
        inventoryTarget: dto.inventoryTarget
          ? new Prisma.Decimal(dto.inventoryTarget)
          : null,
  ...
  async createSopPlanReview(tenantId: string, planId: string, dto: any) {
    ...
    return prisma.sopPlanReview.create({
      data: {
        ...
        decisions: dto.decisions || Prisma.JsonNull,
      },
    });
  }
}
