...
describe("Finance Hardening Services", () => {
  ...
  describe("IntercompanyLoansService", () => {
    ...
    it("should create a loan agreement successfully", async () => {
      ...
      vi.mocked(prisma.intercompanyLoan.create).mockResolvedValue(
        mockLoan as any,
      );
    ...
    it("should calculate accrued simple interest correctly", async () => {
      ...
      const mockDrawdowns = [
        { amount: 100000, drawdownDate: new Date("2026-07-01") },
      ];
      const mockRepayments = [] as any[];

      vi.mocked(prisma.intercompanyLoan.findFirst).mockResolvedValue(
        mockLoan as any,
      );
      vi.mocked(prisma.loanDrawdown.findMany).mockResolvedValue(
        mockDrawdowns as any,
      );
      vi.mocked(prisma.loanRepayment.findMany).mockResolvedValue(
        mockRepayments as any,
      );

      const res = await service.calculateAccruedInterest(
        tenantId,
        "loan-1",
        "2026-07-31",
      );
  ...
  describe("AssetLifecycleService", () => {
    ...
    it("should create revaluation and adjust currentValue", async () => {
      ...
      vi.mocked(prisma.assetRevaluation.create).mockResolvedValue({
        id: "rev-1",
      } as any);
    ...
    it("should calculate depreciation post-revaluation", async () => {
      ...
      const dep = await service.calculateDepreciationAfterReval(
        tenantId,
        "asset-1",
      );
    ...
    it("should dispose asset and calculate gain/loss", async () => {
      ...
      vi.mocked(prisma.assetDisposal.create).mockResolvedValue({
        id: "disp-1",
        gainLoss: 10000,
      } as any);
  ...
  describe("CashPoolingService", () => {
    ...
    it("should create concentration cash pool definition", async () => {
      vi.mocked(prisma.cashPool.create).mockResolvedValue({
        id: "pool-1",
      } as any);
    ...
    it("should run sweep concentration concentration successfully", async () => {
      ...
      vi.mocked(prisma.cashPoolRun.create).mockResolvedValue({
        id: "run-1",
      } as any);
  ...
  describe("ConsolidationDeepService", () => {
    ...
    it("should register average and closing consolidation rates", async () => {
      vi.mocked(prisma.consolidationRate.upsert).mockResolvedValue({
        id: "rate-1",
      } as any);
    ...
    it("should run consolidated translations and balance ctaAmount", async () => {
      const mockRates = [{ averageRate: 1.1, closingRate: 1.12 }];
      const mockJournals = [{ debit: 100000, credit: 50000 }];

      vi.mocked(prisma.consolidationRate.findMany).mockResolvedValue(
        mockRates as any,
      );
      vi.mocked(prisma.journalEntry.findMany).mockResolvedValue(
        mockJournals as any,
      );
      vi.mocked(prisma.consolidationRun.create).mockResolvedValue({
        id: "run-1",
        ctaAmount: 2750,
      } as any);

      const res = await service.runConsolidatedTranslation(
        tenantId,
        "2026-07",
        "USD",
      );
