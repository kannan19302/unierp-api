...
export class InventoryAnalyticsController {
  ...
  @Get("slow-moving")
  @ApiOperation({
    summary: "Slow-moving inventory items (no outbound in N days)",
  })
  getSlowMoving(
    @Req() req: AuthReq,
    @Query("warehouseId") warehouseId?: string,
    ...
    return this.service.getSlowMovingInventory(
      req.user.tenantId,
      warehouseId,
  ...
  getDio(@Req() req: AuthReq, @Query("warehouseId") warehouseId?: string) {
    return this.service.getDaysInventoryOutstanding(
      req.user.tenantId,
      warehouseId,
    );
  }
  ...
  getFillRate(@Req() req: AuthReq, @Query("days") days?: string) {
    return this.service.getFillRateReport(
      req.user.tenantId,
  ...
  getVolumeTrends(
    ...
    return this.service.getVolumeTrends(
      req.user.tenantId,
      warehouseId,
      days ? parseInt(days) : 30,
    );
  ...
  @Get("shrinkage")
  @ApiOperation({
    summary: "Shrinkage and loss analysis from negative adjustments",
  })
  getShrinkage(
    @Req() req: AuthReq,
    @Query("warehouseId") warehouseId?: string,
    ...
    return this.service.getShrinkageReport(
      req.user.tenantId,
      warehouseId,
      days ? parseInt(days) : 90,
    );
