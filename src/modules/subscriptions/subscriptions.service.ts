...
export class SubscriptionsService {
  ...
  async findAll(
    ...
    const [items, total] = await Promise.all([
      prisma.subscription.findMany({
        ...
        include: {
          lines: true,
          _count: { select: { invoices: true, usage: true } },
        },
      }),
  ...
  async update(
    ...
    if (dto.unitAmount !== undefined)
      updateData.unitAmount = new Prisma.Decimal(dto.unitAmount);
  ...
  async cancel(tenantId: string, id: string, immediate?: boolean) {
    const sub = await prisma.subscription.findFirst({
      where: { id, tenantId, status: { in: ["ACTIVE", "TRIALING", "PAUSED"] } },
    });
    if (!sub)
      throw new NotFoundException("Active/Trialing subscription not found");
  ...
  async runBilling(tenantId: string) {
    const due = await prisma.subscription.findMany({
      where: {
        ...
        OR: [{ billingCycles: null }, { billingCycles: { gt: 0 } }],
      },
      include: { lines: true },
    });
  ...
  async getMetrics(tenantId: string) {
    ...
    const [activeSubs, newThisMonth, churnedThisMonth, allActive] =
      await Promise.all([
        prisma.subscription.findMany({
    ...
    const arr = mrr * 12;
    const churnRate =
      prevMonthActive > 0 ? (churnedThisMonth / prevMonthActive) * 100 : 0;
  ...
  async getMrrBreakdown(tenantId: string) {
  ...
  async getChurnCount(tenantId: string) {
  ...
  async getOverviewMetrics(tenantId: string) {
    const [active, paused, canceled, trialing, total] = await Promise.all([
      ...
      prisma.subscription.count({ where: { tenantId, status: "TRIALING" } }),
      prisma.subscription.count({ where: { tenantId } }),
    ]);
    return { active, paused, canceled, trialing, total };
  }
