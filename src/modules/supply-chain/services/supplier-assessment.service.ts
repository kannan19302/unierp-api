...
export class SupplierAssessmentService {
  async list(
    ...
    const where: Prisma.SupplierAssessmentWhereInput = {
      tenantId,
      ...(opts.vendorId ? { vendorId: opts.vendorId } : {}),
      ...(opts.status ? { status: opts.status } : {}),
    };
    const orderBy: Prisma.SupplierAssessmentOrderByWithRelationInput =
      opts.sortBy
        ? { [opts.sortBy]: opts.sortOrder ?? "desc" }
        : { createdAt: "desc" };
    const [data, total] = await Promise.all([
      prisma.supplierAssessment.findMany({
        where,
        orderBy,
        skip,
        take: limit,
  ...
  async getById(tenantId: string, id: string) {
    ...
    if (!assessment)
      throw new NotFoundException(`Supplier assessment not found: ${id}`);
    return assessment;
  }
  ...
  async create(tenantId: string, dto: Prisma.SupplierAssessmentCreateInput) {
    const existing = await prisma.supplierAssessment.findFirst({
      where: { tenantId, assessmentNumber: dto.assessmentNumber },
    });
    if (existing)
      throw new BadRequestException(
        `Assessment number already exists: ${dto.assessmentNumber}`,
      );
  ...
  async update(
    tenantId: string,
    id: string,
    dto: Prisma.SupplierAssessmentUpdateInput,
  ) {
  ...
  async delete(tenantId: string, id: string) {
    await this.getById(tenantId, id);
    return prisma.supplierAssessment.update({
      where: { id },
  ...
  async complete(
    ...
    if (assessment.status === "CANCELLED")
      throw new BadRequestException("Cannot complete a cancelled assessment");
    return prisma.supplierAssessment.update({
      where: { id },
      data: {
        status: "COMPLETED",
