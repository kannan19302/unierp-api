...
describe("FinanceEventHandler", () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(financeService, "createInvoice").mockResolvedValue({
      id: "inv-1",
      invoiceNumber: "INV-AUTO-DEL-1",
    } as never);
  ...
  it("should auto-create invoice when sales delivery is created", async () => {
    ...
    vi.mocked(prisma.salesOrder.findFirst).mockResolvedValue({
      ...
      orgId: "org-1",
      customerId: "cust-1",
      lineItems: [{ productId: "prod-1", unitPrice: 100, taxRate: 10 }],
    } as never);
    ...
    const createInvoiceCallArgs = vi.mocked(financeService.createInvoice).mock
      .calls[0];
  ...
  it("should handle errors gracefully without throwing", async () => {
    ...
    vi.mocked(prisma.salesOrder.findFirst).mockRejectedValue(
      new Error("Database error"),
    );

    // Should not throw
    await expect(
      eventHandler.handleSalesDeliveryCreated(mockEvent),
