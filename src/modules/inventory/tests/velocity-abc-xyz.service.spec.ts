...
describe("VelocityAbcXyzService", () => {
  ...
  it("activateRun supersedes previous active run and activates draft", async () => {
    ...
    vi.mocked(prisma.velocityClassificationRun.updateMany).mockResolvedValue({
      count: 1,
    } as any);
    vi.mocked(prisma.velocityClassificationRun.update).mockResolvedValue({
      id: "r2",
      status: "ACTIVE",
    } as any);
  ...
  it("computeClassification classifies top revenue product as A", async () => {
    ...
    vi.mocked(prisma.velocityClassificationItem.deleteMany).mockResolvedValue({
      count: 0,
    } as any);
    vi.mocked(prisma.velocityClassificationItem.createMany).mockResolvedValue({
      count: 3,
    } as any);
    vi.mocked(prisma.velocityClassificationRun.update).mockImplementation(
      async ({ data }: any) => ({ id: "r1", ...data }),
    );
    ...
    const items = [
      {
        ...
        monthlyDemandSeries: [10, 10, 10, 10],
      },
      {
        productId: "p2",
        totalRevenue: 15000,
        totalQuantitySold: 50,
        monthlyDemandSeries: [5, 10, 5, 5],
      },
      {
        productId: "p3",
        totalRevenue: 5000,
        totalQuantitySold: 20,
        monthlyDemandSeries: [1, 8, 3, 2],
      },
    ];
    await svc.computeClassification(T, "r1", items);

    const createManyCall = vi.mocked(
      prisma.velocityClassificationItem.createMany,
  ...
  it("computeClassification assigns XYZ based on CV", async () => {
    ...
    const stable = createManyCall.data.find(
      (d: any) => d.productId === "p-stable",
    );
    const erratic = createManyCall.data.find(
      (d: any) => d.productId === "p-erratic",
    );
  ...
  it("computeClassification throws BadRequestException for non-DRAFT run", async () => {
    ...
    await expect(svc.computeClassification(T, "r1", [])).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("upsertPolicy updates existing policy", async () => {
    ...
    vi.mocked(prisma.velocitySlottingPolicy.update).mockResolvedValue({
      id: "sp1",
  ...
  it("recordSnapshot updates when month already exists", async () => {
    ...
    vi.mocked(prisma.productVelocitySnapshot.update).mockResolvedValue({
      id: "sn1",
      quantitySold: 200,
    } as any);
    const result = await svc.recordSnapshot(T, {
