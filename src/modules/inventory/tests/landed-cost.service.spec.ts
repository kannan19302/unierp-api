...
describe("LandedCostService", () => {
  ...
  describe("getVoucher", () => {
    ...
    it("throws NotFoundException when not found", async () => {
      ...
      await expect(svc.getVoucher(TENANT, "missing")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("createVoucher", () => {
    it("creates voucher with generated number", async () => {
      ...
      expect(prisma.landedCostVoucher.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ voucherNumber: "LCV-00001" }),
        }),
      );
    });
  });
  ...
  describe("submitVoucher", () => {
    it("transitions DRAFT to SUBMITTED", async () => {
      ...
      vi.mocked(prisma.landedCostVoucher.update).mockResolvedValue(
    ...
    it("throws if no charge lines", async () => {
      ...
      await expect(svc.submitVoucher(TENANT, "v1")).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("cancelVoucher", () => {
    it("cancels SUBMITTED voucher", async () => {
      ...
      vi.mocked(prisma.landedCostVoucher.update).mockResolvedValue(
        mockVoucher({ status: "CANCELLED" }) as never,
      );
    ...
    it("throws if ALLOCATED", async () => {
      ...
      await expect(svc.cancelVoucher(TENANT, "v1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("deleteVoucher", () => {
    it("deletes DRAFT voucher", async () => {
      ...
      vi.mocked(prisma.landedCostVoucher.delete).mockResolvedValue(
        mockVoucher() as never,
      );
    ...
    it("throws if not DRAFT", async () => {
      ...
      await expect(svc.deleteVoucher(TENANT, "v1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("addChargeLine", () => {
    it("adds line to DRAFT voucher and recalcs total", async () => {
      ...
      vi.mocked(prisma.landedCostChargeLine.create).mockResolvedValue({
        id: "cl1",
        amount: "500",
      } as never);
      vi.mocked(prisma.landedCostChargeLine.aggregate).mockResolvedValue({
        _sum: { amount: "500" },
      } as never);
      vi.mocked(prisma.landedCostVoucher.update).mockResolvedValue(
        mockVoucher() as never,
      );
      const result = await svc.addChargeLine(TENANT, "v1", {
        chargeType: "FREIGHT",
        amount: 500,
      });
    ...
    it("throws if voucher not DRAFT", async () => {
      ...
      await expect(
        svc.addChargeLine(TENANT, "v1", { chargeType: "FREIGHT", amount: 100 }),
  ...
  describe("removeChargeLine", () => {
    it("removes charge line from DRAFT", async () => {
      ...
      vi.mocked(prisma.landedCostChargeLine.delete).mockResolvedValue(
        {} as never,
      );
      vi.mocked(prisma.landedCostChargeLine.aggregate).mockResolvedValue({
        _sum: { amount: "0" },
  ...
  describe("linkReceipt", () => {
    it("links a receipt to DRAFT voucher", async () => {
      ...
      vi.mocked(prisma.landedCostReceiptLink.create).mockResolvedValue({
        id: "rl1",
      } as never);
      const result = await svc.linkReceipt(TENANT, "v1", {
        stockEntryId: "se1",
        totalQty: 10,
        totalValue: 1000,
      });
  ...
  describe("allocate", () => {
    it("allocates SUBMITTED voucher", async () => {
      ...
      vi.mocked(prisma.landedCostAllocation.deleteMany).mockResolvedValue({
        count: 0,
      } as never);
      vi.mocked(prisma.landedCostAllocation.createMany).mockResolvedValue({
        count: 1,
      } as never);
      vi.mocked(prisma.landedCostVoucher.update).mockResolvedValue(
        mockVoucher({ status: "ALLOCATED" }) as never,
    ...
    it("throws if not SUBMITTED", async () => {
      ...
      await expect(svc.allocate(TENANT, "v1")).rejects.toThrow(
        BadRequestException,
      );
    });
