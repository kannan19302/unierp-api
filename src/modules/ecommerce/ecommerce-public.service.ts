...
export class EcommercePublicService {
  ...
  async addCartItem(
    ...
    if (cart.status !== "ACTIVE") {
      throw new BadRequestException(
        `Cart is ${cart.status.toLowerCase()} and can no longer be modified`,
      );
    }
    ...
    if (!listing)
      throw new NotFoundException("Product listing not found or not published");
  ...
  async updateCartItem(
    ...
    const item = await prisma.cartItem.findFirst({
      where: { id: itemId, tenantId, cartId: cart.id },
    });
