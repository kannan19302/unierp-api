...
export class PaymentSchedulesService {
  ...
  async bulkCreateFromPo(
    ...
    orgId: string,
    purchaseOrderId: string,
    milestones: Array<{ dueDate: string; amount: number }>,
  ) {
  ...
  async getStats(tenantId: string) {
    ...
    const overdue = schedules.filter(
      (s) => s.status === "PENDING" && s.dueDate < new Date(),
    );
    return {
      total: schedules.length,
      totalAmount: schedules.reduce((sum, s) => sum + Number(s.amount), 0),
      byStatus: schedules.reduce((acc: Record<string, number>, s) => {
  ...
  async getSchedulesByPurchaseOrder(
    tenantId: string,
    purchaseOrderId: string,
  ): Promise<any[]> {
  ...
  async markOverdue(tenantId: string, id: string) {
    ...
    if (schedule.status !== "PENDING")
      throw new BadRequestException(
        "Only pending schedules can be marked overdue",
      );
  ...
  async getPaymentForecast(tenantId: string, months: number = 6) {
    ...
    for (const s of schedules) {
      ...
      if (!monthlyForecast[key])
        monthlyForecast[key] = { total: 0, count: 0, schedules: [] };
    ...
    const totalProjected = schedules.reduce(
      (s, schedule) => s + Number(schedule.amount),
      0,
    );
    ...
    return {
      ...
      monthlyForecast: Object.entries(monthlyForecast).map(([month, data]) => ({
        month,
        ...data,
      })),
      currency: "USD",
