...
describe("CostingMethodsService", () => {
  ...
  it("upsertProfile creates when no existing profile", async () => {
    ...
    const result = await svc.upsertProfile(T, "u1", {
      productId: "p1",
      warehouseId: "w1",
      method: "FIFO",
    });
  ...
  it("upsertProfile updates existing profile", async () => {
    ...
    vi.mocked(prisma.inventoryCostProfile.update).mockResolvedValue({
      id: "cp1",
      method: "FIFO",
    } as any);
    const result = await svc.upsertProfile(T, "u1", {
  ...
  it("addCostLayer creates layer and updates WAC", async () => {
    ...
    vi.mocked(prisma.inventoryCostLayer.create).mockResolvedValue({
      id: "cl1",
      qtyReceived: 100,
      unitCost: 10,
    } as any);
  ...
  it("consumeLayer throws when insufficient stock", async () => {
    ...
    vi.mocked(prisma.inventoryCostLayer.update).mockResolvedValue({} as any);
    await expect(svc.consumeLayer(T, "cp1", 50)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("createAdjustment auto-numbers ICA-XXXXXX", async () => {
    ...
    vi.mocked(prisma.inventoryCostAdjustment.create).mockResolvedValue({
      id: "adj1",
      adjustmentNumber: "ICA-000010",
    } as any);
    const result = await svc.createAdjustment(T, "u1", {
      profileId: "cp1",
    ...
    expect(prisma.inventoryCostAdjustment.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ adjustmentNumber: "ICA-000010" }),
      }),
    );
