...
export class DataQualityController {
  ...
  async deduplicate(
    ...
    return this.dataQualityService.deduplicate(tenantId, modelName, body.fields);
  }
  ...
  async mergeDuplicates(
    @Param('modelName') modelName: string,
    @Body() body: { primaryId: string; duplicateIds: string[]; mergeStrategy?: any[] },
    @Req() req: AuthenticatedRequest,
  ) {
    const tenantId = req.tenantId || req.user?.tenantId;
    return this.dataQualityService.mergeDuplicates(
      tenantId, modelName, body.primaryId, body.duplicateIds, body.mergeStrategy,
    );
  }
