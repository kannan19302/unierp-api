...
describe("CrmGuidedSellingService", () => {
  ...
  describe("action configs", () => {
    ...
    it("getActionConfig throws if missing", async () => {
      vi.mocked(prisma.crmNextBestActionConfig.findFirst).mockResolvedValue(
        null,
      );
      await expect(service.getActionConfig(TENANT, "bad")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("toggleActionConfig flips isActive", async () => {
      ...
      vi.mocked(prisma.crmNextBestActionConfig.update).mockResolvedValue(
        {} as any,
      );
      await service.toggleActionConfig(TENANT, "c1");
      expect(prisma.crmNextBestActionConfig.update).toHaveBeenCalledWith(
  ...
  describe("suggestions", () => {
    it("generateSuggestions creates suggestions from configs", async () => {
      ...
      vi.mocked(prisma.crmActionSuggestion.create).mockResolvedValue({
        id: "s1",
      } as any);
      const r = await service.generateSuggestions(
        TENANT,
        "opp1",
        "OPPORTUNITY",
      );
    ...
    it("generateSuggestions creates default if no configs", async () => {
      ...
      expect(r).toHaveLength(1);
      expect(
        vi.mocked(prisma.crmActionSuggestion.create).mock.calls[0][0].data
          .actionType,
      ).toBe("FOLLOW_UP");
    });

    it("acceptSuggestion throws if not SUGGESTED", async () => {
      vi.mocked(prisma.crmActionSuggestion.findFirst).mockResolvedValue({
        id: "s1",
        status: "ACCEPTED",
      } as any);
      await expect(service.acceptSuggestion(TENANT, "s1")).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("dismissSuggestion throws if not SUGGESTED", async () => {
      ...
      await expect(service.dismissSuggestion(TENANT, "s1")).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("completeSuggestion throws if not ACCEPTED", async () => {
      ...
      await expect(service.completeSuggestion(TENANT, "s1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("playbooks", () => {
    it("createPlaybook creates with dto", async () => {
      ...
      vi.mocked(prisma.crmGuidedSellingPlaybook.create).mockResolvedValue({
        id: "p1",
      } as any);
    ...
    it("updatePlaybook increments version", async () => {
      vi.mocked(prisma.crmGuidedSellingPlaybook.findFirst).mockResolvedValue({
        id: "p1",
        version: 1,
      } as any);
      vi.mocked(prisma.crmGuidedSellingPlaybook.update).mockResolvedValue(
        {} as any,
      );
      await service.updatePlaybook(TENANT, "p1", { name: "Updated" });
      expect(prisma.crmGuidedSellingPlaybook.update).toHaveBeenCalledWith(
  ...
  describe("deal readiness", () => {
    it("scoreDealReadiness throws if opp missing", async () => {
      ...
      await expect(service.scoreDealReadiness(TENANT, "bad")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("scoreDealReadiness computes score", async () => {
      ...
      vi.mocked(prisma.crmDealReadinessScore.create).mockResolvedValue({
        id: "ds1",
        score: 60,
      } as any);
  ...
  describe("analytics", () => {
    it("getSuggestionAnalytics returns computed data", async () => {
      vi.mocked(prisma.crmActionSuggestion.count)
