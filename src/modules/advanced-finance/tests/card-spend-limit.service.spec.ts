...
describe("CardSpendLimitService", () => {
  ...
  describe("checkAuthorization", () => {
    ...
    it("denies when transaction would exceed the active spend limit cap", async () => {
      ...
      vi.mocked(prisma.cardSpendLimit.update).mockResolvedValue({
        breachCount: 1,
      } as any);
    ...
    it("allows and atomically reserves spend when under the cap", async () => {
      ...
      vi.mocked(prisma.$queryRaw).mockResolvedValue([{ id: "limit-1" }]);
    ...
    it("denies when category (MCC) limit is exceeded even if card limit is fine", async () => {
      ...
      vi.mocked(prisma.$queryRaw).mockResolvedValue([]);
      vi.mocked(prisma.cardCategoryLimit.update).mockResolvedValue({
        breachCount: 1,
      } as any);
    ...
    it("only enforces an EMPLOYEE-scoped limit that matches the card holder", async () => {
      ...
      const findManyArgs = vi.mocked(prisma.cardSpendLimit.findMany).mock
        .calls[0][0] as any;
      const scopeOr = findManyArgs.where.OR;
      expect(scopeOr).toContainEqual({
        scopeType: "EMPLOYEE",
        scopeId: "emp-1",
      });
      expect(scopeOr).toContainEqual({
  ...
  describe("resetExpiredPeriods", () => {
    it("zeroes currentSpend and rolls the period forward for expired limits", async () => {
      ...
      const updateCall = vi.mocked(prisma.cardSpendLimit.update).mock
        .calls[0][0] as any;
  ...
  describe("requestLimitIncrease", () => {
    it("rejects a requested cap that is not greater than the current cap", async () => {
      vi.mocked(prisma.cardSpendLimit.findFirst).mockResolvedValue({
        id: "limit-1",
        amountCap: 500,
      } as any);
      await expect(
        service.requestLimitIncrease(TENANT, "limit-1", "user-1", {
          requestedCap: 400,
        }),
    ...
    it("throws NotFoundException when the limit does not exist", async () => {
      ...
      await expect(
        service.requestLimitIncrease(TENANT, "missing", "user-1", {
          requestedCap: 1000,
        }),
  ...
  describe("freezeCard / unfreezeCard", () => {
    it("freezes an active card", async () => {
      ...
      vi.mocked(prisma.corporateCard.update).mockResolvedValue({
        id: CARD_ID,
        isFrozen: true,
      } as any);
