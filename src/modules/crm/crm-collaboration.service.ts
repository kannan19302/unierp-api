...
export class CrmCollaborationService {
  ...
  async deleteComment(tenantId: string, id: string, userId: string) {
    ...
    return prisma.crmComment.update({
      where: { id },
  ...
  async followRecord(
    ...
    try {
      ...
      if (
        ...
        return prisma.crmFollower.findFirst({
          where: { tenantId, userId, entityType, entityId },
        });
      }
      throw e;
    }
  ...
  async unfollowRecord(
    ...
    const follower = await prisma.crmFollower.findFirst({
      where: { tenantId, userId, entityType, entityId },
    });
