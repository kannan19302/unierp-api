...
export class ConsolidationDeepService {
  async createConsolidationRate(
    ...
    return prisma.consolidationRate.upsert({
      ...
      update: {
        ...
        closingRate: dto.closingRate,
        historicalRate: dto.historicalRate,
      },
    });
  }
  ...
  async runConsolidatedTranslation(
    ...
    const rates = await prisma.consolidationRate.findMany({
      where: { tenantId, period },
    });
    if (rates.length === 0)
      throw new BadRequestException(
        "Consolidation rates not set for this period",
      );
  ...
  async getConsolidatedFinancialStatements(tenantId: string, period: string) {
    ...
    if (!run)
      throw new NotFoundException("No consolidation run found for this period");

    // Calculate consolidated P&L / BS sums with elimination deductions
    const elims = await prisma.consolidationElimination.findMany({
      where: { tenantId, consolidationId: run.id },
    });
