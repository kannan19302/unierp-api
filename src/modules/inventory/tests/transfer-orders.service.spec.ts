...
describe("TransferOrdersService", () => {
  ...
  describe("getTransferOrder", () => {
    ...
    it("throws NotFoundException when not found", async () => {
      ...
      await expect(svc.getTransferOrder(TENANT, "missing")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("createTransferOrder", () => {
    it("creates order with auto-number", async () => {
      ...
      expect(prisma.transferOrder.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ transferNumber: "TO-00001" }),
        }),
      );
    });
    ...
    it("throws if same from/to warehouse", async () => {
      await expect(
        svc.createTransferOrder(TENANT, USER, {
          fromWarehouseId: "wh1",
          toWarehouseId: "wh1",
          lines: [],
        }),
  ...
  describe("submitForApproval", () => {
    it("moves DRAFT to PENDING_APPROVAL", async () => {
      ...
      vi.mocked(prisma.transferOrder.update).mockResolvedValue(
    ...
    it("throws if no lines", async () => {
      ...
      await expect(svc.submitForApproval(TENANT, "to1")).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("approve", () => {
    ...
    it("throws if not PENDING_APPROVAL", async () => {
      ...
      await expect(svc.approve(TENANT, "to1", USER)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("ship", () => {
    it("ships APPROVED order", async () => {
      ...
      vi.mocked(prisma.transferOrderLine.update).mockResolvedValue(
        mockLine() as never,
      );
      vi.mocked(prisma.transferOrder.update).mockResolvedValue(
    ...
    it("throws if shipped qty exceeds requested", async () => {
      ...
      vi.mocked(prisma.transferOrder.findFirst).mockResolvedValue(
        mockOrder({
          status: "APPROVED",
          lines: [mockLine({ requestedQty: "100" })],
        }) as never,
      );
      await expect(
        svc.ship(TENANT, "to1", USER, {
    ...
    it("throws if not APPROVED", async () => {
      ...
      await expect(
        svc.ship(TENANT, "to1", USER, { shippedLines: [] }),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("cancel", () => {
    ...
    it("throws if already completed", async () => {
      ...
      await expect(svc.cancel(TENANT, "to1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("closeOut", () => {
    ...
    it("throws if not PARTIALLY_RECEIVED", async () => {
      ...
      await expect(svc.closeOut(TENANT, "to1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("addLine", () => {
    it("adds line to DRAFT order", async () => {
      ...
      vi.mocked(prisma.transferOrderLine.create).mockResolvedValue(
        mockLine() as never,
      );
      const result = await svc.addLine(TENANT, "to1", {
        productId: "prod1",
        requestedQty: 50,
      });
    ...
    it("throws if order is IN_TRANSIT", async () => {
      ...
      await expect(
        svc.addLine(TENANT, "to1", { productId: "prod1", requestedQty: 50 }),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("getInTransitSummary", () => {
    it("returns in-transit summary", async () => {
      ...
      vi.mocked(prisma.transferOrder.findMany).mockResolvedValue([
        mockOrder({
          status: "IN_TRANSIT",
          lines: [mockLine({ shippedQty: "80", receivedQty: "50" })],
        }),
      ] as never);
