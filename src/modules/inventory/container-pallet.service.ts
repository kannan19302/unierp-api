...
export class ContainerPalletService {
  ...
  async createLoadPlan(
    ...
    return prisma.loadPlan.create({
      data: {
        ...
        ...data,
        plannedLoadDate: data.plannedLoadDate
          ? new Date(data.plannedLoadDate)
          : null,
      } as any,
    });
  }

  async transitionLoadPlan(
    tenantId: string,
    id: string,
    action: string,
    _userId: string,
    ...
    const transitions: Record<string, { from: string[]; to: string }> = {
      ...
      cancel: {
        from: ["DRAFT", "OPTIMIZING", "READY", "IN_LOADING"],
        to: "CANCELLED",
      },
    };
    ...
    if (!t.from.includes(plan.status)) {
      throw new BadRequestException(
        `Cannot ${action} a plan in status ${plan.status}`,
      );
    }
  ...
  async removePalletFromLoadPlan(
    tenantId: string,
    planId: string,
    palletId: string,
  ) {
    await this.requireLoadPlan(tenantId, planId);
    await prisma.loadPlanPallet.deleteMany({
      where: { id: palletId, loadPlanId: planId },
    });
    await this.recalcLoadPlanStats(planId);
    return { removed: true };
  }

  async addItemToLoadPlan(
    tenantId: string,
    planId: string,
    data: {
      productId: string;
  ...
  async getLoadPlanUtilization(tenantId: string, id: string) {
    ...
    const maxPayload = containerType
      ? Number((containerType as any).maxPayloadKg ?? 0)
      : 0;
    const maxVolume = containerType
      ? Number((containerType as any).cubicMeters ?? 0)
      : 0;
    const weightPct =
      maxPayload > 0 ? (Number(plan.totalWeightKg) / maxPayload) * 100 : 0;
    const volumePct =
      maxVolume > 0 ? (Number(plan.totalVolumeM3) / maxVolume) * 100 : 0;
    return {
      planId: id,
  ...
  async sealCarton(tenantId: string, planId: string, cartonId: string) {
    ...
    return prisma.loadCarton.update({
      where: { id: cartonId },
      data: { sealed: true } as any,
    });
  }
  ...
  async printCartonLabel(tenantId: string, planId: string, cartonId: string) {
    ...
    return prisma.loadCarton.update({
      where: { id: cartonId },
      data: { labelPrinted: true } as any,
    });
  }

  async addItemToCarton(
    tenantId: string,
    planId: string,
    cartonId: string,
    data: {
    ...
    if (!carton) throw new NotFoundException(`Carton ${cartonId} not found`);
    if ((carton as any).sealed)
      throw new BadRequestException("Carton is already sealed");
    const { quantity, ...rest } = data;
  ...
  async getDashboard(tenantId: string) {
    const [palletTypes, containerTypes, loadStats, packingStats] =
      await Promise.all([
        prisma.palletType.count({ where: { tenantId } }),
        prisma.containerType.count({ where: { tenantId } }),
        prisma.loadPlan.groupBy({
          by: ["status"],
          where: { tenantId },
          _count: true,
        }),
        prisma.packingPlan.groupBy({
          by: ["status"],
          where: { tenantId },
          _count: true,
        }),
    ...
    const loadByStatus = Object.fromEntries(
      loadStats.map((r) => [r.status, r._count]),
    );
    const packByStatus = Object.fromEntries(
      packingStats.map((r) => [r.status, r._count]),
    );
    ...
    return {
      ...
      loadPlans: {
        ...
        inProgress:
          (loadByStatus["OPTIMIZING"] ?? 0) +
          (loadByStatus["READY"] ?? 0) +
          (loadByStatus["IN_LOADING"] ?? 0),
      ...
      packingPlans: {
        ...
        active:
          (packByStatus["CONFIRMED"] ?? 0) + (packByStatus["PACKING"] ?? 0),
  ...
  private async recalcLoadPlanStats(planId: string) {
    const pallets = await prisma.loadPlanPallet.findMany({
      where: { loadPlanId: planId },
    });
    const totalWeightKg = pallets.reduce(
      (s, p) => s + Number((p as any).grossWeightKg ?? 0),
      0,
    );
  ...
  private async recalcPackingPlanStats(planId: string) {
    const cartons = await prisma.loadCarton.findMany({
      where: { packingPlanId: planId },
    });
    const totalWeightKg = cartons.reduce(
      (s, c) => s + Number((c as any).grossWeightKg ?? 0),
      0,
    );
