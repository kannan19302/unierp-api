...
export class ManufacturingEnterpriseController {
  ...
  @ApiOperation({
    summary: "Production performance with OEE, throughput, yield, cycle time",
  })
  ...
  @ApiOperation({
    summary: "Quality analysis with Pareto, CP/CPK, defect rates",
  })
  ...
  async getQualityAnalysis(
    ...
    return this.service.getQualityAnalysis(
      req.user.tenantId,
      productId,
      period || "MONTHLY",
    );
  ...
  @ApiOperation({
    summary: "Capacity-constrained production plan with load vs capacity",
  })
  @Get("production-plan")
  @Permissions("manufacturing.work-order.read")
  async getProductionPlanning(
    ...
    return this.service.getProductionPlanning(
      req.user.tenantId,
      horizon || "MONTHLY",
    );
  }
  ...
  @ApiOperation({
    summary: "MRP explosion with gross/net requirements and planned orders",
  })
  @Get("mrp/:productId")
  ...
  async getMaterialRequirements(
    ...
    return this.service.getMaterialRequirements(
      req.user.tenantId,
      productId,
      Number(demand || 100),
    );
  ...
  @ApiOperation({
    summary:
      "Manufacturing yield: first-pass yield, roll-through yield, defect density",
  })
  @Get("yield/:productId")
  @Permissions("manufacturing.quality.read")
  async getManufacturingYield(
    ...
    return this.service.getManufacturingYield(
      req.user.tenantId,
      productId,
      period || "MONTHLY",
    );
  ...
  @ApiOperation({
    summary: "Equipment effectiveness with OEE, MTBF, MTTR, downtime analysis",
  })
  ...
  async getEquipmentEffectiveness(
    ...
    return this.service.getEquipmentEffectiveness(
      req.user.tenantId,
      workCenterId,
      period || "MONTHLY",
    );
  ...
  @ApiOperation({
    summary: "Manufacturing unit cost breakdown (material + labor + overhead)",
  })
  ...
  async getManufacturingCost(
    ...
    return this.service.getManufacturingCost(
      req.user.tenantId,
      productId,
      period || "MONTHLY",
    );
  ...
  @ApiOperation({
    summary: "Sustainability metrics: energy, waste, recycling rate",
  })
  @Get("sustainability")
  @Permissions("manufacturing.work-order.read")
  async getSustainabilityMetrics(
    ...
    return this.service.getSustainabilityMetrics(
      req.user.tenantId,
      period || "MONTHLY",
    );
  }
