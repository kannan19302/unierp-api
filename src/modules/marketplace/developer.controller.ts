...
export class DeveloperController {
  ...
  async myVendor(@Req() req: AuthenticatedRequest) {
    return this.vendors.getOrCreateMyVendor(
      req.user.userId,
      req.user.tenantId,
      { name: req.user.name, contactEmail: req.user.email },
    );
  }
  ...
  async updateMyVendor(
    ...
    const vendor = await this.vendors.getOrCreateMyVendor(
      req.user.userId,
      req.user.tenantId,
      { name: req.user.name, contactEmail: req.user.email },
    );
  ...
  async createBundle(
    ...
    @ZodBody(z.any())
    body: { manifest: any; channel?: string; changelog?: string },
  ) {
    return this.vendors.createBundle(packageId, body.manifest, {
  ...
  async reject(
    ...
    return this.vendors.rejectBundle(
      bundleId,
      req.user.userId,
      body.reviewNotes,
    );
  ...
  async rollback(
    @Req() req: AuthenticatedRequest,
    @Param("packageId") packageId: string,
    @ZodBody(z.any()) body: { bundleId: string },
  ) {
    return this.vendors.rollbackPackage(
      packageId,
      body.bundleId,
      req.user.userId,
    );
