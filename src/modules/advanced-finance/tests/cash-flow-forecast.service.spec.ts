...
describe("CashFlowForecastService", () => {
  ...
  describe("saveForecastWeekOverride", () => {
    it("should create week override if it does not exist", async () => {
      ...
      vi.mocked(prisma.forecastWeek.create).mockResolvedValue({
        id: "ow-1",
      } as any);

      const result = await service.saveForecastWeekOverride(
        "tenant-1",
        new Date(),
        {
          adjustments: 2500,
    ...
    it("should update override if it already exists", async () => {
      ...
      vi.mocked(prisma.forecastWeek.findUnique).mockResolvedValue(
        mockExisting as any,
      );
      vi.mocked(prisma.forecastWeek.update).mockResolvedValue({
        id: "ow-1",
        adjustments: 2500,
      } as any);
  ...
  describe("get13WeekForecast", () => {
    ...
    it("should multiply project values with scenario multipliers if provided", async () => {
      ...
      vi.mocked(prisma.forecastScenario.findFirst).mockResolvedValue(
        mockScenario as any,
      );
