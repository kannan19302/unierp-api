...
describe("CustomerPortalService — online invoice payment collection", () => {
  ...
  it("initiates a payment intent for an outstanding invoice", async () => {
    ...
    (
      prisma.portalPaymentIntent.create as ReturnType<typeof vi.fn>
    ).mockImplementation(({ data }) =>
      Promise.resolve({ id: "intent1", ...data }),
    );

    const result = await service.initiateInvoicePayment(
      TENANT,
      CUSTOMER,
      "user1",
      "inv1",
    ...
    expect(prisma.portalPaymentIntent.create).toHaveBeenCalledWith(
  ...
  it("rejects initiating payment for an already-paid invoice", async () => {
    ...
    await expect(
      service.initiateInvoicePayment(TENANT, CUSTOMER, "user1", "inv1", {
        amount: 10,
      }),
  ...
  it("confirms a payment, records a Payment row, and marks the invoice PAID when fully paid", async () => {
    (
      prisma.portalPaymentIntent.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({
      id: "intent1",
      tenantId: TENANT,
      customerId: CUSTOMER,
    ...
    (
      prisma.portalPaymentIntent.update as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "intent1", status: "SUCCEEDED" });
  ...
  it("marks the intent FAILED and throws when the mock gateway simulates a decline", async () => {
    ...
    await expect(
      service.confirmInvoicePayment(TENANT, CUSTOMER, "intent1", {
        simulateDecline: true,
      }),
    ).rejects.toThrow("declined");
    expect(prisma.portalPaymentIntent.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: "intent1" },
        data: { status: "FAILED" },
      }),
