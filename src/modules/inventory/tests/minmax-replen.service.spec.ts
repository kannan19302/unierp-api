...
describe("MinMaxReplenService", () => {
  ...
  it("upsertLevel creates a new level", async () => {
    ...
    const result = await svc.upsertLevel(T, "u1", {
      productId: "p1",
      warehouseId: "w1",
      minQty: 10,
      maxQty: 100,
  ...
  it("upsertLevel updates existing level", async () => {
    ...
    vi.mocked(prisma.minMaxLevel.update).mockResolvedValue({
      id: "lv1",
      maxQty: 200,
    } as any);
    const result = await svc.upsertLevel(T, "u1", {
  ...
  it("runReplenishment creates suggestions for below-min products", async () => {
    ...
    vi.mocked(prisma.replenSuggestion.create).mockResolvedValue({
      id: "sg1",
    } as any);
    vi.mocked(prisma.replenRunLog.create).mockResolvedValue({
      id: "rl1",
      runNumber: "RRL-000001",
  ...
  it("runReplenishment uses reorderQty override when set", async () => {
    ...
    vi.mocked(prisma.replenSuggestion.create).mockImplementation(
      async ({ data }: any) => ({ id: "sg1", suggestedQty: data.suggestedQty }),
    );
    vi.mocked(prisma.replenRunLog.create).mockResolvedValue({
      id: "rl1",
    } as any);
    ...
    expect(prisma.replenSuggestion.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ suggestionNumber: "RS-000006" }),
      }),
    );
  });
  ...
  it("approveSuggestion transitions OPEN → APPROVED", async () => {
    ...
    vi.mocked(prisma.replenSuggestion.update).mockResolvedValue({
      id: "sg1",
      status: "APPROVED",
    } as any);
  ...
  it("approveSuggestion throws BadRequestException for non-OPEN", async () => {
    ...
    await expect(svc.approveSuggestion(T, "sg1", "u1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("markOrdered throws BadRequestException for non-APPROVED", async () => {
    ...
    await expect(svc.markOrdered(T, "sg1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("cancelSuggestion throws BadRequestException for RECEIVED", async () => {
    ...
    await expect(
      svc.cancelSuggestion(T, "sg1", "u1", "duplicate"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("cancelSuggestion cancels OPEN suggestion", async () => {
    ...
    const result = await svc.cancelSuggestion(
      T,
      "sg1",
      "u1",
      "No longer needed",
  ...
  it("getDashboard returns aggregate stats", async () => {
    ...
    vi.mocked(prisma.replenRunLog.findFirst).mockResolvedValue({
      id: "rl12",
      runNumber: "RRL-000012",
    } as any);
