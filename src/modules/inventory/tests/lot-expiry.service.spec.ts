...
describe("LotExpiryService", () => {
  ...
  it("registerLot creates lot record", async () => {
    ...
    const result = await svc.registerLot(T, "u1", {
      lotNumber: "LOT-001",
      productId: "p1",
      warehouseId: "w1",
  ...
  it("quarantineLot transitions ACTIVE → QUARANTINE", async () => {
    ...
    vi.mocked(prisma.lotExpiryRecord.update).mockResolvedValue({
      id: "lot1",
      status: "QUARANTINE",
    } as any);
  ...
  it("quarantineLot throws for non-ACTIVE", async () => {
    ...
    await expect(svc.quarantineLot(T, "lot1", "reason")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("getFEFOPick throws when insufficient stock", async () => {
    ...
    await expect(svc.getFEFOPick(T, "p1", "w1", 50)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("dismissAlert marks alert as dismissed", async () => {
    ...
    vi.mocked(prisma.lotExpiryAlert.update).mockResolvedValue({
      id: "al1",
      dismissed: true,
    } as any);
  ...
  it("disposeLot auto-numbers LDR-XXXXXX", async () => {
    ...
    vi.mocked(prisma.lotDisposalRecord.create).mockResolvedValue({
      id: "dr1",
      disposalNumber: "LDR-000001",
    } as any);
    vi.mocked(prisma.lotExpiryRecord.update).mockResolvedValue({} as any);
    const result = await svc.disposeLot(T, "u1", {
      lotId: "lot1",
    ...
    expect(prisma.lotDisposalRecord.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ disposalNumber: "LDR-000001" }),
      }),
    );
