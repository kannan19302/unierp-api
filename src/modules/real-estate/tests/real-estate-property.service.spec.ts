...
describe("RealEstatePropertyService", () => {
  ...
  beforeEach(() => {
    mockPrisma = {
      realEstatePropertyInspection: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "insp-1", propertyId: "prop-1" }]),
        create: vi
          .fn()
      ...
      realEstateRentCollectionLog: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "rent-1", leaseId: "lse-1" }]),
        create: vi
          .fn()
          .mockImplementation(({ data }) =>
            Promise.resolve({ id: "rent-1", ...data }),
          ),
      },
      realEstateListingSyndicate: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "syn-1", propertyId: "prop-1" }]),
        create: vi
          .fn()
          .mockImplementation(({ data }) =>
            Promise.resolve({ id: "syn-1", ...data }),
          ),
      },
    };
  ...
  it("should list property inspections", async () => {
    const result = await service.getInspections("tenant-1", {
      propertyId: "prop-1",
    });
