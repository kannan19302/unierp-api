...
export class CrmWinLossService {
  async getReasons(tenantId: string, category?: string) {
    ...
    return prisma.winLossReason.findMany({
      where,
  ...
  async createReason(
    ...
    dto: WinLossReasonInput,
  ) {
    return prisma.winLossReason.create({
  ...
  async updateReason(
    tenantId: string,
    id: string,
    dto: Partial<WinLossReasonInput>,
  ) {
  ...
  async deleteReason(tenantId: string, id: string) {
    ...
    if (!existing) throw new NotFoundException("Reason not found");
    return prisma.winLossReason.update({
      where: { id },
  ...
  async recordWinLoss(tenantId: string, dto: RecordWinLossInput) {
    ...
    const data: any = {
      stage: dto.winLossReasonId ? "Closed Won" : "Closed Lost",
    };
  ...
  async getWinLossAnalytics(
    ...
    const byCompetitor = competitors.map((c) => ({
      ...
      wonDeals: opportunities.filter(
        (o) => o.competitorId === c.id && o.stage === "Closed Won",
      ).length,
      lostDeals: opportunities.filter(
        (o) => o.competitorId === c.id && o.stage === "Closed Lost",
      ).length,
    }));
    return {
      total,
