...
export class SupplierCertificationService {
  async list(
    ...
    const where: Prisma.SupplierCertificationWhereInput = {
      ...
      ...(opts.expiryBefore
        ? { expiryDate: { lte: new Date(opts.expiryBefore) } }
        : {}),
    };
  ...
  async update(
    tenantId: string,
    id: string,
    dto: Prisma.SupplierCertificationUpdateInput,
  ) {
  ...
  async getExpiringCertifications(tenantId: string, days: number) {
    ...
    return prisma.supplierCertification.findMany({
      where: { tenantId, status: "ACTIVE", expiryDate: { lte: target } },
      include: { vendor: { select: { id: true, name: true } } },
    });
  }
  ...
  async renewCertification(
    tenantId: string,
    id: string,
    dto: { issueDate: string; expiryDate: string; certificateNumber: string },
  ) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      return tx.supplierCertification.create({
        data: {
          ...rest,
          tenantId,
          status: "ACTIVE",
