...
describe("EcommerceAdminService", () => {
  ...
  describe("StorefrontCategory", () => {
    it("creates a category and rejects a duplicate slug within the same tenant", async () => {
      ...
      vi.mocked(prisma.storefrontCategory.create).mockResolvedValue({
        id: "cat-1",
      } as never);
    ...
    it("throws NotFoundException updating a category that does not belong to the tenant", async () => {
      ...
      await expect(
        service.updateCategory("tenant-1", "cat-x", { name: "New" }),
    ...
    it("unassigns listings before deleting a category", async () => {
      ...
      vi.mocked(prisma.storefrontCategory.delete).mockResolvedValue({
        id: "cat-1",
      } as never);
      ...
      expect(prisma.storefrontCategory.delete).toHaveBeenCalledWith({
        where: { id: "cat-1" },
      });
    });
  });
  ...
  describe("ProductListing", () => {
    it("rejects linking a Product that belongs to a different tenant", async () => {
      ...
      await expect(
        service.createListing("tenant-1", {
          productId: "prod-from-other-tenant",
          isPublished: true,
          sortOrder: 0,
        }),
      ).rejects.toThrow(NotFoundException);
    ...
    it("rejects a duplicate listing for the same product", async () => {
      ...
      vi.mocked(prisma.productListing.findUnique).mockResolvedValue({
        id: "listing-1",
      } as never);
    ...
    it("creates a listing when the product belongs to the tenant and is not already listed", async () => {
      ...
      vi.mocked(prisma.productListing.create).mockResolvedValue({
        id: "listing-1",
      } as never);

      const result = await service.createListing("tenant-1", {
        productId: "prod-1",
        isPublished: true,
        sortOrder: 0,
      });
