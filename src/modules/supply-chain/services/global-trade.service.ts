...
export class GlobalTradeService {
  ...
  async createHsCode(tenantId: string, dto: any) {
    ...
    if (existing)
      throw new BadRequestException(`HS Code ${dto.code} already exists`);
  ...
  async getCountriesOfOrigin(tenantId: string) {
    return prisma.countryOfOrigin.findMany({
  ...
  async getImportDeclarations(
    ...
    const data = await prisma.importDeclaration.findMany({
      where,
      include: { lines: true },
  ...
  async getImportDeclarationById(tenantId: string, id: string) {
    const decl = await prisma.importDeclaration.findFirst({
      where: { id, tenantId },
      include: { lines: true },
    });
      ...
      const decl = await tx.importDeclaration.create({
        data: {
          ...
          insuranceCost: dto.insuranceCost
            ? new Prisma.Decimal(dto.insuranceCost)
            : null,
      ...
      if (dto.lines?.length) {
        await tx.importDeclarationLine.createMany({
          data: dto.lines.map((l: any) => ({
            tenantId,
            importDeclarationId: decl.id,
            ...l,
            quantity: new Prisma.Decimal(l.quantity),
            unitValue: l.unitValue ? new Prisma.Decimal(l.unitValue) : null,
            totalValue:
              l.unitValue && l.quantity
                ? new Prisma.Decimal(l.unitValue * l.quantity)
                : null,
          })),
        });
      }
      return tx.importDeclaration.findUnique({
        where: { id: decl.id },
        include: { lines: true },
      });
    });
  ...
  async updateImportDeclarationStatus(
    tenantId: string,
    id: string,
    status: string,
  ) {
  ...
  async getExportDeclarations(
    tenantId: string,
    params?: { status?: string; page?: number; limit?: number },
  ) {
    const where: any = { tenantId };
    if (params?.status) where.status = params.status;
    const data = await prisma.exportDeclaration.findMany({
      where,
      include: { lines: true },
      orderBy: { createdAt: "desc" },
      skip: params?.page ? (params.page - 1) * (params.limit || 20) : 0,
    ...
    return prisma.$transaction(async (tx) => {
      const decl = await tx.exportDeclaration.create({
        data: {
          tenantId,
          declarationNumber: dto.declarationNumber,
          portOfExport: dto.portOfExport || null,
          ...
          invoiceValue: dto.invoiceValue
            ...
            : null,
          currency: dto.currency || "USD",
          exportLicense: dto.exportLicense || null,
          eccn: dto.eccn || null,
          notes: dto.notes || null,
          createdBy: userId || null,
        },
      ...
      return tx.exportDeclaration.findUnique({
        where: { id: decl.id },
        include: { lines: true },
      });
    });
  ...
  async getComplianceScreenings(
    tenantId: string,
    params?: { status?: string; screenType?: string },
  ) {
    const where: any = { tenantId };
    if (params?.status) where.status = params.status;
    if (params?.screenType) where.screenType = params.screenType;
    return prisma.tradeComplianceScreening.findMany({
      where,
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async createComplianceScreening(tenantId: string, dto: any, userId?: string) {
    return prisma.tradeComplianceScreening.create({
      data: { tenantId, ...dto, createdBy: userId || null, status: "PENDING" },
    });
  }

  async resolveComplianceScreening(
    tenantId: string,
    id: string,
    resolution: string,
    userId?: string,
  ) {
    const screen = await prisma.tradeComplianceScreening.findFirst({
      where: { id, tenantId },
    });
    if (!screen) throw new NotFoundException("Compliance screening not found");
    return prisma.tradeComplianceScreening.update({
      where: { id },
      data: {
        status: resolution === "BLOCKED" ? "MATCH_FOUND" : "PENDING",
        resolution,
        reviewedBy: userId,
        reviewedAt: new Date(),
      },
