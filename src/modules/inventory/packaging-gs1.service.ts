...
export class PackagingGs1Service {
  ...
  async upsertSpec(
    ...
    return prisma.packagingSpec.upsert({
      ...
      create: {
        ...
        lengthMm: d(data.lengthMm),
        widthMm: d(data.widthMm),
        heightMm: d(data.heightMm),
        grossWeightKg: d(data.grossWeightKg),
        netWeightKg: d(data.netWeightKg),
  ...
  async deactivateSpec(tenantId: string, id: string) {
    ...
    return prisma.packagingSpec.update({
      where: { id },
      data: { active: false },
    });
  }
  ...
  async getPackagingHierarchy(tenantId: string, productId: string) {
    ...
    return {
      ...
      levels: specs.map((s, i) => ({
        ...
        cumulativeUnits: specs
          .slice(0, i + 1)
          .reduce((acc, x) => acc * x.unitsPerLevel, 1),
        primaryBarcode: s.barcodes[0]?.barcodeValue ?? null,
        dimensions:
          s.lengthMm && s.widthMm
            ? `${s.lengthMm}×${s.widthMm}×${s.heightMm} mm`
            : null,
      })),
    };
  }
  ...
  async listBarcodes(tenantId: string, packagingSpecId?: string) {
    return prisma.itemBarcode.findMany({
      where: {
        tenantId,
        ...(packagingSpecId ? { packagingSpecId } : {}),
        active: true,
      },
  ...
  async addBarcode(
    ...
    const existing = await prisma.itemBarcode.findFirst({
      where: { tenantId, barcodeValue: data.barcodeValue },
    });
    if (existing)
      throw new ConflictException(
        `Barcode ${data.barcodeValue} already exists`,
      );
  ...
  async deactivateBarcode(tenantId: string, id: string) {
    ...
    return prisma.itemBarcode.update({
      where: { id },
      data: { active: false },
    });
  }
  ...
  async listGs1Ais(tenantId: string) {
    return prisma.gs1ApplicationIdentifier.findMany({
      where: { tenantId },
      orderBy: { ai: "asc" },
    });
  }
  ...
  async upsertGs1Ai(
    ...
    return prisma.gs1ApplicationIdentifier.upsert({
      ...
      update: {
        ...
        fnc1Required: data.fnc1Required ?? false,
  ...
  async listLabelTemplates(tenantId: string, templateType?: string) {
    return prisma.labelTemplate.findMany({
      where: {
        tenantId,
        active: true,
        ...(templateType ? { templateType: templateType as any } : {}),
      },
      orderBy: { name: "asc" },
    });
  }
  ...
  async updateLabelTemplate(
    tenantId: string,
    id: string,
    data: Partial<{ content: string; widthMm: number; heightMm: number }>,
  ) {
  ...
  async getLabelForSpec(
    ...
    if (!assignment)
      throw new NotFoundException(
        `No label assignment found for spec ${packagingSpecId}`,
      );
  ...
  async allocateSscc(
    ...
    const existing = await prisma.ssccRecord.count({
      where: { tenantId, gs1CompanyPrefix },
    });
  ...
  async markSsccUsed(tenantId: string, sscc: string, shipmentRef: string) {
    const record = await prisma.ssccRecord.findFirst({
      where: { tenantId, sscc },
    });
  ...
  async getDashboard(tenantId: string) {
    const [specs, barcodes, templates, ssccTotal, ssccUsed, gs1Ais] =
      await Promise.all([
