...
export class SaasExpansionService {
  ...
  async getAppById(tenantId: string, id: string) {
    const app = await this.p.saasApp.findFirst({
      where: { id, tenantId },
      include: {
  ...
  async createAppVersion(tenantId: string, dto: any) {
    ...
    if (!app) throw new NotFoundException("App not found");
    return this.p.saasAppVersion.create({
      data: {
        tenantId,
        ...dto,
  ...
  async installApp(
    ...
    if (!app) throw new NotFoundException("App not found");
    const existing = await this.p.saasAppInstallation.findFirst({
      where: {
        tenantId,
        appId,
        installingTenantId,
        status: { in: ["INSTALLING", "ACTIVE"] },
      },
    });
  ...
  async uninstallApp(tenantId: string, id: string) {
    const install = await this.p.saasAppInstallation.findFirst({
      where: { id, tenantId },
    });
    if (!install) throw new NotFoundException("Installation not found");
    return this.p.$transaction(async (tx: any) => {
      const updated = await tx.saasAppInstallation.update({
        where: { id },
  ...
  async getSubscriptionPlans(tenantId: string) {
    return this.p.saasSubscriptionPlan.findMany({
  ...
  async createSubscriptionPlan(tenantId: string, dto: any) {
    return this.p.saasSubscriptionPlan.create({
      data: { tenantId, ...dto, price: new Prisma.Decimal(dto.price) },
    });
  }

  async updateSubscriptionPlan(tenantId: string, id: string, dto: any) {
    const plan = await this.p.saasSubscriptionPlan.findFirst({
      where: { id, tenantId },
    });
  ...
  async deleteSubscriptionPlan(tenantId: string, id: string) {
    ...
    if (!plan) throw new NotFoundException("Plan not found");
    return this.p.saasSubscriptionPlan.update({
      where: { id },
      data: { isActive: false },
    });
  }
  ...
  async getSubscriptions(
    ...
    const [data, total] = await Promise.all([
      this.p.saasSubscription.findMany({
        where,
        orderBy: { createdAt: "desc" },
        skip,
        take,
        include: { plan: true, lineItems: true },
      }),
  ...
  async createSubscription(tenantId: string, dto: any) {
    ...
    else if (dto.billingInterval === "YEARLY")
      periodEnd.setFullYear(periodEnd.getFullYear() + 1);
    else if (dto.billingInterval === "QUARTERLY")
      periodEnd.setMonth(periodEnd.getMonth() + 3);
    return this.p.saasSubscription.create({
      data: {
        tenantId,
        subscribingTenantId: dto.subscribingTenantId,
        planId: dto.planId,
        ...
        trialEndsAt: dto.trialDays
          ? new Date(Date.now() + dto.trialDays * 86400000)
          : null,
  ...
  async updateSubscriptionStatus(
    ...
    const sub = await this.p.saasSubscription.findFirst({
      where: { id, tenantId },
    });
  ...
  async getUsageMeters(tenantId: string) {
    return this.p.saasUsageMeter.findMany({
      where: { tenantId, isActive: true },
      orderBy: { name: "asc" },
    });
  }
  ...
  async createUsageMeter(tenantId: string, dto: any) {
    const existing = await this.p.saasUsageMeter.findFirst({
      where: { tenantId, slug: dto.slug },
    });
    if (existing)
      throw new BadRequestException(
        "Usage meter with this slug already exists",
      );
  ...
  async recordUsage(tenantId: string, dto: any) {
    ...
    if (!sub) throw new NotFoundException("Subscription not found");
    const meter = await this.p.saasUsageMeter.findFirst({
    ...
    return this.p.saasUsageRecord.create({
      data: {
        ...
        meterId: dto.meterId,
        usage: new Prisma.Decimal(dto.usage),
        metadata: dto.metadata || {},
      },
    });
  }
  ...
  async getUsageRecords(
    ...
    return this.p.saasUsageRecord.findMany({
      where,
  ...
  async createInvoice(tenantId: string, dto: any) {
    ...
    if (dto.couponCode) {
      const coupon = await this.p.saasCoupon.findFirst({
        where: { tenantId, code: dto.couponCode.toUpperCase(), isActive: true },
      });
      if (coupon) {
        if (coupon.type === "PERCENTAGE")
          discountAmount =
            (dto.lineItems.reduce(
              (s: number, li: any) => s + li.unitPrice * li.quantity,
              0,
            ) *
              Number(coupon.value)) /
            100;
    ...
    return this.p.$transaction(async (tx: any) => {
      const invoice = await tx.saasInvoice.create({
        data: {
          ...
          invoiceNumber,
          billingTenantId: dto.billingTenantId,
          status: "PENDING",
      ...
      for (const li of dto.lineItems) {
        await tx.saasInvoiceLineItem.create({
          data: {
            ...
            type: li.type || "SUBSCRIPTION",
            quantity: li.quantity || 1,
            unitPrice: new Prisma.Decimal(li.unitPrice),
            totalPrice: new Prisma.Decimal(li.unitPrice * li.quantity),
          },
        });
      }
      if (dto.couponCode) {
        const coupon = await this.p.saasCoupon.findFirst({
          where: { tenantId, code: dto.couponCode.toUpperCase() },
        });
      ...
      return tx.saasInvoice.findFirst({
        where: { id: invoice.id },
  ...
  async recordPayment(tenantId: string, dto: any) {
    const invoice = await this.p.saasInvoice.findFirst({
    ...
    return this.p.$transaction(async (tx: any) => {
      ...
      const newStatus =
        newPaid >= Number(invoice.totalAmount) ? "PAID" : invoice.status;
  ...
  async getCoupons(tenantId: string) {
    return this.p.saasCoupon.findMany({
      where: { tenantId },
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async createCoupon(tenantId: string, dto: any) {
    const existing = await this.p.saasCoupon.findFirst({
      where: { tenantId, code: dto.code.toUpperCase() },
    });
    if (existing) throw new BadRequestException("Coupon code already exists");
    return this.p.saasCoupon.create({
      data: {
        tenantId,
        ...dto,
  ...
  async getFeatureFlags(tenantId: string) {
    return this.p.saasFeatureFlag.findMany({
      where: { tenantId },
      orderBy: { name: "asc" },
    });
  }
  ...
  async toggleFeatureFlag(tenantId: string, id: string, isEnabled: boolean) {
    const ff = await this.p.saasFeatureFlag.findFirst({
      where: { id, tenantId },
    });
    if (!ff) throw new NotFoundException("Feature flag not found");
    return this.p.saasFeatureFlag.update({
      where: { id },
      data: { isEnabled },
    });
  }
  ...
  async deleteTenantSetting(tenantId: string, category: string, key: string) {
    const setting = await this.p.saasTenantSetting.findFirst({
  ...
  async getDomains(tenantId: string) {
    return this.p.saasTenantDomain.findMany({
      where: { tenantId },
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async addDomain(tenantId: string, dto: any) {
    const existing = await this.p.saasTenantDomain.findFirst({
      where: { domain: dto.domain },
    });
    if (existing) throw new BadRequestException("Domain already exists");
    const verificationToken = crypto.randomBytes(32).toString("hex");
    if (dto.isPrimary) {
      await this.p.saasTenantDomain.updateMany({
        where: { tenantId },
  ...
  async verifyDomain(tenantId: string, id: string) {
    const domain = await this.p.saasTenantDomain.findFirst({
      where: { id, tenantId },
    });
    if (!domain) throw new NotFoundException("Domain not found");
    return this.p.saasTenantDomain.update({
      where: { id },
  ...
  async getWebhookEndpoints(tenantId: string) {
    return this.p.saasWebhookEndpoint.findMany({
      where: { tenantId },
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async createWebhookEndpoint(tenantId: string, dto: any) {
    ...
    return this.p.saasWebhookEndpoint.create({
      data: { tenantId, ...dto, secret },
    });
  }

  async updateWebhookEndpoint(tenantId: string, id: string, dto: any) {
    const wh = await this.p.saasWebhookEndpoint.findFirst({
      where: { id, tenantId },
    });
  ...
  async getWebhookDeliveries(
    ...
    if (query.status) where.status = query.status;
    const [data, total] = await Promise.all([
      this.p.saasWebhookDelivery.findMany({
        where,
        orderBy: { createdAt: "desc" },
        skip,
        take,
  ...
  async getApiKeys(tenantId: string) {
    return this.p.saasApiKey.findMany({
      where: { tenantId, isActive: true },
      orderBy: { createdAt: "desc" },
      include: { scopes: true },
    });
  ...
  async revokeApiKey(tenantId: string, id: string) {
    const apiKey = await this.p.saasApiKey.findFirst({
      where: { id, tenantId },
    });
    if (!apiKey) throw new NotFoundException("API key not found");
    return this.p.saasApiKey.update({
      where: { id },
      data: { isActive: false },
    });
  }
  ...
  async getSupportTickets(
    ...
    const [data, total] = await Promise.all([
      this.p.saasSupportTicket.findMany({
        where,
    ...
    return this.p.$transaction(async (tx: any) => {
      const message = await tx.saasSupportTicketMessage.create({
        data: {
          ...
          authorType: dto.authorType || "USER",
        },
      });
      if (dto.attachments && dto.attachments.length > 0) {
        await tx.saasSupportTicketAttachment.createMany({
          data: dto.attachments.map((a: any) => ({
            tenantId,
      ...
      return tx.saasSupportTicketMessage.findFirst({
        where: { id: message.id },
        include: { attachments: true },
      });
    });
  ...
  async deleteAnnouncement(tenantId: string, id: string) {
    const ann = await this.p.saasAnnouncement.findFirst({
      where: { id, tenantId },
    });
  ...
  async getMaintenanceWindows(tenantId: string) {
    return this.p.saasMaintenanceWindow.findMany({
      where: { tenantId },
  ...
  async createMaintenanceWindow(tenantId: string, dto: any, userId: string) {
    return this.p.saasMaintenanceWindow.create({
      data: {
        ...
        notifyTenants: dto.notifyTenants !== false,
        createdBy: userId,
      },
    });
  }
  ...
  async updateMaintenanceWindowStatus(
    ...
    const mw = await this.p.saasMaintenanceWindow.findFirst({
      where: { id, tenantId },
    });
    ...
    return this.p.saasMaintenanceWindow.update({
      where: { id },
      data: updateData,
    });
  }
