    ...
    Prisma: {
      Decimal: class Decimal {
        ...
        gt(other: Decimal | number) {
          const checkVal =
            other instanceof Decimal ? other.value : Number(other);
...
describe("TaxEngineService - Dunning Cadence Execution", () => {
  ...
  it("should process overdue invoices and skip if already dunned at that level", async () => {
    const mockLevels = [
      {
        ...
        levelName: "First Notice",
        status: "ACTIVE",
      },
      {
        id: "level-2",
        daysOverdue: 15,
        feeAmount: new Prisma.Decimal(25),
    ...
    vi.mocked(prisma.invoiceDunningLog.findFirst).mockResolvedValue({
      id: "existing-log",
    } as any); // Already processed
    vi.mocked(prisma.dunningRun.update).mockResolvedValue({
      id: "run-1",
      status: "COMPLETED",
      totalInvoices: 0,
    } as any);
