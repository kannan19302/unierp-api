...
export class CrmRevenueIntelligenceService {
  ...
  async generateAndSendDigests(
    ...
    const openAlerts = await prisma.pipelineRiskAlert.findMany({
      ...
      include: {
        opportunity: {
          select: { id: true, name: true, amount: true, assignedToId: true },
        },
      },
    });
  ...
  private async sendDigestForScope(
    ...
    const newAlertCount = alerts.filter(
      (a) => a.createdAt >= periodStart,
    ).length;
    const dealIds = new Set(
      alerts.map((a) => a.opportunity?.id).filter(Boolean),
    );
    ...
    this.eventEmitter.emit("notification.send", {
      ...
      type:
        scope === "MANAGER"
          ? "CRM_REVENUE_INTELLIGENCE_TEAM_DIGEST"
          : "CRM_REVENUE_INTELLIGENCE_REP_DIGEST",
      title,
      body,
      channel: "IN_APP",
    });
