...
vi.mock("@unerp/database", () => {
  ...
  return {
    prisma: {
      performanceObligation: m(),
      asc606ContractModification: m(),
      asc606DeferredRevenueRollForward: m(),
      revenueSchedule: m(),
    },
  };
});
...
describe("Asc606DeepService", () => {
  ...
  it("getPerformanceObligationById throws on not found", async () => {
    ...
    await expect(
      service.getPerformanceObligationById("t1", "bad"),
    ).rejects.toThrow(NotFoundException);
  });

  it("createPerformanceObligation creates record", async () => {
    vi.mocked(prisma.performanceObligation.create).mockResolvedValue({
      id: "po-1",
    } as any);
    const result = await service.createPerformanceObligation("t1", "o1", {
      description: "test",
      transactionPrice: 1000,
      obligationType: "GOODS",
      satisfactionTiming: "POINT_IN_TIME",
  ...
  it("satisfyPerformanceObligation throws if already satisfied", async () => {
    vi.mocked(prisma.performanceObligation.findFirst).mockResolvedValue({
      id: "po-1",
      status: "SATISFIED",
    } as any);
    await expect(
      service.satisfyPerformanceObligation("t1", "po-1"),
  ...
  it("allocateTransactionPrice allocates by SSP", async () => {
    vi.mocked(prisma.performanceObligation.findMany).mockResolvedValue([
      {
        id: "po-1",
        ssp: new Prisma.Decimal(60),
        transactionPrice: new Prisma.Decimal(30),
        description: "A",
      } as any,
    ]);
  ...
  it("getContractModifications returns modifications", async () => {
    vi.mocked(prisma.asc606ContractModification.findMany).mockResolvedValue([
      { id: "cm-1" },
    ] as any);
  ...
  it("createContractModification creates record", async () => {
    vi.mocked(prisma.asc606ContractModification.create).mockResolvedValue({
      id: "cm-1",
    } as any);
    const result = await service.createContractModification("t1", "o1", {
      contractRef: "C-001",
      modNumber: "M1",
      modificationDate: "2026-06-01",
      modType: "PRICE_CHANGE",
      originalConsideration: 100,
      modifiedConsideration: 120,
  ...
  it("approveContractModification throws if not draft", async () => {
    vi.mocked(prisma.asc606ContractModification.findFirst).mockResolvedValue({
      id: "cm-1",
      status: "APPROVED",
    } as any);
    await expect(
      service.approveContractModification("t1", "cm-1", "user1"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("applyContractModification throws if not approved", async () => {
    ...
    await expect(
      service.applyContractModification("t1", "cm-1"),
    ).rejects.toThrow(BadRequestException);
  });

  it("getDeferredRevenueRollForwards returns roll-forwards", async () => {
    vi.mocked(
      prisma.asc606DeferredRevenueRollForward.findMany,
    ).mockResolvedValue([{ id: "rf-1" }] as any);
    const result = await service.getDeferredRevenueRollForwards("t1");
    expect(result).toHaveLength(1);
  });

  it("createDeferredRevenueRollForward computes balances", async () => {
    vi.mocked(prisma.asc606DeferredRevenueRollForward.create).mockResolvedValue(
      { id: "rf-1" } as any,
    );
    const result = await service.createDeferredRevenueRollForward("t1", "o1", {
      period: "2026-07",
      openingBalance: 1000,
      additions: 500,
      recognized: 300,
  ...
  it("computeRollForwardFromSchedules upserts roll-forward", async () => {
    ...
    vi.mocked(prisma.asc606DeferredRevenueRollForward.upsert).mockResolvedValue(
      { id: "rf-1" } as any,
    );
    const result = await service.computeRollForwardFromSchedules(
      "t1",
      "o1",
      "2026-07",
    );
  ...
  it("getRevenueAging returns aging buckets", async () => {
    vi.mocked(prisma.revenueSchedule.findMany).mockResolvedValue([
      {
        deferredAmount: 500,
  ...
  it("getAsc606Dashboard returns dashboard data", async () => {
    vi.mocked(prisma.performanceObligation.findMany).mockResolvedValue([
      { revenueDeferred: 100, revenueRecognized: 50, status: "ACTIVE" },
    ] as any);
    vi.mocked(
      prisma.asc606DeferredRevenueRollForward.findMany,
