...
describe("InventoryService — wave pick, consignment inventory, receipt traceability", () => {
  ...
  describe("wave pick", () => {
    it("aggregates line items across multiple sales orders into wave items", async () => {
      ...
      db.salesOrder.findFirst
        .mockResolvedValueOnce({
          id: "so1",
          lineItems: [{ productId: "p1", quantity: 5 }],
        })
        .mockResolvedValueOnce({
          id: "so2",
          lineItems: [{ productId: "p1", quantity: 3 }],
        });
      db.inventoryItemBin.findFirst.mockResolvedValue({
        binLocationId: "bin1",
      });
      db.pickWave.create.mockImplementation(({ data }: any) =>
        Promise.resolve({ id: "wave1", ...data }),
      );

      const result = await service.createPickWave("t1", "org-1", "u1", {
        warehouseId: "w1",
        salesOrderIds: ["so1", "so2"],
      });

      expect(db.pickWave.create).toHaveBeenCalledWith(
    ...
    it("marks a wave item PICKED when the full quantity is recorded", async () => {
      db.pickWaveItem.findFirst.mockResolvedValue({
        id: "wi1",
        quantity: { toString: () => "5" },
      });
      db.pickWaveItem.update.mockImplementation(({ data }: any) =>
        Promise.resolve({ id: "wi1", ...data }),
      );
    ...
    it("rejects completing a wave with unpicked items", async () => {
      ...
      await expect(service.completePickWave("t1", "wave1")).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("completes a wave once every item is picked", async () => {
      ...
      db.pickWave.update.mockResolvedValue({
        id: "wave1",
        status: "COMPLETED",
      });
  ...
  describe("consignment inventory", () => {
    it("rejects consuming more than the on-hand quantity", async () => {
      db.consignmentStock.findFirst.mockResolvedValue({
        id: "cs1",
        quantityOnHand: { toString: () => "5" },
        unitCost: { toString: () => "10" },
      });
      await expect(
        service.recordConsignmentConsumption("t1", "cs1", { quantity: 10 }),
    ...
    it("records consumption and computes total cost from unit cost", async () => {
      db.consignmentStock.findFirst.mockResolvedValue({
        id: "cs1",
        quantityOnHand: { toString: () => "20" },
        unitCost: { toString: () => "10" },
      });
      db.consignmentStock.update.mockResolvedValue({});
      db.consignmentConsumption.create.mockImplementation(({ data }: any) =>
        Promise.resolve(data),
      );

      const result = await service.recordConsignmentConsumption("t1", "cs1", {
        quantity: 4,
      });
    ...
    it("marks an unbilled consumption as billed", async () => {
      db.consignmentConsumption.findFirst.mockResolvedValue({
        id: "cc1",
        billed: false,
      });
      db.consignmentConsumption.update.mockResolvedValue({
        id: "cc1",
        billed: true,
      });
      const result = await service.markConsignmentConsumptionBilled(
        "t1",
        "cc1",
      );
    ...
    it("throws NotFound marking an unknown consumption billed", async () => {
      ...
      await expect(
        service.markConsignmentConsumptionBilled("t1", "missing"),
  ...
  describe("receipt with traceability", () => {
    it("creates the stock entry, serial numbers, and batch in one call", async () => {
      vi.spyOn(service, "createStockEntry").mockResolvedValue({
        id: "se1",
      } as any);
      vi.spyOn(service, "submitStockEntry").mockResolvedValue({
        id: "se1",
        status: "SUBMITTED",
      } as any);
      db.serialNumber.create.mockImplementation(({ data }: any) =>
        Promise.resolve({ id: "sn1", ...data }),
      );
      db.batch.create.mockImplementation(({ data }: any) =>
        Promise.resolve({ id: "b1", ...data }),
      );

      const result = await service.receiveWithTraceability(
        "t1",
        "org-1",
        "u1",
        {
