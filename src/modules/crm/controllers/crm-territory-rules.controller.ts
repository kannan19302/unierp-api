...
import {
  ...
  AssignLeadInput,
} from "../services/crm-territory-rules.service";
...
export class CrmTerritoryRulesController {
  ...
  async create(
    @Req() req: AuthenticatedRequest,
    @ZodBody(createTerritoryRuleSchema) dto: CreateTerritoryRuleInput,
  ) {
  ...
  async update(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(updateTerritoryRuleSchema) dto: UpdateTerritoryRuleInput,
  ) {
  ...
  async assign(
    @Req() req: AuthenticatedRequest,
    @ZodBody(assignLeadSchema) dto: AssignLeadInput,
  ) {
    return this.svc.assignLead(req.user.tenantId, dto.leadId);
  }

  @ApiOperation({
    summary: "Bulk re-run territory assignment for all open leads",
  })
  @Post("reassign-all")
  @Permissions("crm.lead.update")
  @TrackChanges("Lead")
