...
describe("ColdChainWriteoffService", () => {
  ...
  it("upsertRequirement creates new when none exists", async () => {
    ...
    const result = await svc.upsertRequirement(T, {
      productId: "p1",
      minTempCelsius: 2,
      maxTempCelsius: 8,
    });
  ...
  it("upsertRequirement updates when existing", async () => {
    ...
    vi.mocked(prisma.coldChainRequirement.update).mockResolvedValue({
      id: "r1",
      active: true,
    } as any);
    const result = await svc.upsertRequirement(T, {
  ...
  it("logExcursion classifies CRITICAL severity for large temp deviation", async () => {
    ...
    vi.mocked(prisma.temperatureExcursion.create).mockImplementation(
      async ({ data }: any) => ({ id: "e1", severity: data.severity }),
    );
    const result = await svc.logExcursion(T, {
      productId: "p1",
      warehouseId: "w1",
      recordedTempC: 25,
      excursionStartAt: "2026-07-13T10:00:00Z",
      loggedById: "u1",
    });
  ...
  it("reviewExcursion updates status", async () => {
    ...
    vi.mocked(prisma.temperatureExcursion.update).mockResolvedValue({
      id: "e1",
      status: "QUARANTINED",
    } as any);
    const result = await svc.reviewExcursion(T, "e1", {
      status: "QUARANTINED",
      reviewedById: "u1",
    });
  ...
  it("createWriteDown throws BadRequestException when proposed >= original value", async () => {
    await expect(
      svc.createWriteDown(T, {
        warehouseId: "w1",
        productId: "p1",
        quantity: 10,
        originalValuePerUnit: 5,
        proposedValuePerUnit: 6,
        writeDownReason: "Test",
        requestedById: "u1",
      }),
  ...
  it("createWriteDown creates write-down with auto-number", async () => {
    ...
    const result = await svc.createWriteDown(T, {
      ...
      productId: "p1",
      quantity: 10,
      originalValuePerUnit: 10,
      proposedValuePerUnit: 5,
      writeDownReason: "Aging",
      requestedById: "u1",
    });
  ...
  it("approveWriteDown transitions to APPROVED", async () => {
    ...
    vi.mocked(prisma.stockWriteDownRequest.update).mockResolvedValue({
      id: "wd1",
      status: "APPROVED",
    } as any);
  ...
  it("approveWriteDown throws BadRequestException for non-pending", async () => {
    ...
    await expect(svc.approveWriteDown(T, "wd1", "u2")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("createWriteOff calculates totalWriteOff correctly", async () => {
    ...
    vi.mocked(prisma.stockWriteOffRecord.create).mockImplementation(
      async ({ data }: any) => ({
        id: "wo1",
        totalWriteOff: data.totalWriteOff,
      }),
    );
    ...
    expect(prisma.stockWriteOffRecord.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ writeOffNumber: "WO-000001" }),
      }),
    );
  });
  ...
  it("completeWriteOff throws BadRequestException for non-approved", async () => {
    ...
    await expect(svc.completeWriteOff(T, "wo1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("getDashboard returns aggregate counts", async () => {
    ...
    vi.mocked(prisma.stockWriteOffRecord.groupBy).mockResolvedValue([
      { _sum: { totalWriteOff: 50000 } },
    ] as any);
