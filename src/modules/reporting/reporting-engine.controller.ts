...
export class ReportingEngineController {
  ...
  async exportQuery(
    ...
    const result = await this.engine.executeQuery(
      req.user.tenantId,
      body.entity,
      { filters: body.filters, limit: body.limit || 1000 },
    );
    const semanticLayer = this.engine.getSemanticLayer();
    const entityDef = semanticLayer.find((e) => e.name === body.entity);
    const columns = (entityDef?.fields || []).map((f) => ({
      header: f.label,
      key: f.name,
      width: 20,
    }));
    ...
    if (body.format === "csv")
      return this.exportService.exportCsv(res, filename, columns, result.data);
    if (body.format === "xlsx")
      return this.exportService.exportXlsx(res, filename, columns, result.data);
    return this.exportService.exportPdf(
      res,
      filename,
      columns,
      result.data,
      `${entityDef?.label || body.entity} Report`,
    );
  }
}
