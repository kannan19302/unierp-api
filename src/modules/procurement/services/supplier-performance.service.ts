...
export class SupplierPerformanceService {
  ...
  async createScorecard(
    ...
    if (existing)
      throw new BadRequestException(
        "Scorecard already exists for this vendor and period",
      );
  ...
  async updateScorecard(
    ...
    if (data.fillRateScore !== undefined)
      updateData.fillRateScore = data.fillRateScore;
    if (data.overallScore !== undefined)
      updateData.overallScore = data.overallScore;
    if (data.onTimeDeliveries !== undefined)
      updateData.onTimeDeliveries = data.onTimeDeliveries;
    if (data.lateDeliveries !== undefined)
      updateData.lateDeliveries = data.lateDeliveries;
    if (data.defectiveUnits !== undefined)
      updateData.defectiveUnits = data.defectiveUnits;
    if (data.totalUnitsReceived !== undefined)
      updateData.totalUnitsReceived = data.totalUnitsReceived;
  ...
  async createAssessment(
    ...
    if (existing)
      throw new BadRequestException("Assessment number already exists");
  ...
  async getSupplierRiskProfile(tenantId: string, supplierId: string) {
    ...
    if (!profile)
      throw new NotFoundException("Risk profile not found for this supplier");
    return profile;
  }
  ...
  async getSupplierComparison(tenantId: string, supplierIds: string[]) {
    ...
    return {
      comparison: Array.from(latestByVendor.entries())
        .map(([vendorId, sc]) => ({
          vendorId,
          vendorName: sc.vendor.name,
          overallScore: Number(sc.overallScore || 0),
