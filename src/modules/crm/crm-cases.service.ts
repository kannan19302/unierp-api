...
export class CrmCasesService {
  ...
  async getSlaStatus(tenantId: string) {
    ...
    const breached = openCases.filter(
      (c) => c.slaDeadline && c.slaDeadline < now,
    );
    const atRisk = openCases.filter(
      (c) =>
        c.slaDeadline &&
        c.slaDeadline >= now &&
        c.slaDeadline.getTime() - now.getTime() < 2 * 60 * 60 * 1000,
    );
    const onTrack = openCases.filter(
      (c) => !breached.includes(c) && !atRisk.includes(c),
    );
    ...
    return {
      ...
      metrics: {
        ...
        daysSinceCreation: Math.floor(
          (now.getTime() - found.createdAt.getTime()) / (1000 * 60 * 60 * 24),
        ),
      },
    };
  }
