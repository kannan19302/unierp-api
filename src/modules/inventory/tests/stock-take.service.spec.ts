...
const mockItem = (o: Record<string, unknown> = {}) => ({
  ...
  productId: "prod1",
  systemQty: {
    equals: (x: unknown) => String(x) === "0",
    minus: (x: unknown) => ({
      equals: (y: unknown) => false,
      div: () => ({ times: () => ({}) }),
      times: () => ({}),
    }),
    toString: () => "100",
...
describe("StockTakeService", () => {
  ...
  describe("createStockTake", () => {
    it("creates with auto-number", async () => {
      ...
      expect(prisma.stockTake.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ stockTakeNumber: "ST-00001" }),
        }),
      );
    });
  });
  ...
  describe("startStockTake", () => {
    it("starts a DRAFT stock take with sheets", async () => {
      ...
      vi.mocked(prisma.stockTake.update).mockResolvedValue(
        mockSt({ status: "IN_PROGRESS" }) as never,
      );
    ...
    it("throws if no sheets", async () => {
      ...
      await expect(svc.startStockTake(TENANT, "st1")).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("cancelStockTake", () => {
    it("cancels non-terminal stock take", async () => {
      ...
      vi.mocked(prisma.stockTake.update).mockResolvedValue(
        mockSt({ status: "CANCELLED" }) as never,
      );
    ...
    it("throws if already posted", async () => {
      ...
      await expect(svc.cancelStockTake(TENANT, "st1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("createCountSheet", () => {
    ...
    it("throws if stock take is not DRAFT or IN_PROGRESS", async () => {
      ...
      await expect(svc.createCountSheet(TENANT, "st1", {})).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("approveVariance", () => {
    it("approves a PENDING variance", async () => {
      ...
      vi.mocked(prisma.stockTakeVariance.update).mockResolvedValue(
    ...
    it("throws if not PENDING", async () => {
      ...
      await expect(svc.approveVariance(TENANT, "var1", USER)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("throws if not found", async () => {
      ...
      await expect(svc.approveVariance(TENANT, "bad", USER)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("rejectVariance", () => {
    it("rejects a PENDING variance", async () => {
      ...
      vi.mocked(prisma.stockTakeVariance.update).mockResolvedValue(
        mockVariance({ status: "REJECTED" }) as never,
      );
      const result = await svc.rejectVariance(
        TENANT,
        "var1",
        "Recount mismatch",
      );
  ...
  describe("approveStockTake", () => {
    it("approves when no pending variances remain", async () => {
      ...
      vi.mocked(prisma.stockTake.update).mockResolvedValue(
        mockSt({ status: "APPROVED" }) as never,
      );
    ...
    it("throws if pending variances remain", async () => {
      ...
      await expect(svc.approveStockTake(TENANT, "st1", USER)).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("postStockTake", () => {
    it("posts an APPROVED stock take", async () => {
      ...
      vi.mocked(prisma.stockTakeVariance.findMany).mockResolvedValue([
        mockVariance({ status: "APPROVED" }),
      ] as never);
      vi.mocked(prisma.stockTakeVariance.update).mockResolvedValue(
        mockVariance() as never,
      );
    ...
    it("throws if not APPROVED", async () => {
      ...
      await expect(svc.postStockTake(TENANT, "st1", USER)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
