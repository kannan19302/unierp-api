...
export class CommunicationBotsService {
  ...
  async createBot(
    ...
    const existing = await prisma.connectBot.findFirst({
      where: { tenantId, channelId, name: dto.name },
    });
    if (existing)
      throw new BadRequestException(
        `Bot "${dto.name}" already exists in this channel.`,
      );
  ...
  async updateBot(
    ...
    botId: string,
    userId: string,
    dto: { name?: string; avatar?: string; isActive?: boolean },
  ) {
    const bot = await prisma.connectBot.findFirst({
      where: { id: botId, tenantId },
    });
    if (!bot) throw new NotFoundException("Bot not found");
    const membership = await prisma.channelMember.findFirst({
      where: { channelId: bot.channelId, userId },
    });
    if (!membership || !["OWNER", "ADMIN"].includes(membership.role))
      throw new ForbiddenException();
  ...
  async postAsBot(tenantId: string, botId: string, content: string) {
    const bot = await prisma.connectBot.findFirst({
      where: { id: botId, tenantId, isActive: true },
    });
  ...
  async postViaWebhook(
    webhookUrl: string,
    payload: { content: string; username?: string; avatar?: string },
  ) {
  ...
  async setWebhookUrl(
    ...
    if (!membership || !["OWNER", "ADMIN"].includes(membership.role))
      throw new ForbiddenException();
    const existing = await prisma.connectBot.findFirst({
      where: { webhookUrl, id: { not: botId } },
    });
    if (existing)
      throw new BadRequestException(
        "Webhook URL already in use by another bot",
      );
    return prisma.connectBot.update({
      where: { id: botId },
      data: { webhookUrl },
    });
  }
