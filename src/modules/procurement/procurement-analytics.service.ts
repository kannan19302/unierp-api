...
export class ProcurementAnalyticsService {
  async getSpendByVendor(
    ...
    for (const po of orders) {
      ...
      if (!byVendor[key])
        byVendor[key] = { vendorName: po.vendor.name, total: 0, count: 0 };
      byVendor[key].total += Number(po.totalAmount);
      byVendor[key].count += 1;
    }

    return Object.entries(byVendor)
      .map(([vendorId, data]) => ({ vendorId, ...data, percentage: 0 }))
  ...
  async getBudgetOverview(tenantId: string) {
    ...
    const totalBudgeted = orders.reduce(
  ...
  async getSpendForecast(tenantId: string, months: number = 3) {
    ...
    const monthsArr = Object.entries(monthly).sort((a, b) =>
      a[0].localeCompare(b[0]),
    );
    const avgMonthlySpend =
      monthsArr.length > 0
        ? Math.round(
            (monthsArr.reduce((s, [, d]) => s + d.total, 0) /
              monthsArr.length) *
              100,
          ) / 100
        : 0;
  ...
  async getVendorComparison(tenantId: string, vendorIds?: string[]) {
    ...
    for (const v of Object.values(byVendor)) {
      v.avgOrderValue =
        v.orderCount > 0
          ? Math.round((v.totalSpend / v.orderCount) * 100) / 100
          : 0;
    }
  ...
  async getPurchaseOrderCycleTime(tenantId: string, vendorId?: string) {
    ...
    const cycleTimes = orders
      .map((po) => ({
        ...
        cycleDays:
          Math.round(
            ((po.orderDate.getTime() - po.createdAt.getTime()) /
              (1000 * 60 * 60 * 24)) *
              10,
          ) / 10,
      }))
    ...
    const avgCycleTime =
      cycleTimes.length > 0
        ? Math.round(
            (cycleTimes.reduce((s, c) => s + c.cycleDays, 0) /
              cycleTimes.length) *
              10,
          ) / 10
        : 0;
    ...
    return {
      ...
      minCycleDays:
        cycleTimes.length > 0
          ? Math.min(...cycleTimes.map((c) => c.cycleDays))
          : 0,
      maxCycleDays:
        cycleTimes.length > 0
          ? Math.max(...cycleTimes.map((c) => c.cycleDays))
          : 0,
      orders: cycleTimes,
    };
  }
  ...
  async getSavingsOpportunities(tenantId: string) {
    ...
    const vendorSpend: Record<
      string,
      { vendorName: string; totalSpend: number; orderCount: number }
    > = {};
    for (const po of orders) {
      const key = po.vendorId;
      if (!vendorSpend[key])
      ...
      .map((v) => {
        ...
        return {
          ...
          consolidationCandidate:
            (spend?.totalSpend || 0) < 10000 &&
            Number(score?.overallScore || 100) < 80,
          potentialSavings: spend
            ? Math.round(spend.totalSpend * 0.05 * 100) / 100
            : 0,
        };
      })
    ...
    return {
      ...
      vendorsNeedingImprovement: opportunities.filter((o) => o.needsImprovement)
        .length,
      consolidationCandidates: opportunities.filter(
        (o) => o.consolidationCandidate,
      ).length,
    };
  }
    ...
    return vendors
      .map((v) => {
        ...
        return {
          ...
          lastScorecardPeriod: sc
            ? `${sc.periodStart.toISOString().substring(0, 10)}`
            : null,
        };
      })
      .sort((a, b) => b.totalSpend - a.totalSpend);
  ...
  private async _getScorecardStats(tenantId: string) {
    ...
    return {
      ...
      avgOverall:
        latest.length > 0
          ? Math.round(
              (latest.reduce((s, sc) => s + Number(sc.overallScore || 0), 0) /
                latest.length) *
                100,
            ) / 100
          : 0,
      avgQuality:
        latest.length > 0
          ? Math.round(
              (latest.reduce((s, sc) => s + Number(sc.qualityScore || 0), 0) /
                latest.length) *
                100,
            ) / 100
          : 0,
      avgDelivery:
        latest.length > 0
          ? Math.round(
              (latest.reduce((s, sc) => s + Number(sc.deliveryScore || 0), 0) /
                latest.length) *
                100,
            ) / 100
          : 0,
