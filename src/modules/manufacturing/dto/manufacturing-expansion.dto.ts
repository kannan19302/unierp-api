...
export const QualityCheckTemplateSchema = z.object({
  ...
  description: z.string().optional(),
  category: z
    .enum(["INSPECTION", "TEST", "CERTIFICATION"])
    .default("INSPECTION"),
  checks: z.array(
    z.object({
      name: z.string(),
      type: z
        .enum(["PASS_FAIL", "MEASUREMENT", "VISUAL", "RANGE"])
        .default("PASS_FAIL"),
...
export const TimeEntrySchema = z.object({
  ...
  activityType: z
    .enum(["PRODUCTION", "SETUP", "DOWNTIME", "MAINTENANCE"])
    .default("PRODUCTION"),
  notes: z.string().optional(),
});
...
export type QualityCheckTemplateDto = z.infer<
  typeof QualityCheckTemplateSchema
>;
