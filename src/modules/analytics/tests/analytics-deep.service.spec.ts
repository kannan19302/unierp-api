...
describe("AnalyticsDeepService", () => {
  ...
  describe("KPI Definitions", () => {
    ...
    it("should get single KPI definition", async () => {
      vi.mocked(prisma.analyticsKpiDefinition.findFirst).mockResolvedValue({
        id: "k1",
        name: "Revenue",
        code: "REV",
        formula: "SUM(amount)",
    ...
    it("should throw on missing KPI", async () => {
      ...
      await expect(
        service.getKpiDefinition("t1", "nonexistent"),
      ).rejects.toThrow(NotFoundException);
    });

    it("should create KPI definition", async () => {
      vi.mocked(prisma.analyticsKpiDefinition.create).mockResolvedValue({
        id: "k1",
        name: "New KPI",
        code: "NKPI",
        formula: "COUNT(*)",
      ...
      const result = await service.createKpiDefinition("t1", {
        name: "New KPI",
        code: "NKPI",
        formula: "COUNT(*)",
      });
    ...
    it("should update KPI definition", async () => {
      ...
      vi.mocked(prisma.analyticsKpiDefinition.update).mockResolvedValue({
        id: "k1",
        name: "Updated KPI",
        code: "UKPI",
        formula: "SUM(*)",
      ...
      const result = await service.updateKpiDefinition("t1", "k1", {
        name: "Updated KPI",
      });
    ...
    it("should delete KPI definition", async () => {
      ...
      expect(prisma.analyticsKpiDefinition.delete).toHaveBeenCalledWith({
        where: { id: "k1" },
      });
    });
  });
  ...
  describe("Trend Analysis", () => {
    ...
    it("should compute trend analysis with raw query", async () => {
      ...
      vi.mocked(prisma.analyticsKpiValue.aggregate).mockResolvedValue({
        _avg: { value: 5000 },
      } as any);
  ...
  describe("Scheduled Exports", () => {
    ...
    it("should create scheduled export", async () => {
      vi.mocked(prisma.analyticsScheduledExport.create).mockResolvedValue({
        id: "e1",
        name: "Test Export",
        source: "INVOICE",
        format: "CSV",
      ...
      const result = await service.createScheduledExport("t1", {
        name: "Test Export",
        source: "INVOICE",
        format: "CSV",
        scheduleType: "ONCE",
  ...
  describe("Cross-Filter Dashboards", () => {
    it("should get cross-filter dashboards", async () => {
      vi.mocked(
        prisma.analyticsCrossFilterDashboard.findMany,
      ).mockResolvedValue([
        {
          id: "d1",
          name: "Sales Dashboard",
          description: "Cross-filter sales",
          tenantId: "t1",
          createdAt: new Date(),
          updatedAt: new Date(),
        },
    ...
    it("should create cross-filter dashboard", async () => {
      vi.mocked(prisma.analyticsCrossFilterDashboard.create).mockResolvedValue({
        id: "d1",
        name: "New Dashboard",
        tenantId: "t1",
      } as any);
      const result = await service.createCrossFilterDashboard("t1", {
        name: "New Dashboard",
      });
  ...
  describe("BI Metric Catalog", () => {
    ...
    it("should create BI metric", async () => {
      vi.mocked(prisma.analyticsBiMetricDefinition.create).mockResolvedValue({
        id: "b1",
        name: "New Metric",
        category: "SALES",
        source: "INVOICE",
      ...
      const result = await service.createBiMetricDefinition("t1", {
        name: "New Metric",
        category: "SALES",
        source: "INVOICE",
        expression: "COUNT(*)",
    ...
    it("should update BI metric", async () => {
      vi.mocked(prisma.analyticsBiMetricDefinition.findFirst).mockResolvedValue(
        { id: "b1", tenantId: "t1" } as any,
      );
      vi.mocked(prisma.analyticsBiMetricDefinition.update).mockResolvedValue({
        id: "b1",
        name: "Updated",
      } as any);
      const result = await service.updateBiMetricDefinition("t1", "b1", {
        name: "Updated",
      });
    ...
    it("should delete BI metric with tenant check", async () => {
      ...
      expect(prisma.analyticsBiMetricDefinition.delete).toHaveBeenCalledWith({
        where: { id: "b1" },
      });
    });
    ...
    it("should throw on missing BI metric when deleting", async () => {
      ...
      await expect(
        service.deleteBiMetricDefinition("t1", "nonexistent"),
      ).rejects.toThrow(NotFoundException);
    });
  });
});
