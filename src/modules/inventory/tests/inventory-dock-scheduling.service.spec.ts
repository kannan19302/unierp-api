...
describe("InventoryService — yard/dock appointment scheduling", () => {
  ...
  describe("createDockAppointment", () => {
    ...
    it("allows a non-overlapping booking on the same dock door", async () => {
      ...
      db.dockAppointment.create.mockResolvedValue({
        id: "appt1",
        status: "SCHEDULED",
      });
  ...
  describe("lifecycle", () => {
    it("checks in a scheduled appointment", async () => {
      db.dockAppointment.findFirst.mockResolvedValue({
        id: "appt1",
        status: "SCHEDULED",
      });
      db.dockAppointment.update.mockResolvedValue({
        id: "appt1",
        status: "CHECKED_IN",
      });
    ...
    it("throws NotFound checking in a non-scheduled appointment", async () => {
      ...
      await expect(
        service.checkInDockAppointment("t1", "appt1"),
    ...
    it("rejects completing an already-completed appointment", async () => {
      ...
      await expect(
        service.completeDockAppointment("t1", "appt1"),
      ).rejects.toThrow(BadRequestException);
    });
