...
describe("EcommercePublicService", () => {
  ...
  describe("getPublicConfig", () => {
    it("404s when the storefront is disabled, even if a config row exists", async () => {
      ...
      await expect(service.getPublicConfig("t1")).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe("tenant isolation", () => {
    ...
    it("404s a cart lookup across tenants — tenant-a cannot read tenant-b's cart via sessionToken alone", async () => {
      ...
      await expect(
        service.getCart("tenant-a", "token-belongs-to-tenant-b"),
      ).rejects.toThrow(NotFoundException);
      expect(prisma.cart.findFirst).toHaveBeenCalledWith(
        expect.objectContaining({
          where: {
            tenantId: "tenant-a",
            sessionToken: "token-belongs-to-tenant-b",
          },
        }),
      );
    });
  ...
  describe("cart", () => {
    it("rejects adding an item for a listing that is not published", async () => {
      ...
      await expect(
        service.addCartItem("t1", "session-1", {
          productListingId: "listing-unpublished",
          quantity: 1,
        }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it("snapshots the effective price (priceOverride wins over Product.sellPrice) when adding an item", async () => {
      ...
      await service.addCartItem("t1", "session-1", {
        productListingId: "listing-1",
        quantity: 2,
      });
      ...
      expect(prisma.cartItem.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ unitPriceSnapshot: 15, quantity: 2 }),
        }),
      );
    });
