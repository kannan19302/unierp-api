...
export class ProcurementSourcingService {
  async getSourcingProjects(
    tenantId: string,
    params?: { status?: string; page?: number; limit?: number },
  ) {
    const where: any = { tenantId, isActive: true };
    if (params?.status) where.status = params.status;
    const data = await prisma.sourcingProject.findMany({
      where,
      include: {
        participants: true,
  ...
  async getSourcingProjectById(tenantId: string, id: string) {
    const project = await prisma.sourcingProject.findFirst({
      where: { id, tenantId },
      include: {
        participants: true,
    ...
    return prisma.$transaction(async (tx) => {
      const project = await tx.sourcingProject.create({
        data: {
          ...
          expectedSavings: dto.expectedSavings
            ? new Prisma.Decimal(dto.expectedSavings)
            : null,
      ...
      return tx.sourcingProject.findUnique({
        where: { id: project.id },
  ...
  async updateSourcingProject(tenantId: string, id: string, dto: any) {
    ...
    if (!project) throw new NotFoundException("Sourcing project not found");
    return prisma.sourcingProject.update({
      where: { id },
      data: {
        ...dto,
        estimatedValue: dto.estimatedValue
        ...
        actualValue: dto.actualValue
          ? new Prisma.Decimal(dto.actualValue)
          : undefined,
      },
    });
  }
  ...
  async createSupplierEvaluation(tenantId: string, dto: any) {
    return prisma.$transaction(async (tx) => {
      const totalWeight =
        dto.criteria?.reduce((s: number, c: any) => s + (c.weight || 0), 0) ||
        0;
      const weightedScore =
        dto.criteria?.reduce(
      ...
      if (dto.criteria?.length) {
        await tx.supplierEvaluationCriterion.createMany({
          data: dto.criteria.map((c: any) => ({
            tenantId,
            evaluationId: evalRecord.id,
            criterionName: c.criterionName,
            weight: new Prisma.Decimal(c.weight),
            score: c.score != null ? new Prisma.Decimal(c.score) : null,
            weightedScore:
              c.score != null ? new Prisma.Decimal(c.score * c.weight) : null,
            comments: c.comments || null,
          })),
        });
      }
      return tx.supplierEvaluation.findUnique({
  ...
  async createBidAnalysis(tenantId: string, dto: any) {
    return prisma.sourcingProject
      .findFirst({ where: { id: dto.projectId, tenantId } })
      .then(async (project) => {
        if (!project) throw new NotFoundException("Sourcing project not found");
        const bids = dto.bids || [];
        const amounts = bids
        ...
        return prisma.bidAnalysis.create({
          data: {
            tenantId,
            projectId: dto.projectId,
            analysisNumber: `BA-${Date.now()}`,
            totalBids: bids.length,
            lowestBid: amounts.length
              ? new Prisma.Decimal(Math.min(...amounts))
              : null,
            highestBid: amounts.length
              ? new Prisma.Decimal(Math.max(...amounts))
              : null,
            averageBid: amounts.length
              ? new Prisma.Decimal(
                  amounts.reduce((s: number, a: number) => s + a, 0) /
                    amounts.length,
                )
              : null,
            medianBid: amounts.length
              ? new Prisma.Decimal(
              ...
              : null,
            currency: dto.currency || "USD",
            awardRecommendation: dto.awardRecommendation || null,
            notes: dto.notes || null,
          },
        });
      });
    ...
    return prisma.contractAward.create({
      data: {
        ...
        awardAmount: dto.awardAmount
          ? new Prisma.Decimal(dto.awardAmount)
          : null,
        currency: dto.currency || "USD",
        validUntil: dto.validUntil ? new Date(dto.validUntil) : null,
        termsSummary: dto.termsSummary || null,
        awardDate: new Date(),
        createdBy: userId || null,
      },
    });
  ...
  async getContractById(tenantId: string, id: string) {
    ...
    if (!contract)
      throw new NotFoundException("Procurement contract not found");
    return contract;
  }
    ...
    return prisma.$transaction(async (tx) => {
      const contract = await tx.procurementContract.create({
        data: {
          ...
          autoRenewNoticeDays: dto.autoRenewNoticeDays || null,
          contractValue: dto.contractValue
            ? new Prisma.Decimal(dto.contractValue)
            : null,
          currency: dto.currency || "USD",
          maximumValue: dto.maximumValue
            ? new Prisma.Decimal(dto.maximumValue)
            : null,
          estimatedAnnualValue: dto.estimatedAnnualValue
            ? new Prisma.Decimal(dto.estimatedAnnualValue)
            : null,
      ...
      if (dto.priceSchedules?.length) {
        await tx.procurementContractPriceSchedule.createMany({
          data: dto.priceSchedules.map((p: any) => ({
            tenantId,
            contractId: contract.id,
            productId: p.productId || null,
            productSku: p.productSku || null,
            productName: p.productName || null,
            negotiatedPrice: new Prisma.Decimal(p.negotiatedPrice),
            currency: p.currency || "USD",
            priceType: p.priceType || "UNIT",
            minQty: p.minQty ? new Prisma.Decimal(p.minQty) : null,
            maxQty: p.maxQty ? new Prisma.Decimal(p.maxQty) : null,
      ...
      if (dto.volumeCommitments?.length) {
        await tx.procurementContractVolumeCommitment.createMany({
          data: dto.volumeCommitments.map((v: any) => ({
            tenantId,
            contractId: contract.id,
            productId: v.productId || null,
            productSku: v.productSku || null,
            committedQty: new Prisma.Decimal(v.committedQty),
            commitmentPeriod: v.commitmentPeriod,
            startDate: new Date(v.startDate),
            endDate: new Date(v.endDate),
            uom: v.uom || "EA",
            rebateRate: v.rebateRate ? new Prisma.Decimal(v.rebateRate) : null,
            penaltyRate: v.penaltyRate
              ? new Prisma.Decimal(v.penaltyRate)
              : null,
          })),
        });
      }
      return tx.procurementContract.findUnique({
        ...
        include: { priceSchedules: true, volumeCommitments: true },
      });
    });
  }
  ...
  async updateContractStatus(tenantId: string, id: string, status: string) {
    ...
    return prisma.procurementContract.update({
      where: { id },
      data: updateData,
    });
  }
  ...
  async getProcurementIntelligence(tenantId: string, category?: string) {
    ...
    return prisma.procurementIntelligence.findMany({
      where,
      orderBy: { createdAt: "desc" },
      take: 20,
    });
  ...
  async generateSpendAnalysis(tenantId: string) {
    ...
    return prisma.procurementIntelligence.create({
      data: {
        ...
        reportName: `Spend Analysis ${new Date().toISOString().slice(0, 7)}`,
        reportPeriod: new Date().toISOString().slice(0, 7),
        totalSpend: new Prisma.Decimal(spendAmount),
        totalSavings: new Prisma.Decimal(spendAmount * 0.08),
        savingsTarget: new Prisma.Decimal(spendAmount * 0.12),
        savingsPct: new Prisma.Decimal(8),
        poCount,
        supplierCount: vendorCount,
        reportData: {
  ...
  async generateSupplierConcentration(tenantId: string) {
    ...
    const spendByVendor: Record<
      string,
      { vendorName: string; total: number; count: number }
    > = {};
    for (const po of purchaseOrders) {
      const vid = po.vendorId || "unknown";
      if (!spendByVendor[vid])
        spendByVendor[vid] = {
          vendorName: (po as any).vendor?.name || "Unknown",
          total: 0,
          count: 0,
        };
    ...
    const totalSpend = Object.values(spendByVendor).reduce(
      (s, v) => s + v.total,
      0,
    );
    ...
    const concentrationRisk =
      totalSpend > 0 && topVendorSpend / totalSpend > 0.3
        ? "HIGH"
        : totalSpend > 0 && topVendorSpend / totalSpend > 0.2
          ? "MEDIUM"
          : "LOW";
    return prisma.procurementIntelligence.create({
      data: {
        ...
        reportName: `Supplier Concentration ${new Date().toISOString().slice(0, 7)}`,
        reportPeriod: new Date().toISOString().slice(0, 7),
        totalSpend: new Prisma.Decimal(totalSpend),
        supplierCount: Object.keys(spendByVendor).length,
        reportData: {
          generatedAt: new Date(),
          topVendors,
          concentrationRisk,
          totalSpend,
          vendorCount: Object.keys(spendByVendor).length,
        },
  ...
  async getOnboardingWorkflows(tenantId: string, params?: { status?: string }) {
    const where: any = { tenantId, isActive: true };
    if (params?.status) where.status = params.status;
    return prisma.supplierOnboardingWorkflow.findMany({
      where,
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async createOnboardingWorkflow(tenantId: string, dto: any, userId?: string) {
    return prisma.supplierOnboardingWorkflow.create({
      data: {
        tenantId,
        vendorId: dto.vendorId,
        vendorName: dto.vendorName || null,
        ...
        documents: dto.documents || Prisma.JsonNull,
        bankInfo: dto.bankInfo || Prisma.JsonNull,
        insuranceInfo: dto.insuranceInfo || Prisma.JsonNull,
        startedAt: new Date(),
        createdBy: userId || null,
      },
    });
  ...
  async updateOnboardingStep(tenantId: string, id: string, step: string) {
    const wf = await prisma.supplierOnboardingWorkflow.findFirst({
      where: { id, tenantId },
    });
    ...
    return prisma.supplierOnboardingWorkflow.update({
      where: { id },
      data: updateData,
    });
  }
