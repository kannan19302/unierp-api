...
export class ExpenseManagementService {
  ...
  private assertDraft(report: { status: string } | null) {
    ...
    if (report.status !== "DRAFT") {
      throw new BadRequestException(
        "Expense items can only be modified while the report is DRAFT.",
      );
    }
  }
  ...
  async addExpenseItem(
    ...
    if (dto.isMileage) {
      ...
      if (!rate)
        throw new BadRequestException(
          "No mileage rate configured for this date.",
        );
      mileageRateApplied = Number(rate.ratePerMile);
      amount = Number(
        (mileageRateApplied * (dto.mileageDistance || 0)).toFixed(2),
      );
    }
    ...
    if (dto.isPerDiem) {
      const rate = await prisma.perDiemRate.findFirst({
        where: {
          tenantId,
          location: dto.perDiemLocation || "DEFAULT",
          isActive: true,
        },
      });
      if (!rate)
        throw new BadRequestException(
          "No per-diem rate configured for this location.",
        );
      perDiemRateApplied = Number(rate.dailyRate);
      amount = Number((perDiemRateApplied * (dto.perDiemDays || 0)).toFixed(2));
    }

    const { violation, reason } = await this.checkPolicy(
      tenantId,
      dto.category,
      amount,
      !!dto.receiptUrl,
    ...
    const item = await prisma.expenseReportItem.create({
      data: {
        ...
        mileageDistance:
          dto.mileageDistance != null
            ? new Prisma.Decimal(dto.mileageDistance)
            : null,
        mileageRateApplied:
          mileageRateApplied != null
            ? new Prisma.Decimal(mileageRateApplied)
            : null,
        isPerDiem: !!dto.isPerDiem,
        perDiemDays:
          dto.perDiemDays != null ? new Prisma.Decimal(dto.perDiemDays) : null,
        perDiemRateApplied:
          perDiemRateApplied != null
            ? new Prisma.Decimal(perDiemRateApplied)
            : null,
  ...
  async updateExpenseItem(
    ...
    const item = await prisma.expenseReportItem.findFirst({
      where: { id: itemId, tenantId },
    });
    if (!item) throw new NotFoundException("Expense item not found");
    const report = await prisma.expenseReport.findFirst({
      where: { id: item.expenseReportId, tenantId },
    });
    this.assertDraft(report);

    const nextAmount =
      dto.amount != null ? Number(dto.amount) : Number(item.amount);
    const nextCategory = (dto.category as string) || item.category;
    const nextReceipt =
      dto.receiptUrl != null ? String(dto.receiptUrl) : item.receiptUrl;
    const { violation, reason } = await this.checkPolicy(
      tenantId,
      nextCategory,
      nextAmount,
  ...
  private async checkPolicy(
    ...
    if (
      policy.maxAmountPerItem != null &&
      amount > Number(policy.maxAmountPerItem)
    ) {
      reasons.push(
        `Exceeds per-item limit of ${policy.maxAmountPerItem} for ${category}`,
      );
    }
    if (
      Number(policy.receiptRequiredAbove) > 0 &&
      amount > Number(policy.receiptRequiredAbove) &&
      !hasReceipt
    ) {
      reasons.push(
        `Receipt required for ${category} expenses above ${policy.receiptRequiredAbove}`,
      );
    }
    return {
  ...
  async scanReceipt(
    ...
    const amountMatch =
      text.match(/(?:total|amount)\s*[:$]?\s*\$?([0-9]+\.[0-9]{2})/i) ||
  ...
  async submitExpenseReport(tenantId: string, reportId: string) {
    ...
    if (report.status !== "DRAFT") {
      throw new BadRequestException(
        "Only DRAFT expense reports can be submitted.",
      );
    }
    const itemCount = await prisma.expenseReportItem.count({
      where: { expenseReportId: reportId },
    });
    if (itemCount === 0) {
      throw new BadRequestException(
        "Cannot submit an expense report with no line items.",
      );
    }
  ...
  async approveExpenseReport(
    ...
    if (report.status !== "SUBMITTED") {
      throw new BadRequestException(
        "Only SUBMITTED expense reports can be approved.",
      );
    }
    const nextStatus = report.requiresSecondApproval
  ...
  async secondApproveExpenseReport(
    ...
    if (report.status !== "PENDING_SECOND_APPROVAL") {
      throw new BadRequestException(
        "Only reports PENDING_SECOND_APPROVAL can receive second-level approval.",
      );
    }
    const updated = await prisma.expenseReport.update({
      where: { id: reportId },
  ...
  async rejectExpenseReport(
    ...
    if (
      ...
      throw new BadRequestException(
        "Only SUBMITTED or PENDING_SECOND_APPROVAL expense reports can be rejected.",
      );
    }
    const updated = await prisma.expenseReport.update({
      where: { id: reportId },
  ...
  async markExpenseReportPaid(tenantId: string, reportId: string) {
    ...
    if (!journal)
      throw new BadRequestException("Failed to post reimbursement journal.");
      ...
      entries: [
        {
          ...
          debit: Number(report.totalAmount),
          credit: 0,
        ...
        },
        {
          ...
          debit: 0,
          credit: Number(report.totalAmount),
  ...
  async getPolicies(tenantId: string) {
    return prisma.expenseCategoryPolicy.findMany({
      where: { tenantId },
  ...
  async upsertPolicy(
    ...
    return prisma.expenseCategoryPolicy.upsert({
      ...
      create: {
        ...
        category: dto.category,
        maxAmountPerItem:
          dto.maxAmountPerItem != null
            ? new Prisma.Decimal(dto.maxAmountPerItem)
            : null,
      ...
      update: {
        ...
        receiptRequiredAbove:
          dto.receiptRequiredAbove != null
            ? new Prisma.Decimal(dto.receiptRequiredAbove)
            : undefined,
  ...
  async deletePolicy(tenantId: string, id: string) {
    ...
    if (!policy)
      throw new NotFoundException("Expense category policy not found");
  ...
  async getMileageRates(tenantId: string) {
    return prisma.mileageRate.findMany({
      where: { tenantId },
      orderBy: { effectiveDate: "desc" },
    });
  }
  ...
  async getPerDiemRates(tenantId: string) {
    return prisma.perDiemRate.findMany({
      where: { tenantId },
  ...
  async getCorporateCards(tenantId: string) {
    ...
    const employeeIds = [...new Set(cards.map((c) => c.employeeId))];
    const employees = await prisma.employee.findMany({
    ...
    return cards.map((c) => {
      ...
      const limit = c.spendLimits[0];
      return {
        id: c.id,
        cardHolderName: employee
          ? `${employee.firstName} ${employee.lastName}`
          : c.nickname || "Unassigned",
        lastFourDigits: c.last4,
        provider: c.provider,
        departmentId: employee?.departmentId ?? null,
        monthlyLimit: limit ? Number(limit.amountCap) : 0,
        currentSpend: limit ? Number(limit.currentSpend) : 0,
        status: c.isFrozen ? "FROZEN" : c.isActive ? "ACTIVE" : "INACTIVE",
      };
    });
  }
  ...
  async importCardTransactions(
    ...
    for (const t of transactions) {
      const auth = await this.cardSpendLimitService.checkAuthorization(
        tenantId,
        cardId,
        t.amount,
        t.mccCategory,
  ...
  async matchCardTransactionToItem(
    tenantId: string,
    transactionId: string,
    itemId: string,
  ) {
    const transaction = await prisma.corporateCardTransaction.findFirst({
