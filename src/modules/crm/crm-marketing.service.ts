...
export class CrmMarketingService {
  ...
  async createCampaign(
    ...
    if (existing)
      throw new BadRequestException(
        `Campaign with name "${dto.name}" already exists.`,
      );
  ...
  async getWorkflowRules(tenantId: string) {
    return prisma.crmWorkflowRule.findMany({
      where: { tenantId, deletedAt: null },
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async updateWorkflowRule(
    ...
    const existing = await prisma.crmWorkflowRule.findFirst({
  ...
  async enrollInSequence(
    ...
    const seq = await prisma.emailSequence.findFirst({
      where: { id: sequenceId, tenantId, deletedAt: null, isActive: true },
    });
    if (!seq) throw new NotFoundException("Sequence not found or inactive");
    if (!dto.contactId && !dto.leadId)
      throw new BadRequestException("Must provide contactId or leadId");
  ...
  async getWebForms(tenantId: string) {
    return prisma.webToLeadForm.findMany({
      where: { tenantId, deletedAt: null },
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async updateWebForm(
    tenantId: string,
    id: string,
    dto: UpdateWebToLeadFormInput,
  ) {
    const existing = await prisma.webToLeadForm.findFirst({
      where: { id, tenantId, deletedAt: null },
    });
  ...
  async submitWebForm(formId: string, dto: SubmitWebFormInput) {
    ...
    await prisma.webToLeadForm.update({
      where: { id: formId },
