...
export class ProcurementApprovalsService {
  ...
  async approvePurchaseOrder(tenantId: string, id: string) {
    ...
    if (!["SUBMITTED", "PENDING_APPROVAL"].includes(po.status)) {
      throw new BadRequestException(
        `Purchase order status ${po.status} cannot be approved`,
      );
    }
  ...
  async rejectPurchaseOrder(tenantId: string, id: string, _reason?: string) {
    ...
    if (!["SUBMITTED", "PENDING_APPROVAL"].includes(po.status)) {
      throw new BadRequestException(
        `Purchase order status ${po.status} cannot be rejected`,
      );
    }
    ...
    return {
      ...
      purchaseOrders: {
        ...
        pending: purchaseOrders.filter(
          (p) => p.status === "SUBMITTED" || p.status === "PENDING_APPROVAL",
        ).length,
  ...
  async getApprovalHistory(
    ...
    if (params.type === "requisition")
      whereReqs.status = { in: ["APPROVED", "REJECTED"] };
    if (params.type === "purchase-order")
      wherePos.status = { in: ["APPROVED", "CANCELLED"] };
    ...
    if (params.type === "requisition" || !params.type) {
      ...
      if (params.type === "requisition") {
        return paginatedResult(
          requisitions.map((r) => ({ ...r, type: "requisition" })),
          total,
          params,
        );
      }
    ...
    return paginatedResult(
      purchaseOrders.map((po) => ({ ...po, type: "purchase-order" })),
      totalPo,
      params,
    );
  }
  ...
  async delegateApproval(
    tenantId: string,
    userId: string,
    dto: { delegateToUserId: string; fromDate: string; toDate: string },
  ) {
    return {
      tenantId,
      delegatedBy: userId,
  ...
  async getApprovalStatistics(tenantId: string) {
    ...
    const avgRequisitionApprovalTime =
      approvedReqs
      ...
      purchaseOrders: {
        ...
        approvalRate:
          purchaseOrders.length > 0
            ? Math.round((approvedPos.length / purchaseOrders.length) * 10000) /
              100
            : 0,
      },
    };
