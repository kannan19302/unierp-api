...
vi.mock("@unerp/database", () => {
  ...
  return {
    prisma: {
      transferPricingPolicy: m(),
      transferPricingAdjustment: m(),
      apportionmentFactor: m(),
    },
  };
});
...
describe("GlobalTaxDeepService", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    service = new GlobalTaxDeepService();
  });

  it("getTransferPricingPolicies returns policies", async () => {
    vi.mocked(prisma.transferPricingPolicy.findMany).mockResolvedValue([
      { id: "tp-1" },
    ] as any);
  ...
  it("getTransferPricingPolicyById throws on not found", async () => {
    ...
    await expect(
      service.getTransferPricingPolicyById("t1", "bad"),
    ).rejects.toThrow(NotFoundException);
  });

  it("createTransferPricingPolicy creates record", async () => {
    vi.mocked(prisma.transferPricingPolicy.create).mockResolvedValue({
      id: "tp-1",
    } as any);
    const result = await service.createTransferPricingPolicy("t1", "o1", {
      policyName: "IP Royalty",
      policyType: "ROYALTIES",
      method: "CUP",
  ...
  it("approveTransferPricingPolicy updates status", async () => {
    vi.mocked(prisma.transferPricingPolicy.findFirst).mockResolvedValue({
      id: "tp-1",
      isActive: false,
    } as any);
    vi.mocked(prisma.transferPricingPolicy.update).mockResolvedValue({
      id: "tp-1",
      isActive: true,
    } as any);
    const result = await service.approveTransferPricingPolicy(
      "t1",
      "tp-1",
      "user1",
    );
  ...
  it("getTransferPricingAdjustments returns adjustments", async () => {
    vi.mocked(prisma.transferPricingAdjustment.findMany).mockResolvedValue([
      { id: "adj-1" },
    ] as any);
  ...
  it("createTransferPricingAdjustment computes direction", async () => {
    ...
    vi.mocked(prisma.transferPricingAdjustment.create).mockResolvedValue({
      id: "adj-1",
      adjustmentDirection: "INCREASE",
    } as any);
    const result = await service.createTransferPricingAdjustment("t1", "o1", {
      policyId: "tp-1",
  ...
  it("reviewTransferPricingAdjustment throws if not draft", async () => {
    vi.mocked(prisma.transferPricingAdjustment.findFirst).mockResolvedValue({
      id: "adj-1",
      status: "REVIEWED",
    } as any);
    await expect(
      service.reviewTransferPricingAdjustment("t1", "adj-1", "user1"),
  ...
  it("approveTransferPricingAdjustment throws if not reviewed", async () => {
    ...
    await expect(
      service.approveTransferPricingAdjustment("t1", "adj-1", "user1"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("postTransferPricingAdjustment throws if not approved", async () => {
    ...
    await expect(
      service.postTransferPricingAdjustment("t1", "adj-1"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("createApportionmentFactor computes factor pct", async () => {
    vi.mocked(prisma.apportionmentFactor.create).mockResolvedValue({
      id: "af-1",
    } as any);
    const result = await service.createApportionmentFactor("t1", "o1", {
      fiscalYear: "2026",
      jurisdiction: "CA",
      factorType: "SALES",
      numerator: 500000,
    ...
    vi.mocked(prisma.apportionmentFactor.findMany).mockResolvedValue([
      {
        jurisdiction: "CA",
        factorType: "SALES",
        factorPct: new Prisma.Decimal(0.25),
      } as any,
      {
        jurisdiction: "CA",
        factorType: "PROPERTY",
        factorPct: new Prisma.Decimal(0.15),
      } as any,
    ]);
    const result = await service.computeEffectiveApportionment(
      "t1",
      "o1",
      "2026",
    );
  ...
  it("computeEffectiveApportionment throws if no factors", async () => {
    vi.mocked(prisma.apportionmentFactor.findMany).mockResolvedValue([]);
    await expect(
      service.computeEffectiveApportionment("t1", "o1", "2026"),
    ).rejects.toThrow(NotFoundException);
  });
  ...
  it("getTransferPricingSummary aggregates by status", async () => {
    ...
    vi.mocked(prisma.transferPricingAdjustment.findMany).mockResolvedValue([
      { adjustmentAmount: 100, status: "DRAFT" },
    ] as any);
