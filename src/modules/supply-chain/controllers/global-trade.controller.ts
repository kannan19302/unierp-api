...
export class GlobalTradeController {
  ...
  listHsCodes(
    ...
    return this.tradeSvc.getHsCodes(req.user.tenantId, {
      search,
  ...
  listImportDeclarations(
    ...
    return this.tradeSvc.getImportDeclarations(req.user.tenantId, {
      page: page ? Number(page) : undefined,
      limit: limit ? Number(limit) : undefined,
      status,
    });
  ...
  createImportDeclaration(
    @Req() req: AuthRequest,
    @ZodBody(createImportDeclarationSchema)
    body: z.infer<typeof createImportDeclarationSchema>,
  ) {
    return this.tradeSvc.createImportDeclaration(
      req.user.tenantId,
      body,
      req.user.userId,
    );
  ...
  updateImportStatus(
    ...
    return this.tradeSvc.updateImportDeclarationStatus(
      req.user.tenantId,
      id,
      body.status,
    );
  ...
  listExportDeclarations(
    @Req() req: AuthRequest,
    @Query("page") page?: string,
    @Query("limit") limit?: string,
    @Query("status") status?: string,
  ) {
    return this.tradeSvc.getExportDeclarations(req.user.tenantId, {
      page: page ? Number(page) : undefined,
      limit: limit ? Number(limit) : undefined,
      status,
    });
  ...
  createExportDeclaration(
    @Req() req: AuthRequest,
    @ZodBody(createExportDeclarationSchema)
    body: z.infer<typeof createExportDeclarationSchema>,
  ) {
    return this.tradeSvc.createExportDeclaration(
      req.user.tenantId,
      body,
      req.user.userId,
    );
  ...
  listScreenings(
    ...
    return this.tradeSvc.getComplianceScreenings(req.user.tenantId, {
      status,
      screenType,
    });
  }
  ...
  createScreening(
    ...
    return this.tradeSvc.createComplianceScreening(
      req.user.tenantId,
      body,
      req.user.userId,
    );
  ...
  resolveScreening(
    @Req() req: AuthRequest,
    @Param("id") id: string,
    @ZodBody(z.object({ resolution: z.string() })) body: { resolution: string },
  ) {
    return this.tradeSvc.resolveComplianceScreening(
      req.user.tenantId,
      id,
      body.resolution,
      req.user.userId,
