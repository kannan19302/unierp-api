...
describe("CrmPipelineRiskService", () => {
  ...
  it("creates a STAGE_STALL alert for a deal that exceeded its stage threshold", async () => {
    ...
    (
      prisma.pipelineRiskAlert.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue(null);
    (
      prisma.pipelineRiskAlert.findMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue([]);
    (
      prisma.pipelineRiskAlert.create as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
    ...
    expect(prisma.pipelineRiskAlert.create).toHaveBeenCalledWith(
    ...
    expect(emit).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ opportunityId: "opp1" }),
    );
  });
  ...
  it("auto-resolves alerts whose deal no longer meets any risk condition", async () => {
    ...
    (prisma.activity.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: "act1",
    }); // has recent activity -> no NO_ACTIVITY risk
    (
      prisma.pipelineRiskAlert.findMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue([
    ...
    (
      prisma.pipelineRiskAlert.update as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});

    const result = await service.recomputeAlerts(TENANT);

    expect(result.resolved).toBe(1);
    expect(prisma.pipelineRiskAlert.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: "alert1" },
  ...
  it("acknowledgeAlert rejects an already-resolved alert", async () => {
    ...
    await expect(
      service.acknowledgeAlert(TENANT, "a1", "user1"),
  ...
  it("acknowledgeAlert throws NotFoundException for a missing alert", async () => {
    ...
    await expect(
      service.acknowledgeAlert(TENANT, "missing", "user1"),
  ...
  it("snoozeAlert sets status SNOOZED with a future snoozedUntil", async () => {
    (
      prisma.pipelineRiskAlert.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "a1", status: "OPEN" });
    (
      prisma.pipelineRiskAlert.update as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
