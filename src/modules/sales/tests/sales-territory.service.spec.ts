...
describe("SalesTerritoryService", () => {
  ...
  beforeEach(() => {
    service = new SalesTerritoryService();
    vi.clearAllMocks();
  });

  it("should list territories", async () => {
    vi.mocked(prisma.salesTerritory.findMany).mockResolvedValue([
      {
        id: "t1",
        name: "North America",
  ...
  it("should get territory by id", async () => {
    vi.mocked(prisma.salesTerritory.findFirst).mockResolvedValue({
      id: "t1",
      name: "North America",
      parent: null,
      children: [],
  ...
  it("should create territory", async () => {
    ...
    vi.mocked(prisma.salesTerritory.create).mockResolvedValue({
      id: "t1",
      name: "EMEA",
    } as never);
    const result = await service.createTerritory("tenant-1", "org-1", {
      name: "EMEA",
    });
  ...
  it("should add and remove member", async () => {
    ...
    vi.mocked(prisma.salesTeamMember.create).mockResolvedValue({
      id: "m1",
    } as never);
    await service.addMember("tenant-1", "t1", { userId: "u1" });

    vi.mocked(prisma.salesTeamMember.findFirst).mockResolvedValue({
      id: "m1",
    } as never);
  ...
  it("should create assignment rule", async () => {
    ...
    vi.mocked(prisma.territoryAssignmentRule.create).mockResolvedValue({
      id: "r1",
    } as never);
    const result = await service.createRule("tenant-1", "org-1", {
      territoryId: "t1",
      name: "West Coast Rule",
      ruleType: "GEOGRAPHY",
    });
  ...
  it("should assign entity to territory", async () => {
    ...
    const result = await service.assignEntity("tenant-1", {
      territoryId: "t1",
      entityType: "LEAD",
      entityIds: ["e1", "e2"],
    });
  ...
  it("should create forecast", async () => {
    ...
    vi.mocked(prisma.salesTerritoryForecast.create).mockResolvedValue({
      id: "f1",
      period: "2026-Q3",
    } as never);
    const result = await service.createForecast("tenant-1", "org-1", {
      territoryId: "t1",
      period: "2026-Q3",
  ...
  it("should realign territory", async () => {
    ...
    vi.mocked(prisma.salesTerritory.update).mockResolvedValue({
      id: "t1",
    } as never);
    const result = await service.realignTerritory(
      "tenant-1",
      "org-1",
      { territoryId: "t1", newManagerId: "new-mgr", reason: "Reorg" },
      "user-1",
    );
    expect(result).toBeDefined();
  });
  ...
  it("should return territory analytics", async () => {
    ...
    vi.mocked(prisma.salesTerritoryForecast.findMany).mockResolvedValue([
      { pipelineValue: 1000000, forecastValue: 800000 },
    ] as never);
