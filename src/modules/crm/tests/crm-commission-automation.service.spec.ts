...
describe("CrmCommissionAutomationService", () => {
  ...
  it("rejects a tier where maxAttainmentPct <= minAttainmentPct", async () => {
    (
      prisma.commissionPlan.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "plan1" });
    await expect(
      service.addTier(TENANT, "plan1", {
        minAttainmentPct: 50,
        maxAttainmentPct: 40,
        commissionRate: 5,
        sortOrder: 0,
  ...
  it("calculates a tiered payout with the correct accelerator band and no SPIFFs", async () => {
    ...
    (
      prisma.commissionPayout.upsert as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "payout1", totalPayout: 14400 });
    (
      prisma.commissionPayoutSpiffLine.deleteMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});

    const results = await service.calculatePayouts(
      TENANT,
      "org-system-default",
      { planId: "plan1", period: "2026-Q3" },
    );

    expect(results).toHaveLength(1);
    // attainment = 120000/100000 = 120% -> top tier (100%+, rate 12%) -> 120000 * 0.12 = 14400
    expect(prisma.commissionPayout.upsert).toHaveBeenCalledWith(
    ...
    expect(emit).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ userId: "user-a", planId: "plan1" }),
    );
  });
  ...
  it("applies a NEW_LOGO spiff bonus for a customer with no prior wins", async () => {
    ...
    (prisma.quota.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([
      { userId: "user-a", amount: 50000, period: "2026-Q3" },
    ]);
    (prisma.opportunity.findMany as ReturnType<typeof vi.fn>).mockResolvedValue(
      [
        {
    ...
    (
      prisma.commissionPayout.upsert as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "payout1" });
    (
      prisma.commissionPayoutSpiffLine.deleteMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
    (
      prisma.commissionPayoutSpiffLine.createMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});

    await service.calculatePayouts(TENANT, "org-system-default", {
      planId: "plan1",
      period: "2026-Q3",
    });
  ...
  it("throws when no Quota rows exist for the period", async () => {
    (
      ...
      id: "plan1",
      tiers: [
        { minAttainmentPct: 0, maxAttainmentPct: null, commissionRate: 10 },
      ],
      spiffs: [],
    });
    (prisma.quota.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([]);
    ...
    await expect(
      service.calculatePayouts(TENANT, "org-system-default", {
        planId: "plan1",
        period: "2026-Q3",
      }),
  ...
  it("approvePayout rejects a payout that is not DRAFT", async () => {
    (
      prisma.commissionPayout.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "p1", status: "APPROVED" });
    await expect(
      service.approvePayout(TENANT, "p1", "user-approver"),
  ...
  it("markPaid rejects a payout that is not APPROVED", async () => {
    ...
    await expect(service.markPaid(TENANT, "p1")).rejects.toThrow(
