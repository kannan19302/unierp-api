...
export class WarehouseOpsService {
  ...
  async createTask(
    tenantId: string,
    dto: z.infer<typeof createWarehouseTaskSchema>,
  ) {
  ...
  async getBinTransfer(tenantId: string, id: string) {
    const record = await prisma.binTransferRequest.findFirst({
      where: { id, tenantId },
    });
  ...
  async createBinTransfer(
    tenantId: string,
    dto: z.infer<typeof createBinTransferSchema>,
  ) {
    if (dto.fromBin === dto.toBin)
  ...
  async completeBinTransfer(tenantId: string, id: string) {
    ...
    if (record.status !== "APPROVED" && record.status !== "IN_PROGRESS") {
      throw new BadRequestException(
        "Transfer must be APPROVED or IN_PROGRESS to complete",
      );
    }
    return prisma.binTransferRequest.update({
      where: { id },
  ...
  async verifyGrn(
    tenantId: string,
    id: string,
    dto: z.infer<typeof verifyGrnLinesSchema>,
  ) {
    const grn = await prisma.goodsReceiptNote.findFirst({
      where: { id, tenantId },
    });
    if (!grn) throw new NotFoundException("GRN not found");
    if (grn.status !== "DRAFT")
      throw new BadRequestException("GRN must be in DRAFT status to verify");
    ...
    await prisma.$transaction([
      ...
      prisma.goodsReceiptNote.update({
        where: { id },
  ...
  async qualityCheckGrn(tenantId: string, id: string) {
    ...
    if (grn.status !== "VERIFIED")
      throw new BadRequestException(
        "GRN must be VERIFIED before quality check",
      );
  ...
  async completeGrnPutaway(tenantId: string, id: string) {
    ...
    if (!["VERIFIED", "QUALITY_CHECK"].includes(grn.status)) {
      throw new BadRequestException(
        "GRN must be VERIFIED or QUALITY_CHECK to complete put-away",
      );
    }
    return prisma.goodsReceiptNote.update({
      where: { id },
  ...
  async rejectGrn(tenantId: string, id: string, reason: string) {
    ...
    if (!grn) throw new NotFoundException("GRN not found");
    if (["COMPLETE", "REJECTED"].includes(grn.status))
  ...
  async getGrnDashboard(tenantId: string) {
    const [draft, verified, qualityCheck, complete, rejected] =
      await Promise.all([
        ...
        prisma.goodsReceiptNote.count({
          where: { tenantId, status: "QUALITY_CHECK" },
        }),
        prisma.goodsReceiptNote.count({
  ...
  async createPackingSession(
    tenantId: string,
    dto: z.infer<typeof createPackingSessionSchema>,
  ) {
  ...
  async addCarton(
    tenantId: string,
    sessionId: string,
    dto: z.infer<typeof addCartonSchema>,
  ) {
    const session = await prisma.packingSession.findFirst({
        ...
        data: {
          ...
          length:
            dto.length !== undefined
              ...
              : undefined,
          width:
            dto.width !== undefined ? new Prisma.Decimal(dto.width) : undefined,
          height:
  ...
  async sealCarton(tenantId: string, sessionId: string, cartonId: string) {
    const carton = await prisma.packingCarton.findFirst({
      where: { id: cartonId, sessionId, tenantId },
    });
    if (!carton)
      throw new NotFoundException("Carton not found in this session");
    if (carton.sealedAt) throw new BadRequestException("Carton already sealed");
    return prisma.packingCarton.update({
      where: { id: cartonId },
  ...
  async completePackingSession(tenantId: string, id: string) {
    ...
    if (session.status !== "OPEN")
      throw new BadRequestException("Session is not OPEN");
    if (session.cartons.length === 0)
      throw new BadRequestException("Cannot complete session with no cartons");

    const totalWeight = session.cartons.reduce(
    ...
    return prisma.packingSession.update({
      ...
      data: {
        ...
        completedAt: new Date(),
        totalWeight:
          totalWeight > 0 ? new Prisma.Decimal(totalWeight) : undefined,
      },
      include: { cartons: true },
    });
  }
