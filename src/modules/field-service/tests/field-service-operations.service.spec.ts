...
describe("FieldServiceOperationsService", () => {
  ...
  beforeEach(() => {
    mockPrisma = {
      fieldServiceWarranty: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "wrn-1", assetId: "ast-1" }]),
        create: vi
          .fn()
          .mockImplementation(({ data }) =>
            Promise.resolve({ id: "wrn-1", ...data }),
          ),
      },
      fieldServiceWorkOrderExpense: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "exp-1", workOrderId: "wo-1" }]),
        create: vi
          .fn()
          .mockImplementation(({ data }) =>
      ...
      fieldServiceInspectionChecklist: {
        findMany: vi
          .fn()
          .mockResolvedValue([{ id: "chk-1", workOrderId: "wo-1" }]),
        create: vi
          .fn()
          .mockImplementation(({ data }) =>
  ...
  it("should list warranties", async () => {
    const result = await service.getWarranties("tenant-1", {
      assetId: "ast-1",
    });
  ...
  it("should create inspection checklist", async () => {
    const data = {
      workOrderId: "wo-1",
      title: "Pre-service Safety Inspection",
    };
