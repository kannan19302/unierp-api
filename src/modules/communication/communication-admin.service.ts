...
export class CommunicationAdminService {
  ...
  async getModeration(tenantId: string, channelId: string) {
    const mod = await prisma.channelModeration.findFirst({
  ...
  async setSlowMode(
    ...
    if (!valid.includes(slowModeSecs))
      throw new BadRequestException(
        `Invalid slow mode. Use: ${valid.join(", ")}`,
      );
  ...
  async setWhoCanPost(
    ...
    if (!["EVERYONE", "ADMINS_ONLY", "MODERATORS_ONLY"].includes(whoCanPost))
      throw new BadRequestException();
    const membership = await prisma.channelMember.findFirst({
      where: { channelId, userId },
    });
  ...
  async canUserPost(
    ...
    if (mod.whoCanPost === "ADMINS_ONLY")
      return ["OWNER", "ADMIN"].includes(membership.role);
    if (mod.whoCanPost === "MODERATORS_ONLY")
      return ["OWNER", "ADMIN"].includes(membership.role);
    return true;
  }
  ...
  async updateTab(
    ...
    const tab = await prisma.channelTab.findFirst({
      where: { id: tabId, tenantId },
    });
    if (!tab) throw new NotFoundException("Tab not found");
    const membership = await prisma.channelMember.findFirst({
      where: { channelId: tab.channelId, userId },
    });
    if (!membership || !["OWNER", "ADMIN"].includes(membership.role))
      throw new ForbiddenException();
  ...
  async getChannelAnalytics(tenantId: string, channelId: string, days = 30) {
    ...
    const total = snapshots.reduce(
      ...
      { messageCount: 0, activeUsers: 0, reactions: 0 },
    );
    ...
    for (const channel of channels) {
      const [messageCount, activeUsers, reactions] = await Promise.all([
        prisma.message.count({
          where: { tenantId, channelId: channel.id, createdAt: { gte: today } },
        }),
        prisma.message
          .groupBy({
            by: ["userId"],
        ...
        prisma.messageReaction.count({
          where: {
            message: { channelId: channel.id, tenantId },
            createdAt: { gte: today },
          },
        }),
      ]);
