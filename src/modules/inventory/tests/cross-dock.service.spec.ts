...
describe("CrossDockService", () => {
  ...
  it("createOrder — zero qty throws", async () => {
    await expect(
      svc.createOrder(tenantId, userId, {
        warehouseId: "wh-1",
        productId: "p1",
        expectedQty: 0,
      }),
  ...
  it("receiveGoods — transitions PENDING→RECEIVING", async () => {
    ...
    vi.mocked(prisma.crossDockOrder.update).mockResolvedValue({
      id: "o1",
      status: "RECEIVING",
    } as never);
    vi.mocked(prisma.crossDockEvent.create).mockResolvedValue({} as never);
    ...
    expect(vi.mocked(prisma.crossDockOrder.update)).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: "RECEIVING" }),
      }),
    );
  ...
  it("receiveGoods — cancelled order throws", async () => {
    ...
    await expect(svc.receiveGoods(tenantId, userId, "o1", 10)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("stageOrder — RECEIVING→STAGING", async () => {
    ...
    expect(vi.mocked(prisma.crossDockOrder.update)).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: "STAGING" }),
      }),
    );
  });
  ...
  it("stageOrder — non-RECEIVING status throws", async () => {
    ...
    await expect(svc.stageOrder(tenantId, userId, "o1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("cancelOrder — completed order throws", async () => {
    ...
    await expect(
      svc.cancelOrder(tenantId, userId, "o1", "reason"),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("getDashboard — returns aggregates", async () => {
    ...
    vi.mocked(prisma.crossDockStation.groupBy).mockResolvedValue([
