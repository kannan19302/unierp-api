...
export class CrmCampaignManagementService {
  ...
  async buildAudience(
    ...
    if (criteria.entityType === "LEAD") {
      ...
      for (const f of criteria.filters) {
        if (f.field === "status")
          (where as Record<string, unknown>)["status"] = f.value;
        if (f.field === "source")
          (where as Record<string, unknown>)["source"] = f.value;
        if (f.field === "industry")
          (where as Record<string, unknown>)["industry"] = {
            contains: f.value,
            mode: "insensitive",
          };
      }
  ...
  async getCampaignROI(
    ...
    return {
      ...
      roi,
      costPerLead:
        leadsGenerated > 0 ? Math.round(totalCost / leadsGenerated) : 0,
      costPerConversion:
        wonDeals.length > 0 ? Math.round(totalCost / wonDeals.length) : 0,
      leadsGenerated,
  ...
  async getCampaignMembers(
    tenantId: string,
    campaignId: string,
    _filters?: { status?: string; page?: number; limit?: number },
  ): Promise<{
    data: Array<{
      id: string;
      name: string;
  ...
  async getAttributionAnalysis(
    ...
    const channelMap = new Map<
      string,
      { revenue: number; touchpoints: number }
    >();
    for (const opp of wonOpps) {
      for (const a of opp.activities) {
        ...
        if (model === "LINEAR" && opp.activities.length > 0) {
          ...
          model === "LAST_TOUCH" &&
          opp.activities[opp.activities.length - 1]?.id === a.id
        ) {
          entry.revenue += Number(opp.amount || 0);
        }
    ...
    return {
      ...
      channels: Array.from(channelMap.entries())
        .map(([channel, v]) => ({
          ...
          pct:
            totalRevenue > 0 ? Math.round((v.revenue / totalRevenue) * 100) : 0,
        }))
  ...
  async getMarketingLists(tenantId: string): Promise<
    Array<{
      id: string;
      name: string;
      type: "STATIC" | "DYNAMIC";
  ...
  async getCampaignPipelineInfluence(tenantId: string): Promise<
    Array<{
      campaignId: string;
      campaignName: string;
      touchedDeals: number;
  ...
  async getMarketingFunnel(tenantId: string): Promise<
    Array<{
      stage: string;
      count: number;
      conversionRate: number;
  ...
  > {
    ...
    const qualifiedLeads = await prisma.lead.count({
      where: { tenantId, deletedAt: null, status: "QUALIFIED" },
    });
    ...
    return [
      { stage: "Visitors", count: visitors, conversionRate: 100, value: 0 },
      {
        ...
        count: leadCount,
        conversionRate:
          visitors > 0 ? Math.round((leadCount / visitors) * 100) : 0,
        value: 0,
      },
      {
        stage: "MQL",
        count: mql,
        conversionRate: leadCount > 0 ? Math.round((mql / leadCount) * 100) : 0,
        value: 0,
      },
      {
        stage: "SQL",
        count: sql,
        conversionRate: mql > 0 ? Math.round((sql / mql) * 100) : 0,
        value: 0,
      },
      {
        stage: "Opportunity",
        count: oppCount,
        conversionRate: sql > 0 ? Math.round((oppCount / sql) * 100) : 0,
        value: 0,
      },
      {
        stage: "Customer",
        count: wonCount,
        conversionRate:
          oppCount > 0 ? Math.round((wonCount / oppCount) * 100) : 0,
        value: Number(wonValue._sum?.amount || 0),
      },
    ];
  }
  ...
  async getMarketingDashboardKPIs(tenantId: string): Promise<{
    ...
    const mqlCount = await prisma.lead.count({
      where: { tenantId, deletedAt: null, score: { gte: 50 } },
    });
    ...
    return {
      ...
      sqlCount,
      conversionRate:
        totalLeads > 0 ? Math.round((sqlCount / totalLeads) * 100) : 0,
  ...
  async getChannelMixAnalysis(tenantId: string): Promise<
    Array<{
      channel: string;
      campaignCount: number;
      totalBudget: number;
  ...
  > {
    ...
    return Array.from(channelMap.entries()).map(([channel, v]) => ({
      ...
      roi: 0,
      pctOfBudget:
        totalBudget > 0 ? Math.round((v.budget / totalBudget) * 100) : 0,
    }));
  }
}
