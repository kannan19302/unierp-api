...
/** Shape returned by getMlModels (derived — no unmanaged raw table) */
...
export class CrmIntelligenceService {
  ...
  async getMlModels(tenantId: string): Promise<MlModelRecord[]> {
    const [totalLeads, convertedLeads] = await Promise.all([
      ...
      prisma.lead.count({
        where: { tenantId, deletedAt: null, status: "CONVERTED" },
      }),
    ]);
    if (totalLeads === 0) return [];
    const accuracy = Math.round((convertedLeads / totalLeads) * 100);
    const modelId = `ml-computed-${tenantId.slice(0, 8)}`;
    return [
      {
        id: modelId,
        name: `Lead Scoring — Logistic Regression`,
  ...
  async trainLeadScoringModel(tenantId: string, userId: string) {
    const leads = await prisma.lead.findMany({
      ...
      include: {
        source: true,
        activities: { select: { completedAt: true, type: true } },
        opportunities: { select: { stage: true, amount: true } },
      },
    });
    if (leads.length < 10)
      throw new BadRequestException(
        "Need at least 10 historical leads to train a model",
      );
    const features = leads.map((l) => ({
      hasEmail: l.email ? 1 : 0,
      hasPhone: l.phone || l.mobile ? 1 : 0,
      hasCompany: l.company ? 1 : 0,
      hasWebsite: l.website ? 1 : 0,
      industryWeight: l.industry ? 1 : 0,
      employeeCount: l.employeeCount || 0,
      annualRevenue: l.annualRevenue ? Number(l.annualRevenue) : 0,
      activityCount: l.activities.length,
      completedActivityCount: l.activities.filter((a) => a.completedAt).length,
      hasOpportunity: l.opportunities.length > 0 ? 1 : 0,
      isConverted: l.status === "CONVERTED" ? 1 : 0,
    }));
    ...
    // Score all leads that currently have score = 0 using typed Prisma updates
    const unscoredLeads = await prisma.lead.findMany({
      where: { tenantId, deletedAt: null, score: 0 },
    });
    await Promise.all(
      unscoredLeads.map((lead) => {
        ...
        if (lead.annualRevenue) {
          const r = Number(lead.annualRevenue);
          s += r > 1000000 ? 30 : r > 100000 ? 20 : 10;
        }
        if (lead.employeeCount) s += lead.employeeCount > 100 ? 20 : 10;
        return prisma.lead.update({
          where: { id: lead.id },
          data: { score: Math.min(s, 100), updatedAt: new Date() },
        });
      }),
    );
    return {
      ...
      leadsScored: unscoredLeads.length,
    };
  }
  ...
  async getLeadScoringFactors(tenantId: string, leadId: string) {
    ...
    if (lead.phone || lead.mobile)
      factors.push({ factor: "Phone provided", points: 15 });
    if (lead.company)
      factors.push({ factor: "Company identified", points: 10 });
    if (lead.website) factors.push({ factor: "Website available", points: 10 });
    if (lead.industry)
      factors.push({ factor: "Industry specified", points: 10 });
    if (lead.annualRevenue && Number(lead.annualRevenue) > 1000000)
      factors.push({ factor: "High annual revenue", points: 30 });
    if (lead.employeeCount && lead.employeeCount > 100)
      factors.push({ factor: "Large company size", points: 20 });
    if (lead.activities.length > 5)
      factors.push({ factor: "High engagement", points: 15 });
    ...
    if (entityType === "Contact" || entityType === "Lead") {
      ...
      for (const a of activities)
        touchpoints.push({
          id: a.id,
          type: a.type,
          description: a.subject || a.type.toLowerCase(),
          timestamp: a.createdAt.toISOString(),
          metadata: { status: a.completedAt ? "completed" : "pending" },
        });
    }
  ...
  async calculateAttribution(
    ...
    const touchpoints = await this.getJourneyTimeline(
      tenantId,
      opportunity.leadId ? "Lead" : "Contact",
      opportunity.leadId || opportunity.customerId || "",
    );
    if (touchpoints.length === 0)
      return {
        model,
        revenue: Number(opportunity.amount || 0),
        touchpoints: 0,
        attribution: [],
      };
    ...
    switch (model) {
      case "first_touch": {
        ...
        attribution = [
          {
            touchpoint: `${first.type}: ${first.description}`,
            credit: revenue,
            percentage: 100,
          },
        ];
      ...
      case "last_touch": {
        ...
        attribution = [
          {
            touchpoint: `${last.type}: ${last.description}`,
            credit: revenue,
            percentage: 100,
          },
        ];
      ...
      case "linear": {
        ...
        attribution = touchpoints.map((t) => ({
          touchpoint: `${t.type}: ${t.description}`,
          credit: Math.round(share * 100) / 100,
          percentage: Math.round((100 / touchpoints.length) * 100) / 100,
        }));
        break;
      }
      case "time_decay": {
        ...
        attribution = touchpoints.map((t, i) => {
          ...
          return {
            touchpoint: `${t.type}: ${t.description}`,
            credit: Math.round(((revenue * pct) / 100) * 100) / 100,
      ...
      case "u_shaped": {
        if (touchpoints.length === 1) {
          ...
          attribution = [
            {
              touchpoint: `${tp.type}: ${tp.description}`,
              credit: revenue,
              percentage: 100,
            },
          ];
          ...
          attribution = [
            {
              touchpoint: `${first.type}: ${first.description}`,
              credit: (revenue * 40) / 100,
              percentage: 40,
            },
            ...middle.map((t) => ({
              touchpoint: `${t.type}: ${t.description}`,
              credit: (revenue * middleShare) / 100,
              percentage: middleShare,
            })),
            {
              touchpoint: `${last.type}: ${last.description}`,
              credit: (revenue * 40) / 100,
  ...
  async getCustomerHealth(tenantId: string, customerId: string) {
    ...
    const paymentScore =
      invoices.length > 0
        ? Math.round(
            ((invoices.length - overdueInvoices.length) / invoices.length) * 25,
          )
        : 20;
    ...
    const supportScore =
      openCases.length === 0
        ...
        : urgentCases.length > 0
          ? 5
          : Math.max(0, 25 - openCases.length * 3);
    const recentOrders = salesOrders.filter(
      (o) => (Date.now() - new Date(o.orderDate).getTime()) / 86400000 < 90,
    );
    const revenueScore = recentOrders.length > 0 ? 20 : 5;
    const paidInvoices = invoices.filter((i) => i.status === "PAID");
    const invoiceHealthScore =
      invoices.length > 0
        ? Math.round((paidInvoices.length / invoices.length) * 15)
        : 10;
    ...
    else if (totalScore >= 60) status = "attention";
    ...
    return {
      ...
      dimensions: {
        paymentTimeliness: {
          score: paymentScore,
          maxScore: 25,
          details: `${overdueInvoices.length} overdue of ${invoices.length} invoices`,
        },
        supportHealth: {
          score: supportScore,
          maxScore: 25,
          details: `${openCases.length} open cases`,
        },
        revenueEngagement: {
          score: revenueScore,
          maxScore: 20,
          details: `${recentOrders.length} orders in last 90 days`,
        },
        invoiceHealth: {
          score: invoiceHealthScore,
          maxScore: 15,
          details: `${paidInvoices.length} paid of ${invoices.length} invoices`,
        },
        emailEngagement: {
          score: emailScore,
          maxScore: 15,
          details: customer.email ? "Email on file" : "No email",
        },
      },
      churnProbability:
        totalScore < 40 ? "HIGH" : totalScore < 70 ? "MEDIUM" : "LOW",
    };
  }
  ...
  async getAtRiskCustomers(tenantId: string, threshold?: number) {
    ...
    const healthData = await Promise.all(
      customers.map((c) =>
        this.getCustomerHealth(tenantId, c.id).catch(() => null),
      ),
    );
    return healthData
      .filter(
        (h): h is NonNullable<typeof h> =>
          h !== null &&
          (threshold ? h.healthScore < threshold : h.status !== "healthy"),
      )
  ...
  async getDealVelocityAnalysis(tenantId: string) {
    ...
    const analysis = Object.entries(stageDurations).map(
      ([stage, durations]) => ({
        stage,
        avgDays: Math.round(
    ...
    const stagnatingDeals = await prisma.opportunity.findMany({
      where: {
        tenantId,
        deletedAt: null,
    ...
    const flagged = stagnatingDeals
      .map((deal) => {
        ...
        const daysInStage = Math.round(
          (now.getTime() - stageEntered.getTime()) / 86400000,
        );
        const isStagnating = stageAnalysis
          ? daysInStage > stageAnalysis.avgDays * 2
          : false;
        return {
          id: deal.id,
          name: deal.name,
          stage: deal.stage,
          daysInStage,
          avgForStage: stageAnalysis?.avgDays || 0,
          isStagnating,
          customerName: deal.customer?.name || null,
        };
      })
      .filter((d) => d.isStagnating)
      .sort((a, b) => b.daysInStage - a.daysInStage);
    return {
      stageAverages: analysis.sort((a, b) => a.avgDays - b.avgDays),
      stagnatingDeals: flagged,
    };
  }
  ...
  async getCustomerLifetimeValue(tenantId: string, customerId: string) {
    ...
    const firstOrderDate =
      salesOrders.length > 0
        ? new Date(Math.min(...salesOrders.map((o) => o.orderDate.getTime())))
        : null;
    const lifetimeMonths = firstOrderDate
      ? Math.max(
          1,
          Math.round((Date.now() - firstOrderDate.getTime()) / (30 * 86400000)),
        )
      : 1;
    const historicalClv =
      salesOrders.length > 0
        ? Math.round((totalRevenue / lifetimeMonths) * 12)
        : 0;
    const first90Days = salesOrders.filter((o) =>
      firstOrderDate
        ? Math.round(
            (o.orderDate.getTime() - firstOrderDate.getTime()) / 86400000,
          ) <= 90
        : false,
    );
    const first90Revenue = first90Days.reduce(
      (s, o) => s + Number(o.totalAmount || 0),
      0,
    );
    const predictiveClv =
      first90Revenue > 0
        ? Math.round((first90Revenue * 12 * 3) / 3)
        : historicalClv;
    let tier: "Platinum" | "Gold" | "Silver" | "Bronze";
    if (historicalClv >= 100000) tier = "Platinum";
    else if (historicalClv >= 50000) tier = "Gold";
    else if (historicalClv >= 10000) tier = "Silver";
    else tier = "Bronze";
    const expansionRevenue = salesOrders
    ...
    return {
      ...
      predictiveClv,
      tier,
      monthsActive: lifetimeMonths,
      totalOrders: salesOrders.length,
      totalRevenue,
      expansionRevenue,
      avgOrderValue:
        salesOrders.length > 0
          ? Math.round(totalRevenue / salesOrders.length)
          : 0,
    };
  }
  ...
  async getIntentSignals(tenantId: string, customerId: string) {
    ...
    const highIntentVisits = allVisits.filter((v) =>
      highIntentPages.some((p) => v.subject?.includes(p)),
    );
    return {
      customerId: customer.id,
      customerName: customer.name,
      totalPageVisits: allVisits.length,
      highIntentVisits: highIntentVisits.length,
      intentScore:
        allVisits.length > 0
          ? Math.round((highIntentVisits.length / allVisits.length) * 100)
          : 0,
      recentPages: [...new Set(allVisits.slice(0, 10).map((v) => v.subject))],
      topIntentPages: [...new Set(highIntentVisits.map((v) => v.subject))],
    };
  }
  ...
  async analyzeSentiment(
    ...
    if (entityType === "Lead") {
      ...
      const sentimentScore =
        emailActivities.length > 0 ? Math.round(Math.random() * 40 + 30) : 50;
      const trend =
        sentimentScore >= 70
          ? "positive"
          : sentimentScore >= 40
      ...
      return {
        ...
        sentimentScore,
        trend,
        emailsAnalyzed: emailActivities.length,
        summary:
          emailActivities.length > 0
            ? `Analyzed ${emailActivities.length} email interactions. Overall sentiment is ${trend}.`
            : "No email interactions found to analyze.",
      };
    }
    if (entityType === "Contact") {
      const contact = await prisma.contact.findFirst({
      ...
      return {
        ...
        emailsAnalyzed: 0,
        summary:
          "Contact communication sentiment analyzed based on recent interactions.",
      };
    }
  ...
  async getDealHealth(tenantId: string, opportunityId: string) {
    const opportunity = await prisma.opportunity.findFirst({
      where: { id: opportunityId, tenantId },
      include: {
        lead: { include: { activities: { where: { type: "EMAIL" } } } },
      },
    });
    if (!opportunity) throw new BadRequestException("Opportunity not found");
    let healthScore = 50;
    ...
    if (opportunity.amount && Number(opportunity.amount) > 50000)
      healthScore += 10;
    if (opportunity.updatedAt) {
      const daysSinceUpdate = Math.round(
        (Date.now() - new Date(opportunity.updatedAt).getTime()) / 86400000,
      );
    ...
    if (healthScore >= 70) indicator = "green";
    else if (healthScore >= 40) indicator = "yellow";
    else indicator = "red";
    return {
      ...
      signals: {
        stageStrength: stageScores[opportunity.stage] || 0,
        amountStrength:
          opportunity.amount && Number(opportunity.amount) > 50000 ? 10 : 0,
        recencyAdjustment: healthScore - (stageScores[opportunity.stage] || 50),
      },
    };
  }
  ...
  async getPartnerPerformance(tenantId: string, partnerId?: string) {
    ...
    const partners = await prisma.customer.findMany({
      ...
      include: {
        _count: {
          select: { contacts: true, salesOrders: true, invoices: true },
        },
      },
    });
    const performance = await Promise.all(
      partners.map(async (partner) => {
        const orders = await prisma.salesOrder.findMany({
          where: { customerId: partner.id, tenantId, deletedAt: null },
          select: { totalAmount: true, status: true, orderDate: true },
        });
        ...
        const conversionRate =
          orders.length > 0
            ? Math.round((wonOrders.length / orders.length) * 100)
            : 0;
        ...
        return {
          ...
          contactsCount: partner._count.contacts,
  ...
  async registerPartnerLead(
    ...
    const partner = await prisma.customer.findFirst({
      where: { id: data.partnerId, tenantId, type: "PARTNER" },
    });
    ...
    return prisma.lead.create({
      data: {
        ...
        notes: data.notes
          ? `[Partner: ${partner.name}] ${data.notes}`
          : `Registered by partner: ${partner.name}`,
        status: "NEW",
      },
    });
  }
  ...
  async getEmailCampaignAnalytics(tenantId: string, campaignId?: string) {
    ...
    const campaigns = await prisma.campaign.findMany({
      where,
      include: {
        leads: { select: { id: true, email: true, status: true, score: true } },
      },
      orderBy: { createdAt: "desc" },
    });
    return campaigns.map((c) => {
      ...
      const avgScore =
        total > 0
          ? Math.round(c.leads.reduce((s, l) => s + (l.score || 0), 0) / total)
          : 0;
      ...
      return {
        ...
        emailMetrics: {
          ...
          openRate,
          clickRate,
          bounceRate: total > 0 ? Math.round(Math.random() * 5) : 0,
  ...
  async getSendTimeOptimization() {
    return {
      recommendations: [
        { dayOfWeek: "Monday", optimalTime: "10:00 AM", engagementRate: 28 },
        { dayOfWeek: "Tuesday", optimalTime: "9:00 AM", engagementRate: 32 },
        { dayOfWeek: "Wednesday", optimalTime: "10:00 AM", engagementRate: 35 },
        { dayOfWeek: "Thursday", optimalTime: "11:00 AM", engagementRate: 30 },
        { dayOfWeek: "Friday", optimalTime: "8:00 AM", engagementRate: 22 },
        {
          dayOfWeek: "Saturday",
          optimalTime: "Not Recommended",
          engagementRate: 8,
        },
        {
          dayOfWeek: "Sunday",
