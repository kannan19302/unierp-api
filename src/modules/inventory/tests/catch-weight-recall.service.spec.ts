...
describe("CatchWeightRecallService", () => {
  ...
  it("upsertConfig creates or updates config", async () => {
    ...
    const result = await svc.upsertConfig(T, {
      productId: "p1",
      nominalWeightKg: 1.5,
    });
  ...
  it("deactivateConfig marks config inactive", async () => {
    ...
    vi.mocked(prisma.catchWeightConfig.update).mockResolvedValue({
      id: "cfg1",
      active: false,
    } as any);
  ...
  it("captureReading calculates WITHIN_TOLERANCE correctly", async () => {
    ...
    vi.mocked(prisma.catchWeightReading.create).mockResolvedValue({
      id: "r1",
    ...
    const result = await svc.captureReading(T, "user1", {
      ...
      referenceId: "r1",
      nominalQty: 5,
      actualWeightKg: 7.5, // 5 × 1.5 = 7.5 expected → 0% variance
    });
  ...
  it("captureReading flags OVER_TOLERANCE when weight exceeds upper bound", async () => {
    ...
    vi.mocked(prisma.catchWeightReading.create).mockImplementation(
      async ({ data }: any) => ({
        id: "r1",
    ...
    vi.mocked(prisma.catchWeightReading.groupBy).mockResolvedValue([
      {
        varianceStatus: "WITHIN_TOLERANCE",
        _count: 10,
        _avg: { variancePct: 1.2 },
      } as any,
      {
        varianceStatus: "OVER_TOLERANCE",
        _count: 2,
        _avg: { variancePct: 7.5 },
      } as any,
    ]);
  ...
  it("upsertTare creates or updates tare", async () => {
    ...
    expect(
      await svc.upsertTare(T, { containerLabel: "CRATE-A", tareWeightKg: 0.5 }),
    ).toHaveProperty("id");
  });
  ...
  it("issueRecall transitions DRAFT→ISSUED", async () => {
    ...
    vi.mocked(prisma.productRecall.update).mockResolvedValue({
      id: "rc1",
      status: "ISSUED",
    } as any);
  ...
  it("issueRecall throws BadRequestException if not in DRAFT", async () => {
    ...
    await expect(svc.issueRecall(T, "rc1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("addAffectedStock increments totalUnitsAffected", async () => {
    ...
    vi.mocked(prisma.recallAffectedStock.create).mockResolvedValue({
      id: "as1",
      qtyAffected: 100,
    } as any);
    const result = await svc.addAffectedStock(T, "rc1", {
      warehouseId: "wh1",
      qtyAffected: 100,
    });
  ...
  it("quarantineStock throws NotFoundException when stock not found", async () => {
    ...
    await expect(svc.quarantineStock(T, "rc1", "bad", 10)).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it("sendNotices marks all unsent notices as sent", async () => {
    ...
    vi.mocked(prisma.recallCustomerNotice.updateMany).mockResolvedValue({
      count: 3,
    } as any);
    vi.mocked(prisma.productRecall.update).mockResolvedValue({} as any);
  ...
  it("cancelRecall prevents cancelling a COMPLETED recall", async () => {
    ...
    await expect(svc.cancelRecall(T, "rc1")).rejects.toThrow(
      BadRequestException,
    );
  });
