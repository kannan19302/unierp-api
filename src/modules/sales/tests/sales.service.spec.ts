    ...
    $transaction: vi.fn((fn: (tx: unknown) => Promise<unknown>) =>
      fn({
        ...
        creditNote: {
          create: vi
            .fn()
            .mockResolvedValue({ id: "cn-1", noteNumber: "CN-SR-001" }),
        },
        salesReturn: {
          create: vi
            .fn()
            .mockResolvedValue({ id: "sr-1", returnNumber: "SR-001" }),
        },
        salesReturnItem: {
          create: vi.fn(),
        },
...
describe("SalesService", () => {
  ...
  describe("createQuotation", () => {
    it("should create a quotation with line items", async () => {
      ...
      const dto = {
        ...
        lineItems: [
          { description: "Widget", quantity: 10, unitPrice: 50, taxRate: 10 },
        ],
      };
    ...
    it("should throw when customer not found", async () => {
      ...
      const dto = {
        ...
        validUntil: new Date().toISOString(),
        lineItems: [
          { description: "Widget", quantity: 1, unitPrice: 10, taxRate: 0 },
        ],
      };
  ...
  describe("convertQuotationToOrder", () => {
    it("should convert an accepted quotation to a sales order", async () => {
      ...
      const result = await service.convertQuotationToOrder(
        "tenant-1",
        "q-1",
        "user-1",
      );
  ...
  describe("recordOrderPayment", () => {
    it("should record order payment and update status", async () => {
      ...
      const result = await service.recordOrderPayment(
        "tenant-1",
        "so-1",
        500,
        "CREDIT_CARD",
  ...
  describe("convertToPurchaseOrders", () => {
    it("should convert a SalesOrder to PurchaseOrder(s) grouped by preferred vendor", async () => {
        ...
        lineItems: [
          {
            ...
            product: {
              name: "Product A",
              reorderRules: [{ preferredVendorId: "vendor-A" }],
            },
          },
          {
            id: "so-item-2",
            ...
            product: {
              name: "Product B",
              reorderRules: [{ preferredVendorId: "vendor-B" }],
            },
          },
        ],
      };
      ...
      vi.mocked(prisma.salesOrder.findFirst).mockResolvedValue(
        mockSalesOrder as never,
      );
      vi.mocked(prisma.vendor.findFirst).mockResolvedValue({
        id: "vendor-fallback",
      } as never);

      const result = await service.convertToPurchaseOrders(
        "tenant-1",
        "so-1",
        "user-1",
      );
