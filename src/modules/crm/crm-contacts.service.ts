...
export class CrmContactsService {
  async getContacts(
    ...
    if (query?.sortBy === "firstName")
      ...
      orderBy.lastName = query.sortOrder || "asc";
  ...
  async deleteContact(tenantId: string, id: string) {
    ...
    if (!existing) throw new NotFoundException("Contact not found");
    const emailUpdate = existing.email
      ? `${existing.email}.deleted.${id}`
      : null;
    return prisma.contact.update({
      where: { id },
      data: {
  ...
  async getContactTimeline(tenantId: string, contactId: string) {
    ...
    const opportunities = contact.customerId
      ? await prisma.opportunity.findMany({
          where: { tenantId, customerId: contact.customerId, deletedAt: null },
          orderBy: { createdAt: "desc" },
          take: 20,
        })
      : [];
  ...
  async findDuplicateContacts(tenantId: string) {
    ...
    const dupes: Array<{
      contactA: (typeof contacts)[0];
      contactB: (typeof contacts)[0];
      reason: string;
    }> = [];
  ...
  async mergeContacts(tenantId: string, dto: MergeContactsInput) {
    const primary = await prisma.contact.findFirst({
      where: { id: dto.primaryContactId, tenantId },
    });
    const secondary = await prisma.contact.findFirst({
      where: { id: dto.secondaryContactId, tenantId },
    });
    if (!primary || !secondary)
      throw new NotFoundException("Contact not found");
    return prisma.$transaction(async (tx) => {
      await tx.activity.updateMany({
        where: { contactId: dto.secondaryContactId },
        data: { contactId: dto.primaryContactId },
      });
      const secTags = await tx.contactTagLink.findMany({
        where: { contactId: dto.secondaryContactId },
      });
      for (const t of secTags) {
        const exists = await tx.contactTagLink.findFirst({
          where: { contactId: dto.primaryContactId, tagId: t.tagId },
        });
        if (!exists)
          await tx.contactTagLink.create({
            data: { contactId: dto.primaryContactId, tagId: t.tagId },
          });
      }
      await tx.contactTagLink.deleteMany({
        where: { contactId: dto.secondaryContactId },
      });
      await tx.contact.update({
        where: { id: dto.secondaryContactId },
  ...
  async getContactById(tenantId: string, id: string) {
    ...
    const recentActivities = activities.filter(
      (act) => new Date(act.createdAt) >= thirtyDaysAgo,
    );
    ...
    if (
      ...
      contact.lastContactedAt?.getTime() !== lastContact?.getTime()
    ) {
