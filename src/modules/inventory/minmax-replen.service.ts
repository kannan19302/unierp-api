...
export class MinMaxReplenService {
  ...
  async upsertLevel(
    ...
    if (dto.minQty < 0)
      throw new BadRequestException("minQty cannot be negative");
  ...
  async deactivateLevel(tenantId: string, levelId: string, userId: string) {
    const level = await prisma.minMaxLevel.findFirst({
      where: { id: levelId, tenantId },
    });
    if (!level)
      throw new NotFoundException(`Min-max level ${levelId} not found`);
    return prisma.minMaxLevel.update({
      where: { id: levelId },
  ...
  async runReplenishment(
    ...
    let sugCount = 0;
    const existingCount = await prisma.replenSuggestion.count({
      where: { tenantId },
    });
  ...
  async listSuggestions(
    ...
    const [data, total] = await Promise.all([
      prisma.replenSuggestion.findMany({
        where,
  ...
  async approveSuggestion(tenantId: string, suggId: string, userId: string) {
    const sugg = await prisma.replenSuggestion.findFirst({
      where: { id: suggId, tenantId },
    });
    if (!sugg) throw new NotFoundException(`Suggestion ${suggId} not found`);
    if (sugg.status !== "OPEN")
      throw new BadRequestException(
        `Suggestion must be OPEN to approve; current: ${sugg.status}`,
      );
  ...
  async markOrdered(tenantId: string, suggId: string) {
    ...
    if (sugg.status !== "APPROVED")
      throw new BadRequestException(
        `Suggestion must be APPROVED to mark as ordered`,
      );
    return prisma.replenSuggestion.update({
      where: { id: suggId },
  ...
  async markReceived(tenantId: string, suggId: string) {
    ...
    if (!sugg) throw new NotFoundException(`Suggestion ${suggId} not found`);
    if (sugg.status !== "ORDERED")
      throw new BadRequestException(
        `Suggestion must be ORDERED to mark as received`,
      );
    return prisma.replenSuggestion.update({
      where: { id: suggId },
  ...
  async cancelSuggestion(
    ...
    if (["RECEIVED", "CANCELLED"].includes(sugg.status)) {
      throw new BadRequestException(
        `Cannot cancel a ${sugg.status} suggestion`,
      );
    }
    return prisma.replenSuggestion.update({
      where: { id: suggId },
