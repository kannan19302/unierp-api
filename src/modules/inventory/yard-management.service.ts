...
export class YardManagementService {
  ...
  async deleteDockDoor(tenantId: string, id: string) {
    ...
    if (active > 0)
      throw new BadRequestException(
        "Cannot delete a door with active appointments",
      );
  ...
  async createAppointment(
    ...
    if (dto.dockDoorId) {
      const door = await prisma.dockDoor.findFirst({
        where: { tenantId, id: dto.dockDoorId },
      });
      ...
      if (conflict > 0)
        throw new BadRequestException(
          "Dock door has a conflicting appointment in the ±2h window",
        );
    }
  ...
  async checkIn(
    ...
    await prisma.gatePass.create({
      data: {
        ...
        vehicleWeight:
          dto.vehicleWeight != null
            ? new Prisma.Decimal(dto.vehicleWeight)
            : undefined,
      },
    });
  ...
  async startLoading(tenantId: string, id: string) {
    ...
    if (a.status !== "CHECKED_IN")
      throw new BadRequestException(
        "Appointment must be CHECKED_IN to start loading",
      );
    return prisma.yardAppointment.update({
      where: { id },
  ...
  async complete(
    ...
    if (!["CHECKED_IN", "LOADING"].includes(a.status)) {
      throw new BadRequestException(
        "Appointment must be CHECKED_IN or LOADING to complete",
      );
    }

    if (a.dockDoorId) {
      await prisma.dockDoor.update({
        where: { id: a.dockDoorId },
  ...
  async rescheduled(
    tenantId: string,
    id: string,
    dto: { scheduledAt: string; dockDoorId?: string },
  ) {
    const a = await this.getAppointment(tenantId, id);
  ...
  async listYardMoves(tenantId: string, warehouseId?: string, status?: string) {
    return prisma.yardMove.findMany({
      ...
      include: {
        appointment: { select: { appointmentNumber: true, carrierName: true } },
      },
      orderBy: { createdAt: "desc" },
    });
  }

  async createYardMove(
    tenantId: string,
    dto: {
      warehouseId: string;
      trailerNumber: string;
  ...
  async startYardMove(tenantId: string, id: string) {
    ...
    if (move.status !== "PENDING")
      throw new BadRequestException("Yard move must be PENDING to start");
    return prisma.yardMove.update({
      where: { id },
  ...
  async completeYardMove(tenantId: string, id: string) {
    ...
    if (move.status !== "IN_PROGRESS")
      throw new BadRequestException(
        "Yard move must be IN_PROGRESS to complete",
      );

    await prisma.yardMove.update({
      where: { id },
  ...
  async departYardInventory(tenantId: string, id: string) {
    const inv = await prisma.yardInventory.findFirst({
      where: { tenantId, id },
    });
    ...
    return prisma.yardInventory.update({
      where: { id },
  ...
  async getTurnaroundReport(tenantId: string, warehouseId?: string) {
    ...
    const rows = completed.map((a) => {
      const waitMins =
        a.checkedInAt && a.loadingStartAt
          ? (a.loadingStartAt.getTime() - a.checkedInAt.getTime()) / 60000
          : null;
      const loadingMins =
        a.loadingStartAt && a.completedAt
          ? (a.completedAt.getTime() - a.loadingStartAt.getTime()) / 60000
          : null;
      const totalMins =
        a.checkedInAt && a.completedAt
          ? (a.completedAt.getTime() - a.checkedInAt.getTime()) / 60000
          : null;
    ...
    const avgTurnaround = totals.length
      ? totals.reduce((s, r) => s + (r.totalMins ?? 0), 0) / totals.length
      : 0;
