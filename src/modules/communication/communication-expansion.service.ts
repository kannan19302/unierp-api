...
export class CommunicationExpansionService {
  ...
  async updateChatRoom(tenantId: string, id: string, dto: UpdateChatRoomInput) {
    ...
    return prisma.chatRoom.update({ where: { id }, data: dto as any });
  }
  ...
  async getMessages(
    ...
    const [data, total] = await Promise.all([
      prisma.chatMessage.findMany({
        ...
        include: { reactions: true, readReceipts: true },
      }),
  ...
  async addReaction(
    ...
    userId: string,
    messageId: string,
    dto: CreateMessageReactionInput,
  ) {
    const existing = await prisma.messageReaction.findFirst({
      where: { tenantId, messageId, userId, emoji: dto.emoji },
    });
  ...
  async getVideoCalls(tenantId: string) {
    return prisma.videoCallRoom.findMany({
      where: { tenantId },
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async createVideoCall(
    tenantId: string,
    userId: string,
    dto: CreateVideoCallRoomInput,
  ) {
    return prisma.videoCallRoom.create({
      data: {
        tenantId,
        createdBy: userId,
  ...
  async getFileShares(tenantId: string) {
    return prisma.communicationFileShare.findMany({
      where: { tenantId },
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async createFileShare(
    tenantId: string,
    userId: string,
    dto: CreateFileShareInput,
  ) {
    return prisma.communicationFileShare.create({
      data: { tenantId, uploadedBy: userId, ...dto },
    });
  }
  ...
  async createAnnouncement(
    ...
    return prisma.announcement.create({
      data: {
        ...
        ...rest,
        targets: targets
          ? { create: targets.map((t) => ({ tenantId, ...t })) }
          : undefined,
      },
      include: { targets: true },
    });
