...
describe("YardManagementService", () => {
    ...
    it("filters by warehouseId", async () => {
      ...
      expect(prisma.dockDoor.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ warehouseId: "wh2" }),
        }),
      );
    });
  });
  ...
  describe("updateDockDoor", () => {
    it("updates door status", async () => {
      ...
      vi.mocked(prisma.dockDoor.update).mockResolvedValue(
        mockDoor({ status: "MAINTENANCE" }) as never,
      );
      const result = await svc.updateDockDoor(TENANT, "door1", {
        status: "MAINTENANCE",
      });
  ...
  describe("deleteDockDoor", () => {
    ...
    it("throws if active appointments exist", async () => {
      ...
      await expect(svc.deleteDockDoor(TENANT, "door1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("createAppointment", () => {
    it("creates appointment with auto-number", async () => {
      ...
      expect(prisma.yardAppointment.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ appointmentNumber: "YA-000001" }),
        }),
      );
  ...
  describe("checkIn", () => {
    it("checks in a SCHEDULED appointment", async () => {
      ...
      vi.mocked(prisma.gatePass.create).mockResolvedValue({
        id: "gp1",
        passNumber: "GP-000001",
      } as never);
      vi.mocked(prisma.yardAppointment.update).mockResolvedValue(
    ...
    it("throws if not SCHEDULED", async () => {
      ...
      await expect(svc.checkIn(TENANT, "appt1", USER, {})).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("startLoading", () => {
    ...
    it("throws if not CHECKED_IN", async () => {
      ...
      await expect(svc.startLoading(TENANT, "appt1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("complete", () => {
    it("completes a LOADING appointment", async () => {
      ...
      vi.mocked(prisma.gatePass.update).mockResolvedValue({
        id: "gp1",
      } as never);
      vi.mocked(prisma.yardAppointment.update).mockResolvedValue(
    ...
    it("throws if not in CHECKED_IN or LOADING", async () => {
      ...
      await expect(svc.complete(TENANT, "appt1", USER, {})).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("markNoShow", () => {
    it("marks SCHEDULED appointment as NO_SHOW", async () => {
      ...
      vi.mocked(prisma.yardAppointment.update).mockResolvedValue(
        mockAppt({ status: "NO_SHOW" }) as never,
      );
    ...
    it("throws if not SCHEDULED", async () => {
      ...
      await expect(svc.markNoShow(TENANT, "appt1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("cancelAppointment", () => {
    it("cancels a SCHEDULED appointment", async () => {
      ...
      vi.mocked(prisma.yardAppointment.update).mockResolvedValue(
        mockAppt({ status: "CANCELLED" }) as never,
      );
    ...
    it("throws if already terminal", async () => {
      ...
      await expect(svc.cancelAppointment(TENANT, "appt1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("startYardMove", () => {
    it("starts a PENDING move", async () => {
      ...
      vi.mocked(prisma.yardMove.update).mockResolvedValue(
    ...
    it("throws if not PENDING", async () => {
      ...
      await expect(svc.startYardMove(TENANT, "move1")).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("completeYardMove", () => {
    it("completes an IN_PROGRESS move and updates yard inventory location", async () => {
      ...
      vi.mocked(prisma.yardMove.update).mockResolvedValue(
        mockMove({ status: "COMPLETE" }) as never,
      );
      vi.mocked(prisma.yardInventory.updateMany).mockResolvedValue({
        count: 1,
      } as never);
    ...
    it("throws if not IN_PROGRESS", async () => {
      ...
      await expect(svc.completeYardMove(TENANT, "move1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("listYardInventory", () => {
    it("returns active yard inventory (no departedAt)", async () => {
      ...
      expect(prisma.yardInventory.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ departedAt: null }),
        }),
      );
    });
  });
  ...
  describe("departYardInventory", () => {
    ...
    it("throws if already departed", async () => {
      ...
      await expect(svc.departYardInventory(TENANT, "yi1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
