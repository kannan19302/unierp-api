...
describe("InventoryService — pick-wave/sales-order integration, kit BOM versioning", () => {
  ...
  describe("completePickWave fulfillment integration", () => {
    it("advances linked sales orders to PROCESSING once the wave completes", async () => {
      ...
      expect(db.salesOrder.updateMany).toHaveBeenCalledWith(
  ...
  describe("kit BOM versioning", () => {
    it("creates version 1 when no prior version exists", async () => {
      ...
      db.kitVersion.create.mockImplementation(({ data }: any) =>
      ...
      expect(result.componentsSnapshot).toEqual([
        { productId: "p1", productName: "Component A", quantity: 2 },
      ]);
    });
    ...
    it("throws NotFound activating an unknown version", async () => {
      db.kitVersion.findFirst.mockResolvedValue(null);
      await expect(
        service.activateKitVersion("t1", "kit-1", "missing"),
      ).rejects.toThrow(NotFoundException);
    });

    it("replaces live components with the snapshot when activating a version", async () => {
      db.kitVersion.findFirst.mockResolvedValue({
        id: "v1",
        componentsSnapshot: [{ productId: "p2", quantity: 5 }],
      });
      vi.spyOn(service, "getProductKitById").mockResolvedValue({
        id: "kit-1",
        components: [],
      } as any);
      ...
      expect(db.productKitItem.deleteMany).toHaveBeenCalledWith({
        where: { tenantId: "t1", kitId: "kit-1" },
      });
      expect(db.productKitItem.createMany).toHaveBeenCalledWith(
        expect.objectContaining({
