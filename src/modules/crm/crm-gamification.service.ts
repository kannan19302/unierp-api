...
export class CrmGamificationService {
  ...
  async computeLeaderboard(
    ...
    const whereOpp: Prisma.OpportunityWhereInput = {
      ...
      ...(range
        ? { actualCloseDate: { gte: range.start, lt: range.end } }
        : {}),
    };
    ...
    const statsByUser = new Map<
      string,
      { dealsWon: number; revenue: number; activityCount: number }
    >();
    ...
    for (const a of activities) {
      ...
      const s = statsByUser.get(a.assignedToId) ?? {
        dealsWon: 0,
        revenue: 0,
        activityCount: 0,
      };
    ...
    const ranked = Array.from(statsByUser.entries())
      .map(([userId, s]) => ({
        ...
        ...s,
        points: Math.round(
          s.dealsWon * 100 + s.revenue / 1000 + s.activityCount * 2,
        ),
      }))
    ...
    this.eventEmitter.emit("crm.gamification.leaderboard_computed", {
      tenantId,
      period,
  ...
  async computeStreaks(tenantId: string, orgId: string) {
    ...
    for (const a of activities) {
      if (!a.assignedToId) continue;
      if (!activityDaysByUser.has(a.assignedToId))
        activityDaysByUser.set(a.assignedToId, new Set());
    ...
    for (const d of wonDeals) {
      ...
      if (!dealDaysByUser.has(d.assignedToId))
        dealDaysByUser.set(d.assignedToId, new Set());
      dealDaysByUser.get(d.assignedToId)!.add(dayKey(d.actualCloseDate));
    }

    const computeStreakFromDays = (
      days: Set<string>,
    ): { current: number; best: number; lastDate: Date | null } => {
    ...
    const allUsers = new Set([
      ...activityDaysByUser.keys(),
      ...dealDaysByUser.keys(),
    ]);
    for (const userId of allUsers) {
      const activityStreak = computeStreakFromDays(
        activityDaysByUser.get(userId) ?? new Set(),
      );
      const dealsStreak = computeStreakFromDays(
        dealDaysByUser.get(userId) ?? new Set(),
      );

      await prisma.salesStreak.upsert({
        where: {
          tenantId_userId_streakType: {
            tenantId,
            userId,
            streakType: "ACTIVITY",
          },
  ...
  async listStreaks(tenantId: string) {
    const rows = await prisma.salesStreak.findMany({
      where: { tenantId },
  ...
  async createBadge(tenantId: string, orgId: string, dto: CreateBadgeInput) {
    ...
    return prisma.gamificationBadge.create({
      data: {
        ...
        criteriaValue: new Prisma.Decimal(dto.criteriaValue),
  ...
  async updateBadge(tenantId: string, id: string, dto: UpdateBadgeInput) {
    const existing = await prisma.gamificationBadge.findFirst({
  ...
  async evaluateBadges(tenantId: string, orgId: string) {
    ...
    const dealsByUser = new Map<
      string,
      { count: number; revenue: number; maxDeal: number }
    >();
    ...
    const streaks = await prisma.salesStreak.findMany({
      where: { tenantId, streakType: "ACTIVITY" },
    });
    ...
    for (const [userId, stats] of dealsByUser.entries()) {
      for (const badge of badges) {
        ...
        let earned = false;
        if (badge.criteriaType === "DEALS_WON_COUNT")
          earned = stats.count >= threshold;
        else if (badge.criteriaType === "REVENUE_TOTAL")
          earned = stats.revenue >= threshold;
        else if (badge.criteriaType === "FIRST_DEAL") earned = stats.count >= 1;
        else if (badge.criteriaType === "DEAL_SIZE_ABOVE")
          earned = stats.maxDeal >= threshold;
        else if (badge.criteriaType === "ACTIVITY_STREAK")
          earned = (streakByUser.get(userId) ?? 0) >= threshold;
        if (!earned) continue;

        const existing = await prisma.gamificationBadgeAward.findUnique({
          where: {
            tenantId_badgeId_userId: { tenantId, badgeId: badge.id, userId },
          },
        });
        ...
        await prisma.gamificationBadgeAward.create({
          data: {
            ...
            userId,
            context: {
              dealsWon: stats.count,
              revenue: stats.revenue,
              maxDeal: stats.maxDeal,
            },
          },
        });
        awarded++;
        this.eventEmitter.emit("crm.gamification.badge_awarded", {
          tenantId,
          badgeId: badge.id,
          userId,
  ...
  async getMySummary(tenantId: string, userId: string, period = "ALL_TIME") {
    ...
    if (!snapshot && badgeAwards.length === 0 && streaks.length === 0) {
      throw new BadRequestException(
        "No gamification data yet for this user — run recompute first",
      );
    }
    return {
