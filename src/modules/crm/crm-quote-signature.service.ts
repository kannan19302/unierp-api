...
export class CrmQuoteSignatureService {
  ...
  async getSignatureByToken(token: string) {
    const sig = await prisma.quotationSignature.findUnique({
      where: { token },
      include: { quotation: true, certificate: true },
    });
  ...
  async signQuotation(dto: SignQuotationInput) {
    ...
    if (sig.status === "SIGNED")
      throw new BadRequestException("This quotation has already been signed");
    ...
    const documentHash = createHash("sha256")
      .update(
        `${sig.quotationId}|${sig.quotation.quotationNumber}|${sig.signerName}|${sig.signerEmail}|${dto.signatureData}|${signedAt.toISOString()}`,
      )
      .digest("hex");
    ...
    const auditTrail: AuditEvent[] = [
      { event: "REQUESTED", at: sig.createdAt.toISOString() },
      {
        ...
        detail: dto.ipAddress ? `from ${dto.ipAddress}` : undefined,
      },
      {
        event: "SIGNED",
        at: signedAt.toISOString(),
  ...
  async getCertificate(tenantId: string, signatureId: string) {
    ...
    if (!cert)
      throw new NotFoundException(
        "Signature certificate not found (quotation is not yet signed)",
      );
    return cert;
  }
  ...
  private formatCertificateDocument(
    cert: Awaited<ReturnType<CrmQuoteSignatureService["getCertificate"]>>,
  ) {
    const lines = [
      ...
      "",
      "Audit Trail:",
      ...(cert.auditTrail as unknown as AuditEvent[]).map(
