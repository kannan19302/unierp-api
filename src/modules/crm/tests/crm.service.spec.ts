...
describe("CrmService", () => {
  ...
  describe("Customers", () => {
    it("getCustomers should return customers list", async () => {
      const mockCustomers = [
        {
          ...
          tenantId,
          deletedAt: null,
          _count: { invoices: 0, quotations: 0, salesOrders: 0 },
        },
      ];
      ...
      expect(mockPrisma.customer.findMany).toHaveBeenCalledWith({
        ...
        include: {
          _count: {
            select: { invoices: true, quotations: true, salesOrders: true },
          },
        },
        orderBy: { name: "asc" },
      });
    ...
    it("getCustomerSummary should return metrics and lists", async () => {
      ...
      mockPrisma.invoice.aggregate.mockResolvedValue({
        _sum: { totalAmount: 3000, paidAmount: 1000 },
      });
    ...
    it("getCustomerById should throw NotFoundException", async () => {
      ...
      await expect(
        service.getCustomerById(tenantId, "nonexistent"),
    ...
    it("updateCustomer should update existing customer", async () => {
      ...
      const result = await service.updateCustomer(tenantId, "c1", {
        name: "Updated Corp",
      });
    ...
    it("updateCustomerStatus should throw on invalid status", async () => {
      await expect(
        service.updateCustomerStatus(tenantId, "c1", "INVALID"),
    ...
    it("updateCustomerStatus should throw if customer not found", async () => {
      mockPrisma.customer.findFirst.mockResolvedValue(null);
      await expect(
        service.updateCustomerStatus(tenantId, "c1", "ACTIVE"),
      ).rejects.toThrow(NotFoundException);
    });
      ...
      const mockNotes = [
        {
          id: "n1",
          type: "NOTE",
          subject: "[CUSTOMER:c1] First note",
        ...
        },
        {
          id: "n2",
          type: "CALL",
          subject: "[CUSTOMER:c1] Called client",
    ...
    it("addCustomerNote should throw if customer not found", async () => {
      ...
      await expect(
        service.addCustomerNote(tenantId, orgId, "nonexistent", dto),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it("bulkUpdateCustomerStatus should update multiple customers", async () => {
      ...
      const result = await service.bulkUpdateCustomerStatus(
        tenantId,
        ["c1", "c2"],
        "ON_HOLD",
      );
    ...
    it("bulkUpdateCustomerStatus should throw on invalid status", async () => {
      await expect(
        service.bulkUpdateCustomerStatus(tenantId, ["c1"], "INVALID"),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("exportCustomers should return CSV-ready data", async () => {
      ...
      const result = await service.exportCustomers(tenantId, {
        status: "ACTIVE",
        search: "Test",
      });
  ...
  describe("Contacts", () => {
    ...
    it("createContact should throw if customer not found", async () => {
      ...
      await expect(service.createContact(tenantId, orgId, dto)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("Leads", () => {
    it("getLeads should return leads with status filter", async () => {
      ...
      expect(mockPrisma.lead.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { tenantId, deletedAt: null, status: "NEW" },
        }),
      );
    });
        ...
        async (cb: (tx: unknown) => Promise<unknown>) => {
          const tx = {
            customer: {
              create: vi
                .fn()
                .mockResolvedValue({ id: "c-new", name: "Stark Industries" }),
            },
            contact: {
            ...
            opportunity: {
              create: vi
                .fn()
                .mockResolvedValue({ id: "o-new", name: "Test Opp" }),
            },
            lead: {
              update: vi
                .fn()
                .mockResolvedValue({ id: "l1", status: "CONVERTED" }),
            },
          };
          return cb(tx);
        },
    ...
    it("convertLead should throw if already converted", async () => {
      ...
      await expect(service.convertLead(tenantId, orgId, "l1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("Pipelines", () => {
    it("getPipelines should return list", async () => {
      mockPrisma.salesPipeline.findMany.mockResolvedValue([
        { id: "p1", name: "Default", tenantId },
      ]);
    ...
    it("createPipeline should unset default when isDefault=true", async () => {
      const dto = {
        name: "New Pipeline",
        stages: [{ label: "Stage 1", probability: 10, order: 0 }],
        isDefault: true,
      };
      mockPrisma.salesPipeline.create.mockResolvedValue({
        id: "p-new",
        ...dto,
        tenantId,
      });
      await service.createPipeline(tenantId, dto);
      expect(mockPrisma.salesPipeline.updateMany).toHaveBeenCalledWith({
        where: { tenantId },
  ...
  describe("Opportunities", () => {
    it("getOpportunities should filter by pipeline", async () => {
      mockPrisma.opportunity.findMany.mockResolvedValue([
        { id: "o1", name: "Test Deal", tenantId, deletedAt: null },
      ]);
      const result = await service.getOpportunities(tenantId, {
        pipelineId: "p1",
      });
      expect(result).toBeDefined();
      expect(mockPrisma.opportunity.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { tenantId, deletedAt: null, pipelineId: "p1" },
        }),
      );
    });
    ...
    it("updateOpportunityStage should update stage and optional fields", async () => {
      ...
      const result = await service.updateOpportunityStage(
        tenantId,
        "o1",
        "CLOSED_WON",
        100,
  ...
  describe("Activities", () => {
    it("getActivities should filter by leadId", async () => {
      mockPrisma.activity.findMany.mockResolvedValue([
        { id: "a1", type: "CALL", subject: "Test Call", tenantId },
      ]);
          ...
          leads: [
            {
              id: "l1",
              status: "CONVERTED",
              opportunities: [{ id: "o1", stage: "CLOSED_WON", amount: 5000 }],
            },
          ],
          createdAt: new Date(),
        },
  ...
  describe("Vendors", () => {
    it("should create and list vendors", async () => {
      mockPrisma.vendor.findMany.mockResolvedValue([
        { id: "v1", name: "Test Vendor", tenantId, deletedAt: null },
      ]);
    ...
    it("should update vendor details", async () => {
      ...
      const result = await service.updateVendor(tenantId, "v1", {
        name: "Updated Vendor",
      });
    ...
    it("should get and add vendor notes", async () => {
      ...
      const createdNote = await service.addVendorNote(tenantId, orgId, "v1", {
        content: "Log entry",
        type: "NOTE",
      });
    ...
    it("should bulk update vendor statuses", async () => {
      ...
      const result = await service.bulkUpdateVendorStatus(
        tenantId,
        ["v1", "v2"],
        "PREFERRED",
      );
