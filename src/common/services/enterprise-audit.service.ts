...
export class EnterpriseAuditService {
  ...
  async getSecurityAudit(
    ...
    const securityActions = [
      ...
      'PERMISSION_CHANGE', 'ROLE_CHANGE', 'MFA_ENABLED', 'MFA_DISABLED',
      'API_KEY_CREATED', 'API_KEY_REVOKED', 'ACCOUNT_LOCKED', 'ACCOUNT_UNLOCKED',
      'TWO_FACTOR_ENABLED', 'TWO_FACTOR_DISABLED', 'SESSION_REVOKED',
      'SECURITY_SETTING_CHANGED', 'LOGIN_FROM_NEW_DEVICE',
    ];
  ...
  async getComplianceReport(
    ...
    const dataModifications = allEvents.filter(
      (e) => ['CREATE', 'UPDATE', 'DELETE'].includes(e.action),
    ).length;
  ...
  private async writeXlsx(res: Response, columns: { header: string; key: string; width?: number }[], rows: Record<string, any>[], filename: string): Promise<void> {
