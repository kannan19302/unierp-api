...
describe("SalesSubscriptionService", () => {
  ...
  beforeEach(() => {
    service = new SalesSubscriptionService();
    vi.clearAllMocks();
  });
  ...
  it("should throw on not found", async () => {
    ...
    await expect(service.getSubscriptionById("tenant-1", "x")).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it("should record usage", async () => {
    ...
    const result = await service.recordUsage("tenant-1", "s1", {
  ...
  it("should run dunning process", async () => {
    vi.mocked(prisma.subscriptionInvoice.findMany).mockResolvedValue([
      {
        invoiceId: "inv-1",
        invoice: { dueDate: new Date(Date.now() - 86400000 * 10) },
  ...
  it("should return analytics", async () => {
    ...
    vi.mocked(prisma.subscriptionInvoice.aggregate).mockResolvedValue({
      _sum: { amount: 120000 },
    } as never);
