...
export class Form1099Service {
  ...
  async listVendorsWithProfiles(tenantId: string, taxYear: number) {
    ...
    const ytdMap = new Map(
      ytdByVendor.map((r) => [r.vendorId, Number(r._sum.paidAmount || 0)]),
    );
  ...
  async upsertVendorProfile(
    ...
    return prisma.vendor1099Profile.upsert({
      ...
      create: {
        ...
        w9ReceivedDate: dto.w9ReceivedDate
          ? new Date(dto.w9ReceivedDate)
          : undefined,
      ...
      update: {
        ...
        ...(dto.w9ReceivedDate && {
          w9ReceivedDate: new Date(dto.w9ReceivedDate),
        }),
        ...(dto.stateFilingRequired !== undefined && {
          stateFilingRequired: dto.stateFilingRequired,
        }),
  ...
  async runTinMatch(tenantId: string, vendorId: string) {
    ...
    if (!vendor) throw new NotFoundException("Vendor not found");
    const profile = await prisma.vendor1099Profile.findUnique({
      where: { vendorId },
    });
    if (!profile)
      throw new BadRequestException(
        "Vendor has no 1099 profile yet — create one first",
      );
    ...
    const matched = Boolean(
      profile.taxIdMasked && vendor.name.trim().length > 0,
    );
  ...
  async setBackupWithholding(
    ...
    if (rate !== undefined && (rate < 0 || rate > 100)) {
      throw new BadRequestException(
        "Backup withholding rate must be between 0 and 100",
      );
    }
    return prisma.vendor1099Profile.upsert({
      where: { vendorId },
      create: {
        tenantId,
        vendorId,
        backupWithholdingActive: active,
        backupWithholdingRate: new Prisma.Decimal(rate ?? 24),
      },
      update: {
        backupWithholdingActive: active,
        ...(rate !== undefined && {
          backupWithholdingRate: new Prisma.Decimal(rate),
        }),
      },
    });
  }
  ...
  async getW9Checklist(tenantId: string, vendorId: string) {
    ...
    return {
      ...
      missingItems: [
        ...
        (!profile || profile.tinMatchStatus !== "MATCHED") &&
          "TIN not verified via TIN match",
      ].filter(Boolean),
    };
  }
  ...
  async getThresholdReport(tenantId: string, taxYear: number) {
    ...
    const eligible = vendors.filter(
      (v) => v.profile?.is1099Vendor && v.crossesThreshold,
    );
  ...
  async generateForms(tenantId: string, userId: string, taxYear: number) {
    ...
    for (const v of report.eligible) {
      ...
      const federalWithholding = v.profile!.backupWithholdingActive
        ? Math.round(
            v.ytdPaid * (Number(v.profile!.backupWithholdingRate) / 100) * 100,
          ) / 100
        : 0;
  ...
  async updateForm(
    ...
    if (form.status !== "DRAFT")
      throw new BadRequestException("Only DRAFT forms may be edited");

    const boxAmounts =
      dto.boxAmounts ?? (form.boxAmounts as Record<string, number>);
    ...
    return prisma.form1099.update({
      ...
      data: {
        ...
        ...(dto.federalWithholding !== undefined && {
          federalWithholding: new Prisma.Decimal(dto.federalWithholding),
        }),
        ...(dto.state !== undefined && { state: dto.state }),
        ...(dto.stateWithholding !== undefined && {
          stateWithholding: new Prisma.Decimal(dto.stateWithholding),
        }),
  ...
  async markReady(tenantId: string, id: string, userId: string) {
    const form = await this.getForm(tenantId, id);
    if (form.status !== "DRAFT")
      throw new BadRequestException("Only DRAFT forms can be marked ready");
    return prisma.form1099.update({
      where: { id },
  ...
  async fileForm(tenantId: string, id: string, userId: string) {
    ...
    if (form.status !== "READY")
      throw new BadRequestException("Only READY forms can be filed");
    return prisma.form1099.update({
      where: { id },
  ...
  async voidForm(tenantId: string, id: string, userId: string) {
    const form = await this.getForm(tenantId, id);
    if (form.status === "VOID")
      throw new BadRequestException("Form already void");
    return prisma.form1099.update({
      where: { id },
      data: { status: "VOID", voidedAt: new Date(), updatedBy: userId },
    });
  }
  ...
  async correctForm(
    ...
    id: string,
    userId: string,
    dto: { boxAmounts: Record<string, number>; notes?: string },
  ) {
    const original = await this.getForm(tenantId, id);
    if (original.status !== "FILED")
      throw new BadRequestException("Only FILED forms can be corrected");
    if (original.correction)
      throw new BadRequestException(
        "A correction already exists for this form",
      );

    const totalAmount = Object.values(dto.boxAmounts).reduce(
      (sum, v) => sum + Number(v),
      0,
    );
  ...
  async createBatch(
    tenantId: string,
    userId: string,
    dto: { taxYear: number; name: string; formIds: string[] },
  ) {
    if (dto.formIds.length === 0)
      throw new BadRequestException("Batch must include at least one form");
    ...
    if (forms.length !== dto.formIds.length) {
      throw new BadRequestException(
        "All forms must exist, belong to the tax year, and be in READY status",
      );
    }
    const totalAmount = forms.reduce(
    ...
    await prisma.form1099.updateMany({
      where: { id: { in: dto.formIds }, tenantId },
  ...
  async efileBatch(tenantId: string, id: string, userId: string) {
    ...
    if (batch.status !== "GENERATED")
      throw new BadRequestException("Only GENERATED batches can be e-filed");
    ...
    const updated = await prisma.form1099Batch.update({
      ...
      data: {
        ...
        efileConfirmation: confirmation,
      },
    });
    await prisma.form1099.updateMany({
      ...
      data: {
        status: "FILED",
        filedAt: new Date(),
        efileSubmissionId: confirmation,
        updatedBy: userId,
      },
    });
    return updated;
