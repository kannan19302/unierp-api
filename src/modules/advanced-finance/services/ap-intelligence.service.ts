...
export class ApIntelligenceService {
  ...
  async runDuplicateScan(tenantId: string) {
    ...
    for (let i = 0; i < invoices.length; i++) {
      for (let j = i + 1; j < invoices.length; j++) {
        ...
        const amountMatch =
          Math.abs(Number(a.totalAmount) - Number(b.totalAmount)) < 0.01;
        ...
        if (amountMatch && customerMatch && dateDiff <= sevenDays) {
          flags.push({
            ...
            matchCriteria: {
              amount: true,
              customer: true,
              dateDiffDays: Math.round(dateDiff / 86400000),
            },
          });
        }
      }
    ...
    for (const f of flags) {
      const exists = await prisma.aPDuplicateFlag.findFirst({
        where: {
          tenantId,
          invoiceId: f.invoiceId,
          duplicateInvoiceId: f.duplicateInvoiceId ?? undefined,
        },
      });
      if (!exists) {
        const flag = await prisma.aPDuplicateFlag.create({
          data: {
            tenantId,
            ...f,
            matchCriteria: f.matchCriteria as never,
            status: "PENDING",
          },
        });
        created.push(flag);
  ...
  async reviewDuplicateFlag(
    tenantId: string,
    id: string,
    dto: { status: string; notes?: string; reviewedBy: string },
  ) {
    const flag = await prisma.aPDuplicateFlag.findFirst({
      where: { id, tenantId },
    });
  ...
  async listApprovalPolicies(tenantId: string) {
    return prisma.aPApprovalPolicy.findMany({
  ...
  async updateApprovalPolicy(
    ...
    const policy = await prisma.aPApprovalPolicy.findFirst({
      where: { id, tenantId },
    });
    if (!policy) throw new NotFoundException("Approval policy not found");
    return prisma.aPApprovalPolicy.update({
      where: { id },
      data: dto as never,
    });
  }
  ...
  async getApprovalPolicyForAmount(tenantId: string, amount: number) {
    const policies = await prisma.aPApprovalPolicy.findMany({
      where: { tenantId, isActive: true },
      orderBy: { thresholdMin: "asc" },
    });
  ...
  async getGrniAging(tenantId: string) {
    ...
    for (const r of records) {
      const days = Math.floor(
        (now.getTime() - r.receivedDate.getTime()) / 86400000,
      );
  ...
  async markGrniInvoiced(tenantId: string, id: string, invoiceId: string) {
    ...
    return prisma.grniRecord.update({
      where: { id },
      data: { status: "INVOICED", invoiceId, invoicedDate: new Date() },
    });
  }
  ...
  async getApCashFlowObligation(tenantId: string, days: number = 90) {
    ...
    const buckets: { "0-30": number; "31-60": number; "61-90": number } = {
      "0-30": 0,
      "31-60": 0,
      "61-90": 0,
    };
    for (const inv of invoices) {
      ...
      const daysTodue = Math.floor(
        (inv.dueDate.getTime() - now.getTime()) / 86400000,
      );
    ...
    return {
      totalObligation: Object.values(buckets).reduce((a, b) => a + b, 0),
      buckets,
  ...
  async getVendorScorecard(tenantId: string, vendorId: string) {
    ...
    const duplicateFlags = await prisma.aPDuplicateFlag.count({
      where: { tenantId },
    });
    return {
      vendorId,
      ...
      invoiceAccuracyPct:
        invoiceCount > 0
          ? (100 - (duplicateFlags / invoiceCount) * 100).toFixed(1)
          : "100.0",
      disputeRate: "0.0",
      overallScore: 90,
    };
