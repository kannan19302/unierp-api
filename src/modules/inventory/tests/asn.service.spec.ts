...
describe("AsnService", () => {
  ...
  it("createAsn — auto-numbers ASN-000001", async () => {
    ...
    vi.mocked(prisma.advanceShippingNotice.create).mockResolvedValue({
      id: asnId,
      asnNumber: "ASN-000001",
    } as never);

    const result = await svc.createAsn(tenantId, userId, {
      vendorId: "v1",
      warehouseId: "wh-1",
    });
    expect(vi.mocked(prisma.advanceShippingNotice.create)).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ asnNumber: "ASN-000001" }),
      }),
    );
  ...
  it("addLineItem — zero qty throws", async () => {
    ...
    await expect(
      svc.addLineItem(tenantId, asnId, { productId: "p1", expectedQty: 0 }),
  ...
  it("addLineItem — throws on RECEIVED ASN", async () => {
    ...
    await expect(
      svc.addLineItem(tenantId, asnId, { productId: "p1", expectedQty: 10 }),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it("markInTransit — non-PENDING throws", async () => {
    ...
    await expect(svc.markInTransit(tenantId, asnId)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("receiveLineItem — auto-creates SHORTAGE discrepancy", async () => {
    ...
    vi.mocked(prisma.aSNLineItem.findFirst).mockResolvedValue({
      id: "li1",
      productId: "p1",
      expectedQty: 100,
      receivedQty: 0,
    } as never);
    vi.mocked(prisma.aSNLineItem.update).mockResolvedValue({
      id: "li1",
      receivedQty: 80,
    } as never);
    vi.mocked(prisma.advanceShippingNotice.update).mockResolvedValue(
    ...
    vi.mocked(prisma.asnDiscrepancy.create).mockResolvedValue({
      id: "d1",
    ...
    expect(vi.mocked(prisma.asnDiscrepancy.create)).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ discrepancyType: "SHORTAGE" }),
      }),
    );
  });
  ...
  it("receiveLineItem — auto-creates OVERAGE discrepancy", async () => {
    ...
    expect(vi.mocked(prisma.asnDiscrepancy.create)).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ discrepancyType: "OVERAGE" }),
      }),
    );
  });
  ...
  it("cancelAsn — RECEIVED throws", async () => {
    ...
    await expect(svc.cancelAsn(tenantId, asnId)).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("resolveDiscrepancy — already resolved throws", async () => {
    ...
    await expect(
      svc.resolveDiscrepancy(tenantId, userId, "d1", "fixed"),
    ).rejects.toThrow(BadRequestException);
  });
