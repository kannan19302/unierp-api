...
describe("ExpenseManagementService", () => {
  ...
  beforeEach(() => {
    ...
    } as unknown as CardSpendLimitService;
  ...
  describe("addExpenseItem — policy engine", () => {
    it("flags a policy violation when amount exceeds the category max", async () => {
      ...
      vi.mocked(prisma.expenseReportItem.create).mockResolvedValue({
        id: "item-1",
      } as any);
      vi.mocked(prisma.expenseReportItem.findMany).mockResolvedValue([
        { amount: 250, taxAmount: 0, policyViolation: true },
      ] as any);
      ...
      const createCall = vi.mocked(prisma.expenseReportItem.create).mock
        .calls[0][0] as any;
      expect(createCall.data.policyViolation).toBe(true);
      expect(createCall.data.policyViolationReason).toContain(
        "Exceeds per-item limit",
      );
    });
  ...
  describe("addExpenseItem — mileage & per diem", () => {
    it("computes amount from mileage distance × configured rate", async () => {
      ...
      vi.mocked(prisma.mileageRate.findFirst).mockResolvedValue({
        ratePerMile: 0.67,
      } as any);
      ...
      vi.mocked(prisma.expenseReportItem.findMany).mockResolvedValue([
        { amount: 67, taxAmount: 0, policyViolation: false },
      ] as any);
      vi.mocked(prisma.expenseReport.update).mockResolvedValue({} as any);
    ...
    it("computes amount from per-diem days × configured location rate", async () => {
      ...
      vi.mocked(prisma.perDiemRate.findFirst).mockResolvedValue({
        dailyRate: 75,
      } as any);
      vi.mocked(prisma.expenseCategoryPolicy.findFirst).mockResolvedValue(null);
      vi.mocked(prisma.expenseReportItem.create).mockResolvedValue({
      ...
      vi.mocked(prisma.expenseReportItem.findMany).mockResolvedValue([
        { amount: 225, taxAmount: 0, policyViolation: false },
      ] as any);
      vi.mocked(prisma.expenseReport.update).mockResolvedValue({} as any);
  ...
  describe("submitExpenseReport", () => {
    it("rejects submitting a report with zero line items", async () => {
      ...
      await expect(
        service.submitExpenseReport(TENANT, "rep-1"),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("approveExpenseReport — second-approval routing", () => {
    it("routes to PENDING_SECOND_APPROVAL when the report requires second approval", async () => {
      ...
      vi.mocked(prisma.expenseReport.update).mockResolvedValue({
        status: "PENDING_SECOND_APPROVAL",
      } as any);

      const result = await service.approveExpenseReport(
        TENANT,
        "rep-1",
        "user-1",
      );
      ...
      expect(prisma.expenseReport.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ status: "PENDING_SECOND_APPROVAL" }),
        }),
      );
  ...
  describe("secondApproveExpenseReport", () => {
    it("rejects second approval unless status is PENDING_SECOND_APPROVAL", async () => {
      ...
      await expect(
        service.secondApproveExpenseReport(TENANT, "rep-1", "user-1"),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("markExpenseReportPaid — GL posting", () => {
    it("posts a balanced reimbursement journal and stores the journal id", async () => {
      ...
      vi.spyOn(glService, "createJournal").mockResolvedValue({
        id: "journal-1",
      } as any);
      vi.mocked(prisma.expenseReport.update).mockResolvedValue({
        status: "PAID",
        glJournalId: "journal-1",
      } as any);
    ...
    it("rejects paying a report that is not APPROVED", async () => {
      ...
      await expect(
        service.markExpenseReportPaid(TENANT, "rep-1"),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("corporate card matching", () => {
    it("matches a card transaction to an expense item and marks it MATCHED", async () => {
      vi.mocked(prisma.corporateCardTransaction.findFirst).mockResolvedValue({
        id: "txn-1",
      } as any);
      vi.mocked(prisma.expenseReportItem.findFirst).mockResolvedValue({
        id: "item-1",
      } as any);
      vi.mocked(prisma.expenseReportItem.update).mockResolvedValue({} as any);
      vi.mocked(prisma.corporateCardTransaction.update).mockResolvedValue({
        id: "txn-1",
        status: "MATCHED",
      } as any);

      const result = await service.matchCardTransactionToItem(
        TENANT,
        "txn-1",
        "item-1",
      );
    ...
    it("throws NotFoundException for an unknown transaction", async () => {
      ...
      await expect(
        service.matchCardTransactionToItem(TENANT, "nope", "item-1"),
