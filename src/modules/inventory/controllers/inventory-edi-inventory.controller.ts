...
} from "../services/inventory-edi-inventory.service";
...
export class InventoryEdiInventoryController {
  ...
  createTransaction(
    @Req() req: AuthReq,
    @ZodBody(createEdiTransactionSchema) dto: any,
  ) {
  ...
  updateStatus(
    @Req() req: AuthReq,
    @Param("id") id: string,
    @ZodBody(updateEdiStatusSchema) dto: any,
  ) {
    return this.service.updateTransactionStatus(
      req.user.tenantId,
      id,
      dto.status,
      dto.errorMessage,
