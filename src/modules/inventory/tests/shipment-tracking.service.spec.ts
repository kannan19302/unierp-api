...
describe("ShipmentTrackingService", () => {
  ...
  it("createInboundShipment — auto-numbers IS-000001", async () => {
    ...
    vi.mocked(prisma.inboundShipment.create).mockResolvedValue({
      id: "s1",
      shipmentNumber: "IS-000001",
    } as never);

    const result = await svc.createInboundShipment(tenantId, {
      warehouseId: "wh-1",
    });
    expect(vi.mocked(prisma.inboundShipment.create)).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ shipmentNumber: "IS-000001" }),
      }),
    );
  ...
  it("updateInboundStatus — valid transition EXPECTED→IN_TRANSIT", async () => {
    ...
    vi.mocked(prisma.inboundShipment.update).mockResolvedValue({
      id: "s1",
      status: "IN_TRANSIT",
    } as never);
  ...
  it("updateInboundStatus — invalid transition throws", async () => {
    ...
    await expect(
      svc.updateInboundStatus(tenantId, "s1", "EXPECTED"),
  ...
  it("createOutboundShipment — auto-numbers OS-000001", async () => {
    ...
    vi.mocked(prisma.outboundShipment.create).mockResolvedValue({
      id: "s2",
      shipmentNumber: "OS-000001",
    } as never);

    const result = await svc.createOutboundShipment(tenantId, {
      warehouseId: "wh-1",
    });
  ...
  it("updateOutboundStatus — PACKED→SHIPPED sets shipDate", async () => {
    ...
    expect(vi.mocked(prisma.outboundShipment.update)).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ shipDate: expect.any(Date) }),
      }),
    );
  });
  ...
  it("updateExceptionStatus — RESOLVED sets resolvedAt", async () => {
    ...
    vi.mocked(prisma.shipmentException.update).mockResolvedValue({
      id: "exc1",
      status: "RESOLVED",
    } as never);

    await svc.updateExceptionStatus(
      tenantId,
      "exc1",
      userId,
      ShipmentExceptionStatus.RESOLVED,
      "Found and delivered",
    );
