// @ts-nocheck
import {
  Controller, Post, Patch, Delete, Body, Param, UseGuards, Req,
} from '@nestjs/common';
...
export class BulkOperationsController {
  ...
  async bulkCreate(
    ...
    return this.bulkOperationsService.bulkCreate(tenantId, modelName, body.records);
  }
  ...
  async bulkUpdate(
    ...
    const tenantId = req.tenantId || req.user?.tenantId;
    return this.bulkOperationsService.bulkUpdate(tenantId, modelName, body.ids, body.updates);
  }
  ...
  async bulkRestore(
    ...
    const tenantId = req.tenantId || req.user?.tenantId;
    return this.bulkOperationsService.bulkRestore(tenantId, modelName, body.ids);
  }
  ...
  async bulkStatusChange(
    ...
    const tenantId = req.tenantId || req.user?.tenantId;
    return this.bulkOperationsService.bulkStatusChange(tenantId, modelName, body.ids, body.status);
  }
}
