...
describe("VendorRmaService (extra)", () => {
  ...
  it("should throw NotFoundException for missing RMA", async () => {
    ...
    await expect(service.getRmaById("tenant-1", "nonexistent")).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it("should enforce tenant isolation on getRmaById", async () => {
    prisma.vendorRmaRequest.findFirst.mockResolvedValue(null);
    await expect(service.getRmaById("tenant-2", "1")).rejects.toThrow(
      NotFoundException,
    );
  ...
  it("should update shipment status to PACKED", async () => {
    prisma.vendorReturnShipment.findFirst.mockResolvedValue({
      id: "shp-1",
      status: "PENDING",
    });
    prisma.vendorReturnShipment.update.mockResolvedValue({
      id: "shp-1",
      status: "PACKED",
    });
  ...
  it("should resolve RMA with credit", async () => {
    prisma.vendorRmaRequest.findFirst.mockResolvedValue({
      id: "rma-1",
      status: "AUTHORIZED",
    });
    prisma.vendorReturnShipment.updateMany.mockResolvedValue({ count: 1 });
    prisma.vendorRmaRequest.update.mockResolvedValue({
      id: "rma-1",
      status: "COMPLETED",
      vendor: { name: "Vendor A" },
      shipments: [],
    ...
    const result = await service.resolveRma("tenant-1", "rma-1", {
      resolution: "Accepted",
      creditAmount: 500,
      creditMemoRef: "CM-001",
    });
  ...
  it("should throw BadRequestException when resolving non-AUTHORIZED RMA", async () => {
    ...
    await expect(
      service.resolveRma("tenant-1", "rma-1", { resolution: "Accepted" }),
  ...
  it("should cancel RMA with reason", async () => {
    ...
    const result = await service.cancelRma(
      "tenant-1",
      "rma-1",
      "No longer needed",
    );
  ...
  it("should throw BadRequestException when cancelling completed RMA", async () => {
    ...
    await expect(service.cancelRma("tenant-1", "rma-1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("should get RMAs by date range", async () => {
    prisma.vendorRmaRequest.findMany.mockResolvedValue([
      { id: "1", vendor: { name: "Vendor A" }, shipments: [] },
    ]);
    prisma.vendorRmaRequest.count.mockResolvedValue(1);

    const result = await service.getRmasByDateRange(
      "tenant-1",
      "2026-01-01",
      "2026-06-30",
      {},
  ...
  it("should get detailed RMA stats with byVendor and resolution time", async () => {
    ...
    prisma.vendorReturnShipment.findMany.mockResolvedValue([
      { status: "DELIVERED", creditAmount: 500 },
    ]);
  ...
  it("should list shipments for an RMA", async () => {
    prisma.vendorReturnShipment.findMany.mockResolvedValue([
      { id: "shp-1", warehouse: { name: "Main WH" } },
    ]);
