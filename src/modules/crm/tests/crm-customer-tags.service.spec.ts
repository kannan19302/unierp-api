...
describe("CrmCustomersService — Customer Tags", () => {
  ...
  it("lists customer tags scoped to tenant", async () => {
    ...
    expect(
      (prisma.customerTag.findMany as ReturnType<typeof vi.fn>).mock
        .calls[0][0],
    ).toEqual({
  ...
  it("creates a customer tag with tenant scoping and default color", async () => {
    (prisma.customerTag.create as ReturnType<typeof vi.fn>).mockImplementation(
      ({ data }) => Promise.resolve({ id: "tag-1", ...data }),
    );
    const tag = await service.createCustomerTag(TENANT_A, {
      name: "Enterprise",
      color: "#3b82f6",
    });
  ...
  it("deletes a customer tag belonging to the tenant", async () => {
    (
      prisma.customerTag.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "tag-1", tenantId: TENANT_A });
    (prisma.customerTag.delete as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: "tag-1",
    });
    await service.deleteCustomerTag(TENANT_A, "tag-1");
    expect(prisma.customerTag.delete).toHaveBeenCalledWith({
      where: { id: "tag-1" },
    });
  });
  ...
  it("throws NotFoundException deleting a tag from another tenant (tenant isolation)", async () => {
    ...
    await expect(service.deleteCustomerTag(TENANT_B, "tag-1")).rejects.toThrow(
      NotFoundException,
    );
  ...
  it("assigns a tag to a customer within the same tenant", async () => {
    ...
    (
      prisma.customerTagLink.create as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "link-1", customerId: "cust-1", tagId: "tag-1" });
    const link = await service.assignCustomerTag(TENANT_A, "cust-1", "tag-1");
    expect(link.customerId).toBe("cust-1");
    expect(prisma.customer.findFirst).toHaveBeenCalledWith({
      where: { id: "cust-1", tenantId: TENANT_A },
    });
  });
  ...
  it("throws NotFoundException assigning a tag to a customer from another tenant (cross-tenant 404)", async () => {
    ...
    await expect(
      service.assignCustomerTag(TENANT_B, "cust-1", "tag-1"),
  ...
  it("removes a tag assignment", async () => {
    (
      prisma.customerTagLink.findFirst as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "link-1", customerId: "cust-1", tagId: "tag-1" });
    (
      prisma.customerTagLink.delete as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "link-1" });
    await service.removeCustomerTag("cust-1", "tag-1");
    expect(prisma.customerTagLink.delete).toHaveBeenCalledWith({
      where: { id: "link-1" },
    });
  });
  ...
  it("throws NotFoundException removing a non-existent tag assignment", async () => {
    ...
    await expect(
      service.removeCustomerTag("cust-1", "tag-missing"),
    ).rejects.toThrow(NotFoundException);
  });
});
