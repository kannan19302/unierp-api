...
describe("BuilderService", () => {
  ...
  beforeEach(async () => {
    ...
    workflowsService = module.get<BuilderWorkflowsService>(
      BuilderWorkflowsService,
    );
    ...
    webContentService = module.get<BuilderWebContentService>(
      BuilderWebContentService,
    );
  ...
  const models = [
    ...
    },
    {
      ...
      prismaMock: prisma.builderWorkflow,
      createArgs: ["t1", { name: "n", docType: "d" }],
      updateArgs: ["t1", "id", { name: "n2" }],
    },
    {
      entity: "Dashboard",
      serviceName: "dashboardsService",
      prismaMock: prisma.builderDashboard,
      createArgs: ["t1", { name: "n", refreshRate: 60 }],
      updateArgs: ["t1", "id", { name: "n2" }],
    },
    {
      entity: "Module",
      serviceName: "service",
      prismaMock: prisma.builderModule,
      createArgs: ["t1", { name: "n", slug: "s" }],
      updateArgs: ["t1", "id", { name: "n2" }],
    },
    {
    ...
    },
    {
      entity: "BlogPost",
      serviceName: "webContentService",
      prismaMock: prisma.blogPost,
      createArgs: ["t1", { title: "t", slug: "s" }],
      updateArgs: ["t1", "id", { title: "t2" }],
    },
  ];
  ...
  describe("DataImport", () => {
    ...
    it("should create data import", async () => {
      ...
      expect(
        await service.createDataImport("t1", {
          name: "n",
  ...
  describe("Stats", () => {
    ...
    it("should aggregate and sort recent items", async () => {
      ...
      (prisma.builderForm.findMany as any).mockResolvedValue([
        { id: "f1", name: "Form 1", status: "DRAFT", updatedAt: d1 },
      ]);
      (prisma.builderWorkflow.findMany as any).mockResolvedValue([
        { id: "w1", name: "WF 1", status: "ACTIVE", updatedAt: d2 },
      ]);
  ...
  describe("SchemaRegistry", () => {
    it("should get schema by id", async () => {
      (prisma.schemaRegistry.findFirst as any).mockResolvedValue({
        id: "s1",
        name: "Test",
      });
      const result = await service.getSchemaRegistryById("t1", "s1");
      expect(result).toEqual({ id: "s1", name: "Test" });
      expect(prisma.schemaRegistry.findFirst).toHaveBeenCalledWith({
  ...
  describe("publishForm", () => {
    it("should create a new schema and link it to the page on first publish", async () => {
      ...
      (prisma.$transaction as any).mockImplementation(async (fn: any) => {
        ...
        prisma.schemaRegistry.create = vi
          .fn()
          .mockResolvedValue({ id: "schema1", slug: "crm_vehicle-request" });
        return fn({ schemaRegistry: prisma.schemaRegistry });
      });
      (prisma.pageRegistry.update as any).mockResolvedValue({
        ...page,
        schemaId: "schema1",
        status: "PUBLISHED",
      });
    ...
    it("should update existing schema on re-publish", async () => {
      ...
      (prisma.pageRegistry.findFirst as any).mockResolvedValue(page);
      (prisma.$transaction as any).mockImplementation(async (fn: any) => {
        prisma.schemaRegistry.findUnique = vi
          .fn()
          .mockResolvedValue({ id: "schema1" });
        prisma.schemaRegistry.update = vi.fn().mockResolvedValue({
          id: "schema1",
          name: "Vehicle Request Updated",
        });
        return fn({ schemaRegistry: prisma.schemaRegistry });
      });
      (prisma.pageRegistry.update as any).mockResolvedValue({
    ...
    it("should throw if page not found", async () => {
      ...
      await expect(service.publishForm("t1", "nonexistent")).rejects.toThrow(
        "Page not found",
      );
    });
  });
  ...
  describe("getCustomRecords with query", () => {
    ...
    const records = [
      {
        id: "r1",
        data: { full_name: "Alice", status: "Active" },
        createdAt: "2026-01-01",
      },
      {
        id: "r2",
        data: { full_name: "Bob", status: "Inactive" },
        createdAt: "2026-01-02",
      },
      {
        id: "r3",
        data: { full_name: "Charlie", status: "Active" },
    ...
    it("should still apply RBAC scrubbing with query params", async () => {
      ...
      const result = await service.getCustomRecords("t1", "s1", ["viewer"], {
        search: "something",
      });
  ...
  describe("testRunAutomationRule", () => {
    ...
    it("should fail if no actions configured", async () => {
      ...
      await expect(service.testRunAutomationRule("t1", "r1")).rejects.toThrow(
        "Automation rule has no actions to test",
      );
    });
  });
  ...
  describe("publishModule", () => {
    ...
    it("creates an immutable release and bumps the patch version by default", async () => {
      ...
      (prisma.builderModule.update as any).mockImplementation(({ data }: any) =>
        Promise.resolve({ ...baseModule, ...data }),
      );

      const result = await service.publishModule(
        "t1",
        "m1",
        { scope: "GLOBAL" },
        "u1",
  ...
  describe("installBuilderApp", () => {
    it("provisions pages/schemas and records the install", async () => {
      (prisma.builderModule.findFirst as any).mockResolvedValue({
        id: "m1",
        name: "A",
        slug: "a",
        currentReleaseId: "r1",
      ...
      (prisma.appRelease.findFirst as any).mockResolvedValue({
        ...
        snapshot: {
          ...
          pages: [
            { slug: "tickets", name: "Tickets", type: "form", formId: "f1" },
          ],
          dataModels: [],
        },
      });
      ...
      (prisma.installedApp.upsert as any).mockImplementation(
        ({ create }: any) => Promise.resolve({ id: "ins1", ...create }),
      );
    ...
    it("throws when app has no published release", async () => {
      ...
      await expect(service.installBuilderApp("t1", "m1")).rejects.toThrow(
  ...
  describe("uninstallBuilderApp", () => {
    it("tears down provisioned artifacts and removes the install", async () => {
      (prisma.installedApp.findUnique as any).mockResolvedValue({
        id: "ins1",
        provisioned: { pageRegistryIds: ["pg1"], schemaRegistryIds: ["sch1"] },
      });
  ...
  describe("getGlobalPerformanceStats", () => {
    it("should aggregate and calculate stats from databases", async () => {
      ...
      (prisma.schemaRegistry.findMany as any).mockResolvedValue([
        {
          ...
          module: "sales",
          name: "Orders Schema",
          _count: { customRecords: 8 },
        },
      ]);
      (prisma.customRecord.findMany as any).mockImplementation(
        ({ take }: any) => {
          if (take === 10) {
      ...
      expect(res.charts.submissionsByApp).toEqual([
        { appName: "Sales App", count: 8 },
      ]);
      const junTrend = res.charts.monthlySubmissionsTrend.find(
        (t: any) => t.month === "Jun",
      );
    ...
    it("should handle exceptions gracefully and return default empty structures", async () => {
      ...
      (prisma.inventoryItem.findMany as any).mockRejectedValue(
        new Error("DB Error"),
      );
      (prisma.builderModule.findMany as any).mockRejectedValue(
        new Error("DB Error"),
      );
      (prisma.customRecord.findMany as any).mockRejectedValue(
        new Error("DB Error"),
      );
