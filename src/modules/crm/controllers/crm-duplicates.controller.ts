...
import {
  ...
  MergePairInput,
} from "../services/crm-duplicates.service";
...
export class CrmDuplicatesController {
  ...
  async create(
    ...
    @ZodBody(createDuplicateRuleSchema) dto: CreateDuplicateRuleInput,
  ) {
  ...
  async update(
    @Req() req: AuthenticatedRequest,
    @Param("id") id: string,
    @ZodBody(updateDuplicateRuleSchema) dto: UpdateDuplicateRuleInput,
  ) {
  ...
  async scan(
    @Req() req: AuthenticatedRequest,
    @Query("entity") entity: "leads" | "contacts" | "customers" | "accounts",
  ) {
  ...
  async find(
    @Req() req: AuthenticatedRequest,
    @ZodBody(findDuplicatesSchema) dto: FindDuplicatesInput,
  ) {
    return {
      data: await this.svc.findDuplicates(
        req.user.tenantId,
        dto.entity,
        dto.recordId,
      ),
  ...
  async mergeLeads(
    @Req() req: AuthenticatedRequest,
    @ZodBody(mergePairSchema) dto: MergePairInput,
  ) {
