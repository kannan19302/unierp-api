...
describe("CrmDuplicatesService", () => {
  ...
  it("creates a duplicate rule", async () => {
    (
      prisma.duplicateRule.create as ReturnType<typeof vi.fn>
  ...
  it("scans and groups leads with matching normalized email", async () => {
    (
      prisma.duplicateRule.findMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue([
    ...
    expect(result[0].records.map((r: any) => r.id).sort()).toEqual([
      "l1",
      "l2",
    ]);
  });
  ...
  it("merges a source lead into a target, filling missing fields and soft-deleting source", async () => {
    ...
    const result = await service.mergeLeads(TENANT, {
      winnerId: "t",
      loserIds: ["s"],
      fieldChoices: { company: "s" },
    });
    expect(result.merged).toBe(true);
    const firstUpdate = (prisma.lead.update as ReturnType<typeof vi.fn>).mock
      .calls[0][0];
    ...
    const secondUpdate = (prisma.lead.updateMany as ReturnType<typeof vi.fn>)
      .mock.calls[0][0];
  ...
  it("merges accounts (customers) and SOFT-deletes losers via updateMany + deletedAt, consistent with CrmCustomersService.deleteCustomer", async () => {
    ...
    (prisma.customer.updateMany as ReturnType<typeof vi.fn>).mockResolvedValue({
      count: 1,
    });
    ...
    const patchCall = (prisma.customer.update as ReturnType<typeof vi.fn>).mock
      .calls[0][0];
