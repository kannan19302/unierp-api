...
export class SupplierRiskService {
  async getRiskProfiles(
    tenantId: string,
    params?: { riskCategory?: string; page?: number; limit?: number },
  ) {
    const where: any = { tenantId, isActive: true };
    if (params?.riskCategory) where.riskCategory = params.riskCategory;
    const data = await prisma.supplierRiskProfile.findMany({
      where,
      include: {
        factors: true,
  ...
  async getRiskProfileByVendor(tenantId: string, vendorId: string) {
    let profile = await prisma.supplierRiskProfile.findFirst({
    ...
    if (!profile) {
      profile = await prisma.supplierRiskProfile.create({
        data: {
          tenantId,
          vendorId,
          overallRiskScore: 50,
        ...
        include: { factors: true, alerts: true, diversity: true },
      });
    }
    return profile;
  }
  ...
  async createRiskProfile(tenantId: string, dto: any) {
    return prisma.$transaction(async (tx) => {
      const scores = [
        dto.financialHealth,
        dto.geopoliticalRisk,
        dto.operationalRisk,
        dto.complianceRisk,
        dto.qualityRisk,
        dto.concentrationRisk,
      ...
      const profile = await tx.supplierRiskProfile.create({
        data: {
          ...
          financialHealth: dto.financialHealth
            ? new Prisma.Decimal(dto.financialHealth)
            : null,
          geopoliticalRisk: dto.geopoliticalRisk
            ? new Prisma.Decimal(dto.geopoliticalRisk)
            : null,
          operationalRisk: dto.operationalRisk
            ? new Prisma.Decimal(dto.operationalRisk)
            : null,
          complianceRisk: dto.complianceRisk
            ? new Prisma.Decimal(dto.complianceRisk)
            : null,
          qualityRisk: dto.qualityRisk
            ? new Prisma.Decimal(dto.qualityRisk)
            : null,
          concentrationRisk: dto.concentrationRisk
            ? new Prisma.Decimal(dto.concentrationRisk)
            : null,
        },
      });
      if (dto.factors?.length) {
        await tx.supplierRiskFactor.createMany({
          data: dto.factors.map((f: any) => ({
            tenantId,
            profileId: profile.id,
            factorType: f.factorType,
            factorName: f.factorName,
            score: new Prisma.Decimal(f.score),
            weight: f.weight
              ? new Prisma.Decimal(f.weight)
      ...
      return tx.supplierRiskProfile.findUnique({
        where: { id: profile.id },
  ...
  async updateRiskProfile(tenantId: string, id: string, dto: any) {
    ...
    const scores = [
      (dto.financialHealth ?? profile.financialHealth)
        ? Number(profile.financialHealth)
        : null,
      (dto.geopoliticalRisk ?? profile.geopoliticalRisk)
        ? Number(profile.geopoliticalRisk)
        : null,
      (dto.operationalRisk ?? profile.operationalRisk)
        ? Number(profile.operationalRisk)
        : null,
      (dto.complianceRisk ?? profile.complianceRisk)
        ? Number(profile.complianceRisk)
        : null,
      (dto.qualityRisk ?? profile.qualityRisk)
        ? Number(profile.qualityRisk)
        : null,
      (dto.concentrationRisk ?? profile.concentrationRisk)
        ? Number(profile.concentrationRisk)
        : null,
    ].filter((s): s is number => s != null);
    const avgScore = scores.length
      ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length)
      : 50;
    return prisma.supplierRiskProfile.update({
      where: { id },
      data: {
        ...dto,
        overallRiskScore: avgScore,
  ...
  async getRiskAlerts(
    ...
    return prisma.supplierRiskAlert.findMany({
      where,
  ...
  async createRiskAlert(tenantId: string, dto: any) {
    ...
    if (!profile) throw new NotFoundException("Risk profile not found");
    return prisma.supplierRiskAlert.create({
      data: {
        tenantId,
  ...
  async resolveRiskAlert(
    ...
    const alert = await prisma.supplierRiskAlert.findFirst({
      where: { id, tenantId },
    });
    if (!alert) throw new NotFoundException("Risk alert not found");
    return prisma.supplierRiskAlert.update({
      where: { id },
      data: {
        status: "RESOLVED",
        resolvedBy: userId,
  ...
  async getAlternativeSourcing(tenantId: string, productId?: string) {
    ...
    return prisma.alternativeSourcing.findMany({
      where,
      orderBy: { createdAt: "desc" },
    });
  }
  ...
  async getSupplierDiversity(tenantId: string, diversityType?: string) {
    ...
    return prisma.supplierDiversity.findMany({
      where,
      include: { profile: true },
      orderBy: { createdAt: "desc" },
    });
  ...
  async createSupplierDiversity(tenantId: string, profileId: string, dto: any) {
    ...
    return prisma.supplierDiversity.create({
      data: {
        tenantId,
        profileId,
        diversityType: dto.diversityType,
        certificationBody: dto.certificationBody || null,
        ...
        spendAmount: dto.spendAmount
          ? new Prisma.Decimal(dto.spendAmount)
          : null,
  ...
  async getRiskHeatMap(tenantId: string) {
    const profiles = await prisma.supplierRiskProfile.findMany({
    ...
    return {
      vendors: profiles.map((p) => ({
        ...
        overallRiskScore: p.overallRiskScore ? Number(p.overallRiskScore) : 50,
        riskCategory: p.riskCategory,
        financialHealth: p.financialHealth ? Number(p.financialHealth) : null,
        geopoliticalRisk: p.geopoliticalRisk
          ? Number(p.geopoliticalRisk)
          : null,
        operationalRisk: p.operationalRisk ? Number(p.operationalRisk) : null,
        complianceRisk: p.complianceRisk ? Number(p.complianceRisk) : null,
        qualityRisk: p.qualityRisk ? Number(p.qualityRisk) : null,
        concentrationRisk: p.concentrationRisk
          ? Number(p.concentrationRisk)
          : null,
      })),
      summary: {
        ...
        averageScore: profiles.length
          ? Math.round(
              profiles.reduce(
                (s, p) => s + Number(p.overallRiskScore || 50),
                0,
              ) / profiles.length,
            )
          : 0,
