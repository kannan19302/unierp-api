...
import {
  ...
  SignQuotationInput,
} from "../services/crm-quote-signature.service";
...
export class CrmQuoteSignatureController {
  ...
  async request(
    @Req() req: AuthenticatedRequest,
    @ZodBody(requestSignatureSchema) dto: RequestSignatureInput,
  ) {
  ...
  @ApiOperation({
    summary:
      "Render the certificate document (text content a PDF export would embed)",
  })
  @Get("certificates/:signatureId/document")
  @Permissions("crm.opportunity.read")
  async certificateDocument(
    @Req() req: AuthenticatedRequest,
    @Param("signatureId") signatureId: string,
  ) {
